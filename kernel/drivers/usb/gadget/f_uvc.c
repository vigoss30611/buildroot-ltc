...
#define UVC_STATUS_MAX_PACKET_SIZE		64	/* 64 bytes status */
...
uvc_function_set_alt(struct usb_function *f, unsigned interface, unsigned alt)
{
	...
	*/
	...
	if (!bulk_streaming_ep) {
		INFO(f->config->cdev, "uvc: isochronous mode\n");
		switch (alt) {
		case 0:
		...
		INFO(f->config->cdev, "uvc: bulk mode\n");
		switch (uvc->state) {
		...
		case UVC_STATE_STREAMING:
			//some mobile app may send this message when reconnect
			//do not disable video ep, because in streamon flow ,video ep will not be enabled again,
			#if 0
			if (uvc->video.ep->driver_data) {
				if (uvc->video.ep) {
...
int
uvc_function_suspend(struct usb_function *f, u8 suspend_opt)
{
	struct uvc_device *uvc = to_uvc(f);
	struct v4l2_event v4l2_event;
	...
	/* printk("%s end point=%d\n", __func__, suspend_opt); */
	(void)suspend_opt;

	/* printk(KERN_ERR"%s UVC_EVENT_STREAMOFF\n", __func__); */
	memset(&v4l2_event, 0, sizeof(v4l2_event));
	v4l2_event.type = UVC_EVENT_STREAMOFF;
	v4l2_event_queue(uvc->vdev, &v4l2_event);
...
uvc_function_unbind(struct usb_configuration *c, struct usb_function *f)
{
	...
	usb_ep_dequeue(cdev->gadget->ep0, uvc->control_req); /* dequeue ep0 request */
		...
		const struct uvc_descriptor_header * const *ss_streaming)
{
	...
	if (bulk_streaming_ep)
		uvc->func.func_suspend = uvc_function_suspend;
