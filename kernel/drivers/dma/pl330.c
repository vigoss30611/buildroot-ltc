...
struct pl022_tx_rx_sync{
	int pl022_pages;
	int pl022_ev_tx;
	int pl022_ev_rx;
};
static struct pl022_tx_rx_sync pl022_sync ={
	.pl022_pages =-1,
	.pl022_ev_tx =-1,
	.pl022_ev_rx =-1,
};

int pl011_wait_ready(void)
{
	while (1) {
		udelay(1);
		if (!pl022_sync.pl022_pages)
			return 0;
	}
}
EXPORT_SYMBOL(pl011_wait_ready);
...
static int pl330_update(const struct pl330_info *pi)
{
	...
	for (ev = 0; ev < pi->pcfg.num_events; ev++) {
		if (   ( val & (1 << ev )) || 
			( ( ev == pl022_sync.pl022_ev_tx ) &&( pl022_sync.pl022_pages>0 ) &&(pl022_sync.pl022_pages%2 == 1) )  ) { /* Event occurred */
			...
			if (pl022_sync.pl022_ev_tx >= 0){	
				if ( (ev == pl022_sync.pl022_ev_tx) && (pl022_sync.pl022_pages%2 == 0) )
					continue;
			}
			...
			if (pl022_sync.pl022_ev_tx >= 0){
				if( ( (ev == pl022_sync.pl022_ev_tx)||(ev == pl022_sync.pl022_ev_rx) )
					&& ( pl022_sync.pl022_pages>0 ))
					pl022_sync.pl022_pages--;
			}	
			_start(thrd);
...
static int pl330_control(struct dma_chan *chan, enum dma_ctrl_cmd cmd, unsigned long arg)
{
	...
	case DMA_SLAVE_CONFIG:
		...
		pch->byteswap = slave_config->byteswap;
...
{
    struct dma_pl330_chan *pch = to_pchan(chan);
	return dma_cookie_status(chan, cookie, txstate, pch->pause);
}
...
static struct dma_async_tx_descriptor *
pl330_prep_slave_sg_pl022(struct dma_chan *chan, struct scatterlist *sgl,
        ...
        unsigned long flg, void *context)
{
    ...
    {
        //pl022 tx
        thread = pch->pl330_chid;
        pl022_sync.pl022_ev_tx = thread->ev;
        pl022_sync.pl022_pages = sg_len * 2;
    }
    else if(direction == DMA_DEV_TO_MEM)
    {
        //pl022 rx
        thread = pch->pl330_chid;
        pl022_sync.pl022_ev_rx = thread->ev;
    }
    addr = pch->fifo_addr;
...
pl330_probe(struct amba_device *adev, const struct amba_id *id)
{
	...
	clk = clk_get_sys("dma-pl330","dma-pl330");
	if (IS_ERR(clk)){
		dev_err(&adev->dev,"get dma-pl330 bus clk fail\n");
		ret = -ENODEV;
		return -ENOMEM;
	}
	ret = clk_prepare_enable(clk);
	if(ret){
		dev_err(&adev->dev,"prepare dma-pl330 bus clk fail\n");
		goto clk_unprepare;
	}
