...
static u8 ocp_judge_read_byte(struct r8152 *tp, u16 type, u16 index, s32 *status)
{
	u32 data;
	u8 shift = index & 3;
	...
	if (type == MCU_TYPE_PLA)
		*status = pla_ocp_read(tp, index, sizeof(data), &data);
	else
		*status = usb_ocp_read(tp, index, sizeof(data), &data);
...
static void rtl8152_disable(struct r8152 *tp)
{
	...
	for (i = 0; i < 1000; i++) {
		ocp_data = ocp_judge_read_byte(tp, MCU_TYPE_PLA, PLA_OOB_CTRL, &status);
		if (status < 0)  {
			printk(KERN_WARNING"func:%s,%d read fifo(i=%d) may not success when disconnect.\n", __FUNCTION__, __LINE__, i);
			break;
		}
