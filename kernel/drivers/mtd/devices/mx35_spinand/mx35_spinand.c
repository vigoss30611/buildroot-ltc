 ...
 *
 * Copyright (c) 2015 Macronix International Co. LTD.
 *
...
static int spinand_probe(struct spi_device *spi_nand)
{
	...
	if (data && data->nr_parts)
		dev_warn(&spi_nand->dev, "ignoring %d default partitions on %s\n",
			data->nr_parts, mtd->name);
...
#else
	del_mtd_device(dev_get_drvdata(&spi->dev));
#endif
...
/*
 * Attention:
 *   Add target device or driver name into this table if needed.
 */
static const struct spi_device_id spinand_ids[] = {
	{"mx35", 0}, {"spinand", 0}, {"mx35_spinand", 0},
	{"n25q128", 0}, /* for Xilinx Zynq */
	{},
};
...
static int __init mx35_spinand_init(void)
{
	return spi_register_driver(&spinand_driver);
}


static void __exit mx35_spinand_exit(void)
{
	spi_unregister_driver(&spinand_driver);
}


module_init(mx35_spinand_init);
module_exit(mx35_spinand_exit);
