...
static int apollo_gpio_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < ARRAY_SIZE(apollo_gpio); i++) {
		...
		if(apollo_gpio[i].code == KEY_POWER) {
			...
			else if (apollo_gpio[i].code == KEY_RESET)
			...
			if (gpio_is_valid(apollo_gpio[i].index)) {
				error = gpio_request(apollo_gpio[i].index, apollo_gpio[i].name);
				if (error) {
			...
			apollo_gpio[i].irq = gpio_to_irq(apollo_gpio[i].index);
			gpio_direction_input(apollo_gpio[i].index);
