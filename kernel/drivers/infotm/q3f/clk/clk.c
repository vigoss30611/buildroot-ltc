...
static struct imapx_clk_init bus_clk[] = {
	...
	CLK_INIT("venc264-ctrl", "imap-venc264", "imap-venc264", BUS4),
...
static int imapx_get_clks_id(int id)
{
	...
	if (id < bus1 || id >= venc264)
		return -1;

	src = dev_clk_info[id - bus1].clk_src;
	return pll_clk_id[src];
}
