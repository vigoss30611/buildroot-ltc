...
{
	__clk_writel((r & 0x3f), BUS_CLK_DIVI_R(index));
}
...
static int imapx_dev_enable(struct clk_hw *hw)
{
	...
	if (dev->type == DDRPHY_CLK_SRC || dev->type == APB_CLK_SRC || dev->type == BUS6)
		return 0;
	...
	/*
	 * if the dev->info->clk_src equal DEFAULT_SRC
	 * it means we need't care, keep it as default source
	 * */
	if(dev->info->clk_src != DEFAULT_SRC){
		/*before enable clk, set the source to default value*/
		index = dev->info->clk_src;
...
static void imapx_dev_disable(struct clk_hw *hw)
{
	...
	if (dev->type == DDRPHY_CLK_SRC || dev->type == APB_CLK_SRC)
		return;
	...
	/*when disable a clk, set the source to 7, which
	 * may be positive to power saving*/
	val = __clk_readl(BUS_CLK_SRC(dev->type));
	val &= ~(0x7 << 0);
	val |= 0x7;
	__clk_writel(val, BUS_CLK_SRC(dev->type));
				 ...
				 unsigned long *prate)
{
	...
	info->clk_divider = dev_clk_div(*prate, rate, 1, 64) - 1;
	info->nco_en = 0;
				...
				unsigned long parent_rate)
{
	...
	if (dev->type == APB_CLK_SRC || dev->type == BUS6)
		return -1;
