...
static int q3f_gpio_direction(struct gpio_chip *chip, unsigned offset)
{
	uint8_t tmp;
	...
	/* In q3f, return 1 means gpio direction output. But
	 * 1 means input in gpio system. So we reverse here. */
	return !tmp;
}

static int q3f_gpio_input(struct gpio_chip *chip, unsigned offset)
{
	struct imapx_gpio_chip *igc = to_imapx_gpiochip(chip);
	uint8_t tmp;
	int cnt = 0;
...
static int q3f_gpio_output(struct gpio_chip *chip,
...
static int q3f_gpio_get_value(struct gpio_chip *chip, unsigned offset)
{
	struct imapx_gpio_chip *igc = to_imapx_gpiochip(chip);
	uint8_t tmp;
	int cnt = 0;
...
static void q3f_gpio_set_value(struct gpio_chip *chip,
					unsigned offset, int value)
{
	struct imapx_gpio_chip *igc = to_imapx_gpiochip(chip);
...
static int q3f_gpio_to_irq(struct gpio_chip *chip, unsigned offset)
{
	struct imapx_gpio_chip *igc = to_imapx_gpiochip(chip);
...
static void q3f_gpio_irq_ack(struct irq_data *d)
{
...
static void q3f_gpio_irq_mask(struct irq_data *d)
{
	unsigned long gpio = d->hwirq;
	//pr_err("%s: gpio=%d\n", __func__, gpio);
...
static void q3f_gpio_irq_unmask(struct irq_data *d)
{
	unsigned long gpio = d->hwirq;
	//pr_err("%s: gpio=%d\n", __func__, gpio);
...
static int q3f_gpio_irq_set_type(struct irq_data *d, unsigned int type)
{
	unsigned long gpio = d->hwirq;
	int trigger;
...
static void q3f_gpio_irq_handler(unsigned int irq, struct irq_desc *desc)
{
	struct imapx_gpio_bank *bank;
	int gpio, pin, type;
...
struct gpio_chip q3f_gpio_chip = {
	.label	= "q3f-gpiochip",
	.ngpio	= 125,
	.direction_input = q3f_gpio_input,
	.direction_output = q3f_gpio_output,
	.get_direction = q3f_gpio_direction,
	.get = q3f_gpio_get_value,
	.set = q3f_gpio_set_value,
	.to_irq = q3f_gpio_to_irq,
};

struct irq_chip q3f_irq_chip = {
	.name = "q3f-irqchip",
	.irq_ack = q3f_gpio_irq_ack,
	.irq_mask = q3f_gpio_irq_mask,
	.irq_unmask = q3f_gpio_irq_unmask,
...
int q3f_gpio_save(void)
{
	unsigned long flags;
	int i, j;

	local_irq_save(flags);
	for (i = 0; i < ARRAY_SIZE(q3f_gpio_bank); i++) {
		struct imapx_gpio_bank *bank = &q3f_gpio_bank[i];
		for (j = 0; j < bank->nr; j++) {
...
EXPORT_SYMBOL(q3f_gpio_save);

int q3f_gpio_restore(void)
{
	unsigned long flags;
	int i, j;
...
EXPORT_SYMBOL(q3f_gpio_restore);

static struct platform_driver q3f_gpiochip_driver = {
	.probe	= q3f_gpiochip_probe,
	.driver	= {
		.name = "imap-gpiochip",
		.owner = THIS_MODULE,
	},
...
arch_initcall(q3f_gpiochip_init);
