...
/** Send an extended read request to the slave device with the already requested address (u16 baseAddr, E-DDC).
 * @param baseAddr base address of the EDID module registers
 */
...
/** Set the segment pointer. (u16 baseAddr, should follow the formula N/2 where N is the block number).
 * @param baseAddr base address of the EDID module registers
 ...
 * @param baseAddr base address of the EDID module registers
 * @param mask to enable or disable the masking (u16 baseAddr, true to mask, ie true to stop seeing the intrrupt).
 */
