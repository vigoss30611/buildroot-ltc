...
/*Note: for OSD_Q3*/
static int set_enable(int idsx, int *params, int osd_num);
...
static int osd0_buf_index = 0;

static int set_enable(int idsx, int *params, int osd_num)
{
	int enable = *params;
	dss_trace("ids%d, enable %d\n", idsx, enable);
	return osd_n_enable(idsx, osd_num, enable);
}
...
static void osd_set_no_pre_out(int idsx, int window,
	...
	int overlay_x1, int overlay_y1, int overlay_x2, int overlay_y2)
{
	...
	if (!window) {
		...
		ids_writeword(idsx, OVCW0PCAR,
				((overlay_x1) << OVCWxPCAR_LeftTopX) |
				((overlay_y1) << OVCWxPCAR_LeftTopY));
		ids_writeword(idsx, OVCW0PCBR,
				((overlay_x2 - 1) << OVCWxPCBR_RightBotX) |
				((overlay_y2 - 1) << OVCWxPCBR_RightBotY));
		...
		if (in_format == NV12 || in_format == NV21) {
			uv_addr = ((u8*)gdma_addr) +
				prescaler_virt_in_width * prescaler_virt_in_height;
	...
	int in_width, int in_height, int out_width, int out_height, int bypass)
{
	...
	ids_write(idsx, OVCPSVSSR, OVCPSVSSR_VW_WIDTH,
		OVCPSVSSR_VW_WIDTH_SZ, prescaler_virt_in_width);
...
int scaler_config_size(unsigned int window,
...
void osd_set_parameter(void)
{
	...
	if (osd_win_id == 0) {
		...
		else if (osd0_buf_index == 1)
			ids_writeword(0, OVCPSB1SAR, (u32)pb); /*Note: Y or rgb*/
		else if (osd0_buf_index == 2)
			ids_writeword(0, OVCPSB2SAR, (u32)pb); /*Note: Y or rgb*/
		...
		if (in_format == NV21 || in_format == NV12) {
			/*Note: used to nv format*/
			pPsc = pb + (prescaler_left_top_x +
						prescaler_left_top_y * prescaler_virt_in_width);

			if (osd0_buf_index == 0)
				ids_writeword(0, OVCPSB0SAR, (u32)pPsc); /*Note: Y or rgb*/
			else if (osd0_buf_index == 1)
				ids_writeword(0, OVCPSB1SAR, (u32)pPsc); /*Note: Y or rgb*/
			else if (osd0_buf_index == 2)
				ids_writeword(0, OVCPSB2SAR, (u32)pPsc); /*Note: Y or rgb*/

			/*Note: CbCr*/
			if (prescaler_virt_in_height && prescaler_virt_in_width) {
				pPsc = pb + prescaler_virt_in_width * prescaler_virt_in_height
					+ (prescaler_left_top_x +
						(prescaler_left_top_y * prescaler_virt_in_width >> 1));
			} else {
				pPsc = pb + main_dtd.mHActive * main_dtd.mVActive;
			}

			if (osd0_buf_index == 0)
				ids_writeword(0, OVCPSCB0SAR, (u32)pPsc); /*Note: CbCr0*/
			else if (osd0_buf_index == 1)
				ids_writeword(0, OVCPSCB1SAR, (u32)pPsc); /*Note: CbCr1*/
			else if (osd0_buf_index == 2)
				ids_writeword(0, OVCPSCB2SAR, (u32)pPsc); /*Note: CbCr2*/

			ids_write(0, OVCPSCCR, OVCPSCCR_BUFSEL, 2, osd0_buf_index);

			dss_trace("CbCr : 0x%x\n", (u32)pPsc);
		}

		osd0_buf_index = (osd0_buf_index + 1)%3;
	}else {
...
static int dss_init_module(int idsx)
{
	...
	dss_info("enrefetch[%d] = %d, waittime[%d] = %d\n", idsx,
		enrefetch[idsx], idsx, waittime[idsx]);
