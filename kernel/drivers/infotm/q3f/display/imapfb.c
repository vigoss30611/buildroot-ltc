...
static int imapfb_create_fb(dtd_t dtd, imapfb_info_t *info)
{
	...
	/*Note: create 2 framebuffer devices in q3f, 
...
{
#if defined(CONFIG_SND_SOC_IP2906) && defined(CONFIG_Q3F_CVBS_I2C_IP2906)
...
static int __init imapfb_probe(struct platform_device *pdev)
{
	...
	if (core.lcd_interface == DSS_INTERFACE_LCDC) {
		...
		dtd.mHActive = 360;
		dtd.mVActive = 240;
		/*Note: 360x240 nv12 input can not run at q3f, it is a bug*/
	} else {
		dss_err("can not support interface :%d\n", core.lcd_interface);
		goto dealloc_fb;
	}
