...
static int coronampw_gpio_direction(struct gpio_chip *chip, unsigned offset)
{
	uint8_t tmp;
	...
	/* In coronampw, return 1 means gpio direction output. But
	 * 1 means input in gpio system. So we reverse here. */
	return !tmp;
}

static int coronampw_gpio_input(struct gpio_chip *chip, unsigned offset)
{
	struct imapx_gpio_chip *igc = to_imapx_gpiochip(chip);
	uint8_t tmp;
	int cnt = 0;
...
static int coronampw_gpio_output(struct gpio_chip *chip,
...
static int coronampw_gpio_get_value(struct gpio_chip *chip, unsigned offset)
{
	struct imapx_gpio_chip *igc = to_imapx_gpiochip(chip);
	uint8_t tmp;
	int cnt = 0;
...
static void coronampw_gpio_set_value(struct gpio_chip *chip,
					unsigned offset, int value)
{
	struct imapx_gpio_chip *igc = to_imapx_gpiochip(chip);
...
static int coronampw_gpio_to_irq(struct gpio_chip *chip, unsigned offset)
{
	struct imapx_gpio_chip *igc = to_imapx_gpiochip(chip);
...
static void coronampw_gpio_irq_ack(struct irq_data *d)
{
	unsigned long gpio = d->hwirq;
	//pr_err("%s: gpio=%d\n", __func__, gpio);

	if (d->irq > 15 && d->irq < 256)
		return;

	writel(0, GPIO_INTPEND(gpio));
}

static void coronampw_gpio_irq_mask(struct irq_data *d)
{
	unsigned long gpio = d->hwirq;
	//pr_err("%s: gpio=%d\n", __func__, gpio);
...
static void coronampw_gpio_irq_unmask(struct irq_data *d)
{
	unsigned long gpio = d->hwirq;
	//pr_err("%s: gpio=%d\n", __func__, gpio);
...
static int coronampw_gpio_irq_set_type(struct irq_data *d, unsigned int type)
{
	unsigned long gpio = d->hwirq;
	int trigger;
...
static void coronampw_gpio_irq_handler(unsigned int irq, struct irq_desc *desc)
{
	struct imapx_gpio_bank *bank;
	int gpio, pin, type;
...
struct gpio_chip coronampw_gpio_chip = {
	.label	= "coronampw-gpiochip",
	.ngpio	= 125,
	.direction_input = coronampw_gpio_input,
	.direction_output = coronampw_gpio_output,
	.get_direction = coronampw_gpio_direction,
	.get = coronampw_gpio_get_value,
	.set = coronampw_gpio_set_value,
	.to_irq = coronampw_gpio_to_irq,
};

struct irq_chip coronampw_irq_chip = {
	.name = "coronampw-irqchip",
	.irq_ack = coronampw_gpio_irq_ack,
	.irq_mask = coronampw_gpio_irq_mask,
	.irq_unmask = coronampw_gpio_irq_unmask,
	.irq_set_type = coronampw_gpio_irq_set_type,
};
...
struct imapx_gpio_bank coronampw_gpio_bank[] = {
	BANK_INFO(0, GIC_GP0_ID, 16),
	BANK_INFO(1, GIC_GP1_ID, 16),
	BANK_INFO(2, GIC_GP2_ID, 16),
	BANK_INFO(3, GIC_GP3_ID, 16),
	BANK_INFO(4, GIC_GP4_ID, 16),
	BANK_INFO(5, GIC_GP5_ID, 16),
	BANK_INFO(6, GIC_GP6_ID, 16),
	BANK_INFO(7, GIC_GP7_ID, 16),
};

struct imapx_gpio_chip imapx_gpio_chip[] = {
	[0] = {
		.compatible = "coronampw",
		.chip = &coronampw_gpio_chip,
		.irq_chip = &coronampw_irq_chip,
		.bankcount = 8,
		.bank = coronampw_gpio_bank,
	},
};
...
static int coronampw_gpiochip_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < igc->bankcount; i++) {
		if (bank[i].irq != -1) {
			irq_set_chained_handler(bank[i].irq, coronampw_gpio_irq_handler);
...
int coronampw_gpio_save(void)
{
	unsigned long flags;
	int i, j;

	local_irq_save(flags);
	for (i = 0; i < ARRAY_SIZE(coronampw_gpio_bank); i++) {
		struct imapx_gpio_bank *bank = &coronampw_gpio_bank[i];
		for (j = 0; j < bank->nr; j++) {
...
EXPORT_SYMBOL(coronampw_gpio_save);

int coronampw_gpio_restore(void)
{
	unsigned long flags;
	int i, j;
...
EXPORT_SYMBOL(coronampw_gpio_restore);

static struct platform_driver coronampw_gpiochip_driver = {
	.probe	= coronampw_gpiochip_probe,
	.driver	= {
		.name = "imap-gpiochip",
		.owner = THIS_MODULE,
	},
...
static int __init coronampw_gpiochip_init(void)
{
	return platform_driver_register(&coronampw_gpiochip_driver);
}
arch_initcall(coronampw_gpiochip_init);
