...
static int imapfb_create_fb(dtd_t dtd, imapfb_info_t *info)
{
	...
	/*Note: create 2 framebuffer devices in coronampw, 
...
{
#if defined(CONFIG_SND_SOC_IP2906) && defined(CONFIG_CORONAMPW_CVBS_I2C_IP2906)
...
static ssize_t dbgfs_read(char __user *buff, size_t count, loff_t *ppos, enum dbgfs_type type)
{
	...
	case IDS_FPS:
		...
		dss_err("cur_cnt = %u, lst_cnt = %u, cur_t = %u, lst_t = %u \n",
			core.frame_cur_cnt, core.frame_last_cnt, core.cur_timestamp, core.last_timestamp);
#endif
...
static ssize_t dbgfs_set_color_bar_write(struct file *file, const char __user *buff,
				    size_t count, loff_t *ppos)
{
	ssize_t ret = 0;
	int err = 0;
...
static int __init imapfb_probe(struct platform_device *pdev)
{
	...
	if (core.lcd_interface == DSS_INTERFACE_LCDC) {
		...
		dtd.mHActive = 360;
		dtd.mVActive = 240;
		/*Note: 360x240 nv12 input can not run at coronampw, it is a bug*/
	} else {
		dss_err("can not support interface :%d\n", core.lcd_interface);
		goto dealloc_fb;
	}
	...
	fill_framebuffer_logo((u32)imapfb_info[0].map_cpu_f1,  (u32*)&lcd_dtd.mHActive, (u32*)&lcd_dtd.mVActive);
...
#else
	ids_fill_dummy_yuv((u32)imapfb_info[0].map_cpu_f1 , (u32)dtd.mHActive, (u32)dtd.mVActive, 0);

	#if defined( TEST_2_OSD_BLENDING)
	fill_framebuffer_logo((u32)imapfb_info[1].map_cpu_f1 , (u32*)&lcd_dtd.mHActive, (u32*)&lcd_dtd.mVActive);
	#endif
...
/*
Chip ID(CORONAMPW)    efuse-id       lcd
Q3420P           3420010       Lock
Q3420F           3420110       Lock
C20                3420310       ok
C23                3420210       ok
