				...
				unsigned *selector)
{
	...
	ret = mp8845c_cpu_write(dev, mp->vout_reg, (vsel|0x80));
	if (ret < 0) {
...
static int mp8845c_get_voltage(struct regulator_dev *rdev)
{
	...
	ret = mp8845c_cpu_read(dev, mp->vout_reg, &tmp);
	if (ret < 0) {
...
static struct mp8845c_regulator mp8845c_regulator[] = {
	MP8845C_REGULATOR(FOR_VCPU, 0x00, 600000, 1449600, 6700, 128, mp8845c_ops),
};
...
static int mp8845c_regulator_probe(struct platform_device *pdev)
{
	...
	pdata = pdev->dev.platform_data;/*TODO*/
	mp->dev = &pdev->dev;
...
MODULE_DESCRIPTION("MP8845C regulator driver");
MODULE_LICENSE("GPL");
