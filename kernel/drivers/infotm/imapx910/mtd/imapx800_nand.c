...
static unsigned int nf2_randc_seed_hw (struct imapx800_nand_chip *imapx800_chip, unsigned int raw_seed, unsigned int cycle, int polynomial)
{
	...
	if (timeout >= 20) {
		result = -1;
		NAND_DEBUG(KERN_WARNING," nf2_randc_seed_hw timeout\n");
	}
	return result;
...
static int nf2_soft_reset(struct imapx800_nand_chip *imapx800_chip, int num)
{
	...
	if (timeout >= INFOTM_DMA_BUSY_TIMEOUT) {
		NAND_DEBUG(KERN_WARNING," nf2_soft_reset timeout, tmp = 0x%x, num = %d\n", tmp, num);
		return -1;
	}
...
static void imapx800_nand_set_retry_level(struct infotm_nand_chip *infotm_chip, int chipnr, int retry_level)
{
	...
	NAND_DEBUG(KERN_WARNING," [nand debug %d] retry level%d %x chipnr %d\n", 
			__LINE__, retry_level, (unsigned)*(infotm_chip->retry_cmd_list + chipnr * 7 + retry_level), chipnr);
...
static int imapx800_nand_get_eslc_para(struct infotm_nand_chip *infotm_chip)
{
	...
	if (infotm_nand_save->eslc_para_magic == IMAPX800_ESLC_PARA_MAGIC) {
		...
		if (eslc_parameter->parameter_set_mode == START_END) {
			...
			for (i=0; i<eslc_parameter->register_num; i++) {
				eslc_parameter->parameter_offset[0][i] = infotm_nand_save->eslc_para[0][i];
				NAND_DEBUG(KERN_WARNING," imapx para[%02d] %02x\n", i, eslc_parameter->parameter_offset[0][i]);
			}
...
static int imapx800_nand_get_retry_table(struct infotm_nand_chip *infotm_chip)
{
	...
	if (!strncmp((char*)plat->name, NAND_NFTL_NAME, strlen((const char*)NAND_NFTL_NAME))){
		for (i=0; i<7; i++)
			pri_info("====imapx800%d %x chipnr 0\n", i, (unsigned)*(infotm_chip->retry_cmd_list + 0 + i));
		for (i=0; i<7; i++)
			pri_info("====1imapx800%d %x chipnr 1\n", i, (unsigned)*(infotm_chip->retry_cmd_list + 7 + i));
	}
#endif
	return error;
}
	...
	*(uint16_t *)(oob_buffer + 2) = IMAPX800_UBOOT_MAGIC;
	for (blk_num=0; blk_num<para_reserved_blks; blk_num++) {
		...
		for (page_addr=0; page_addr<IMAPX800_PARA_ONESEED_SAVE_PAGES; page_addr++) {
			...
			status = chip->waitfunc(mtd, chip);
			if (status & NAND_STATUS_FAIL)
				NAND_DEBUG(KERN_WARNING," infotm nand para write failed at %d \n", page_addr);
		}
...
static void imapx800_nand_get_save_para(struct infotm_nand_chip *infotm_chip)
{
	...
	list_for_each_safe(l, n, &imapx800_chip->chip_list) {
		...
		if (!strncmp((char*)plat->name, NAND_NORMAL_NAME, strlen((const char*)NAND_NORMAL_NAME))) {
			...
			infotm_nand_save->version_magic = VERSION_MAGIC;
...
static int imapx800_nand_hw_init(struct infotm_nand_chip *infotm_chip)
{
	...
	if(imapx_pad_init("nand") != 0){
		NAND_DEBUG(KERN_WARNING," nand io config error\n");
		return -ENOENT;
	}

	imapx800_chip->sys_clk = clk_get_sys("imap-nandflash", "imap-nand");
	if (IS_ERR(imapx800_chip->sys_clk)) {
		NAND_DEBUG(KERN_WARNING," iMAP NAND: Failed to get bus clock.\n");
		return -ENOENT;
	}
	...
	if(IS_ERR(imapx800_chip->ecc_clk)) {
		NAND_DEBUG(KERN_WARNING," iMAP NAND: Failed to get ecc clock.\n");
		return -ENOENT;
	}
	...
	if (nf2_soft_reset(imapx800_chip, NF_RESET)) {
		NAND_DEBUG(KERN_WARNING," nf2_soft_reset error\n");
		return -1;
	}
...
static int imapx800_nand_options_confirm(struct infotm_nand_chip *infotm_chip)
{
	...
	if (i < max_ecc_mode) {
		...
		NAND_DEBUG(KERN_WARNING," soft ecc or none ecc just support in linux self nand base please selected it at platform options\n");
		error = -ENXIO;
	}
	...
	if (options_selected > options_define) {
		NAND_DEBUG(KERN_WARNING," internal mode error for selected internal mode: %s force internal mode to : %s\n", infotm_nand_internal_string[options_selected >> 16], infotm_nand_internal_string[options_define >> 16]);
		options_selected = options_define;
	}
		...
		case NAND_RANDOMIZER_MODE:
			...
			if (!strncmp((char*)plat->name, NAND_BOOT_NAME, strlen((const char*)NAND_BOOT_NAME))) {
				...
				if (busw) {
					...
					if (infotm_chip->uboot0_unit == IMAPX820_UBOOT0_UNIT) {
						for (j=0; j<infotm_chip->max_seed_cycle; j++) {
							for(i = 0; i < 8; i++) {
								infotm_chip->data_seed[j][i] = *(unsigned *)((unsigned *)imapx800_seed + j * 8 + i);
								infotm_chip->ecc_seed[j][i] =  *(unsigned *)((unsigned *)imapx800_ecc_seed_2k + j * 8 + i);
							}
						}
					} else {
						for (j=0; j<infotm_chip->max_seed_cycle; j++) {
...
{
	//NAND_DEBUG("imapx800_nand_enable_chip %d\n", chipnr);
...
{
	struct imapx800_nand_chip *imapx800_chip = infotm_chip_to_imapx800(infotm_chip);
	//NAND_DEBUG("imapx800_nand_select_chip %d\n", chipnr);
...
static int imapx800_nand_cmdfifo_start(struct infotm_nand_chip *infotm_chip, int timeout)
{
	...
	if ((imapx800_chip->nand_irq_no) && (timeout > 0)) {
		...
		if (time_out_cnt >= INFOTM_NAND_BUSY_TIMEOUT) {
			NAND_DEBUG(KERN_WARNING," nand_cmdfifo_start %x \n", value_int);
			ret = -EIO;
		}

		if (value_int & NF_RB_TIMEOUT_INT) {
			NAND_DEBUG(KERN_WARNING," nand_cmdfifo_start RB_TIMEOUT %x \n", value_int);
			ret = -EBUSY;
		}
	}
...
static int imapx800_nand_dma_waiting(struct infotm_nand_chip *infotm_chip, struct imapx800_nand_chip *imapx800_chip, int ecc_enable, int secc_used, int trans_dir, int prog_type, int timeout)
{
	 ...
	 *(2)Date:2014-8-4
	 *(3)Reason:
	 *(3.1)For solving the bug:INFOTM_DMA_BUSY_TIMEOUT when poweroff.
	 *(3.2)Not only during shutdown time,at other time when write with error like "DMA_TIME_OUT",this modification is also 
	 *     adaptive.
	 *(4)Analyse:There are 2 possibilities for INFOTM_DMA_BUSY_TIMEOUT 
	 *(4.1)The interrupt is not reported to cpu,but the DMA operation is done.
	 *(4.2)The interrupt is not reported to cpu,and the DMA operation is not done.
	 *(5)Codes:
	 *(5.1)These 4 variables are used to judge whether the DMA operation finished?
	*/
    uint32_t intr_status_register = 0;//0x8c                                  
    uint32_t fsm_status_register = 0;//0x64                                   
    uint32_t working_status_register = 0;//0x94                               
    uint32_t report_timeout_err = 0xffff; //judge result: 0,not timeout; 0!,timeout
	...
	*/
	...
	if ((imapx800_chip->nand_irq_no) && (timeout > 0)) {
		...
		if (time_out_cnt == 0) {
			 ...
			 *(2)Date:2014-8-4
			 *(3)Reason:
			 *(3.1)Read registers to judge whether the TRXF-FIFO operation is finished?
			 *     -->If finished,for the TRXF is empty,that is to say,it is finished,only some Reason
			 *     caused timeout,but not really timeout.
			 *     -->If not finished yet,it is really timeout,we will return the error to FTL.
			*/
            report_timeout_err = 0xffff; //default:timeout                                        
 			intr_status_register = readl(imapx800_chip->regs + NF2INTR);          
        	working_status_register = readl(imapx800_chip->regs + NF2WSTA);       
        	fsm_status_register = readl(imapx800_chip->regs + NF2TFST);           
        	//TRXF-DONE                                                           
        	if( intr_status_register & int_finish)                                
              	report_timeout_err = 0;                                         
        	//SDMA is done,working status is not busy                             
        	if( 0x11100 == working_status_register )                              
              	report_timeout_err = 0;                                         
        	//TRX-status                                                          
        	if( 0x10080 == fsm_status_register )                                  
                report_timeout_err = 0;                                

		    if( report_timeout_err ){
			  	NAND_DEBUG(KERN_WARNING," INFOTM_DMA_BUSY_TIMEOUT int %d %d \n", imapx800_chip->nand_irq_type, imapx800_chip->nand_irq_condition);
				#if NAND_TEST
				tmp = imapx_pad_get_mode(PADSRANGE(0, 15));
				NAND_DEBUG(KERN_WARNING," [nand debug %d] pad mode 0x%x\n",__LINE__, tmp);
				NAND_DEBUG(KERN_WARNING," infotm_chip->clk = %ld, bus clk %ld\n", clk_get_rate(imapx800_chip->ecc_clk),  clk_get_rate(imapx800_chip->sys_clk));
				#endif 
				if (imapx800_chip->nand_irq_condition == 0)
					ret = -EBUSY;
			}
		...
		if (time_out_cnt >= INFOTM_DMA_BUSY_TIMEOUT) {
			NAND_DEBUG(KERN_WARNING," INFOTM_DMA_BUSY_TIMEOUT \n");
#if NAND_TEST 
			tmp = imapx_pad_get_mode(PADSRANGE(0, 15));
			NAND_DEBUG(KERN_WARNING," [nand debug %d] pad mode 0x%x\n",__LINE__, tmp);
			NAND_DEBUG(KERN_WARNING," infotm_chip->clk = %ld, bus clk %ld\n", clk_get_rate(imapx800_chip->ecc_clk),  clk_get_rate(imapx800_chip->sys_clk));
		...
		if (value_int & NF_RB_TIMEOUT_INT) {
			NAND_DEBUG(KERN_WARNING," INFOTM_DMA_BUSY_TIMEOUT RB_TIMEOU \n");
			ret = -EBUSY;
		}
...
static int imapx800_nand_hwecc_correct(struct infotm_nand_chip *infotm_chip, unsigned size, unsigned oob_size)
{
	...
	if (size % chip->ecc.size) {
		NAND_DEBUG(KERN_WARNING," error parameter size for ecc correct %x\n", size);
		ret = -EINVAL;
		goto exit2;
	}
	...
	if (size) {
		...
		if (ecc_result & ((1 << (size / chip->ecc.size)) - 1)) {
			//NAND_DEBUG ("nand communication have uncorrectable ecc error %x\n", ecc_result);
			ret = -EIO;
...
exit1:
	if (oob_size) {
		...
		if (ecc_result & SECC0_RESULT_FLAG) {
			//NAND_DEBUG ("nand communication have uncorrectable secc error %x\n", ecc_result);
			ret = -EIO;
			infotm_chip->secc_status = 1;
			goto exit2;
...
static void imapx800_nand_boot_write_page_hwecc(struct mtd_info *mtd, struct nand_chip *chip, const uint8_t *buf)
{
	...
	if (infotm_chip->valid_chip[i]) {
		...
		for (j=0; j<dma_cycle; j++) {
			if(irom_V21 == IMAPX9_NEW_IROM){
				if (infotm_nand_save->head_magic == IMAPX800_UBOOT_PARA_MAGIC){
					*(uint32_t *)oob_buf = (crc32((0 ^ 0xffffffffL), buf + j * nand_page_size, nand_page_size)) ^ 0xffffffffL;
				}
			}
			if (j == (dma_cycle - 1))
...
static int imapx800_nand_boot_read_page_hwecc(struct mtd_info *mtd, struct nand_chip *chip, uint8_t *buf, int page, int oob_len)
{
	...
	if (infotm_chip->valid_chip[i]) {
		...
		do {
			...
			if (error) {
				NAND_DEBUG(KERN_WARNING," nand boot read page error %d\n", error);
				goto exit;
			}
			...
			if ((mtd->ecc_stats.failed - stats.failed) && (retry_count == 0)) {
				...
				if (j >= nand_oobavail_size) {
					...
					for (k=0; k<nand_oobavail_size; k++) {
						NAND_DEBUG(KERN_WARNING," %02x ", oob_buf[k]);
					}
		...
		if (mtd->ecc_stats.failed - stats.failed) {
			NAND_DEBUG(KERN_WARNING,"read page retry failed %x %d \n ", page_addr, infotm_chip->retry_level[i]);
			goto exit;
		} else {
			if (retry_count > 0)
				NAND_DEBUG(KERN_WARNING,"read page retry success %x %d \n ", page_addr, retry_count);
			goto exit;
		}
...
static void imapx800_nand_check_uboot0_status(struct mtd_info *mtd, struct nand_chip *chip)
{
	...
	do {
		...
		if ((error != 0) || (mtd->ecc_stats.failed - stats.failed))
			NAND_DEBUG(KERN_WARNING," page %d read failed %d\n", i, error);
...
static int infotm_nand_probe(struct infotm_nand_platform *plat, struct imapx800_nand_chip *imapx800_chip)
{
	...
	if (infotm_chip == NULL) {
		NAND_DEBUG(KERN_WARNING," no memory for flash infotm_chip\n");
		err = -ENOMEM;
		goto exit_error;
	}
	...
	if (!strncmp((char*)plat->name, NAND_BOOT_NAME, strlen((const char*)NAND_BOOT_NAME))) {
		...
		infotm_nand_save = (struct infotm_nand_para_save *)imapx800_chip->resved_mem;
		infotm_chip->infotm_nand_saveenv = saveenv;
		if (infotm_nand_save->head_magic != IMAPX800_UBOOT_PARA_MAGIC)
...
static void nand_read_id(struct imapx800_nand_chip *imapx800_chip)
{
	int tmp;
	char buf[8];
	...
	writel(0x1, imapx800_chip->regs + NF2FFCE);
	trx_afifo_ncmd(imapx800_chip, 0x0, 0x0, 0x90);
	trx_afifo_ncmd(imapx800_chip, 0x0, 0x1, 0x00);
	trx_afifo_nop (imapx800_chip, 0, 8);
	trx_afifo_ncmd(imapx800_chip, 0xf, 0x3, 0x00);
	writel(0x0, imapx800_chip->regs + NF2FFCE);

	writel(0xe, imapx800_chip->regs + NF2CSR);
	writel(0x1, imapx800_chip->regs + NF2STRR);

	while(1) {
		tmp = (readl(imapx800_chip->regs + NF2INTR) & 1);
		if(tmp & NF_TRXF_FINISH_INT)
			break;
	}

	writel(0xf, imapx800_chip->regs + NF2CSR);
	writel(0xffffff, imapx800_chip->regs + NF2INTR);

	*(unsigned int *)buf = readl(imapx800_chip->regs + NF2STSR0);
	*(unsigned int *)(buf + 4) = readl(imapx800_chip->regs + NF2STSR1);
	NAND_DEBUG(KERN_WARNING," ID: %02x %02x %02x %02x %02x %02x %02x %02x\n", buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], buf[6], buf[7]);
}
...
static int imapx800_nand_suspend(struct platform_device *pdev, pm_message_t pm)
{
	...
	NAND_DEBUG(KERN_WARNING,"nand_suspend \n");
#if NAND_TEST
			tmp = imapx_pad_get_mode(PADSRANGE(0, 15));
			NAND_DEBUG(KERN_WARNING," [nand debug %d] pad mode 0x%x\n",__LINE__, tmp);
			NAND_DEBUG(KERN_WARNING," infotm_chip->clk = %ld, bus clk %ld\n", clk_get_rate(imapx800_chip->ecc_clk),  clk_get_rate(imapx800_chip->sys_clk));
...
static void imapx800_nand_shutdown(struct platform_device *pdev)
{
	...
	list_for_each_safe(l, n, &imapx800_chip->chip_list) {
		...
		plat = infotm_chip->platform;
		if(plat == NULL){
			printk("nand plat is NULL, inand need found!\n");
			return;
		}
		if (!strncmp((char*)plat->name, NAND_NFTL_NAME, strlen((const char*)NAND_NFTL_NAME))) {
			mtd = &infotm_chip->mtd;
			...
			NAND_DEBUG(KERN_WARNING,"nand_shutdown %d %d \n", infotm_chip->retry_level[0], infotm_chip->retry_level[1]);
			clk_disable_unprepare(imapx800_chip->sys_clk);
			clk_disable_unprepare(imapx800_chip->ecc_clk);

			if (imapx_pad_init("nand") != 0)
				NAND_DEBUG(KERN_WARNING," nand shutdown io config error\n");
...
static int imapx800_nand_probe(struct platform_device *pdev)
{
	...
	if (imapx800_chip == NULL) {
		NAND_DEBUG(KERN_WARNING," no memory for flash info\n");
		err = -ENOMEM;
		goto exit_error;
	}
	...
	if (imapx800_chip->sys_info_buf == NULL) {
		NAND_DEBUG(KERN_WARNING," no memory for flash info buf\n");
		err = -ENOMEM;
		goto exit_error;
	}
	...
	for (i=0; i<infotm_nand_dev->dev_num; i++) {
		...
		if (plat == NULL) {
			NAND_DEBUG(KERN_WARNING," error for not platform data\n");
			continue;
		}
		err = infotm_nand_probe(plat, imapx800_chip);
		if (err) {
			NAND_DEBUG(KERN_WARNING," %s dev probe failed %d\n", plat->name, err);
			continue;
		}
	}
...
#define DRV_NAME	"infotm_imapx910_nand"
#define DRV_VERSION	"FTL V6.0 & NAND V9.0 20140701"
#define DRV_AUTHOR	"xiaojun_yoyo"
#define DRV_DESC	"Infotm nand flash linux driver for Imapx910"
...
{
	NAND_DEBUG(KERN_ALERT, " %s, Version %s (c) 2014 InfotmIC.\n", DRV_DESC, DRV_VERSION);
