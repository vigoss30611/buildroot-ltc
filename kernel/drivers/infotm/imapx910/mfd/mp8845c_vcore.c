...
static struct i2c_client *i2c_client_core;
...
int mp8845c_core_read(struct device *dev, uint8_t reg,uint8_t *val)
{
	int ret;
...
int mp8845c_core_write(struct device *dev, uint8_t reg, uint8_t val)
{
	int ret;
	struct mp8845c *mp8845c = dev_get_drvdata(dev);
...
static int mp8845c_init_chip(struct mp8845c *mp8845c)
{
	...
	mp8845c_core_read(mp8845c->dev, SYS_CNTL2_REG, &tmp);
	tmp |= 1 << 5;
	mp8845c_core_write(mp8845c->dev, SYS_CNTL2_REG, tmp);
				...
				const struct i2c_device_id *id) 
{
	...
	printk(KERN_ERR "mp8845c vcore probe\n");
...
static int __init mp8845c_init(void)
{
	...
	info.platform_data = &mp8845c_core_platform_data;
	strlcpy(info.type, "mp8845c-core", I2C_NAME_SIZE);
...
MODULE_DESCRIPTION("mp8845c driver for core");
MODULE_LICENSE("GPL");
