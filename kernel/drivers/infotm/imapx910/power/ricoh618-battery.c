...
#include <mach/ricoh61x_battery_init.h>
#include <mach/pad.h>
...
#define RICOH61x_ENTER_LOW_VOL			85
...
struct ricoh61x_battery_info {
	...
	struct delayed_work	repwron_work;
	struct delayed_work	otg_mode_work;
#ifdef ENABLE_LOW_BATTERY_DETECTION
...
static int id_pin_st, repwron_st;
int g_full_flag;
int charger_irq;
int g_soc;
int g_fg_on_mode;
...
static struct ricoh61x_battery_info *g_info;
int ricoh61x_charger_in(void)
{
	int err;
	uint8_t status;

	err = ricoh61x_read(g_info->dev->parent, CHGSTATE_REG, &status);
	if (err < 0)
		return 0;

	return (status & 0xC0) ? 1 : 0;
}
EXPORT_SYMBOL(ricoh61x_charger_in);
...
static int set_Rlow(struct ricoh61x_battery_info *info)
{
	...
	if (Vocv < get_OCV_voltage(info,3))
	{
		...
		RICOH_DBG("PMU: Modify RBAT from %d to %d ", info->soca->Rbat, info->soca->R_low);
...
static int Set_back_ocv_table(struct ricoh61x_battery_info *info)
{
	...
	{		
		RICOH_DBG("PMU: Modify back RBAT from %d to %d ",  info->soca->R_low,info->soca->Rbat);
	...
	if (val != 0){ /* connect adp or usb */
		if (calc_ocv(info) < get_OCV_voltage(info, RICOH61x_OCV_OFFSET_BOUND) )
		{
			if(0 == info->soca->ocv_table_low[0]){
				...
				RICOH_DBG("PMU : %s : OCV table %d 0x%x\n",__func__,i * 10, ocv_table[i]);
...
//		printk("INT_IR_CHGSTS1 = 0x%x\n",val); */
	...
	RICOH_DBG("PMU: cc_sum = %d: cc_cap_res= %d: \n", cc_sum, cc_cap_res);
...
static int get_target_use_cap(struct ricoh61x_battery_info *info)
{
	...
	for (i = 0; i <= 10; i = i+1) {
		...
		ocv_table[i] = temp;
		RICOH_DBG("PMU : %s : ocv_table %d is %d v\n",
	...
	RICOH_DBG("PMU: -------  Ocv_now_table= %d: Rsys_now= %d =======\n",
	       Ocv_now_table, Rsys_now);

	RICOH_DBG("PMU: -------  Rsys= %d: cutoff_ocv= %d: Ocv_ZeroPer_now= %d =======\n",
	...
	for (i = 1; i < 11; i++) {
		RICOH_DBG("PMU : %s : ocv_table %d is %d v Ocv_ZerPernow is %d\n",
	...
	RICOH_DBG("PMU: -------  Target_Cutoff_Vol= %d: Ocv_ZeroPer_now= %d: start_per= %d =======\n",
	...
	RICOH_DBG("PMU: ------- FA_CAP_now= %d: RE_CAP_now= %d: CC_OnePer_step= %d: target_use_cap= %d: hurry_up_flg= %d -------\n",
...
static int calib_ocvTable(struct ricoh61x_battery_info *info, int vbat_ocv)
{
	...
	if (info->soca->Ibat_ave > RICOH61x_REL1_SEL_VALUE) {
		RICOH_DBG("PMU: %s IBAT > 64mA -- Not Calibration --\n", __func__);
		return 0;
	}
	...
	RICOH_DBG("PMU : %s :max %d min %d current %d\n",__func__,
		info->soca->OCV100_max,info->soca->OCV100_min,vbat_ocv);
	RICOH_DBG("PMU : %s : New OCV 100 = 0x%x\n",__func__,ocv100_new);
	...
	ricoh61x_scaling_OCV_table(info, cutoff_ocv/1000, 
		ocv100_new/1000, &start_per, &end_per);
	...
	for (i = 0; i <= 10; i = i+1) {
		...
		RICOH_DBG("PMU: %s : * %d0%% voltage = %d uV\n",
	...
	if (RICOH61x_SOCA_STABLE == info->soca->status) {
				...
				if (g_full_flag == 1) {
					...
					if ((calculated_ocv < get_OCV_voltage(info, (RICOH61x_ENTER_FULL_STATE_OCV - 1)))
						...
						|| (info->soca->Ibat_ave < info->ch_icchg*50 + 100) )
						{
							...
							RICOH_DBG("PMU:full_rate= %d: Delta= %d:\n", full_rate, info->soca->cc_delta);
		...
		if(info->soca->Ibat_ave >= 0) {
			...
			if (info->soca->rsoc_ready_flag == 0) {
				...
				RICOH_DBG("PMU FG_RESET : %s : facap is %d re_cap is %d soc is %d\n",__func__,fa_cap,re_cap,temp_soc);
			} else {
				re_cap = get_check_fuel_gauge_reg(info, RE_CAP_H_REG, RE_CAP_L_REG,
								0x7fff);
			}
	...
	if (RICOH61x_SOCA_DISP == info->soca->status) {
		...
		if (info->first_pwon) {
			...
			if (calculated_ocv < get_OCV_voltage(info, 0)) {
				...
				if(info->soca->rsoc_ready_flag == 0) {
					info->soca->status = RICOH61x_SOCA_STABLE;
					RICOH_DBG("PMU FG_RESET : %s : initial  dsoc  is  %d\n",__func__,info->soca->displayed_soc);
				} else if  (Ibat < 0) {
					if (info->soca->displayed_soc < 300) {
						info->soca->target_use_cap = 0;
						info->soca->status = RICOH61x_SOCA_LOW_VOL;
...
end_flow:
	...
	if (g_fg_on_mode
		...
		RICOH_DBG("display_work:writing 0x7f PSWR_REG\n");
		if (err < 0)
			dev_err(info->dev, "Error in writing PSWR_REG\n");
		g_soc = 0x7F;
		err = calc_capacity_in_period(info, &cc_cap,
		...
		err = ricoh61x_write(info->dev->parent, PSWR_REG, val);
		RICOH_DBG("display_work:writing display PSWR_REG\n");
		if (err < 0)
			dev_err(info->dev, "Error in writing PSWR_REG\n");
	...
	else if(RICOH61x_SOCA_UNSTABLE != info->soca->status)
	{
			...
			err = calc_capacity_in_period(info, &cc_cap,
							 &is_charging, false);
			if(err<0)
				RICOH_DBG("Wrong when calc_capacity\n");
			...
			if(cc_cap > 100)
				{	
					...
					RICOH_DBG("left is %d,diff is %d,inipswr is %d  \n",cc_left,soc_diff,info->soca->init_pswr);
					info->soca->init_pswr += soc_diff;
					RICOH_DBG("after init_pswr is %d \n",info->soca->init_pswr);
	...
	RICOH_DBG("PMU:STATUS= %d: IBAT= %d: VSYS= %d: VBAT= %d: DSOC= %d: RSOC= %d:\n",
...
static int ricoh61x_init_fgsoca(struct ricoh61x_battery_info *info)
{
	...
	for (i = 0; i <= 10; i = i+1) {
		info->soca->ocv_table[i] = get_OCV_voltage(info, i);
		RICOH_DBG(KERN_INFO "PMU: %s : * %d0%% voltage = %d uV\n",
				 __func__, i, info->soca->ocv_table[i]);
	}
	...
	RICOH_DBG("PMU: %s : * Rbat = %d mOhm   n_cap = %d mAH\n",
...
static void drv_bus_set(struct ricoh61x_battery_info *info, int val)
{
//	if (info->drvbus < 0)
//		return;

//	imapx_pad_set_mode(1, 1, info->drvbus);
//	imapx_pad_set_dir(0, 1, info->drvbus);
//	imapx_pad_set_outdat(!!val, 1, info->drvbus);
	if (info->drvbus < 0)
		gpio_direction_output(info->drvbus, !val);
	else
		gpio_direction_output(info->drvbus, !!val);
...
static int otg_id_state(struct ricoh61x_battery_info *info)
{
	int ret;
	int state = 1;
//	if (info->extraid < 0)
//		return -1;
	...
	if (gpio_is_valid(info->extraid))
		state = gpio_get_value(info->extraid);

	return state;
}

static void ricoh61x_host_work(struct work_struct *work)
{
	struct ricoh61x_battery_info *info = container_of(work,
		struct ricoh61x_battery_info, host_work.work);
	int tmp = otg_id_state(info);
	...
	if (tmp) {
		if (id_pin_st == 0) {
			...
			drv_bus_set(info, 0);
			msleep(20);
			ricoh61x_clr_bits(info->dev->parent, 0xB3, 0x10);
		}
	} else {
		if (id_pin_st == 1) {
			id_pin_st = 0;
			ricoh61x_set_bits(info->dev->parent, 0xB3, 0x10);
			msleep(20);
			drv_bus_set(info, 1);
		}
	}

	schedule_delayed_work(&info->host_work, msecs_to_jiffies(500));
}

static void ricoh61x_repwron_work(struct work_struct *work)
{
	struct ricoh61x_battery_info *info = container_of(work,
		struct ricoh61x_battery_info, repwron_work.work);
	int tmp = ricoh61x_charger_in();
	...
	if (tmp) {
		...
		if (repwron_st == 1) {
			repwron_st = 0;
			ricoh61x_set_repwron(0);
		}
	}

	schedule_delayed_work(&info->repwron_work, msecs_to_jiffies(500));
}

static int check_jeita_status(struct ricoh61x_battery_info *info,
				bool *is_jeita_updated)
...
*/
{
	...
	*is_jeita_updated = false;
	...
	if (info->soca->ready_fg) {
		...
		RICOH_DBG("JEITA: %s *** cannot update by resetting FG ******\n", __func__);
		goto out;
	}
	...
	RICOH_DBG("PMU: %s *** Temperature: %d, vfchg: %d, SW status: %d, chg_status: %d ******\n",
		 __func__, temp, vfchg, info->soca->status, info->soca->chg_status);

	if (temp <= 0 || 55 <= temp) {
		/* 1st and 5th temperature ranges (~0, 55~) */
		RICOH_DBG("PMU: %s *** Temp(%d) is out of 0-55 ******\n", __func__, temp);
		...
		*is_jeita_updated = true;
		...
		if (vfchg != info->jt_vfchg_h) {
			RICOH_DBG("PMU: %s *** 0<Temp<12, update to vfchg=%d ******\n", 
			...
			*is_jeita_updated = true;
			...
			RICOH_DBG("PMU: %s *** 0<Temp<50, already set vfchg=%d, so no need to update ******\n",
					__func__, info->jt_vfchg_h);
		...
		if (vfchg != info->jt_vfchg_h) {
			RICOH_DBG("PMU: %s *** 12<Temp<50, update to vfchg==%d ******\n", __func__, info->jt_vfchg_h);
			...
			*is_jeita_updated = true;
			...
			RICOH_DBG("PMU: %s *** 12<Temp<50, already set vfchg==%d, so no need to update ******\n", 
					__func__, info->jt_vfchg_h);
		...
		if (vfchg != info->jt_vfchg_l) {
			RICOH_DBG("PMU: %s *** 50<Temp<55, update to vfchg==%d ******\n", __func__, info->jt_vfchg_l);
			...
			*is_jeita_updated = true;
			...
			RICOH_DBG("JEITA: %s *** 50<Temp<55, already set vfchg==%d, so no need to update ******\n", 
					__func__, info->jt_vfchg_l);
	...
	get_power_supply_status(info);
	RICOH_DBG("PMU: %s *** Hope updating value in this timing after checking jeita, chg_status: %d, is_jeita_updated: %d ******\n",
		 __func__, info->soca->chg_status, *is_jeita_updated);

	return 0;
	
out:
	RICOH_DBG("PMU: %s ERROR ******\n", __func__);
	return err;
}
...
static void ricoh61x_jeita_work(struct work_struct *work)
{
	...
	if (0 == ret) {
		...
		RICOH_DBG("PMU: %s *** Call check_jeita_status() in jeita_work, err:%d ******\n", 
							__func__, ret);
		queue_delayed_work(info->monitor_wqueue, &info->jeita_work,
					 RICOH61x_FG_RESET_TIME * HZ);
	}
...
static void check_charging_state_work(struct work_struct *work)
{
	...
	if (((chargeCurrent >= 0x3ffc && chargeCurrent <= 0x3fff)
		...
		RICOH_DBG("PMU:%s --- No battery !! Enter Factory mode ---\n"
				, __func__);
	...
	RICOH_DBG("PMU:%s --- Battery exist !! Return Normal mode ---0x%2x\n"
			, __func__, val);
...
static void ricoh61x_scaling_OCV_table(struct ricoh61x_battery_info *info, int cutoff_vol, int full_vol, int *start_per, int *end_per)
{
	...
	if (info->soca->ocv_table_def[0] > cutoff_vol * 1000) {
		*start_per = 0;
		RICOH_DBG("PMU : %s : setting value of cuttoff_vol(%d) is out of range(%d) \n",__func__, cutoff_vol, info->soca->ocv_table_def[0]);
	} else {
		for (i = 1; i < 11; i++) {
...
//		info->soca->cutoff_ocv = info->soca->target_vsys - Ibat_min * info->soca->Rsys / 1000;
		...
		if( temp < info->fg_target_vsys) {
			...
			RICOH_DBG("PMU : %s : setting value of target vsys(%d) is out of range(%d)\n",__func__, info->fg_target_vsys, temp);
		}
	}

	//for debug
	RICOH_DBG("PMU : %s : target_vsys is %d target_ibat is %d\n",
	...
	RICOH_DBG("PMU : %s : original 0x%x, before 0x%x, after 0x%x\n",
...
//	info->first_pwon = (val == 0) ? 1 : 0;
	...
	RICOH_DBG("PMU FG_RESET : %s : initial pswr = %d\n",__func__,info->soca->init_pswr);
	...
	/* set VINDET 2.9V */
	ret = ricoh61x_write(info->dev->parent, VINDAC_REG, 0x03);
	if (ret < 0) {
		dev_err(info->dev, "Error in writing the control register\n");
		return ret;
	}
...
static int ricoh61x_init_charger(struct ricoh61x_battery_info *info)
{
	...
	if (err < 0) {
		...
		//debug messeage
	err = ricoh61x_read(info->dev->parent, CHGISET_REG,&val);
	RICOH_DBG("PMU : %s : after CHGISET_REG (0x%x) is 0x%x info->ch_ichg is 0x%x info->ch_icchg is 0x%x\n",
	__func__,CHGISET_REG,val,
	info->ch_ichg,info->ch_icchg);

		//debug messeage
	err = ricoh61x_read(info->dev->parent, BATSET1_REG,&val);
	RICOH_DBG("PMU : %s : before BATSET1_REG (0x%x) is 0x%x info->ch_vbatovset is 0x%x\n",
	__func__,BATSET1_REG,val,info->ch_vbatovset);
	
	/* BATSET1_REG(0xBA) setting */
	err = ricoh61x_read(info->dev->parent, BATSET1_REG, &val);
	if (err < 0) {
		dev_err(info->dev,"Error in read BATSET1 register %d\n", err);
		goto free_device;
	}
	...
	if (err < 0) {
		...
		//debug messeage
	err = ricoh61x_read(info->dev->parent, BATSET1_REG,&val);
	RICOH_DBG("PMU : %s : after BATSET1_REG (0x%x) is 0x%x info->ch_vbatovset is 0x%x\n",__func__,BATSET1_REG,val,info->ch_vbatovset);
	
		//debug messeage
	err = ricoh61x_read(info->dev->parent, BATSET2_REG,&val);
	RICOH_DBG("PMU : %s : before BATSET2_REG (0x%x) is 0x%x info->ch_vrchg is 0x%x info->ch_vfchg is 0x%x \n",__func__,BATSET2_REG,val,info->ch_vrchg,info->ch_vfchg);
	...
	RICOH_DBG("PMU : %s : test test val %d, val2 %d vfchg %d\n", __func__, val, val2, vfchg_val);
	...
	RICOH_DBG("PMU : %s : test test val %d, val2 %d icchg %d\n", __func__, val, val2, icchg_val);
	...
	RICOH_DBG("PMU : %s : 100 min %d, 100 max %d vfchg %d icchg %d rbat %d\n",__func__,
...
static void otg_mode_work(struct work_struct *work)
{	
	struct ricoh61x_battery_info *info	
		= container_of(work, struct ricoh61x_battery_info, otg_mode_work.work);	
	int state = get_otg_charge_mode();	
	RICOH_INFO("%s: otg_mode = 0x%x\n", __func__, state);	
	if (state == 1)	
		ricoh61x_write(info->dev->parent, REGISET2_REG, 0x0e);	
}
...
static void charger_irq_work(struct work_struct *work)
{
	...
	if(info->chg_ctr & 0x02) {
		uint8_t sts;
		ret = ricoh61x_read(info->dev->parent, 
				RICOH61x_INT_MON_CHGCTR, &sts)7;
		if (ret < 0)
			dev_err(info->dev, "Error in reading the control register\n");
	...
	case	0: // plug out USB/ADP
			RICOH_DBG("%s : val = %d plug out\n",__func__, val);
			break;
	case	1: // plug in ADP
			RICOH_DBG("%s : val = %d plug in ADPt\n",__func__, val);
	...
	case	3:// plug in USB/ADP
			RICOH_DBG("%s : val = %d plug in ADP USB\n",__func__, val);
			break;
	default:
			RICOH_DBG("%s : val = %d unknown\n",__func__, val);
			break;
	}
	
	if (charger_wakeup) {
		RICOH_ERR("%s: Charger Wakeup\n", __func__);
		charger_wakeup = 0;
...
static irqreturn_t charger_usb_isr(int irq, void *battery_info)
{
	...
	queue_work(info->workqueue, &info->irq_work);
	schedule_delayed_work(&info->otg_mode_work, msecs_to_jiffies(500));
...
static int set_pswr(struct ricoh61x_battery_info *info)
{
	...
	RICOH_DBG("display_work:writing display PSWR_REG\n");
	if (err < 0)
		RICOH_DBG("display_work:Error in writing PSWR_REG\n");
...
static int calc_capacity(struct ricoh61x_battery_info *info)
{
	...
	if (info->soca->rsoc_ready_flag  != 0) {
		...
		RICOH_DBG("PMU FG_RESET : %s : capacity %d pswr %d cc_delta %d\n",__func__,	capacity_l, info->soca->init_pswr, cc_delta);
	}
...
static int get_battery_temp_2(struct ricoh61x_battery_info *info)
{
	...
	if (0 != temp_gain)
		...
		RICOH_DBG("PMU %s Skip to compensate temperature\n", __func__);
		goto out;
	}
	...
	if (temp < -120000) {
		...
		RICOH_DBG("PMU %s low temperature %d\n", __func__, new_temp/10);  
	} else if (temp > 520000) {
		/* High Temperature */
		if (0 != (2500 - Vthm)) {
		...
		RICOH_DBG("PMU %s [Rthm] Rthm %d[ohm]\n", __func__, Rthm);
		...
		RICOH_DBG("PMU %s high temperature %d\n",
			 __func__, new_temp/10);  
	} else {
		/* the unit of new_temp is 0.1 degree */
		new_temp = temp / 1000;
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		val->intval = ret;
		info->status = ret;
		RICOH_DBG("Power Supply Status is %d\n",	info->status);
		break;
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		...
		RICOH_DBG("battery voltage is %d mV\n", info->cur_voltage);
		break;
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		RICOH_DBG("battery capacity is %d%%\n",	info->capacity);
		break;
	...
	case POWER_SUPPLY_PROP_TEMP:
		if (info->soca->ready_fg) {
			...
			RICOH_DBG("battery temperature is %d degree\n",
				info->battery_temp);
		} else {
	...
	case POWER_SUPPLY_PROP_TIME_TO_EMPTY_NOW:
		if (info->soca->ready_fg) {
			...
			RICOH_DBG("time of empty battery is %d minutes\n",
							info->time_to_empty);
		} else {
			//val->intval = -EINVAL;
	 ...
	 case POWER_SUPPLY_PROP_TIME_TO_FULL_NOW:
		if (info->soca->ready_fg) {
			...
			RICOH_DBG("time of full battery is %d minutes\n",
							info->time_to_full);
		} else {
			//val->intval = -EINVAL;
...
static int ricoh61x_battery_probe(struct platform_device *pdev)
{
	...
	struct pmu_cfg *ip;
	int type_n;
	int ret, temp, drv_bus;

	RICOH_ERR("PMU: %s : version is %s\n", __func__,RICOH61x_BATTERY_VERSION);
	...
	type_n = Battery_Type();
	temp = sizeof(pdata->type)/(sizeof(struct ricoh618_battery_type_data));
	if(type_n  >= temp)
	{
		RICOH_DBG("%s : Battery type num is out of range\n", __func__);
		type_n = 0;
	}
	RICOH_DBG("%s type_n=%d,temp is %d\n", __func__, type_n,temp);
	...
	{
		RICOH_DBG("%s : Battery num is out of range\n", __func__);
		info->num = 0;
	}
	RICOH_DBG("%s info->num=%d,temp is %d\n", __func__, info->num,temp);
	...
	info->drvbus = ip->drvbus;
	info->extraid = ip->extraid;

	
	drv_bus = (info->drvbus < 0) ? abs(info->drvbus) : info->drvbus;
	if (gpio_is_valid(drv_bus)) {
		ret = gpio_request(drv_bus, "otg_drv_bus");
		if (ret) {
			pr_err("failed request gpio for otg_drv_bus\n");
			goto err_request_drvbus;
		}
	}
	...
	INIT_DELAYED_WORK(&info->jeita_work, ricoh61x_jeita_work);
	INIT_DELAYED_WORK(&info->changed_work, ricoh61x_changed_work);

	id_pin_st = otg_id_state(info);
	if (id_pin_st) {
		drv_bus_set(info, 0);
		msleep(20);
		ricoh61x_clr_bits(info->dev->parent, 0xB3, 0x10);
	...
	ricoh61x_read(info->dev->parent, CHGSTATE_REG, &ret);
	if (ret&0xC0) {
		repwron_st = 1;
		ricoh61x_set_repwron(1);
	} else {
	...
	INIT_DELAYED_WORK(&info->host_work, ricoh61x_host_work);
	schedule_delayed_work(&info->host_work, msecs_to_jiffies(500));
	INIT_DELAYED_WORK(&info->repwron_work, ricoh61x_repwron_work);
	schedule_delayed_work(&info->repwron_work, msecs_to_jiffies(500));
	INIT_DELAYED_WORK(&info->otg_mode_work, otg_mode_work);

	/* Charger IRQ workqueue settings */
	charger_irq = pdata->irq;


	ret = request_threaded_irq(charger_irq,
					NULL, charger_in_isr,
	...
	rtc_gpio_irq_enable(1, 1);/* enable rtc_gp1 irq */
...
#endif
	cancel_delayed_work(&info->jeita_work);
	cancel_delayed_work(&info->host_work);
	cancel_delayed_work(&info->repwron_work);
	cancel_delayed_work(&info->otg_mode_work);
	cancel_work_sync(&info->irq_work);
...
static int ricoh61x_battery_suspend(struct device *dev)
{
	...
	if (g_fg_on_mode
		...
		ret = ricoh61x_write(info->dev->parent, PSWR_REG, val);
		if (ret < 0)
			RICOH_DBG("Error in writing PSWR_REG\n");
...
/*	flush_work(&info->irq_work); */
	...
	drv_bus_set(info, 0);
	msleep(300);
	ricoh61x_clr_bits(dev->parent, 0xC2, 0x2);
