...
static int apollo3_power_domain_probe(struct platform_device *pdev)
{
	int ret;
...
static struct platform_driver apollo3_power_domain_driver = {
	.probe	= apollo3_power_domain_probe,
	.driver	= {
		.owner	= THIS_MODULE,
		.name	= "power_domain",
	},
...
static int __init apollo3_power_domain_init(void)
{
	int ret;
	...
	ret = platform_driver_register(&apollo3_power_domain_driver);
	if (ret < 0) {
...
static void __exit apollo3_power_domain_exit(void)
{
	return;
}

module_init(apollo3_power_domain_init);
module_exit(apollo3_power_domain_exit);
MODULE_LICENSE("GPL");
