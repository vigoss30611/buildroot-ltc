...
static int apollo3_gpio_direction(struct gpio_chip *chip, unsigned offset)
{
	uint8_t tmp;
	...
	/* In apollo3, return 1 means gpio direction output. But
	 * 1 means input in gpio system. So we reverse here. */
	return !tmp;
}

static int apollo3_gpio_input(struct gpio_chip *chip, unsigned offset)
{
	struct imapx_gpio_chip *igc = to_imapx_gpiochip(chip);
	uint8_t tmp;
	int cnt = 0;
...
static int apollo3_gpio_output(struct gpio_chip *chip,
...
static int apollo3_gpio_get_value(struct gpio_chip *chip, unsigned offset)
{
	struct imapx_gpio_chip *igc = to_imapx_gpiochip(chip);
	uint8_t tmp;
	int cnt = 0;
...
static void apollo3_gpio_set_value(struct gpio_chip *chip,
					unsigned offset, int value)
{
	struct imapx_gpio_chip *igc = to_imapx_gpiochip(chip);
...
static int apollo3_gpio_to_irq(struct gpio_chip *chip, unsigned offset)
{
	struct imapx_gpio_chip *igc = to_imapx_gpiochip(chip);
...
static void apollo3_gpio_irq_ack(struct irq_data *d)
{
...
static void apollo3_gpio_irq_mask(struct irq_data *d)
{
	unsigned long gpio = d->hwirq;
	//pr_err("%s: gpio=%d\n", __func__, gpio);
...
static void apollo3_gpio_irq_unmask(struct irq_data *d)
{
	unsigned long gpio = d->hwirq;
	//pr_err("%s: gpio=%d\n", __func__, gpio);
...
static int apollo3_gpio_irq_set_type(struct irq_data *d, unsigned int type)
{
	unsigned long gpio = d->hwirq;
	int trigger;
...
static void apollo3_gpio_irq_handler(unsigned int irq, struct irq_desc *desc)
{
	struct imapx_gpio_bank *bank;
	int gpio, pin, type;
...
struct gpio_chip apollo3_gpio_chip = {
	.label	= "apollo3-gpiochip",
	.ngpio	= 125,
	.direction_input = apollo3_gpio_input,
	.direction_output = apollo3_gpio_output,
	.get_direction = apollo3_gpio_direction,
	.get = apollo3_gpio_get_value,
	.set = apollo3_gpio_set_value,
	.to_irq = apollo3_gpio_to_irq,
};

struct irq_chip apollo3_irq_chip = {
	.name = "apollo3-irqchip",
	.irq_ack = apollo3_gpio_irq_ack,
	.irq_mask = apollo3_gpio_irq_mask,
	.irq_unmask = apollo3_gpio_irq_unmask,
	.irq_set_type = apollo3_gpio_irq_set_type,
};
...
struct imapx_gpio_bank apollo3_gpio_bank[] = {
...
struct imapx_gpio_chip imapx_gpio_chip[] = {
	[0] = {
		.compatible = "apollo3",
		.chip = &apollo3_gpio_chip,
		.irq_chip = &apollo3_irq_chip,
		.bankcount = 8,
		.bank = apollo3_gpio_bank,
	},
};
...
static int apollo3_gpiochip_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < igc->bankcount; i++) {
		if (bank[i].irq != -1) {
			irq_set_chained_handler(bank[i].irq, apollo3_gpio_irq_handler);
...
int apollo3_gpio_save(void)
{
	unsigned long flags;
	int i, j;

	local_irq_save(flags);
	for (i = 0; i < ARRAY_SIZE(apollo3_gpio_bank); i++) {
		struct imapx_gpio_bank *bank = &apollo3_gpio_bank[i];
		for (j = 0; j < bank->nr; j++) {
...
EXPORT_SYMBOL(apollo3_gpio_save);

int apollo3_gpio_restore(void)
{
	unsigned long flags;
	int i, j;
...
EXPORT_SYMBOL(apollo3_gpio_restore);

static struct platform_driver apollo3_gpiochip_driver = {
	.probe	= apollo3_gpiochip_probe,
	.driver	= {
		.name = "imap-gpiochip",
		.owner = THIS_MODULE,
	},
...
static int __init apollo3_gpiochip_init(void)
{
	return platform_driver_register(&apollo3_gpiochip_driver);
}
arch_initcall(apollo3_gpiochip_init);
