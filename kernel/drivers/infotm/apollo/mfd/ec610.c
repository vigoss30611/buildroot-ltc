...
void ec610_chip_init(struct ec610 *ec610, struct pmu_cfg *ip)
{
	...
	ec610_read(ec610->dev, EC610_WAKE_FLAG, &value);
	if (value & 0x1)
		wake_flag = 1;
	else
		wake_flag = 0;

	ec610_clr_bits(ec610->dev, EC610_WAKE_CTL, 0x1);
	
	ec610_read(ec610->dev, 0x4d, &value);
	value |= 0x08;
	ec610_write(ec610->dev, 0x4d, value);
	ec610_read(ec610->dev, 0x80, &value);
	value |= 0x0e;
	ec610_write(ec610->dev, 0x80, value);
	...
	/*ec610_read(ec610->dev, 0x1, &value);
	value |= 0x1;
	ec610_write(ec610->dev, 0x1, value);*/

	ec610_clr_bits(ec610->dev, EC610_PROTECT_CTL0, 0x03);
	ec610_set_bits(ec610->dev, EC610_PROTECT_CTL0, 0x01);
...
{
	u8 value=0;
	int index, abs_index, pin_st;
	
	// set wakeup sources.
	ec610_i2c_read(ec610_i2c_client, EC610_WAKE_CTL, &value);
	value &= ~EC610_WK_EN_ONOFF_S;
	value |= (EC610_WK_EN_ONOFF_L | EC610_ONOFF_SL_RST | EC610_WK_EN_EIRQ);

	if (item_exist("power.dcdetect")) {
		index = item_integer("power.dcdetect", 1);
		abs_index = (index>0)? index : (-1*index);
		if (gpio_is_valid(abs_index)) {
			pin_st = (index>0)? gpio_get_value(abs_index) : (!gpio_get_value(abs_index));
			if (pin_st)
				value &= ~EC610_WK_EN_VINOK;
			else
				value |= EC610_WK_EN_VINOK;
...
static ssize_t ec610_poweron_show(struct class *class,
		struct class_attribute *attr, char *buf)
{
	uint8_t val;
	ec610_read(&ec610_i2c_client->dev, 0x00, &val);
...
static struct class_attribute ec610_class_attrs[] = {
	...
	__ATTR(on, 0444, ec610_poweron_show, NULL),
	__ATTR_NULL
};
...
static ssize_t wake_flag_show(struct class *class,
		struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", wake_flag);
}

static ssize_t wake_flag_store(struct class *class,
				struct class_attribute *attr,
				const char *buf, size_t count)
{
	wake_flag = simple_strtoul(buf, NULL, 16);

	return count;
}

static struct class_attribute wake_class_attrs[] = {
	__ATTR(state, 0644, wake_flag_show, wake_flag_store),
	__ATTR_NULL
};

static struct class wake_class = {
	.name = "wakeup",
	.class_attrs = wake_class_attrs,
};
			      ...
			      const struct i2c_device_id *id)
{
	...
	ec610_set_bits(&ec610_i2c_client->dev, 0x41, 0x20);

#ifdef CONFIG_PMIC_DEBUG_IF
	class_register(&ec610_class);
#endif
	class_register(&wake_class);
...
static void ec610_shutdown(struct i2c_client *client)
{
	...
	if (ec610->irq)
		ec610_irq_exit(ec610);*/
    //printk("ec610_shutdown, gSensorEnable: %d\n", gSensorEnable);
#ifndef CONFIG_INFT_CAR_ACC_ENABLE
	if(!gSensorEnable) {
        ec610_write(ec610->dev, 0x82, 0x00);
    } else 
#endif
    {
        value = 0x18;
        ec610_write(ec610->dev, 0x82, value);
        ec610_i2c_read(ec610_i2c_client, EC610_MFP_CTL5, &value);
        value &= ~EC610_IRQS_TYPE;
        value ^= EC610_IRQS_TYPE_RE;
        ec610_i2c_write(ec610_i2c_client, EC610_MFP_CTL5, value);
    }

	ec610_i2c_read(ec610_i2c_client, 0x82, &value);
    printk("[ec6103]reg: 0x82, value: 0x%x", value);
}
