...
static videoParams_t param_video;
static audioParams_t param_audio;
static productParams_t param_product;
static hdcpParams_t param_hdcp;
...
{
	param_audio.mInterfaceType=audio_t->mInterfaceType;
    	param_audio.mCodingType=audio_t->mCodingType;
    	param_audio.mChannelAllocation=audio_t->mChannelAllocation;
   	param_audio.mSampleSize=audio_t->mSampleSize;
    	param_audio.mSamplingFrequency=audio_t->mSamplingFrequency;
	api_Audio_Cfg(&param_video, &param_audio, &param_product, &param_hdcp);
...
//	irq   = imapx_pad_irq_number(index);
	...
	if(!inited) {
		...
		if(ret < 0) {
			printk(KERN_ERR "failed to request IRQ for hplg.\n");
			return -2;
		}
...
static int dss_init_module(int idsx)
{
	static int ret, inited = 0;
	unsigned char * addr;
	
	dss_trace("ids%d\n", idsx);
	/* fpga don't test hdmi by sun*/
	//return 0;
...
//		msleep(10);
	...
	if (!inited) {
		ret =request_irq(GIC_HDMITX_ID, hdmi_handler, 
