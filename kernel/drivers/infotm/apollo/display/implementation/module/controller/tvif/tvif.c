...
static int set_vic(int idsx, int *params, int num)
{
...
#else
		clk_set_rate(ids1_eitf, /*297000000*/256000000); // changed by linyun.xiong @2015-09-04 ??? for test
#endif
	}
	...
	if (idsx == 0)	// Currently CVBS only
		...
		if (vic == 3 || vic == 18)
			clkdiv = 10;//8; // changed by linyun.xiong @2015-09-04 ??? for test
...
static int dss_init_module(int idsx)
{
	...
	if ((pt == PRODUCT_MID)) {
		printk("In lcd mode, tvif don't test on fpga\n");
		return 0;
	}
