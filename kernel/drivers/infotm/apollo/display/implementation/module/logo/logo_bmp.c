...
unsigned char __initdata logo[] = {
#ifdef CONFIG_LCD_SPI_OTA5182A
#include "black_car_320_240.h"
#elif CONFIG_LCD_SPI_ILI8961
#include "black_car_800_480.h"
#endif
};

unsigned char __initdata hdmi_logo[] = {
#ifdef CONFIG_HDMI_LOGO_ENABLE	
#include "black_car_1920_1080.h"
#endif
};

int __init fill_framebuffer_logo ( unsigned int fb_viraddr, unsigned int * bmp_width, unsigned int * bmp_height )
{
  *bmp_width = 360;
  *bmp_height = 240;
  memcpy ( fb_viraddr, logo, sizeof ( logo ) );
  return 0;
}
EXPORT_SYMBOL ( fill_framebuffer_logo );

int __init fill_framebuffer_hdmi_logo ( unsigned int fb_viraddr, unsigned int * bmp_width, unsigned int * bmp_height )
{
  *bmp_width = 1920;
  *bmp_height = 1080;
  memcpy ( fb_viraddr, hdmi_logo, sizeof ( hdmi_logo ) );
  return 0;
}
EXPORT_SYMBOL ( fill_framebuffer_hdmi_logo);
