       ...
       u16 format, u32 width, dma_addr_t addr)
{
   ...
   ids_write(idsx, OVCW1CR, OVCWxCR_BPPMODE, 5, format?17:11);
   ids_writeword(idsx, OVCW1VSSR, width);
   ids_writeword(idsx, OVCW1PCAR, 
           ((0) << x_lefttop) | ((0) << y_lefttop));
   ...
   ids_writeword(idsx, OVCW1B0SAR, addr);
   ids_write(idsx, OVCW1CR, OVCWxCR_BUFSEL, 2, 0);
   ids_write(idsx, OVCDCR, OVCDCR_UpdateReg, 1, 1);
}
