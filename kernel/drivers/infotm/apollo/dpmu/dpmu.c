...
static const char *bus1_dev[BUS1_NUM] = {"ISP", "CAMIF", "MIPICSI"};
static const char *bus3_dev[BUS3_NUM] = {"IDS1/HDMI", "CRYPTO",	"FODET", "ISPOST"};
static const char *bus4_dev[BUS4_NUM] = {"VDEC_G1", "VENC_H1", "VDEC_G2", "VENC_H2"};
static const char *bus6_dev[BUS6_NUM] = {"USB_Host", "USB_OTG", "SD1", "SD2", "SD0", "GDMA", "Ethernet"};
...
static void dpmu_state(void)
{
	...
	printk("cpu0: 0x%.8x, 0x%.8x, 0x%.8x, 0x%.8x\n", readl(CPU_WBSTC0), readl(CPU_RBSTC0), readl(CPU_WDATC0), readl(CPU_RDATC0));
...
{
	if(all_irq_flag != 1){
		c0_wd[VAL_CNT] = readl(CPU_WDATC0);
		c0_rd[VAL_CNT] = readl(CPU_RDATC0);
		...
		c0_wb[VAL_CNT] = readl(CPU_WBSTC0);
		c0_rb[VAL_CNT] = readl(CPU_RBSTC0);
...
static void calc_bus_h(uint32_t mask_id, uint32_t compare_id)
{
...
static void calc_id(uint32_t id)
{
	...
	if(id == 0){
		...
		calc_bus_h(ID_NMSK, ID_NCMP);
	}
	if(id == 1){
		calc_bus_l(MSK_L, CMP_L0);
		calc_bus_h(MSK_H, CMP_H0);
	}
	if(id == 2){
		calc_bus_l(MSK_L, CMP_L1);
		calc_bus_h(MSK_H, CMP_H1);
	}
	if(id == 3){
		calc_bus_l(MSK_L, CMP_L2);
		calc_bus_h(MSK_H, CMP_H2);
	}
	if(id == 4){
		writel(MSK_L,		BUS3_IDMSK);
		writel(CMP_M3,		BUS3_IDCMP);
		writel(MSK_L,		BUS4_IDMSK);
		writel(CMP_M3,		BUS4_IDCMP);
		calc_bus_h(MSK_H, CMP_H3);
	}
	if(id == 5)
		calc_bus_h(MSK_H, CMP_H4);
	if(id == 6)
		calc_bus_h(MSK_H, CMP_H5);
	if(id == 7)
		calc_bus_h(MSK_H, CMP_H6);
	...
	if(id == 0){
		c0_wd[0] = readl(CPU_WDATC0);
		c0_rd[0] = readl(CPU_RDATC0);
	}
...
static int cmp_id_h[10] = {0x0, 0x20, 0x40, 0x60,0x80, 0xa0, 0xc0};
...
static int dpmu_device(char *str)
{
	...
	for(i=0; i<BUS6_NUM; i++){
		if(strncmp(buf, bus6_dev[i], 8) == 0){
			ret = device_cal((int)BUS6_IDMSK, MSK_H, cmp_id_h[i]);
			if (ret != 0)
				goto
				   err_dev;	
			printk("bus6:%d 	%s:W: %4lld.%.2lldMB/s   R: %4lld.%.2lldMB/s\n", i, bus6_dev[i], CA_M(calc(ave(b6_wd))), CA_B(calc(ave(b6_wd))), CA_M(calc(ave(b6_rd))), CA_B(calc(ave(b6_rd))));
			goto find_dev;
		}
	}
...
static void print_all(void)
{
	...
	for(i = 1; i < (BUS6_NUM+1); i++)
		printk("	%-8s:W: %4lld.%.2lldMB/s   R: %4lld.%.2lldMB/s\n", bus6_dev[i-1], CA_M(calc(b6_wd[i])), CA_B(calc(b6_wd[i])), CA_M(calc(b6_rd[i])), CA_B(calc(b6_rd[i])));
