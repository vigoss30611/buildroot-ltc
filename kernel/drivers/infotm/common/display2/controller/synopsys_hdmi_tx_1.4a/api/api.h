 ...
 * @gpio_hpd gpio number if use gpio instead of HPD pin, zero if use HPD pin
 */
...
/**
 * Get the HDCP encryption state
 * @return a the state of the HDCP encryption state
 * @note Used for debug purposes
 */
...
/* Set mCurrentState */
int api_SetState(state_t);

/* Get EDID state */
int api_GetEdidState(void);

/* Set EDID state */
int api_SetEdidState(edid_status_t);
