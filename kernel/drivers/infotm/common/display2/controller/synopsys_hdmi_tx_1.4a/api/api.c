...
int api_SetState(state_t state)
{
	api_mCurrentState = state;
	return 0;
}

int api_GetEdidState(void)
{
	return edid_mStatus;
}

int api_SetEdidState(edid_status_t state)
{
	edid_mStatus = state;
	return 0;
}
...
int api_Audio_Cfg(videoParams_t *video,audioParams_t *audio,
        productParams_t * product, hdcpParams_t * hdcp)
{
    int audioOn = (audio != 0 && videoParams_GetHdmi(video));
    int hdcpOn = (hdcp != 0 && (control_SupportsHdcp(api_mBaseAddress)
...
void api_EventHandler(int gpio_hpd)
{
	//u8 i = 0;
	u8 state = 0;
	u8 handled = FALSE;
	...
	if (gpio_hpd)
		api_mHpd = gpio_get_value(gpio_hpd);
	else
...
for (state = 0; state < 10; state++)
{
	...
	//	state = control_InterruptEdidState(api_mBaseAddress);
		//if ((state != 0) || (api_mHpd != oldHpd))
		if (state != 0)		// William. Not auto read EDID after plug in
		{
			...
			if (edid_EventHandler(api_mBaseAddress, api_mHpd, state) == EDID_DONE)
			{
				...
				{	
					api_mEventRegistry[EDID_READ_EVENT](gpio_hpd);
				}
			}
	...
	{ /* HDMI mode */
		if (api_EdidHdmivsdb(&vsdb) == TRUE)
		{
			...
			if (videoParams_GetEncodingOut(video) == YCC444
					&& videoParams_GetColorResolution(video) > 8
...
/*				if (videoParams_GetTmdsClock(video) > (hdmivsdb_GetMaxTmdsClk(
						&vsdb) * 500) && hdmivsdb_GetMaxTmdsClk(&vsdb))
				{
