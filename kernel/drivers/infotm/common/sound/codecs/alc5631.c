...
#ifdef ALC_DBG
#define alc_dbg(x...)	alc_dbg(x);
#else
#define alc_dbg(x...)	do {} while (0)
#endif

static struct timer_list rt5631_hp_timer;
struct snd_soc_codec *rt5631_codec;
struct switch_dev rt5631_hp_dev;
...
static int rt5631_hp_status = UNPLUG;
...
struct rt5631_priv {
	...
	struct work_struct hpswitchwork;
		...
		unsigned int reg, unsigned int value)
{
	...
	if (ret < 0) {
		dev_err(codec->dev,
				"Write: Failed to set private addr: %d\n", ret);
		goto err;
	}
		...
		struct snd_soc_codec *codec, unsigned int reg)
{
	...
	if (ret < 0) {
		dev_err(codec->dev,
				"Read: Failed to set private addr: %d\n", ret);
		return ret;
	}
...
/* MIC Input Type */
static const char *const rt5631_input_mode[] = {
...
static const char *const rt5631_spk_ratio[] = {
...
static const char *const rt5631_adc_data_sel[] = {
...
static const char *const rt5631_adcr_fun_sel[] = {"Stereo ADC", "Voice ADC"};
...
static const char *const rt5631_vdac_clk_sel[] = {"SYSCLK1", "SYSCLK2"};
		...
		struct snd_soc_dapm_widget *sink)
{
	...
	reg &= RT5631_SYSCLK_SOUR_SEL_PLL;
	return reg;
}
		...
		struct snd_soc_dapm_widget *sink)
{
	...
	value = reg & RT5631_SYSCLK_SOUR_SEL_PLL;
		...
		struct snd_soc_dapm_widget *sink)
{
	...
	value = reg & RT5631_SYSCLK2_SOUR_SEL_PLL2;
	if (reg & RT5631_ADCR_FUN_VADC) {
		if (reg & RT5631_VDAC_CLK_SOUR_SCLK2)
			return value;
		else
		...
		struct snd_soc_dapm_widget *sink)
{
	...
	reg &= RT5631_VDAC_CLK_SOUR_SCLK2;
	if (reg)
		return reg;
	else
		return 0;
...
static void rt5631_cap_work(struct work_struct *work)
{
	...
	int ret = 0;
	alc_dbg("%s(%d) -- ADC Power On!!!\n", __func__, __LINE__);
	ret = snd_soc_update_bits(codec, RT5631_PWR_MANAG_ADD1,
			RT5631_PWR_ADC_L_CLK | RT5631_PWR_ADC_R_CLK,
			RT5631_PWR_ADC_L_CLK | RT5631_PWR_ADC_R_CLK);
	alc_dbg("rt5631_cap_work..... is %d\n", ret);
	mdelay(250);
	return;
}
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMD:
		alc_dbg("%s(%d) -- SND_SOC_DAPM_POST_PMD\n",
				__func__, __LINE__);
		snd_soc_update_bits(codec, RT5631_PWR_MANAG_ADD1,
				RT5631_PWR_ADC_L_CLK | RT5631_PWR_ADC_R_CLK, 0);
		break;
	case SND_SOC_DAPM_POST_PMU:
		alc_dbg("%s(%d) -- SND_SOC_DAPM_POST_PMU\n",
				__func__, __LINE__);
		/*delay time can be adjusted to enable ADC power*/
		schedule_delayed_work(&rt5631_delay_cap, msecs_to_jiffies(500));
		break;
	default:
		break;
	}
...
static const char *const rt5631_spkvoll_sel[] = {"Vmid", "SPKMIXL"};
...
static const char *const rt5631_hpvoll_sel[] = {"Vmid", "OUTMIXL"};
...
static const char *const rt5631_outvoll_sel[] = {"Vmid", "OUTMIXL"};
...
static const char *const rt5631_outvolr_sel[] = {"Vmid", "OUTMIXR"};
...
static const char *const rt5631_hpvolr_sel[] = {"Vmid", "OUTMIXR"};
...
static const char *const rt5631_spkvolr_sel[] = {"Vmid", "SPKMIXR"};
...
static const char *const rt5631_spol_src_sel[] = {
...
static const char *const rt5631_spor_src_sel[] = {
...
/* MONO Input */
static const char *const rt5631_mono_src_sel[] = {
...
/* Left HPO Input */
static const char *const rt5631_hpl_src_sel[] = {
	"Left HPVOL", "Left DAC"
};
...
/* Right HPO Input */
static const char *const rt5631_hpr_src_sel[] = {
...
static const struct snd_soc_dapm_widget rt5631_dapm_widgets[] = {
	...
	SND_SOC_DAPM_ADC_E("Left ADC", "HIFI Capture", SND_SOC_NOPM, 0, 0,
			adc_event, SND_SOC_DAPM_PRE_PMD |
...
void rt5631_switch_hp(void)
{
	u16 pwr_reg;
	struct snd_soc_codec *codec = rt5631_codec;

	pwr_reg = snd_soc_read(codec, RT5631_INT_ST_IRQ_CTRL_2);

	if (pwr_reg & 0x0c)
		rt5631_hp_status = UNPLUG;
	else
		rt5631_hp_status = PLUG;

	switch_set_state(&rt5631_hp_dev, rt5631_hp_status);
}

static void rt5631_hp_timer_func(unsigned long x)
{
	struct rt5631_priv *rt5631 = 0;
	rt5631 = snd_soc_codec_get_drvdata(rt5631_codec);
	schedule_work(&rt5631->hpswitchwork);

	mod_timer(&rt5631_hp_timer,
...
static int rt5631_codec_reinit(struct snd_soc_codec *codec)
{
	int iface = 0;
	...
	snd_soc_update_bits(codec, RT5631_PWR_MANAG_ADD3,
			RT5631_PWR_FAST_VREF_CTRL,
			RT5631_PWR_FAST_VREF_CTRL);
	snd_soc_write(codec, RT5631_PWR_MANAG_ADD1, 0x93e0);
	snd_soc_write(codec, RT5631_PWR_MANAG_ADD2, 0xf028);
	snd_soc_write(codec, RT5631_PWR_MANAG_ADD3, 0xe01e);
	snd_soc_write(codec, RT5631_PWR_MANAG_ADD4, 0xcc00);

//	iface = RT5631_SDP_MODE_SEL_MASTER;
//	snd_soc_write(codec, RT5631_SDP_CTRL, iface);
	snd_soc_update_bits(codec, RT5631_SDP_CTRL, RT5631_SDP_MODE_SEL_MASK, RT5631_SDP_MODE_SEL_MASTER);
	snd_soc_update_bits(codec, RT5631_SDP_CTRL, RT5631_SDP_I2S_DL_MASK, 0);
	snd_soc_update_bits(codec, RT5631_STEREO_AD_DA_CLK_CTRL,
			RT5631_I2S_PRE_DIV_MASK | RT5631_I2S_LRCK_SEL_N_BCLK_MASK | RT5631_ADDA_FILTER_CLK_SEL_MASK,
			0x1000);
	...
	/* for record */
	snd_soc_write(codec, RT5631_ADC_REC_MIXER, 0xb0f0);
	snd_soc_write(codec, RT5631_MIC_CTRL_2, 0x6000);
	snd_soc_write(codec, RT5631_PWR_MANAG_ADD1, 0x9fe0);
	snd_soc_write(codec, RT5631_PWR_MANAG_ADD2, 0xfc28);

	return 0;
}

static ssize_t rt5631_fpga_spec_config(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	rt5631_codec_reinit(rt5631_codec);
	return 0;
}
static DEVICE_ATTR(fpga_conf, 0444, rt5631_fpga_spec_config, NULL);
#endif
...
{
	struct rt5631_priv *rt5631 = snd_soc_codec_get_drvdata(codec);
	struct codec_cfg *cfg = rt5631->cfg;
...
#if defined(CONFIG_APOLLO_FPGA_PLATFORM) || defined(CONFIG_Q3F_FPGA_PLATFORM) || defined(CONFIG_APOLLO3_FPGA_PLATFORM) || defined(CONFIG_CORONAMPW_FPGA_PLATFORM)
	ret = device_create_file(codec->dev, &dev_attr_fpga_conf);
	if (ret < 0) {
		dev_err(codec->dev,
			"Failed to create fpga_conf sysfs files: %d\n", ret);
	}
#endif
	/*
	 * jason & chuanjin 2012-10-10 for nanmo request :
	 * disable protect function,[nanmo MID 0.5% speaker  NG  issue
	 */
	snd_soc_write(codec, 0x48, 0xf428);

	/*add timer to read the status of hp*/
	setup_timer(&rt5631_hp_timer, rt5631_hp_timer_func, 0);
	mod_timer(&rt5631_hp_timer,
			jiffies + msecs_to_jiffies(1000));
...
#if defined(CONFIG_APOLLO_FPGA_PLATFORM) || defined(CONFIG_Q3F_FPGA_PLATFORM) || defined(CONFIG_APOLLO3_FPGA_PLATFORM)||defined(CONFIG_CORONAMPW_FPGA_PLATFORM)
	device_remove_file(codec->dev, &dev_attr_fpga_conf);
#endif
		...
		const struct i2c_device_id *id)
{
	...
	struct codec_cfg *rt5631_cfg;
	int ret;

	rt5631_cfg = i2c->dev.platform_data;
	if (!rt5631_cfg) {
		dev_err(&i2c->dev, "rt5631 get platform_data failed\n");
		return -EINVAL;
	}
	if (rt5631_cfg->hp_io < 0) {
		rt5631_hp_dev.name = "h2w";
		switch_dev_register(&rt5631_hp_dev);
	}

	rt5631 = kzalloc(sizeof(struct rt5631_priv), GFP_KERNEL);
	if (NULL == rt5631) {
		dev_err(&i2c->dev, "mem alloc for rt5631 failed\n");
		return -ENOMEM;
	}
	INIT_WORK(&rt5631->hpswitchwork, hp_switch_work);
	rt5631->cfg = rt5631_cfg;
	msleep(100);
	...
	if (ret < 0) {
		dev_err(&i2c->dev, "register rt5631 codec err: %d\n", ret);
		return ret;
	}
	return 0;
}
...
static void __exit rt5631_exit(void)
{
	i2c_del_driver(&rt5631_i2c_driver);
}
module_exit(rt5631_exit);
