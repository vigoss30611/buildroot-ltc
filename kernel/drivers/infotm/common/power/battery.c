...
int batt_init_adc(struct platform_device *pdev)
{
	int ret  = 0;
	struct adc_request *adc_req;
	struct battery_cfg *pdata;

	if(pdev && pdev->dev.platform_data) {
		pdata = pdev->dev.platform_data;
	} else {
		pr_err("pdev == NULL || pdev->dev.platform_data, batt init failed!\n");
		return -EINVAL;
	}

	adc_req = kzalloc(sizeof(struct adc_request), GFP_KERNEL);
	if(!adc_req) {
		pr_err("%s can't alloc adc_request\n", __func__);
		return -EINVAL;
	}

	adc_req->id = pdata->adc_chan;
	adc_req->mode = ADC_DEV_SAMPLE;
	adc_req->label = "imap-adc";

	//adc_req.label = pdata->model;
	batt_info.batt_adc = adc_request(adc_req);
	if(!batt_info.batt_adc || IS_ERR(batt_info.batt_adc)) {
		pr_err("can't request adc channel%d for battery\n", adc_req->id);
		batt_info.batt_adc = NULL;
		batt_info.batt_adc_ratio = 0;
		//batt_info.batt_adc_ref = 0;
		//batt_info.batt_adc_bits = 0;
		ret = -EINVAL;
		goto err;
	}
	batt_info.batt_adc_ratio = pdata->divider_ratio;
	//batt_info.batt_adc_ref = pdata->ref_volt;
	//batt_info.batt_adc_bits = pdata->adc_bits;
err:
	kfree(adc_req);
	return ret;
}

void batt_deinit_adc(void) {

	if(!batt_info.batt_adc || IS_ERR((const void *)batt_info.batt_adc)) {
		return;
	}

	adc_free(batt_info.batt_adc);
	return;
}

unsigned int batt_get_adc_val(void) {
	unsigned int val, val1; 
	int ret;

	if(!batt_info.batt_adc)
		return 0;

	adc_enable(batt_info.batt_adc);
	ret = adc_read_raw(batt_info.batt_adc, &val, &val1);
	adc_disable(batt_info.batt_adc);
	if(ret != ADC_VAL_INT) {
		//something wrong here
		pr_err("read adc fail!\n");
		return 0;
	}
	else {
...
static int g_charger_on = -1;
void batt_set_charging_status(int batt_status)
{
    g_charger_on = batt_status;
    printk("batt_set_charging_status, status: %d\n", batt_status);
...
{
	if (g_charger_on == POWER_SUPPLY_STATUS_CHONNGDIANBAO_CHARGING || 
            g_charger_on == POWER_SUPPLY_STATUS_CHARGING)
		return 1;
	else 		
		return 0;
}
...
static int imap_batt_chg_batt_isfull(void)
{
	...
	int abs_index;
	int pin_val;
#ifdef CONFIG_INFT_HAS_NO_BATTERY
    return 1;
#endif
...
#endif	
	{
		...
		if(item_exist("power.full"))
		{
			...
			if(pin_val > 0)
			{
				...
				if(batt_info.batt_voltage > 4100)
					return 1;
				else
					return 0;
			}
...
//		imapx_pad_set_pull(abs_index, 0, 1);
	...
	if(item_exist("power.dcdetect")){
		...
		if (gpio_is_valid(abs_index)) {
			ret = gpio_request(abs_index, "dc-detect");
			if (ret) {
				pr_err("failed request gpio for dc-detect\n");
				return;
			}
		}
		gpio_direction_input(abs_index);
...
{
	int voltage;
	long long long_long_adc_val = adc_val;
	BATT_DBG("%s, adc_val =%d, batt_adc_ratio = %d\n", __func__, adc_val, batt_info.batt_adc_ratio);
	 
	/* unit : mV */
#if 0
	if(batt_info.batt_adc_bits > 0 && batt_info.batt_adc_ratio > 0 && batt_info.batt_adc_ref > 0) {
		/* voltage(mV) = ((adc_val / 1<<adc_bits) * ref_volt(mV)) * (divider_ratio / 1000) */
		/* in which divider_ratio = ( batt_volt / adc_sample_volt ) * 1000 */
		voltage = (int)((long_long_adc_val * batt_info.batt_adc_ratio * batt_info.batt_adc_ref)>>(batt_info.batt_adc_bits) / 1000); 
	}
#endif
	if(batt_info.batt_adc_ratio > 0) {
		voltage = (int)((long)((long_long_adc_val * batt_info.batt_adc_ratio * ADC_REF)>>(ADC_BITS)) / 1000); 
	}
...
static void imap_battery_mon_update(void)
{
	...
	batt_val_temp = imap_get_batt_v(adc_temp);
...
static void imap_battery_get_first_voltage(void)
{
	...
	if (batt_ave <= 3500 && !isacon()) {
		...
		else if(!strcmp(imapx_pmu_cfg.name,"ip6205") || !strcmp(imapx_pmu_cfg.name,"ip6208")) {
			ip620x_deep_sleep();
		}
		while(1);
	}
...
static void imap_battery_get_initialization(void)
{
	...
	batt_info.cap_timer_cnt_max = 5;// 2 sec = 10 * 2 * 0.5 sec
	batt_info.cap_timer_cnt_min = 1;
	batt_info.cap_timer_cnt_design = batt_info.cap_timer_cnt_max;
	batt_info.cap_cnt = 0;
	batt_info.batt_update_flag = 1;
	batt_info.batt_low_flag = 0;
...
int imap_battery_pcbtest_get_voltage(void)
{
    ...
    bat_pcbtest_voltage = imap_get_batt_v(adc_pcb);
...
static int imap_battery_probe(struct platform_device *pdev)
{
	...
	if(ret = batt_init_adc(pdev))
		goto init_adc_failed;
