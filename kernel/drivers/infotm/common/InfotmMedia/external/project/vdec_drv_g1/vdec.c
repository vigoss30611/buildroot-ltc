...
#define VDEC_MODULE_NAME "vdec_g1"
...
static IM_RET dvfs_deinit(void)
{
    ...
    if(vpu_dvfs.mode == DVFS_AUTO) {
        ...
        dvfs_set_level(0, DVFS_LEVEL_MIN);
    }
...
static int vdec_probe(struct platform_device *pdev)
{
    ...
    // bus clock.
    vdec_param.bus_clock = clk_get_sys("imap-vdec", "imap-vdec");
...
#endif
#endif /*CONFIG_VDEC_DVFS_SUPPORT*/
    clk_set_rate(vdec_param.bus_clock, 460000000);
...
void vdec_module_enable(void)
{
    ...
    writel(0x1, IO_ADDRESS(SYSMGR_VDEC_BASE + 0x20));  //enable vdec mmu
...
void vdec_module_disable(void)
{
    ...
    writel(0x0, IO_ADDRESS(SYSMGR_VDEC_BASE + 0x20));   //disable vdec mmu
...
static int __init vdec_init(void)
{
    ...
    if(ret != 0) {
        DBGMSG(ERR, "add vdec device resource error \n");
        return ret;
    }
#if 0
    ...
    if(ret != 0) {
        DBGMSG(ERR, "register vdec device error \n");
        return ret;
    }
