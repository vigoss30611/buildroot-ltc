...
#ifdef G1G2_MUTEX
    if(fpriv->inst_state & USE_COWORK) {
        g1g2_cowork_release(USER_G1); 
    }
#endif 
    vdec_open_count--;
...
#endif
{
    ...
    switch(cmd)
    {
...
#endif
#ifdef G1G2_MUTEX
            if (g1g2_cowork_reserve(USER_G1) == USER_CHANGED) {
                vdec_module_reset();
                writel(0x1, vdec_param.g1g2_switch_virt_addr);	// g1g2 switch ,g1 enable
            }

            mutex_lock(&vdec_lock);
            fpriv->inst_state |= USE_COWORK;
            mutex_unlock(&vdec_lock);
#endif
            break;
        case VDEC_INSTANCE_UNLOCK:
            ...
            mutex_lock(&vdec_lock);
#ifdef G1G2_MUTEX
            if (g1g2_cowork_release(USER_G1) == 0) {
                //vdec_module_disable();
            } else {
                //printk("g1 is working, donot power down  \n");
            }
            fpriv->inst_state &= ~USE_COWORK;
#endif
#ifdef MMU_SUPPORT 
...
static void  g1_set_clk()
{
    struct clk * clk;
    int clkFre, err;
    clk = clk_get_sys("imap-vdec264-ctrl", "imap-vdec264-ctrl");
    ...
    clk = clk_get_sys("imap-vdec264", "imap-vdec264");
    err = clk_prepare_enable(clk);
    if (err) {
...
static void  g1_set_clk_x15()
{
    struct clk * clk;
    int clkFre, err;

    clk = clk_get_sys("imap-vdec", "imap-vdec");
    err = clk_prepare_enable(clk);
    if (err)
    {
        pr_err("imap-vdec264-ctrl clock prepare enable fail, error:%d!\n", err);
        clk_disable_unprepare(clk);
        clk_put(clk);
        return;
    }
...
void vdec_module_disable(void)
{
    ...
    if(g1g2_cowork_get_user() != USER_G2) { 
        //writel(0x0, vdec_param.mempool_virt_addr + 0xc820);	// mempool vdec mode.
        module_power_down_lite(SYSMGR_VDEC264_BASE);
    } else {
        module_power_down_lite(SYSMGR_VDEC264_BASE);
    }
...
static int __init vdec_init(void)
{
    ...
    if (item_exist("vdec.x15clk") && item_integer("vdec.x15clk", 1))
    {
        g1_set_clk_x15();
    }
    else
