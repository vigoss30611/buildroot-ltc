...
#define GSLX680_I2C_NAME 	"gslx681"
...
//	imapx_pad_set_outdat(0, 1, tsp_reset_index);/*output 1*/
//	msleep(20);
...
static int gsl_ts_write(struct i2c_client *client, u8 addr, u8 *pdata, int datalen)
{
	...
	if (datalen > 125)
	{
		printk("[gslX681] %s too big datalen = %d!\n", __func__, datalen);
		return -1;
	}
...
static int gsl_ts_read(struct i2c_client *client, u8 addr, u8 *pdata, unsigned int datalen)
{
	...
	if (ret < 0)
	{
		printk("[gslX681] %s set data address fail!\n", __func__);
		return ret;
	}
...
//	u32 flag_printk=console_loglevel;
	...
	print_info("[gslX681]=============gsl_load_fw start==============\n");
//	console_loglevel = -1;
...
//	console_loglevel = flag_printk;
	print_info("[gslX681]=============gsl_load_fw end==============\n");
...
static int test_i2c(struct i2c_client *client)
{
	...
	if  (ret  < 0)  
		...
		print_info("[gslX681] I read reg 0xf0 is %x\n", read_buf);
	
	msleep(2);
	ret = gsl_ts_write(client, 0xf0, &write_buf, sizeof(write_buf));
	if(ret  >=  0 )
		print_info("[gslX681] I write reg 0xf0 0x12\n");
	...
	if(ret <  0 )
		...
		print_info("[gslX681] I read reg 0xf0 is 0x%x\n", read_buf);
...
#endif
	ret = gsl_ts_write(client, 0xe0, &tmp, 1);
    if (ret < 0)
        printk("startup GSL chip write error\n");
	msleep(10);	
}
...
static void init_chip(struct i2c_client *client)
{
	...
	if(rc < 0)
	{
		printk("[gslX681] ------gslX680 test_i2c error------\n");	
		return;
	}	
	clr_reg(client);
	reset_chip(client);
...
static void check_mem_data(struct i2c_client *client)
{
	...
	{
		printk("[gslX681]#########check mem read 0xb0 = %x %x %x %x #########\n", read_buf[3], read_buf[2], read_buf[1], read_buf[0]);
		init_chip(client);
	}
}
...
static int gsl_config_write_proc(struct file *file, const char *buffer, unsigned long count, void *data)
{
	...
	if(count > CONFIG_LEN)
	{
		printk("[gslX681] size not match [%d:%ld]\n", CONFIG_LEN, count);
        	return -EFAULT;
	}
	
	if(copy_from_user(gsl_read, buffer, (count<CONFIG_LEN?count:CONFIG_LEN)))
	{
		printk("[gslX681] copy from user fail\n");
        	return -EFAULT;
	}
	print_info("[gslX681][%s][%s]\n",__func__,gsl_read);
...
static void show_touch_cinfo(struct gsl_touch_info *cinfo){
	int i = 0;
	for(i =0;i <cinfo->finger_num;i++){
...
static void gslX680_ts_worker(struct work_struct *work)
{
	...
	if(tmp1>0&&tmp1<0xffffffff)
	{
		...
		print_info("[gslX681] tmp1=%08x,buf[0]=%02x,buf[1]=%02x,buf[2]=%02x,buf[3]=%02x\n",
...
queue_monitor_work:
	if(gsl_suspend_flag == 0)	
		queue_delayed_work(gsl_monitor_workqueue, &gsl_monitor_work, 100);
}
#endif
//static void rise_power(struct i2c_client *client)
static void reduce_power(struct i2c_client *client)
{
	u8 buf[4] = {0};
	/*0XFF0700DC Ð´1*/
#if 0
	...
	buf[2] = 0;
	buf[3] = 0;
	gsl_ts_write(client,0x5c,buf,4);

	/*0XFF080000 ×îºóÒ»¸öbitÐ´1.£¨½«csensorÍ£µô¡££©*/
	buf[0] = 1;
	buf[1] = 0xfe;
	buf[2] = 0x10;
	buf[3] = 0;
	gsl_ts_write(client,0xf0,buf,4);
	gsl_ts_read(client,0x0,buf,4);
	buf[0] |= 0x1;
	gsl_ts_write(client,0x0,buf,4);
#endif
	/*0XE4 Ð´B5*/
	buf[0] = 0xb5;
	buf[1] = 0;
	...
	gsl_ts_read(client,0xe4,buf,4);
	print_info("[gslX681]######## e4 = %x %x %x %x #########\n", buf[3], buf[2], buf[1], buf[0]);
}
//static void reduce_power(struct i2c_client *client)
static void rise_power(struct i2c_client *client)
{
	...
	/*0XFF080000 ×îºóÒ»¸öbitÐ´0.£¨½«csensorÍ£µô¡££©*/
	buf[0] = 1;
	buf[1] = 0xfe;
	buf[2] = 0x10;
	buf[3] = 0;
...
//		ret = imapx_pad_irq_clear(tsp_irq_index);
//		if (ret) {
//			printk("imapx pad irq clear error %d\n", ret);
//			return IRQ_HANDLED;
//		}
//	}
//	else
//		printk("[GSLX681] Error: No irq pending.\n");
...
//	imapx_pad_irq_config(tsp_irq_index, INTTYPE_RISING, FILTER_MAX);/* set trigger mode and filter */
	...
	input_mt_init_slots(input_device, (MAX_CONTACTS+1), INPUT_MT_DIRECT);
#else
...
static int gsl_ts_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	int i;
    printk("[gslX681] I am in gsl_ts_suspend\n");
	reset_chip(ts->client);
#ifdef GSL_MONITOR
	gsl_suspend_flag = 1;
	printk( "[gslX681] gsl_ts_suspend () : cancel gsl_monitor_work\n");
...
{
	//struct gsl_ts *ts = container_of(cl, struct gsl_ts, client);
	struct gsl_ts *ts = i2c_get_clientdata(client);
	int i;
...
//    printk("gsl_ts_resume () set tsp irq index\n");
...
//	printk( "gsl_ts_resume () : queue gsl_monitor_work\n");
  ...
  //	printk("I'am in gsl_ts_resume() finish\n");
	return 0;
}
...
//	_tsp_irq_num = imapx_pad_irq_number(tsp_irq_index);/* get irq */
	_tsp_irq_num = gpio_to_irq(tsp_irq_index);/* get irq */
	print_info("[gslX681]ts.int is %d, pad irq %d, ts.reset is %d\n",tsp_irq_index,_tsp_irq_num, tsp_reset_index);	
	...
	rc=  request_irq(_tsp_irq_num, gsl_ts_irq, IRQF_TRIGGER_RISING, client->name, ts);
	if (rc < 0) {
		printk( "[gslX681]gsl_probe: request irq failed %d\n", rc);
...
{
	struct gsl_ts *ts = i2c_get_clientdata(client);
	print_info("[gslX681]==gsl_ts_remove=\n");
...
static int __init gsl_ts_init(void)
{
    ...
    printk(KERN_INFO "[gslX681]-Enter %s\n", __func__);

   	if(item_exist("ts.model"))
	{
		if(item_equal("ts.model", GSLX680_I2C_NAME, 0))
		{
    			...
    			if (!adapter) {
	    			printk("[gslX681]****** get_adapter error! ******\n"); 
    			}
    			client = i2c_new_device(adapter, &info);

    			ret = i2c_add_driver(&gsl_ts_driver);   
    			printk(KERN_INFO "[gslX681]-i2c_add_driver return %d\n",ret);
    			return ret;
		}
		else
			printk(KERN_INFO "%stouchscreen is not %s\n", __func__,GSLX680_I2C_NAME);
	}
	else
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("GSLX681 touchscreen controller driver");
