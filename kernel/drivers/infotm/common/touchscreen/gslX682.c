...
#define GSLX680_I2C_NAME 	"gslX682"
...
int gslx680_i2c_test(void)
{
    int ret;
    ret = test_i2c(gsl_client);
    if (ret < 0)
        return -1;

    return 0;
}
EXPORT_SYMBOL(gslx680_i2c_test);

static int gslX680_init(void)
{	
	print_info("gslx682: %s\n", __func__);
...
static ssize_t gslx682_reg_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    int i, ret = 0;
    u32 read_buf = 0;
    printk("Start show gslx682 register\n");
    for (i = 0; i < ARRAY_SIZE(show_reg); i++)
    {
        ret = gsl_ts_read(gsl_client, show_reg[i], &read_buf, sizeof(read_buf));
        if (ret < 0)
            printk("I read reg 0x%x failed\n", show_reg[i]);
        else
            printk("0x%x : 0x%x\n", show_reg[i], read_buf);
    }
...
static DEVICE_ATTR(gslx682_reg, 0444, gslx682_reg_show, NULL);

static struct attribute *gslx682_attribute[] = {
    &dev_attr_gslx682_reg.attr,
    NULL
};

static const struct attribute_group gslx682_attr_group = {
    .attrs = gslx682_attribute,
};
...
/*	if(imapx_pad_irq_pending(tsp_irq_index)) {
		...
		printk("[GSLX682] Error: No irq pending.\n");
*/
			...
			const struct i2c_device_id *id)
{
	...
	printk(KERN_INFO "GSLX682 Enter %s\n", __func__);
...
//	_tsp_irq_num = imapx_pad_irq_number(tsp_irq_index);/* get irq */
	...
	print_info("ts.int is %d, pad irq %d, ts.reset is %d\n",tsp_irq_index,_tsp_irq_num, tsp_reset_index);	
	...
	if (rc < 0) {
		printk( "gsl_probe: request irq failed %d\n", rc);
...
*/
    ...
    int ret = 0;
    ret = sysfs_create_group(&ts->input->dev.kobj, &gslx682_attr_group);
    if (ret)
        printk("Creat gslx682 file failed\n");
...
static int __exit gsl_ts_remove(struct i2c_client *client)
{
	...
	free_irq(ts->irq, ts);
	destroy_workqueue(ts->wq);
    sysfs_remove_group(&ts->input->dev.kobj, &gslx682_attr_group);
...
static int __init gsl_ts_init(void)
{
   	...
   	if(item_exist("ts.model"))
	{
		if(item_equal("ts.model", GSLX680_I2C_NAME, 0))
		{
    			...
    			printk("[gslX682]-i2c_add_driver return %d\n",ret);
    			return ret;
		}
		else
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("GSLX682 touchscreen controller driver");
