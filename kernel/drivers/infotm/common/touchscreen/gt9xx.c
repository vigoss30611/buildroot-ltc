...
{
    // adjust x fengwu 0425
    x = 480-x;
    printk("ID:%d, adjust X:%d, Y:%d, W:%d\n", id, x, y, w);
...
static int gsl_ts_write(struct i2c_client *client, u8 addr, u8 *pdata, int datalen)
{
	...
	if (datalen > 125)
	{
		printk("[xxxxxxxx] %s too big datalen = %d!\n", __func__, datalen);
		return -1;
	}
...
static int gsl_ts_read(struct i2c_client *client, u8 addr, u8 *pdata, unsigned int datalen)
{
	...
	if (ret < 0)
	{
		printk("[xxxxxxxx] %s set data address fail!\n", __func__);
		return ret;
	}
...
static int test_i2c(struct i2c_client *client)
{
	...
	if  (ret  < 0)  
		...
		printk("[xxxxxxxx] I read reg 0xf0 is %x\n", read_buf);

	msleep(2);
	ret = gsl_ts_write(client, 0xf0, &write_buf, sizeof(write_buf));
	if(ret  >=  0 )
		printk("[xxxxxxxx] I write reg 0xf0 0x12\n");
	...
	if(ret <  0 )
		...
		printk("[xxxxxxxx] I read reg 0xf0 is 0x%x\n", read_buf);
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data * ts)
{
    ...
    while(retry++ < 10)
    {
        ...
        ret = test_i2c(ts->client);//gtp_i2c_test(ts->client);
        if (ret > 0)
        {
            GTP_INFO("GTP wakeup sleep.");
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
    ...
    /////////power/////linyun
    ret = GTP_GPIO_REQUEST(GTP_POWER_PORT, "GTP_POWER_PORT");
    //if (ret < 0)
    //{
    //    GTP_ERROR("Failed to request GPIO:%d, ERRNO:%d",(s32)GTP_RST_PORT,ret);
    //    ret = -ENODEV;
    //}
    GTP_GPIO_OUTPUT(GTP_POWER_PORT, 0);
	msleep(50);
    gpio_set_value(GTP_POWER_PORT, 0);
	msleep(50);
	gpio_set_value(GTP_POWER_PORT, 1);
	msleep(100);
	GTP_ERROR("gtp_request_io_port");
    /////////////////////
    ...
    //}
	GTP_ERROR("gtp_request_io_port1");
	msleep(10);
    ...
    //    ret = -ENODEV;
    //}
    GTP_ERROR("gtp_request_io_port22");
		
    GTP_GPIO_AS_INPUT(GTP_RST_PORT);
    GTP_ERROR("gtp_request_io_port2 ret=%d",ret);
   ...
   GTP_ERROR("gtp_request_io_port3");
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(GTP_RST_PORT)) {
		rc = gpio_request(GTP_RST_PORT, "ts_int");
		if (rc) {
...
;
	if (gpio_is_valid(GTP_INT_PORT)) {
		rc = gpio_request(GTP_INT_PORT, "ts_reset");
		if (rc) {
...
#endif
    ret =  test_i2c(ts->client); //gtp_i2c_test(client);
    if (ret < 0)
    {
...
static int __init goodix_ts_init(void)
{
	...
	{
		if(item_equal("ts.model", "gt9147", 0))
		{

                 GTP_DEBUG_FUNC();   
				    GTP_INFO("GTP driver installing...GTP_INT_PORT=%d",GTP_INT_PORT);
    			...
    			printk(KERN_INFO "[goodix_ts_init]-i2c_add_driver return %d\n",ret);
				//while (1);
    			return ret;
			
		}
		else
			printk(KERN_INFO "[goodix_ts_init] %s: touchscreen is not gt9147\n", __func__);
	}
	else
		printk(KERN_ERR "[goodix_ts_init]%s: touchscreen is not exist\n", __func__);
	return -1;
