...
static int module_attribute_iterate(struct list_head * head, int attr, void ** funcs)
{
	...
	list_for_each_entry(ms, head, serial) {
		ret = module_find_attribute(ms, attr, &func);
		if (ret) {
			funcs[i] = func;
			i++;
		}
		if (ms->parallel) {
			list_for_each_entry(mp, ms->parallel, parallel) {
				if (strcmp(mp, ms->parallel_select) == 0) {
					ret = module_find_attribute(mp, attr, &func);
					if (ret) {
						funcs[i] = func;
						i++;
					}
					if (strcmp(ms->name, "interface") == 0 && mp->serial) {
						ret = module_find_attribute(list_entry(mp->serial.next, struct module_node, serial), attr, &func);
						if (ret) {
							funcs[i] = func;
							i++;
						}
...
static int abstraction_set_attribute(struct abstract_terminal * terminal, int attr, int *param, int num)
{
	struct module_node * module;
	int idsx = terminal->idsx;
	struct list_head * head = &path[idsx];
	int ret;
...
static int find_terminal_attributes(struct module_node * terminal, struct abstract_terminal * ab_terminal)
{
	int idsx = terminal->idsx;
	struct list_head * head = &path[idsx];
...
static int find_terminals(struct module_node * intf, struct module_node ** terminals)
{
...
int abstraction_init(void)
{
	...
	for (idsx = 0; idsx < 2; idsx++) {
		if (find_interface(&path[idsx], &intf))
			return -1;
		path_terminal_num = find_terminals(intf, terminals);
		...
		for (i = 0; i < path_terminal_num; i++) {
			abstract_terminals[tn].name = terminals[i]->name;
			abstract_terminals[tn].idsx = terminals[i]->idsx;
			/* set attributes */
			find_terminal_attributes(terminals[i], &abstract_terminals[tn]);
			/* rgb seq */
			if (module_attr_find(terminals[i], ATTR_GET_RGB_SEQ, &m_attr))
				m_attr->func(terminals[i]->idsx, &abstract_terminals[tn].rgb_seq, 1);
			else
				abstract_terminals[tn].rgb_seq = SEQ_RGB;
			/* rgb bpp */
			if (module_attr_find(terminals[i], ATTR_GET_RGB_BPP, &m_attr))
				m_attr->func(terminals[i]->idsx, &abstract_terminals[tn].rgb_bpp, 1);
			else
				abstract_terminals[tn].rgb_bpp = RGB888;
			/* rgb bt656 */
			if (module_attr_find(terminals[i], ATTR_GET_BT656, &m_attr))
				m_attr->func(terminals[i]->idsx, &abstract_terminals[tn].bt656, 1);
			else
