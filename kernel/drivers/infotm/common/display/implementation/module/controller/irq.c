...
int ids_irq_initialize(int idsx)
{
	...
	if (!irq_init[idsx]) {
		ret = request_irq(irq, dss_irq_handler, IRQF_DISABLED, name, &idsx_priv[idsx]);	// IRQF_ONESHOT ?
		if (ret) {
...
irqreturn_t dss_irq_handler(int irq, void *data)
{
	...
	if (vclkfac) {
		...
		//dss_info("IDS%d: insufficient bandwidth. CDOWN = 0x%X, RFRM_NUM = 0x%X, CLKVAL = 0x%X\n", 
