...
struct module_node lcdc_module[2] = {
	{	.name = "lcdc",
...
static void set_emif_effic(int idsx, int en)
{
	uint32_t emif;
...
static int set_pixel_clk(int idsx)
{
	...
	if (gdtd[idsx].mPixelClock == 2700 || gdtd[idsx].mPixelClock == 2702)
		...
		for (i = 1; i < 8; i++) {
			...
			ret = clk_set_rate(ids0_eitf, reqclk * 1000);
			if (ret < 0) {
				dss_err("clk_set_rate(ids0_eitf, %d) failed\n", reqclk);
				break;
			}
			retclk = clk_get_rate(ids0_eitf);
			retclk/= 1000;
