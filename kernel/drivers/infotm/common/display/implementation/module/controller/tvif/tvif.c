...
static int set_rgb_seq(int idsx, int *params, int num);
...
//	{TRUE, ATTR_SET_RGB_SEQ, set_rgb_seq},
	{FALSE, ATTR_END, NULL},
};

struct module_node tvif_module[2] = {
	{	.name = "tvif",
...
static dtd_t gdtd[2];
static int rgb_seq[2];
static int rgb_seq_regval[2] = {0x06, 0x06};
...
static int set_vic(int idsx, int *params, int num)
{
	...
	/* DSTYPE: TV display */
	ids_write(idsx, LCDCON5, 11, 2, 0x2);

	/* IFTYPE: ITU-R BT601/BT656 */
	ids_write(idsx, OVCDCR, 1, 1, 0x1);
	/* Full VActive */
	if (dtd.mInterlaced)
	...
	printk("TVIF-BT656-idsx = %d \n",idsx);

	if (idsx == 1) {	// BT656
		dss_info("TVIF-BT656-%s vic=%d\n", vic ==3?"NTSC":(vic ==18?"PAL":"Invalid"),vic);

		/* TVIF: Interlace */
		ids_write(idsx, OVCDCR, 0, 1, 1);
		...
		printk("TVIF-BT656- TVCMCR = 0x%08x\n",reg_temp[2]);
		if (vic == 3) {		// NTSC
    ...
    dss_info("TVIF-BT656- Tvcmcr = 0x%08x\n",ids_readword(idsx,IMAP_TVCMCR));
