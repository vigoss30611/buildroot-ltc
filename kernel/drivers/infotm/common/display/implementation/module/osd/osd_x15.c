...
extern int hdmi_get_current_vic();
...
static int set_vic(int idsx, int *params, int num)
{
	...
	if (idsx == 0) {
		...
		ids_writeword(idsx, OVCW0PCBR,
				((width-1) << OVCWxPCBR_RightBotX) | ((height-1) << OVCWxPCBR_RightBotY));	
		ids_writeword(1, OVCW0VSSR, width);
		ids_writeword(1, OVCW0PCBR,
				((width-1) << OVCWxPCBR_RightBotX) | ((height-1) << OVCWxPCBR_RightBotY));	
	}
#endif
	...
	if (idsx == 1) {
		if((item_integer("hdmi.scale", 0) == 1)){
			...
			int scaled_width = dtd.mHActive & (~0x3);
			int scaled_height = dtd.mVActive & (~0x3);
			...
			int scale_w =1024*1024/scaled_width;
			int scale_h = 600*1024/scaled_height;
			...
			/*
			   for(i=0; i<6*64; i++){
			   ids_writeword(1, 0x5200+i*0x4, scl_params[i]);
			   }
			   for(i=0; i<8; i++){
			   ids_writeword(1, 0x5C08+i*0x4, 0x1FFFFFFF);
			   }
			   ids_write(1, 0x5C04, 0, 1, 0);
			   ids_write(1, 0x5C04, 2, 1, 1);
			   */
		}else {
			/* Update Overlay params. During dynamic changing, must update by write 1 to this bit */
