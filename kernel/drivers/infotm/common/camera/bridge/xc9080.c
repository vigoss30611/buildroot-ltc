...
static struct chip_resource *g_res = NULL;
static struct i2c_adapter *adapter = NULL;
static struct i2c_client *client = NULL;
static struct infotm_bridge_dev* dev = NULL;

static long xc9080_ioctl(struct file *flip, unsigned int ioctl_cmd,
	    unsigned long ioctl_param);
static int xc9080_open(struct inode *inode , struct file *filp);
static int xc9080_close(struct inode *inode , struct file *filp);

static ssize_t xc9080_show(struct device *dev, struct device_attribute *attr, char *buf);
static ssize_t xc9080_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size);

static int xc9080_bypass(struct chip_resource *res, int on_off);

static struct file_operations xc9080_fops = {
	.owner = THIS_MODULE,
	.open  = xc9080_open,
	.release = xc9080_close,
	.unlocked_ioctl = xc9080_ioctl,
};
static struct miscdevice xc9080_dev = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = XC9080_NAME,
	.fops = &xc9080_fops
};

DEVICE_ATTR(xc9080_bridge_info, 0444, xc9080_show, xc9080_store);
static ssize_t xc9080_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct sensor_info* info = g_res->info;
	return sprintf(buf, "i2c_addr=0x%x\n\nlanes=%d\n"
				"csi match freq=%d\noclk=%p\nreset_pin=%d\n",
				info->i2c_addr, info->lanes,
				info->csi_freq, info->oclk, info->reset_pin);
}
static ssize_t xc9080_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
...
static ssize_t xc9080_enable_show(struct class *class,
		struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", enable_status);
}

static void xc9080_enable(int mode)
{
	if (mode != enable_status) {
		if (mode == 1) {
			bridge_log(B_LOG_DEBUG, "enable=%d\n", mode);
			gpio_set_value(g_res->reset_pin, 0);
			mdelay(20);
			gpio_set_value(g_res->reset_pin, 1);
			mdelay(20);
			enable_status = 1;
		} else if (mode == 0) {
			bridge_log(B_LOG_DEBUG, "enable=%d\n", mode);
			...
			bridge_log(B_LOG_DEBUG, "enable=%d is novalid\n", mode);
		}
	}
}
static ssize_t xc9080_enable_store(struct class *class,
...
static struct class_attribute xc9080_class_attrs[] = {
	__ATTR(enable, 0644, xc9080_enable_show, xc9080_enable_store),
	__ATTR_NULL
};
static struct class xc9080_class = {
	.name = XC9080_NAME,
	.class_attrs = xc9080_class_attrs,
};

static int xc9080_i2c_single_read(struct chip_resource *res,
		const u16 addr, u8 *value)
{
	...
	bridge_log(B_LOG_DEBUG, "i2c_addr=0x%x reg=0x%x value=0x%x ret=%d\n",
		client->addr, addr, *value, retval);
	return retval;
}
static int xc9080_i2c_single_write(struct chip_resource *res,
...
static int xc9080_read_id(struct chip_resource *res)
{
	...
	bridge_log(B_LOG_INFO, "xc9080: ID-0x%X%X read-0x%X%X\n",
...
static int xc9080_init(struct _infotm_bridge* bridge, unsigned int mode)
{
	int ret = 0;
	struct chip_resource *res = NULL;
	struct sensor_info* info = NULL;

	bridge_log(B_LOG_DEBUG, "%s\n", __func__);
	...
	res->client = info->client;
	res->reset_pin = (info->reset_pin > 0) ? info->reset_pin : -(info->reset_pin); /* info->reset_pin; */
	res->info = info;
	g_res = res;

	bridge_log(B_LOG_DEBUG, "%s client=%p adapter=%p reset_pin=%d\n",
		__func__, res->client, adapter, res->reset_pin);

	gpio_set_value(res->reset_pin, 1);
	mdelay(30);

	class_register(&xc9080_class);
	enable_status = 1;

	ret = xc9080_read_id(res);
	if(ret) {
		bridge_log(B_LOG_ERROR, "xc9080 read id failed\n");
		return -1;
	}

	bridge_log(B_LOG_INFO, "xc9080 init successfully\n");
	return 0;
...
static int xc9080_exit(unsigned int mode)
{
	class_unregister(&xc9080_class);
	kfree(g_res->writebuf);
	kfree(g_res);

	bridge_log(B_LOG_INFO, "xc9080 driver removed and exit.\n");
	return 0;
}

static int xc9080_control(unsigned int cmd,unsigned int arg)
{
	static int binit = 0;
	static struct bridge_hw_cfg hw_cfg;
	...
	switch(cmd) {
	case BRIDGE_G_CFG:
		if (copy_to_user((void *)arg, &hw_cfg, sizeof(hw_cfg)))
			return -EFAULT;
		break;

	case BRIDGE_S_CFG:
		if (copy_from_user(&hw_cfg, (struct bridge_hw_cfg *)arg, sizeof(hw_cfg))) {
			bridge_log(B_LOG_ERROR, "BRIDGE_S_CFG copy from user error\n");
			return -EACCES;
		}
		bridge_log(B_LOG_DEBUG, "enable = %d\n", hw_cfg.enable);

		xc9080_enable(hw_cfg.enable);

		if (hw_cfg.mode == XC9080_KERNAL_INIT_MODE &&
			hw_cfg.enable && !binit) {
			bridge_log(B_LOG_INFO, "xc9080 kernel init mode \n");
			xc9080_init_sequence(g_res);
			binit = 1;
		}

		if (!hw_cfg.enable && binit)
			binit = 0;
...
static int __init xc9080_driver_init(void)
{
	int ret = 0;

	ret = misc_register(&xc9080_dev);
	if(ret) {
		goto err;
	}
	ret = device_create_file(xc9080_dev.this_device, &dev_attr_xc9080_bridge_info);
	if(ret) {
		goto err1;
	}
	...
	strncpy(dev->name, XC9080_NAME, MAX_STR_LEN);
	dev->init = xc9080_init;
	dev->exit = xc9080_exit;
	dev->control = xc9080_control;

	bridge_dev_register(dev);
	return ret;

err2:
	device_remove_file(xc9080_dev.this_device, &dev_attr_xc9080_bridge_info);
err1:
	misc_deregister(&xc9080_dev);
err:
	return -ENODEV;
}

static void __exit xc9080_driver_exit(void)
{
...
module_init(xc9080_driver_init);
module_exit(xc9080_driver_exit);
MODULE_AUTHOR("Davinci.Liang");
MODULE_DESCRIPTION("driver for xc9080 mipi bridge.");
