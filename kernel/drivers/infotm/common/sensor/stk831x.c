...
#define STK_TUNE_DELAY 125
		...
		#include "stk8313.h"//<linux/stk8313.h>
	#elif defined CONFIG_SENSORS_STK8312
		#include "stk8312.h"//<linux/stk8312.h>
	#else
...
#define STK8313_INPUT_DEV_NAME	"STK831x"
...
#define STK831X_INIT_ODR	   5//	3		//2//2:100Hz, 3:50Hz, 4:25Hz 5:12.5HZ 6:6.25HZ 7:3.125HZ
#define STK831X_SAMPLE_TIME_MIN_NO		2
#define STK831X_SAMPLE_TIME_NO		8
const static int STK831X_SAMPLE_TIME[STK831X_SAMPLE_TIME_NO] = {2500, 5000, 10000, 20000, 40000,80000,160000,320000};
static struct stk831x_data *stk831x_data_ptr;
static int event_since_en = 0;
static int event_since_en_limit = 20;
static int g_level_flag = 2;
#if (!STK_ACC_POLLING_MODE)
...
static int gsensor_fetch_sysconfig_para(void)
{
	...
	dprintk(DEBUG_INIT, "[stk831x]========%s===================\n", __func__);

	type = script_get_item("gsensor_para", "gsensor_used", &val);

	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		pr_err("[stk831x]%s: type err  device_used = %d. \n", __func__, val.val);
		goto script_get_err;
	}
	device_used = val.val;
	...
	if (1 == device_used) {
		...
		if(SCIRPT_ITEM_VALUE_TYPE_INT != type){
			pr_err("[stk831x]%s: type err twi_id = %d. \n", __func__, val.val);
			goto script_get_err;
		}
		twi_id = val.val;

		dprintk(DEBUG_INIT, "[stk831x]%s: twi_id is %d. \n", __func__, twi_id);

		if(SCIRPT_ITEM_VALUE_TYPE_INT != script_get_item("gsensor_para", "gsensor_direct_x", &val)){
			pr_err("[stk831x]%s: line: %d: script_get_item err. \n", __FILE__, __LINE__);
			goto script_get_err;
		}
		...
		pr_err("[stk831x]%s: gsensor_unused. \n",  __func__);
		ret = -1;
	}
...
static int STK_i2c_Rx(char *rxData, int length)
{
	...
	if (retry > 3)
	{
		printk(KERN_ERR "[stk831x]%s: i2c error, retry over 3\n", __func__);
		return -EIO;
	}
	else
		return 0;
...
int stk831x_i2c_test(void)
{
    int ret;
    char buffer[0] = "";
    buffer[0] = STK831X_XOUT;
    ret = STK_i2c_Rx(buffer, 1);    
    if (ret < 0)
        return -1;

    return 0;
}
EXPORT_SYMBOL(stk831x_i2c_test);
...
static int STK831X_SetVD(struct stk831x_data *stk)
{
	...
	if(result < 0)
	{
		printk(KERN_ERR "[stk831x]%s: read back error, result=%d\n", __func__, result);
		return result;
	}
	...
	if(reg24 != 0)
	{
		buffer[0] = 0x24;
		buffer[1] = reg24;
		printk(KERN_INFO "[stk831x]%s:write 0x%x to 0x24\n",  __func__, buffer[1]);
		result = STK_i2c_Tx(buffer, 2);
		if (result < 0)
		{
			printk(KERN_ERR "[stk831x]%s:failed\n", __func__);
			return result;
		}
	}
	else
	{
		printk(KERN_INFO "[stk831x]%s: reg24=0, do nothing\n", __func__);
		return 0;
	}
	...
	if(buffer[0] != reg24)
	{
		printk(KERN_ERR "[stk831x]%s: error, reg24=0x%x, read=0x%x\n", __func__, reg24, buffer[0]);
		return -1;
	}
	printk(KERN_INFO "[stk831x]%s: successfully", __func__);
	return 0;
}
	...
	if (stk_tune_done==0)
	{
		if( event_since_en >= STK_TUNE_DELAY)
		{
			...
			if(stk_tune_index == STK_TUNE_NUM)
			{
				...
				if (result < 0)
				{
					printk(KERN_ERR "[stk831x]%s:failed, result=0x%x\n", __func__, result);
					return;
				}
				mode_reg = buffer[0];
				...
				event_since_en = 0;
				printk(KERN_INFO "[stk831x]%s:TUNE done, %d,%d,%d\n", __func__,
...
static int STK831x_CheckReading(int acc[], bool clear)
{
	...
	if(acc[0] == 127 || acc[0] == -128 || acc[1] == 127 || acc[1] == -128 ||
			acc[2] == 127 || acc[2] == -128)
	{
		printk(KERN_INFO "[stk831x]%s: acc:%o,%o,%o\n", __func__, acc[0], acc[1], acc[2]);
		check_result++;
	}
	if(clear)
	{
		if(check_result == 3)
		{
			...
			printk(KERN_INFO "[stk831x]%s: incorrect reading\n", __func__);
			check_result = 0;
			return 1;
		}
		check_result = 0;
...
static int STK831x_ReadSensorData(struct stk831x_data *stk)
{
	...
	if (result < 0)
	{
		printk(KERN_ERR "[stk831x]%s:i2c transfer error\n", __func__);
		return result;
	}
	...
	//printk(KERN_INFO "[stk831x]%s:RAW  %4d,%4d,%4d\n", __func__, acc_xyz[0],	acc_xyz[1],
	//		acc_xyz[2]);
#endif
...
#ifdef STK_DEBUG_RAWDATA
	//printk(KERN_INFO "[stk831x]%s:After FIR  %4d,%4d,%4d\n", __func__, acc_xyz[0],
	//	acc_xyz[1], acc_xyz[2]);
#endif
...
//	printk(KERN_INFO "[stk831x]%s:%4d,%4d,%4d\n", __func__, stk->raw_data[0],
...
/*
	...
	count++;
	if(count%2 == 0){
		input_report_key(stk->input_dev, KEY_SPORT, 1);
	}else{
		input_report_key(stk->input_dev, KEY_SPORT, 0);
	}
	input_sync(stk->input_dev);
...
static int STK831x_SetEnable(struct stk831x_data *stk, char en)
{
	...
	gSensorEnable = atomic_read(&stk->enabled);
	return 0;
...
#ifdef STK_DEBUG_PRINT
#ifdef STK831X_HOLD_ODR
	printk(KERN_INFO "[stk831x]%s:sdelay_us=%d, Hold delay = %d\n", __func__, sdelay_us, STK831X_SAMPLE_TIME[STK831X_INIT_ODR]);
#else
	printk(KERN_INFO "[stk831x]%s:sdelay_us=%d\n", __func__, sdelay_us);
#endif
#endif
	mutex_lock(&stk->write_lock);
	if(stk->delay == sr_no)
...
static int STK831x_SetRange(char srange)
{
	...
	char buffer[2] = "";
#ifdef STK_DEBUG_PRINT
	printk(KERN_INFO "[stk831x]%s:range=0x%x\n", __func__, srange);
#endif

	if(srange >= 3)
	{
		printk(KERN_ERR "[stk831x]%s:parameter out of range\n", __func__);
		return -1;
	}
...
static int STK831x_ReadByteOTP(char rReg, char *value)
{
	...
	*value = 0;
	...
	if(redo == 10)
	{
		printk(KERN_ERR "[stk831x]%s:OTP read repeat read 10 times! Failed!\n", __func__);
	...
	*value = buffer[0];
	...
	printk(KERN_INFO "[stk831x]%s: read 0x%x=0x%x\n", __func__, rReg, *value);
#endif
	return 0;
...
static int STK831x_WriteByteOTP(char wReg, char value)
{
	...
	do
	{
		...
		if (result < 0)
		{
			printk(KERN_ERR "[stk831x]%s:failed, err=0x%x\n", __func__, result);
			goto eng_i2c_w_err;
		}
		buffer[0] = 0x3E;
		buffer[1] = value_xor;
		...
		do
		{
			...
			if(buffer[0]& 0x80)
			{
				...
				{
#ifdef STK_DEBUG_CALI
					printk(KERN_INFO "[stk831x]%s: write 0x%x=0x%x successfully\n", __func__, wReg, value);
#endif
					re_write = 0xFF;
					break;
				}
				...
				{
					printk(KERN_ERR "[stk831x]%s: write 0x%x=0x%x, read 0x%x=0x%x, try again\n", __func__, wReg, value_xor, wReg, read_back);
	...
	if(re_write == 10)
	{
		printk(KERN_ERR "[stk831x]%s: write 0x%x fail, read=0x%x, write=0x%x, target=0x%x\n", __func__, wReg, read_back, value_xor, value);
		return -STK_K_FAIL_OTP_5T;
	}
...
static int STK831x_WriteOffsetOTP(struct stk831x_data *stk, int FT, char offsetData[])
{
	...
	if(FT==1)
	{
		...
		if(regR[0]&0x10)
		{
			printk(KERN_ERR "[stk831x]%s: 0x7F=0x%x\n", __func__, regR[0]);
	...
	{
		printk(KERN_INFO "[stk831x]%s: ft pre-trimmed\n", __func__);
		ft_pre_trim = 1;
	}
...
#ifdef STK_DEBUG_CALI
	printk(KERN_INFO "[stk831x]%s:OTP step1 Success!\n", __func__);
#endif
	buffer[0] = 0x2A;
	result = STK_i2c_Rx(buffer, 1);
	if (result < 0)
...
#ifdef STK_DEBUG_CALI
	printk(KERN_INFO "[stk831x]%s:OTP step2 Success!\n", __func__);
#endif
	result = STK831x_ReadByteOTP(0x7F, &regR[0]);
	if(result < 0)
		goto eng_i2c_err;
	...
	if(result < 0)
		goto eng_i2c_err;
#ifdef STK_DEBUG_CALI
	printk(KERN_INFO "[stk831x]%s:OTP step3 Success!\n", __func__);
#endif
	return 0;

eng_i2c_err:
	printk(KERN_ERR "[stk831x]%s: read/write eng i2c error, result=0x%x\n", __func__, result);
	return result;

common_i2c_error:
	printk(KERN_ERR "[stk831x]%s: read/write common i2c error, result=0x%x\n", __func__, result);
	return result;
}
...
static int STK831x_SetCali(struct stk831x_data *stk, char sstate)
{
	...
	printk(KERN_INFO "[stk831x]%s: delay =%d ms\n", __func__, real_delay_ms);
...
#ifdef STK_DEBUG_CALI
	printk(KERN_INFO "[stk831x]%s:stk831x_placement=%d\n", __func__, stk831x_placement);
#endif
	...
	if(store_location == 1)
	{
		...
		for(axis=0;axis<3;axis++)
		{
			...
			{
				printk(KERN_ERR "[stk831x]%s: set offset to register fail!, char_offset[%d]=%d,reg_offset[%d]=%d\n",
		...
		if(result)
		{
			printk(KERN_ERR "[stk831x]%s: calibration check fail, result=0x%x\n", __func__, result);
		...
		else
		{
			...
			if(result)
			{
				printk(KERN_INFO "[stk831x]%s:write calibration failed\n", __func__);
				atomic_set(&stk->cali_status, -result);
			}
			else
			{
				printk(KERN_INFO "[stk831x]%s successfully\n", __func__);
	...
	else if(store_location >= 2)
	{
		for(axis=0; axis<3; axis++)
		{
			...
			if( (char_offset[axis]>>7)==0)
			{
				...
				{
					printk(KERN_ERR "[stk831x]%s: offset[%d]=0x%x is too large, limit to 0x1f\n",
			...
			else
			{
				...
				{
					printk(KERN_ERR "[stk831x]%s: offset[%d]=0x%x is too large, limit to 0x20\n",
									__func__, axis, char_offset[axis]);
		...
		printk(KERN_INFO "[stk831x]%s: OTP offset:0x%x,0x%x,0x%x\n", __func__, char_offset[0], char_offset[1], char_offset[2]);
		if(store_location == 2)
		{
			...
			{
				printk(KERN_ERR "[stk831x]%s fail, result=%d\n", __func__, result);
			}
		}
...
err_i2c_rw:
	...
	if(org_enable)
		STK831x_SetEnable(stk, 1);
	printk(KERN_ERR "[stk831x]%s: i2c read/write error, err=0x%x\n", __func__, result);
...
static int STK831x_GetCali(struct stk831x_data *stk)
{
...
#ifdef STK_TUNE
	printk(KERN_INFO "[stk831x]%s: stk_tune_done=%d, stk_tune_index=%d, stk_tune_offset=%d,%d,%d\n", __func__,
	...
	if ((stk_get_file_content(r_buf, STK_ACC_CALI_FILE_SIZE)) == 0)
	{
		if(r_buf[0] == STK_ACC_CALI_VER0 && r_buf[1] == STK_ACC_CALI_VER1)
		{
			...
			offset[2] = r_buf[4];
			mode = r_buf[5];
			printk(KERN_INFO "[stk831x]%s:file offset:%#02x,%#02x,%#02x,%#02x\n",
				__func__, offset[0], offset[1], offset[2], mode);
		}
		else
		{
			printk(KERN_ERR "[stk831x]%s: cali version number error! r_buf=0x%x,0x%x,0x%x,0x%x,0x%x\n",
				__func__, r_buf[0], r_buf[1], r_buf[2], r_buf[3], r_buf[4]);
		}
	}
	else
		printk(KERN_INFO "[stk831x]%s: No file offset\n", __func__);

	for(cnt=0x43;cnt<0x49;cnt++)
	{
		result = STK831x_ReadByteOTP(cnt, &(regR[cnt-0x43]));
		if(result < 0)
			printk(KERN_ERR "[stk831x]%s: STK831x_ReadByteOTP failed, ret=%d\n", __func__, result);
	}
	printk(KERN_INFO "[stk831x]%s: OTP 0x43-0x49:%#02x,%#02x,%#02x,%#02x,%#02x,%#02x\n", __func__, regR[0],
	...
	printk(KERN_INFO "[stk831x]%s: OTP 0x53-0x59:%#02x,%#02x,%#02x,%#02x,%#02x,%#02x\n", __func__, regR[0],
		regR[1], regR[2],regR[3], regR[4], regR[5]);
...
static int STK831x_Init(struct stk831x_data *stk, struct i2c_client *client)
{
...
#ifdef CONFIG_SENSORS_STK8312
	printk(KERN_INFO "[stk831x]%s: Initialize stk8312\n", __func__);
#elif defined CONFIG_SENSORS_STK8313
	printk(KERN_INFO "[stk831x]%s: Initialize stk8313\n", __func__);
#endif
	...
	buffer[0] = STK831X_TILT;
	buffer[1] = 0xA0;
	result = STK_i2c_Tx(buffer, 2);
	if (result < 0)
	{
		printk(KERN_ERR "[stk831x]%s:interrupt init failed\n", __func__);
		return result;
	}
	...
	if (result < 0)
	{
		printk(KERN_ERR "[stk831x]%s:set range failed\n", __func__);
		return result;
	}
...
static void stk_handle_first_en(struct stk831x_data *stk)
{
	...
	if ((stk_get_file_content(r_buf, STK_ACC_CALI_FILE_SIZE)) == 0)
	{
		if(r_buf[0] == STK_ACC_CALI_VER0 && r_buf[1] == STK_ACC_CALI_VER1)
		{
			...
			stk_tune_offset_record[2] = offset[2];
#endif
			printk(KERN_INFO "[stk831x]%s: set offset:%d,%d,%d, mode=%d\n", __func__, offset[0], offset[1], offset[2], mode);
	...
	else if(stk_tune_offset_record[0]!=0 || stk_tune_offset_record[1]!=0 || stk_tune_offset_record[2]!=0)
	{
		...
		stk_tune_done = 1;
		atomic_set(&stk->cali_status, STK_K_SUCCESS_TUNE);
		printk(KERN_INFO "[stk831x]%s: set offset:%d,%d,%d\n", __func__, stk_tune_offset_record[0],
	...
	printk(KERN_INFO "[stk831x]%s: finish, cali_status = 0x%x\n", __func__, atomic_read(&stk->cali_status));
	return;
}
...
static int32_t stk_get_ic_content(struct stk831x_data *stk)
{
	...
	{
		atomic_set(&stk->cali_status, STK_K_SUCCESS_FT2);
		printk(KERN_INFO "[stk831x]%s: OTP 2 used\n", __func__);
		return 2;
	}
	if(regR&0x10)
	{
		atomic_set(&stk->cali_status, STK_K_SUCCESS_FT1);
		printk(KERN_INFO "[stk831x]%s: OTP 1 used\n", __func__);
		return 1;
	}
	return 0;
}
...
static int stk_store_in_ic( struct stk831x_data *stk, char otp_offset[], char FT_index, uint32_t delay_ms)
{
...
#ifdef STK_DEBUG_CALI
	//printk(KERN_INFO "[stk831x]%s:Check All OTP Data after write 0x2B 0x2F 0x33\n", __func__);
	//STK831x_ReadAllOTP();
#endif
	...
	if(result)
	{
		printk(KERN_ERR "[stk831x]%s: calibration check1 fail, FT_index=%d\n", __func__, FT_index);
		goto ic_err_misc;
	}
#ifdef STK_DEBUG_CALI
	...
	if(result < 0)
	{
		printk(KERN_INFO "[stk831x]%s: write OTP%d fail\n", __func__, FT_index);
		goto ic_err_misc;
	}
	...
	if(result)
	{
		printk(KERN_ERR "[stk831x]%s: calibration check2 fail\n", __func__);
		goto ic_err_misc;
	}
	return 0;
...
static int32_t stk_get_file_content(char * r_buf, int8_t buf_size)
{
    ...
    if(IS_ERR(cali_file))
	{
        printk(KERN_ERR "[stk831x]%s: filp_open error, no offset file!\n", __func__);
        return -ENOENT;
	}
	else
	{
		...
		if(ret < 0)
		{
			printk(KERN_ERR "[stk831x]%s: read error, ret=%d\n", __func__, ret);
...
static int stk_permission_thread(void *data)
{
	...
	set_fs(fs);
	printk(KERN_INFO "[stk831x]%s exit, retry=%d\n", __func__, retry);
	return 0;
}
...
static int stk_store_in_file(char offset[], char mode)
{
    ...
    if(IS_ERR(cali_file))
	{
        printk(KERN_ERR "[stk831x]%s: filp_open error!\n", __func__);
	...
	else
	{
		...
		{
			printk(KERN_ERR "[stk831x]%s: write error!\n", __func__);
			filp_close(cali_file,NULL);
		...
		if(ret < 0)
		{
			printk(KERN_ERR "[stk831x]%s: read error!\n", __func__);
			filp_close(cali_file,NULL);
			return -STK_K_FAIL_R_BACK;
		}
		set_fs(fs);

		//printk(KERN_INFO "[stk831x]%s: read ret=%d!\n", __func__, ret);
		for(i=0;i<STK_ACC_CALI_FILE_SIZE;i++)
		{
			if(r_buf[i] != w_buf[i])
			{
				printk(KERN_ERR "[stk831x]%s: read back error, r_buf[%x](0x%x) != w_buf[%x](0x%x)\n",
...
#endif
{
	...
	printk(KERN_INFO "333333333333[stk831x]%s: cmd = 0x%x\n", __func__, cmd);	
...
{
	printk(KERN_ERR"[stk8313] stk_mems_irq_handler \n");
...
static void stk_mems_wq_function(struct work_struct *work)
{
	...
	count++;
	if(count%2 == 0){
		input_report_key(stk831x_data_ptr->input_dev, KEY_SPORT, 1);
	}else{
		input_report_key(stk831x_data_ptr->input_dev, KEY_SPORT, 0);
	}
	input_sync(stk831x_data_ptr->input_dev);
	if(count >= 100)
		count = 0;
	
	printk(KERN_ERR"1111--G-sensor Detected collision, Reporting uevent\n");
...
static int stk831x_irq_setup(struct i2c_client *client, struct stk831x_data *stk_int)
{
	...
	if (gpio_request(gpio, "EINT"))
	{
		printk(KERN_ERR "[stk831x]%s:gpio_request() failed\n",__func__);
		return -1;
	}
	...
	if ( irq < 0 )
	{
		printk(KERN_ERR "[stk831x]%s:gpio_to_irq() failed\n",__func__);
		return -1;
	}
	client->irq = irq;
	stk_int->irq = irq;
#endif //#if ADDITIONAL_GPIO_CFG
	printk(KERN_INFO "[stk831x]%s: irq # = %d irq_gpio=%d\n", __func__, irq,gpio);
	if(irq < 0)
		printk(KERN_ERR "[stk831x]%s: irq number was not specified!\n", __func__);
	//error = request_irq(client->irq, stk_mems_irq_handler, IRQF_TRIGGER_RISING , "stk-mems", stk_int);
	error = request_irq(client->irq, stk_mems_irq_handler, IRQF_TRIGGER_RISING , "stk-mems", stk_int);
	if (error < 0)
	{
		printk(KERN_ERR "[stk831x]%s: request_irq(%d) failed for (%d)\n", __func__, client->irq, error);
		return -1;
	}
	printk("requst_irq register ok\n");
	//disable_irq(irq);
	return irq;
}
		...
		const char *buf, size_t count)
{
	...
	if (error)
	{
		printk(KERN_ERR "[stk831x]%s: strict_strtoul failed, error=0x%x\n", __func__, error);
		return error;
	}
	if ((data == 0)||(data==1))
		STK831x_SetEnable(stk,data);
	else
		printk(KERN_ERR "[stk831x]%s: invalud argument, data=%ld\n", __func__, data);
	return count;
}
		...
		const char *buf, size_t count)
{
	...
	printk(KERN_INFO "[stk831x]%s: reg[0x%x]=0x%x\n", __func__, w_reg[0], w_reg[1]);
	buffer[0] = w_reg[0];
	buffer[1] = w_reg[1];
	...
	const char *buf, size_t count)
{
	...
	if (error)
	{
		printk(KERN_ERR "[stk831x]%s: strict_strtoul failed, error=%d\n", __func__, error);
		return error;
	}

	if(data > MAX_FIR_LEN)
	{
		printk(KERN_ERR "[stk831x]%s: firlen exceed maximum filter length\n", __func__);
	}
	else if (data < 1)
	{
...
#else
	printk(KERN_ERR "[stk831x]%s: firlen is not supported\n", __func__);
#endif
	return count;
}
static ssize_t stk831x_sensitivity_show(struct device *dev,
                   struct device_attribute *attr, char *buf)
{
...
static ssize_t stk831x_sensitivity_store(struct device *dev,
                    struct device_attribute *attr,
                    const char *buf, size_t count)
{
	int level, result=0;
	...
	g_level_flag = simple_strtoul(buf, NULL, 10); 
	printk(KERN_ERR"[----------] g_level_flag = %d \n.",g_level_flag);
	switch(g_level_flag)
	{
		case 1:
			level = 1;
...
static DEVICE_ATTR(sensitivity,S_IWUGO | S_IRUGO,stk831x_sensitivity_show,stk831x_sensitivity_store);
...
static int stk831x_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk(KERN_INFO "------------------> I am int stk838x.c <--------------------- \n");
	printk(KERN_INFO "stk831x_probe: driver version:%s\n",STK_ACC_DRIVER_VERSION);
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C))
	{
		printk(KERN_ERR "[stk831x]%s:i2c_check_functionality error\n", __func__);
		error = -ENODEV;
	...
	if (!stk)
	{
		printk(KERN_ERR "[stk831x]%s:memory allocation error\n", __func__);
		error = -ENOMEM;
	...
	{
		printk(KERN_ERR "[stk831x]%s:create_workqueue error\n", __func__);
		error = -EPERM;
	...
	if (error)
	{
		printk(KERN_ERR "[stk831x]%s:stk831x initialization failed\n", __func__);
	...
	{
		error = -ENOMEM;
		printk(KERN_ERR "[stk831x]%s:input_allocate_device failed\n", __func__);
...
//	input_set_abs_params(stk->input_dev, ABS_Z, -512, 511, 0, 0);
		...
		input_set_capability(stk->input_dev, EV_KEY, KEY_SPORT);
		
		stk->input_dev->name = STK8313_INPUT_DEV_NAME;   
		stk->input_dev->phys = "stk8313/input2";
		
		stk->input_dev->id.bustype = BUS_HOST;
		stk->input_dev->dev.parent = &client->dev;
		stk->input_dev->id.vendor = 0x000A;
		stk->input_dev->id.product = 0x000A;
		stk->input_dev->id.version = 0x0A00;
		set_bit(EV_KEY, stk->input_dev->evbit);
#endif

	error = input_register_device(stk->input_dev);
	if (error)
	{
		printk(KERN_ERR "[stk831x]%s:Unable to register input device: %s\n", __func__, stk->input_dev->name);
	...
	if (error)
	{
		printk(KERN_ERR "[stk831x]%s: misc_register failed\n", __func__);
		goto exit_misc_device_register_error;
	}
	
	stk->stk831x_kobj = kobject_create_and_add("attribute", &stk_device.this_device->kobj);
	if (!stk->stk831x_kobj){		
		printk("kobject create or add failed!\n");
		goto err_unregister_device;
	}
	
	error = sysfs_create_group(stk->stk831x_kobj, &stk831x_attribute_group);
	if (error)
	{
		printk(KERN_ERR "[stk831x]%s: sysfs_create_group failed\n", __func__);
...
static int gsensor_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	...
	if(twi_id == adapter->nr){
		pr_info("[stk8312]%s: Detected chip %s at adapter %d, address 0x%02x\n",
...
int stk831x_auto_detect = 0;
EXPORT_SYMBOL(stk831x_auto_detect);
...
//	struct device *device;
	...
	if(gsensor_fetch_sysconfig_para()){
		printk("[stk831x]%s:gsensor_fetch_sysconfig_para  err.\n", __func__);
		return -1;
	}
#endif
//20130806 Infotm Item Interface acccessed for i2c_driver being added,
//other parts of this file(except DoI2cScanDetectDev) and the header file
//are derived from STK standard driver code which is ver1.8
	if(item_exist("sensor.grivaty") &&
      (item_equal("sensor.grivaty.model", "stk831x", 0)|| DoI2cScanDetectDev()))
	{
...
#ifdef STK_ALLWINNER_PLATFORM
	printk("[stk831x]======stk831x init ok======\n");
#endif	/* #ifdef STK_ALLWINNER_PLATFORM */
	return ret;
}
