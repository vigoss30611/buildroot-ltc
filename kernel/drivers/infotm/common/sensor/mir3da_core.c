...
int mir3da_module_detect(PLAT_HANDLE handle)
{
    ...
    for(i=0;i<sizeof(mir3da_gsensor)/sizeof(mir3da_gsensor[0]);i++) {
	    ...
	    if(res != 0) {
			MI_ERR("1-mir3da_register_read error\n");
		    return res;
	    }
        ...
        if(mir3da_gsensor[i].chip_id.value == cid) {
	        ...
	        if(res != 0) {
				MI_ERR("2-mir3da_register_read error\n");
		        return res;
	        }
...
int i =0;
    ...
    MI_ERR("Probe gsensor module: %s", mir3da_gsensor[gsensor_mod].asic);
	...
	printk("at mir3da_chip_resume\n");
    res = mir3da_chip_resume(handle);
    if(res) {
				MI_ERR("chip resume fail!!\n");
				return 0;
    }
	mir3da_open_interrupt(handle, 2);
...
int mir3da_chip_resume(MIR_HANDLE handle)
{
	...
	if(gsensor_type<0){
		printk("at mir3da_parse_chip_info\n");
	...
	if(gsensor_chip_info.asic==ASIC_2512B){
		...
		if(reg_data ==0x26 || reg_data ==0x4c){
			printk("at mir3da_register_mask_write\n");
    ...
    bLoad = FILE_CHECKING;
    readOffsetCnt = -1;    
	printk("at manual_load_cali_file\n");
   // manual_load_cali_file(handle);
#endif
...
int mir3da_set_sensitivity(MIR_HANDLE handle, int level)
{
	int   res = 0;

	switch(level){
		case 1:
			res |=  mir3da_register_mask_write(handle, NSA_REG_ACTIVE_THRESHOLD,0xff,0x20);
			break;
		case 2:
			res |=  mir3da_register_mask_write(handle, NSA_REG_ACTIVE_THRESHOLD,0xff,0x40);
			break;
		case 3:
			res |=  mir3da_register_mask_write(handle, NSA_REG_ACTIVE_THRESHOLD,0xff,0x60);
			break;
		default:
			return 0;
	}
...
int mir3da_open_interrupt(MIR_HANDLE handle,int level)
{
	int   res = 0;
	
 	//printk("Jicky mir3da_open_interrupt \n");
	switch(level){
	...
	res |=  mir3da_register_mask_write(handle,NSA_REG_INT_LATCH,0x8F,0x81);//3 1s  6 8s
	res |=  mir3da_register_mask_write(handle,NSA_REG_ACTIVE_DURATION,0xff,0x03 );	
	res |=  mir3da_register_mask_write(handle,NSA_REG_INTERRUPT_SETTINGS1,0xff,0x07);  //0x3

    res |= mir3da_register_mask_write(handle,NSA_REG_POWERMODE_BW, 0xFF, 0x5E);
    res |= mir3da_register_mask_write(handle,NSA_REG_ODR_AXIS_DISABLE, 0xFF, 0x06);		

	return res;
}

int mir3da_close_interrupt(MIR_HANDLE handle){
	int   res = 0;

	res = mir3da_register_write(handle, NSA_REG_INTERRUPT_SETTINGS1,0x00 );
	res |= mir3da_register_write(handle, NSA_REG_INTERRUPT_MAPPING1,0x00 );
