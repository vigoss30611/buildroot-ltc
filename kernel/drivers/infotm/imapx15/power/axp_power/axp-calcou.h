		...
		if(BATT_VOL_OPT){
			if(charger->rest_cap >= 20){
				...
				if(charger->bat_current_direction == 0){//discharge
					...
					if(charger->batt_low_flag == 0){
						if(charger->rest_cap <= charger->batt_cap_low_design){
							charger->batt_low_flag = 1;
...
//#endif
			...
			axp_batt_msuspend(charger);

			msleep(1);
			charger->pre_sts = read_rtc_gpx(2);
			printk("suspend: charger->pre_sts = %d\n", charger->pre_sts);
			return 0;
		}
	...
	charger->ocv = ((v[0] << 4) + (v[1] & 0x0f)) * 11 /10;
			if(charger->bat_det == 1){
				...
				if(flag_coulomb){
					...
					/*出错处理，如果初始base_cap估计偏小，导致充满后用库仑计计算的电量低于100%*/
			if((charger->ocv > 4100) && (charger->rest_cap < 100) && (charger->bat_current_direction == 0x0) && (charger->ext_valid == 1)){
...
#if 0			//still use coulomb to caculate capacity, so mask this code
