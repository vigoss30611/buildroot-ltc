...
#define BATCAP				((item_exist("batt.capacity"))?(item_integer("batt.capacity", 0)):(2300))
//#define BATCAP              2300
...
*/
/* booting chargering current, mA */
#define STACHGCUR			500
/* earlysuspend chargering current, mA */
#define EARCHGCUR			1200
/* suspend chargering current, mA */
#define SUSCHGCUR			EARCHGCUR
/* power-off chargering current, mA */
...
#define	AXP_IRQ_CHAOV		(1<<10) //电池充电完成
#define	AXP_IRQ_CHAST		(1<<11) //电池充电开始
...
#define	AXP_IRQ_PEKLO		(1<<16) //power键长按
#define	AXP_IRQ_PEKSH		(1<<17) //power键短按
...
#define AXP_IRQ_EXTLOWARN2	(1<<24) //APS低压警告电压2
#define AXP_IRQ_EXTLOWARN1	(1<<25) //APS低压警告电压1
...
#define AXP_IRQ_GPIO0TG		(1<<0) //GPIO0输入边沿触发
#define AXP_IRQ_GPIO1TG		(1<<1) //GPIO1输入边沿触发，ADC输入IRQ
#define AXP_IRQ_GPIO2TG		(1<<2) //GPIO2输入边沿触发
#define AXP_IRQ_GPIO3TG		(1<<3) //GPIO3输入边沿触发
#define AXP_IRQ_PEKFE		(1<<5) //power键下降沿触发
#define AXP_IRQ_PEKRE		(1<<6) //power键上升沿触发
#define AXP_IRQ_TIMER		(1<<7) //计时器超时

/*选择需要打开的中断使能*/
#define AXP_NOTIFIER_ON_LOW			(AXP_IRQ_USBIN | \
							AXP_IRQ_USBRE | \
							AXP_IRQ_ACIN | \
							AXP_IRQ_USBOV | \
							AXP_IRQ_ACRE | \
...
#define AXP_NOTIFIER_ON_HIGH		(AXP_IRQ_PEKFE | \
							AXP_IRQ_PEKRE | \
							AXP_IRQ_GPIO2TG | \
							AXP_IRQ_GPIO3TG)
