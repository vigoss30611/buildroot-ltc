...
struct axp_mfd_chip {
	...
	int		type;
	uint32_t	irqs_enabled_low;
	uint32_t	irqs_enabled_high;
...
struct axp_mfd_chip_ops {
	...
	int	(*enable_irqs)(struct axp_mfd_chip *, uint32_t irqs_low, uint32_t irqs_high);
	int	(*disable_irqs)(struct axp_mfd_chip *, uint32_t irqs_low, uint32_t irqs_high);
	int	(*read_irqs)(struct axp_mfd_chip *, uint32_t *irqs_low, uint32_t *irqs_high);
	int	(*clear_irqs)(struct axp_mfd_chip *, uint32_t irqs_low, uint32_t irqs_high);
};
...
#define AXP_IPS_SET							POWER20_IPS_SET
#endif
