...
static int imapx15_gpio_direction(struct gpio_chip *chip, unsigned offset)
{
	uint8_t tmp;
	...
	/* In iMAPx15, return 1 means gpio direction output. But
	 * 1 means input in gpio system. So we reverse here. */
	return !tmp;
}

static int imapx15_gpio_input(struct gpio_chip *chip, unsigned offset)
{
	struct imapx_gpio_chip *igc = to_imapx_gpiochip(chip);
	uint8_t tmp;
	int cnt = 0;
...
static int imapx15_gpio_output(struct gpio_chip *chip,
...
static int imapx15_gpio_get_value(struct gpio_chip *chip, unsigned offset)
{
	struct imapx_gpio_chip *igc = to_imapx_gpiochip(chip);
	uint8_t tmp;
	int cnt = 0;
...
static void imapx15_gpio_set_value(struct gpio_chip *chip,
					unsigned offset, int value)
{
	struct imapx_gpio_chip *igc = to_imapx_gpiochip(chip);
...
static int imapx15_gpio_to_irq(struct gpio_chip *chip, unsigned offset)
{
	struct imapx_gpio_chip *igc = to_imapx_gpiochip(chip);
...
static void imapx15_gpio_irq_ack(struct irq_data *d)
{
...
static void imapx15_gpio_irq_mask(struct irq_data *d)
{
	unsigned long gpio = d->hwirq;
	//pr_err("%s: gpio=%d\n", __func__, gpio);
...
static void imapx15_gpio_irq_unmask(struct irq_data *d)
{
	unsigned long gpio = d->hwirq;
	//pr_err("%s: gpio=%d\n", __func__, gpio);
...
static int imapx15_gpio_irq_set_type(struct irq_data *d, unsigned int type)
{
	unsigned long gpio = d->hwirq;
	int trigger;
	//pr_err("%s: gpio=%d\n", __func__, gpio);
...
static void imapx15_gpio_irq_handler(unsigned int irq, struct irq_desc *desc)
{
	struct imapx_gpio_bank *bank;
	int gpio, pin, type;
...
struct gpio_chip imapx15_gpio_chip = {
	.label	= "imapx15-gpiochip",
	.ngpio	= 137,
	.direction_input = imapx15_gpio_input,
	.direction_output = imapx15_gpio_output,
	.get_direction = imapx15_gpio_direction,
	.get = imapx15_gpio_get_value,
	.set = imapx15_gpio_set_value,
	.to_irq = imapx15_gpio_to_irq,
};

struct irq_chip imapx15_irq_chip = {
	.name = "imapx15-irqchip",
	.irq_ack = imapx15_gpio_irq_ack,
	.irq_mask = imapx15_gpio_irq_mask,
	.irq_unmask = imapx15_gpio_irq_unmask,
	.irq_set_type = imapx15_gpio_irq_set_type,
};
...
struct imapx_gpio_bank imapx15_gpio_bank[] = {
	BANK_INFO(0, 64, 16),
	BANK_INFO(1, 65, 16),
	BANK_INFO(2, 66, 16),
	BANK_INFO(3, 67, 16),
...
struct imapx_gpio_chip imapx_gpio_chip[] = {
	[0] = {
		.compatible = "x15",
		.chip = &imapx15_gpio_chip,
		.irq_chip = &imapx15_irq_chip,
		.bankcount = 9,
		.bank = imapx15_gpio_bank,
	},
};
...
static int imapx15_gpiochip_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < igc->bankcount; i++) {
		irq_set_chained_handler(bank[i].irq, imapx15_gpio_irq_handler);
...
int imapx15_gpio_save(void)
{
	unsigned long flags;
	int i, j;

	local_irq_save(flags);
	for (i = 0; i < ARRAY_SIZE(imapx15_gpio_bank); i++) {
		struct imapx_gpio_bank *bank = &imapx15_gpio_bank[i];
		for (j = 0; j < bank->nr; j++) {
...
EXPORT_SYMBOL(imapx15_gpio_save);

int imapx15_gpio_restore(void)
{
	unsigned long flags;
	int i, j;
...
EXPORT_SYMBOL(imapx15_gpio_restore);

static struct platform_driver imapx15_gpiochip_driver = {
	.probe	= imapx15_gpiochip_probe,
	.driver	= {
		.name = "imap-gpiochip",
		.owner = THIS_MODULE,
	},
...
static int __init imapx15_gpiochip_init(void)
{
	return platform_driver_register(&imapx15_gpiochip_driver);
}
arch_initcall(imapx15_gpiochip_init);
