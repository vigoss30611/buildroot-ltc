...
extern int clk_create_debugfs(struct dentry *parent);
/**
...
static int clk_disable_unused(void)
{
    ...
    //fengwu add to enable osc clk for G2. 20150415
    {
		pr_warn("clk: clk_disable_unused\n");
		//return 0; 
    }
...
{
    printk("clk_set_rate %s ,oldrate:%d, new rate:%d\n", clk->name, clk->rate, rate);
	...
	if (rate == clk->rate)
    {
        printk("clk_set_rate same\n");
		goto out;
    }

	if ((clk->flags & CLK_SET_RATE_GATE) && clk->prepare_count) {
        printk("clk_set_rate busy flag: %x\n", clk->flags);
		ret = -EBUSY;
		goto out;
	}
	...
	if (!top) {
        printk("clk_set_rate inv value: %d\n", rate);
		ret = -EINVAL;
		goto out;
	}
