...
struct led_cfg_data imapx_led_cfg_data[5] = {
	[0] = {
			.model_name	= "NOLED",
			.gpio = -1,
			.polarity = -1,
	...
	for(i=0;i<5;i++){
		...
		if(item_exist(gpio)) {
			...
			switch(pwm_id)
				{
					case 0:
						imapx_led_cfg.led_data[i].gpio = 85;
						break;
					case 1:
						imapx_led_cfg.led_data[i].gpio = 86;
						break;
					case 2:
						imapx_led_cfg.led_data[i].gpio = 113;
						break;
					case 3:
						imapx_led_cfg.led_data[i].gpio = 114;
						break;
					case 4:
						imapx_led_cfg.led_data[i].gpio = 110;
						break;
				}
			}
...
static int __init imapx_item_parse_adc_key(void) {
	...
	if(item_exist("keys.reset") && item_equal("keys.reset", "adc", 0)) {
		...
		adc_buttons_cfg[4].code = KEY_MENU; 
