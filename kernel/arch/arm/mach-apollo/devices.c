/***************************************************
** arch/arm/mach-imapx15/devices.c
**
...
static struct dw_mci_board imap_mmc1_platdata = {
	...
	.bus_hz = /*50*/35 * 1000 * 1000, // changed by linyun.xiong @2015-09-04 
	.detect_delay_ms = 500,
	.init = imap_mmc1_init,
...
static struct platform_device imap_bt_codec_device = {
	.name = "bt",
	.id = 0,
};
...
///////////////////////////////////////////////linyun
#define IMAPX15_MIPI_CSI_DEV
#ifdef IMAPX15_MIPI_CSI_DEV

#include "linux/camera/csi/csi_reg.h"

//csi power on cfg
static int imapx_csi_init()
{
	printk("--->imapx_csi_init\n");
...
//csi power down 
static int imapx_csi_deinit()
{
	printk("--->imapx_csi_deinit\n");
        module_power_down(SYSMGR_MIPI_BASE);

        //int ret = __raw_readl(SYS_CLOCK_GATE_EN(IO_ADDRESS(SYSMGR_MIPI_BASE)));
        //printk("camif gate enable register value :0x%x", ret);

        return 0;
}

static struct imapx_cam_board imapx_csi_pdata = {
        .init = imapx_csi_init,
        .deinit = imapx_csi_deinit,
};

//csi device structure
static struct resource imap_csi_resource[] = {
        [0] = {
                        .start = CSI_REG_BASE,
                        .end   = CSI_REG_BASE + CSI_REG_SIZE - 1,
                        .flags = IORESOURCE_MEM,
        },
        [1] = {
                        .start = GIC_CSI0_ID,
...
static u64 csi_dma_mask = DMA_BIT_MASK(32);

struct platform_device imap_csi_device = {
        .name = "imapx-mipi-csi-ctrl",
        .id = -1,
        .num_resources = ARRAY_SIZE(imap_csi_resource),
        .resource = imap_csi_resource,
        .dev = {
                .platform_data = &imapx_csi_pdata,
                .dma_mask = &csi_dma_mask,
                .coherent_dma_mask = DMA_BIT_MASK(32),
        }
};
#endif
...
static struct display_device display_hdmi_device = {
	.name = "hdmi_tx",
	.id = 0,
	.path = 0,
};
...
#include <linux/gpio_keys.h>
//add by xiongbiao
static struct gpio_keys_button imap_buttons[] = {
	{
		.gpio		= 148,//key1
		.code		= KEY_CAMERA,
		.desc		= "SHOOT_KEY1",
...
static struct gpio_keys_platform_data imap_button_data = {
	.buttons	= imap_buttons,
	.nbuttons	= ARRAY_SIZE(imap_buttons),
};

static struct platform_device imap_button_device = {
	.name		= "gpio-keys",
	.id		= -1,
	.num_resources	= 0,
	.dev		= {
		.platform_data	= &imap_button_data,
	}
};
#endif
...
static struct i2c_board_info __initdata ec610_info[] = {
	{
...
#else
	imapx_pad_set_value(113, !!command);
#endif
}
...
#ifdef CONFIG_LCD_SPI_OTA5182A
static void ota5182a_cs_control(u32 command)
{
//	printk(KERN_ERR "pl022: dummy chip select control, CS=0x%x\n", command);
//	printk(KERN_ERR "%s ssp:::::::cs--->%d\n", __func__, !!command);
#ifdef CONFIG_ARCH_APOLLO
	imapx_pad_set_mode(139, "output");
	imapx_pad_set_value(139, !!command);
#else
	imapx_pad_set_value(113, !!command);
#endif
}
...
static void ili9806S_cs_control(u32 command)
{
//	printk(KERN_ERR "pl022: dummy chip select control, CS=0x%x\n", command);
//	printk(KERN_ERR "%s ssp:::::::cs--->%d\n", __func__, !!command);
#ifdef CONFIG_ARCH_APOLLO
	imapx_pad_set_mode(167, "output");
	imapx_pad_set_value(167, !!command);
#else
	imapx_pad_set_value(113, !!command);
#endif
}

static struct pl022_config_chip ili9806s_chip_info = {
	.com_mode = POLLING_TRANSFER,
	.iface = SSP_INTERFACE_MOTOROLA_SPI,
	.hierarchy = SSP_MASTER,
	.slave_tx_disable = 1,
	...
	.wait_state = SSP_MWIRE_WAIT_ZERO,
	.duplex = SSP_MICROWIRE_CHANNEL_FULL_DUPLEX,
	.cs_control = ili9806S_cs_control,
};
static struct spi_board_info spi_lcd_info[] = {
	{
	 .modalias = "ili9806s",
	 ...
	 .chip_select = 1,
	 .mode = SPI_CPHA | SPI_CPOL | SPI_MODE_3 |SPI_CS_HIGH,
	 .controller_data = &ili9806s_chip_info,
	 //.controller_data = &spi_flash_chip_info,
	 .platform_data = NULL,
	 },
};
...
struct audio_register audio[] = {
    ...
    [1] = {
    	.codec_name = "bt",
    	.ctrl = NOR_MODE,
    	.codec_device = &imap_bt_codec_device,
    	.imapx_audio_cfg_init = imapx_bt_init,
    },
};
...
#endif
#if defined(CONFIG_LCD_SPI_ILI8961)||defined(CONFIG_LCD_SPI_OTA5182A) || defined(CONFIG_LCD_SPI_ILI9806S)
...
#endif
	i2c_register_board_info(0, ec610_info, ARRAY_SIZE(ec610_info));
