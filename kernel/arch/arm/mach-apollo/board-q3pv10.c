...
void q3pv10_reset(char mode, const char *cmd)
{
	...
	if (cmd) {
		if (!strncmp(cmd, "recovery_ota", strlen("recovery_ota")))
			rtcbit_set("resetflag", BOOT_ST_RECOVERY_OTA);
		else if (!strncmp(cmd, "recovery", strlen("recovery")))
			rtcbit_set("resetflag", BOOT_ST_RECOVERY);
		else if (!strncmp(cmd, "charger", strlen("charger")))
