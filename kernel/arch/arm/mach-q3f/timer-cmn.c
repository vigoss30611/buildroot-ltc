...
static long q3f_cmn_get_clock_rate(const char *name)
{
	long rate;
 ...
 */
static irqreturn_t q3f_cmn_timer_interrupt(int irq, void *dev_id)
{
...
static void q3f_cmn_set_mode(enum clock_event_mode mode,
...
static struct irqaction cmn_timer_irq = {
	...
	.handler	= q3f_cmn_timer_interrupt,
	.dev_id		= &cmn_clockevent,
};

void __init q3f_cmn_timer_init(void __iomem *base, unsigned int irq,
	const char *name)
{
...
void q3f_cmn_timer_update(long x)
{
	long rate = q3f_cmn_get_clock_rate(cmn_clockevent.name);
	clkevt_reload = DIV_ROUND_CLOSEST(rate, HZ);
	cmn_clockevent.mult = div_sc(rate, NSEC_PER_SEC, cmn_clockevent.shift);
	if (cmn_enabled == 1)
		q3f_cmn_set_mode(CLOCK_EVT_MODE_PERIODIC, &cmn_clockevent);
}
#endif

int q3f_cmn_timer_resume(void)
{
	q3f_cmn_set_mode(CLOCK_EVT_MODE_PERIODIC, NULL);
	return 0;
}
