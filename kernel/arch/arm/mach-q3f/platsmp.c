/*
 *  linux/arch/arm/mach-q3f/platsmp.c
 *
...
void __cpuinit q3f_secondary_init(unsigned int cpu)
{
}

int __cpuinit q3f_boot_secondary(unsigned int cpu, struct task_struct *idle)
{
	/*
	...
	q3f_reset_core(cpu, IMAPX15_SWIRL_POINT);
	/* put the old boot vector back */

	writel(virt_to_phys(q3f_secondary_startup),
			IMAPX15_SYS_PCADDR);
	smp_wmb();
	flush_cache_all();
	udelay(100);
 ...
 */
void __init q3f_smp_init_cpus(void)
{
...
void __init q3f_smp_prepare_cpus(unsigned int max_cpus)
{
	int i;
	/*
...
static inline void q3f_do_lowpower(unsigned int cpu, int *spurious)
{
	/*
...
void q3f_cpu_power_down(unsigned int cpu)
{
	uint32_t tmp;
...
int q3f_cpu_kill(unsigned int cpu)
{
	int k;
 ...
 */
void q3f_cpu_die(unsigned int cpu)
{
...
int q3f_cpu_disable(unsigned int cpu)                           
{                                                                    
...
struct smp_operations q3f_smp_ops __initdata = { 
	.smp_init_cpus		= q3f_smp_init_cpus,
	.smp_prepare_cpus   = q3f_smp_prepare_cpus,  
	.smp_secondary_init = q3f_secondary_init,    
	.smp_boot_secondary = q3f_boot_secondary,    
#ifdef CONFIG_HOTPLUG_CPU                          
	.cpu_kill       = q3f_cpu_kill,              
	.cpu_die        = q3f_cpu_die,               
	.cpu_disable	= q3f_cpu_disable,
#endif                                             
};                                                 
