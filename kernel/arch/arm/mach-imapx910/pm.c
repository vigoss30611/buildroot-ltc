...
extern void imapx910_cpu_resume(void);
extern int imapx910_cpu_save(unsigned long *saveblk, long);
...
static void imapx910_gic_save(void)
{
	int i = 0;
	int length = 0;
	int count = 0;
...
static void imapx910_gic_restore(void)
{
	int i = 0;
	int length = 0;
	int count = 0;
...
static void imapx910_gic_disable(void)
{
...
void imapx910_pm_cb_flushcache(void)
{
	flush_cache_all();
	outer_flush_all();
}

static int imapx910_pm_prepare(void)
{
	rtcbit_set("sleeping", 1);
	pcloc_value = readl(phys_to_virt(0x800000e0));
	writel(virt_to_phys(imapx910_cpu_resume), phys_to_virt(0x800000e0));
	return 0;
}
...
static void imapx910_module_power_on(void)
{
	int val;
...
static int imapx910_cpu_suspend(unsigned long arg)
{
	...
	imapx910_gic_disable();
	imapx910_pm_cb_flushcache();
	...
	PM_INFO("---> step 2 <---\n");
	imapx910_enter_lowpower();
	panic("sleep resumed to originator?");
}

static int imapx910_pm_enter(suspend_state_t state)
{
	if (item_equal("board.arch", "a5pv20", 0))
		imap_sram_reinit();
	...
	writel(pcloc_value, phys_to_virt(0x800000e0)); 
	writel(0x3, IO_ADDRESS(SYSMGR_RTC_BASE + 0xC));
	writel(0x3, IO_ADDRESS(SYSMGR_RTC_BASE + 0x4));

	imapx910_gic_restore();
	pad_case_resume();
	module_power_on(SYSMGR_GPIO_BASE);
	imapx910_gpio_restore();

	return 0;
}

static void imapx910_pm_finish(void)
{
	PM_INFO("++ %s ++\n", __func__);
}

static void imapx910_pm_wake(void)
{
	imapx910_module_power_on();
	/* for uart resume */
...
static const struct platform_suspend_ops imapx910_pm_ops = {
	.enter = imapx910_pm_enter,
	.prepare = imapx910_pm_prepare,
	.finish = imapx910_pm_finish,
	.valid = suspend_valid_only_mem,
	.wake = imapx910_pm_wake,
};
...
{
	PM_INFO("++ %s ++\n", __func__);
	pm_cpu_sleep = imapx910_cpu_suspend;
	suspend_set_ops(&imapx910_pm_ops);
	return;
}
