 ...
 *  Author:summer
 *  Date:2014-6-18
 *  Operation:copy and modify
 *  Reason:
 *  (1)Items was loaded from nand to ddr in uboot .
 *  (2)It was originally loaded to the reserved memory,then copied to 0x3c000000.
 *  (3)This coused a bug when loading uboot-logo :kernel can't get any data of items,
 *     and i believe some modules in uboot must use the memory starts at 0x3c000000.
 *  (4)So,in kernel,we won't get items from 0x3c000000.We directly get the reserved memory.
 *  */
int __init item_init(char *mem, int len)
{
	...
	printk(KERN_ERR "MEM--%x, LEN--%x\n",mem,len);
	item_dbg("%s\n", __func__);
	items_mem    = mem;
	
	items_mem = rbget("itemrrtb");
	if(items_mem != NULL)
	{
		mem = phys_to_virt(items_mem);
		s = mem;
	}
 ...
 *  (3)This coused a bug when loading uboot-logo :kernel can't get any data of items,
 *     and i believe some modules in uboot must use the memory starts at 0x3c000000.
 *  (4)So,in kernel,we won't get items from 0x3c000000.
int __init item_init(char *mem, int len)
{
	const char *s = mem;
	int i;
