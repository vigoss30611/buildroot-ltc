...
void __init imapx910_init_gpio(void)
{
	 ...
	 * Operation:withdraw
	 * Reason:
	 * (1)We want to display the logo when uboot starts up.
	 * (2)Also we want the logo keeps until kernel starts up.
	 * (3)IF not,the procedure will not be continous.
	 * */
	...
	pr_err("imapx910 init gpio: ngpio = %d\n", ngpio);
}
