...
struct tempmon_cfg imapx_tempmon_cfg = {
	.model = "NONE",
};
...
static int __init imapx_item_parse_gpio_key(void)
{
	...
	imapx_gpio_key_cfg.menu =
		item_exist("keys.menu") ? item_integer("keys.menu", 1) : -ITEMS_EINT;
	imapx_gpio_key_cfg.back =
		item_exist("keys.back") ? item_integer("keys.back", 1) : -ITEMS_EINT;
	imapx_gpio_key_cfg.home =
		item_exist("keys.home") ? item_integer("keys.home", 1) : -ITEMS_EINT;
	imapx_gpio_key_cfg.volup =
		item_exist("keys.volup") ? item_integer("keys.volup", 1) : -ITEMS_EINT;
	imapx_gpio_key_cfg.voldown =
		item_exist("keys.voldown") ? item_integer("keys.voldown", 1) : -ITEMS_EINT;
	imapx_gpio_key_cfg.reset =
		item_exist("keys.reset") ? item_integer("keys.reset", 1) : -ITEMS_EINT;
	return PARSE_OK;
}

static int __init imapx_item_parse_tempmon(void)
{
	if(item_exist("sensor.thermal.model"))
			item_string(imapx_tempmon_cfg.model, "sensor.thermal.model", 0);
	return PARSE_OK;
}
/*
 *pasre PMU
...
static int __init imapx_item_parse_camera(void)
{
	...
	if(camera_front_cfg.exist) {
		...
		camera_front_cfg.pdown_pin =
			item_exist("camera.front.power.down") ?
			item_integer("camera.front.power.down", 1) : -1;

		camera_front_cfg.reset_pin =
			item_exist("camera.front.reset") ?
	...
	if(item_exist("camera.rear.interface")) {
		item_string(camera_rear_cfg.interface,
				"camera.rear.interface", 0);
	...
	if(camera_rear_cfg.exist) {
		if(item_exist("camera.rear.model"))
			item_string(camera_rear_cfg.name,
					"camera.rear.model", 0);
		if(item_exist("camera.rear.ctrl")) {
			item_string(camera_rear_cfg.ctrl_bus,
					"camera.rear.ctrl", 0);
			camera_rear_cfg.bus_chn =
				item_integer("camera.rear.ctrl", 1);
		}
		...
		camera_rear_cfg.pdown_pin =
			item_exist("camera.rear.power.down") ?
			item_integer("camera.rear.power.down", 1) : -1;

		camera_rear_cfg.reset_pin =
			item_exist("camera.rear.reset") ?
	...
	for (i = 0; i < nr; i++) {
		if (mps->need_parse == 1) {
			...
			case IMAPX_GPIO_KEY:
				...
				break;
			case IMAPX_TEMPMON:
				mps->parse_done = imapx_item_parse_tempmon();
				break;
			case IMAPX_WIFI:
