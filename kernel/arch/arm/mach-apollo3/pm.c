...
extern void apollo3_cpu_resume(void);
extern int apollo3_cpu_save(unsigned long *saveblk, long);
...
static void apollo3_gic_save(void)
{
	int i = 0;
	int length = 0;
	int count = 0;
...
static void apollo3_gic_restore(void)
{
	int i = 0;
	int length = 0;
	int count = 0;
...
static void apollo3_gic_disable(void)
{
...
void apollo3_pm_cb_flushcache(void)
{
	flush_cache_all();
	outer_flush_all();
}

static int apollo3_pm_prepare(void)
{
	rtcbit_set("sleeping", 6);
	pcloc_value = readl(0xc00000e0);
	writel(virt_to_phys(apollo3_cpu_resume), 0xc00000e0);
	return 0;
}
...
{
	struct clk *uart = clk_get_sys("imap-uart", NULL);
	int rate, ibr, fbr;
	uint32_t uart_im;
...
static void apollo3_module_power_on(void)
{
	int val;
...
static int apollo3_cpu_suspend(unsigned long arg)
{
...
*/
	...
	__raw_writel(0x99, rtc_sys + SYS_INT_CLR);
	__raw_writel(0x02, rtc_sys + SYS_RST_CLR);
	__raw_writel(0x04, rtc_sys + SYS_POWUP_CLR);
	__raw_writel(0x09, rtc_sys + SYS_INT_MASK);
	...
	pr_err("---> step 2 <---\n");
	apollo3_enter_lowpower();
	panic("sleep resumed to originator?");
}

static int apollo3_pm_enter(suspend_state_t state)
{
	if (item_equal("board.arch", "a5pv20", 0) 
			|| item_equal("board.arch", "q3pv10", 0)
			|| item_equal("board.arch", "apollo3pv10", 0))
		imap_sram_reinit();
	...
	writel(pcloc_value, 0xc00000e0);
	writel(0x1, IO_ADDRESS(SYSMGR_RTC_BASE + 0xC));
	writel(0x1, IO_ADDRESS(SYSMGR_RTC_BASE + 0x4));

	apollo3_gic_restore();
	pad_case_resume();
	module_power_on(SYSMGR_GPIO_BASE);
	apollo3_gpio_restore();

	return 0;
}

static void apollo3_pm_finish(void)
{
	PM_INFO("++ %s ++\n", __func__);
}

static void apollo3_pm_wake(void)
{
	apollo3_module_power_on();
	/* for uart resume */
...
static const struct platform_suspend_ops apollo3_pm_ops = {
	.enter = apollo3_pm_enter,
	.prepare = apollo3_pm_prepare,
	.finish = apollo3_pm_finish,
	.valid = suspend_valid_only_mem,
	.wake = apollo3_pm_wake,
};
...
{
	PM_INFO("++ %s ++\n", __func__);
	pm_cpu_sleep = apollo3_cpu_suspend;
	suspend_set_ops(&apollo3_pm_ops);
	return;
}
