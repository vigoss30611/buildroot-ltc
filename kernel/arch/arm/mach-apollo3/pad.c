...
static int pull_get(uint16_t index) {
	/*
 	 * apollo3 has fixed pull mode, software
...
static int imapx_module_set_iomux(const char *module) {
	int i = 0;
	uint8_t tmp;
	struct module_mux_info *muxinfo = imapx_module_mux_info, *mux1info = imapx_module_mux1_info;
	...
	for(i = 0 ; i < ARRAY_SIZE(imapx_module_mux1_info); i++) {
		if(!strcmp(module, mux1info[i].name)) {
			tmp =  readb(PAD_IOMUX1_MODE);
			if(mux1info[i].muxval)
				tmp |= (1  << mux1info[i].muxbit);
			else
				tmp &= ~(1 << mux1info[i].muxbit);
			pr_info("[%s] set iomux1 %x\n", mux1info[i].name, tmp);
			writeb(tmp, PAD_IOMUX1_MODE);
			return 0;
		}
	}
	return -1;
