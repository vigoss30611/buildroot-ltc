/*
 *  linux/arch/arm/mach-apollo3/timer-cmn.c
 *
...
static long apollo3_cmn_get_clock_rate(const char *name)
{
	long rate;
 ...
 */
static irqreturn_t apollo3_cmn_timer_interrupt(int irq, void *dev_id)
{
...
static void apollo3_cmn_set_mode(enum clock_event_mode mode,
...
static struct irqaction cmn_timer_irq = {
	...
	.handler	= apollo3_cmn_timer_interrupt,
	.dev_id		= &cmn_clockevent,
};

void __init apollo3_cmn_timer_init(void __iomem *base, unsigned int irq,
	const char *name)
{
	struct clock_event_device *evt = &cmn_clockevent;
	long rate = apollo3_cmn_get_clock_rate(name);
...
void apollo3_cmn_timer_update(long x)
{
	long rate = apollo3_cmn_get_clock_rate(cmn_clockevent.name);
	clkevt_reload = DIV_ROUND_CLOSEST(rate, HZ);
	cmn_clockevent.mult = div_sc(rate, NSEC_PER_SEC, cmn_clockevent.shift);
	if (cmn_enabled == 1)
		apollo3_cmn_set_mode(CLOCK_EVT_MODE_PERIODIC, &cmn_clockevent);
}
#endif

int apollo3_cmn_timer_resume(void)
{
	apollo3_cmn_set_mode(CLOCK_EVT_MODE_PERIODIC, NULL);
	return 0;
}
