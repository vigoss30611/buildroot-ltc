/***************************************************
** arch/arm/mach-apollo3/devices.c
**
...
#ifdef CONFIG_ADC_KEYS_IP6303

static struct adc_keys_platform_data ip6303_adc_button_data;

static struct platform_device ip6303_adc_button_device = {
	.name 		= "ip6303-adc-keys",
	.id 		= -1,
	.dev		= {
		.platform_data	= &ip6303_adc_button_data,
	}
};
...
static struct i2c_board_info __initdata ip6313_info[] = {
	{
		I2C_BOARD_INFO("ip6313", 0x30),
		//.irq		= IP6208_PMIC_IRQ,
		.platform_data = &ip6313_platform,
	},
};
...
#ifdef CONFIG_APOLLO3_FPGA_PLATFORM
...
void __init imapx_hwcfg_fix_to_device(struct module_parse_status *mps, int nr)
{
	...
	for (i = 0; i < nr; i++) {
...
#endif 
#ifdef CONFIG_ADC_KEYS_IP6303
			memset(&ip6303_adc_button_data, 0x0, sizeof(struct adc_keys_platform_data));
			for (j = 0, k = 0; j < MAX_ADC_KEYS_NUM; j++) {
				if (adc_buttons_cfg[j].valid ) {
					memcpy(&ip6303_adc_button_data.buttons[k], &adc_buttons_cfg[j]
						, sizeof(struct adc_keys_button));
					k++;
					ip6303_adc_button_data.nbuttons = k;
				}
			}		
#endif 
		...
		case IMAPX_PMU:
			ip6313_platform.ip = &imapx_pmu_cfg;
...
void __init apollo3_init_devices(void)
{
...
#if defined(CONFIG_APOLLO3_LCD_SPI_ILI8961)
	spi_register_board_info(spi_lcd_info, ARRAY_SIZE(spi_lcd_info));
#endif

#if defined(CONFIG_APOLLO3_LCD_SPI_OTA5182A)
	...
	if(!strcmp(imapx_pmu_cfg.name ,"ip6313")){
		i2c_register_board_info(0, ip6313_info, ARRAY_SIZE(ip6313_info));
	}
#endif
#ifdef CONFIG_APOLLO3_MFD_IP6303
	if(!strcmp(imapx_pmu_cfg.name ,"ip6303")){
