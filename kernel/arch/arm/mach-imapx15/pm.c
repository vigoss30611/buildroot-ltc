...
extern void imapx15_cpu_resume(void);
extern int imapx15_cpu_save(unsigned long *saveblk, long);
...
static void imapx15_gic_save(void)
{
	int i = 0;
	int length = 0;
	int count = 0;
...
static void imapx15_gic_restore(void)
{
	int i = 0;
	int length = 0;
	int count = 0;
...
static void imapx15_gic_disable(void)
{
...
void imapx15_pm_cb_flushcache(void)
{
	flush_cache_all();
	outer_flush_all();
}

static int imapx15_pm_prepare(void)
{
	rtcbit_set("resetflag", BOOT_ST_RESUME);
	rtcbit_set("sleeping", 1);
	pcloc_value = readl(0xc00000e0);
	writel(virt_to_phys(imapx15_cpu_resume), 0xc00000e0);
	return 0;
}
...
static void imapx15_module_power_on(void)
{
	int val;
...
static int imapx15_cpu_suspend(unsigned long arg)
{
	...
	PM_INFO("---> step 2 <---\n");
	imapx15_enter_lowpower();
	panic("sleep resumed to originator?");
}

static int imapx15_pm_enter(suspend_state_t state)
{
	if (item_equal("board.arch", "a5pv20", 0))
		imap_sram_reinit();
	...
	imapx15_gpio_restore();

	return 0;
}

static void imapx15_pm_finish(void)
{
	PM_INFO("++ %s ++\n", __func__);
}

static void imapx15_pm_wake(void)
{
	imapx15_module_power_on();
	/* for uart resume */
...
static const struct platform_suspend_ops imapx15_pm_ops = {
	.enter = imapx15_pm_enter,
	.prepare = imapx15_pm_prepare,
	.finish = imapx15_pm_finish,
	.valid = suspend_valid_only_mem,
	.wake = imapx15_pm_wake,
};
...
{
	PM_INFO("++ %s ++\n", __func__);
	pm_cpu_sleep = imapx15_cpu_suspend;
	suspend_set_ops(&imapx15_pm_ops);
	return;
}
