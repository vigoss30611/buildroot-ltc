...
static struct gpio_led leds_data[BOARD_LED_NUMS] = {
	[0] = {
		.name = "led0",
		.gpio = -1,
		.default_state = 0,
...
struct platform_device imapx15_nand_device = {
	.name = "infotm_imapx15_nand",
	.id = -1,
	.num_resources = ARRAY_SIZE(imap_nand_resource),
	.resource = imap_nand_resource,
	.dev = {
...
static struct resource imap_g2d_resource[] = {
	[0] = {
		.start  = G2D_BASE_ADDR,
		.end    = G2D_BASE_ADDR + G2D_BASE_SIZE -1,
		.flags  = IORESOURCE_MEM,
	},
	[1] = {
		.start  = SYSMGR_G2D_BASE,
		.end    = SYSMGR_G2D_BASE + 0x100 -1,
		.flags  = IORESOURCE_MEM,
	},
	[2] = {
		.start  = G2D_MMU_BASE_ADDR,
		.end    = G2D_MMU_BASE_ADDR + G2D_MMU_BASE_SIZE -1,
		.flags  = IORESOURCE_MEM,
	},
	[3] = {
		.start  = GIC_G2D_ID,
...
static struct platform_device *imap_devices[] __initdata = {
	...
	/*	&imap_iic4_device,
		&imap_iic5_device, */   /* imapx15 only have 4 I2C */
...
{
	imapx_pad_set_value(113, !!command); /* CS0 */
...
static void imapx_audio_data_register(struct audio_register *data, int id)
{
	...
	data->imapx_audio_cfg_init(data->codec_name,
			data->cpu_dai_name, subdata->i2s_mode, imapx_audio_cfg.spdif_en, id);
}
			...
			case IMAPX_AUDIO:
				for (j = 0; j < ARRAY_SIZE(audio); j++) {
					if (!strcmp(audio[j].codec_name,
						...
						imapx_audio_data_register(&audio[j], j);
