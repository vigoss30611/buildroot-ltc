/*
 *  linux/arch/arm/mach-coronampw/timer-cmn.c
 *
...
static long coronampw_cmn_get_clock_rate(const char *name)
{
	long rate;
 ...
 */
static irqreturn_t coronampw_cmn_timer_interrupt(int irq, void *dev_id)
{
...
static void coronampw_cmn_set_mode(enum clock_event_mode mode,
...
static struct clock_event_device cmn_clockevent = {
	...
	.set_mode	= coronampw_cmn_set_mode,
	.rating		= 250,
...
static struct irqaction cmn_timer_irq = {
	...
	.handler	= coronampw_cmn_timer_interrupt,
	.dev_id		= &cmn_clockevent,
};

void __init coronampw_cmn_timer_init(void __iomem *base, unsigned int irq,
	const char *name)
{
	struct clock_event_device *evt = &cmn_clockevent;
	long rate = coronampw_cmn_get_clock_rate(name);
...
void coronampw_cmn_timer_update(long x)
{
	long rate = coronampw_cmn_get_clock_rate(cmn_clockevent.name);
	clkevt_reload = DIV_ROUND_CLOSEST(rate, HZ);
	cmn_clockevent.mult = div_sc(rate, NSEC_PER_SEC, cmn_clockevent.shift);
	if (cmn_enabled == 1)
		coronampw_cmn_set_mode(CLOCK_EVT_MODE_PERIODIC, &cmn_clockevent);
}
#endif

int coronampw_cmn_timer_resume(void)
{
	coronampw_cmn_set_mode(CLOCK_EVT_MODE_PERIODIC, NULL);
	return 0;
}
