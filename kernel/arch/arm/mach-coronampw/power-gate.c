/* arch/arm/mach-coronampw/power-gate.c
 *
...
int module_reset(uint32_t module_addr, uint32_t reset_sel)
{
	...
	/* module syncrst clk gate en */
	__pow_writel( __pow_readl(SYS_SYNCRST_CLK_EN(module_addr)) | (1 << 7), SYS_SYNCRST_CLK_EN(module_addr));
	__pow_writel(reset_sel, SYS_SOFT_RESET(module_addr));
	do {} while (i--);
	__pow_writel(DISABLE, SYS_SOFT_RESET(module_addr));
	/* module syncrst clk gate diable */
//	__pow_writel( __pow_readl(SYS_SYNCRST_CLK_EN(module_addr)) & (~(1 << 7)), SYS_SYNCRST_CLK_EN(module_addr));
...
int __module_power_on(uint32_t module_addr, int lite)
{
	...
	if(module_addr == SYSMGR_ADC_BASE)
	...
	/* module syncrst clk gate diable */
//	__pow_writel( __pow_readl(SYS_SYNCRST_CLK_EN(module_addr)) & (~(1 << 7)), SYS_SYNCRST_CLK_EN(module_addr));
	if(module_addr == SYSMGR_SPIMUL_BASE)
		__pow_writel(0x1, module_addr + 0x20);
	/*for coronampw, This Bit ctrl Iram for post*/
	if(module_addr == SYSMGR_ISPOST_BASE)
		__pow_writel(0x2, module_addr + 0x20);
...
{
	__pow_writel(cfg, SYS_SPEC_FLAG(addr));
    return 1;
}
...
int module_power_down(uint32_t module_addr)
{
	...
	if(module_addr == SYSMGR_SPIMUL_BASE)
		__pow_writel(0x0, module_addr + 0x20);
...
void coronampw_reset_core(int index, uint32_t hold_base)
{
	uint32_t tmp;                                        
...
void coronampw_reset_module(void)
{
