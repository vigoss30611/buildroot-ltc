...
extern void coronampw_cpu_resume(void);
extern int coronampw_cpu_save(unsigned long *saveblk, long);
...
static void coronampw_gic_save(void)
{
	int i = 0;
	int length = 0;
	int count = 0;
...
static void coronampw_gic_restore(void)
{
	int i = 0;
	int length = 0;
	int count = 0;
...
static void coronampw_gic_disable(void)
{
...
void coronampw_pm_cb_flushcache(void)
{
	flush_cache_all();
	outer_flush_all();
}

static int coronampw_pm_prepare(void)
{
	rtcbit_set("resetflag", BOOT_ST_RESUME);
	rtcbit_set("sleeping", 1);
	pcloc_value = readl(phys_to_virt(IMAP_SDRAM_BASE + 0xe0));
	writel(virt_to_phys(coronampw_cpu_resume), phys_to_virt(IMAP_SDRAM_BASE + 0xe0));
	return 0;
}
...
static void coronampw_module_power_on(void)
{
	int val;
...
static int coronampw_cpu_suspend(unsigned long arg)
{
	...
	coronampw_gic_disable();
	coronampw_pm_cb_flushcache();
...
*/
	...
	pr_err("---> step 2 <---\n");
	coronampw_enter_lowpower();
	panic("sleep resumed to originator?");
}

static int coronampw_pm_enter(suspend_state_t state)
{
	if (item_equal("board.arch", "a5pv20", 0) 
			|| item_equal("board.arch", "q3pv10", 0))
		imap_sram_reinit();
	...
	coronampw_gic_restore();
	pad_case_resume();
	module_power_on(SYSMGR_GPIO_BASE);
	coronampw_gpio_restore();

	return 0;
}

static void coronampw_pm_finish(void)
{
	PM_INFO("++ %s ++\n", __func__);
}

static void coronampw_pm_wake(void)
{
	coronampw_module_power_on();
	/* for uart resume */
...
static const struct platform_suspend_ops coronampw_pm_ops = {
	.enter = coronampw_pm_enter,
	.prepare = coronampw_pm_prepare,
	.finish = coronampw_pm_finish,
	.valid = suspend_valid_only_mem,
	.wake = coronampw_pm_wake,
};
...
{
	PM_INFO("++ %s ++\n", __func__);
	pm_cpu_sleep = coronampw_cpu_suspend;
	suspend_set_ops(&coronampw_pm_ops);
	return;
}
