...
#define IMAP_DEBUG_UART_BASE	0x20803000
#define IMAP_DEBUG_BAUDRATE	115200
//#define IMAP_DEBUG_UART_CLK	23076923
#define CONFIG_FPGA_UART_CLK	24000000
#define IMAP_DEBUG_UART_CLK	99000000
#define CORONAMPW_SYSMGR_PAD_BASE	0x2d009000
...
static void gpio_func_mode(int io_index)
{
    ...
    cnt = io_index/8;
    num = io_index&0x7;
    val = __raw_readl(CORONAMPW_SYSMGR_PAD_BASE + 0x64 + cnt*4);
    val &=~(1<<num);
    __raw_writel(val, CORONAMPW_SYSMGR_PAD_BASE + 0x64 + cnt*4);
}
...
static inline void arch_decomp_setup(void)
{
	 ...
	 */
	//volatile uint8_t *uart_reg = (volatile uint8_t *)IMAP_DEBUG_UART_BASE;
	int32_t val, clk, baudrate;
	int32_t ibr, fbr;
#if defined(CONFIG_CORONAMPW_FPGA_PLATFORM)
	clk  = CONFIG_FPGA_UART_CLK; 
#else
	clk  = IMAP_DEBUG_UART_CLK; 
#endif
	baudrate = IMAP_DEBUG_BAUDRATE;

	module_init_cfg(0x2d012000);
	gpio_func_mode(86);
	gpio_func_mode(87);
