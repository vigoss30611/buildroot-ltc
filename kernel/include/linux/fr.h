...
struct fr {
	...
	void *fp;
	int magic;
	unsigned long timeout; // units: jiffies
	atomic_t freed;
    int en_oldest;
};
#define FR_MAGIC 0x06121717
...
extern int fr_get_buf(struct fr* fr, struct fr_buf** buf);
extern int fr_put_buf(struct fr* fr, struct fr_buf* buf);
extern int fr_get_ref(struct fr* fr, struct fr_buf** ref);
extern int fr_put_ref(struct fr *fr, struct fr_buf *buf);
extern int fr_float_setmax(struct fr *fr, int max);
extern void fr_set_timeout(struct fr *fr, int timeout_in_ms);
