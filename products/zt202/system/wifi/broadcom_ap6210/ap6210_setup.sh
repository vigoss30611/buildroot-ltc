...
AP_PATH="/mnt1/wifi/broadcom_ap6210"
...
if [ ""$1 = "STA" ]; then
    ...
    insmod /mnt1/ko/bcmdhd.ko "firmware_path=$AP_PATH/firmware/fw_bcm43438a0.bin nvram_path=$AP_PATH/firmware/nvram_ap6212.txt iface_name=wlan0"
...
if [ ""$1 = "AP" ]; then        
	...
	busybox ifconfig wlan0 down
        rmmod bcmdhd
        insmod /mnt1/ko/bcmdhd.ko "firmware_path=$AP_PATH/firmware/fw_bcm43438a0_apsta.bin nvram_path=$AP_PATH/firmware/nvram_ap6212.txt iface_name=wlan0"
        echo 1 > /sys/class/rfkill/rfkill1/state
        cd $AP_PATH
	...
	if [ "$2" = "" ]; then
	    ssid="INFOTM_cardv_"`busybox ifconfig |grep wlan0| awk '{print $5}'| awk -F':' '{print $4 $5 $6}'`
	else
	    ssid=$2
	fi
