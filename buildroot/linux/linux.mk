...
ifeq ($(LINUX_VERSION),local)
LINUX_SRCDIR = $(BR2_LINUX_KERNEL_LOCAL_PATH)
LINUX_MAINTAINED = YES
else ifeq ($(LINUX_VERSION),custom)
...
LINUX_VERSION_PROBED = $(shell $(MAKE) $(LINUX_MAKE_FLAGS) -C $(LINUX_SRCDIR) \
					   O=$(LINUX_BUILDDIR) --no-print-directory -s kernelrelease)
...
KERNEL_ARCH_PATH=arch/x86
else ifeq ($(KERNEL_ARCH),x86_64)
KERNEL_ARCH_PATH=arch/x86
else
KERNEL_ARCH_PATH=arch/$(KERNEL_ARCH)
endif
...
LINUX_IMAGE_PATH=$(LINUX_BUILDDIR)/$(KERNEL_ARCH_PATH)/boot/images/$(LINUX_IMAGE_NAME)
else
LINUX_IMAGE_PATH=$(LINUX_BUILDDIR)/$(KERNEL_ARCH_PATH)/boot/$(LINUX_IMAGE_NAME)
endif
...
define LINUX_APPLY_PATCHES
	for p in $(LINUX_PATCHES) ; do \
		if echo $$p | grep -q -E "^ftp://|^http://" ; then \
			buildroot/support/scripts/apply-patches.sh $(LINUX_SRCDIR) $(DL_DIR) `basename $$p` ; \
		elif test -d $$p ; then \
			buildroot/support/scripts/apply-patches.sh $(LINUX_SRCDIR) $$p linux-\*.patch ; \
		else \
			buildroot/support/scripts/apply-patches.sh $(LINUX_SRCDIR) `dirname $$p` `basename $$p` ; \
		fi \
	done
endef
...
KERNEL_SOURCE_CONFIG = $(LINUX_SRCDIR)/$(KERNEL_ARCH_PATH)/configs/$(call qstrip,$(BR2_LINUX_KERNEL_DEFCONFIG))_defconfig
else ifeq ($(BR2_LINUX_KERNEL_USE_CUSTOM_CONFIG),y)
KERNEL_SOURCE_CONFIG = $(BR2_LINUX_KERNEL_CUSTOM_CONFIG_FILE)
endif

define LINUX_CONFIGURE_CMDS
	cp $(KERNEL_SOURCE_CONFIG) $(LINUX_SRCDIR)/$(KERNEL_ARCH_PATH)/configs/buildroot_defconfig
	$(TARGET_MAKE_ENV) $(MAKE1) $(LINUX_MAKE_FLAGS) -C $(LINUX_SRCDIR) \
		O=$(LINUX_BUILDDIR) buildroot_defconfig
	rm $(LINUX_SRCDIR)/$(KERNEL_ARCH_PATH)/configs/buildroot_defconfig
	...
	yes '' | $(TARGET_MAKE_ENV) $(MAKE1) $(LINUX_MAKE_FLAGS) -C $(LINUX_SRCDIR) \
		O=$(LINUX_BUILDDIR) oldconfig
endef

ifeq ($(BR2_LINUX_KERNEL_DTS_SUPPORT),y)
ifeq ($(BR2_LINUX_KERNEL_DTB_IS_SELF_BUILT),)
define LINUX_BUILD_DTB
	$(TARGET_MAKE_ENV) $(MAKE) $(LINUX_MAKE_FLAGS) -C $(LINUX_SRCDIR) $(KERNEL_DTBS) \
		O=$(LINUX_BUILDDIR)
endef
define LINUX_INSTALL_DTB
	# dtbs moved from arch/<ARCH>/boot to arch/<ARCH>/boot/dts since 3.8-rc1
	cp $(addprefix \
		$(LINUX_SRCDIR)/$(KERNEL_ARCH_PATH)/boot/$(if $(wildcard \
		$(addprefix $(LINUX_SRCDIR)/$(KERNEL_ARCH_PATH)/boot/dts/,$(KERNEL_DTBS))),dts/),$(KERNEL_DTBS)) \
		$(BINARIES_DIR)/
endef
...
define LINUX_APPEND_DTB
	if [ -e $(LINUX_BUILDDIR)/$(KERNEL_ARCH_PATH)/boot/$(KERNEL_DTS_NAME).dtb ]; then \
		cat $(LINUX_BUILDDIR)/$(KERNEL_ARCH_PATH)/boot/$(KERNEL_DTS_NAME).dtb; \
	else \
		cat $(LINUX_BUILDDIR)/$(KERNEL_ARCH_PATH)/boot/dts/$(KERNEL_DTS_NAME).dtb; \
	fi >> $(LINUX_BUILDDIR)/$(KERNEL_ARCH_PATH)/boot/zImage
endef
...
LINUX_APPEND_DTB += $(sep) MKIMAGE_ARGS=`$(MKIMAGE) -l $(LINUX_IMAGE_PATH) |\
        ...
        -d $(LINUX_BUILDDIR)/$(KERNEL_ARCH_PATH)/boot/zImage $(LINUX_IMAGE_PATH);
endif
endif
...
define LINUX_BUILD_CMDS
	$(if $(BR2_LINUX_KERNEL_USE_CUSTOM_DTS),
		cp $(BR2_LINUX_KERNEL_CUSTOM_DTS_PATH) $(LINUX_BUILDDIR)/$(KERNEL_ARCH_PATH)/boot/dts/)
	$(TARGET_MAKE_ENV) $(MAKE) $(LINUX_MAKE_FLAGS) -C $(LINUX_SRCDIR) \
		O=$(LINUX_BUILDDIR) $(LINUX_IMAGE_NAME)
	@if grep -q "CONFIG_MODULES=y" $(@D)/.config; then 	\
		$(TARGET_MAKE_ENV) $(MAKE) $(LINUX_MAKE_FLAGS) -C $(LINUX_SRCDIR) \
		O=$(LINUX_BUILDDIR) modules ;	\
	fi
	$(LINUX_BUILD_DTB)
	$(LINUX_APPEND_DTB)
endef
...
define LINUX_INSTALL_TARGET_CMDS
	...
	@if grep -q "CONFIG_MODULES=y" $(@D)/.config; then 	\
		$(TARGET_MAKE_ENV) $(MAKE1) $(LINUX_MAKE_FLAGS) -C $(LINUX_SRCDIR) \
		O=$(LINUX_BUILDDIR) modules_install; \
...
include $(sort $(wildcard buildroot/linux/linux-ext-*.mk))
