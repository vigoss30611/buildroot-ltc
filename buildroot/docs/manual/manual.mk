...
manual-update-lists: manual-check-dependencies-lists
	...
	$(Q)BR2_DEFCONFIG="" TOPDIR=$(TOPDIR) O=$(O)/docs/manual/.build \
		BR2_EXTERNAL=$(TOPDIR)/buildroot/support/dummy-external \
		python -B $(TOPDIR)/buildroot/support/scripts/gen-manual-lists.py

# we can't use suitable-host-package here because that's not available in
# the context of 'make release'
manual-check-dependencies:
	$(Q)if [ -z "$(shell buildroot/support/dependencies/check-host-asciidoc.sh)" ]; then \
...
$$(O)/docs/$(1)/$(1).$(4): docs/$(1)/$(1).txt \
	...
	$(Q)a2x $(6) -f $(2) -d book -L -r $(TOPDIR)/buildroot/docs/images \
	        -D $$(@D) $$(@D)/.build/$(1).txt
	-$(Q)rm -rf $$(@D)/.build
endef
