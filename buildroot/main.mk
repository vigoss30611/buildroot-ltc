...
# absolute path
TOPDIR:=$(shell pwd)
CONFIG_CONFIG_IN=buildroot/Config.in
CONFIG=buildroot/support/kconfig
DATE:=$(shell date +%Y%m%d)

# Compute the full local version string so packages can use it as-is
# Need to export it, so it can be got from environment in children (eg. mconf)
export BR2_VERSION_FULL:=$(BR2_VERSION)$(shell $(TOPDIR)/buildroot/support/scripts/setlocalversion)
...
# BR2_EXTERNAL forced to $(TOPDIR)/system
BR2_EXTERNAL = $(TOPDIR)/system

BR2_EXTERNAL_SOURCE:=$(BR2_EXTERNAL)/

BUILD_DIR:=$(BASE_DIR)/build
STAMP_DIR:=$(BASE_DIR)/stamps
BINARIES_DIR:=$(BASE_DIR)/images
TARGET_DIR:=$(BASE_DIR)/system
TARGET_INITRD_DIR:=$(BASE_DIR)/root
...
GNU_HOST_NAME:=$(shell buildroot/support/gnuconfig/config.guess)
...
TARGET_SKELETON=$(TOPDIR)/buildroot/system/skeleton
...
include buildroot/Makefile.legacy
endif

include buildroot/package/Makefile.in
include buildroot/support/dependencies/dependencies.mk

# We also need the various per-package makefiles, which also add
# each selected package to TARGETS if that package was selected
# in the .config file.
include buildroot/toolchain/helpers.mk
include buildroot/toolchain/*/*.mk

# Include the package override file if one has been provided in the
# configuration.
PACKAGE_OVERRIDE_FILE=$(call qstrip,$(BR2_PACKAGE_OVERRIDE_FILE))
ifneq ($(PACKAGE_OVERRIDE_FILE),)
-include buildroot/$(PACKAGE_OVERRIDE_FILE)
endif

include $(sort $(wildcard buildroot/package/*/*.mk))

include buildroot/boot/common.mk
include buildroot/linux/linux.mk
include buildroot/minlinux/minlinux.mk
include buildroot/system/system.mk
...
include buildroot/fs/common.mk
...
$(HOST_DEPS): $(BASE_TARGETS)
	@

dirs: $(BUILD_DIR) $(STAGING_DIR) $(TARGET_DIR) $(TARGET_INITRD_DIR) \
...
.PHONY: all world toolchain dirs clean distclean source outputmakefile \
	...
	$(BUILD_DIR) $(STAGING_DIR) $(TARGET_DIR) $(TARGET_INITRD_DIR) \
	$(HOST_DIR) $(BINARIES_DIR) $(STAMP_DIR)
...
$(BUILD_DIR)/.root:
	mkdir -p $(TARGET_INITRD_DIR)
	#rsync -a $(RSYNC_VCS_EXCLUSIONS) \
	#	--exclude .empty --exclude '*~' \
	#	$(TARGET_SKELETON)/ $(TARGET_INITRD_DIR)/
	#cp buildroot/support/misc/target-dir-warning.txt $(TARGET_DIR_WARNING_FILE)
	@mkdir -p $(TARGET_INITRD_DIR)/lib
	@ln -snf lib $(TARGET_INITRD_DIR)/$(LIB_SYMLINK)
	@mkdir -p $(TARGET_INITRD_DIR)/usr
	@ln -snf lib $(TARGET_INITRD_DIR)/usr/$(LIB_SYMLINK)
	touch $@
...
$(TARGET_DIR): $(BUILD_DIR)/.system
endif

$(TARGET_INITRD_DIR): $(BUILD_DIR)/.root

STRIP_FIND_CMD = find $(TARGET_DIR) $(TARGET_INITRD_DIR)
...
ifneq ($(BR2_HAVE_DOCUMENTATION),y)
	...
	if [ "`ls -A $(TARGET_DIR)/usr/share`" = "" ]; then \
...
ifeq ($(BR2_PACKAGE_PYTHON_PYC_ONLY),y)
	...
	$(STRIP_FIND_CMD) | xargs -i $(STRIPCMD) {} 2>/dev/null || echo "AAAAAAAAAAAAAAAAAAA->STRIP OK" true
...
toolchain-eclipse-register:
	./buildroot/support/scripts/eclipse-register-toolchain `readlink -f $(O)` $(notdir $(TARGET_CROSS)) $(BR2_ARCH)
...
legal-info: dirs legal-info-clean legal-info-prepare $(TARGETS_LEGAL_INFO) \
	...
	@cat buildroot/support/legal-info/README.header >>$(LEGAL_REPORT)
	@if [ -r $(LEGAL_WARNINGS) ]; then \
		cat buildroot/support/legal-info/README.warnings-header \
...
%_defconfig: $(BUILD_DIR)/buildroot-config/conf $(TOPDIR)/buildroot/configs/%_defconfig outputmakefile
	@mkdir -p $(BUILD_DIR)/buildroot-config
	@$(COMMON_CONFIG_ENV) $< --defconfig=$(TOPDIR)/buildroot/configs/$@ $(CONFIG_CONFIG_IN)

%_defconfig: $(BUILD_DIR)/buildroot-config/conf $(BR2_EXTERNAL)/configs/%_defconfig outputmakefile
	@mkdir -p $(BUILD_DIR)/buildroot-config
	@$(COMMON_CONFIG_ENV) $< --defconfig=$(BR2_EXTERNAL)/configs/$@ $(CONFIG_CONFIG_IN)

%_defconfig: $(BUILD_DIR)/buildroot-config/conf $(TOPDIR)/output/product/configs/%_defconfig outputmakefile
	@mkdir -p $(BUILD_DIR)/buildroot-config
	@$(COMMON_CONFIG_ENV) $< --defconfig=$(TOPDIR)/output/product/configs/$@ $(CONFIG_CONFIG_IN)
...
outputmakefile:
ifeq ($(NEED_WRAPPER),y)
	$(Q)$(TOPDIR)/buildroot/support/scripts/mkmakefile $(TOPDIR) $(O)
endif
...
clean:
	rm -rf $(TARGET_DIR) $(BINARIES_DIR) $(HOST_DIR) \
		...
		$(LEGAL_INFO_DIR) $(TARGET_INITRD_DIR) \
		$(BASE_DIR)/product/system/root/.ispddk \
		$(BASE_DIR)/product/items.itm.tmp
...
ifeq ($(BR2_MINLINUX_KERNEL),y)
	@echo '  minlinux-menuconfig       - Run min Linux kernel menuconfig'
	@echo '  minlinux-savedefconfig    - Run min Linux kernel savedefconfig'
endif
...
include buildroot/docs/manual/manual.mk
