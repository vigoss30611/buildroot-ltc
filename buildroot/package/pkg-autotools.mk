...
define CONFIG_UPDATE
	for file in config.guess config.sub; do \
		for i in $$(find $(1) -name $$file); do \
			cp buildroot/support/gnuconfig/$$file $$i; \
		done; \
	done
endef
...
define UPDATE_CONFIG_HOOK
       ...
       $$(call CONFIG_UPDATE, $$($$(PKG)_SRCDIR))
endef
	...
	$(Q)if test "$$($$(PKG)_LIBTOOL_PATCH)" = "YES" \
		...
		for i in `find $$($$(PKG)_SRCDIR) -name ltmain.sh`; do \
			...
			if test $$$${ltmain_version} = '1.5'; then \
				buildroot/support/scripts/apply-patches.sh $$$${i%/*} buildroot/support/libtool buildroot-libtool-v1.5.patch; \
			elif test $$$${ltmain_version} = "2.2"; then\
				buildroot/support/scripts/apply-patches.sh $$$${i%/*} buildroot/support/libtool buildroot-libtool-v2.2.patch; \
			elif test $$$${ltmain_version} = "2.4"; then\
				buildroot/support/scripts/apply-patches.sh $$$${i%/*} buildroot/support/libtool buildroot-libtool-v2.4.patch; \
			fi \
		done \
	fi
endef
...
define MOVE_SRC_HOOK
@echo ">>>>>>>>>>>>>>>>>>>>>>sync code from source for build"
rsync -aui $$($$(PKG)_ORIG_SRCDIR)/* $$($$(PKG)_SRCDIR)/
endef


ifeq ($$($(2)_MAINTAINED),YES)
$(2)_ORIG_SRCDIR = $(BR2_EXTERNAL_SOURCE)/$(1)
$(2)_SRCDIR = $$($(2)_BUILDDIR)
$(2)_CONFDIR = $$($(2)_BUILDDIR)
$(2)_PRE_CONFIGURE_HOOKS += MOVE_SRC_HOOK
$(2)_PRE_BUILD_HOOKS += MOVE_SRC_HOOK
endif
