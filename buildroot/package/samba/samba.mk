...
define SAMBA_INSTALL_INITSCRIPTS_CONFIG
	...
	@if [ ! -f $(TARGET_DIR)/etc/init.d/S91smb ]; then \
		$(INSTALL) -m 0755 -D buildroot/package/samba/S91smb $(TARGET_DIR)/etc/init.d/S91smb; \
	fi
	# install config
	@if [ ! -f $(TARGET_DIR)/etc/samba/smb.conf ]; then \
		$(INSTALL) -m 0755 -D buildroot/package/samba/simple.conf $(TARGET_DIR)/etc/samba/smb.conf; \
	fi
endef
