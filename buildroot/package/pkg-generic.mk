...
# Only show the download message if it isn't already downloaded
	...
	$(if $(filter YES,$($(PKG)_MAINTAINED)),\
		$(info "Downloading not needed"),\
		$(call do_download)\
	)
ifeq ($(DL_MODE),DOWNLOAD)
	$(Q)mkdir -p $(@D)
...
# find the package directory (typically buildroot/package/<pkgname>) and the
# prefix of the patches
define do_patch
	@$(call step_start,patch)
	$(foreach hook,$($(PKG)_PRE_PATCH_HOOKS),$(call $(hook))$(sep))
	$(foreach p,$($(PKG)_PATCH),buildroot/support/scripts/apply-patches.sh $($(PKG)_SRCDIR) $(DL_DIR) $(notdir $(p))$(sep))
	$(Q)( \
	for D in $(PATCH_BASE_DIRS); do \
	  if test -d $${D}; then \
	    if test -d $${D}/$($(PKG)_VERSION); then \
	      buildroot/support/scripts/apply-patches.sh $($(PKG)_SRCDIR) $${D}/$($(PKG)_VERSION) \*.patch \*.patch.$(ARCH) || exit 1; \
	    else \
	      buildroot/support/scripts/apply-patches.sh $($(PKG)_SRCDIR) $${D} \*.patch \*.patch.$(ARCH) || exit 1; \
	    fi; \
	  fi; \
	done; \
	)
...
ifeq ($($(2)_MAINTAINED),YES)
$(2)_SRCDIR				?= $(BR2_EXTERNAL_SOURCE)/$(1)
$(2)_CONFDIR			=  $$($(2)_SRCDIR)
$(2)_BUILDDIR		    ?= $$($(2)_DIR)
else
...
else ifeq ($(1),minlinux)
$(2)_KCONFIG_VAR = BR2_MINLINUX_KERNEL
else ifeq ($(4),buildroot/boot/uboot)
$(2)_KCONFIG_VAR = BR2_TARGET_$(2)
else ifeq ($(4),buildroot/toolchain/)
