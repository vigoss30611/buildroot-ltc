...
define NTP_INSTALL_TARGET_CMDS
	...
	$(if $(BR2_PACKAGE_NTP_NTPD), install -m 755 buildroot/package/ntp/S49ntp $(TARGET_DIR)/etc/init.d/S49ntp)
	@if [ ! -f $(TARGET_DIR)/etc/default/ntpd ]; then \
		install -m 755 -d $(TARGET_DIR)/etc/default ; \
		install -m 644 buildroot/package/ntp/ntpd.etc.default $(TARGET_DIR)/etc/default/ntpd ; \
	fi
endef
