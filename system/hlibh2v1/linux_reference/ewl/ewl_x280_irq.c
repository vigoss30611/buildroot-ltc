...
i32 h2_EWLReserveHw(const void *inst)
{
    ...
    if(h2_binary_semaphore_wait(enc->semid, 0) != 0)
    {
        PTRACE("h2_binary_semaphore_wait error: %s\n", strerror(errno));
        return EWL_ERROR;
    }
    ...
    if(h2_binary_semaphore_wait(enc->semid, 1) != 0)
    {
        PTRACE("h2_binary_semaphore_wait error: %s\n", strerror(errno));
        if(h2_binary_semaphore_post(enc->semid, 0) != 0)
        {
            ...
            cnt++;
            if(cnt <100){
                PTRACE("h2_binary_semaphore_post error: %s\n", strerror(errno));
            }
            //assert(0);
        }
        return EWL_ERROR;
    ...
    PTRACE("h2_EWLReserveHw: ENC HW locked by PID %d\n", getpid());
