...
static HEVCEncInst g_hevc_encoder;

/* SW/HW shared memories for input/output buffers */
static EWLLinearMem_t gInPictureMem;
static EWLLinearMem_t gNoUsePictureStabMem;
static EWLLinearMem_t gOutBufMem;
static i32 gOutBufSizeMax;               /* gOutBufMem max in bytes */
static EWLLinearMem_t gScaledPictureMem;
static u32 gMultislice_encoding;
static u32 gOutput_byte_stream;
static FILE *gOutStreamFile;
  HEVCEncIn gEncIn;
  HEVCEncOut gEncOut;

static i32 encode_start(struct test_bench *tb, HEVCEncInst encoder, commandLine_s *cml);
static i32 encode_in_progress(struct test_bench *tb, HEVCEncInst encoder, commandLine_s *cml);
static i32 encode_end(struct test_bench *tb, HEVCEncInst encoder, commandLine_s *cml);
...
static void h265_init_tb( struct test_bench *pTb, commandLine_s cml)
{
  memset(pTb, 0, sizeof(struct test_bench));
  pTb->input = cml.input;
  pTb->output     = cml.output;
  pTb->first_picture    = cml.first_picture;
  pTb->last_picture   = cml.last_picture;
  pTb->ifr_num      = cml.ifr_num;
  pTb->ifr_den      = cml.ifr_den;
  pTb->ofr_num      = cml.ofr_num;
  pTb->ofr_den      = cml.ofr_den;
  pTb->test_data_files    = cml.test_data_files;
  pTb->width      = cml.width;
  pTb->height     = cml.height;
  pTb->idr_interval   = cml.intraPicRate;
  pTb->byteStream   = cml.byteStream;
  pTb->interlacedFrame = cml.interlacedFrame;

}

static int h265_init( struct test_bench  *pTb, commandLine_s  cml)
{
  ...
  int ret;
  if ((ret = OpenEncoder(&cml, &g_hevc_encoder)) != 0)
  {
    printf("OpenEncoder error:%d\n", ret);
    return -ret;
  }

  h265_init_tb(pTb, cml);
  ...
  HEVCEncSetTestId(g_hevc_encoder, cml.testId);

  /* Allocate input and output buffers */
  if (AllocRes(&cml, g_hevc_encoder) != 0)
  {
    FreeRes(g_hevc_encoder);
    CloseEncoder(g_hevc_encoder);
...
int main(i32 argc, char **argv)
{
  ...
  cml.input = (argv[3]);
  cml.inputFormat = 0;
  gMultislice_encoding = (cml.sliceSize!=0&&(((cml.height+63)/64)>cml.sliceSize))?1:0;
  gOutput_byte_stream = cml.byteStream?1:0;

  struct test_bench  tb;
  h265_init(&tb, cml); 

  ret = encode_start(&tb, g_hevc_encoder, &cml);
  encode_in_progress(&tb, g_hevc_encoder, &cml);
    encode_end(&tb, g_hevc_encoder, &cml);
...
static int getCtbRows(int heigh)
{
    return (heigh+63)/64;
}

int getSrcImgSize (commandLine_s *cml)
{
    return  cml->lumWidthSrc * cml->lumHeightSrc *
...
/*------------------------------------------------------------------------------
  encode
------------------------------------------------------------------------------*/
i32 encode_start(struct test_bench *tb, HEVCEncInst encoder, commandLine_s *cml)
{
  printf("%s enter\n", __FUNCTION__);
  u32 frameCntTotal = 0;
  ...
  gEncIn.timeIncrement = 0;
  gEncIn.busOutBuf = gOutBufMem.busAddress;
  gEncIn.outBufSize = gOutBufMem.size;
  gEncIn.pOutBuf = gOutBufMem.virtualAddress;
  gOutStreamFile = tb->out;

  /* Video, sequence and picture parameter sets */
  { 
    printf("%s -> HEVCEncStrmStart\n", __FUNCTION__);
    if (HEVCEncStrmStart(encoder, &gEncIn, &gEncOut))
    {
    ...
    if (cml->byteStream == 0)
    {
      //WriteNalSizesToFile(nalfile, gEncOut.pNaluSizeBuf,
      //                    gEncOut.numNalus);
      WriteNals(tb->out, gOutBufMem.virtualAddress, gEncOut.pNaluSizeBuf,
                gEncOut.numNalus, 0);
    }
    else
    {
      WriteStrm(tb->out, gOutBufMem.virtualAddress, gEncOut.streamSize, 0);
    }
    //total_bits += gEncOut.streamSize * 8;
    //streamSize += gEncOut.streamSize;
  } 
  ...
  { 
    ...
    gEncIn.busLuma = gInPictureMem.busAddress;
    tb->lum = (u8 *)gInPictureMem.virtualAddress;

    gEncIn.busChromaU = gEncIn.busLuma + size_lum;
    tb->cb = tb->lum + size_lum;
    gEncIn.busChromaV = gEncIn.busChromaU + size_ch;
    tb->cr = tb->cb + size_ch;
  }

  printf("Reading input from file <%s>,\n frame size %d bytes.\n",
         cml->input, getSrcImgSize(cml));
  tb->validencodedframenumber=0;
  //ctb_rows=(cml->height+63)/64;
  tb->GDR_interval=getCtbRows(cml->height)-1;
  if(tb->idr_interval>tb->GDR_interval) tb->GDR_interval=tb->idr_interval;
  

  return OK;

error:
  Error(2, ERR, "encode_start() fails");
...
static int encode_end(struct test_bench *tb, HEVCEncInst encoder, commandLine_s *cml)
{
    printf("%s enter\n", __FUNCTION__);
      // end  
    int ret = HEVCEncStrmEnd(encoder, &gEncIn, &gEncOut);
    if (ret == HEVCENC_OK)
    {
      if (cml->byteStream == 0)
      {
        //WriteNalSizesToFile(nalfile, gEncOut.pNaluSizeBuf,gEncOut.numNalus);
        WriteNals(tb->out, gOutBufMem.virtualAddress, gEncOut.pNaluSizeBuf,
                  gEncOut.numNalus, 0);
      }
      else
...
static int encode_in_progress(struct test_bench *tb, HEVCEncInst encoder, commandLine_s *cml)
{
    printf("%s enter\n", __FUNCTION__);
    ...
    while(true){
        #if 0
    if (read_picture(tb, cml->inputFormat, getSrcImgSize(cml), cml->lumWidthSrc, cml->lumHeightSrc))
    {
    ...
    //frameCntTotal++;
    #endif
    ret = read_picture2(tb, cml->inputFormat, getSrcImgSize(cml), cml->lumWidthSrc, cml->lumHeightSrc);
    printf("encode_in_progress read_picture2 return:%d\n", ret);
    
    {
        printf("%s dynamicenableGDR==0 enableGDR==0\n", __FUNCTION__);
      if (tb->idr_interval == 0)
      {
        gEncIn.codingType = (gEncIn.poc == 0) ? HEVCENC_INTRA_FRAME : HEVCENC_PREDICTED_FRAME;
      }
      else
      {
        gEncIn.codingType = ((gEncIn.poc % tb->idr_interval) == 0) ? HEVCENC_INTRA_FRAME : HEVCENC_PREDICTED_FRAME;
      }
    }
    
    printf("Encoding %i %s codeType =%d\n", tb->picture_cnt, tb->input, gEncIn.codingType);
    //select rps based on frame type
    if (gEncIn.codingType == HEVCENC_INTRA_FRAME)
    {
      //refrest IDR poc 
      gEncIn.poc = 0;
      //gEncIn.rps_id = 1;
    ...
    ret = HEVCEncStrmEncode(encoder, &gEncIn, &gEncOut, &HEVCSliceReady, NULL);
    gettimeofday(&endTime, 0);
    int diff = (endTime.tv_sec-startTime.tv_sec)*1000 + (endTime.tv_usec-startTime.tv_usec)/1000;
    printf("%s HEVCEncStrmEncode:%d, time:%dms(%dus)\n", __FUNCTION__, ret, diff, (endTime.tv_usec-startTime.tv_usec));
    switch (ret)
    {
      case HEVCENC_FRAME_READY:

        tb->picture_cnt++;
        if (gEncOut.streamSize == 0)
          break;
#ifdef TEST_DATA
        //EncTraceWriteProfile(encoder, gEncIn.poc);
        //write_recon_sw(encoder, gEncIn.poc);
        EncTraceUpdateStatus();
#endif
#ifdef TEST_DATA
        if(cml->outReconFrame==1)
          EncTraceRecon(encoder, gEncIn.poc);
#endif //TRACE_RECON

        HEVCEncGetRateCtrl(encoder, &rc);
        /* Write scaled encoder picture to file, packed yuyv 4:2:2 format */
        WriteScaled((u32 *)gEncOut.scaledPicture, encoder);
        if (gEncOut.streamSize != 0)
        {
          if((gMultislice_encoding==0)||(ENCH2_SLICE_READY_INTERRUPT==0)||(cml->hrdConformance==1))
          {
            if (cml->byteStream == 0)
            {
              //WriteNalSizesToFile(nalfile, gEncOut.pNaluSizeBuf,
          ...
          gEncIn.timeIncrement = tb->ofr_den;
...
int OpenEncoder(commandLine_s *cml, HEVCEncInst *pEnc)
{
  ...
  if (cml->scaledWidth * cml->scaledHeight)
  {
    if (h2_EWLMallocRefFrm(((struct hevc_instance *)encoder)->asic.ewl, cml->scaledWidth * cml->scaledHeight * 2,
                        &gScaledPictureMem) != EWL_OK)
    {
      gScaledPictureMem.virtualAddress = NULL;
      gScaledPictureMem.busAddress = 0;
  ...
  if ((ret = HEVCEncGetPreProcessing(encoder, &preProcCfg)) != HEVCENC_OK)
  {
    ...
    printf("HEVCEncGetPreProcessing ret:%d\n", ret);
    CloseEncoder(encoder);
    return -1;
  }
  printf
  ...
  preProcCfg.busAddressScaledBuff = gScaledPictureMem.busAddress;
  preProcCfg.virtualAddressScaledBuff = gScaledPictureMem.virtualAddress;
  preProcCfg.sizeScaledBuff = gScaledPictureMem.size;
  ...
  if ((ret = HEVCEncSetPreProcessing(encoder, &preProcCfg)) != HEVCENC_OK)
  {
    ...
    printf ("HEVCEncSetPreProcessing ret:%d\n", ret);
    CloseEncoder(encoder);
    return (int)ret;
  }
#endif
...
void CloseEncoder(HEVCEncInst encoder)
{
  ...
  if (gScaledPictureMem.virtualAddress != NULL)
    h2_EWLFreeLinear(((struct hevc_instance *)encoder)->asic.ewl, &gScaledPictureMem);
  ...
  while ((ret = get_option(argc, argv, options, &prm)) != -1)
  {
    ...
    switch (prm.short_opt)
    {
      ...
      case '0':
        ...
        if (strcmp(prm.longOpt, "gOutBufSizeMax") == 0)
          gOutBufSizeMax = atoi(p);
...
void default_parameter(commandLine_s *cml)
{
  ...
  cml->intraPicRate   = 5;  // only first is IDR.
...
i32 read_picture2(struct test_bench *tb, u32 inputFormat, u32 src_img_size, u32 src_width, u32 src_height)
{
    static int seekPos = 0;
    int ret = read(tb->in, tb->lum , seekPos, src_img_size);
    seekPos += src_img_size;
    printf("read_picture2 seekPos:%d\n", seekPos);
    return ret;
...
int AllocRes(commandLine_s *cmdl, HEVCEncInst enc)
{
  ...
  gInPictureMem.virtualAddress = NULL;
  gOutBufMem.virtualAddress = NULL;
  gNoUsePictureStabMem.virtualAddress = NULL;
  ...
  if (gOutBufSizeMax == 0) gOutBufSizeMax = 12;
  outbufSize = 4 * gInPictureMem.size < ((u32)gOutBufSizeMax * 1024 * 1024) ?
               4 * gInPictureMem.size : ((u32)gOutBufSizeMax * 1024 * 1024);
  ...
  printf("\tInput buffer bus address:   0x%08x\n", gInPictureMem.busAddress);
  printf("\tInput buffer user address:  %p\n", gInPictureMem.virtualAddress);
  printf("\tOutput buffer size:         %d bytes\n", gOutBufMem.size);
  printf("\tOutput buffer bus address:  0x%08x\n", gOutBufMem.busAddress);
  printf("\tOutput buffer user address: %p\n",  gOutBufMem.virtualAddress);
...
{
  if (gInPictureMem.virtualAddress != NULL)
    h2_EWLFreeLinear(((struct hevc_instance *)enc)->asic.ewl, &gInPictureMem);
  if (gNoUsePictureStabMem.virtualAddress != NULL)
    h2_EWLFreeLinear(((struct hevc_instance *)enc)->asic.ewl, &gNoUsePictureStabMem);
  if (gOutBufMem.virtualAddress != NULL)
    h2_EWLFreeLinear(((struct hevc_instance *)enc)->asic.ewl, &gOutBufMem);
}
/*------------------------------------------------------------------------------
...
void HEVCSliceReady(HEVCEncSliceReady *slice)
{
  ...
  if(gMultislice_encoding&&(ENCH2_SLICE_READY_INTERRUPT))
  {
    if (slice->slicesReadyPrev == 0)    /* New frame */
    {
        ...
        if (gOutput_byte_stream == 0)
        {
          WriteNals(gOutStreamFile, (u32*)strmPtr, slice->sliceSizes,
                    slice->nalUnitInfoNum+slice->slicesReady, 0);
        }
        else
        {

          WriteStrm(gOutStreamFile, (u32*)strmPtr, streamSize, 0);
        }
    ...
    else
    {
      ...
      if (gOutput_byte_stream == 0)
      {
        WriteNals(gOutStreamFile, (u32*)strmPtr, slice->sliceSizes+slice->nalUnitInfoNum+slice->slicesReadyPrev,
