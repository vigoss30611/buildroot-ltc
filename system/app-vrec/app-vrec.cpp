...
//    char temp[] = "REC_VIDEO_%ts-___%us-%ue_%c_%l";
    ...
    memset(info->videoextra_channel, 0, VPLAY_CHANNEL_MAX);
...
//            system(input);
            ...
            if (input[4] == ' '){
                ...
                printf("mute ->%d\n", value);
                vplay_mute_recorder(recorder, (VPlayMediaType)value, 1);
            }
        }
            ...
            if (input[6] == ' '){
                ...
                vplay_mute_recorder(recorder, (VPlayMediaType)value, 0);
            }
        }
        ...
        while((opt = getopt_long(argc, argv,"u:v:V:a:t:r:R:f:c:s:b:m:S:F:d:T:k:e:l:gi",recorder_long_options,&option_index)) != -1 ){
            ...
            switch (opt){
                ...
                case 'a' :
                    ...
                    break;
                case 't' :
                    g_stRecorderInfo.audio_format.type = (audio_codec_type_t)atoi(optarg);
                    break;
                case 'r' :
                    g_stRecorderInfo.audio_format.sample_rate = atoi(optarg);
                    break;
                case 'f' :
                    g_stRecorderInfo.audio_format.sample_fmt = (sample_format_t)atoi(optarg);
                    break;
                case 'c' :
