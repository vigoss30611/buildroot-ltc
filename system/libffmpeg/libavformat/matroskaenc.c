...
static int mkv_write_frames_info(AVFormatContext *s)
{
    char frame_info[100] = {0};
    AVIOContext *pb = s->pb;

    sprintf(frame_info, "infotm:");
    for (int i = 0; i < s->nb_streams && i<2; i++)
    {
         sprintf(frame_info, "%s%10lld:", frame_info, s->streams[i]->nb_frames);
    }
    avio_seek(pb, -1, SEEK_END);
    avio_write(pb, frame_info, strlen(frame_info));
    return 0;
}
