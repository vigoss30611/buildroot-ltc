...
static int matroska_parse_frames(AVFormatContext *s)
{
    #define FRAME_TAG_LENGTH 30

    MatroskaDemuxContext *matroska = s->priv_data;
    MatroskaTrack *tracks = matroska->tracks.elem;
    int64_t cur_pos = 0, file_pos = 0;
    char delims[] = ":";
    char frame_info[100] = {0};
    char *result = NULL;

    cur_pos = avio_tell(matroska->ctx->pb);
    file_pos = avio_size(matroska->ctx->pb) - FRAME_TAG_LENGTH;
    if (file_pos <= 0)
        return -1;
    avio_seek(matroska->ctx->pb, file_pos, SEEK_SET);
    avio_read(matroska->ctx->pb, frame_info, FRAME_TAG_LENGTH);

    result = strtok( frame_info, delims );
    if ((result) && (strstr(result, "infotm")))
    {
        for (int i = 0; (i < matroska->tracks.nb_elem) && (result != NULL); i++)
        {
            MatroskaTrack *track = &tracks[i];
            AVStream *st = track->stream;
            ...
            if (result)
            {
                st->frame_num = atoi(result);
            }
        }
    }
    avio_seek(matroska->ctx->pb, cur_pos, SEEK_SET);
    return 0;
}
