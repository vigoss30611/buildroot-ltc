...
static const AVOption options[] = {
    ...
    { "uuid_data",    "User uuid data", offsetof(MOVMuxContext, uuid_data),   AV_OPT_TYPE_STRING, {.str = NULL}, .flags = AV_OPT_FLAG_ENCODING_PARAM },
    { NULL },
};
...
static int mov_write_track_uuid_tag(AVIOContext *pb, MOVMuxContext *mov,
                                    MOVTrack *track)
{
    int64_t pos = avio_tell(pb);
    ...
    if ((mov->uuid_data) &&
        (strlen(mov->uuid_data) > 0) &&
        (track->enc->codec_type == AVMEDIA_TYPE_VIDEO))
    {
        avio_wb32(pb, strlen(mov->uuid_data) + 4);
        ffio_wfourcc(pb, "uuid");
        avio_write(pb, mov->uuid_data, strlen(mov->uuid_data));
    }
                              ...
                              MOVTrack *track, AVStream *st)
{
    ...
    if(track->private_control_flag != 0)
        ffio_wfourcc(pb, "infv");
    else
    ...
    mov_write_track_uuid_tag(pb, mov, track);
...
static int mov_write_user_data_tag(AVIOContext *pb, MOVMuxContext *mov,
                                     AVFormatContext *s)
{
	int size = 0;
	if(s->user_data == NULL || s->user_data_size <= 0){
		size = 33;
		avio_wb32(pb, 33); /* size */
		ffio_wfourcc(pb, "user");
		avio_wb32(pb, 0);
		avio_wb32(pb, 0);
		ffio_wfourcc(pb, "mdir");
		ffio_wfourcc(pb, "appl");
		...
		printf("###########fill user custom data ->%p %d\n", s->user_data, s->user_data_size);
    	int64_t pos = avio_tell(pb);
		size = s->user_data_size + 8;
		avio_wb32(pb, size); /* size */
		ffio_wfourcc(pb, "user");
		avio_write(pb, s->user_data, size);
		size = update_size(pb,pos );
	}
    return size;
}
                              ...
                              AVFormatContext *s)
{
    ...
    mov_write_user_data_tag(pb, mov, s);
    size = update_size(pb, pos);
    return size;
}
                              ...
                              AVFormatContext *s)
{
    ...
    for (i = 0; i < mov->nb_streams; i++) {
        if (mov->tracks[i].entry > 0 || mov->flags & FF_MOV_FLAG_FRAGMENT) {
            mov->tracks[i].private_control_flag = 0;
            if (mov->tracks[i].enc->codec_type == AVMEDIA_TYPE_VIDEO)
            {
	         mov->tracks[i].private_control_flag = mov->private_control_flag&(1<<i);
            }
...
static int mov_write_header(AVFormatContext *s)
{
    ...
    mov->fc = s;
    mov->private_control_flag = s->private_control_flag;
