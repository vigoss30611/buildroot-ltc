 ...
 **
 *******************************************************************************/
int app_ble_client_deregister(UINT16 client_num)
{
    tBSA_STATUS status;
    ...
    switch (event)
    {
        ...
        case BSA_BLE_CL_OPEN_EVT:
            ...
            if (p_data->cli_open.status == BSA_SUCCESS)
            {
                ...
                if(is_q6m_disc_device(p_data->cli_open.bd_addr) || is_q6m_device_by_attr(p_blecl_db_elmt))
                    q6m_handle_on_conn_up(p_data->cli_open.bd_addr, p_data->cli_open.conn_id);
                else
                    flood_bridge_handle_on_conn_up(0, p_data->cli_open.bd_addr, p_data->cli_open.conn_id);
            }
            break;
        ...
        case BSA_BLE_CL_CLOSE_EVT:
            ...
            flood_bridge_handle_on_conn_down(0, p_data->cli_close.remote_bda, p_data->cli_close.conn_id);
            q6m_handle_on_conn_down(p_data->cli_close.remote_bda);
            break;
        ...
        case BSA_BLE_CL_NOTIF_EVT:
            ...
            else  /* notification */
            {
                ...
                flood_bridge_handle_notification_data(p_data->cli_notif.conn_id, p_data->cli_notif.char_id.char_id, p_data->cli_notif.value, p_data->cli_notif.len);
                q6m_handle_notification_data(p_data->cli_notif.conn_id, 
                    p_data->cli_notif.char_id.srvc_id.id.uuid.uu.uuid16, 
                    p_data->cli_notif.char_id.char_id.uuid.uu.uuid16,
                    p_data->cli_notif.value, 
