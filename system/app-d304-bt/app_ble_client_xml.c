...
#define tAPP_BLE_CLIENT_XML_FILENAME "/config/bt/bt_ble_client_devices.xml"
#endif
...
int app_ble_client_xml_write(tAPP_BLE_CLIENT_DB_ELEMENT *p_element)
{
    ...
    int i;
#if defined(USE_IPC_FILE_OP)
    if ((fd = ipc_file_open_mode(tAPP_BLE_CLIENT_XML_FILENAME, O_WRONLY | O_CREAT | O_TRUNC, 0666)) < 0)
#else
