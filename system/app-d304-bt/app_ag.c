...
#define APP_AG_XML_REM_DEVICES_FILE_PATH "/config/bt/bt_devices.xml"
...
tBSA_STATUS app_ag_close(void)
{
    ...
    if (status != BSA_SUCCESS)
    {
        ...
        app_qiwo_ag_send_disconnect_rsp(status);
    }
...
void app_ag_cback(tBSA_AG_EVT event, tBSA_AG_MSG *p_data)
{
    ...
    case BSA_AG_OPEN_EVT: /* RFCOM level connection */
        ...
        else
        {
            ...
            app_qiwo_ag_send_connect_rsp(p_data->open.status); //qiwo
        }
    ...
    case BSA_AG_CONN_EVT: /* Service level connection */
        ...
        app_qiwo_ag_save_connect_addr_for_unpair(p_data->conn.bd_addr);
        app_ag_open_audio();
        break;
    ...
    case BSA_AG_CLOSE_EVT: /* Connection Closed (for info)*/
        ...
        app_ag_cb.opened = FALSE;
        app_qiwo_ag_send_disconnect_rsp(p_data->close.status); //qiwo
        break;
    ...
    case BSA_AG_AUDIO_OPEN_EVT: /* Audio Open Event */
        ...
        app_qiwo_ag_send_connect_rsp(p_data->hdr.status); //qiwo
        break;
...
#endif
		app_qiwo_ag_send_disconnect_rsp(p_data->hdr.status); //qiwo
        break;
