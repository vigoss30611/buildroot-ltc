...
/* Default local Name */
#define APP_DEFAULT_BT_NAME             "QiwoIPC"
...
#define APP_XML_CONFIG_FILE_PATH            "/config/bt/bt_config.xml"
#define APP_XML_REM_DEVICES_FILE_PATH       "/config/bt/bt_devices.xml"
...
int app_mgr_set_bt_config(BOOLEAN enable)
{
    ...
    bt_config.config_mask = BSA_DM_CONFIG_VISIBILITY_MASK | BSA_DM_CONFIG_NAME_MASK 
                          | BSA_DM_CONFIG_DEV_CLASS_MASK | BSA_DM_CONFIG_CHANNEL_MASK;
    ...
    sprintf(bt_addr, "%s/%x:%x:%x:%x:%x:%x", "bt_addr", bt_config.bd_addr[0], bt_config.bd_addr[1], bt_config.bd_addr[2], 
    				bt_config.bd_addr[3], bt_config.bd_addr[4], bt_config.bd_addr[5]);
    event_send("qiwo_ble_data", bt_addr, strlen(bt_addr) + 1);
    return 0;
}
...
int app_mgr_get_local_addr(BD_ADDR bd_addr)
{
    int status;
    tBSA_DM_GET_CONFIG bt_config;
    ...
    APP_DEBUG1("First host disabled channel:%d", bt_config.first_disabled_channel);
    APP_DEBUG1("Last host disabled channel:%d", bt_config.last_disabled_channel);
    bdcpy(bd_addr, bt_config.bd_addr);
...
int app_mgr_config(void)
{
    ...
    app_mgr_get_local_addr(local_bd_addr);
    ...
    if (status < 0)
    {
        ...
        //app_xml_config.bd_addr[4] = rand_r(&rand_seed);  /* henry li 20151128 */
        //app_xml_config.bd_addr[5] = rand_r(&rand_seed);  /* henry li 20151128 */
