 ...
 *
 * Description: PCM record & AEC demo
 *
 * Author:
...
#define WAV_FILE_PATH   "/mnt/sd0/record_aec_pcm.wav"
...
int main(int s32Argc, char** ps8Argv)
{
    ...
    audio_fmt_t stFmt = {
        .channels = 2,
        .bitwidth = 32,
        .samplingrate = 8000, //recommanded sample rate for AEC
        .sample_size = 1024,
    };
    ...
    printf("pcm_name: %s\n"
           ...
           "fmt.channels: %d\n"
           "time: %ds\n",
           ps8ChnName, s32Volume, ps8WavFile, stFmt.bitwidth,
    ...
    // enable AEC for recording
    s32Ret = audio_enable_aec(s32Handle, 1);
    if (s32Ret < 0) {
        printf("Enable AEC err: %d\n", s32Ret);
        goto err_put_chn;
    }
    ...
    // disable AEC for recording
    s32Ret = audio_enable_aec(s32Handle, 0);
