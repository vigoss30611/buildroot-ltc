...
{
    struct wifi_config_t stConnectStaConfig, *stForgetStaConfig, *stTempForgetStaConfig;
    ...
    while (fgets(s8Buf, MAXLINE, stdin) != NULL) {
        ...
        case 5:
            if (g_s32EnableSta) {
                stForgetStaConfig = wifi_sta_get_config();
                while (stForgetStaConfig != NULL) {
                    wifi_sta_forget(stForgetStaConfig);
                    stTempForgetStaConfig = stForgetStaConfig;
                    stForgetStaConfig = wifi_sta_iterate_config(stTempForgetStaConfig);
                    free(stTempForgetStaConfig);
                }
            }
            break;
        case 6:
