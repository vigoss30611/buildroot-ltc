 ...
 *
 * Description: Recorde Stream in Slow-motion mode.
 *
 * Author:
...
void RecodeStream(VRecorderInfo *pstInfo)
{
    ...
    if (vplay_control_recorder(pstRecorder, VPLAY_RECORDER_SLOW_MOTION, &s32Rate) <= 0)
    {
        printf("start slow motion error ->%d\n", s32Rate);
        goto end;
    }
