 ...
 *
 * Description: Recorder Set New Segment Cmd.
 *
 * Author:
...
void RecodeStream(VRecorderInfo *pstInfo)
{
    ...
    time(&s32StartTime);
    s32Now = s32NewSegment = s32StartTime;
    printf("start recorder ok:%ld\n", s32StartTime);
    while (s32Now - s32StartTime <= RECODE_TIME)
    {
        if ((s32Now - s32NewSegment) > SEGMENT_INTERVAL)
        {
            if (vplay_control_recorder(pstRecorder, VPLAY_RECORDER_NEW_SEGMENT, NULL) <= 0)
            {
                printf("start new segment error\n");
                break;
            }
            s32NewSegment = s32Now;
        }
        sleep(1);
        time(&s32Now);
    }
