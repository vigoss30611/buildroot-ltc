 ...
 *
 * Description: Recorder Set User Data to Stream.
 *
 * Author:
...
void RecodeStream(VRecorderInfo *pstInfo)
{
    ...
    vplay_udta_info_t stUserData;
    char pu8Data[] = "Infotm set user data";
    int s32Length = strlen(pu8Data);
    time_t s32StartTime;
    time_t s32Now;
    ...
    printf("udta data->%s<-\n", pu8Data);
    stUserData.buf = pu8Data;
    stUserData.size = s32Length;
    if (vplay_control_recorder(pstRecorder, VPLAY_RECORDER_SET_UDTA, &stUserData) <= 0)
    {
        printf("recorder set user data error\n");
        goto end;
    }
