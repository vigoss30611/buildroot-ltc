 ...
 *
 * Description: Recorde Stream in Fast-motion mode.
 *
 * Author:
...
void RecodeStream(VRecorderInfo *pstInfo)
{
    ...
    if (vplay_control_recorder(pstRecorder, VPLAY_RECORDER_FAST_MOTION, &s32Rate) <= 0)
    {
        printf("start fast motion error ->%d\n", s32Rate);
        goto end;
    }
