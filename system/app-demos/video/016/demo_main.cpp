 ...
 *
 * Description: Encode H264 ES Stream and Dynamic Trigger I Frame
 *
 * Author:
   ...
   */
    ...
    */
    while (true == pstPara->bThreadStart)
    {
        ...
        {
            video_trigger_key_frame(s_VideoChannel);
            if (ret < 0)
            {
                printf("video_trigger_key_frame Error ret:%d\n", ret);
            }
            else
            {
                printf("video_trigger_key_frame sucess\n");
            }
        ...
        // dynamic Trigger Encode I frame, can Trigger it any time
        if(u32EncodeNUmber == 100)
        {
            ret = video_trigger_key_frame(s_VideoChannel);
            if (ret < 0)
            {
                printf("video_trigger_key_frame Error ret:%d\n", ret);
            }
