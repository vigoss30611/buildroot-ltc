 ...
 *
 * Description: Encode H264 ES Stream and Dynamic Change Rate Control Mode(CBR/VBR/FixQP)
 *
 * Author:
...
void VencSetRateCtrl(EN_VIDEO_RATE_CTRL_TYPE enRatectrlMode)
{
    ...
    {
        printf("video_set_ratectrl %s Sucess\n", (enRatectrlMode == 0) ? "CBR":((enRatectrlMode == 1) ? "VBR" : "FIXQP"));
    }
}
   ...
   */
    ...
    */
    while (true == pstPara->bThreadStart)
    {
        ...
        // dynamic change ratecontrol mode, can set at any time
        if(u32EncodeNUmber == 100)
        {
            VencSetRateCtrl(VENC_VBR_MODE);
        }
        else if(u32EncodeNUmber == 200)
        {
            VencSetRateCtrl(VENC_FIXQP_MODE);
        }
        else if(u32EncodeNUmber == 300)
        {
            VencSetRateCtrl(VENC_CBR_MODE);
        }
    }
