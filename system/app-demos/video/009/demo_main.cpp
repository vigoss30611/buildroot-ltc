 ...
 *
 * Description: Encode 720P User Define Frame Rate(<= Input fps) H264 ES Stream with ISP Input(30fps)
 *
 * Author:
...
static const char *s_VideoChannel = "enc720p-stream";
static const char *s_ISPChannel = "isp";
...
/*
 * @brief   set video frc (frame rate convert) Infomation only support
 * @param s32TargetFps -IN: the target fps that convert down
 * @return void
 */
void VencSetFps(int s32TargetFps)
{
    int ret;
    int s32fps = 0;

    if(camera_get_fps(s_ISPChannel, &s32fps) < 0)
    {
        printf("camera_get_fps Error\n");
    }
    else
    {
        if (s32TargetFps > s32fps)
        {
            printf("invalid parameter s32TargetFps:%d\n", s32TargetFps);
        }
        ret = video_set_fps(s_VideoChannel, s32TargetFps);
        if (ret < 0)
        {
            printf("video_set_fps Error ret:%d\n", ret);
        }
        else
        {
            printf("video_set_fps sucess s32TargetFps:%d\n",s32TargetFps);
        }
    }
}
...
void *VencStartStreamProc(void *pThreadPara)
{
    ...
    uint32_t u32EncodeNumber = 0, u32LastEncodeNumber;
   ...
   */
    ...
    sprintf(s8pFileName, "/mnt/sd0/%s_%d_%d.h264", s_VideoChannel, s32Width, s32Height);
    ...
    s64StartTime = st_TimeVal.tv_sec*1000 + st_TimeVal.tv_usec/1000;
    ...
    */
    while (true == pstPara->bThreadStart)
    {
        ...
        gettimeofday(&st_TimeVal, NULL);
        s64EndTime = st_TimeVal.tv_sec*1000 + st_TimeVal.tv_usec/1000;
        if(s64EndTime -  s64StartTime > 1000)
        {
            printf("-----Fps----->>%d<<-----\n", u32EncodeNumber - u32LastEncodeNumber);
            u32LastEncodeNumber = u32EncodeNumber;
            s64StartTime = s64EndTime;
        }
        ...
        //enabel or disable set roi, can do it at any time
        if(u32EncodeNumber == 100)
        {
            VencSetFps(27);
        }
        else if(u32EncodeNumber == 500)
        {
            VencSetFps(17);
        }
    }
