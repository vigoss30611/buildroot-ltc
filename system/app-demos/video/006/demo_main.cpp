 ...
 *
 * Description: Encode 1080P H264 ES Stream with ISP Input and Set ROI Info
 *
 * Author:
...
/*
 * @brief   set video ROI (region of Interest) Infomation
 * @param bEnable -IN: enable roi setting or disable roi setting
 * @return void
 */
void VencSetRoiInfo(bool bEnable)
{
    int ret;
    ST_VIDEO_ROI_INFO stRoiInfo;
    ...
    if(true == bEnable)
    {
        ...
        if (ret < 0 )
        {
            printf("video_get_resolution error ->%d\n", ret);
            return;
        }
        // first roi info
        ...
        stRoiInfo.roi[0].x = 0;
        stRoiInfo.roi[0].y = 0;
        stRoiInfo.roi[0].w = s32Width / 2;
        stRoiInfo.roi[0].h = s32Height / 2;
        stRoiInfo.roi[0].qp_offset = -2;

        //second roi info
        stRoiInfo.roi[1].enable = 1;
        stRoiInfo.roi[1].x = s32Width / 2;
        stRoiInfo.roi[1].y = s32Height / 2;
        stRoiInfo.roi[1].w = s32Width / 4;
        stRoiInfo.roi[1].h = s32Height / 4;
        stRoiInfo.roi[1].qp_offset = -4;

        ret = video_set_roi(s_VideoChannel,&stRoiInfo);
        if (ret < 0 )
        {
            printf("video_set_roi enable error ->%d\n", ret);
        }
        else
        {
            printf("video_set_roi enable sucess\n");
        }
    }
    else
    {
        stRoiInfo.roi[0].enable = 0;
        stRoiInfo.roi[0].enable = 0;
        ret = video_set_roi(s_VideoChannel, &stRoiInfo);
        if (ret < 0 )
        {
            printf("video_set_roi disable error ->%d\n", ret);
        }
        else
        {
            printf("video_set_roi disable sucess\n");
        }
    }
   ...
   */
    ...
    */
    while (true == pstPara->bThreadStart)
    {
        ...
        else if(u32EncodeNumber == 200)
        {
            VencSetRoiInfo(false);
        }
    }
