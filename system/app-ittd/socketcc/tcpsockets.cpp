...
TCPSocket::SendData(const void *pData, int iDataLen, unsigned int uiFlags)
{
    ...
    int iSendBytes;
    int iOffset, iTransferLen;
    unsigned char* pBuf = (unsigned char*)pData;
    iOffset = 0;
    iTransferLen = iDataLen;

	do
	{
		iSendBytes = Send(pBuf + iOffset, iTransferLen, uiFlags);
		if (iSendBytes < 0)
			return iSendBytes;
		iOffset += iSendBytes;
		iTransferLen -= iSendBytes;
	} while (iTransferLen > 0);
...
TCPSocket::RecvData(void *pBuffer, int iBufferLen, unsigned int uiFlags)
{
    ...
    iOffset = 0;
    iTransferLen = iBufferLen;

	do
	{
		iRecvBytes = Recv(pBuf + iOffset, iTransferLen, uiFlags);
		if (iRecvBytes < 0)
			return iRecvBytes;
		iOffset += iRecvBytes;
		iTransferLen -= iRecvBytes;
	} while (iTransferLen > 0);
