...
class Deamon
{
    private:
        ...
        static bool        m_bKillFlag;
        static bool        m_bRestartFlag;
        static Condition   m_CondSignal;

        static void SignalHandler(int iSig);

        char        *m_pcConfigFileName, m_szException[1024];
        TCPServer   *m_pcTheServer;
        int 		m_iListenPort;
    ...
    public:
        enum DaemonError { errAbnormalTermination };
                    ...
                    Deamon(const char *pcConfigFile, int iListenPort);
                    ~Deamon();
