...
#ifdef USE_MATH_NEON
	decimalCCM = ceil((double)logf_neon(float(1<<CC_COEFF_0_FRAC)));
#else
...
#ifdef USE_MATH_NEON
	decimalOff = ceil((double)logf_neon(float(1<<CC_OFFSET_0_FRAC)));
#else
...
#ifdef USE_MATH_NEON
    decimalWBG = ceil((double)logf_neon(float(1<<WHITE_BALANCE_GAIN_0_FRAC)));
#else
