...
void VisionLive::ObjectRegistry::registerObject(const QString &objectName, QObject *object, const QString &path)
{
	...
	{
		emit logError("Couldn't register object - " + path + objectName, tr("ObjectRegistry::registerObject()"));
	}
...
void VisionLive::ObjectRegistry::deregisterObject(const QString &name)
{
	...
	{		
		...
		for(it2 = tmp.begin(); it2 != tmp.end(); it2++)
		{
			...
			if(it != _objects.end())
			{
				emit logMessage("Deregistering object - " + (*it).second + (*it).first->objectName(), tr("ObjectRegistry::registerObject()"));
				_objects.erase(it);
			}
			else
			{
				emit logError("Couldn't deregister object - " + (*it).second + (*it).first->objectName(), tr("ObjectRegistry::deregisterObject()"));
			}
		}
	...
	{
		emit logError("Couldn't deregister object - " + name, tr("ObjectRegistry::deregisterObject()"));
	}
