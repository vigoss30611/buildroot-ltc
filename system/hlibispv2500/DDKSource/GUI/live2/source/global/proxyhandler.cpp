...
void VisionLive::ProxyHandler::initImageHandler()
{
	...
	QObject::connect(_pImageHandler, SIGNAL(imageReceived(QImage *, int, int, int, int, int)), this, SIGNAL(imageReceived(QImage *, int, int, int, int, int)));
	...
	QObject::connect(_pImageHandler, SIGNAL(histogramReceived(std::vector<cv::Mat>, int, int)), this, SIGNAL(histogramReceived(std::vector<cv::Mat>, int, int)));
...
void VisionLive::ProxyHandler::initCommandHandler()
{
	...
	QObject::connect(_pCommandHandler, SIGNAL(FB_received(QMap<QString, QString>)), this, SIGNAL(FB_received(QMap<QString, QString>)));
...
void VisionLive::ProxyHandler::connect(IMG_UINT16 cmdPort, IMG_UINT16 imgPort)
{
	...
	if(_pCommandHandler && _commandHandlerState == VisionLive::DISCONNECTED)
	{
		_commandHandlerState = VisionLive::CONNECTING;
		_pCommandHandler->connect(cmdPort);
	}

	if(_pImageHandler && _imageHandlerState == VisionLive::DISCONNECTED)
	{
		_imageHandlerState = VisionLive::CONNECTING;
		_pImageHandler->connect(imgPort);
	}

	if(_pCommandHandler && _commandHandlerState == VisionLive::CONNECTED && 
		_pImageHandler && _imageHandlerState == VisionLive::CONNECTED)
	{
...
void VisionLive::ProxyHandler::LSH_set(std::string filename)
{
	if(_pCommandHandler)
	{
		_pCommandHandler->LSH_set(filename);
	}
}
