...
void VisionLive::CapturePreview::initHistograms()
{
	for(unsigned int i = 0; i < _pItem->getRecData()->HistDatas.size(); i++)
	{
		...
		pHistogram->histogramReceived(_pItem->getRecData()->HistDatas[i].histograms, 
			_pItem->getRecData()->HistDatas[i].nChannels, 
			_pItem->getRecData()->HistDatas[i].nElem);
...
void VisionLive::CapturePreview::updateLineView()
{
	if(_pLineView)
	{
		QImage *image = _pItem->getRecData()->frameData[_currentFrame].convertToQImage();
		unsigned int step = 3;
		QMap<QString, std::vector<std::vector<int> > > lineViewData = VisionLive::Algorithms::generateLineView(image, _marker.toPoint(), step);
		_pLineView->lineViewReceived(_marker.toPoint(), lineViewData, step);
		delete image;
	}
}
...
void VisionLive::CapturePreview::markerPointChanged(QPointF pos, Qt::MouseButton button)
{
	if(button == Qt::LeftButton)
	{
		if(_pItem && _pMarkerItem)
		{
			...
			if(!(_marker.x() < 0 || _marker.x() > _pItem->getRecData()->frameData[_currentFrame].data.cols ||
				_marker.y() < 0 || _marker.y() > _pItem->getRecData()->frameData[_currentFrame].data.rows))
			{
