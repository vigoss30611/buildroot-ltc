...
void VisionLive::ModuleR2Y::removeObjectValueChangedMark()
{
	...
	case CUSTOM_CSS:
		...
		R2Y_offsetU_sb->setStyleSheet(CSS_CUSTOM_QDOUBLESPINBOX);
		R2Y_offsetV_sb->setStyleSheet(CSS_CUSTOM_QDOUBLESPINBOX);
		break;
	...
	case DEFAULT_CSS:
		...
		R2Y_offsetU_sb->setStyleSheet(CSS_DEFAULT_QDOUBLESPINBOX);
		R2Y_offsetV_sb->setStyleSheet(CSS_DEFAULT_QDOUBLESPINBOX);
		break;
	}
}
...
void VisionLive::ModuleR2Y::load(const ISPC::ParameterList &config)
{
	...
	R2Y_offsetU_sb->setMinimum(ISPC::ModuleR2Y::R2Y_OFFSETU.min);
	R2Y_offsetV_sb->setMinimum(ISPC::ModuleR2Y::R2Y_OFFSETV.min);
	...
	R2Y_offsetU_sb->setMaximum(ISPC::ModuleR2Y::R2Y_OFFSETU.max);
	R2Y_offsetV_sb->setMaximum(ISPC::ModuleR2Y::R2Y_OFFSETV.max);
	...
	DEFINE_PREC(R2Y_offsetU_sb, 1024.0000f);
	DEFINE_PREC(R2Y_offsetV_sb, 1024.0000f);
	...
	R2Y_offsetU_sb->setValue(r2y.fOffsetU);
	R2Y_offsetV_sb->setValue(r2y.fOffsetV);
}

void VisionLive::ModuleR2Y::save(ISPC::ParameterList &config) const
{
	ISPC::ModuleR2Y r2y;
	r2y.load(config);
	...
	r2y.fOffsetU = R2Y_offsetU_sb->value();
	r2y.fOffsetV = R2Y_offsetV_sb->value();
