...
void VisionLive::ModuleGMA::save(ISPC::ParameterList &config) const
{
...
void VisionLive::ModuleGMA::GMA_set(int index)
{
	if(_pProxyHandler && GMA_changeGLUT_lb->itemText(index) != GMA_CURR_GAMMA_NAME)
	{
		...
		QVector<QPointF> redData = ((GMAWidget *)GMA_curveTabs_tw->widget(index))->getData(0);
		QVector<QPointF> greenData = ((GMAWidget *)GMA_curveTabs_tw->widget(index))->getData(1);
		QVector<QPointF> blueData = ((GMAWidget *)GMA_curveTabs_tw->widget(index))->getData(2);
