...
private:
	...
	std::string _LSHGridFile; // Holds filename of lsh grid
	int LSH_set(ParamSocket *socket); // Sets LSH module grid
...
signals:
	...
	void FB_received(QMap<QString, QString> params); // Emited to indicate feedback received
...
public slots:
	...
	void LSH_set(std::string filename); // Called from LSH to apply LSH module grid
