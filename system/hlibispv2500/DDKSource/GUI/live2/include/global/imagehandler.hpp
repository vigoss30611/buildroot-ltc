...
private:
	...
	int setImageSending(ParamSocket *socket); // Enables and disables sending imge through socket
...
signals:
	...
	void imageReceived(QImage *pImage, int type, int format, int mosaic, int width, int height); // Emited to indicate next frame received
	...
	void histogramReceived(std::vector<cv::Mat> histData, int nChannels, int nElem); // Emited to indicate histogram generated
