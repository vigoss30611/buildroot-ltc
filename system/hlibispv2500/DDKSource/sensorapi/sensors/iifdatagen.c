    ...
    SENSOR_MODE *psModes)
{
    ...
    if (NULL != psCam->pszFilename)
    {
        ...
        psModes->ui16VerticalTotal = psCam->sImage.info.ui32Height + psCam->ui16VBlanking;
...
IMG_RESULT IIFDG_Enable(SENSOR_HANDLE hHandle)
{
    ...
    if (NULL != psCam->pszFilename && NULL != psCam->pConnection)
    {
        ...
        {
            psCam->pDatagen->eFormat = CI_DGFMT_PARALLEL_10;
            LOG_DEBUG("IntDG enable Parallel 10b\n");
        }
        else if (12 >= psCam->sImage.info.ui8BitDepth)
        {
            psCam->pDatagen->eFormat = CI_DGFMT_PARALLEL_12;
            LOG_DEBUG("IntDG enable Parallel 12b\n");
        }
        else
        {
...
IMG_RESULT IIFDG_Insert(SENSOR_HANDLE hHandle)
{
    ...
    if (SENSOR_STATE_RUNNING == sStatus.eState)
    {
        ...
        ret = SimImageIn_loadFLX(psCam->pszFilename, &(psCam->sImage),
...
IMG_RESULT IIFDG_ExtendedSetSourceFile(SENSOR_HANDLE hHandle, const char *filename)
{
    ...
    if (SENSOR_STATE_RUNNING != sStatus.eState)
    {
        ...
        ret = SimImageIn_loadFLX(psCam->pszFilename, &(psCam->sImage), 0);
        if (IMG_SUCCESS != ret)
        {
