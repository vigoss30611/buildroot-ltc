...
IMG_RESULT KRN_DG_DebugDumpDataGenerator(IMG_HANDLE hFile, const IMG_CHAR *pszHeader, IMG_UINT32 ui32ContextNb, IMG_BOOL bDumpFields)
{
    ...
    if ( pszHeader != NULL )
    {
        ...
        {
            CI_FATAL("failed to write header before dumping Data Generator %d registers\n", ui32ContextNb);
            return IMG_ERROR_FATAL;
        }
    }

    if ( KRN_CI_DebugRegistersDump(hFile, felix_test_dg_fields, gNElem, g_pDGDriver->hRegFelixDG[ui32ContextNb], bDumpFields, IMG_FALSE) != IMG_SUCCESS )
    {
...
IMG_RESULT KRN_DG_DebugDumpRegisters(const IMG_CHAR *pszFilename, const IMG_CHAR *pszHeader, IMG_BOOL bDumpFields, IMG_BOOL bAppendFile)
{
    ...
    {
		if ( pszHeader != NULL ) sprintf(header, "%s - data generator %u registers:\n", pszHeader, i);
		else sprintf(header, "context %u registers:\n", i);

        if ( (iRet=KRN_DG_DebugDumpDataGenerator(hFile, header, i, bDumpFields)) != IMG_SUCCESS )
        {
...
IMG_RESULT KRN_DG_DebugRegistersTest(void)
{
    ...
    {
        char buff[64];
        sprintf(buff, "read-write all datagen %d registers", i);
        CI_PDUMP_COMMENT(g_pDGDriver->hRegFelixDG[i], buff);
        KRN_CI_DebugReg(g_pDGDriver->hRegFelixDG[i], felix_test_dg_fields, gNElem, IMG_TRUE);
    }
    ...
    if ( g_bUseTransif ) // check if it has pending frames...
    {
        ...
        if ( g_pDGDriver->aActiveCamera[dg]->sList_pending.ui32Elements > 0 )
        {
            ...
            do
            {
                ...
                TALREG_ReadWord32(g_pDGDriver->hRegFelixDG[dg], FELIX_TEST_DG_DG_INTER_STATUS_OFFSET, &tmpReg);
            }
            while (!REGIO_READ_FIELD(tmpReg, FELIX_TEST_DG, DG_INTER_STATUS, DG_INT_END_OF_FRAME));
        }
    }
}
