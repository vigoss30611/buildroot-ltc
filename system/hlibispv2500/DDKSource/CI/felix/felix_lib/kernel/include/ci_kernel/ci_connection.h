...
/**
 * @brief Update the deshading grid associated to a kernel-side pipeline object
 *
 * @warning Does not verify if the current deshading grid is used or allocated.
 *
 * Converts the given deshading matrix directly to device memory.
 *
 * @param pPipeline containing its .userConfiguration copied from user space
 * - asserts it is not NULL
 *
 * @return IMG_SUCCESS
 * @return IMG_ERROR_INVALID_PARAMETERS if copy_from_user failed
 *
 * @see Delegates to SYS_MemAlloc() and SYS_MemFree() to manage the allocation
 */
...
#ifdef INFOTM_ISP
extern int IsLowLight;
extern unsigned int g_ui32Size;
extern SYS_MEM g_buffer;
IMG_RESULT INT_CI_PipelineSetYUV(KRN_CI_CONNECTION *pConnection, int flag);

IMG_RESULT INT_CI_RegGet(KRN_CI_CONNECTION *pConnection,
    struct CI_REG_PARAM __user *pUserParam);

IMG_RESULT INT_CI_RegSet(KRN_CI_CONNECTION *pConnection,
    const struct CI_REG_PARAM __user *pUserParam);
#endif //INFOTM_ISP
/**
 * @}
 */
