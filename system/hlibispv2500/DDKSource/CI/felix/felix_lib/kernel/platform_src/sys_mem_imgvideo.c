    ...
    void **pvCpuKmAddr, IMG_PHYSADDR *ppaAddr)
{
    ...
    if (ppaAddr != NULL) {
        ...
        cpuPAddr = SYSMEMU_CpuKmAddrToCpuPAddr(pSysDev->sMemPool,
            *pvCpuKmAddr);
    ...
    IMG_RESULT *ret)
{
    ...
    pPages = SYSMEMU_ImportPages(pSysDev->sMemPool, pSysDev, allocationSize,
...
{
    IMG_HANDLE phPagesHandle = psMem; // this is a (SYSMEMU_sPages *)
    SYSMEMU_FreePages(phPagesHandle);
}
    ...
    struct vm_area_struct *vma)
{
    ...
    cpuPAddr = SYSMEMU_CpuKmAddrToCpuPAddr(pSysDev->sMemPool, cpuVirt);
