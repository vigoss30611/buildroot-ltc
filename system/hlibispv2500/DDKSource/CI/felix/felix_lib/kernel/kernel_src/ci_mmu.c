    ...
    IMG_RESULT *ret)
{
...
#if 0 // enable only to debug pagefault and see state of pages before start
    ...
    // others are not needed or using the internal functions
#endif
    ...
    IMG_UINT8 ui8NRequestor, IMG_BOOL8 bBypass)
{
   ...
   CI_INFO("Using MMU of %u Bits\n", uiMMUSize);
    ...
    for ( i = 0 ; i < ui8NDir ; i++ )
    {
        ...
        if ( uiMMUSize > 32 )
        {
            // the HW is expected a shifted value when mmu has extended address enabled
    ...
    for ( i = 0 ; i < CI_MMU_HW_TILED_HEAPS ; i++ )
    {
        ...
        TALREG_WriteWord32(pMMU->hRegFelixMMU,
            ...
            pMMU->aHeapStart[CI_TILED_IMAGE_HEAP0 + i]
            +pMMU->aHeapSize[CI_TILED_IMAGE_HEAP0 + i]-1);// inclusive address
    }
    ...
    IMG_BOOL8 bFlushAll)
{
    ...
    sprintf(message, "MMU dir %d: %s", uiDirectory, bFlushAll?"INVALDC":"FLUSH");
...
IMG_RESULT KRN_CI_MMUPause(KRN_CI_MMU *pMMU, IMG_BOOL8 bPause)
{
    ...
    if (IMG_SUCCESS != ret)
    {
        ...
        // BRN51166 sim did not implement auto-clear for the pause bit
        return IMG_SUCCESS;
#endif
        return IMG_ERROR_FATAL;
    }
