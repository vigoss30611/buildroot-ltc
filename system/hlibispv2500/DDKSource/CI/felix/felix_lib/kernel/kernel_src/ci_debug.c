...
IMG_RESULT KRN_CI_DriverStartPdump(KRN_CI_DRIVER *pDriver, IMG_UINT8 enablePdump)
{
    ...
    if (enablePdump > 1 )
    {
        ...
        TALPDUMP_AddMemspaceToContext(pdumpname, dgMMU);
...
void KRN_CI_PipelineUpdateSave(KRN_CI_SHOT *pShot)
{
	...
	IMG_UINT32 reg = 0;
	REGIO_WRITE_FIELD(reg, FELIX_SAVE, CONTEXT_LOGS, SAVE_CONFIG_TAG, pShot->pPipeline->userPipeline.ui8PrivateValue);
...
IMG_RESULT KRN_CI_DebugDumpShot(IMG_CONST KRN_CI_SHOT *pShot, IMG_CONST IMG_CHAR *pszFilename, IMG_CONST IMG_CHAR *pszHeader, IMG_BOOL bDumpFields, IMG_BOOL bAppendFile)
{
	...
	sprintf(header, "config struct (0x%08"IMG_PTRDPR"X)\n", Platform_MemGetDevMem( &(pShot->hwLoadStructure), 0 ));
	if ( (iRet = KRN_CI_DebugDumpLoadStruct(Platform_MemGetTalHandle(&(pShot->hwLoadStructure), 0, &off), hFile, header, bDumpFields)) != IMG_SUCCESS )
	{
