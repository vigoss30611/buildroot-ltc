...
static void IMG_CI_ShotFreeBuffers(KRN_CI_SHOT *pBuffer)
{
    ...
    {
        KRN_CI_BufferDestroy(pBuffer->phwEncoderOutput);
        pBuffer->phwEncoderOutput = NULL;
    }
    if (pBuffer->phwDisplayOutput)
    {
        KRN_CI_BufferDestroy(pBuffer->phwDisplayOutput);
        pBuffer->phwDisplayOutput = NULL;
    }
    if (pBuffer->phwHDRExtOutput)
    {
        KRN_CI_BufferDestroy(pBuffer->phwHDRExtOutput);
        pBuffer->phwHDRExtOutput = NULL;
    }
    if (pBuffer->phwRaw2DExtOutput)
    {
        KRN_CI_BufferDestroy(pBuffer->phwRaw2DExtOutput);
    ...
    if (pBuffer->hwLoadStructure.uiAllocated > 0)
    {
        CI_DEBUG("Freeing hwLoadStructure...\n");
        ret = SYS_MemFree(&(pBuffer->hwLoadStructure));
        if (IMG_SUCCESS != ret)
        {
...
IMG_RESULT IMG_CI_ShotMallocBuffers(KRN_CI_SHOT *pBuffer)
{
    ...
    if (pBuffer->phwEncoderOutput
        ...
        || pBuffer->hwLoadStructure.uiAllocated > 0
    ...
    */
    ...
    sprintf(message, "save struct (ctx %d)",
        pBuffer->pPipeline->userPipeline.ui8Context);
    ...
    */
    ...
    CI_DEBUG("load struct (ctx %d)\n", 
        ...
        ret = SYS_MemAlloc(&(pBuffer->hwLoadStructure),
    ...
    */
    ...
    CI_DEBUG("DPF write map (ctx %d)\n", 
        pBuffer->pPipeline->userPipeline.ui8Context);
#endif
    if (pBuffer->pPipeline->userPipeline.ui32DPFWriteMapSize > 0)
    {
#ifdef INFOTM_ISP
        sprintf(szName, "DPFWrite(%d)", count);
        ret = SYS_MemAlloc(&(pBuffer->sDPFWrite.sMemory),
            pBuffer->pPipeline->userPipeline.ui32DPFWriteMapSize, pHeap, 0, szName);
#else
        ret = SYS_MemAlloc(&(pBuffer->sDPFWrite.sMemory),
            pBuffer->pPipeline->userPipeline.ui32DPFWriteMapSize, pHeap, 0);
    ...
    pBuffer->userShot.uiDPFSize =
        pBuffer->pPipeline->userPipeline.ui32DPFWriteMapSize;

    /*
     * ENS output buffer
     */
    if ((pBuffer->pPipeline->userPipeline.eOutputConfig & CI_SAVE_ENS) != 0)
    {
#ifdef FELIX_FAKE
        ...
        pBuffer->userShot.uiENSSize =
            HW_CI_EnsOutputSize(
            pBuffer->pPipeline->userPipeline.sEncStats.ui8Log2NCouples,
            (pBuffer->pPipeline->userPipeline.sImagerInterface.\
            ui16ImagerSize[1] + 1) * 2);
...
#endif //INFOTM_ISP
{
    ...
    if (ui32Size > 0)
    {
        ...
        // BRN47344: see ci_shot.c for details
        if (pHeap && g_psCIDriver->sHWInfo.rev_ui8Major == 1)
        {
            CI_PDUMP_COMMENT(g_psCIDriver->sTalHandles.hMemHandle,
                "BRN47344: allocate an extra page for output buffer prefetch");
            ui32ToAllocate += PAGE_SIZE;
        }
...
{
    const CI_PIPELINE *pUserConfig = &(pPipeline->userPipeline);
    struct CI_TILINGINFO sTilingInfo;
    struct CI_SIZEINFO sSizeInfo;
    IMG_RESULT ret;
    IMG_BOOL8 bTiling = IMG_FALSE;
    IMG_UINT32 sensorWidth =
        (pUserConfig->sImagerInterface.ui16ImagerSize[0] + 1)*CI_CFA_WIDTH;
    IMG_UINT32 sensorHeight =
        (pUserConfig->sImagerInterface.ui16ImagerSize[1] + 1)*CI_CFA_HEIGHT;
    ...
    if (TYPE_NONE != pUserConfig->eDispType.eBuffer)
    {
        ...
        // if bayer format is DE therefore not scaled down
        if (TYPE_BAYER != pUserConfig->eDispType.eBuffer
            && bScaleOutput && !pUserConfig->sEncoderScaler.bBypassScaler)
        {
            ui32Width = (pUserConfig->sDisplayScaler.aOutputSize[0] + 1) * 2;
            ui32Height = pUserConfig->sDisplayScaler.aOutputSize[1] + 1;
        }
    ...
    if (TYPE_NONE != pUserConfig->eEncType.eBuffer)
    {
        ...
        if (bScaleOutput && !pUserConfig->sEncoderScaler.bBypassScaler)
        {
            ui32Width = (pUserConfig->sEncoderScaler.aOutputSize[0] + 1) * 2;
            ui32Height = pUserConfig->sEncoderScaler.aOutputSize[1] + 1;
        }
...
struct MMUDirectory *pDirectory)
{
    ...
    if (ret == IMG_SUCCESS)
    {
        ...
        ret = SYS_MemMap(&(pBuffer->hwLoadStructure), pDirectory, MMU_RO);
    }
...
IMG_RESULT KRN_CI_ShotUnmapMemory(KRN_CI_SHOT *pBuffer)
{
    ...
    SYS_MemUnmap(&(pBuffer->hwLoadStructure));
...
IMG_RESULT KRN_CI_ShotGetStatsConfig(KRN_CI_SHOT *pShot)
{
    ...
    pPipeline = &(pShot->pPipeline->userPipeline);

    pShot->userShot.eOutputConfig = pPipeline->eOutputConfig;
    IMG_MEMCPY(&(pShot->userShot.sExposureStats),
        &(pPipeline->sExposureStats), sizeof(CI_MODULE_EXS));
    IMG_MEMCPY(&(pShot->userShot.sFocusStats),
        &(pPipeline->sFocusStats), sizeof(CI_MODULE_FOS));
    IMG_MEMCPY(&(pShot->userShot.sWhiteBalanceStats),
        &(pPipeline->sWhiteBalanceStats), sizeof(CI_MODULE_WBS));
    IMG_MEMCPY(&(pShot->userShot.sHistStats),
        &(pPipeline->sHistStats), sizeof(CI_MODULE_HIS));
    IMG_MEMCPY(&(pShot->userShot.sFlickerStats),
        &(pPipeline->sFlickerStats), sizeof(CI_MODULE_FLD));
    IMG_MEMCPY(&(pShot->userShot.sEncStats),
        &(pPipeline->sEncStats), sizeof(CI_MODULE_ENS));
