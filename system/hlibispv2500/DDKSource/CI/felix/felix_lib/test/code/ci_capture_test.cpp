...
TEST_F(CI_Capture, start_stop)
{
    ...
    TALREG_ReadWord32(g_psCIDriver->sTalHandles.hRegFelixContext[pPipeline->ui8Context], FELIX_CONTEXT0_CONTEXT_CONTROL_OFFSET, &ui32Val);
...
TEST_F(CI_Capture, privateValue)
{
    ...
    oldPrivate = pPipeline->ui8PrivateValue;
    ...
    pPipeline->ui8PrivateValue++; // change capture private value
...
TEST_F(CI_Capture, multiContext_LS)
{
    ...
    TALREG_ReadWord32(g_psCIDriver->sTalHandles.hRegFelixContext[pPipeline->ui8Context], FELIX_CONTEXT0_LS_BUFFER_ALLOCATION_OFFSET, &ui32Val);
    ui32Val = REGIO_READ_FIELD(ui32Val, FELIX_CONTEXT0, LS_BUFFER_ALLOCATION, LS_CONTEXT_OFFSET)*2; // given as pair of pixels
    EXPECT_EQ(pUserConnection->sLinestoreStatus.aStart[pPipeline->ui8Context], ui32Val);

    EXPECT_EQ(IMG_SUCCESS, CI_PipelineCreate(&pOtherPipeline, pUserConnection));

    memcpy(pOtherPipeline, pPipeline, sizeof(CI_PIPELINE));
    pOtherPipeline->ui8Context = 1;
    ASSERT_NE(pOtherPipeline->ui8Context, pPipeline->ui8Context) << "the pipeline object should have different ctx";

    EXPECT_EQ(IMG_SUCCESS, CI_PipelineRegister(pOtherPipeline));

    TALREG_ReadWord32(g_psCIDriver->sTalHandles.hRegFelixContext[pOtherPipeline->ui8Context], FELIX_CONTEXT0_LS_BUFFER_ALLOCATION_OFFSET, &ui32Val);
    ui32Val = REGIO_READ_FIELD(ui32Val, FELIX_CONTEXT0, LS_BUFFER_ALLOCATION, LS_CONTEXT_OFFSET)*2; // given as pair of pixels
    EXPECT_NE(pUserConnection->sLinestoreStatus.aStart[pOtherPipeline->ui8Context], ui32Val) << "LS should not have been written yet";
    ...
    TALREG_ReadWord32(g_psCIDriver->sTalHandles.hRegFelixContext[pOtherPipeline->ui8Context], FELIX_CONTEXT0_LS_BUFFER_ALLOCATION_OFFSET, &ui32Val);
    ui32Val = REGIO_READ_FIELD(ui32Val, FELIX_CONTEXT0, LS_BUFFER_ALLOCATION, LS_CONTEXT_OFFSET)*2; // given as pair of pixels
    EXPECT_EQ(pUserConnection->sLinestoreStatus.aStart[pOtherPipeline->ui8Context], ui32Val) << "LS should have been written now...";
...
TEST_F(CI_Capture, dataExtraction)
{
    ...
    pPrivateCapture->userPipeline.eDataExtraction = CI_INOUT_FILTER_LINESTORE;
    ASSERT_EQ(0, pPipeline->ui8Context);
    ...
    otherCapture->eDataExtraction = CI_INOUT_BLACK_LEVEL;

    otherCapture->ui8Context = 1;
    otherCapture->sImagerInterface.ui8Imager = 1;
    otherCapture->ui16MaxDispOutWidth = 32;
    otherCapture->ui16MaxDispOutHeight = 32;
