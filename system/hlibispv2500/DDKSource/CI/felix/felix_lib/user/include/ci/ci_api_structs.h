...
typedef struct CI_HWINFO
{
    ...
    /**
     * @brief The number of veritcal taps used by the encoder scaler for the
     * luma component.
     */
     ...
     * luma component (scaling is done before YCC to RGB convertion)
     */
    IMG_UINT8 scaler_ui8DispHLumaTaps;
    /**
     * @brief The number of vertical taps used by the display scaler for the
     * luma component (scaling is done before the YCC to RGB convertion)
     */
     ...
     * physical addresses)
     */
    IMG_UINT32 uiMMUEnabled;
    /** @brief device mmu page size */
    IMG_UINT32 ui32MMUPageSize;
    /** 
...
typedef struct CI_REG_PARAM
{
    IMG_CHAR * pchRegSpaceName;
 ...
 * It does not have to be the same than the register ofset
 */
enum CI_SAVE_CONFIG_FLAGS
{
    ...
    CI_SAVE_HIST_GLOBAL = CI_SAVE_WHITEBALANCE << 1,
...
typedef struct CI_PIPELINE
{
    ...
    /** @brief Configuration of the saler for the display */
    ...
    /** @warning Cannot be updated after registration */
...
typedef struct CI_BUFFID
{
     ...
     *
     * @note Needs to be a multiplie of SYSMEM_ALIGNMENT
     */
     ...
     *
     * @note Does NOT need to be a multiplie of SYSMEM_ALIGNMENT
     */
...
typedef struct CI_SHOT
{
    ...
    /** @brief is the HDR output tiled (using pipeline's tiling shceme) */
    ...
    IMG_UINT8 ui8PrivateValue;
    /**
     * @brief Set to IMG_TRUE if an error occured while capturing the frame
     */
 ...
 *
 * Used for HDR insertion buffer access
 */
...
typedef struct CI_GASKET_INFO
{
    ...
    /**
     * @brief if None then gasket is diabled otherwise combinaison of
     * @ref CI_GASKETTYPE
     *
...
{
    CI_DGFMT_PARALLEL_10 = 0,
    CI_DGFMT_PARALLEL_12,
...
typedef struct CI_DATAGEN_CONVERTER
{
    CI_CONV_FMT eFormat;
    /**
     * @brief Pointer to one of the convertion function
     *
     * @param[in] inPixels input data
     * @param[in] inPixelStride distance between 2 pixels in the input in bytes
     * @param[in] NpixelsPerCh number of pixels to convert
     * @param[out] outPixels output data
     *
     * @return number of written bytes
     */
    IMG_SIZE (*pfnConverter)(const IMG_UINT16 *inPixels,
