 ...
 *
 * Implements the IMGMMU Heap functions and use a private structure container to store the TAL handles to ADDR objects.
 *
...
/**
 * @brief The Heap information - contains an IMGMMU_Heap that is given to the caller
 */
struct TAL_Heap
{
	ADDR_sContext sTalHeapCtx; ///< @brief TAL heap context
	ADDR_sRegion sTalHeapRegion; ///< @brief TAL Heap region associated to the heap context
	const char *pszMemoryRegionName; ///< @brief TAL memory region name (from configuration)

	IMG_UINT32 uiNAlloc; ///< @brief Number of associated allocations
	IMG_SIZE uiAllocSize; ///< @brief current total alloc size - allocation may not be contiguous but helps for debugging
	IMGMMU_Heap sHeapInfo; ///< @brief MMU lib heap info part (public element)
};
...
{
	struct TAL_Heap *pHeap;	///< @brief Associated heap
...
IMGMMU_Heap* IMGMMU_HeapCreate(IMG_UINTPTR uiVirtAddrStart, IMG_SIZE uiAllocAtom, IMG_SIZE uiSize, IMG_RESULT *pResult)
{
	...
	{
		MMU_LogError("Wrong input param %" IMG_SIZEPR "u not multiple of %" IMG_SIZEPR "u (%u), %" IMG_SIZEPR "u not multiple of %" IMG_SIZEPR "u (%u)\n",
			uiSize, uiAllocAtom, uiSize%uiAllocAtom, uiVirtAddrStart, uiAllocAtom, uiVirtAddrStart%uiAllocAtom);
...
IMGMMU_HeapAlloc* IMGMMU_HeapAllocate(IMGMMU_Heap *pHeap, IMG_SIZE uiSize, IMG_RESULT *pResult)
{
	...
	proposedAddr = ADDR_CxMalloc1(&(pInternalHeap->sTalHeapCtx), (IMG_CHAR*)pInternalHeap->pszMemoryRegionName, uiSize, pHeap->uiAllocAtom );
	pNeo->sVirtualMem.uiVirtualAddress = (IMG_UINTPTR)proposedAddr; // because of 32b/64b difference of size in uintptr_t and that CxMalloc1 returns -1 as wrong address instead of having a return code
	pNeo->sVirtualMem.uiAllocSize = uiSize;
	pNeo->pHeap = pInternalHeap;

	// ADDR_CxMalloc1 put this value as default... so it is a mess because 32b and 64b need a casting or not...
	if ( proposedAddr == -1LL )
	{
...
IMG_RESULT IMGMMU_HeapFree(IMGMMU_HeapAlloc *pHeapAlloc)
{
	...
	pInternalAlloc = container_of(pHeapAlloc, struct TAL_HeapAlloc, sVirtualMem);

	IMG_ASSERT(pInternalAlloc->pHeap->uiNAlloc > 0);
	pInternalAlloc->pHeap->uiNAlloc--;
	pInternalAlloc->pHeap->uiAllocSize -= pInternalAlloc->sVirtualMem.uiAllocSize;

	ADDR_CxFree(&(pInternalAlloc->pHeap->sTalHeapCtx), (IMG_CHAR*)pInternalAlloc->pHeap->pszMemoryRegionName, pHeapAlloc->uiVirtualAddress);
