...
******************************************************************************
 ...
 @note if TAL_MEM_NAME defined it will be used as a prefix for the pdump device
 memory names
...
struct AllocCell
{
    IMG_HANDLE talHandle; // parent allocator handle
    IMG_UINT64 ui64Offset; // index in the map - duplicated for ease of search
};
...
struct IntMMUPage
{
	...
	IMG_HANDLE sMemspaceCB; ///< @brief Memory handle (TAL_sMemspaceCB*) used for allocation - needed to run pdump variables commands
    struct AllocCell sAllocCell; ///< @brief store this address as physical page
};
...
struct MMUPage* IMGMMU_TALPageAlloc()
{
	...
	pNeo = (struct IntMMUPage*)IMG_CALLOC(1, sizeof(struct IntMMUPage));
	if ( pNeo == NULL )
	{
		return NULL;
	}

	pCpuVirt = IMG_MALLOC(IMGMMU_GetCPUPageSize());
	if ( pCpuVirt == NULL )
	{
		IMG_FREE(pNeo);
		return NULL;
...
#ifdef TAL_MEM_MMU_NAME
    {
        ...
        sprintf(name, "%s%d", TAL_MEM_MMU_NAME, pageallocnb++);
        allocName = name;
    }
#endif

	if ( TALMEM_Malloc(g_pTalMemoryRegionHandle, (IMG_CHAR*)pCpuVirt, IMGMMU_GetCPUPageSize(), IMGMMU_GetCPUPageSize(), &(tmpHandle), MMU_MALLOC_UPDATE_DEVICE, allocName) != IMG_SUCCESS )
	{
		IMG_FREE(pCpuVirt);
		IMG_FREE(pNeo);
		return NULL;
	}
	pNeo->sMemspaceCB = g_pTalMemoryRegionHandle;
	pNeo->sPage.uiCpuVirtAddr = (IMG_UINTPTR)pCpuVirt;
    pNeo->sAllocCell.talHandle = tmpHandle;
    pNeo->sAllocCell.ui64Offset = TALDEBUG_GetDevMemAddress(tmpHandle);
    pNeo->sPage.uiPhysAddr = pNeo->sAllocCell.ui64Offset;//(IMG_UINT64)&(pNeo->sAllocCell);
	
    IMG_ASSERT(g_allocCellMap.find(pNeo->sAllocCell.ui64Offset) == g_allocCellMap.end());
    g_allocCellMap[pNeo->sAllocCell.ui64Offset] = pNeo->sAllocCell;

	return &(pNeo->sPage); // returns the address to the public part of the structure
}
...
void IMGMMU_TALPageFree(struct MMUPage *pPage)
{
    ...
    std::map<IMG_UINT64, struct AllocCell>::iterator it = g_allocCellMap.find(pIntPage->sAllocCell.ui64Offset);
...
void IMGMMU_TALPageWrite(struct MMUPage *pWriteTo, unsigned int offset, IMG_UINT64 uiToWrite, unsigned int flags)
{
    struct IntMMUPage *pIntPage = NULL;
	...
	if ( uiToWrite != 0 )
	{
        ...
        std::map<IMG_UINT64, struct AllocCell>::const_iterator it = g_allocCellMap.find(toSearch);
        IMG_ASSERT(it != g_allocCellMap.end());
        ...
        result = TALINTVAR_WriteMemRef(pToWrite->talHandle, uiToWrite-pToWrite->ui64Offset, pIntPage->sMemspaceCB, TAL_INT_VAR);
		
		if ( result == IMG_SUCCESS && (g_MMU_PHYS_SIZE-IMGMMU_VIRT_SIZE) > 0 )
		{
			// does a right shift to fit a 40b address in 32b - if not used and the MMU is more than 32b the "upper address" register of the MMU HW can be used to set which 4GB of the memory should be used
			result = TALINTVAR_RunCommand(TAL_PDUMPL_INTREG_SHR, pIntPage->sMemspaceCB, TAL_INT_VAR, pIntPage->sMemspaceCB, TAL_INT_VAR, pIntPage->sMemspaceCB, (g_MMU_PHYS_SIZE-IMGMMU_VIRT_SIZE), IMG_FALSE);
		}

		if ( result == IMG_SUCCESS )
		{
			result = TALINTVAR_RunCommand(TAL_PDUMPL_INTREG_OR, pIntPage->sMemspaceCB, TAL_INT_VAR, pIntPage->sMemspaceCB, TAL_INT_VAR, pIntPage->sMemspaceCB, flags, IMG_FALSE);
		}

		if ( result == IMG_SUCCESS )
		{
			// memory is stored in 32b -> offset of entry*4
            result = TALINTVAR_WriteToMem32(pIntPage->sAllocCell.talHandle, offset*4, pIntPage->sMemspaceCB, TAL_INT_VAR);
		}
...
void IMGMMU_TALPageUpdate(struct MMUPage *pPage)
{
    ...
    // as the LDB will override the dynamic addresses written in IMGMMU_TALPageWrite
    // but some updates may be needed on systems with a cache!
    // it is however useful when debugging page failures (to see state of MMU pages)
    IMG_ASSERT(g_pTalMemoryRegionHandle != NULL);
...
IMGMMU_TALAlloc* TALALLOC_Malloc(IMG_HANDLE sTalMemHandle, IMG_SIZE uiSize, IMG_RESULT *pResult)
{
    IMG_INT32 i32NCPUPages = 0; // number of CPU pages
    IMG_INT32 i;
    IMG_INT32 nDevPerCPUPage = 1;
	IMGMMU_TALAlloc *pNeo = NULL;
	...
	pNeo = (IMGMMU_TALAlloc*)IMG_CALLOC(1, sizeof(IMGMMU_TALAlloc));
	if ( pNeo == NULL )
	{
		goto pNeo_failed;
	}

	pNeo->aPhysAlloc = (IMG_HANDLE*)IMG_CALLOC(i32NCPUPages, sizeof(IMG_HANDLE));
	if ( pNeo->aPhysAlloc == NULL )
	{
		goto aPhysAlloc_failed;
	}
    pNeo->aDevAddr = (IMG_UINT64*)IMG_CALLOC(i32NCPUPages, sizeof(IMG_UINT64));
	if ( pNeo->aDevAddr == NULL )
	{
		goto aDevAddr_failed;
	}

    pNeo->uiAllocSize = i32NCPUPages*pagesize;
	pNeo->pHostLinear = IMG_MALLOC(pNeo->uiAllocSize);
	pNeo->sTalMemHandle = sTalMemHandle;
	pNeo->uiNCPUPages = i32NCPUPages;

	if ( pNeo->pHostLinear == NULL )
	{
	...
	{
		char *pCPU = (char*)pNeo->pHostLinear;
        char *allocName = NULL;
		IMG_HANDLE tmpHandle;
#ifdef TAL_MEM_NAME
        char name[64];
        {
            ...
            IMG_ASSERT(strlen(TAL_MEM_NAME) <= 64-6); // 10^6 block is 4GB for 4kB blocks..

            sprintf(name, "%s%d", TAL_MEM_NAME, allocnb++);
            allocName = name;
        }
#endif
		...
		if ( TALMEM_Malloc(sTalMemHandle, pCPU, pagesize, 32, &(tmpHandle), IMG_FALSE, allocName) != IMG_SUCCESS )
		{
			goto dev_add_failed;
		}
		
		// use the TAL handle as a physical address (it will be converted back when using IMGMMU_TALPageWrite)
		pNeo->aPhysAlloc[i] = tmpHandle;// TO_ADDR(tmpHandle);
        pNeo->aDevAddr[i] = TALDEBUG_GetDevMemAddress(tmpHandle);
        
        struct AllocCell sCell;
        sCell.talHandle = tmpHandle;
        sCell.ui64Offset = pNeo->aDevAddr[i];

        IMG_ASSERT(g_allocCellMap.find(sCell.ui64Offset) == g_allocCellMap.end());
...
dev_add_failed:
    ...
    while(i>=0)
    {
        pNeo->aDevAddr[i] = 0; // 0 isn't really illegal but it cleans a bit
	    TALMEM_Free((IMG_HANDLE*)&(pNeo->aPhysAlloc[i]));
	    i--;
    }
    IMG_FREE(pNeo->pHostLinear);
pHostLinear_failed:
    IMG_FREE(pNeo->aDevAddr);
aDevAddr_failed:
    IMG_FREE(pNeo->aPhysAlloc);
aPhysAlloc_failed:
    IMG_FREE(pNeo);
pNeo_failed:
...
IMG_RESULT TALALLOC_Free(IMGMMU_TALAlloc *pAlloc)
{
    ...
    for ( i = 0 ; i < pAlloc->uiNCPUPages ; i++ )
	{
        ...
        std::map<IMG_UINT64, struct AllocCell>::iterator it = g_allocCellMap.find(
            TALDEBUG_GetDevMemAddress(pAlloc->aPhysAlloc[i]));
        IMG_ASSERT(it != g_allocCellMap.end());
        g_allocCellMap.erase(it);
...
IMG_RESULT TALALLOC_UpdateHostRegion(IMGMMU_TALAlloc *pAlloc, IMG_UINT64 offset, IMG_UINT64 size)
{
    IMG_INT64 page_idx;
	...
	ret = TALMEM_UpdateHostRegion(pAlloc->aPhysAlloc[page_idx++], first_adress, first_size);
	while(remaining_size > 0 && ret == IMG_SUCCESS)
	{
        current_size = std::min(remaining_size, (IMG_INT64)pagesize);
		ret = TALMEM_UpdateHostRegion(pAlloc->aPhysAlloc[page_idx++], 0, current_size);
...
IMG_RESULT TALALLOC_UpdateDeviceRegion(IMGMMU_TALAlloc *pAlloc, IMG_UINT64 offset, IMG_UINT64 size)
{
    IMG_INT64 page_idx;
	IMG_UINT64 first_page, first_adress, first_size;
	IMG_INT64 remaining_size, current_size;
	...
	{
        current_size = std::min(remaining_size, (IMG_INT64)pagesize);
		ret = TALMEM_UpdateDeviceRegion(pAlloc->aPhysAlloc[page_idx++], 0, current_size);
