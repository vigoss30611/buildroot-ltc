    ...
    struct MMUDirectory *pDirectory, IMG_RESULT *pResult)
{
    ...
    pNeo = (struct MMUPageTable*)IMG_CALLOC(1, sizeof(struct MMUPageTable));
    if ( pNeo == NULL )
    {
    ...
    pNeo->pPage = pDirectory->sConfiguration.pfnPageAlloc();
    if ( pNeo->pPage == NULL )
    {
    ...
    MMU_LogDebug("Create page table (phys addr 0x%" IMG_I64PR "x CPU Virt 0x%"\
        IMG_PTRDPR "x)\n", pNeo->pPage->uiPhysAddr, 
        pNeo->pPage->uiCpuVirtAddr);
    ...
    IMG_RESULT *pResult)
{
    ...
    pNeo = (struct MMUDirectory*)IMG_CALLOC(1, sizeof(struct MMUDirectory));
    if ( pNeo == NULL )
    {
    ...
    pNeo->pDirectoryPage = pMMUConfig->pfnPageAlloc();
    ...
    unsigned int uiMapFlags, IMG_RESULT *pResult)
{
    ...
    IMG_MEMCPY(&(pNeo->sDevVirtAddr), pDevVirtAddr, sizeof(IMGMMU_HeapAlloc));
    ...
    for ( i = 0 ; i < ui32NEntries ; i++ )
    {
        for ( d = 0 ; d < duplicate ; d++ )
        {
            ...
            if ( pDirectory->ppPageMap[dirOffset] == NULL )
            {
                pDirectory->ppPageMap[dirOffset] = mmu_PageTableCreate(pDirectory, pResult);
                if ( pDirectory->ppPageMap[dirOffset] == NULL )
                {
                    MMU_LogError("failed to create a non-existing page table\n");
