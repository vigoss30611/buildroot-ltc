...
******************************************************************************
 ...
 @brief Allocation using TAL Memory interface - example of non Unified Memory Access allocator
 ...
 *
 * @warning All memory operations on blocks allocated by TALALLOC should be using TALALLOC_ComputeOffset() to get the correct TAL IMG_HANDLE/ offset for that operation.
 *
 * @ingroup IMGMMU_lib
 * @{
 */
 ...
 *
 * @note The address of pPage->sMMUPage can be used as the first MMUPage of the chain used for allocation when invocking IMGMMU_DirectoryMap().
 */
typedef struct MMU_TALAlloc
{
	void *pHostLinear; ///< @brief linear copy of all the sub-pages - this is "host" memory
	IMG_SIZE uiAllocSize; ///< @brief allocation size in bytes
	
	IMG_SIZE uiNCPUPages; ///< @brief number of pages associated with this allocation in aPhysAlloc
	
	IMG_HANDLE sTalMemHandle; ///< @brief associated TAL memory region with the tal device memory (TAL_sMemSpaceCB*)
    
	IMG_HANDLE *aPhysAlloc; ///< @brief physical address for the CPU pages - this is just casted IMG_HANDLE (TAL_psDeviceMemCB)

    IMG_UINT64 *aDevAddr; ///< @brief physical addresses to give when mapping using the MMU
} IMGMMU_TALAlloc;
 ...
 * A "host" linear memory will also be allocated to allow direct modification of the memory.
 *
 * @warning The allocated host linear memory is not memset to any value (it is using IMG_MALLOC)
 *
 * It is a non Unified Memory Access system, therefore memory should be updated:
 ...
 * @param uiSize in bytes to allocate (will be round up to match the number of pages) - asserts that it is not NULL
 * @param pResult result output - asserts that it is not NULL
 *
 * @return The pointer to the IMGMMU_TALAlloc structure and pResult to IMG_SUCCESS
 * @return NULL in case of error and pResult contains:
 * @li IMG_ERROR_INVALID_PARAMETERS if the allocation failed to round up the size
 * @li IMG_ERROR_MALLOC_FAILED if the structure allocation failed (including its components and the TAL host linear equivalent of the memory to allocate)
...
/**
 * @brief Returns the IMG_HANDLE to use with TAL to read/write at this offset of the allocation.
 *
 ...
 * @code
 ...
 if ( (pCurrMem = TALALLOC_ComputeOffset(pMMUAlloc, &uiOff2)) == NULL )
 {
 ...
 * @param[in,out] uiOff the current offset value (and output the result offset) - asserts it is not NULL
 *
 ...
 * @brief Function that can be used when compiling the MMU lib with TAL allocator
 * @{
 */
...
/**
 * @brief This function can be used to set the memory region name (from the TAL) associated with a heap
 *
 * @param pHeap the heap to alterate (asserts if NULL)
 * @param pszMemoryRegionName name of the memory region in the TAL
 */
void IMGMMU_TALHeapSetMemRegion(struct MMUHeap *pHeap, const char *pszMemoryRegionName);

/**
 * @brief Set TAL handle (TAL_sMemspaceCB*) to use from now on in IMGMMU_TALPageAlloc()
 */
 ...
 *
 * @note TALMEM_Malloc() is called without the UpdateDevice according to compilation time macro MMU_MALLOC_UPDATE_DEVICE (default to IMG_TRUE)
 *
 ...
 *
 * @note The given physical address is casted to a IMG_HANDLE (TAL_psDeviceMemCB) generated when allocating using TALALLOC_Malloc() - if 0 then the casting is not done
 *
 ...
 *
 * @warning Do not use to have pdumps that are replayable if memory base address changed
 */
void IMGMMU_TALPageUpdate(struct MMUPage *pPage);

/**
 * @brief Similar to IMGMMU_DirectoryGetPage() but get the TAL memory handle directly
 *
