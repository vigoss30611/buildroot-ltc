...
IMG_UINT32 mmu_GetPCEntry(IMG_UINT32 ui32MMUId, IMG_UINT64 ui64VirtAddr, IMG_PUINT64 pui64PCEntry)
{
	...
	{
		printf("WARNING: Cannot read from page directory, address 0x%" IMG_I64PR "X\n", ui64PCAddr);
...
IMG_UINT32 mmu_GetPageTableEntry(IMG_UINT32 ui32MMUId, IMG_UINT64 ui64VirtAddr, IMG_UINT64 ui64PDEntry, IMG_PUINT64 pui64PageTableEntry)
{
	...
	{
		printf("WARNING: Cannot read from page table, address 0x%" IMG_I64PR "X\n", ui64PageTabAddr);
		return IMG_ERROR_INVALID_PARAMETERS;
	}
	
	if ((*pui64PageTableEntry & MMU_VALID_MASK) == 0)
	{
		printf("WARNING: Invalid Page Table Entry at address 0x%" IMG_I64PR "X, 0x%" IMG_I64PR "X\n", ui64PageTabAddr, *pui64PageTableEntry);
...
{	
	...
	{
	case MMU_TYPE_VP_40BIT:
	case MMU_TYPE_VARIABLE_PAGE_SIZE_32BIT:
	...
	case MMU_TYPE_4K_PAGES_32BIT_256B_TILING:
	case MMU_TYPE_4K_PAGES_36BIT_512B_TILING:
	case MMU_TYPE_4K_PAGES_32BIT_512B_TILING:
	case MMU_TYPE_4K_PAGES_36BIT_256B_TILING:
	case MMU_TYPE_4K_PAGES_40BIT_256B_TILING:
	case MMU_TYPE_4K_PAGES_40BIT_512B_TILING:
...
IMG_VOID mmu_ErrorOutput(IMG_UINT32 ui32MMUId, IMG_UINT64 ui64VirtAddr)
{
	...
	{
		fprintf(hErrorFile, "Page Catalogue Address: 0x%" IMG_I64PR "X\n", psContextCB->ui64PCAddress);
	...
	fprintf(hErrorFile, "Page Directory Address: 0x%" IMG_I64PR "X\n", ui64PageDirAddress);
	...
	fprintf(hErrorFile, "Page Table Address: 0x%" IMG_I64PR "X\n", ui64PageTabAddr);
...
)
{
	...
	while (ui64SizeRemaining > 0)
    {
		...
		if ((bTiled == IMG_FALSE) && (psContextCB->ui32MMUType ==  MMU_TYPE_VARIABLE_PAGE_SIZE_32BIT) && ((ui64CurDevVirtAddr & THIS_BUS_WIDTH.ui64Vaddr_PTandPageMask) == 0))
			ui32ReadSize = mmu_GetPageSize(ui32MMUId, ui64VirtAddr);
        ...
        memset(pui8Buffer, 0, ui32BytesToProcess);
		...
		if (rResult != IMG_SUCCESS)
		{
            ...
            /* Processed by Pdump2ToPdump1 to indicate a page fault */
			if (psContextCB->pfnReadPage(ui64CurDevVirtAddr, ui32BytesToProcess, 0) != IMG_SUCCESS)
				return IMG_ERROR_INVALID_PARAMETERS;
		}
		else
		{
...
{
	MMU_sContextCB			*psContextCB = mmu_GetContext(ui32MMUId);
	IMG_UINT64				ui64PageDirAddr = 0, ui64PageDirEntryAddr = 0;
