    ...
    struct CI_SIZEINFO *pResult)
{
    ...
    if ( pResult->ui32Stride != 0 ) 
    {
        ...
        // warning: supported formats in YUV for Felix are only Byte interleave
        ...
        if ( pTileInfo )
        {
            ...
            IMG_UINT32 tmp = maxLCbStr; // use same stride for luma and chroma
            while ( tmp >>= 1 )
            {
                StrLog2++;
            }
            ...
            tmp = pResult->ui32CHeight%(pTileInfo->ui32TilingHeight);
            if ( tmp != 0 )
            {
                pResult->ui32CHeight += pTileInfo->ui32TilingHeight - tmp;
            }
