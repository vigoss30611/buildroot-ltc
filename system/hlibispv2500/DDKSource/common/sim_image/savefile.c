    ...
    IMG_UINT16 **pOutput, IMG_SIZE *pOutsize)
{
    ...
    *pOutsize = imageSize[0]*imageSize[1]*sizeof(IMG_UINT16);
    outputBuffer = (IMG_UINT16*)IMG_MALLOC(*pOutsize);
    ...
    for ( l = 0 ; l < imageSize[1]/2 ; l++ )
    {
        ...
        for ( i = 0 ; i < width32 && o < outWidth16 ; )
        {
            ...
            for ( p = 0 ; p < 2*8 ; p++ )
            {
                ...
                outOdd[p] = (pOdd[i]>>(32-leftBits))&mask; // for the moment
                ...
                else if ( i+1 < width32 )
                {
                    ...
                    {
                        outEven[p] |=
                            (pEven[i]&((1<<(12-leftBits))-1))<<(leftBits);
                        outOdd[p] |=
                            (pOdd[i]&((1<<(12-leftBits))-1))<<(leftBits);
                    }
    ...
    IMG_UINT16 **pOutput, IMG_SIZE *pOutsize)
{
    ...
    *pOutput = outputBuffer;
    ...
    for ( l = 0 ; l < imageSize[1]/2 ; l++ )
    {
        ...
        for ( i = 0 ; i+2 < width32 && o < outWidth16 ; )
        {
    ...
    IMG_SIZE stride, IMG_UINT8 **pOutput, IMG_SIZE *pOutsize)
{
    ...
    *pOutsize = imageSize[0]*imageSize[1]*sizeof(IMG_UINT8); 
    outputBuffer = (IMG_UINT8*)IMG_MALLOC(*pOutsize);
