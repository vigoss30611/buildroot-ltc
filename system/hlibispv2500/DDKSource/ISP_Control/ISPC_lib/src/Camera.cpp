    ...
    bool updateControl)
{
    ...
    if (this->pipeline->getCIPipeline()->uiTotalPipeline <= 1)
    {
        if (updateControl)
        {
            if (!shot.bFrameError)
            {
#ifdef INFOTM_ISP
                ret = control.runControlModules(UPDATE_SHOT, shot.metadata, shot.metadata);
#else
            ...
            {
#ifdef INFOTM_ISP
                LOG_WARNING("context: %d, frame is erroneous - skipping control "\
                    "module writing\n", ctxNumber);
#else
...
#ifdef INFOTM_ISP
IMG_RESULT ISPC::Camera::DualSensorUpdate(bool updateControl, Shot &shot, Shot &shot2)
{
    IMG_RESULT ret;

    if (updateControl)
    {
        if (!shot.bFrameError && !shot2.bFrameError)
        {
            ret = control.runControlModules(UPDATE_SHOT, shot.metadata, shot2.metadata);
            if (ret)
            {
                LOG_ERROR("Unable to run control modules\n");
                return ret;
...
IMG_RESULT ISPC::Camera::setYUVConst(int flag)
{

    if (CAM_CAPTURING != state)
    {
        LOG_ERROR("Can't change encoder buffer address when is not running");
        return IMG_ERROR_UNEXPECTED_STATE;
    }
    ...
    return pipeline->setYUVConst(flag);
}
...
//#endif
//
#ifdef AUTO_CHANGE_TNM_STRENGTH
IMG_RESULT ISPC::Camera::DynamicChangeToneMappingStrength()
{
    ControlAE *pAE = getControlModule<ISPC::ControlAE>();
    ISPC::ModuleTNM *pTNM = pipeline->getModule<ISPC::ModuleTNM>();

    static double fOriginalFlatFactor       = pTNM->fFlatFactor; 
    static double fOriginalStaturation      = pTNM->fColourSaturation;
    static double fOriginalColourConfidence = pTNM->fColourConfidence;
    static double fOriginalFlatMin          = pTNM->fFlatMin;
    static double fOriginalWeightLine       = pTNM->fWeightLine;
    static double fOriginalWeightLocal      = pTNM->fWeightLocal;


    if (NULL == this->getSensor()) {
        LOG_ERROR("Sensor does not exist (== NULL)\n");
        return IMG_ERROR_FATAL;
    }

    if (NULL == pAE) {
        LOG_ERROR("ControlAE does not enable (== NULL)\n");
        return IMG_ERROR_FATAL;
    }
#ifdef INFOTM_Q360_PROJECT

    if(AvgGain <= 4.5)
    {
        pTNM->fFlatFactor       = fOriginalFlatFactor;
        pTNM->fColourSaturation = fOriginalStaturation;

        pTNM->fColourConfidence = fOriginalColourConfidence;
        pTNM->fFlatMin          = fOriginalFlatMin;
        pTNM->fWeightLine       = fOriginalWeightLine;
        pTNM->fWeightLocal      = fOriginalWeightLocal;
    ...
    {
        pTNM->fFlatFactor = fOriginalFlatFactor*2/5;
        pTNM->fColourSaturation = fOriginalStaturation*4/5;

        pTNM->fColourConfidence = fOriginalColourConfidence*2/3;
        pTNM->fFlatMin          = fOriginalFlatMin/2;
        pTNM->fWeightLine       = fOriginalWeightLine*3/4;
        pTNM->fWeightLocal      = fOriginalWeightLocal*2/3;
    ...
    {
        pTNM->fFlatFactor = fOriginalFlatFactor/6;
        pTNM->fColourSaturation = fOriginalStaturation*3/5;

        pTNM->fColourConfidence = fOriginalColourConfidence*2/5;
        pTNM->fFlatMin          = fOriginalFlatMin/4;
        pTNM->fWeightLine       = fOriginalWeightLine/2;
        pTNM->fWeightLocal      = fOriginalWeightLocal*2/5;
    ...
    {
        pTNM->fFlatFactor = fOriginalFlatFactor/8;
        pTNM->fColourSaturation = fOriginalStaturation/2;

        pTNM->fColourConfidence = fOriginalColourConfidence*2/5;
        pTNM->fFlatMin          = fOriginalFlatMin/5;
        pTNM->fWeightLine       = fOriginalWeightLine/3;
        pTNM->fWeightLocal      = fOriginalWeightLocal/5;
...
#ifdef AUTO_CHANGE_DNS_STRENGTH
#define DNS_STRENGTH_RANGE 0.3
IMG_RESULT ISPC::Camera::DynamicChangeDenoiseStrength()
{
    ControlAE *pAE = getControlModule<ISPC::ControlAE>();
    ModuleDNS *pDNS = this->pipeline->getModule<ISPC::ModuleDNS>();
    double dbDnsStrengthRange;
    double dbGainRange;
    double dbNewDnsStrength;

    static double dbOriginalDnsStrength = pDNS->fStrength;;
    ...
    if (NULL == pDNS) {
        LOG_ERROR("ControlDNS does not enable (== NULL)\n");
        return IMG_ERROR_FATAL;
    }

#if !defined(INFOTM_Q360_PROJECT)

    dbDnsStrengthRange = dbOriginalDnsStrength - DNS_STRENGTH_RANGE;
    if (0.0 > dbDnsStrengthRange) {
        dbDnsStrengthRange = 0.0;
    }
    dbDnsStrengthRange = dbOriginalDnsStrength - dbDnsStrengthRange;

    dbGainRange = this->getSensor()->getMaxGain() - this->getSensor()->getMinGain();
    dbNewDnsStrength = dbOriginalDnsStrength - (dbDnsStrengthRange / dbGainRange * (this->getSensor()->getGain() - this->getSensor()->getMinGain()));
    //printf("dbOriginalNdsStrength= %f, dbNewDnsStrength = %f\n", dbOriginalDnsStrength, dbNewDnsStrength);   
    //ModuleDNS

    pDNS->fStrength = dbNewDnsStrength;
#else
    if (AvgGain <= 3)
    {
         pDNS->bCombine = IMG_FALSE;
         pDNS->fStrength = dbOriginalDnsStrength;
    }
    else
    {
         pDNS->bCombine = IMG_TRUE;
         
         if (AvgGain <= 4.0)
         {
             pDNS->fStrength = 1.8;//0.6;
             
         }
         else if (AvgGain <= 5.0)
         {
             pDNS->fStrength = 2.0;//0.8;
             
         }
         else if (AvgGain <= 6.0)
         {
             pDNS->fStrength = 2.2;//1.0;
             
         }
         else if (AvgGain <= 7.0)
         {
             pDNS->fStrength = 2.4;//1.2;
         }
         else if (AvgGain <= 7.5)
         {
             pDNS->fStrength = 2.8;
...
#define SHA_STRENGTH_RANGE 0.1
#define SHA_EDGE_SCALER_RANGE 0.1

//According sensor gain to dynamic change sharpness strength and edge scaler for IPC project.
#ifdef AUTO_CHANGE_SHA_STRENGTH
IMG_RESULT ISPC::Camera::DynamicChangeSharpnessStrength()
{
    ControlAE *pAE = getControlModule<ISPC::ControlAE>();
    ISPC::ModuleSHA *pSHA = this->pipeline->getModule<ISPC::ModuleSHA>();

    static double dbOriginalShaStrength   = pSHA->fStrength;
    static double dbOriginalShaEdgeScaler = pSHA->fEdgeScale;
    static double dbOriginalDetail = pSHA->fDetail;
    static double dbOriginalDenoiseTau = pSHA->fDenoiseTau;
    static double dbOriginalDenoiseSigma = pSHA->fDenoiseSigma;
    ...
    if (NULL == pSHA) {
        LOG_ERROR("ControlSHA does not enable (== NULL)\n");
        return IMG_ERROR_FATAL;
    }
    
//    printf("==>old gain %f, strength %f, edgeScaler %f, detail %f, tau %f, sigma %f\n", 
//       this->getSensor()->getGain(), pSHA->fStrength, pSHA->fEdgeScale, pSHA->fDetail, pSHA->fDenoiseTau, pSHA->fDenoiseSigma);
#ifdef INFOTM_Q360_PROJECT

    if (AvgGain <= 4.0)
    {
        pSHA->fStrength     = dbOriginalShaStrength;
        pSHA->fEdgeScale    = dbOriginalShaEdgeScaler;
        pSHA->fDetail       = dbOriginalDetail;
        pSHA->fDenoiseTau   = dbOriginalDenoiseTau;
        pSHA->fDenoiseSigma = dbOriginalDenoiseSigma;
    }
    else if (AvgGain <= 5.0)
    {
        pSHA->fStrength     = (double)(dbOriginalShaStrength*4/5);
        pSHA->fEdgeScale    = (double)(dbOriginalShaEdgeScaler*2/3);
        pSHA->fDetail       = (double)(dbOriginalDetail/3);
        pSHA->fDenoiseTau   = (double)(dbOriginalDenoiseTau*3/2);
        pSHA->fDenoiseSigma = (double)(dbOriginalDenoiseSigma*3/2);
    }
    else if (AvgGain <= 6.0)
    {
        pSHA->fStrength     = (double)(dbOriginalShaStrength*3/5);
        pSHA->fEdgeScale    = (double)(dbOriginalShaEdgeScaler*1/2);
        pSHA->fDetail       = (double)(dbOriginalDetail/5);
        pSHA->fDenoiseTau   = (double)(dbOriginalDenoiseTau*5/4);
        pSHA->fDenoiseSigma = (double)(dbOriginalDenoiseSigma*5/4);
    }

    else if (AvgGain <= 7.0)
    {
        pSHA->fStrength     = (double)(dbOriginalShaStrength*3/5);
        pSHA->fEdgeScale    = (double)(dbOriginalShaEdgeScaler*1/3);
        pSHA->fDetail       = (double)(dbOriginalDetail/7);
        pSHA->fDenoiseTau   = (double)(dbOriginalDenoiseTau*2.5/2);
        pSHA->fDenoiseSigma = (double)(dbOriginalDenoiseSigma*3/2);
    }
    else
    {
        pSHA->fStrength     = dbOriginalShaStrength/6;
        pSHA->fEdgeScale    = dbOriginalShaEdgeScaler/10;
        pSHA->fDetail       = dbOriginalDetail/20;
        pSHA->fDenoiseTau   = dbOriginalDenoiseTau*2;
        pSHA->fDenoiseSigma = dbOriginalDenoiseSigma*3;
    }
...
#include <sys/time.h>
#include <unistd.h>
IMG_RESULT ISPC::Camera::DynamicChangeHistogramRegion()
{
    ControlAE *pAE = getControlModule<ISPC::ControlAE>();
    ISPC::ModuleHIS *pHIS = this->pipeline->getModule<ISPC::ModuleHIS>();
    ISPC::ModuleESC *pESC = this->pipeline->getModule<ISPC::ModuleESC>();

    static IMG_BOOL bOriginalEnableGlobal       = pHIS->bEnableGlobal;
    static IMG_BOOL bOriginalEnableROI          = pHIS->bEnableROI;
    static IMG_UINT32 aOriginalGridTileSize_0   = pHIS->aGridTileSize[0], aOriginalGridTileSize_1 = pHIS->aGridTileSize[1];
    static IMG_UINT32 aOriginalGridStartCoord_0 = pHIS->aGridStartCoord[0], aOriginalGridStartCoord_1 = pHIS->aGridStartCoord[1];

#ifdef INFOTM_Q360_PROJECT
    static double fOriginalTargetBrightness     = pAE->getTargetBrightness();
    static double fOriginalBracket              = pAE->getTargetBracket();
    if (!bOriginalEnableROI)
    {
        if (AvgGain <= 7.0)
        {
            if (AvgGain <= 1.0)
            {
                //pAE->setTargetBrightness(fOriginalTargetBrightness*11/10);
                pAE->setTargetBracket(fOriginalBracket/2);
            }
            if (AvgGain <= 5.0)
            {
                //pAE->setTargetBrightness(fOriginalTargetBrightness);
                pAE->setTargetBracket(fOriginalBracket);
            }
            else if (AvgGain <= 6.0)
            {
                //pAE->setTargetBrightness(fOriginalTargetBrightness*6.5/8);
                pAE->setTargetBracket(fOriginalBracket/2);
            }
            else if (AvgGain <= 6.5)
            {
                //pAE->setTargetBrightness(fOriginalTargetBrightness*4.5/6);
                pAE->setTargetBracket(fOriginalBracket/3);
            }
            else
            {
               // pAE->setTargetBrightness(fOriginalTargetBrightness/2);
                pAE->setTargetBracket(fOriginalBracket/3);
            }

            pHIS->bEnableGlobal = bOriginalEnableGlobal;
            pHIS->bEnableROI    = bOriginalEnableROI;
            
            pHIS->aGridStartCoord[0] = aOriginalGridStartCoord_0;
            pHIS->aGridStartCoord[1] = aOriginalGridStartCoord_1;
            
            pHIS->aGridTileSize[0] = aOriginalGridTileSize_0;
            pHIS->aGridTileSize[1] = aOriginalGridTileSize_1;
        }
        else
        {
            pAE->setTargetBrightness(fOriginalTargetBrightness/4);
            pAE->setTargetBracket(fOriginalBracket/5);
            pHIS->bEnableGlobal = IMG_TRUE;
            pHIS->bEnableROI    = IMG_TRUE;

            pHIS->aGridStartCoord[0] = 0;
            pHIS->aGridStartCoord[1] = 0;
            
            pHIS->aGridTileSize[0] = pESC->aRect[2];
            pHIS->aGridTileSize[1] = pESC->aRect[3];
...
/////////////////////////////////////////
    ...
    if (AvgGain <= 7.0)
    {
        ...
        {
            //pAE->setTargetBrightness(fOriginalTargetBrightness*11/10);
            pAE->setTargetBracket(fOriginalBracket*2/3);
        }
        if (AvgGain <= 5.0)
        {
           // pAE->setTargetBrightness(fOriginalTargetBrightness);
    ...
    {
        double NewGain = AvgGain;
        double CurrentBrightness = pAE->getCurrentBrightness();
        pAE->setTargetBrightness(fOriginalTargetBrightness/4);
        if (CurrentBrightness < fOriginalTargetBrightness && (pAE->getTargetBrightness() - CurrentBrightness) >= 0.06)
        {
            pAE->setTargetBracket(fOriginalBracket/4);
            
            pHIS->aGridStartCoord[0] = 0;
            pHIS->aGridStartCoord[1] = 0;
            
            pHIS->aGridTileSize[0] = pESC->aRect[2]/7;
            pHIS->aGridTileSize[1] = pESC->aRect[3]/7; 
        }
        else
        {
            pAE->setTargetBracket(fOriginalBracket/10);
            ...
            while (NewGain > (AvgGain/2))
            {
                NewGain -= 0.5;
                getSensor()->setGain(NewGain);
                usleep(1000);
            }
        }
    }
    ...
    //printf("&&&Bracket %lf, oriTBgt %lf, CBgt %lf, TBgt %lf, avgGain %lf , actGain %lf&&&\n", pAE->getTargetBracket(), fOriginalTargetBrightness, pAE->getCurrentBrightness(), pAE->getTargetBrightness(), AvgGain, this->getSensor()->getGain());
    pHIS->requestUpdate();
    return IMG_SUCCESS;
}
#endif

#ifdef AUTO_CHANGE_DPF_STRENGTH
IMG_RESULT ISPC::Camera::DynamicChangeDPFStrength()
{
        ControlAE *pAE = getControlModule<ISPC::ControlAE>();
        ISPC::ModuleDPF *pDPF = this->pipeline->getModule<ISPC::ModuleDPF>();
    
        static IMG_UINT32 aOriginalThreshold = pDPF->ui32Threshold;
            
#ifdef INFOTM_Q360_PROJECT
        if (AvgGain <= 4.0)
        {
            pDPF->ui32Threshold = aOriginalThreshold*2;
        }
        else if (AvgGain <= 5.0)
        {
            pDPF->ui32Threshold = aOriginalThreshold*3/2;
        }
        else if (AvgGain <= 6.0)
        {
            pDPF->ui32Threshold = aOriginalThreshold*5/4;
        }
        else if (AvgGain <= 7.0)
        {
            pDPF->ui32Threshold = aOriginalThreshold;
        }
        else if (AvgGain <= 8.0)
        {
            pDPF->ui32Threshold = aOriginalThreshold + 2;
...
IMG_RESULT ISPC::Camera::DynamicChangeModuleParam()
{
    // for Q360_PROJECT, other product will do nothing
#if defined(AUTO_CHANGE_SHA_STRENGTH) || defined(AUTO_CHANGE_DNS_STRENGTH) || defined(AUTO_CHANGE_TNM_STRENGTH) || defined(AUTO_CHANGE_HIS_REGION) || defined(AUTO_CHANGE_DPF_STRENGTH)
    CalcAvgGain();
#endif

#ifdef AUTO_CHANGE_SHA_STRENGTH
    DynamicChangeSharpnessStrength();
#endif

#ifdef AUTO_CHANGE_DNS_STRENGTH
    DynamicChangeDenoiseStrength();
#endif

#ifdef AUTO_CHANGE_TNM_STRENGTH
    DynamicChangeToneMappingStrength();
#endif

#ifdef AUTO_CHANGE_HIS_REGION
    DynamicChangeHistogramRegion();
#endif

#ifdef AUTO_CHANGE_DPF_STRENGTH
    DynamicChangeDPFStrength();
#endif
    return IMG_SUCCESS;
}
...
IMG_RESULT ISPC::Camera::DynamicChange3DDenoiseIdx(IMG_UINT32 & ui32DnTargetIdx, bool bForceCheck)
{
    ...
    if (pAE && (!(pAE->isConverged()) || bForceCheck)) {
        double Newgain = 0;
