...
void ISPC::Shot::clear()
{
    ...
    IMG_MEMSET(&(RGB), 0, sizeof(Buffer));
    RGB.pxlFormat = PXL_NONE;
    ...
    const ModuleOUT &globalConfig, const Global_Setup &globalSetup)
{
    ...
    ret = PixelTransformRGB(&stype, globalConfig.displayType);
    if (IMG_SUCCESS == ret)
    {
        // if it is RGB
        this->RGB.data =
            reinterpret_cast<IMG_UINT8 *>(pCIBuffer->pDisplayOutput);
#ifdef INFOTM_ISP		
		this->RGB.phyAddr = pCI_Buffer->pDisplayOutputPhyAddr;
#endif //INFOTM_ISP		

        this->RGB.stride = pCIBuffer->aDispSize[0];
        this->RGB.vstride = pCIBuffer->aDispSize[1];

        this->RGB.height = globalSetup.ui32DispHeight;
        this->RGB.width  = globalSetup.ui32DispWidth;

        this->RGB.pxlFormat = globalConfig.displayType;
        this->RGB.id = pCIBuffer->dispId;
        this->RGB.isTiled = pCIBuffer->bDispTiled == IMG_TRUE;
    }
    else
    {
    ...
    {
        this->YUV.data =
            reinterpret_cast< IMG_UINT8 *>(pCIBuffer->pEncoderOutput);
#ifdef INFOTM_ISP		
    ...
    {
        this->HDREXT.data =
            reinterpret_cast< IMG_UINT8 *>(pCIBuffer->pHDRExtOutput);
#ifdef INFOTM_ISP		
    ...
    {
        this->RAW2DEXT.data =
            reinterpret_cast< IMG_UINT8 *>(pCIBuffer->pRaw2DExtOutput);
#ifdef INFOTM_ISP		
