...
IMG_RESULT ISPC::ModuleVIB::save(ParameterList &parameters, SaveType t) const
{
    ...
    {
    case SAVE_VAL:
        parameters.addParameter(Parameter(VIB_ON.name,
            toString(this->bVibrancyOn)));
        ...
        parameters.addParameter(Parameter(VIB_SATURATION_CURVE.name, values));
        break;

    case SAVE_MIN:
        // bool does not have a min
        parameters.addParameter(Parameter(VIB_ON.name, toString(VIB_ON.def)));

        values.clear();
        for (i = 0 ; i < MIE_SATMULT_NUMPOINTS ; i++)
        {
            values.push_back(toString(VIB_SATURATION_CURVE.min));
        }
        parameters.addParameter(Parameter(VIB_SATURATION_CURVE.name, values));
        break;
    ...
    case SAVE_MAX:
        ...
        for (i = 0 ; i < MIE_SATMULT_NUMPOINTS ; i++)
        {
            values.push_back(toString(VIB_SATURATION_CURVE.max));
        }
        parameters.addParameter(Parameter(VIB_SATURATION_CURVE.name, values));
        break;
    ...
    case SAVE_DEF:
        ...
        values.push_back("// " + getParameterInfo(VIB_SATURATION_CURVE));
        parameters.addParameter(Parameter(VIB_SATURATION_CURVE.name, values));
        break;
    }
