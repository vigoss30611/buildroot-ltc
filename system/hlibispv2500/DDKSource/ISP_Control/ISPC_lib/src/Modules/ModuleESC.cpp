...
IMG_RESULT ISPC::ModuleESC::save(ParameterList &parameters, SaveType t) const
{
    ...
    case SAVE_VAL:
        ...
        {
            parameters.addParameter(Parameter(ESC_RECTTYPE.name,
                ESC_CLIP_STR));
        }
        ...
        {
            parameters.addParameter(Parameter(ESC_CHROMA_MODE.name,
        ...
        parameters.addParameter(Parameter(ESC_ADJUSTCUTOFF.name,
            toString(this->bAdjustTaps)));
        ...
        parameters.addParameter(Parameter(ESC_PITCH.name, values));
        ...
        parameters.addParameter(Parameter(ESC_RECT.name, values));
        break;

    case SAVE_MIN:
        parameters.addParameter(Parameter(ESC_RECTTYPE.name,
            ESC_RECTTYPE.def));  // rectype does not have a min
        parameters.addParameter(Parameter(ESC_CHROMA_MODE.name,
            ESC_CHROMA_MODE.def));  // chroma mode does not have a min
        parameters.addParameter(Parameter(ESC_ADJUSTCUTOFF.name,
            toString(ESC_ADJUSTCUTOFF.def)));
        // adjust cutoff does not have a min

        values.clear();
        for (i = 0 ; i < 2 ; i++)
        {
            values.push_back(toString(ESC_PITCH.min));
        }
        parameters.addParameter(Parameter(ESC_PITCH.name, values));
        ...
        for (i = 0 ; i < 4 ; i++)
        {
            values.push_back(toString(ESC_RECT.min));
        }
        parameters.addParameter(Parameter(ESC_RECT.name, values));
        break;
    ...
    case SAVE_MAX:
        parameters.addParameter(Parameter(ESC_RECTTYPE.name,
            ESC_RECTTYPE.def));  // rectype does not have a max
        parameters.addParameter(Parameter(ESC_CHROMA_MODE.name,
            ESC_CHROMA_MODE.def));  // chroma mode does not have a max
        parameters.addParameter(Parameter(ESC_ADJUSTCUTOFF.name,
            toString(ESC_ADJUSTCUTOFF.def)));
        // adjust cutoff does not have a max

        values.clear();
        for (i = 0 ; i < 2 ; i++)
        {
            values.push_back(toString(ESC_PITCH.max));
        }
        parameters.addParameter(Parameter(ESC_PITCH.name, values));
        ...
        for (i = 0 ; i < 4 ; i++)
        {
            values.push_back(toString(ESC_RECT.max));
        }
        parameters.addParameter(Parameter(ESC_RECT.name, values));
        break;
    ...
    case SAVE_DEF:
        {
            ...
            values.clear();
            values.push_back(ESC_RECTTYPE.def);
            values.push_back(defComment.str());

            parameters.addParameter(Parameter(ESC_RECTTYPE.name, values));
            ...
            values.clear();
            values.push_back(ESC_CHROMA_MODE.def);
            values.push_back(defComment.str());

            parameters.addParameter(Parameter(ESC_CHROMA_MODE.name, values));
            parameters.addParameter(Parameter(ESC_ADJUSTCUTOFF.name,
                toString(ESC_ADJUSTCUTOFF.def)
                + " // " + getParameterInfo(ESC_ADJUSTCUTOFF)));
        }

        values.clear();
        for ( i = 0 ; i < 2 ; i++ )
        {
            values.push_back(toString(ESC_PITCH.def[i]));
        }
        values.push_back("// " + getParameterInfo(ESC_PITCH));
        parameters.addParameter(Parameter(ESC_PITCH.name, values));

        values.clear();
        for ( i = 0 ; i < 4 ; i++ )
        {
            values.push_back(toString(ESC_RECT.def[i]));
        }
        values.push_back("// " + getParameterInfo(ESC_RECT));
        parameters.addParameter(Parameter(ESC_RECT.name, values));
        break;
    }
...
IMG_RESULT ISPC::ModuleESC::setup()
{
    ...
    // : if scaler factor is more that 4 VSS module should be use instead
    if (YVU_420_PL12_8 == glb->encoderType
        || YUV_420_PL12_8 == glb->encoderType
        || YVU_420_PL12_10 == glb->encoderType)
    {
        pMCPipeline->sESC.eSubsampling = EncOut420_scaler;
    }
    else
    {
