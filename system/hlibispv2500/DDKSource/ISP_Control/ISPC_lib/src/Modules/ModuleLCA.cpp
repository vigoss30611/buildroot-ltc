...
IMG_RESULT ISPC::ModuleLCA::save(ParameterList &parameters, SaveType t) const
{
    ...
    case SAVE_VAL:
        ...
        parameters.addParameter(Parameter(LCA_REDPOLY_X.name, values));
        ...
        parameters.addParameter(Parameter(LCA_REDPOLY_Y.name, values));
        ...
        parameters.addParameter(Parameter(LCA_BLUEPOLY_X.name, values));
        ...
        parameters.addParameter(Parameter(LCA_BLUEPOLY_Y.name, values));
        ...
        parameters.addParameter(Parameter(LCA_REDCENTER.name, values));
        ...
        parameters.addParameter(Parameter(LCA_BLUECENTER.name, values));
        ...
        parameters.addParameter(Parameter(LCA_SHIFT.name, values));
    ...
    case SAVE_MIN:
        ...
        for (i = 0 ; i < LCA_COEFFS_NO ; i++)
        {
            values.push_back(toString(LCA_POLY_MIN));
        }
        // same min for all polynomials
        parameters.addParameter(Parameter(LCA_REDPOLY_X.name, values));
        parameters.addParameter(Parameter(LCA_REDPOLY_Y.name, values));
        parameters.addParameter(Parameter(LCA_BLUEPOLY_X.name, values));
        parameters.addParameter(Parameter(LCA_BLUEPOLY_Y.name, values));
        ...
        for (i = 0 ; i < 2 ; i++)
        {
            values.push_back(toString(LCA_CENTER_MIN));
        }
        // same min for all centers
        parameters.addParameter(Parameter(LCA_REDCENTER.name, values));
        parameters.addParameter(Parameter(LCA_BLUECENTER.name, values));
        ...
        for (i = 0 ; i < 2 ; i++)
        {
            values.push_back(toString(LCA_SHIFT.min));
        }
        parameters.addParameter(Parameter(LCA_SHIFT.name, values));
        ...
        for (i = 0 ; i < 2 ; i++)
        {
            values.push_back(toString(LCA_DEC.min));
        }
        parameters.addParameter(Parameter(LCA_DEC.name, values));
        break;
    ...
    case SAVE_MAX:
        ...
        for (i = 0 ; i < LCA_COEFFS_NO ; i++)
        {
            values.push_back(toString(LCA_POLY_MAX));
        }
        // same max for all polynomials
        parameters.addParameter(Parameter(LCA_REDPOLY_X.name, values));
        parameters.addParameter(Parameter(LCA_REDPOLY_Y.name, values));
        parameters.addParameter(Parameter(LCA_BLUEPOLY_X.name, values));
        parameters.addParameter(Parameter(LCA_BLUEPOLY_Y.name, values));
        ...
        for (i = 0 ; i < 2 ; i++)
        {
            values.push_back(toString(LCA_CENTER_MAX));
        }
        // same max for all centers
        parameters.addParameter(Parameter(LCA_REDCENTER.name, values));
        parameters.addParameter(Parameter(LCA_BLUECENTER.name, values));
        ...
        for (i = 0 ; i < 2 ; i++)
        {
            values.push_back(toString(LCA_SHIFT.max));
        }
        parameters.addParameter(Parameter(LCA_SHIFT.name, values));
        ...
        for (i = 0 ; i < 2 ; i++)
        {
            values.push_back(toString(LCA_DEC.max));
        }
        parameters.addParameter(Parameter(LCA_DEC.name, values));
        break;
    ...
    case SAVE_DEF:
        ...
        for (i = 0 ; i < LCA_COEFFS_NO ; i++)
        {
            values.push_back(toString(LCA_POLY_DEF[i]));
        }
        values.push_back("// " + getParameterInfo(LCA_REDPOLY_X));

        // same default for all polynomials
        parameters.addParameter(Parameter(LCA_REDPOLY_X.name, values));
        parameters.addParameter(Parameter(LCA_REDPOLY_Y.name, values));
        parameters.addParameter(Parameter(LCA_BLUEPOLY_X.name, values));
        parameters.addParameter(Parameter(LCA_BLUEPOLY_Y.name, values));
        ...
        for (i = 0 ; i < 2 ; i++)
        {
            values.push_back(toString(LCA_CENTER_DEF[i]));
        }
        values.push_back("// " + getParameterInfo(LCA_REDCENTER));
        // same default for all centers
        parameters.addParameter(Parameter(LCA_REDCENTER.name, values));
        parameters.addParameter(Parameter(LCA_BLUECENTER.name, values));
        ...
        for (i = 0 ; i < 2 ; i++)
        {
            values.push_back(toString(LCA_SHIFT.def[i]));
        }
        values.push_back("// " + getParameterInfo(LCA_SHIFT));
        parameters.addParameter(Parameter(LCA_SHIFT.name, values));

        values.clear();
        for (i = 0 ; i < 2 ; i++)
        {
            values.push_back(toString(LCA_DEC.def[i]));
        }
        values.push_back("// " + getParameterInfo(LCA_DEC));
        parameters.addParameter(Parameter(LCA_DEC.name, values));
        break;
    }
