...
ISPC::ModuleIIF::ModuleIIF(): SetupModuleBase(LOG_TAG)
{
    ...
    aDecimation[0] = IIF_DECIMATION.def[0];
    aDecimation[1] = IIF_DECIMATION.def[1];
...
IMG_RESULT ISPC::ModuleIIF::save(ParameterList &parameters, SaveType t) const
{
    ...
    case SAVE_VAL:
        ...
        parameters.addParameter(Parameter(IIF_CAPRECT_TL.name, values));
        ...
        parameters.addParameter(Parameter(IIF_CAPRECT_BR.name, values));
        ...
        parameters.addParameter(Parameter(IIF_DECIMATION.name, values));

        if (pipeline->getSensor())
        {
            parameters.addParameter(Parameter(IIF_BAYERFMT.name,
    ...
    case SAVE_MIN:
        ...
        for (i = 0 ; i < 2 ; i++)
        {
            values.push_back(toString(IIF_CAPRECT_TL.min));
        }
        parameters.addParameter(Parameter(IIF_CAPRECT_TL.name, values));
        ...
        for (i = 0 ; i < 2 ; i++)
        {
            values.push_back(toString(IIF_CAPRECT_BR.min));
        }
        parameters.addParameter(Parameter(IIF_CAPRECT_BR.name, values));
        ...
        for (i = 0 ; i < 2 ; i++)
        {
            values.push_back(toString(IIF_DECIMATION.min));
        }
        parameters.addParameter(Parameter(IIF_DECIMATION.name, values));

        parameters.addParameter(Parameter(IIF_BAYERFMT.name,
            IIF_BAYERFMT.def));  // string does not have min
        break;
    ...
    case SAVE_MAX:
        ...
        for (i = 0 ; i < 2 ; i++)
        {
            values.push_back(toString(IIF_CAPRECT_TL.max));
        }
        parameters.addParameter(Parameter(IIF_CAPRECT_TL.name, values));
        ...
        for (i = 0 ; i < 2 ; i++)
        {
            values.push_back(toString(IIF_CAPRECT_BR.max));
        }
        parameters.addParameter(Parameter(IIF_CAPRECT_BR.name, values));
        ...
        for (i = 0 ; i < 2 ; i++)
        {
            values.push_back(toString(IIF_DECIMATION.max));
        }
        parameters.addParameter(Parameter(IIF_DECIMATION.name, values));

        parameters.addParameter(Parameter(IIF_BAYERFMT.name,
            IIF_BAYERFMT.def));  // string does not have max
        break;
    ...
    case SAVE_DEF:
        ...
        for (i = 0 ; i < 2 ; i++)
        {
            values.push_back(toString(IIF_CAPRECT_TL.def[i]));
        }
        values.push_back("// " + getParameterInfo(IIF_CAPRECT_TL));
        parameters.addParameter(Parameter(IIF_CAPRECT_TL.name, values));

        values.clear();
        for (i = 0 ; i < 2 ; i++)
        {
            values.push_back(toString(IIF_CAPRECT_BR.def[i]));
        }
        values.push_back("// " + getParameterInfo(IIF_CAPRECT_BR));
        parameters.addParameter(Parameter(IIF_CAPRECT_BR.name, values));

        values.clear();
        for (i = 0 ; i < 2 ; i++)
        {
            values.push_back(toString(IIF_DECIMATION.def[i]));
        }
        values.push_back("// " + getParameterInfo(IIF_DECIMATION));
        parameters.addParameter(Parameter(IIF_DECIMATION.name, values));

        {
            ...
            values.clear();
            values.push_back(IIF_BAYERFMT.def);
            values.push_back(defComment.str());

            parameters.addParameter(Parameter(IIF_BAYERFMT.name, values));
        }
        break;
    }
