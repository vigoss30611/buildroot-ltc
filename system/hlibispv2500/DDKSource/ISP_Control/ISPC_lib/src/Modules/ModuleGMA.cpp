...
const ISPC::ParamDefArray<IMG_UINT16> ISPC::ModuleGMA::CUSTOM_GAM_CURVE("CUSTOM_GAM_CURVE", 0,  65535, CUSTOM_GAM_CURVE_DEF, 63);
...
IMG_RESULT ISPC::ModuleGMA::save(ParameterList &parameters, SaveType t) const
{
    ...
    {
    case SAVE_VAL:
        parameters.addParameter(Parameter(GMA_BYPASS.name,
            toString(this->bBypass)));
        parameters.addParameter(Parameter(USE_CUSTOM_GAM.name,
            toString(this->useCustomGam)));
        ...
        parameters.addParameter(Parameter(CUSTOM_GAM_CURVE.name, values));
        break;

    case SAVE_MIN:
    case SAVE_MAX:
        parameters.addParameter(Parameter(GMA_BYPASS.name,
            toString(GMA_BYPASS.def)));  // bool does not have min/max
        break;
    case SAVE_DEF:
        parameters.addParameter(Parameter(GMA_BYPASS.name,
            toString(GMA_BYPASS.def)
            + " // " + getParameterInfo(GMA_BYPASS)));
        break;
    }
    return IMG_SUCCESS;
}
