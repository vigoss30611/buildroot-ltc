...
IMG_RESULT ISPC::ModuleDSC::save(ParameterList &parameters, SaveType t) const
{
    ...
    case SAVE_VAL:
        ...
        {
            parameters.addParameter(Parameter(DSC_RECTTYPE.name,
                DSC_CLIP_STR));
        }
        ...
        parameters.addParameter(Parameter(DSC_ADJUSTCUTOFF.name,
        ...
        parameters.addParameter(Parameter(DSC_PITCH.name, values));
        ...
        parameters.addParameter(Parameter(DSC_RECT.name, values));
        break;

    case SAVE_MIN:
        // DSC_RECTTYPE does not have a min
        parameters.addParameter(Parameter(DSC_ADJUSTCUTOFF.name,
            toString(DSC_ADJUSTCUTOFF.def)));  // bool do not have min

        values.clear();
        for (i = 0 ; i < 2 ; i++)
        {
            values.push_back(toString(DSC_PITCH.min));
        }
        parameters.addParameter(Parameter(DSC_PITCH.name, values));
        ...
        for (i = 0 ; i < 4 ; i++)
        {
            values.push_back(toString(DSC_RECT.min));
        }
        parameters.addParameter(Parameter(DSC_RECT.name, values));
        break;

    case SAVE_MAX:
        // DSC_RECTTYPE does not have a max
        parameters.addParameter(Parameter(DSC_ADJUSTCUTOFF.name,
            toString(DSC_ADJUSTCUTOFF.def)));  // bool do not have max

        values.clear();
        for (i = 0 ; i < 2 ; i++)
        {
            values.push_back(toString(DSC_PITCH.max));
        }
        parameters.addParameter(Parameter(DSC_PITCH.name, values));
        ...
        for (i = 0 ; i < 4 ; i++)
        {
            values.push_back(toString(DSC_RECT.max));
        }
        parameters.addParameter(Parameter(DSC_RECT.name, values));
        break;
    ...
    case SAVE_DEF:
        {
            ...
            values.clear();
            values.push_back(DSC_RECTTYPE.def);
            values.push_back(defComment.str());

            parameters.addParameter(Parameter(DSC_RECTTYPE.name, values));
            parameters.addParameter(Parameter(DSC_ADJUSTCUTOFF.name,
                toString(DSC_ADJUSTCUTOFF.def)
                + " // " + getParameterInfo(DSC_ADJUSTCUTOFF)));
        }

        values.clear();
        for (i = 0 ; i < 2 ; i++)
        {
            values.push_back(toString(DSC_PITCH.def[i]));
        }
        values.push_back("// " + getParameterInfo(DSC_PITCH));
        parameters.addParameter(Parameter(DSC_PITCH.name, values));

        values.clear();
        for (i = 0 ; i < 4 ; i++)
        {
            values.push_back(toString(DSC_RECT.def[i]));
        }
        values.push_back("// " + getParameterInfo(DSC_RECT));
        parameters.addParameter(Parameter(DSC_RECT.name, values));
        break;
    }
