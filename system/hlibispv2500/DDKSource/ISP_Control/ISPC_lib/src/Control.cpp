...
#ifdef INFOTM_ISP
IMG_RESULT ISPC::Control::runControlModule(ControlID id, const Metadata &metadata, const Metadata &metadata2)
#else
IMG_RESULT ISPC::Control::runControlModule(ControlID id, const Metadata &metadata)
...
#ifdef INFOTM_ISP
    return modIt->second->update(metadata, metadata2);
#else
...
#ifdef INFOTM_ISP
IMG_RESULT ISPC::Control::runControlModules(const Metadata &metadata, const Metadata &metadata2)
#else
...
#endif //INFOTM_DUAL_SENSOR
{
        ...
        modIt++)
    {
        ...
        {
#ifdef INFOTM_ISP
            ret = modIt->second->update(metadata, metadata2);
#else
...
#ifdef INFOTM_ISP
IMG_RESULT ISPC::Control::runControlModules(CtrlUpdateMode updateMode, const Metadata &metadata, const Metadata &metadata2)
#else
IMG_RESULT ISPC::Control::runControlModules(CtrlUpdateMode updateMode, const Metadata &metadata)
#endif //INFOTM_DUAL_SENSOR
{
    bool errorFlag = false;
    std::map<ControlID, ControlModule*>::iterator modIt;
