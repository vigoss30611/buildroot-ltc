...
int initializeFakeDriver(unsigned int useMMU, int gmaCurve, unsigned int pagesize, unsigned int tilingScheme, char *pszTransifLib, char *pszTransifSimParam)
{
#ifdef FELIX_FAKE
    ...
    LOG_INFO("Fake insmod: mmu=%d tiling=%d, gmaCurve=%d PAGE_SIZE=%d\n",
        useMMU, tilingScheme, gmaCurve, pagesize);

    if ( KRN_CI_DriverCreate(&sCIDriver, useMMU, tilingScheme, gmaCurve, &sDevice) != IMG_SUCCESS )
    {
...
#ifdef EXT_DATAGEN
    if ( KRN_DG_DriverCreate(&sDGDriver, useMMU) != IMG_SUCCESS )
    {
