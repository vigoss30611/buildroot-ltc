...
public:
     ...
     * @see releaseShot()
     */
    IMG_RESULT acquireShot(Shot &shot, bool block, bool updateControl);

#ifdef INFOTM_ISP
    IMG_RESULT DualSensorUpdate(bool updateControl, Shot &shot, Shot &shot2);
#endif //INFOTM_ISP
    ...
    //set YUV const
    IMG_RESULT setYUVConst(int flag);
...
//
    ...
    //According sensor gain to dynamic change denoise strength for IPC project.
    IMG_RESULT DynamicChangeDenoiseStrength();
#endif

#ifdef AUTO_CHANGE_SHA_STRENGTH
    //According sensor gain to dynamic change sharpness strength and edge scaler for IPC project.
    IMG_RESULT DynamicChangeSharpnessStrength();
#endif
#ifdef AUTO_CHANGE_HIS_REGION
    IMG_RESULT DynamicChangeHistogramRegion();
#endif

#ifdef AUTO_CHANGE_DPF_STRENGTH
    IMG_RESULT DynamicChangeDPFStrength();
#endif

#ifdef AUTO_CHANGE_TNM_STRENGTH
    IMG_RESULT DynamicChangeToneMappingStrength();
#endif

    IMG_RESULT DynamicChangeModuleParam();
