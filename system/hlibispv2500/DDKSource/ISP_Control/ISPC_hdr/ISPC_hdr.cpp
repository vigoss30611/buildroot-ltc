...
int HDR_Capture::configureCameras()
{
    ...
    this->pCamExtraction = new ISPC::Camera(0, uiSensorID, uiSensorMode,
            uiSensorFlip);
#endif
    if (ISPC::CAM_ERROR == pCamExtraction->state)
    {
    ...
    if (IMG_SUCCESS != ret || ISPC::CAM_ERROR == pCamExtraction->state)
    {
    ...
    if (ISPC::CAM_ERROR == pCamInsertion->state)
    {
        LOG_ERROR("failed to create extraction camera\n");
        return EXIT_FAILURE;
    }
    ...
    ret = ISPC::CameraFactory::populateCameraFromHWVersion(*pCamInsertion,
        pCamExtraction->getSensor());
    if (IMG_SUCCESS != ret || ISPC::CAM_ERROR == pCamInsertion->state)
    {
...
int HDR_Capture::runInsertion()
{
    ...
    if (PXL_NONE != hdr.RGB.pxlFormat)
    {
        str.str("");
        str << "hdr_display.flx";
        
        ret = ISPC::Save::Single(pipeline, hdr,
            ISPC::Save::RGB, str.str());
        if (IMG_SUCCESS != ret)
        {
