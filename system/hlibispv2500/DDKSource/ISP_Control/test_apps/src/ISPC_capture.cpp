...
int main(int argc, char *argv[])
{
    ...
    IMG_BOOL bSaveErroneous = IMG_TRUE;
...
fail_retry:
    ...
    // create a context for the driver connection
    {
...
#else
#ifdef INFOTM_ISP
        ISPC::Camera *camera = new ISPC::Camera(0, ISPC::Sensor::GetSensorId(pszSensor),
        ...
        static ISPC::ControlAE *pAE = new ISPC::ControlAE();
        ...
        if(ISPC::CAM_ERROR == camera->state)
        {
        ...
        if ( !frameExposureInfo || !frameGainInfo )
        {
            LOG_ERROR("failed to allocate the frame information "\
                "buffers\n");
            delete sensor;
            delete frameExposureInfo;
            delete frameGainInfo;
            delete frames;
            cleanHeap();
            delete camera;
            goto fail_retry;
