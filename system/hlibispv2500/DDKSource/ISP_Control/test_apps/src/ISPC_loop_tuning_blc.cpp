...
struct DemoConfiguration
{
    ...
    char *pszFelixSetupArgsFile[MAX_CAM_NUM];
    char *pszSensor[MAX_CAM_NUM];
    ...
    int loadParameters(int argc, char *argv[])
    {
		...
		for (Idx = 0; Idx < MAX_CAM_NUM; Idx++)
		{
	        ...
	        if (RET_FOUND != ret)
	        {
	            ...
	            {
	                fprintf(stderr, "Error: no sensor provided : %s\n", szItemName);
	            }
				continue;
	        }
			nCam++;
...
struct LoopCamera
{
    ...
    LoopCamera(DemoConfiguration &_config, IMG_UINT32 ContextNum): camera(0), config(_config)
    {
        ...
        else
        {
            ...
            //the below codes are for dual sensor raw data extration tempor method
            index = ContextNum;
            memset(tmp, '\0', sizeof(tmp));
            //printf("##########name = %s\n", _config.pszSensor[ContextNum] + (len - 3));
            sprintf(tmp, "%s", _config.pszSensor[ContextNum]);

            //printf("##########tmp = %s\n", tmp);
            len = strlen(_config.pszSensor[ContextNum]);
            if(0 == strncmp(tmp + len -3, "dvp", 3))
                index = 1;
           // printf("index = %d \n", index);
            camera = new ISPC::Camera(ContextNum, ISPC::Sensor::GetSensorId(_config.pszSensor[ContextNum]), _config.sensorMode[ContextNum], flipping, index);
#else
...
#endif //INFOTM_ISP && CONTROL_EXPOSURE_BY_KEY
    {
        ...
        printf("Shot RGB=%s YUV=%s DE=%s HDR=%s TIFF=%s\n",
        ...
        if ( shot.YUV.pxlFormat != PXL_NONE && config.bSaveYUV )
        {
            ...
            globalSetup.ui32EncHeight =globalSetup.ui32EncHeight *nCam;
            outputName.str("");
...
int run(int argc, char *argv[])
{
	...
	for (Idx = 0; Idx < nCam; Idx++)
	{
    	...
    	{
        	std::cout <<"ERROR parsing parameter file \'"<<config.pszFelixSetupArgsFile[Idx]<<"\'"<<std::endl;
        	return EXIT_FAILURE;
		}	
    }

    //LoopCamera loopCam(config);
    LoopCamera* pLoopCam[MAX_CAM_NUM];
	...
	for (Idx = 0; Idx < nCam; Idx++)
	{
		...
		{
			printf("Failed when creating camera\n");
			return EXIT_FAILURE;
		}

		pLoopCam[Idx]->camera->getPipeline()->getCIPipeline()->uiTotalPipeline = nCam;

		if(pLoopCam[Idx]->camera->loadParameters(parameters[Idx]) != IMG_SUCCESS)
		{
			printf("loading pipeline setup parameters[%s] from file: '%s' failed\n", Idx,
		...
		{
			printf("failed to access camera's connection!\n");
			return EXIT_FAILURE;
		}

		config.loadFormats(parameters[Idx]);
		//if for dual camera sensor, set first buffer to double size
		if (nCam > 1)
		{
		    int dual_w = 0, dual_h = 0;
            dual_w = pLoopCam[0]->camera->pipeline->getMCPipeline()->sIIF.ui16ImagerSize[0]*CI_CFA_WIDTH;
            dual_h = pLoopCam[0]->camera->pipeline->getMCPipeline()->sIIF.ui16ImagerSize[1]*CI_CFA_WIDTH;

            dual_h *= 2;

            printf("dual encoder dimension = %d, h= %d\n", dual_w, dual_h);
            pLoopCam[Idx]->camera->setEncoderDimensions(dual_w, dual_h);
		}
		if(pLoopCam[Idx]->camera->program() != IMG_SUCCESS)
		{
			printf("Failed while programming pipeline.\n");
			return EXIT_FAILURE;
		}
	} //End of for (Idx = V2500_MIN_CAM; Idx < V2500_MAX_CAM; Idx++)
    ...
    //ISPC::Sensor *sensor = pLoopCam[dx]->camera->getSensor();
    ...
    while(loopActive)   //Capture loop
    {
		...
		for(Idx = 0; Idx < nCam; Idx++)
		{
	        if ((ret = pLoopCam[Idx]->camera->enqueueShot()) != IMG_SUCCESS)
	        {
	        ...
	        //ISPC::Shot shot;
	        if ((ret = pLoopCam[Idx]->camera->acquireShot(pLoopCam[Idx]->shot)) != IMG_SUCCESS)
	        {
	        ...
	        if (frameCount % 200 == 0)
	        {
	            ...
	            printf("shot.YUV.phy:0x%x, shot.YUV.data:0x%x, stride:%d, height:%d\n", pLoopCam[Idx]->shot.YUV.phyAddr, pLoopCam[Idx]->shot.YUV.data, pLoopCam[Idx]->shot.YUV.stride, pLoopCam[Idx]->shot.YUV.height);
...
*/
            ...
            case AUTO_CAPTURE_KEY:
                {
					...
					for (Idx = 0; Idx < nCam; Idx++)
					{
	                    ulExposureTime = pLoopCam[Idx]->sensor->getExposure();
	                    ulExposureTime -= UP_DOWN_STEP;
	                    pLoopCam[Idx]->sensor->setExposure(ulExposureTime);
	                    iAutoCaptureMode = 1;
	                    iAutoCaptureStage = 0;
	                    lAutoCaptureCount = 0;
		...
		for (Idx = 0; Idx < nCam; Idx++)
		{
	        ...
	        //printf("bSaveResults=%d frameError=%d , ret = %d\n", bSaveResults, pLoopCam[Idx]->shot.bFrameError, ret);

	        if ((ret == IMG_SUCCESS) && bSaveResults && !pLoopCam[Idx]->shot.bFrameError)
	        {
			#if defined(INFOTM_ISP) && defined(CONTROL_EXPOSURE_BY_KEY)
			        	unsigned long ulExposureTime;

			            ulExposureTime = pLoopCam[Idx]->sensor->getExposure();
			            pLoopCam[Idx]->saveResults(pLoopCam[Idx]->shot, config, lAutoCaptureCount, ulExposureTime);
			            ++lAutoCaptureCount;
			#else
