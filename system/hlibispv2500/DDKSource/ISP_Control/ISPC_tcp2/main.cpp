...
@brief point of entry of ISPC_tcp
...
void ctrlcHandler(sig_atomic_t sig)
{
	if(listener)
	{
		if(listener->cleanUp() != IMG_SUCCESS)
		{
...
int main(int argc, char *argv[])
{
	...
	listener = new Listener(appName, sensor, sensorMode, nBuffers, ip, port, DGFrameFile, gasket, aBlanking, sensorFlip);

	if(listener->initCamera() == IMG_SUCCESS)
	{
		if(listener->startConnection() == IMG_SUCCESS)
		{
			listener->run();
		}
		else
