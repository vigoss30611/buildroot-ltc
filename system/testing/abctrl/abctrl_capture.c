...
int capture(int argc, char **argv)
{
	...
	ret = audio_get_master_volume(pcm_name, DEVICE_IN_ALL);
	if (ret < 0) {
	...
	if (ret != volume) {
		ret = audio_set_master_volume(pcm_name, volume, DEVICE_IN_ALL);
		if (ret < 0) {
	...
	length = get_frame_size(&real) * real.sample_size;
	...
	total_len = 0;
	needed = get_frame_size(&real) * real.samplingrate * time;
