  ...
  while (output_thread_run) {
    ...
    if (ret == DEC_PIC_RDY) {
      ...
      WriteOutput(out_file_name, out_file_name_tiled,
				  ...
				  dec_picture.bit_depth_luma, dec_picture.bit_depth_chroma, decPIcture,pic_stride);
...
#define EXPIRY_DATE (u32)0xFFFFFFFF
  ...
  START_SW_PERFORMANCE;
  {
    ...
    ret = HevcDecInit(
        &dec_inst, disable_output_reordering,
        TBGetDecIntraFreezeEnable(&tb_cfg),
        write_raster_out ? G2DEC_OUT_FRM_RASTER_SCAN : DEC_OUT_FRM_TILED_4X4);
  }
  END_SW_PERFORMANCE;
  if (ret != DEC_OK) {
  ...
  /*
  G2SetDecRegister(((struct DecContainer *) dec_inst)->hevc_regs,
  HWIF_DEC_LATENCY,
                 latency_comp);
  G2SetDecRegister(((struct DecContainer *) dec_inst)->hevc_regs,
  HWIF_DEC_CLK_GATE_E,
...
/* allocate memory for stream buffer. if unsuccessful -> exit */
    ...
    if (G2DWLMallocLinear(((struct HevcDecContainer *)dec_inst)->dwl, strm_len,
                        &stream_mem) != DWL_OK) {
...
#ifndef ADS_PERFORMANCE_SIMULATION
    if (G2DWLMallocLinear(((struct HevcDecContainer *)dec_inst)->dwl,
...
#ifndef ADS_PERFORMANCE_SIMULATION
    if (dec_inst)
      G2DWLFreeLinear(((struct HevcDecContainer *)dec_inst)->dwl, &stream_mem);
#else
...
void CropSemiplanarPicture(u8 * data, u8 *crop_data, const HevcCropParams *crop,
                 u32 picWidth, u32 picHeight,
                 u32 bitDepthLuma, u32 bitDepthChroma, u32 picStride)
{
    u16 *pixel_line;
    int i, j, k;
    u32 crop_size = crop->cropOutWidth * crop->cropOutHeight;

    if (bitDepthLuma == 8 && bitDepthChroma == 8)
    {
        /* semi planar -> cropped semi planar */
        /* luma */
        u32 j = crop->cropTopOffset * picStride + crop->cropLeftOffset;
        for (i = 0; i < crop->cropOutHeight; i++)
        {
            memcpy(crop_data + i * crop->cropOutWidth, data + j, crop->cropOutWidth);
            j += picStride;
        }
        /* chroma */
        j = picHeight * picStride + crop->cropTopOffset * picStride / 2 + crop->cropLeftOffset;
        for (i = 0; i < crop->cropOutHeight / 2; i++)
        {
            memcpy(crop_data + crop_size + i * crop->cropOutWidth, data + j, crop->cropOutWidth);
            j += picStride;
        }
    }
    else
    {
        ...
        u8 *line = data + crop->cropTopOffset * picStride;
        pixel_line = (u16 *)malloc(picWidth * 2);
        if (!pixel_line)
        {
        ...
        for (i = 0; i < crop->cropOutHeight; i++)
        {
            for (j = k = 0; j < picWidth; j += 4)
            {
                /* unpack  4 pixel of 10 bits to 16 bits */
                pixel_line[j] = ((u16)line[k] | (((u16)line[k+1] & 0x3) << 8)); // << (16 - bitDepthLuma);
                pixel_line[j + 1] = (((u16)line[k + 1] >> 2) | (((u16)line[k + 2] & 0x0f) << 6)); // << (16 - bitDepthLuma);
                pixel_line[j + 2] = (((u16)line[k + 2] >> 4) | (((u16)line[k + 3] & 0x3f) << 4)); // << (16 - bitDepthLuma);
                pixel_line[j + 3] = (((u16)line[k + 3] >> 6) | (((u16)line[k + 4] & 0xff) << 2)); // << (16 - bitDepthLuma);
                k += 5;
            }
            memcpy((u16 *)crop_data + i * crop->cropOutWidth, pixel_line + crop->cropLeftOffset, crop->cropOutWidth * sizeof(u16));
            line += picStride;
        }

        /* chroma */
        line = data + picHeight * picStride + crop->cropTopOffset * picStride / 2;
        for (i = 0; i < crop->cropOutHeight / 2; i++)
        {
            for (j = k = 0; j < picWidth; j += 4)
            {
                pixel_line[j] = ((u16)line[k] | (((u16)line[k+1] & 0x3) << 8)); // << (16 - bitDepthChroma);
                pixel_line[j + 1] = (((u16)line[k + 1] >> 2) | (((u16)line[k + 2] & 0x0f) << 6)); // << (16 - bitDepthChroma);
                pixel_line[j + 2] = (((u16)line[k + 2] >> 4) | (((u16)line[k + 3] & 0x3f) << 4)); // << (16 - bitDepthChroma);
                pixel_line[j + 3] = (((u16)line[k + 3] >> 6) | (((u16)line[k + 4] & 0xff) << 2)); // << (16 - bitDepthChroma);
                k += 5;
            }
              memcpy((u16 *)crop_data + crop_size + i * crop->cropOutWidth, pixel_line + crop->cropLeftOffset, crop->cropOutWidth * sizeof(u16));
              line += picStride;
        }
        free(pixel_line);
    }
...
void WriteOutput(const char *filename, const char *filename_tiled, u8 *data,
  ...
  if (tiled_mode == DEC_OUT_FRM_TILED_4X4 && convert_tiled_output) {
    ...
    if (raster_scan_size != (pic_width * eff_height * 3 / 2 * pixelWidth / 8)) {
      raster_scan_size = (pic_width * eff_height * 3 / 2 * pixelWidth / 8);
      if (raster_scan != NULL) free(raster_scan);
    ...
    TBTiledToRaster(1, convert_to_frame_dpb ? dpb_mode : DEC_DPB_FRAME, data,
                    raster_scan, pic_width, eff_height, bit_depth);
    if (!mono_chrome)
  ...
  if (crop_display || planar_output) {
    ...
    if (new_size > cropped_pic_size) {
      ...
      if (cropped_pic == NULL) {
        ...
        data = cropped_pic;
        picWidth = crop->cropOutWidth;
        picHeight= crop->cropOutHeight;
  ...
  if (planarOutput) 
  {
      ...
      if (fOutput16 == NULL)
      {
          fOutput16 = fopen(outFileNamePlanar, "wb");
          if(fOutput16 == NULL)
          {
              DEBUG_PRINT(("UNABLE TO OPEN OUTPUT FILE\n"));
              return;
      ...
      CropSemiplanarPicture(data, crop_data, crop, picWidth, picHeight, bitDepthLuma, bitDepthChroma, picStride);
      
      if (bitDepthLuma == 8 && bitDepthChroma == 8)
      {
    			Semiplanar2Planar8(crop_data, (u8 *)planar_data, crop->cropOutWidth, crop->cropOutHeight);

    			fwrite(planar_data, 1, crop_size * 3 / 2, fOutput16);
      }
      else
      {
    			Semiplanar2Planar((u16 *)crop_data, planar_data, crop->cropOutWidth, crop->cropOutHeight);

    			fwrite(planar_data, 2, crop_size * 3 / 2, fOutput16);
      }
    ...
    if (fOutput16 == NULL)
    {
        fOutput16 = fopen(outFileName16, "wb");
        if(fOutput16 == NULL)
        {
            DEBUG_PRINT(("UNABLE TO OPEN OUTPUT FILE\n"));
            return;
