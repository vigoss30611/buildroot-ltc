...
void SetupDefaultParams(struct TestParams* params) {
  ...
  params->format = G2DEC_OUT_FRM_RASTER_SCAN;
...
int ParseParams(int argc, char* argv[], struct TestParams* params) {
...
#if 0
	params->in_file_name = malloc(32);	
	sprintf(params->in_file_name,"/nfs/save0.h265");
	params->file_format = FILEFORMAT_BYTESTREAM;
	params->hw_format = G2DEC_OUT_FRM_RASTER_SCAN;
	return 0;
#endif
  /* read command line arguments */
  while ((c = getopt_long(argc, argv, 
#ifdef DOWN_SCALER
                          "CE:Fi:MmN:O:PpSTtrXZQRsd:f",
#else
                          "CE:Fi:MmN:O:PpSTtrXZQRs:f",
#endif
                          long_options,
                          &option_index)) != -1) {
    switch (c) {
...
#else
        fprintf(stderr,
                  "ERROR: -P is not supported when NO_TB_PP is defined.\n");
        return 1;
#endif
        break;
      case 'Z':
...
#ifdef DOWN_SCALER
  if (flag_d && !(params->hw_format == G2DEC_OUT_FRM_RASTER_SCAN)) {
    fprintf(stderr, "Error: option -d must be used with -Ers -S.\n");
    return 1;
  }
  ...
  printf("ParseParams input file name :%s . optind:%d\n", params->in_file_name, optind);
...
int ResolveOverlap(struct TestParams* params) {
  if (params->format == DEC_OUT_FRM_TILED_4X4 &&
    ...
    fprintf(stderr,
            ...
            "output is tiled 4x4. (i.e. '-Ers' option ignored)\n");
