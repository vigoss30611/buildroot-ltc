...
int main(int argc, char *argv[])
{
	...
	vic = strtoul(argv[2], NULL, 10);

	if (argc >= 4)
		debug = strtoul(argv[3], NULL, 10);

	dlog_info("overlay=%d, vic=%d\n", overlay, vic);
	...
	dlog_dbg("FBIO_GET_PERIPHERAL_VIC\n");
	ret = ioctl(fd, FBIO_GET_PERIPHERAL_VIC, &cur_vic);
	if (ret) {
		dlog_err("ioctl FBIO_GET_PERIPHERAL_VIC failed: %s\n", strerror(errno));
		return ret;
	}
	dlog_dbg("current vic is %d\n", cur_vic);
	...
	dlog_dbg("FBIO_SET_PERIPHERAL_VIC\n");
	ret = ioctl(fd, FBIO_SET_PERIPHERAL_VIC, vic);
	if (ret) {
		dlog_err("ioctl FBIO_SET_PERIPHERAL_VIC failed: %s\n", strerror(errno));
		return ret;
	}
...
usage:
	printf("Usage:\n"
			"\tset_vic <overlay> <vic> [debug]\n"
			"\t@overlay: specify overlay to select the desired path\n"
			"\t@vic: Video Identification Code, refers to vic.c or CEA-861-E protocol\n"
