...
static inline BOOL mouse_in_client(LPARAM lParam, PCONTROL pctrl)
{
	...
	//printf("Button.c: mouse_in_client---pctrl:%d x:%d y:%d\n", pctrl, x, y);
	
    ScreenToClient (GetParent ((HWND)pctrl), &x, &y);
	//printf("Button.c: mouse_in_client---afterScreenToClient---x:%d, y:%d\n", x, y);
	//printf("Button.c: mouse_in_client---checkarea---left:%d, right:%d, top:%d, bottom:%d\n", ((PRECT)&(pctrl->cl))->left, ((PRECT)&(pctrl->cl))->right, ((PRECT)&(pctrl->cl))->top, ((PRECT)&(pctrl->cl))->bottom);
	
    return (PtInRect ((PRECT) &(pctrl->cl), x, y));
}

static int move_check;
...
static int ButtonCtrlProc (HWND hWnd, int uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    DWORD       dwStyle;
    PBUTTONDATA pData;
	//printf("Button.c: ButtonCtrlProc move_check value check is %d\n", move_check);
    ...
    switch(uMsg)
    {
     ...
     //       MG_CHECK_RET(BUTTON_IS_PUSHBTN(pctrl), -1);
        ...
        case MSG_LBUTTONDOWN:
			//printf("Button.c: MSG_LBUTOONDOWN hWnd: %d\n", hWnd);
            if (GetCapture () == hWnd)
			{
			//	printf("Button.c: MSG_LBUTOONDOWN --if--GetCapture() hWnd: %d\n", hWnd);
				
				break;
			}
            SetCapture (hWnd);
            move_check = 1;
		//	printf("Button.c: MSG_LBUTTONDOWN move_check is %d\n", move_check);
            BUTTON_STATUS(pctrl) |= BST_FOCUS;
            change_status_from_pose(pctrl, BST_PUSHED, TRUE, TRUE);
        break;
        ...
        case MSG_LBUTTONUP:
        {
			//printf("Button.c: MSG_LBUTOONUP hWnd: %d\n", hWnd);
         //   if (GetCapture() == hWnd)
         //   {
            //	printf("Button.c: MSG_LBUTOONUP--if--GetCapture hWnd: %d\n", hWnd);
                ReleaseCapture ();

			//	printf("Button.c: MSG_LBUTTONUP---mouse_in_client is %d, move_check is %d\n", mouse_in_client(lParam, pctrl), move_check);
                if (mouse_in_client(lParam, pctrl) && move_check )
				{
			//		printf("Button.c: MSG_LBUTOONUP--if--GetCapture--if--mouse_in_client hWnd: %d\n", hWnd);
                    change_status_from_pose(pctrl, BST_HILITE, TRUE, TRUE);
				}
                else
                {
              //  	printf("Button.c: MSG_LBUTOONUP--if--GetCapture--else--mouse_in_client hWnd: %d\n", hWnd);
        ...
        {
			//printf("Button.c: MSG_MOUSEMOVEIN hWnd: %d\n", hWnd);
            /*leave a button, nomralize it*/
            if (!wParam)
            {
			//	printf("Button.c: MSG_MOUSEMOVEIN--leave button hWnd: %d\n", hWnd);
        ...
        case MSG_MOUSEMOVE:
        {
			//printf("Button.c: MSG_MOUSEMOVE hWnd: %d\n", hWnd);
			ReleaseCapture ();
			move_check = 0;
            /*left button is not downed*/
            if (!(wParam & KS_LEFTBUTTON))
            {
            	//printf("Button.c: MSG_MOUSEMOVE--LeftButtonIsNotDowned hWnd: %d\n", hWnd);
                /*mouse not captured by keydowning on other button*/
                if (GetCapture()==HWND_NULL)
                {
                	//printf("Button.c: MSG_MOUSEMOVE--LeftButtonIsNotDowned--if--GetCapture hWnd: %d\n", hWnd);
          ...
          //  if (GetCapture() == hWnd)
          //  {
          //  	printf("Button.c: MSG_MOUSEMOVE--IfCaptureMouse hWnd: %d\n", hWnd);
                /*normal -- pushed, while lbutton is being downed*/
                if (mouse_in_client(lParam, pctrl))
                {
                //	printf("Button.c: MSG_MOUSEMOVE--CaptureMouse--If--MouseInClient hWnd: %d\n", hWnd);
                    change_status_from_pose(pctrl, BST_PUSHED, FALSE, FALSE);
                }
				else
				{
				//	printf("Button.c: MSG_MOUSEMOVE--CaptureMouse--Else--MouseInClient hWnd: %d\n", hWnd);
