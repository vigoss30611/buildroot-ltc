 ...
 *
 */
int GAL_GetVideoSize(int *w, int *h)
{
    if(current_video->GetFBSize != NULL) {
        return current_video->GetFBSize(current_video, w, h);
    }
    return -1;
}
 ...
 *
 */
int GAL_RefreshSurface(void)
{
    if(current_video->RefreshSurface != NULL) {
        return current_video->RefreshSurface(current_video);
    }
    return -1;
}
