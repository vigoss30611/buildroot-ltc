...
static char *hw_mem = NULL;
static char *virtual_mem = NULL;

pthread_mutex_t refreshMutex = PTHREAD_MUTEX_INITIALIZER;


static void print_finfo(struct fb_fix_screeninfo *finfo);

/* Initialization/Query functions */
static int FB_GetFBSize(_THIS, int *w, int *h);
static int FB_RotateSurface(_THIS);
...
static GAL_VideoDevice *FB_CreateDevice(int devindex)
{
    ...
    this->GetFBSize = FB_GetFBSize;
    if(DOUBLE_BUFFER) 
        this->RefreshSurface = FB_RotateSurface;
...
static void FB_RgbToGray(unsigned int *s, char *d)
{
	int i, j, k = 0;
	char bit[8] = {0};

	for (i = fb_yres - 1; i >= 0; i--) {
		for (j = fb_xres - 1; j >= 0; j--) {
			int color = s[fb_xres * i + j];
			...
			if (k == 8) {
				k = 0;
				//*d = bit[0] | bit[1] << 1 | bit[2] << 2 | bit[3] << 3 | bit[4] << 4 | bit[5] << 5 | bit[6] << 6 | bit[7] << 7;
				*d = (bit[0] << 7) | (bit[1] << 6) | (bit[2] << 5) | (bit[3] << 4) | (bit[4] << 3) | (bit[5] << 2) | (bit[6] << 1) | bit[7];
				d++;
			}
#endif
		}
...
static int FB_RotateSurface(_THIS)
{
    ...
    char file[128] = {0};
	ROTATE_SCREEN = 1;
    //struct timeval start,rotate, end;
    //gettimeofday(&start, 0);
    static char *fb_buf = NULL;
    static char *fb_buf2 = NULL;
    int screensize = fb_xres * fb_yres * 4;
    static int flip = 0;
    if(fb_buf == NULL) {
        fb_buf = (char *) malloc(screensize);
        if(fb_buf == NULL) {
            printf("ERROR malloc fb_buf\n");
            return -1;
        }
    }
    ...
    if(fb_buf2 == NULL) {
        fb_buf2 = (char *) malloc(screensize);
        if(fb_buf2 == NULL) {
            printf("ERROR malloc fb_buf\n");
            return -1;
        }
    }
    if(virtual_mem != NULL && fb_xres > 0) {
        pthread_mutex_lock(&refreshMutex);
        int i, j;
        int width = fb_xres;
        int height = fb_yres;
        if(ROTATE_SCREEN) {
        //printf("FB_RotateSurface, w: %d, h: %d\n", width, height);
#if 0
        for( i = 0; i < height; i++)
        {
            unsigned int *s = (unsigned int *)flip_address[1] + i * width;    
            unsigned int *d = (unsigned int *)fb_buf + (height - 1 - i);
            for(j = 0; j < width; j+=4)
            {
                d[0] = s[0];  
                d[height] = s[1];
                d[height << 1] = s[2];
                d[height << 1 + height ] = s[3];
                d += (height << 2);
                s += 4;
            }
        }
#else
        //memcpy(fb_buf2, flip_address[1], screensize);
        unsigned int *s = (unsigned int *)virtual_mem;
        unsigned int *d = (unsigned int *)fb_buf;
        for(i = 0; i < height; i ++) {
            for(j = 0; j < width; j ++) {
                //d[(width - 1 - j) * height + i] = s[i * width + j]; //ccw
                d[(height - i -1) * width + j] = s[i * width + j]; //ccw
                //d[j * height + height - 1 - i] = s[i * width + j];//cw
            }
        }
#endif
        }
        //gettimeofday(&rotate, 0);
        flip = !flip;
		if (GRAY_SCREEN) {
			FB_RgbToGray(ROTATE_SCREEN ? fb_buf : virtual_mem, fb_buf2);
			memcpy(flip_address[flip], fb_buf2, screensize);
		} else
			memcpy(flip_address[flip], ROTATE_SCREEN ? fb_buf : virtual_mem, screensize);
		
        cache_vinfo.yoffset = flip ? cache_vinfo.yres : 0;
        int ret = ioctl(console_fd, FBIOPAN_DISPLAY, &cache_vinfo);
        //printf("iopan display, console_fd: %d, yoffset: %d, ret: %d\n", console_fd, cache_vinfo.yoffset, ret);
 
        //gettimeofday(&end, 0);
            //printf("rotate time: %d, all: %d\n", ((rotate.tv_sec - start.tv_sec) * 1000 + (rotate.tv_usec - start.tv_usec)/1000), ((end.tv_sec - start.tv_sec) * 1000 + (end.tv_usec - start.tv_usec)/1000));
            //printf("s: 0x%x, d: 0x%x\n", s[width * height - 1], d[width * height - 1]);
...
/*
    if (ioctl(console_fd, FBIOBLANK, FB_BLANK_POWERDOWN) == -1) //close fb
        return -1;                               
    ...
    vinfo.nonstd = 0;
    vinfo.grayscale = 0;
    vinfo.nonstd |= 0 << 8;    //xpos
    vinfo.nonstd |= 0 << 20;   //ypos
    vinfo.nonstd |= 0x0;        //0:rgb 1:yuv
    vinfo.grayscale |= 720 << 8;    //xres
    vinfo.grayscale |= 480 << 20;   //yres
    ...
    if (ioctl(console_fd, FBIOBLANK, FB_BLANK_UNBLANK) == -1)   //open fb
        return -1;   
*/
    /* Get the type of video hardware */
    printf("get screen info\n");
    ...
    switch (finfo.type) {
        case FB_TYPE_PACKED_PIXELS:
            ...
            break;
        default:
            GAL_SetError("NEWGAL>FBCON: Unsupported console hardware:%d instead of %d. fegnwu continue\n", finfo.type, FB_TYPE_PACKED_PIXELS);
            //FB_VideoQuit(this);
            //return(-1);
    }
    ...
    switch (finfo.visual) {
        ...
        case FB_VISUAL_DIRECTCOLOR:
            break;
        default:
            GAL_SetError("NEWGAL>FBCON: Unsupported console hardware:%d instead of %d. fengwu continue.\n", finfo.visual, FB_VISUAL_TRUECOLOR);
            //FB_VideoQuit(this);
            //return(-1);
    }
    ...
    printf("smemstart: 0x%x, mapped_offset: %d, mapped_memlen: %d, pagesize: %d\n", (long)finfo.smem_start, mapped_offset, mapped_memlen, getpagesize());
...
#  endif
    ...
    if(DOUBLE_BUFFER) {
    hw_mem = mapped_mem;
    mapped_mem = (char *) malloc(mapped_memlen);
    virtual_mem = mapped_mem;
    }
    printf("mapped_mem: 0x%x, hw_mem: 0x%x, mapped_memlen: %d\n", mapped_mem, hw_mem, mapped_memlen);
...
//#ifdef __TARGET_STB810__
    ...
    printf("current: 0x%x, mappded_mem: 0x%x, surfaces_mem: 0x%x, surfaces_len: %d\n", current->pixels, mapped_mem, surfaces_mem, surfaces_len);
    ...
    if(DOUBLE_BUFFER) {
        ...
        flip_address[0] = (char *)hw_mem;
        flip_address[1] = (char *)hw_mem + current->h * current->pitch;
        printf("flip_addr, 0: 0x%x, 1: 0x%x\n", flip_address[0], flip_address[1]);
        current->w = width;
        current->h = height;
