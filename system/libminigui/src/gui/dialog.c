        ...
        const char* window_name, const char* layer_name)
{
    ...
    for (i = 0; i < pDlgTemplate->controlnr; i++) {
        ...
        hCtrl = CreateWindowEx2 (pCtrlData->class_name,
                              ...
                              pCtrlData->we_attrs,
                              pCtrlData->dwAddData);
         printf("****CreateMainWindowIndirectParamEx %d, hCtrl:%x. cls:%s, caption:%s\n", 
           i, hCtrl,  pCtrlData->class_name==NULL?"NOCLS":pCtrlData->class_name, 
            pCtrlData->caption==NULL?"NOCAP":pCtrlData->caption);                   
