...
def_paint_desktop (void* context, HDC dc_desktop, const RECT* inv_rc)
{
    ...
    //SetBrushColor (dc_desktop, GetWindowElementPixel (HWND_DESKTOP, WE_BGC_DESKTOP));
    fprintf(stderr, "def_paint_desktop, GetWindowElementPixel:%x, clear alpha\n", GetWindowElementPixel (HWND_DESKTOP, WE_BGC_DESKTOP));
    //SetBrushColor (dc_desktop, RGBA2Pixel (HDC_SCREEN_SYS, 0x0, 0x00, 0x00, 0x0));
    SetBrushColor (dc_desktop, RGB2Pixel (HDC_SCREEN_SYS, 0x01, 0x01, 0x01));
...
static void def_mouse_handler(void* context, int message, 
		                            WPARAM wParam, LPARAM lParam)
{
    printf("def_mouse_handler, msg:%d\n", message);
#ifdef _MGHAVE_MENU
    int x, y;
    switch(message)
    {
