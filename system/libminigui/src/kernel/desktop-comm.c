...
int DesktopWinProc (HWND hWnd, int message, WPARAM wParam, LPARAM lParam)
{
    ...
    if(message!=324 && message != 362)
        {
        x = LOSWORD (lParam);
        y = HISWORD (lParam);
        //printf("DesktopWinProc %s(%d), xy:%d-%d\n", Message2Str(message), message, x, y);
        }
...
//added content about scrolling&sliding by Zhen
        case INFOTMIC_MSG_MOUSE_SCROLLING:
        case INFOTMIC_MSG_MOUSE_SLIDING:    
        {
            flags = (int)wParam;
            x = LOSWORD (lParam);
            y = HISWORD (lParam);
