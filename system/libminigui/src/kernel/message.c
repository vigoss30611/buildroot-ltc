...
{
    //if(msg->message != 325)printf("\tkernel_QueueMessage hwnd %x, msg:%d, wp:%x, lp:%x, \n", msg->hwnd, msg->message, msg->wParam, msg->lParam);
    ...
    if (msg->message == MSG_MOUSEMOVE 
	...
	else if(msg->message == MSG_KEYDOWN 
		...
		while(readpos != msg_que->writepos) {
			a_msg = msg_que->msg + readpos;
			if(a_msg->message == MSG_KEYDOWN
					|| a_msg->message == MSG_KEYUP
					|| a_msg->message == MSG_KEYLONGPRESS
					|| a_msg->message == MSG_KEYALWAYSPRESS) {
				msgnum++;
			}
			readpos ++;
			readpos %= msg_que->len;
		...
		if(msgnum >= 2)		//the number of the key-message in the msg_que
		{
			printf("[LIBGUI_MESSAGE]MSG:key events in the msg_que is out of range, msgnum=%d\n", msgnum);
			goto ret;
		}
	}
...
{
    //printf("GetWndProc , return MainWindowProc\n");
...
int GUIAPI SendMessage (HWND hWnd, int iMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    int ret = (*WndProc)(hWnd, iMsg, wParam, lParam);
    if(iMsg == MSG_PAINT)
        GAL_RefreshSurface();
...
int GUIAPI DispatchMessage (PMSG pMsg)
{
    ...
    if(MSG_CARETBLINK!= pMsg->message && pMsg->message != MSG_TIMER)
    {fprintf (stderr, "msg.c 2: Message, %s(%d): hWnd: %x, wP: %x, lP: %lx. %s\n",
    ...
    if(0){
    if(MSG_DT_RBUTTONDOWN == pMsg->message || pMsg->message == 228 || pMsg->message ==4)
    {
        static int cnt = 0;
        cnt++;
        switch( cnt % 4)
        {
        case 0:
            pMsg->message = MSG_DT_LBUTTONDOWN;
            break;
        case 1:
            pMsg->message = MSG_NCLBUTTONDOWN;
            break;
        case 2:
            pMsg->message = MSG_LBUTTONDOWN;
            break;
        case 3:
            pMsg->message = MSG_LBUTTONDOWN;
            break;
        default:
            pMsg->message = MSG_DT_RBUTTONDOWN;
            break;
        }
        printf("fengwu change msg:%d->%d. cnt:%d(%d)\n", 
            MSG_DT_RBUTTONDOWN, pMsg->message, cnt, cnt%4);
    }
    }
#endif
...
#ifdef _MGHAVE_TRACE_MSG
        fprintf (stderr, "msg.c: Message have been thrown away: %s\n",
            Message2Str (pMsg->message));
#endif
        return -1;
    }
...
#ifdef _MGHAVE_TRACE_MSG
    if(MSG_CARETBLINK!= pMsg->message && pMsg->message!=324 
        && MSG_CSIZECHANGED != pMsg->message 
        && MSG_PAINT != pMsg->message 
        && MSG_ERASEBKGND != pMsg->message)
    {
    fprintf (stderr, "msg.c 3: Message, %s(%d) done, return value: %x\n",
        Message2Str (pMsg->message), pMsg->message, iRet);
    }
#endif
        ...
        while (TRUE) {
            ...
            if (pMsgQueue->dwState & QS_SYNCMSG) {
                if (pMsgQueue->pFirstSyncMsg) {
                    ...
                    printf("SendSyncMessage->DispatchMessage\n");
                    DispatchMessage(&msg);
                }
                else {
                    UNLOCK_MSGQ (pMsgQueue);
