...
void* DesktopMain (void* data)
{
    ...
    fprintf (stderr, "DesktopMain don't MSG_ERASEDESKTOP\n");
    ...
    while (GetMessage(&Msg, HWND_DESKTOP)) {
        ...
        if(Msg.message!=MSG_CARETBLINK && Msg.message!= 324)fprintf (stderr, "Desk Message, %s(%d): hWnd: %#x, wP: %#x, lP: %#lx. %s\n",
...
#ifdef _MGHAVE_TRACE_MSG
        if(Msg.message!=MSG_CARETBLINK && Msg.message!= 324)
        fprintf (stderr, "Desk Message, %s(%d) done, return value: %#x\n",
            Message2Str (Msg.message),Msg.message, iRet);
#endif
    }
