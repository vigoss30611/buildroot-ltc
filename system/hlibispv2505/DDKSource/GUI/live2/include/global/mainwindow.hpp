...
private:
	...
	ModuleLSH *_pModuleLSH; // ModuleLSH widget
    ...
    QAction *_pPDPAction;
	QAction *_pENCAction;
	QAction *_pDISPAction;
	QAction *_pDEAction;
	QAction *_pHDRAction;
	QAction *_pRAW2DAction;
	...
	HTMLViewer *_pISPDoc; // ISP Documentation widget

    bool _connected; // Indicates if VL is connected to ISPC_tcp
...
signals:
	...
	void ISPC_connected(); // Called after successful connection
	void ISPC_disconnected(); // Called after diconnection
...
public slots:
    ...
    void enablePDP(); // Enables PDP output to be generated in ISPC_tcp
	...
	void HWInfoReceived(CI_HWINFO hwInfo); // Called from ImageHandler (through ProxyHandler) to indicate HW info received and update HW version in all modules
	void HWVersionChanged(int hwv); // Called when _pHWVersionOption current index changed

	void aboutVisionLive(); // Displays information about VisionLive
	void aboutQt(); // Displays information about Qt version

	void showISPDoc(); // Displays ISP documentation (HTML)
	void ISPDocLoadingFailed(); // Called when loading docummentation fails to emit logError()

    void openTestTool(); // Openns test tool
    void testToolClosed(); // After closing Test sets _pTest to null
    void saveModulesParams();
    void FB_received(const ISPC::ParameterList list);
};
