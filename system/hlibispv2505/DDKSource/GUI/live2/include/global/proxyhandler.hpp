...
signals:
	...
	void imageReceived(QImage *pImage, int type, int format, int mosaic, int width, int height, bool awbDebugMode);
    ...
    void LSH_added(QString filename);
    void LSH_removed(QString filename);
    void LSH_applyed(QString filename);
    void LSH_received(QString filename);
...
public slots:
	...
	void saveRec_TF(const QString &recName, const QString &fileName, bool saveAsFLX);
	...
	void LSH_add(std::string filename, int temp, unsigned int bitsPerDiff, double wbScale);
    void LSH_remove(std::string filename);
    void LSH_apply(std::string filename);
    void LSH_set(bool enable);
