...
public:
	...
	void setConnected(bool connected); // Sets _connected indicator
	bool isConnected(); // Gets _connected indicator
...
private:
	...
	TestCommandResult executeObjExist(const QString &obj); // Checks if obj object exists
	TestCommandResult executeFileExist(const QString &path); // Checks if file exists
	...
	QMutex _connectedLock; // Synchronizez usage of _connected
...
signals:
	...
	void saveRec_TF(const QString &recName, const QString &fileName, bool saveAsFLX); // Requests ImageHandler through Test and ProxyHandler to call saveRec_TF() 
...
public slots:
    void FB_received(const ISPC::ParameterList &intList, const ISPC::ParameterList extList);
