...
IMG_RESULT VisionLive::Save::open(SaveType _type, ePxlFormat fmt, MOSAICType mos, int width, int height, const std::string &_filename)
{
    ...
    if (!file)
    {
        ...
        case Bayer_TIFF:
            ret = openTiff(fmt, mos, width, height);
            break;
        ...
        case Display:
            ret = openDisplayFile(fmt, width, height);
            break;
...
int VisionLive::ImageHandler::getImage(ParamSocket *pSocket)
{
	...
	if(pSocket->read(&cmd, 11*sizeof(IMG_UINT32), nRead, N_TRIES) != IMG_SUCCESS)
	{
	...
	awbDebugMode = (bool)ntohl(cmd[10]);
    ...
    else if ((ePxlFormat)fmt == PXL_ISP_444IL3YCrCb8 || (ePxlFormat)fmt == PXL_ISP_444IL3YCbCr8 || (ePxlFormat)fmt == PXL_ISP_444IL3YCrCb10 || (ePxlFormat)fmt == PXL_ISP_444IL3YCbCr10)
    {
	...
	if(_ImageData.valid)
	{
		...
		if (awbDebugMode)
		{
			emit notifyStatus(QString(tr("Generating AWB debug image... ")));
			pImage = generateAWBDebugImage(_ImageData);
		}
		else
		{
		...
		if (currConf.generateLineView)
		{
			...
			if (reqImgType == DE)
			{
				lineViewData = VisionLive::Algorithms::generateLineView(_ImageData.data, _ImageData.mosaic, currConf.marker.toPoint(), 3, 0.25);
			}
			else if (reqImgType == RAW2D)
			{
				lineViewData = VisionLive::Algorithms::generateLineView(_ImageData.data, _ImageData.mosaic, currConf.marker.toPoint(), 3, 0.25);
			}
			else
			{
				lineViewData = VisionLive::Algorithms::generateLineView(_ImageData.data, _ImageData.mosaic, currConf.marker.toPoint(), 3);
			}
		...
		emit imageReceived(pImage, (int)reqImgType, (int)fmt, (int)mosaic, (int)width, (int)height, awbDebugMode);
	}
...
int VisionLive::ImageHandler::record(ParamSocket *socket)
{
	...
	// Receive imageInfo from FPGA
	if(socket->read(&cmd, 8*sizeof(IMG_UINT32), nRead, N_TRIES) != IMG_SUCCESS)
	{
		emit logError("Failed to write to socket!", "ImageHandler::record()");
		return IMG_ERROR_FATAL;
	}
	...
	if(!bYUV)
	{
		sYUVType.ui8PackedElements = 1;
		sYUVType.ui8HSubsampling = 1;
		sYUVType.ui8VSubsampling = 1;
		sYUVType.ui8PackedStride = 1;
	}
...
VisionLive::ImageData VisionLive::ImageHandler::generateImageData(IMG_UINT8 *rawData, int width, int height, int stride, pxlFormat fmt, MOSAICType mosaic)
{
	...
	case YUV_422_PL12_8:
        imageOut.info.ui8BitDepth = 8;
        imageOut.info.stride = width;
        bYUV = true;
        break;
    case PXL_ISP_444IL3YCrCb8:
    case PXL_ISP_444IL3YCbCr8:
        imageOut.info.eColourModel = SimImage_RGB24;
        imageOut.info.ui8BitDepth = 8;
        imageOut.info.stride = 3 * width;
        bYUV = false;
        break;
    case PXL_ISP_444IL3YCrCb10:
    case PXL_ISP_444IL3YCbCr10:
        imageOut.info.eColourModel = SimImage_RGB32;
        imageOut.info.ui8BitDepth = 10;
        imageOut.info.stride = 4 * width;
        bYUV = false;
	...
	case YUV_422_PL12_10:
        ...
        break;
    default:
		emit logError("Received an unsupported format - " + QString::number(fmt) + " - " + QString(FormatString(fmt)) + "!", "ImageHandler::generateImageData()");
		return ret;
    }
    ...
    if(imageOut.info.ui8BitDepth > 0)
    {
        ...
        {
            IMG_SIZE outsize;
            IMG_UINT16 outSizes[2] = { (IMG_UINT16) width, (IMG_UINT16) height };
            void *converted = 0;

            SimImageOut_create(&imageOut);

			if(fmt == BAYER_TIFF_10 || fmt == BAYER_TIFF_12)
			{
				convertToPlanarBayerTiff(outSizes, rawData, stride, imageOut.info.ui8BitDepth, &converted, &outsize);
			}
			else
			{
			...
			if (fmt == BAYER_RGGB_8)
			{
				for (int j = 0; j < ret.data.rows; j++)
				{
					for (int i = 0; i < ret.data.cols; i++)
					{
						tmp = ret.data.at<cv::Vec4f>(j, i).val[0];
						ret.data.at<cv::Vec4f>(j, i).val[0] = ret.data.at<cv::Vec4f>(j, i).val[3];
						ret.data.at<cv::Vec4f>(j, i).val[1] = ret.data.at<cv::Vec4f>(j, i).val[1];
						ret.data.at<cv::Vec4f>(j, i).val[2] = ret.data.at<cv::Vec4f>(j, i).val[2];
						ret.data.at<cv::Vec4f>(j, i).val[3] = tmp;
					}
				}
			}
        }
        else if(bYUV)
        {
			...
			{
                if (fmt == YUV_420_PL12_8 || fmt == YUV_422_PL12_8)
                    VisionLive::Algorithms::convert_YUV_420_422_8bit_to_YUV444_CV_8UC3(rawData, yuv444, width, height, false, fmt);
                else
                    VisionLive::Algorithms::convert_YUV_420_422_8bit_to_YUV444_CV_8UC3(rawData, yuv444, width, height, true, fmt);
			}
            else if (imageOut.info.ui8BitDepth == 10)
            {
                if (fmt == YUV_420_PL12_10 || fmt == YUV_422_PL12_10)
                    VisionLive::Algorithms::convert_YUV_420_422_10bit_to_YUV444_CV_8UC3(rawData, yuv444, width, height, false, fmt);
                else
                    VisionLive::Algorithms::convert_YUV_420_422_10bit_to_YUV444_CV_8UC3(rawData, yuv444, width, height, true, fmt);
			}
            else
                emit logError(QStringLiteral("Unsupported format found"),
                    QStringLiteral("VisionLive::ImageHandler::generateImageData()"));

            if (yuv444.empty())
            {
                emit logError(QStringLiteral("YUV conversion failed"),
                    QStringLiteral("VisionLive::ImageHandler::generateImageData()"));
            }
        ...
        else // RGB
        {
            ...
            if (imageOut.info.ui8BitDepth >= 8 && imageOut.info.ui8BitDepth <= 10)
            {
                if (fmt == RGB_888_24 || fmt == RGB_888_32 || fmt == RGB_101010_32)
                    VisionLive::Algorithms::convert_RGB_to_CV_8UC3(rawData, rgb888, width, height, false, fmt);
                else
                    VisionLive::Algorithms::convert_RGB_to_CV_8UC3(rawData, rgb888, width, height, true, fmt);
            }
            else
                emit logError(QStringLiteral("Unsupported format found"),
                QStringLiteral("VisionLive::ImageHandler::generateImageData()"));
            ...
            if (rgb888.empty())
            {
                emit logError(QStringLiteral("RGB conversion failed"),
                    QStringLiteral("VisionLive::ImageHandler::generateImageData()"));
            }
            ...
            // If RGB swap
            unsigned char tmp;
            if (fmt >= RGB_888_24 && fmt <= RGB_101010_32)
            {
                for (int j = 0; j < ret.data.rows; j++)
                {
                    for (int i = 0; i < ret.data.cols; i++)
                    {
                        tmp = ret.data.at<cv::Vec3b>(j, i).val[0];
                        ret.data.at<cv::Vec3b>(j, i).val[0] = ret.data.at<cv::Vec3b>(j, i).val[2];
                        ret.data.at<cv::Vec3b>(j, i).val[1] = ret.data.at<cv::Vec3b>(j, i).val[1];
                        ret.data.at<cv::Vec3b>(j, i).val[2] = tmp;
                    }
                }
            }
            else if (fmt == PXL_ISP_444IL3YCbCr8 || fmt == PXL_ISP_444IL3YCbCr10)
            {
                for (int j = 0; j < ret.data.rows; j++)
                {
                    for (int i = 0; i < ret.data.cols; i++)
                    {
                        tmp = ret.data.at<cv::Vec3b>(j, i).val[1];
                        ret.data.at<cv::Vec3b>(j, i).val[1] = ret.data.at<cv::Vec3b>(j, i).val[2];
                        ret.data.at<cv::Vec3b>(j, i).val[2] = tmp;
                    }
                }
            }
...
void VisionLive::ImageHandler::generateHistogram(int bitdepth)
{
	...
	float range[] = { -1*((float) nElem), (float) (nElem-1)};
	...
	QVector<QVector<QPointF> > finalHistograms;
	for(int c = 0; c < nChannels; c++)
    {
		QVector<QPointF> plain;
		for(int i = nElem; i < histograms[c].rows; i++)
		{
			plain.push_back(QPointF(i-nElem, histograms[c].at<float>(i)));
		}
		finalHistograms.push_back(plain);
	}

	emit histogramReceived(finalHistograms);
}
...
VisionLive::HistData VisionLive::ImageHandler::generateHistogram(const ImageData &imgData)
{
	...
	float range[] = { -1*((float) data.nElem), (float) (data.nElem-1)};
    const float *histRange = {range};
    ...
    {
		QVector<QPointF> plain;
		for(int i = data.nElem; i < histograms[c].rows; i++)
		{
			plain.push_back(QPointF(i-data.nElem, histograms[c].at<float>(i)));
		}
		data.histograms.push_back(plain);
	}
...
QImage *VisionLive::ImageHandler::generateAWBDebugImage(ImageData &imgData)
{
	int pixelBitdepth;
	int redIndex;
	int blueIndex;
	switch (imgData.pxlFormat)
	{
	case RGB_888_24:
	case BGR_888_24:
	case RGB_888_32:
	...
	// threshold defined in TRM "4.10. Auto White Balance Statistics"
	int markPixelValue = 0x6;

	int maxPixelValue = (1 << pixelBitdepth) - 1;
	int shiftBits = pixelBitdepth - 3;
	int nChannels = imgData.data.channels();

	//emit logError("PixelValueBitDepth: " + QString::number(pixelBitdepth) +
	//	" Channels: " + QString::number(nChannels) +
	//	" siftBits: " + QString::number(siftBits) +
	//	" maxPixelValue: " + QString::number(maxPixelValue), QStringLiteral("ImageHandler::generateAWBDebugImage()"));
	if (nChannels == 3)
	{
		for (int j = 0; j < imgData.data.rows; j++)
		{
			for (int i = 0; i < imgData.data.cols; i++)
			{
				if (imgData.data.at<cv::Vec3b>(j, i).val[blueIndex] >> shiftBits >= markPixelValue)
				{
					imgData.data.at<cv::Vec3b>(j, i).val[redIndex] = 0;
					imgData.data.at<cv::Vec3b>(j, i).val[1] = 0;
					imgData.data.at<cv::Vec3b>(j, i).val[blueIndex] = maxPixelValue;
				}
				else
				{
					imgData.data.at<cv::Vec3b>(j, i).val[redIndex] = imgData.data.at<cv::Vec3b>(j, i).val[1];
					//imgData.data.at<cv::Vec3b>(j, i).val[1] = 0;
					imgData.data.at<cv::Vec3b>(j, i).val[blueIndex] = imgData.data.at<cv::Vec3b>(j, i).val[1];
				}

				//imgData.data.at<cv::Vec3b>(j, i).val[blueIndex] =
				//	(imgData.data.at<cv::Vec3b>(j, i).val[blueIndex] >> siftBits == markPixelValue) ? maxPixelValue : imgData.data.at<cv::Vec3b>(j, i).val[1]; // Blue (info)
				//imgData.data.at<cv::Vec3b>(j, i).val[redIndex] = imgData.data.at<cv::Vec3b>(j, i).val[1]; // Red
			}
		}
	}
	else if (nChannels == 4)
	{
		for (int j = 0; j < imgData.data.rows; j++)
		{
			for (int i = 0; i < imgData.data.cols; i++)
			{
				if ((int)imgData.data.at<cv::Vec4f>(j, i).val[blueIndex] >> shiftBits >= markPixelValue)
				{
					imgData.data.at<cv::Vec4f>(j, i).val[redIndex] = 0;
					imgData.data.at<cv::Vec4f>(j, i).val[1] = 0;
					imgData.data.at<cv::Vec4f>(j, i).val[2] = 0;
					imgData.data.at<cv::Vec4f>(j, i).val[blueIndex] = maxPixelValue;
				}
				else
				{
					imgData.data.at<cv::Vec4f>(j, i).val[redIndex] = imgData.data.at<cv::Vec4f>(j, i).val[1];
					//imgData.data.at<cv::Vec4f>(j, i).val[1] = 0;
					imgData.data.at<cv::Vec4f>(j, i).val[2] = imgData.data.at<cv::Vec4f>(j, i).val[1];
					imgData.data.at<cv::Vec4f>(j, i).val[blueIndex] = imgData.data.at<cv::Vec4f>(j, i).val[1];
				}

				//imgData.data.at<cv::Vec4f>(j, i).val[blueIndex] =
				//	((int)imgData.data.at<cv::Vec4f>(j, i).val[blueIndex] >> siftBits == markPixelValue) ? maxPixelValue : imgData.data.at<cv::Vec4f>(j, i).val[1]; // Blue (info)
				//imgData.data.at<cv::Vec4f>(j, i).val[2] = imgData.data.at<cv::Vec4f>(j, i).val[1]; // Green2
				//imgData.data.at<cv::Vec4f>(j, i).val[redIndex] = imgData.data.at<cv::Vec4f>(j, i).val[1]; // Red
			}
		}
	}
	QImage *pImage;

	pImage = imgData.convertToQImage();
...
{
	if((unsigned int)recDataIndex >= _recData.size())
	{
		return;
	}
...
void VisionLive::ImageHandler::saveRecAsFLX(int recDataIndex, const QString &filename)
{
	...
	case RGB: saveType = ISPC::Save::Display; break;
	case DE: saveType = ISPC::Save::Bayer; break;
	case HDR: saveType = ISPC::Save::Display; break;
	case RAW2D: saveType = ISPC::Save::Bayer_TIFF; break;
	default: emit logError(tr("Couldn't save capture (") + _recData[recDataIndex]->name +
				 tr(") - unsupported format!"), "ImageHandler::saveRecAsFLX()");
			 _recLock.unlock();
			 return;
	}
	...
	for(int i = 0; i < _recData[recDataIndex]->nFrames; i++)
	{
		...
		case HDR: s.saveRGB_m(_recData[recDataIndex]->frameData[i].origData); break;
		case RAW2D: s.saveBayerTiff_m(_recData[recDataIndex]->frameData[i].origData); break;
		}
	}
...
void VisionLive::ImageHandler::saveRec_TF(const QString &recName, const QString &fileName, bool saveAsFLX)
{
	_recLock.lock();
	for(unsigned int i = 0; i < _recData.size(); i++)
	{
		if(_recData[i]->name == recName)
		{
			_recLock.unlock();
			if(saveAsFLX)
			{
				saveRecAsFLX(i, fileName);
			}
			else
			{
				saveRecAsJPEG(i, fileName);
			}
			return;
		}
		if(i == _recData.size()-1)
		{
			emit logError(tr("Couldn't save capture (") + recName +
				 tr(") - capture doesn't exist!"), "ImageHandler::saveRec_TF()");
		}
	}
	_recLock.unlock();
}
...
{
	_enableLiveFeed = !_enableLiveFeed;
}
