...
QString VisionLive::TabLogView::getLogs() const
{
	QString ret;
	...
	{
		if(_logs[i].type.size() > maxTypeSize) maxTypeSize = _logs[i].type.size();
		if(_logs[i].desc.size() > maxDescSize) maxDescSize = _logs[i].desc.size();
	}

	for(int i = 0; i < _logs.size(); i++)
	{
		ret += _logs[i].type.leftJustified(maxTypeSize+6, ' ') + _logs[i].desc.leftJustified(maxDescSize+6, ' ') + _logs[i].src + "\n";
	}

	return ret;
}

QString VisionLive::TabLogView::getActionLogs() const 
{ 
	return actionLog_te->toPlainText(); 
}
