...
#define EXPECTED_NUM_HW_INFO 58
	...
	while(_continueRun)
	{
		if(!_cmds.isEmpty())
		{
			...
			case GUICMD_ADD_LSH_GRID:
				emit logMessage("Executing GUICMD_ADD_LSH_GRID", tr("CommandHandler::run()"));
				if(LSH_add(_pSocket) != IMG_SUCCESS)
				{
					emit logError("LSH_add() failed!", tr("CommandHandler::run()"));
					_continueRun = false;
				}
				break;
            case GUICMD_REMOVE_LSH_GRID:
                emit logMessage("Executing GUICMD_REMOVE_LSH_GRID", tr("CommandHandler::run()"));
                if (LSH_remove(_pSocket) != IMG_SUCCESS)
                {
                    emit logError("LSH_remove() failed!", tr("CommandHandler::run()"));
                    _continueRun = false;
                }
                break;
            case GUICMD_SET_LSH_GRID:
                emit logMessage("Executing GUICMD_SET_LSH_GRID", tr("CommandHandler::run()"));
                if (LSH_apply(_pSocket) != IMG_SUCCESS)
                {
                    emit logError("LSH_apply() failed!", tr("CommandHandler::run()"));
                    _continueRun = false;
                }
                break;
            case GUICMD_SET_LSH_ENABLE:
                emit logMessage("Executing GUICMD_SET_LSH_ENABLE", tr("CommandHandler::run()"));
            ...
            case GUICMD_SET_PDP_ENABLE:
                emit logMessage("Executing GUICMD_SET_PDP_ENABLE", tr("CommandHandler::run()"));
                if (PDP_set(_pSocket) != IMG_SUCCESS)
                {
                    emit logError("PDP_set() failed!", tr("CommandHandler::run()"));
                    _continueRun = false;
                }
                break;
			case GUICMD_GET_FB:
		...
		if(_continueRun)
		{
            ...
            if (LSH_get(_pSocket) != IMG_SUCCESS)
            {
                emit logError("LSH_get() failed!", tr("CommandHandler::run()"));
                _continueRun = false;
            }
		}
...
int VisionLive::CommandHandler::getHWInfo(ParamSocket *pSocket)
{
    ...
    if (nParams != EXPECTED_NUM_HW_INFO)
	{
    ...
    _hwInfo.mmu_ui8Enabled = params[52].toInt();
    _hwInfo.mmu_ui32PageSize = params[53].toInt();
    _hwInfo.mmu_ui8RevMajor = params[54].toInt();
    _hwInfo.mmu_ui8RevMinor = params[55].toInt();
    _hwInfo.mmu_ui8RevMaint = params[56].toInt();

	_hwInfo.ui32RefClockMhz = params[57].toInt();
...
int VisionLive::CommandHandler::FB_get(ParamSocket *socket)
{
    ...
    // Get ParameterList
    ISPC::ParameterList extList;
    if (receiveConfiguration(socket, extList) != IMG_SUCCESS)
    {
        emit logError(tr("receiveConfiguration() failed!"), "CommandHandler::FB_get()");
        return IMG_ERROR_FATAL;
    }

    emit FB_received(extList);

	return IMG_SUCCESS;
}

int VisionLive::CommandHandler::receiveConfiguration(ParamSocket *socket, ISPC::ParameterList &list)
{
    IMG_UINT32 cmd[MAX_CMD_SIZE];
    size_t nRead;
    ...
    {
        emit logError(tr("Failed to read from socket!"), "CommandHandler::receiveConfiguration()");
        return IMG_ERROR_FATAL;
    }

    if (ntohl(cmd[0]) != GUICMD_SET_PARAMETER_LIST)
    {
        emit logError(tr("Unexpected command received!"), "CommandHandler::receiveConfiguration()");
        return IMG_ERROR_FATAL;
    }

    emit logMessage(tr("GUICMD_SET_PARAMETER_LIST"), "CommandHandler::receiveConfiguration()");
    ...
    while (continueRunning)
    {
        ...
        {
            emit logError(tr("Failed to write to socket!"), "CommandHandler::receiveConfiguration()");
            return IMG_ERROR_FATAL;
        }
        switch (ntohl(cmd[0]))
        {
        case GUICMD_SET_PARAMETER_NAME:
        {
            ...
            while (readingValues)
            {
                ...
                case GUICMD_SET_PARAMETER_END:
                    ...
                    break;
                default:
                    emit logError(tr("Urecognized command received!"), "CommandHandler::receiveConfiguration()");
                    return IMG_ERROR_FATAL;
                }
            }
        ...
        case GUICMD_SET_PARAMETER_LIST_END:
            emit logMessage(tr("GUICMD_SET_PARAMETER_LIST_END"), "CommandHandler::receiveConfiguration()");
...
int VisionLive::CommandHandler::LSH_add(ParamSocket *socket)
{
	IMG_UINT32 cmd[MAX_CMD_SIZE];
    size_t nWritten;
    ...
    std::string filename;
    for (unsigned int i = 0; i < _LSHGridFile.front().size(); i++)
    {
        filename.push_back(_LSHGridFile.front()[i]);
        if (_LSHGridFile.front()[i] == '/')
            filename.push_back('/');
    }
    ...
    if (!file.is_open())
    {
        emit logError(tr("Unable to open LSH file - ") + filename.c_str(), "CommandHandler::LSH_set()");
        file.close();
        _lock.unlock();
        return IMG_SUCCESS;
    }
    ...
    cmd[0] = htonl(GUICMD_ADD_LSH_GRID);
    if (socket->write(&cmd, sizeof(IMG_UINT32), nWritten, N_TRIES) != IMG_SUCCESS)
    {
    ...
    cmd[1] = htonl(cols);
    cmd[2] = htonl(rows);
    cmd[3] = htonl(_LSHTemp.front());
    cmd[4] = htonl(_LSHBitsPerDiff.front());
    cmd[5] = htonl(_LSHWBScale.front() * 1000);
    if (socket->write(&cmd, 6 * sizeof(IMG_UINT32), nWritten, N_TRIES) != IMG_SUCCESS)
    {
        emit logError(tr("Failed to write to socket!"), "CommandHandler::LSH_set()");
        file.close();
        _lock.unlock();
    ...
    // Receive acknolodgement
    if (socket->read(&cmd, sizeof(IMG_UINT32), nWritten, N_TRIES) != IMG_SUCCESS)
    {
        emit logError(tr("Failed to read to socket!"), "CommandHandler::LSH_set()");
        _lock.unlock();
        return IMG_ERROR_FATAL;
    }
    int ack = ntohl(cmd[0]);
    if (ack != IMG_SUCCESS)
    {
        emit logError(tr("Failed to add LSH grid ") + QString::fromLatin1(filename.c_str()), "CommandHandler::LSH_set()");
        emit riseError(tr("Failed to add LSH grid - ") + QString::fromLatin1(filename.c_str()));
    }
    else
    {
        emit logMessage(tr("LSH grid added ") + QString::fromLatin1(filename.c_str()), "CommandHandler::LSH_set()");
        emit LSH_added(QString::fromLatin1(_LSHGridFile.front().c_str()));
    }
...
int VisionLive::CommandHandler::LSH_remove(ParamSocket *socket)
{
    IMG_UINT32 cmd[MAX_CMD_SIZE];
    size_t nWritten;
    ...
    cmd[0] = htonl(GUICMD_REMOVE_LSH_GRID);
    if (socket->write(&cmd, sizeof(IMG_UINT32), nWritten, N_TRIES) != IMG_SUCCESS)
    {
        emit logError(tr("Failed to write to socket!"), "CommandHandler::LSH_remove()");
        _lock.unlock();
        return IMG_ERROR_FATAL;
    }
    ...
    if (socket->read(&cmd, sizeof(IMG_UINT32), nWritten, N_TRIES) != IMG_SUCCESS)
    {
        emit logError(tr("Failed to read frmo socket!"), "CommandHandler::LSH_set()");
        _lock.unlock();
        return IMG_ERROR_FATAL;
    }
    int ack = ntohl(cmd[0]);
    if (ack != IMG_SUCCESS)
    {
        emit logError(tr("Failed to remove LSH grid ") + QString::fromLatin1(filename.c_str()), "CommandHandler::LSH_remove()");
        emit riseError(tr("Failed to remove LSH grid - ") + QString::fromLatin1(filename.c_str()));
    }
    else
    {
        emit logMessage(tr("LSH grid removed ") + QString::fromLatin1(filename.c_str()), "CommandHandler::LSH_remove()");
        emit LSH_removed(QString::fromLatin1(_LSHGridFile.front().c_str()));
    }
...
int VisionLive::CommandHandler::LSH_apply(ParamSocket *socket)
{
    IMG_UINT32 cmd[MAX_CMD_SIZE];
    size_t nWritten;
    ...
    if (socket->write(&cmd, sizeof(IMG_UINT32), nWritten, N_TRIES) != IMG_SUCCESS)
    {
        emit logError(tr("Failed to write to socket!"), "CommandHandler::LSH_apply()");
        _lock.unlock();
        return IMG_ERROR_FATAL;
    }
    ...
    if (socket->read(&cmd, sizeof(IMG_UINT32), nWritten, N_TRIES) != IMG_SUCCESS)
    {
        emit logError(tr("Failed to read from socket!"), "CommandHandler::LSH_apply()");
        _lock.unlock();
        return IMG_ERROR_FATAL;
    }
    int ack = ntohl(cmd[0]);
    if (ack != IMG_SUCCESS)
    {
        emit logError(tr("Failed to apply LSH grid ") + QString::fromLatin1(filename.c_str()), "CommandHandler::LSH_apply()");
        emit riseError(tr("Failed to apply LSH grid - ") + QString::fromLatin1(filename.c_str()));
    }
    else
    {
        emit logMessage(tr("LSH grid applyed ") + QString::fromLatin1(filename.c_str()), "CommandHandler::LSH_apply()");
        emit LSH_applyed(QString::fromLatin1(_LSHGridFile.front().c_str()));
    }
...
int VisionLive::CommandHandler::LSH_set(ParamSocket *socket)
{
    ...
    cmd[0] = htonl(GUICMD_SET_LSH_ENABLE);
    cmd[1] = htonl(_LSHState);
    if (socket->write(&cmd, 2 * sizeof(IMG_UINT32), nWritten, N_TRIES) != IMG_SUCCESS)
    {
        emit logError(tr("Failed to write to socket!"), "CommandHandler::LSH_set()");
        return IMG_ERROR_FATAL;
...
int VisionLive::CommandHandler::LSH_get(ParamSocket *socket)
{
    IMG_UINT32 cmd[MAX_CMD_SIZE];
    size_t nRead, nWritten;

    cmd[0] = htonl(GUICMD_GET_LSH);
    if (socket->write(&cmd, sizeof(IMG_UINT32), nWritten, N_TRIES) != IMG_SUCCESS)
    {
        emit logError(tr("Failed to write to socket!"), "CommandHandler::AWB_get()");
        return IMG_ERROR_FATAL;
    ...
    if (socket->read(&cmd, sizeof(IMG_UINT32), nRead, N_TRIES) != IMG_SUCCESS)
    {
        emit logError(tr("Failed to read to socket!"), "CommandHandler::LSH_get()");
        return IMG_ERROR_FATAL;
    }
    int filenameLength = ntohl(cmd[0]);
    QString currentLSHMatrix;
    if (filenameLength > 0)
    {
        ...
        currentLSHMatrix = QString::fromLatin1(filename.c_str());
    }

    emit LSH_received(currentLSHMatrix);
...
int VisionLive::CommandHandler::PDP_set(ParamSocket *socket)
{
    IMG_UINT32 cmd[MAX_CMD_SIZE];
    size_t nWritten;

    cmd[0] = htonl(GUICMD_SET_PDP_ENABLE);
    cmd[1] = htonl(_PDPState);
    if (socket->write(&cmd, 2 * sizeof(IMG_UINT32), nWritten, N_TRIES) != IMG_SUCCESS)
    {
        emit logError(tr("Failed to write to socket!"), "CommandHandler::PDP_set()");
        return IMG_ERROR_FATAL;
    }
...
void VisionLive::CommandHandler::LSH_add(std::string filename, int temp, unsigned int bitsPerDiff, double wbScale)
{
	_lock.lock();
	_LSHGridFile.push(filename);
    _LSHTemp.push(temp);
    _LSHBitsPerDiff.push(bitsPerDiff);
    _LSHWBScale.push(wbScale);
	_lock.unlock();
	pushCommand(GUICMD_ADD_LSH_GRID);
}

void VisionLive::CommandHandler::LSH_remove(std::string filename)
{
    _lock.lock();
    _LSHGridFile.push(filename);
    _lock.unlock();
    pushCommand(GUICMD_REMOVE_LSH_GRID);
}

void VisionLive::CommandHandler::LSH_apply(std::string filename)
{
    _lock.lock();
    _LSHGridFile.push(filename);
    _lock.unlock();
...
void VisionLive::CommandHandler::LSH_set(bool enable)
{
    _lock.lock();
    _LSHState = enable;
    _lock.unlock();
    pushCommand(GUICMD_SET_LSH_ENABLE);
}
...
void VisionLive::CommandHandler::PDP_set(bool enable)
{
    _lock.lock();
    _PDPState = enable;
    _lock.unlock();
    pushCommand(GUICMD_SET_PDP_ENABLE);
}