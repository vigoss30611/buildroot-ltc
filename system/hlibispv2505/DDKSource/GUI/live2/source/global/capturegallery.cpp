...
void VisionLive::CaptureGallery::addItem(CaptureGalleryItem *item)
{
	...
	{
		item->setObjectName(item->getRecData()->name + QStringLiteral("_thumbnail"));
		if(_pObjReg)
		{
			_pObjReg->registerChildrenObjects(item, QStringLiteral("Captures/"), item->objectName());
		}
		else
		{
			emit logError("Couldn't register CaptureGalleryItem!", "CaptureGallery::open()");
		}
...
QStringList VisionLive::CaptureGallery::getItemsList()
{
	QStringList ret;

	for(unsigned int i = 0; i < _items.size(); i++)
	{
		ret.push_back(_items[i]->getRecData()->name);
	}

	return ret;
}

void VisionLive::CaptureGallery::setObjectRegistry(ObjectRegistry *objReg) 
{ 
	_pObjReg = objReg; 
}
...
void VisionLive::CaptureGallery::initCaptureGallery()
{
	...
	DisplayArea->setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
...
void VisionLive::CaptureGallery::open(CaptureGalleryItem *item)
{
	...
	preview->setObjectName(item->getRecData()->name + QStringLiteral("_preview"));
	if(_pObjReg)
	{
		_pObjReg->registerChildrenObjects(preview, QStringLiteral("Captures/"), preview->objectName());
	}
	else
	{
		emit logError("Couldn't register CapturePreview!", "CaptureGallery::open()");
	}
		...
		for(unsigned int i = 0; i < _items.size(); i++)
		{
			if(item == _items[i])
			{
				...
				if(filename.isEmpty())
				{
					...
					QString title = tr("Save ") + ImageTypeString(type) + tr(" image");
					QString filter = ImageTypeString(type) + tr("images (*.flx)");
					proposedName += "_" + ImageTypeString(type) + ".flx";
	...
	if(_pProxyHandler)
	{
		for(unsigned int i = 0; i < _items.size(); i++)
		{
			if(item == _items[i])
			{
				...
				if(_pObjReg)
				{
					_pObjReg->deregisterObject(item->objectName());
				}
				else
				{
					emit logError("Couldn't deregister CaptureCalleryItem!", "CaptureGallery::open()");
				}
				item->deleteLater();
...
void VisionLive::CaptureGallery::previewClosed(CapturePreview *preview)
{
	for(unsigned int i = 0; i < _previews.size(); i++)
	{
		if(_previews[i] == preview)
		{
			if(_pObjReg)
			{
				_pObjReg->deregisterObject(_previews[i]->objectName());
			}
			else
			{
				emit logError("Couldn't deregister CapturePreview!", "CaptureGallery::open()");
			}
			delete _previews[i];
