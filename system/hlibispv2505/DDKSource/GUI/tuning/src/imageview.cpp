    ...
    if ( !sBuffer.data.empty() )
    {
        ...
        int nElem;
        {
            ...
            for ( int c = 0 ; c < nChannels ; c++ )
            {
                ...
                cv::calcHist(&channels[c], 1, nChan, cv::Mat(), histograms, 1, &histSize, &histRange, true, false);
#else
                // compute histograms manually
                histograms = cv::Mat::zeros(1, histSize, CV_32F);

                for (int j = 0; j < channels[c].rows; j++)
                {
                    for (int i = 0; i < channels[c].cols; i++)
                    {
                        int l = ceil(channels[c].at<float>(j, i));
                        if (channels[c].at<float>(j, i) > 0)
                        {
                            // if not negative push in upper part
                            l += histSize / 2;
                        }
                        histograms.at<float>(l) = histograms.at<float>(l) +1;
                    }
                }
#endif
                ...
                for ( int i = 0 ; i < histSize ; i++ )
                {
                    ...
                    maxHist = std::max(maxHist, histograms.at<float>(i));
                }
            }
        ...
        for ( int h = 0 ; h < nChannels ; h++ )
        {
            ...
            histogramsPlots[h]->setAxisScale(QwtPlot::yLeft, 0.0, maxHist);
