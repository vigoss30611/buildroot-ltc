...
int LCATuning::tuning()
{
	...
	if(_isVLWidget)
    {
		previewDockWidget->setWidget(resultView);
...
void LCATuning::logMessage(const QString &message)
{
	log_te->append(message);
}

void LCATuning::showLog()
{
	logDockWidget->setVisible(!logDockWidget->isVisible());

	//if(!log_gb->isVisible()) log_btn->setText("Show Logs");
	//else log_btn->setText("Hide Logs");

	//resize(width(), minimumHeight());
}

void LCATuning::showPreview()
{
