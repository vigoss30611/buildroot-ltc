...
int CCMTuning::tuning()
{
	...
	if(_isVLWidget)
	{
		previewDockWidget->setWidget(images_grp);
...
void CCMTuning::saveParameters(ISPC::ParameterList &list, int ccmN) const throw(QtExtra::Exception)
{
	...
	ISPC::Parameter blcSensorBlack(groupName2 + ISPC::ModuleBLC::BLC_SENSOR_BLACK.indexed(ccmN).name, os.str());
	list.addParameter(blcSensorBlack);
	...
	ISPC::Parameter ccmMatrix(ISPC::TemperatureCorrection::WB_CCM_S.indexed(ccmN).name);
	for ( int i = 0 ; i < 9 ; i++ ) ccmMatrix.addValue(ISPC::toString(ccm.aMatrix[i]));
	list.addParameter(ccmMatrix);

	ISPC::Parameter ccmOffset(ISPC::TemperatureCorrection::WB_OFFSETS_S.indexed(ccmN).name);
    for ( int i = 0 ; i < 3 ; i++ ) ccmOffset.addValue(ISPC::toString(ccm.aOffset[i]));
	list.addParameter(ccmOffset);

	ISPC::Parameter wbcGains(ISPC::TemperatureCorrection::WB_GAINS_S.indexed(ccmN).name);
    for ( int i = 0 ; i < 4 ; i++ ) wbcGains.addValue(ISPC::toString(wbc.aWBGain[i]));
	list.addParameter(wbcGains);

	ISPC::Parameter wbcTemp(ISPC::TemperatureCorrection::WB_TEMPERATURE_S.indexed(ccmN).name, std::string(QString::number(temperature->value()).toLatin1()));
...
void CCMTuning::logMessage(const QString &message)
{
	log_te->append(message);
}

void CCMTuning::showLog()
{
	logDockWidget->setVisible(!logDockWidget->isVisible());
}

void CCMTuning::showPreview()
{
