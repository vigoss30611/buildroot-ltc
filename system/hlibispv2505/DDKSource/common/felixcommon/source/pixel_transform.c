...
IMG_INT PixelFormatYccBitDepth(ePxlFormat format)
{
    IMG_INT res;

    if (PXL_ISP_444IL3YCrCb8 == format ||
        PXL_ISP_444IL3YCbCr8 == format)
    {
        res = 8;
    }
    else
    if (PXL_ISP_444IL3YCrCb10 == format ||
        PXL_ISP_444IL3YCbCr10 == format)
    {
        res = 10;
    }
...
IMG_BOOL PixelFormatIsPackedYcc(ePxlFormat format)
{
    if (PXL_ISP_444IL3YCrCb8 == format || PXL_ISP_444IL3YCbCr8 == format ||
        PXL_ISP_444IL3YCrCb10 == format || PXL_ISP_444IL3YCbCr10 == format)
    {
        return IMG_TRUE;
    }
    return IMG_FALSE;
...
IMG_RESULT PixelTransformYUV(PIXELTYPE *pType, ePxlFormat yuv)
{
    if (PXL_NONE == yuv)
    {
    ...
    } else if(PXL_ISP_444IL3YCrCb8 == yuv || PXL_ISP_444IL3YCbCr8 == yuv
        || PXL_ISP_444IL3YCrCb10 == yuv || PXL_ISP_444IL3YCbCr10 == yuv) // 444
    {
            pType->ui8HSubsampling = pType->ui8VSubsampling = 1;
    } else {
        return IMG_ERROR_INVALID_PARAMETERS;
    }
        ...
        || YVU_422_PL12_10 == yuv || YUV_422_PL12_10 == yuv)// 10b packed
    {
        ...
        // YUV_444 in HW 2.7 only supported for display
        // will be supported here for HW 3.0.
    else if(PXL_ISP_444IL3YCrCb8 == yuv || PXL_ISP_444IL3YCbCr8 == yuv)
    {
        pType->ui8BitDepth = 8; // bits

        pType->ui8PackedElements = 1;
        pType->ui8PackedStride = 3; // Bytes
        pType->ePackedStart = PACKED_LSB;
    }
    else if(PXL_ISP_444IL3YCrCb10 == yuv || PXL_ISP_444IL3YCbCr10 == yuv)
    {
        pType->ui8BitDepth = 10; // bits

        pType->ui8PackedElements = 1;
        pType->ui8PackedStride = 4; // Bytes
        pType->ePackedStart = PACKED_LSB;
    }
    else {
        return IMG_ERROR_INVALID_PARAMETERS;
...
IMG_RESULT PixelTransformRGB(PIXELTYPE *pType, ePxlFormat rgb)
{
    if (PXL_NONE == rgb)
    {
        return IMG_ERROR_INVALID_PARAMETERS;
...
IMG_RESULT PixelTransformDisplay(PIXELTYPE *pType, ePxlFormat format)
{
    IMG_RESULT res = PixelTransformRGB(pType, format);

    // check packed YUV format first
    if (PixelFormatIsPackedYcc(format))
    {
        pType->eFmt = format;
        pType->eBuffer = TYPE_YUV;
        pType->eMosaic = MOSAIC_NONE;
        ...
        else if (PXL_ISP_444IL3YCrCb10 == format || PXL_ISP_444IL3YCbCr10 == format)
        {
            pType->ui8BitDepth = 10; // bits
            pType->ui8PackedStride = 4; // Bytes
            res = IMG_SUCCESS;
        ...
        {
            LOG_ERROR("YUV packed format %s not supported", FormatString(format));
            res = IMG_ERROR_INVALID_PARAMETERS;
        }
    }

    if (IMG_SUCCESS != res && IMG_ERROR_INVALID_PARAMETERS != res)
    {
        res = PixelTransformRGB(pType, format);
    }
    return res;
}
...
const char* FormatStringForFilename(ePxlFormat ePxl)
{
    ...
    case PXL_ISP_444IL3YCbCr8:
        return "YVU_444";
    case PXL_ISP_444IL3YCrCb10:
        return "YUV_444-10bit";
    case PXL_ISP_444IL3YCbCr10:
        return "YVU_444-10bit";
...
enum pxlFormat FormatIndex(const char *format)
{
    ...
    int i;
    for (i = PXL_NONE; i < PXL_N; i++)
    {
        const char *str = FormatString((ePxlFormat)i);
