...
/**
 * @brief Test that output size of YUV 8b semiplanar are correct
 */
TEST(ALLOCINFO, YUV8)
{
    ...
    for ( eF = PXL_NONE ; eF < PXL_N ; eF++ )
    {
        if ( PixelTransformYUV(&fmt, (ePxlFormat)eF) == IMG_SUCCESS
            && fmt.ui8BitDepth == 8
            && !PixelFormatIsPackedYcc((ePxlFormat)eF))
        {
...
/**
 * @brief Test that output size of YUV 10b semiplanar are correct
 */
...
/**
 * @brief Test that output size of YUV444 8b interleaved are correct
 */
TEST(ALLOCINFO, YUV444i)
{
    struct CI_SIZEINFO sSize;
    PIXELTYPE fmt;
    ...
    for ( eF = PXL_NONE ; eF < PXL_N ; eF++ )
    {
        if ( PixelFormatIsPackedYcc((ePxlFormat)eF)
            && PixelTransformYUV(&fmt, (ePxlFormat)eF) == IMG_SUCCESS)
        {
            for ( int s = 0 ; s < nSizes ; s++ )
            {
                ASSERT_EQ(IMG_SUCCESS, CI_ALLOC_YUVSizeInfo(&fmt, width[s],
                ...
                // Y|U|V interleaved 8b in 24bits or 10b packed in 32bits
                EXPECT_TRUE(fmt.ui8PackedStride==3||fmt.ui8PackedStride==4)
                    << "YUV444i format " << FormatString((ePxlFormat)eF)
    ...
    ASSERT_GT(nTested, 0) << "no YUV8i format found!";
}

/**
 * @brief Test that output size of YUV semiplanar tiled are correct
 *
...
TEST(ALLOCINFO, YUV444i_tiled)
{
    ...
    ASSERT_GT(nTested, 0) << "no YUV444i tiled format tested!";
}
