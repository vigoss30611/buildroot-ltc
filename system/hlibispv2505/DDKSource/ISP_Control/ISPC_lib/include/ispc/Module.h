...
// to be used these macro expects userlog.h to be included
/** @brief getter from ModuleBase class */
#define MOD_LOG_TAG getLoggingName()

/*
 * Macros used in modules. Use logtag provided to construcftor of
...
enum SetupID {
    ...
    STP_AWS, /**< @brief Auto White Balance Statistics */
...
#endif
    CTRL_LSH, /**< @brief ID for lens shading control */
    /**
...
public:
    ...
    /**
     * @brief so that children can print their state if they want to
     *
     * Format should be similar to the one used in text statistics
     * in @ref Save::saveTxtStats()
     */
...
public:
    ...
    explicit  ControlModule(const std::string &loggingName)
        : ModuleBase(loggingName),
          pipelineList(), pipelineOwner(pipelineList.begin()),
#ifdef INFOTM_ISP
          enabled(true), updateMode(UPDATE_SHOT), pCamera(NULL) {}
#else
          enabled(true), updateMode(UPDATE_SHOT) {}
#endif //INFOTM_ISP
     ...
     *
     */
    virtual IMG_RESULT setPipelineOwner(Pipeline *pipe);

    /** @brief Access to the Pipeline owner of this Module */
    virtual Pipeline* getPipelineOwner() const;
    ...
    */
    ...
    /**
     * @brief Give information about convergence status
     *
     * If algorithm is not using a convergence approach should return true
     */
