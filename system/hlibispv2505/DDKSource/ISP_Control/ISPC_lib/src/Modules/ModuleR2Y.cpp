...
IMG_RESULT ISPC::ModuleR2Y::save(ParameterList &parameters, SaveType t) const
{
    ...
    case SAVE_VAL:
        if (BT601 == eMatrix)
        {
            parameters.addParameter(R2Y_MATRIX_STD,
                std::string(BT601_STR));
        }
        else if (BT709 == eMatrix)
        {
            parameters.addParameter(R2Y_MATRIX_STD,
                    std::string(BT709_STR));
        }
        ...
        parameters.addParameter(R2Y_BRIGHTNESS, this->fBrightness);
        parameters.addParameter(R2Y_CONTRAST, this->fContrast);
        parameters.addParameter(R2Y_SATURATION, this->fSaturation);
        parameters.addParameter(R2Y_HUE, this->fHue);
        parameters.addParameter(R2Y_OFFSETU, this->fOffsetU);
        parameters.addParameter(R2Y_OFFSETV, this->fOffsetV);
        ...
        parameters.addParameter(R2Y_RANGEMULT, values);
        break;

    case SAVE_MIN:
        // matrix std does not have a min
        parameters.addParameterMin(R2Y_MATRIX_STD);
        parameters.addParameterMin(R2Y_BRIGHTNESS);
        parameters.addParameterMin(R2Y_CONTRAST);
        parameters.addParameterMin(R2Y_SATURATION);
        parameters.addParameterMin(R2Y_HUE);
        parameters.addParameterMin(R2Y_OFFSETU);
        parameters.addParameterMin(R2Y_OFFSETV);
        parameters.addParameterMin(R2Y_RANGEMULT);
        break;

    case SAVE_MAX:
        // matrix std does not have a max
        parameters.addParameterMax(R2Y_MATRIX_STD);
        parameters.addParameterMax(R2Y_BRIGHTNESS);
        parameters.addParameterMax(R2Y_CONTRAST);
        parameters.addParameterMax(R2Y_SATURATION);
        parameters.addParameterMax(R2Y_HUE);
        parameters.addParameterMax(R2Y_OFFSETU);
        parameters.addParameterMax(R2Y_OFFSETV);
        parameters.addParameterMax(R2Y_RANGEMULT);
        break;
    ...
    case SAVE_DEF:
        {
            ...
            defComment << "{"
                ...
                << ", " << JFIF_STR
                << "}";
            parameters.addParameterDef(R2Y_MATRIX_STD);
            parameters.getParameter(R2Y_MATRIX_STD.name)->setInfo(
                    defComment.str());
        }
        parameters.addParameterDef(R2Y_BRIGHTNESS);
        parameters.addParameterDef(R2Y_CONTRAST);
        parameters.addParameterDef(R2Y_SATURATION);
        parameters.addParameterDef(R2Y_HUE);
        parameters.addParameterDef(R2Y_OFFSETU);
        parameters.addParameterDef(R2Y_OFFSETV);
        parameters.addParameterDef(R2Y_RANGEMULT);
        break;
    }
    ...
    bool bSwapInput, bool bSwapOutput) const
{
    ...
    LOG_ERROR("Invalid plane value\n");
    return IMG_ERROR_INVALID_PARAMETERS;;
}

const char *ISPC::ModuleR2YBase::stdMatrixName(StdMatrix std)
{
    switch (std)
    {
...
enum ISPC::ModuleR2YBase::StdMatrix ISPC::ModuleR2YBase::stdIndex(const char *name)
{
    int ret = 0;

    for (int i = STD_MIN; i < STD_MAX; i++)
    {
        const char *str = stdMatrixName((StdMatrix)i);
