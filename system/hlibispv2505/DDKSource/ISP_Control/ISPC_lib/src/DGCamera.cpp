...
IMG_RESULT ISPC::DGCamera::acquireShot(Shot &shot, bool block,
    bool updateControl)
{
    IMG_RESULT ret;

#ifndef INFOTM_ISP
    LOG_INFO("acquire shot!\n");
#endif
    ret = Camera::acquireShot(shot, block, updateControl);
    if (IMG_SUCCESS == ret && bOwnSensor)
    {
        ret = this->sensor->waitProcessed();
        if (IMG_ERROR_NOT_SUPPORTED == ret)
        {
            // this sensor does not need wait processed
            ret = IMG_SUCCESS;
        }
    }
