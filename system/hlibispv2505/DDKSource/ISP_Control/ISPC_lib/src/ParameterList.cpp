...
IMG_RESULT ISPC::ParameterList::addParameterMin(
        const ISPC::ParamDefSingle<bool>& paramDef,
        bool overwrite)
{
    return addParameter(paramDef, false, overwrite);
}

IMG_RESULT ISPC::ParameterList::addParameterMax(
        const ISPC::ParamDefSingle<bool>& paramDef,
        bool overwrite)
{
    return addParameter(paramDef, true, overwrite);
}
...
bool ISPC::operator==(const ParameterList &lhs, const ParameterList &rhs)
{
    if (lhs.size() != rhs.size())
        return false;

    ISPC::ParameterList::const_iterator thisIt = lhs.begin();
    ISPC::ParameterList::const_iterator rhsIt = rhs.begin();
...
{
    //if(param.groupsBegin()==param.groupsEnd())
    {
        for(ISPC::ParameterList::const_iterator i = param.begin();
            i!=param.end();++i)
        {
            out << i->second;
        }
        ...
        for(ISPC::ParameterList::const_group_iterator i = param.groupsBegin();
            i!=param.groupsEnd();++i)
        {
            out << i->first << std::endl;
            const std::set<std::string>& params = i->second;
            for(std::set<std::string>::const_iterator j = params.begin();
                j!=params.end(); ++j)
            {
                const ISPC::Parameter* p = param.getParameter(*j);
                if(p)
                {
                    out << *p;
                }
