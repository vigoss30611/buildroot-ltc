...
#ifdef INFOTM_ISP
    this->pCIPipeline->config.bIsLinkMode = false;
#endif
    this->pCIPipeline->config.ui8Context = this->ui8ContextNumber;
...
IMG_RESULT ISPC::Pipeline::programPipeline(bool bUpdateASAP)
{
    ...
    pCIPipeline->config.ui16MaxDispOutWidth = ui32MaxDisplayWidth;
    pCIPipeline->config.ui16MaxDispOutHeight = ui32MaxDisplayHeight;
    pCIPipeline->config.ui16MaxEncOutWidth = ui32MaxEncoderWidth;
    pCIPipeline->config.ui16MaxEncOutHeight = ui32MaxEncoderHeight;

    if (pCIConnection->sHWInfo.eFunctionalities&CI_INFO_SUPPORTED_TILING)
    {
        pCIPipeline->config.bSupportTiling = bEnableTiling ? IMG_TRUE : IMG_FALSE;
    }
    else
    {
        if (bEnableTiling)
        ...
        pCIPipeline->config.bSupportTiling = IMG_FALSE;
    }
...
IMG_RESULT ISPC::Pipeline::releaseShot(Shot &shot)
{
    ...
    IMG_MEMSET(&(shot.DISPLAY), 0, sizeof(Buffer));
    shot.DISPLAY.pxlFormat = PXL_NONE;
...
ISPC::Pipeline::const_iterator ISPC::Pipeline::getModulesStart() const
{
    return moduleRegistry.begin();
}

ISPC::Pipeline::const_iterator ISPC::Pipeline::getModulesEnd() const
{
    return moduleRegistry.end();
}
