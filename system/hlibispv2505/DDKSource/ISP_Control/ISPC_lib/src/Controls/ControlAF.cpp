...
bool ISPC::ControlAF::hasConverged() const
{
    if (AF_SCANNING == getState())
    {
        return false;
    }
    return true;
...
IMG_RESULT ISPC::ControlAF::save(ParameterList &parameters, SaveType t) const
{
    ...
    {
    case SAVE_VAL:
        parameters.addParameter(AF_CENTER_WEIGTH, this->centerWeigth);
        break;

    case SAVE_MIN:
        parameters.addParameterMin(AF_CENTER_WEIGTH);
        break;

    case SAVE_MAX:
        parameters.addParameterMax(AF_CENTER_WEIGTH);
        break;

    case SAVE_DEF:
        parameters.addParameterDef(AF_CENTER_WEIGTH);
        break;
    }
...
std::ostream& ISPC::ControlAF::printState(std::ostream &os) const
{
    os << SAVE_A1 << getLoggingName() << ":" << std::endl;

    os << SAVE_A2 << "config:" << std::endl;
    os << SAVE_A2 << "enabled = " << enabled << std::endl;
    os << SAVE_A3 << "targetFocusDistance = "
        << targetFocusDistance << std::endl;
    os << SAVE_A3 << "centerWeigth = " << centerWeigth << std::endl;

    os << SAVE_A2 << "state:" << std::endl;
    os << SAVE_A3 << "hasConverged = " << hasConverged() << std::endl;
    os << SAVE_A3 << "bestSharpness = " << bestSharpness << std::endl;
    os << SAVE_A3 << "bestFocusDistance = " << bestFocusDistance << std::endl;
    os << SAVE_A3 << "currentState = " << StateName(currentState) << std::endl;
    os << SAVE_A3 << "nextState = " << StateName(nextState) << std::endl;
    os << SAVE_A3 << "scanState = " << ScanStateName(scanState) << std::endl;
    os << SAVE_A3 << "lastCommand = " << CommandName(lastCommand) << std::endl;
    os << SAVE_A3 << "scanInit = " << scanInit << std::endl;
    os << SAVE_A3 << "scanEnd = " << scanEnd << std::endl;
    ...
    case AF_SCANNING:  // Scanning for optimal focus
        ...
        else if (AF_SCAN_ROUGH == scanState)
        {
            ...
            targetFocusDistance = lastFocusDistance
                + (scanEnd - scanInit) / 10;
            targetFocusDistance = ISPC::clip(targetFocusDistance,
            ...
            if (lastFocusDistance == scanEnd)  // finished the rough scanning
            {
                ...
                scanInit = ISPC::clip(scanInit,
                    minFocusDistance, maxFocusDistance);
                scanEnd = ISPC::clip(scanEnd,
                    minFocusDistance, maxFocusDistance);
