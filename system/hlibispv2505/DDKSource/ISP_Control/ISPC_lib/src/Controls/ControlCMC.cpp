...
ISPC::ControlCMC::ControlCMC(const std::string &logname)
...
IMG_RESULT ISPC::ControlCMC::save(ParameterList &parameters, SaveType t) const
{
    ...
    {
    case SAVE_VAL:
        parameters.addParameter(CMC_ENABLE, this->bCmcEnable);
        parameters.addParameter(CMC_DN_TARGET_IDX_CHANGE_ENABLE, this->bDnTargetIdxChageEnable);
        parameters.addParameter(CMC_ENABLE_AVG_GAIN, this->bEnableCalcAvgGain);

        parameters.addParameter(CMC_R2Y_SATURATION_EXPECT, this->fR2YSaturationExpect);

        parameters.addParameter(CMC_SHA_STRENGTH_EXPECT, this->fShaStrengthExpect);

        parameters.addParameter(CMC_NIGHT_MODE_DETECT_ENABLE, this->bCmcNightModeDetectEnable);
        parameters.addParameter(CMC_NIGHT_MODE_DETECT_BRIGHTNESS_ENTER, this->fCmcNightModeDetectBrightnessEnter);
        parameters.addParameter(CMC_NIGHT_MODE_DETECT_BRIGHTNESS_EXIT, this->fCmcNightModeDetectBrightnessExit);
        parameters.addParameter(CMC_NIGHT_MODE_DETECT_GAIN_ENTER, this->fCmcNightModeDetectGainEnter);
        parameters.addParameter(CMC_NIGHT_MODE_DETECT_GAIN_EXIT, this->fCmcNightModeDetectGainExit);
        parameters.addParameter(CMC_NIGHT_MODE_DETECT_WEIGHTING, this->fCmcNightModeDetectWeighting);

        parameters.addParameter(CMC_ENABLE_CAPTURE_IQ, this->bEnableCaptureIQ);
        parameters.addParameter(CMC_CAPTURE_CCM_RATIO, this->fCmcCaptureModeCCMRatio);
        values.clear();
        for (i = 0 ; i < 3 ; i++)
            values.push_back(toString(this->fCmcCaptureR2YRangeMul[i]));
        parameters.addParameter(CMC_R2Y_CAPTURE_RANGE_MUL, values);
        values.clear();
        for (i = 0 ; i < 2 ; i++)
            values.push_back(toString(this->iCmcCaptureInY[i]));
        parameters.addParameter(CMC_TNM_CAPTURE_IN_Y, values);
        values.clear();
        for (i = 0 ; i < 2 ; i++)
            values.push_back(toString(this->iCmcCaptureOutY[i]));
        parameters.addParameter(CMC_TNM_CAPTURE_OUT_Y, values);
        values.clear();
        for (i = 0 ; i < 2 ; i++)
            values.push_back(toString(this->iCmcCaptureOutC[i]));
        parameters.addParameter(CMC_TNM_CAPTURE_OUT_C, values);

        //3D-Denoise target index.
        parameters.addParameter(CMC_DN_TARGET_IDX_FCM, (int)this->ui32DnTargetIdx_fcm);
        ...
        parameters.addParameter(CMC_BLC_SENSOR_BLACK_FCM, values);
        parameters.addParameter(CMC_BLC_SYS_BLACK_FCM, this->ui32SystemBlack_fcm);

        //DNS
        parameters.addParameter(CMC_DNS_STRENGTH_FCM, this->fDnsStrength_fcm);
        parameters.addParameter(CMC_DNS_READ_NOISE_FCM, this->fDnsReadNoise_fcm);
        parameters.addParameter(CMC_DNS_WELLDEPTH_FCM, this->iDnsWellDepth_fcm);

        //SHA
        parameters.addParameter(CMC_SHA_RADIUS_FCM, this->fRadius_fcm);
        parameters.addParameter(CMC_SHA_STRENGTH_FCM, this->fStrength_fcm);
        parameters.addParameter(CMC_SHA_THRESH_FCM, this->fThreshold_fcm);
        parameters.addParameter(CMC_SHA_DETAIL_FCM, this->fDetail_fcm);
        parameters.addParameter(CMC_SHA_EDGE_SCALE_FCM, this->fEdgeScale_fcm);
        parameters.addParameter(CMC_SHA_EDGE_OFFSET_FCM, this->fEdgeOffset_fcm);
        parameters.addParameter(CMC_SHA_DENOISE_BYPASS_FCM, this->bBypassDenoise_fcm);
        parameters.addParameter(CMC_SHADN_TAU_FCM, this->fDenoiseTau_fcm);
        parameters.addParameter(CMC_SHADN_SIGMA_FCM, this->fDenoiseSigma_fcm);

        //R2Y
        parameters.addParameter(CMC_R2Y_BRIGHTNESS_FCM, this->fBrightness_fcm);
        parameters.addParameter(CMC_R2Y_CONTRAST_FCM, this->fContrast_fcm);
        parameters.addParameter(CMC_R2Y_SATURATION_FCM, this->fSaturation_fcm);
        values.clear();
        for (i = 0 ; i < 3 ; i++)
            values.push_back(toString(this->aRangeMult_fcm[i]));
        parameters.addParameter(CMC_R2Y_RANGE_MUL_FCM, values);
        ...
        parameters.addParameter(CMC_HIS_GRID_START_COORDS_FCM, values);
        values.clear();
        for (i = 0 ; i < 2 ; i++) {
            values.push_back(toString(this->iGridTileDimensions_fcm[i]));
        }
        parameters.addParameter(CMC_HIS_GRID_TILE_DIMENSIONS_FCM, values);

        //DPF
        parameters.addParameter(CMC_DPF_WEIGHT_FCM, this->fDpfWeight_fcm);
        parameters.addParameter(CMC_DPF_THRESHOLD_FCM, this->iDpfThreshold_fcm);

        //TNM
        parameters.addParameter(CMC_TNMC_HISTMIN_FCM, this->fHisMin_fcm);
        parameters.addParameter(CMC_TNMC_HISTMAX_FCM, this->fHisMax_fcm);
        parameters.addParameter(CMC_TNMC_ADAPTIVE_FCM, this->bTnmcAdaptive_fcm);

        values.clear();
        for (i = 0 ; i < 2 ; i++) {
            values.push_back(toString(iInY_fcm[i]));
        }
        parameters.addParameter(CMC_TNM_IN_Y_FCM, values);
        values.clear();
        for (i = 0 ; i < 2 ; i++) {
            values.push_back(toString(iOutY_fcm[i]));
        }
        parameters.addParameter(CMC_TNM_OUT_Y_FCM, values);
        values.clear();
        for (i = 0 ; i < 2 ; i++) {
            values.push_back(toString(iOutC_fcm[i]));
        }
        parameters.addParameter(CMC_TNM_OUT_C_FCM, values);
        parameters.addParameter(CMC_TNM_FLAT_FACTOR_FCM, this->fFlatFactor_fcm);
        parameters.addParameter(CMC_TNM_WEIGHT_LINE_FCM, this->fWeightLine_fcm);
        parameters.addParameter(CMC_TNM_COLOUR_CONFIDENCE_FCM, this->fColourConfidence_fcm);
        parameters.addParameter(CMC_TNM_COLOUR_SATURATION_FCM, this->fColourSaturation_fcm);
        parameters.addParameter(CMC_TNMC_ENABLE_GAMMA_FCM, this->bEnableGamma_fcm);
        parameters.addParameter(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_FCM, this->fEqualBrightSupressRatio_fcm);
        parameters.addParameter(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_FCM, this->fEqualDarkSupressRatio_fcm);
        parameters.addParameter(CMC_TNMC_OVERSHOT_THRESHOLD_FCM, fOvershotThreshold_fcm);
        values.clear();
        for (i = 0 ; i < TNMC_WDR_SEGMENT_CNT ; i++) {
            values.push_back(toString(fWdrCeiling_fcm[i]));
        }
        parameters.addParameter(CMC_TNMC_WDR_CEILING_FCM, values);
        values.clear();
        for (i = 0 ; i < TNMC_WDR_SEGMENT_CNT ; i++) {
            values.push_back(toString(fWdrFloor_fcm[i]));
        }
        parameters.addParameter(CMC_TNMC_WDR_FLOOR_FCM, values);
        parameters.addParameter(CMC_TNMC_GAMMA_CRV_MODE_FCM, this->ui32GammaCrvMode_fcm);
        parameters.addParameter(CMC_TNMC_GAMMA_FCM, this->fTnmGamma_fcm);
        parameters.addParameter(CMC_TNMC_BEZIER_CTRL_PNT_FCM, this->fBezierCtrlPnt_fcm);

        //AE
        parameters.addParameter(CMC_AE_TARGET_BRIGHTNESS_FCM, this->targetBrightness_fcm);
        parameters.addParameter(CMC_AE_TARGET_MIN_FCM, this->fAeTargetMin_fcm);

        //Sensor Max Gain
        parameters.addParameter(CMC_SENSOR_MAX_GAIN_FCM, this->fSensorMaxGain_fcm);
        #endif //INFOTM_ENABLE_COLOR_MODE_CHANGE

        #if defined(INFOTM_ENABLE_COLOR_MODE_CHANGE) && defined(INFOTM_ENABLE_GAIN_LEVEL_IDX)
        parameters.addParameter(CMC_DN_TARGET_IDX_FCM_LV1, (int)this->ui32DnTargetIdx_fcm_lv1);

        values.clear();
        for (i = 0 ; i < 4 ; i++) {
            values.push_back(toString(this->ui32BlcSensorBlack_fcm_lv1[i]));
        }
        parameters.addParameter(CMC_BLC_SENSOR_BLACK_FCM_LV1, values);
        parameters.addParameter(CMC_BLC_SYS_BLACK_FCM_LV1, this->ui32SystemBlack_fcm_lv1);

        parameters.addParameter(CMC_DNS_STRENGTH_FCM_LV1, this->fDnsStrength_fcm_lv1);
        parameters.addParameter(CMC_DNS_READ_NOISE_FCM_LV1, this->fDnsReadNoise_fcm_lv1);
        parameters.addParameter(CMC_DNS_WELLDEPTH_FCM_LV1, this->iDnsWellDepth_fcm_lv1);

        parameters.addParameter(CMC_SHA_RADIUS_FCM_LV1, this->fRadius_fcm_lv1);
        parameters.addParameter(CMC_SHA_STRENGTH_FCM_LV1, this->fStrength_fcm_lv1);
        parameters.addParameter(CMC_SHA_THRESH_FCM_LV1, this->fThreshold_fcm_lv1);
        parameters.addParameter(CMC_SHA_DETAIL_FCM_LV1, this->fDetail_fcm_lv1);
        parameters.addParameter(CMC_SHA_EDGE_SCALE_FCM_LV1, this->fEdgeScale_fcm_lv1);
        parameters.addParameter(CMC_SHA_EDGE_OFFSET_FCM_LV1, this->fEdgeOffset_fcm_lv1);
        parameters.addParameter(CMC_SHA_DENOISE_BYPASS_FCM_LV1, this->bBypassDenoise_fcm_lv1);
        parameters.addParameter(CMC_SHADN_TAU_FCM_LV1, this->fDenoiseTau_fcm_lv1);
        parameters.addParameter(CMC_SHADN_SIGMA_FCM_LV1, this->fDenoiseSigma_fcm_lv1);

        parameters.addParameter(CMC_R2Y_CONTRAST_FCM_LV1, this->fContrast_fcm_lv1);
        parameters.addParameter(CMC_R2Y_SATURATION_FCM_LV1, this->fSaturation_fcm_lv1);

        parameters.addParameter(CMC_R2Y_BRIGHTNESS_FCM_LV1, this->fBrightness_fcm_lv1);
        values.clear();
        for (i = 0 ; i < 3 ; i++)
            values.push_back(toString(this->aRangeMult_fcm_lv1[i]));
        parameters.addParameter(CMC_R2Y_RANGE_MUL_FCM_LV1, values);

        values.clear();
        for (i = 0 ; i < 2 ; i++) {
            values.push_back(toString(this->iGridStartCoords_fcm_lv1[i]));
        }
        parameters.addParameter(CMC_HIS_GRID_START_COORDS_FCM_LV1, values);
        values.clear();
        for (i = 0 ; i < 2 ; i++) {
            values.push_back(toString(this->iGridTileDimensions_fcm_lv1[i]));
        }
        parameters.addParameter(CMC_HIS_GRID_TILE_DIMENSIONS_FCM_LV1, values);

        parameters.addParameter(CMC_DPF_WEIGHT_FCM_LV1, this->fDpfWeight_fcm_lv1);
        parameters.addParameter(CMC_DPF_THRESHOLD_FCM_LV1, this->iDpfThreshold_fcm_lv1);

        parameters.addParameter(CMC_TNMC_HISTMIN_FCM_LV1, this->fHisMin_fcm_lv1);
        parameters.addParameter(CMC_TNMC_HISTMAX_FCM_LV1, this->fHisMax_fcm_lv1);
        parameters.addParameter(CMC_TNMC_ADAPTIVE_FCM_LV1, this->bTnmcAdaptive_fcm_lv1);

        values.clear();
        for (i = 0 ; i < 2 ; i++) {
            values.push_back(toString(iInY_fcm_lv1[i]));
        }
        parameters.addParameter(CMC_TNM_IN_Y_FCM_LV1, values);
        values.clear();
        for (i = 0 ; i < 2 ; i++) {
            values.push_back(toString(iOutY_fcm_lv1[i]));
        }
        parameters.addParameter(CMC_TNM_OUT_Y_FCM_LV1, values);
        values.clear();
        for (i = 0 ; i < 2 ; i++) {
            values.push_back(toString(iOutC_fcm_lv1[i]));
        }
        parameters.addParameter(CMC_TNM_OUT_C_FCM_LV1, values);
        parameters.addParameter(CMC_TNM_FLAT_FACTOR_FCM_LV1, this->fFlatFactor_fcm_lv1);
        parameters.addParameter(CMC_TNM_WEIGHT_LINE_FCM_LV1, this->fWeightLine_fcm_lv1);
        parameters.addParameter(CMC_TNM_COLOUR_CONFIDENCE_FCM_LV1, this->fColourConfidence_fcm_lv1);
        parameters.addParameter(CMC_TNM_COLOUR_SATURATION_FCM_LV1, this->fColourSaturation_fcm_lv1);
        parameters.addParameter(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_FCM_LV1, this->fEqualBrightSupressRatio_fcm_lv1);
        parameters.addParameter(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_FCM_LV1, this->fEqualDarkSupressRatio_fcm_lv1);
        parameters.addParameter(CMC_TNMC_OVERSHOT_THRESHOLD_FCM_LV1, fOvershotThreshold_fcm_lv1);
        values.clear();
        for (i = 0 ; i < TNMC_WDR_SEGMENT_CNT ; i++) {
            values.push_back(toString(fWdrCeiling_fcm_lv1[i]));
        }
        parameters.addParameter(CMC_TNMC_WDR_CEILING_FCM_LV1, values);
        values.clear();
        for (i = 0 ; i < TNMC_WDR_SEGMENT_CNT ; i++) {
            values.push_back(toString(fWdrFloor_fcm_lv1[i]));
        }
        parameters.addParameter(CMC_TNMC_WDR_FLOOR_FCM_LV1, values);
        parameters.addParameter(CMC_TNMC_GAMMA_CRV_MODE_FCM_LV1, this->ui32GammaCrvMode_fcm_lv1);
        parameters.addParameter(CMC_TNMC_GAMMA_FCM_LV1, this->fTnmGamma_fcm_lv1);
        parameters.addParameter(CMC_TNMC_BEZIER_CTRL_PNT_FCM_LV1, this->fBezierCtrlPnt_fcm_lv1);

        parameters.addParameter(CMC_DN_TARGET_IDX_FCM_LV2, (int)this->ui32DnTargetIdx_fcm_lv2);

        values.clear();
        for (i = 0 ; i < 4 ; i++) {
            values.push_back(toString(this->ui32BlcSensorBlack_fcm_lv2[i]));
        }
        parameters.addParameter(CMC_BLC_SENSOR_BLACK_FCM_LV2, values);
        parameters.addParameter(CMC_BLC_SYS_BLACK_FCM_LV2, this->ui32SystemBlack_fcm_lv2);

        parameters.addParameter(CMC_DNS_STRENGTH_FCM_LV2, this->fDnsStrength_fcm_lv2);
        parameters.addParameter(CMC_DNS_READ_NOISE_FCM_LV2, this->fDnsReadNoise_fcm_lv2);
        parameters.addParameter(CMC_DNS_WELLDEPTH_FCM_LV2, this->iDnsWellDepth_fcm_lv2);

        parameters.addParameter(CMC_SHA_RADIUS_FCM_LV2, this->fRadius_fcm_lv2);
        parameters.addParameter(CMC_SHA_STRENGTH_FCM_LV2, this->fStrength_fcm_lv2);
        parameters.addParameter(CMC_SHA_THRESH_FCM_LV2, this->fThreshold_fcm_lv2);
        parameters.addParameter(CMC_SHA_DETAIL_FCM_LV2, this->fDetail_fcm_lv2);
        parameters.addParameter(CMC_SHA_EDGE_SCALE_FCM_LV2, this->fEdgeScale_fcm_lv2);
        parameters.addParameter(CMC_SHA_EDGE_OFFSET_FCM_LV2, this->fEdgeOffset_fcm_lv2);
        parameters.addParameter(CMC_SHA_DENOISE_BYPASS_FCM_LV2, this->bBypassDenoise_fcm_lv2);
        parameters.addParameter(CMC_SHADN_TAU_FCM_LV2, this->fDenoiseTau_fcm_lv2);
        parameters.addParameter(CMC_SHADN_SIGMA_FCM_LV2, this->fDenoiseSigma_fcm_lv2);

        parameters.addParameter(CMC_R2Y_CONTRAST_FCM_LV2, this->fContrast_fcm_lv2);
        parameters.addParameter(CMC_R2Y_SATURATION_FCM_LV2, this->fSaturation_fcm_lv2);
        parameters.addParameter(CMC_R2Y_BRIGHTNESS_FCM_LV2, this->fBrightness_fcm_lv2);
        values.clear();
        for (i = 0 ; i < 3 ; i++)
            values.push_back(toString(this->aRangeMult_fcm_lv2[i]));
        parameters.addParameter(CMC_R2Y_RANGE_MUL_FCM_LV2, values);

        values.clear();
        for (i = 0 ; i < 2 ; i++) {
            values.push_back(toString(this->iGridStartCoords_fcm_lv2[i]));
        }
        parameters.addParameter(CMC_HIS_GRID_START_COORDS_FCM_LV2, values);
        values.clear();
        for (i = 0 ; i < 2 ; i++) {
            values.push_back(toString(this->iGridTileDimensions_fcm_lv2[i]));
        }
        parameters.addParameter(CMC_HIS_GRID_TILE_DIMENSIONS_FCM_LV2, values);

        parameters.addParameter(CMC_DPF_WEIGHT_FCM_LV2, this->fDpfWeight_fcm_lv2);
        parameters.addParameter(CMC_DPF_THRESHOLD_FCM_LV2, this->iDpfThreshold_fcm_lv2);

        parameters.addParameter(CMC_TNMC_HISTMIN_FCM_LV2, this->fHisMin_fcm_lv2);
        parameters.addParameter(CMC_TNMC_HISTMAX_FCM_LV2, this->fHisMax_fcm_lv2);
        parameters.addParameter(CMC_TNMC_ADAPTIVE_FCM_LV2, this->bTnmcAdaptive_fcm_lv2);

        values.clear();
        for (i = 0 ; i < 2 ; i++) {
            values.push_back(toString(iInY_fcm_lv2[i]));
        }
        parameters.addParameter(CMC_TNM_IN_Y_FCM_LV2, values);
        values.clear();
        for (i = 0 ; i < 2 ; i++) {
            values.push_back(toString(iOutY_fcm_lv2[i]));
        }
        parameters.addParameter(CMC_TNM_OUT_Y_FCM_LV2, values);
        values.clear();
        for (i = 0 ; i < 2 ; i++) {
            values.push_back(toString(iOutC_fcm_lv2[i]));
        }
        parameters.addParameter(CMC_TNM_OUT_C_FCM_LV2, values);
        parameters.addParameter(CMC_TNM_FLAT_FACTOR_FCM_LV2, this->fFlatFactor_fcm_lv2);
        parameters.addParameter(CMC_TNM_WEIGHT_LINE_FCM_LV2, this->fWeightLine_fcm_lv2);
        parameters.addParameter(CMC_TNM_COLOUR_CONFIDENCE_FCM_LV2, this->fColourConfidence_fcm_lv2);
        parameters.addParameter(CMC_TNM_COLOUR_SATURATION_FCM_LV2, this->fColourSaturation_fcm_lv2);
        parameters.addParameter(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_FCM_LV2, this->fEqualBrightSupressRatio_fcm_lv2);
        parameters.addParameter(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_FCM_LV2, this->fEqualDarkSupressRatio_fcm_lv2);
        parameters.addParameter(CMC_TNMC_OVERSHOT_THRESHOLD_FCM_LV2, fOvershotThreshold_fcm_lv2);
        values.clear();
        for (i = 0 ; i < TNMC_WDR_SEGMENT_CNT ; i++) {
            values.push_back(toString(fWdrCeiling_fcm_lv2[i]));
        }
        parameters.addParameter(CMC_TNMC_WDR_CEILING_FCM_LV2, values);
        values.clear();
        for (i = 0 ; i < TNMC_WDR_SEGMENT_CNT ; i++) {
            values.push_back(toString(fWdrFloor_fcm_lv2[i]));
        }
        parameters.addParameter(CMC_TNMC_WDR_FLOOR_FCM_LV2, values);
        parameters.addParameter(CMC_TNMC_GAMMA_CRV_MODE_FCM_LV2, this->ui32GammaCrvMode_fcm_lv2);
        parameters.addParameter(CMC_TNMC_GAMMA_FCM_LV2, this->fTnmGamma_fcm_lv2);
        parameters.addParameter(CMC_TNMC_BEZIER_CTRL_PNT_FCM_LV2, this->fBezierCtrlPnt_fcm_lv2);

        if (bEnableInterpolationGamma) {
            parameters.addParameter(CMC_DN_TARGET_IDX_FCM_LV3, (int)this->ui32DnTargetIdx_fcm_lv3);

            values.clear();
            for (i = 0 ; i < 4 ; i++) {
                values.push_back(toString(this->ui32BlcSensorBlack_fcm_lv3[i]));
            }
            parameters.addParameter(CMC_BLC_SENSOR_BLACK_FCM_LV3, values);

            parameters.addParameter(CMC_DNS_STRENGTH_FCM_LV3, this->fDnsStrength_fcm_lv3);
            parameters.addParameter(CMC_DNS_READ_NOISE_FCM_LV3, this->fDnsReadNoise_fcm_lv3);
            parameters.addParameter(CMC_DNS_WELLDEPTH_FCM_LV3, this->iDnsWellDepth_fcm_lv3);

            parameters.addParameter(CMC_SHA_RADIUS_FCM_LV3, this->fRadius_fcm_lv3);
            parameters.addParameter(CMC_SHA_STRENGTH_FCM_LV3, this->fStrength_fcm_lv3);
            parameters.addParameter(CMC_SHA_THRESH_FCM_LV3, this->fThreshold_fcm_lv3);
            parameters.addParameter(CMC_SHA_DETAIL_FCM_LV3, this->fDetail_fcm_lv3);
            parameters.addParameter(CMC_SHA_EDGE_SCALE_FCM_LV3, this->fEdgeScale_fcm_lv3);
            parameters.addParameter(CMC_SHA_EDGE_OFFSET_FCM_LV3, this->fEdgeOffset_fcm_lv3);
            parameters.addParameter(CMC_SHA_DENOISE_BYPASS_FCM_LV3, this->bBypassDenoise_fcm_lv3);
            parameters.addParameter(CMC_SHADN_TAU_FCM_LV3, this->fDenoiseTau_fcm_lv3);
            parameters.addParameter(CMC_SHADN_SIGMA_FCM_LV3, this->fDenoiseSigma_fcm_lv3);

            parameters.addParameter(CMC_R2Y_CONTRAST_FCM_LV3, this->fContrast_fcm_lv3);
            parameters.addParameter(CMC_R2Y_SATURATION_FCM_LV3, this->fSaturation_fcm_lv3);

            parameters.addParameter(CMC_R2Y_BRIGHTNESS_FCM_LV3, this->fBrightness_fcm_lv3);
            values.clear();
            for (i = 0 ; i < 3 ; i++)
                values.push_back(toString(this->aRangeMult_fcm_lv3[i]));
            parameters.addParameter(CMC_R2Y_RANGE_MUL_FCM_LV3, values);

            values.clear();
            for (i = 0 ; i < 2 ; i++) {
                values.push_back(toString(this->iGridStartCoords_fcm_lv3[i]));
            }
            parameters.addParameter(CMC_HIS_GRID_START_COORDS_FCM_LV3, values);
            values.clear();
            for (i = 0 ; i < 2 ; i++) {
                values.push_back(toString(this->iGridTileDimensions_fcm_lv3[i]));
            }
            parameters.addParameter(CMC_HIS_GRID_TILE_DIMENSIONS_FCM_LV3, values);

            parameters.addParameter(CMC_DPF_WEIGHT_FCM_LV3, this->fDpfWeight_fcm_lv3);
            parameters.addParameter(CMC_DPF_THRESHOLD_FCM_LV3, this->iDpfThreshold_fcm_lv3);

            parameters.addParameter(CMC_TNMC_HISTMIN_FCM_LV3, this->fHisMin_fcm_lv3);
            parameters.addParameter(CMC_TNMC_HISTMAX_FCM_LV3, this->fHisMax_fcm_lv3);
            parameters.addParameter(CMC_TNMC_ADAPTIVE_FCM_LV3, this->bTnmcAdaptive_fcm_lv3);

            values.clear();
            for (i = 0 ; i < 2 ; i++) {
                values.push_back(toString(iInY_fcm_lv3[i]));
            }
            parameters.addParameter(CMC_TNM_IN_Y_FCM_LV3, values);
            values.clear();
            for (i = 0 ; i < 2 ; i++) {
                values.push_back(toString(iOutY_fcm_lv3[i]));
            }
            parameters.addParameter(CMC_TNM_OUT_Y_FCM_LV3, values);
            values.clear();
            for (i = 0 ; i < 2 ; i++) {
                values.push_back(toString(iOutC_fcm_lv3[i]));
            }
            parameters.addParameter(CMC_TNM_OUT_C_FCM_LV3, values);
            parameters.addParameter(CMC_TNM_FLAT_FACTOR_FCM_LV3, this->fFlatFactor_fcm_lv3);
            parameters.addParameter(CMC_TNM_WEIGHT_LINE_FCM_LV3, this->fWeightLine_fcm_lv3);
            parameters.addParameter(CMC_TNM_COLOUR_CONFIDENCE_FCM_LV3, this->fColourConfidence_fcm_lv3);
            parameters.addParameter(CMC_TNM_COLOUR_SATURATION_FCM_LV3, this->fColourSaturation_fcm_lv3);
            parameters.addParameter(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_FCM_LV3, this->fEqualBrightSupressRatio_fcm_lv3);
            parameters.addParameter(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_FCM_LV3, this->fEqualDarkSupressRatio_fcm_lv3);
            parameters.addParameter(CMC_TNMC_OVERSHOT_THRESHOLD_FCM_LV3, fOvershotThreshold_fcm_lv3);
            values.clear();
            for (i = 0 ; i < TNMC_WDR_SEGMENT_CNT ; i++) {
                values.push_back(toString(fWdrCeiling_fcm_lv3[i]));
            }
            parameters.addParameter(CMC_TNMC_WDR_CEILING_FCM_LV3, values);
            values.clear();
            for (i = 0 ; i < TNMC_WDR_SEGMENT_CNT ; i++) {
                values.push_back(toString(fWdrFloor_fcm_lv3[i]));
            }
            parameters.addParameter(CMC_TNMC_WDR_FLOOR_FCM_LV3, values);
            parameters.addParameter(CMC_TNMC_GAMMA_CRV_MODE_FCM_LV3, this->ui32GammaCrvMode_fcm_lv3);
            parameters.addParameter(CMC_TNMC_GAMMA_FCM_LV3, this->fTnmGamma_fcm_lv3);
            parameters.addParameter(CMC_TNMC_BEZIER_CTRL_PNT_FCM_LV3, this->fBezierCtrlPnt_fcm_lv3);
        }
        #endif //INFOTM_ENABLE_COLOR_MODE_CHANGE, INFOTM_ENABLE_GAIN_LEVEL_IDX

        //3D-Denoise target index.
        parameters.addParameter(CMC_DN_TARGET_IDX_ACM, (int)this->ui32DnTargetIdx_acm);
        ...
        if (bCmcEnable) {
            ...
            parameters.addParameter(CMC_BLC_SENSOR_BLACK_ACM, values);
            parameters.addParameter(CMC_BLC_SYS_BLACK_ACM, this->ui32SystemBlack_acm);

            //DNS
            parameters.addParameter(CMC_DNS_STRENGTH_ACM, this->fDnsStrength_acm);
            parameters.addParameter(CMC_DNS_READ_NOISE_ACM, this->fDnsReadNoise_acm);
            parameters.addParameter(CMC_DNS_WELLDEPTH_ACM, this->iDnsWellDepth_acm);

            //SHA
            parameters.addParameter(CMC_SHA_RADIUS_ACM, this->fRadius_acm);
            parameters.addParameter(CMC_SHA_STRENGTH_ACM, this->fStrength_acm);
            parameters.addParameter(CMC_SHA_THRESH_ACM, this->fThreshold_acm);
            parameters.addParameter(CMC_SHA_DETAIL_ACM, this->fDetail_acm);
            parameters.addParameter(CMC_SHA_EDGE_SCALE_ACM, this->fEdgeScale_acm);
            parameters.addParameter(CMC_SHA_EDGE_OFFSET_ACM, this->fEdgeOffset_acm);
            parameters.addParameter(CMC_SHA_DENOISE_BYPASS_ACM, this->bBypassDenoise_acm);
            parameters.addParameter(CMC_SHADN_TAU_ACM, this->fDenoiseTau_acm);
            parameters.addParameter(CMC_SHADN_SIGMA_ACM, this->fDenoiseSigma_acm);

            //R2Y
            parameters.addParameter(CMC_R2Y_BRIGHTNESS_ACM, this->fBrightness_acm);
            parameters.addParameter(CMC_R2Y_CONTRAST_ACM, this->fContrast_acm);
            values.clear();
            for (i = 0 ; i < 3 ; i++)
                values.push_back(toString(this->aRangeMult_acm[i]));
            parameters.addParameter(CMC_R2Y_RANGE_MUL_ACM, values);
            parameters.addParameter(CMC_R2Y_SATURATION_ACM, this->fSaturation_acm);
            ...
            parameters.addParameter(CMC_HIS_GRID_START_COORDS_ACM, values);
            values.clear();
            for (i = 0 ; i < 2 ; i++) {
                values.push_back(toString(this->iGridTileDimensions_acm[i]));
            }
            parameters.addParameter(CMC_HIS_GRID_TILE_DIMENSIONS_ACM, values);

            //DPF
            parameters.addParameter(CMC_DPF_WEIGHT_ACM, this->fDpfWeight_acm);
            parameters.addParameter(CMC_DPF_THRESHOLD_ACM, this->iDpfThreshold_acm);

            //TNM
            parameters.addParameter(CMC_TNMC_HISTMIN_ACM, this->fHisMin_acm);
            parameters.addParameter(CMC_TNMC_HISTMAX_ACM, this->fHisMax_acm);
            parameters.addParameter(CMC_TNMC_ADAPTIVE_ACM, this->bTnmcAdaptive_acm);

            values.clear();
            for (i = 0 ; i < 2 ; i++) {
                values.push_back(toString(iInY_acm[i]));
            }
            parameters.addParameter(CMC_TNM_IN_Y_ACM, values);
            values.clear();
            for (i = 0 ; i < 2 ; i++) {
                values.push_back(toString(iOutY_acm[i]));
            }
            parameters.addParameter(CMC_TNM_OUT_Y_ACM, values);
            values.clear();
            for (i = 0 ; i < 2 ; i++) {
                values.push_back(toString(iOutC_acm[i]));
            }
            parameters.addParameter(CMC_TNM_OUT_C_ACM, values);
            parameters.addParameter(CMC_TNM_FLAT_FACTOR_ACM, this->fFlatFactor_acm);
            parameters.addParameter(CMC_TNM_WEIGHT_LINE_ACM, this->fWeightLine_acm);
            parameters.addParameter(CMC_TNM_COLOUR_CONFIDENCE_ACM, this->fColourConfidence_acm);
            parameters.addParameter(CMC_TNM_COLOUR_SATURATION_ACM, this->fColourSaturation_acm);
            parameters.addParameter(CMC_TNMC_ENABLE_GAMMA_ACM, this->bEnableGamma_acm);
            parameters.addParameter(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_ACM, this->fEqualBrightSupressRatio_acm);
            parameters.addParameter(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_ACM, this->fEqualDarkSupressRatio_acm);
            parameters.addParameter(CMC_TNMC_OVERSHOT_THRESHOLD_ACM, fOvershotThreshold_acm);
            values.clear();
            for (i = 0 ; i < TNMC_WDR_SEGMENT_CNT ; i++) {
                values.push_back(toString(fWdrCeiling_acm[i]));
            }
            parameters.addParameter(CMC_TNMC_WDR_CEILING_ACM, values);
            values.clear();
            for (i = 0 ; i < TNMC_WDR_SEGMENT_CNT ; i++) {
                values.push_back(toString(fWdrFloor_acm[i]));
            }
            parameters.addParameter(CMC_TNMC_WDR_FLOOR_ACM, values);
            parameters.addParameter(CMC_TNMC_GAMMA_CRV_MODE_ACM, this->ui32GammaCrvMode_acm);
            parameters.addParameter(CMC_TNMC_GAMMA_ACM, this->fTnmGamma_acm);
            parameters.addParameter(CMC_TNMC_BEZIER_CTRL_PNT_ACM, this->fBezierCtrlPnt_acm);

            //AE
            parameters.addParameter(CMC_AE_TARGET_BRIGHTNESS_ACM, this->targetBrightness_acm);
            parameters.addParameter(CMC_AE_TARGET_MIN_ACM, this->fAeTargetMin_acm);

            //Sensor Max Gain
            parameters.addParameter(CMC_SENSOR_MAX_GAIN_ACM, this->fSensorMaxGain_acm);
        }
        #endif //INFOTM_ENABLE_COLOR_MODE_CHANGE

        #if defined(INFOTM_ENABLE_GAIN_LEVEL_IDX)
        parameters.addParameter(CMC_SENSOR_GAIN_LEVEL_CTRL, this->blSensorGainLevelCtrl);
        parameters.addParameter(CMC_SENSOR_GAIN_LV1, this->fSensorGain_lv1);
        parameters.addParameter(CMC_SENSOR_GAIN_LV2, this->fSensorGain_lv2);
        parameters.addParameter(CMC_SENSOR_GAIN_LV1_INTERPOLATION, this->fSensorGainLv1Interpolation);
        parameters.addParameter(CMC_SENSOR_GAIN_LV2_INTERPOLATION, this->fSensorGainLv2Interpolation);
        parameters.addParameter(CMC_ENABLE_INTERPOLATION_GAMMA, this->bEnableInterpolationGamma);
        parameters.addParameter(CMC_INTERPOLATION_GAMMA_0, this->fCmcInterpolationGamma[0]);
        parameters.addParameter(CMC_INTERPOLATION_GAMMA_1, this->fCmcInterpolationGamma[1]);
        parameters.addParameter(CMC_INTERPOLATION_GAMMA_2, this->fCmcInterpolationGamma[2]);

        parameters.addParameter(CMC_CCM_ATTENUATION_LV1, this->dbCcmAttenuation_lv1);
        parameters.addParameter(CMC_CCM_ATTENUATION_LV2, this->dbCcmAttenuation_lv2);
        parameters.addParameter(CMC_CCM_ATTENUATION_LV3, this->dbCcmAttenuation_lv3);

        parameters.addParameter(CMC_DN_TARGET_IDX_ACM_LV1, (int)this->ui32DnTargetIdx_acm_lv1);

        values.clear();
        for (i = 0 ; i < 4 ; i++) {
            values.push_back(toString(this->ui32BlcSensorBlack_acm_lv1[i]));
        }
        parameters.addParameter(CMC_BLC_SENSOR_BLACK_ACM_LV1, values);
        parameters.addParameter(CMC_BLC_SYS_BLACK_ACM_LV1, this->ui32SystemBlack_acm_lv1);

        parameters.addParameter(CMC_DNS_STRENGTH_ACM_LV1, this->fDnsStrength_acm_lv1);
        parameters.addParameter(CMC_DNS_READ_NOISE_ACM_LV1, this->fDnsReadNoise_acm_lv1);
        parameters.addParameter(CMC_DNS_WELLDEPTH_ACM_LV1, this->iDnsWellDepth_acm_lv1);

        parameters.addParameter(CMC_SHA_RADIUS_ACM_LV1, this->fRadius_acm_lv1);
        parameters.addParameter(CMC_SHA_STRENGTH_ACM_LV1, this->fStrength_acm_lv1);
        parameters.addParameter(CMC_SHA_THRESH_ACM_LV1, this->fThreshold_acm_lv1);
        parameters.addParameter(CMC_SHA_DETAIL_ACM_LV1, this->fDetail_acm_lv1);
        parameters.addParameter(CMC_SHA_EDGE_SCALE_ACM_LV1, this->fEdgeScale_acm_lv1);
        parameters.addParameter(CMC_SHA_EDGE_OFFSET_ACM_LV1, this->fEdgeOffset_acm_lv1);
        parameters.addParameter(CMC_SHA_DENOISE_BYPASS_ACM_LV1, this->bBypassDenoise_acm_lv1);
        parameters.addParameter(CMC_SHADN_TAU_ACM_LV1, this->fDenoiseTau_acm_lv1);
        parameters.addParameter(CMC_SHADN_SIGMA_ACM_LV1, this->fDenoiseSigma_acm_lv1);

        parameters.addParameter(CMC_R2Y_CONTRAST_ACM_LV1, this->fContrast_acm_lv1);
        parameters.addParameter(CMC_R2Y_SATURATION_ACM_LV1, this->fSaturation_acm_lv1);

        parameters.addParameter(CMC_R2Y_BRIGHTNESS_ACM_LV1, this->fBrightness_acm_lv1);
        values.clear();
        for (i = 0 ; i < 3 ; i++)
            values.push_back(toString(this->aRangeMult_acm_lv1[i]));
        parameters.addParameter(CMC_R2Y_RANGE_MUL_ACM_LV1, values);

        values.clear();
        for (i = 0 ; i < 2 ; i++) {
            values.push_back(toString(this->iGridStartCoords_acm_lv1[i]));
        }
        parameters.addParameter(CMC_HIS_GRID_START_COORDS_ACM_LV1, values);
        values.clear();
        for (i = 0 ; i < 2 ; i++) {
            values.push_back(toString(this->iGridTileDimensions_acm_lv1[i]));
        }
        parameters.addParameter(CMC_HIS_GRID_TILE_DIMENSIONS_ACM_LV1, values);

        parameters.addParameter(CMC_DPF_WEIGHT_ACM_LV1, this->fDpfWeight_acm_lv1);
        parameters.addParameter(CMC_DPF_THRESHOLD_ACM_LV1, this->iDpfThreshold_acm_lv1);

        parameters.addParameter(CMC_TNMC_HISTMIN_ACM_LV1, this->fHisMin_acm_lv1);
        parameters.addParameter(CMC_TNMC_HISTMAX_ACM_LV1, this->fHisMax_acm_lv1);
        parameters.addParameter(CMC_TNMC_ADAPTIVE_ACM_LV1, this->bTnmcAdaptive_acm_lv1);

        values.clear();
        for (i = 0 ; i < 2 ; i++) {
            values.push_back(toString(iInY_acm_lv1[i]));
        }
        parameters.addParameter(CMC_TNM_IN_Y_ACM_LV1, values);
        values.clear();
        for (i = 0 ; i < 2 ; i++) {
            values.push_back(toString(iOutY_acm_lv1[i]));
        }
        parameters.addParameter(CMC_TNM_OUT_Y_ACM_LV1, values);
        values.clear();
        for (i = 0 ; i < 2 ; i++) {
            values.push_back(toString(iOutC_acm_lv1[i]));
        }
        parameters.addParameter(CMC_TNM_OUT_C_ACM_LV1, values);
        parameters.addParameter(CMC_TNM_FLAT_FACTOR_ACM_LV1, this->fFlatFactor_acm_lv1);
        parameters.addParameter(CMC_TNM_WEIGHT_LINE_ACM_LV1, this->fWeightLine_acm_lv1);
        parameters.addParameter(CMC_TNM_COLOUR_CONFIDENCE_ACM_LV1, this->fColourConfidence_acm_lv1);
        parameters.addParameter(CMC_TNM_COLOUR_SATURATION_ACM_LV1, this->fColourSaturation_acm_lv1);
        parameters.addParameter(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_ACM_LV1, this->fEqualBrightSupressRatio_acm_lv1);
        parameters.addParameter(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_ACM_LV1, this->fEqualDarkSupressRatio_acm_lv1);
        parameters.addParameter(CMC_TNMC_OVERSHOT_THRESHOLD_ACM_LV1, fOvershotThreshold_acm_lv1);
        values.clear();
        for (i = 0 ; i < TNMC_WDR_SEGMENT_CNT ; i++) {
            values.push_back(toString(fWdrCeiling_acm_lv1[i]));
        }
        parameters.addParameter(CMC_TNMC_WDR_CEILING_ACM_LV1, values);
        values.clear();
        for (i = 0 ; i < TNMC_WDR_SEGMENT_CNT ; i++) {
            values.push_back(toString(fWdrFloor_acm_lv1[i]));
        }
        parameters.addParameter(CMC_TNMC_WDR_FLOOR_ACM_LV1, values);
        parameters.addParameter(CMC_TNMC_GAMMA_CRV_MODE_ACM_LV1, this->ui32GammaCrvMode_acm_lv1);
        parameters.addParameter(CMC_TNMC_GAMMA_ACM_LV1, this->fTnmGamma_acm_lv1);
        parameters.addParameter(CMC_TNMC_BEZIER_CTRL_PNT_ACM_LV1, this->fBezierCtrlPnt_acm_lv1);

        parameters.addParameter(CMC_DN_TARGET_IDX_ACM_LV2, (int)this->ui32DnTargetIdx_acm_lv2);

        values.clear();
        for (i = 0 ; i < 4 ; i++) {
            values.push_back(toString(this->ui32BlcSensorBlack_acm_lv2[i]));
        }
        parameters.addParameter(CMC_BLC_SENSOR_BLACK_ACM_LV2, values);
        parameters.addParameter(CMC_BLC_SYS_BLACK_ACM_LV2, this->ui32SystemBlack_acm_lv2);

        parameters.addParameter(CMC_DNS_STRENGTH_ACM_LV2, this->fDnsStrength_acm_lv2);
        parameters.addParameter(CMC_DNS_READ_NOISE_ACM_LV2, this->fDnsReadNoise_acm_lv2);
        parameters.addParameter(CMC_DNS_WELLDEPTH_ACM_LV2, this->iDnsWellDepth_acm_lv2);

        parameters.addParameter(CMC_SHA_RADIUS_ACM_LV2, this->fRadius_acm_lv2);
        parameters.addParameter(CMC_SHA_STRENGTH_ACM_LV2, this->fStrength_acm_lv2);
        parameters.addParameter(CMC_SHA_THRESH_ACM_LV2, this->fThreshold_acm_lv2);
        parameters.addParameter(CMC_SHA_DETAIL_ACM_LV2, this->fDetail_acm_lv2);
        parameters.addParameter(CMC_SHA_EDGE_SCALE_ACM_LV2, this->fEdgeScale_acm_lv2);
        parameters.addParameter(CMC_SHA_EDGE_OFFSET_ACM_LV2, this->fEdgeOffset_acm_lv2);
        parameters.addParameter(CMC_SHA_DENOISE_BYPASS_ACM_LV2, this->bBypassDenoise_acm_lv2);
        parameters.addParameter(CMC_SHADN_TAU_ACM_LV2, this->fDenoiseTau_acm_lv2);
        parameters.addParameter(CMC_SHADN_SIGMA_ACM_LV2, this->fDenoiseSigma_acm_lv2);

        parameters.addParameter(CMC_R2Y_CONTRAST_ACM_LV2, this->fContrast_acm_lv2);
        parameters.addParameter(CMC_R2Y_SATURATION_ACM_LV2, this->fSaturation_acm_lv2);
        parameters.addParameter(CMC_R2Y_BRIGHTNESS_ACM_LV2, this->fBrightness_acm_lv2);
        values.clear();
        for (i = 0 ; i < 3 ; i++)
            values.push_back(toString(this->aRangeMult_acm_lv2[i]));
        parameters.addParameter(CMC_R2Y_RANGE_MUL_ACM_LV2, values);

        values.clear();
        for (i = 0 ; i < 2 ; i++) {
            values.push_back(toString(this->iGridStartCoords_acm_lv2[i]));
        }
        parameters.addParameter(CMC_HIS_GRID_START_COORDS_ACM_LV2, values);
        values.clear();
        for (i = 0 ; i < 2 ; i++) {
            values.push_back(toString(this->iGridTileDimensions_acm_lv2[i]));
        }
        parameters.addParameter(CMC_HIS_GRID_TILE_DIMENSIONS_ACM_LV2, values);

        parameters.addParameter(CMC_DPF_WEIGHT_ACM_LV2, this->fDpfWeight_acm_lv2);
        parameters.addParameter(CMC_DPF_THRESHOLD_ACM_LV2, this->iDpfThreshold_acm_lv2);

        parameters.addParameter(CMC_TNMC_HISTMIN_ACM_LV2, this->fHisMin_acm_lv2);
        parameters.addParameter(CMC_TNMC_HISTMAX_ACM_LV2, this->fHisMax_acm_lv2);
        parameters.addParameter(CMC_TNMC_ADAPTIVE_ACM_LV2, this->bTnmcAdaptive_acm_lv2);

        values.clear();
        for (i = 0 ; i < 2 ; i++) {
            values.push_back(toString(iInY_acm_lv2[i]));
        }
        parameters.addParameter(CMC_TNM_IN_Y_ACM_LV2, values);
        values.clear();
        for (i = 0 ; i < 2 ; i++) {
            values.push_back(toString(iOutY_acm_lv2[i]));
        }
        parameters.addParameter(CMC_TNM_OUT_Y_ACM_LV2, values);
        values.clear();
        for (i = 0 ; i < 2 ; i++) {
            values.push_back(toString(iOutC_acm_lv2[i]));
        }
        parameters.addParameter(CMC_TNM_OUT_C_ACM_LV2, values);
        parameters.addParameter(CMC_TNM_FLAT_FACTOR_ACM_LV2, this->fFlatFactor_acm_lv2);
        parameters.addParameter(CMC_TNM_WEIGHT_LINE_ACM_LV2, this->fWeightLine_acm_lv2);
        parameters.addParameter(CMC_TNM_COLOUR_CONFIDENCE_ACM_LV2, this->fColourConfidence_acm_lv2);
        parameters.addParameter(CMC_TNM_COLOUR_SATURATION_ACM_LV2, this->fColourSaturation_acm_lv2);
        parameters.addParameter(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_ACM_LV2, this->fEqualBrightSupressRatio_acm_lv2);
        parameters.addParameter(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_ACM_LV2, this->fEqualDarkSupressRatio_acm_lv2);
        parameters.addParameter(CMC_TNMC_OVERSHOT_THRESHOLD_ACM_LV2, fOvershotThreshold_acm_lv2);
        values.clear();
        for (i = 0 ; i < TNMC_WDR_SEGMENT_CNT ; i++) {
            values.push_back(toString(fWdrCeiling_acm_lv2[i]));
        }
        parameters.addParameter(CMC_TNMC_WDR_CEILING_ACM_LV2, values);
        values.clear();
        for (i = 0 ; i < TNMC_WDR_SEGMENT_CNT ; i++) {
            values.push_back(toString(fWdrFloor_acm_lv2[i]));
        }
        parameters.addParameter(CMC_TNMC_WDR_FLOOR_ACM_LV2, values);
        parameters.addParameter(CMC_TNMC_GAMMA_CRV_MODE_ACM_LV2, this->ui32GammaCrvMode_acm_lv2);
        parameters.addParameter(CMC_TNMC_GAMMA_ACM_LV2, this->fTnmGamma_acm_lv2);
        parameters.addParameter(CMC_TNMC_BEZIER_CTRL_PNT_ACM_LV2, this->fBezierCtrlPnt_acm_lv2);

        if (bEnableInterpolationGamma) {
            parameters.addParameter(CMC_DN_TARGET_IDX_ACM_LV3, (int)this->ui32DnTargetIdx_acm_lv3);

            values.clear();
            for (i = 0 ; i < 4 ; i++) {
                values.push_back(toString(this->ui32BlcSensorBlack_acm_lv3[i]));
            }
            parameters.addParameter(CMC_BLC_SENSOR_BLACK_ACM_LV3, values);
            parameters.addParameter(CMC_BLC_SYS_BLACK_ACM_LV3, this->ui32SystemBlack_acm_lv3);

            parameters.addParameter(CMC_DNS_STRENGTH_ACM_LV3, this->fDnsStrength_acm_lv3);
            parameters.addParameter(CMC_DNS_READ_NOISE_ACM_LV3, this->fDnsReadNoise_acm_lv3);
            parameters.addParameter(CMC_DNS_WELLDEPTH_ACM_LV3, this->iDnsWellDepth_acm_lv3);

            parameters.addParameter(CMC_SHA_RADIUS_ACM_LV3, this->fRadius_acm_lv3);
            parameters.addParameter(CMC_SHA_STRENGTH_ACM_LV3, this->fStrength_acm_lv3);
            parameters.addParameter(CMC_SHA_THRESH_ACM_LV3, this->fThreshold_acm_lv3);
            parameters.addParameter(CMC_SHA_DETAIL_ACM_LV3, this->fDetail_acm_lv3);
            parameters.addParameter(CMC_SHA_EDGE_SCALE_ACM_LV3, this->fEdgeScale_acm_lv3);
            parameters.addParameter(CMC_SHA_EDGE_OFFSET_ACM_LV3, this->fEdgeOffset_acm_lv3);
            parameters.addParameter(CMC_SHA_DENOISE_BYPASS_ACM_LV3, this->bBypassDenoise_acm_lv3);
            parameters.addParameter(CMC_SHADN_TAU_ACM_LV3, this->fDenoiseTau_acm_lv3);
            parameters.addParameter(CMC_SHADN_SIGMA_ACM_LV3, this->fDenoiseSigma_acm_lv3);

            parameters.addParameter(CMC_R2Y_CONTRAST_ACM_LV3, this->fContrast_acm_lv3);
            parameters.addParameter(CMC_R2Y_SATURATION_ACM_LV3, this->fSaturation_acm_lv3);
            parameters.addParameter(CMC_R2Y_BRIGHTNESS_ACM_LV3, this->fBrightness_acm_lv3);
            values.clear();
            for (i = 0 ; i < 3 ; i++)
                values.push_back(toString(this->aRangeMult_acm_lv3[i]));
            parameters.addParameter(CMC_R2Y_RANGE_MUL_ACM_LV3, values);

            values.clear();
            for (i = 0 ; i < 2 ; i++) {
                values.push_back(toString(this->iGridStartCoords_acm_lv3[i]));
            }
            parameters.addParameter(CMC_HIS_GRID_START_COORDS_ACM_LV3, values);
            values.clear();
            for (i = 0 ; i < 2 ; i++) {
                values.push_back(toString(this->iGridTileDimensions_acm_lv3[i]));
            }
            parameters.addParameter(CMC_HIS_GRID_TILE_DIMENSIONS_ACM_LV3, values);

            parameters.addParameter(CMC_DPF_WEIGHT_ACM_LV3, this->fDpfWeight_acm_lv3);
            parameters.addParameter(CMC_DPF_THRESHOLD_ACM_LV3, this->iDpfThreshold_acm_lv3);

            parameters.addParameter(CMC_TNMC_HISTMIN_ACM_LV3, this->fHisMin_acm_lv3);
            parameters.addParameter(CMC_TNMC_HISTMAX_ACM_LV3, this->fHisMax_acm_lv3);
            parameters.addParameter(CMC_TNMC_ADAPTIVE_ACM_LV3, this->bTnmcAdaptive_acm_lv3);

            values.clear();
            for (i = 0 ; i < 2 ; i++) {
                values.push_back(toString(iInY_acm_lv3[i]));
            }
            parameters.addParameter(CMC_TNM_IN_Y_ACM_LV3, values);
            values.clear();
            for (i = 0 ; i < 2 ; i++) {
                values.push_back(toString(iOutY_acm_lv3[i]));
            }
            parameters.addParameter(CMC_TNM_OUT_Y_ACM_LV3, values);
            values.clear();
            for (i = 0 ; i < 2 ; i++) {
                values.push_back(toString(iOutC_acm_lv3[i]));
            }
            parameters.addParameter(CMC_TNM_OUT_C_ACM_LV3, values);
            parameters.addParameter(CMC_TNM_FLAT_FACTOR_ACM_LV3, this->fFlatFactor_acm_lv3);
            parameters.addParameter(CMC_TNM_WEIGHT_LINE_ACM_LV3, this->fWeightLine_acm_lv3);
            parameters.addParameter(CMC_TNM_COLOUR_CONFIDENCE_ACM_LV3, this->fColourConfidence_acm_lv3);
            parameters.addParameter(CMC_TNM_COLOUR_SATURATION_ACM_LV3, this->fColourSaturation_acm_lv3);
            parameters.addParameter(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_ACM_LV3, this->fEqualBrightSupressRatio_acm_lv3);
            parameters.addParameter(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_ACM_LV3, this->fEqualDarkSupressRatio_acm_lv3);
            parameters.addParameter(CMC_TNMC_OVERSHOT_THRESHOLD_ACM_LV3, fOvershotThreshold_acm_lv3);
            values.clear();
            for (i = 0 ; i < TNMC_WDR_SEGMENT_CNT ; i++) {
                values.push_back(toString(fWdrCeiling_acm_lv3[i]));
            }
            parameters.addParameter(CMC_TNMC_WDR_CEILING_ACM_LV3, values);
            values.clear();
            for (i = 0 ; i < TNMC_WDR_SEGMENT_CNT ; i++) {
                values.push_back(toString(fWdrFloor_acm_lv3[i]));
            }
            parameters.addParameter(CMC_TNMC_WDR_FLOOR_ACM_LV3, values);
            parameters.addParameter(CMC_TNMC_GAMMA_CRV_MODE_ACM_LV3, this->ui32GammaCrvMode_acm_lv3);
            parameters.addParameter(CMC_TNMC_GAMMA_ACM_LV3, this->fTnmGamma_acm_lv3);
            parameters.addParameter(CMC_TNMC_BEZIER_CTRL_PNT_ACM_LV3, this->fBezierCtrlPnt_acm_lv3);
        }
        #endif //INFOTM_ENABLE_GAIN_LEVEL_IDX
        break;

    case SAVE_MIN:
        parameters.addParameterMin(CMC_ENABLE);
        parameters.addParameterMin(CMC_DN_TARGET_IDX_CHANGE_ENABLE);
        parameters.addParameterMin(CMC_ENABLE_AVG_GAIN);

        parameters.addParameterMin(CMC_SHA_STRENGTH_EXPECT);

        parameters.addParameterMin(CMC_NIGHT_MODE_DETECT_ENABLE);
        parameters.addParameterMin(CMC_NIGHT_MODE_DETECT_BRIGHTNESS_ENTER);
        parameters.addParameterMin(CMC_NIGHT_MODE_DETECT_BRIGHTNESS_EXIT);
        parameters.addParameterMin(CMC_NIGHT_MODE_DETECT_GAIN_ENTER);
        parameters.addParameterMin(CMC_NIGHT_MODE_DETECT_GAIN_EXIT);
        parameters.addParameterMin(CMC_NIGHT_MODE_DETECT_WEIGHTING);

        parameters.addParameterMin(CMC_ENABLE_CAPTURE_IQ);
        parameters.addParameterMin(CMC_CAPTURE_CCM_RATIO);
        parameters.addParameterMin(CMC_R2Y_CAPTURE_RANGE_MUL);
        parameters.addParameterMin(CMC_TNM_CAPTURE_IN_Y);
        parameters.addParameterMin(CMC_TNM_CAPTURE_OUT_Y);
        parameters.addParameterMin(CMC_TNM_CAPTURE_OUT_C);

        //3D-Denoise target index.
        parameters.addParameterMin(CMC_DN_TARGET_IDX_FCM);

        #if defined(INFOTM_ENABLE_COLOR_MODE_CHANGE)
        ///////////////////////////////////////Flat Color Mode setting
        //BLC SENSOR BLACK
        parameters.addParameterMin(CMC_BLC_SENSOR_BLACK_FCM);
        parameters.addParameterMin(CMC_BLC_SYS_BLACK_FCM);

        //DNS
        parameters.addParameterMin(CMC_DNS_STRENGTH_FCM);
        parameters.addParameterMin(CMC_DNS_READ_NOISE_FCM);
        parameters.addParameterMin(CMC_DNS_WELLDEPTH_FCM);

        //SHA
        parameters.addParameterMin(CMC_SHA_RADIUS_FCM);
        parameters.addParameterMin(CMC_SHA_STRENGTH_FCM);
        parameters.addParameterMin(CMC_SHA_THRESH_FCM);
        parameters.addParameterMin(CMC_SHA_DETAIL_FCM);
        parameters.addParameterMin(CMC_SHA_EDGE_SCALE_FCM);
        parameters.addParameterMin(CMC_SHA_EDGE_OFFSET_FCM);
        parameters.addParameterMin(CMC_SHA_DENOISE_BYPASS_FCM);
        parameters.addParameterMin(CMC_SHADN_TAU_FCM);
        parameters.addParameterMin(CMC_SHADN_SIGMA_FCM);

        //R2Y
        parameters.addParameterMin(CMC_R2Y_BRIGHTNESS_FCM);
        parameters.addParameterMin(CMC_R2Y_CONTRAST_FCM);
        parameters.addParameterMin(CMC_R2Y_SATURATION_FCM);
        parameters.addParameterMin(CMC_R2Y_RANGE_MUL_FCM);

        //HIS
        parameters.addParameterMin(CMC_HIS_GRID_START_COORDS_FCM);
        parameters.addParameterMin(CMC_HIS_GRID_TILE_DIMENSIONS_FCM);

        //DPF
        parameters.addParameterMin(CMC_DPF_WEIGHT_FCM);
        parameters.addParameterMin(CMC_DPF_THRESHOLD_FCM);

        //TNM
        parameters.addParameterMin(CMC_TNMC_HISTMIN_FCM);
        parameters.addParameterMin(CMC_TNMC_HISTMAX_FCM);
        parameters.addParameterMin(CMC_TNMC_ADAPTIVE_FCM);

        parameters.addParameterMin(CMC_TNM_IN_Y_FCM);
        parameters.addParameterMin(CMC_TNM_OUT_Y_FCM);
        parameters.addParameterMin(CMC_TNM_OUT_C_FCM);
        parameters.addParameterMin(CMC_TNM_FLAT_FACTOR_FCM);
        parameters.addParameterMin(CMC_TNM_WEIGHT_LINE_FCM);
        parameters.addParameterMin(CMC_TNM_COLOUR_CONFIDENCE_FCM);
        parameters.addParameterMin(CMC_TNM_COLOUR_SATURATION_FCM);
        parameters.addParameterMin(CMC_TNMC_ENABLE_GAMMA_FCM);
        parameters.addParameterMin(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_FCM);
        parameters.addParameterMin(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_FCM);
        parameters.addParameterMin(CMC_TNMC_OVERSHOT_THRESHOLD_FCM);
        parameters.addParameterMin(CMC_TNMC_WDR_CEILING_FCM);
        parameters.addParameterMin(CMC_TNMC_WDR_FLOOR_FCM);
        parameters.addParameterMin(CMC_TNMC_GAMMA_CRV_MODE_FCM);
        parameters.addParameterMin(CMC_TNMC_GAMMA_FCM);
        parameters.addParameterMin(CMC_TNMC_BEZIER_CTRL_PNT_FCM);

        //AE
        parameters.addParameterMin(CMC_AE_TARGET_BRIGHTNESS_FCM);
        parameters.addParameterMin(CMC_AE_TARGET_MIN_FCM);

        //Sensor Max Gain
        parameters.addParameterMin(CMC_SENSOR_MAX_GAIN_FCM);
        #endif //INFOTM_ENABLE_COLOR_MODE_CHANGE

        #if defined(INFOTM_ENABLE_COLOR_MODE_CHANGE) && defined(INFOTM_ENABLE_GAIN_LEVEL_IDX)
        parameters.addParameterMin(CMC_DN_TARGET_IDX_FCM_LV1);

        parameters.addParameterMin(CMC_BLC_SENSOR_BLACK_FCM_LV1);
        parameters.addParameterMin(CMC_BLC_SYS_BLACK_FCM_LV1);

        parameters.addParameterMin(CMC_DNS_STRENGTH_FCM_LV1);
        parameters.addParameterMin(CMC_DNS_READ_NOISE_FCM_LV1);
        parameters.addParameterMin(CMC_DNS_WELLDEPTH_FCM_LV1);

        parameters.addParameterMin(CMC_SHA_RADIUS_FCM_LV1);
        parameters.addParameterMin(CMC_SHA_STRENGTH_FCM_LV1);
        parameters.addParameterMin(CMC_SHA_THRESH_FCM_LV1);
        parameters.addParameterMin(CMC_SHA_DETAIL_FCM_LV1);
        parameters.addParameterMin(CMC_SHA_EDGE_SCALE_FCM_LV1);
        parameters.addParameterMin(CMC_SHA_EDGE_OFFSET_FCM_LV1);
        parameters.addParameterMin(CMC_SHA_DENOISE_BYPASS_FCM_LV1);
        parameters.addParameterMin(CMC_SHADN_TAU_FCM_LV1);
        parameters.addParameterMin(CMC_SHADN_SIGMA_FCM_LV1);

        parameters.addParameterMin(CMC_R2Y_CONTRAST_FCM_LV1);
        parameters.addParameterMin(CMC_R2Y_SATURATION_FCM_LV1);
        parameters.addParameterMin(CMC_R2Y_BRIGHTNESS_FCM_LV1);
        parameters.addParameterMin(CMC_R2Y_RANGE_MUL_FCM_LV1);

        parameters.addParameterMin(CMC_HIS_GRID_START_COORDS_FCM_LV1);
        parameters.addParameterMin(CMC_HIS_GRID_TILE_DIMENSIONS_FCM_LV1);

        parameters.addParameterMin(CMC_DPF_WEIGHT_FCM_LV1);
        parameters.addParameterMin(CMC_DPF_THRESHOLD_FCM_LV1);

        parameters.addParameterMin(CMC_TNMC_HISTMIN_FCM_LV1);
        parameters.addParameterMin(CMC_TNMC_HISTMAX_FCM_LV1);
        parameters.addParameterMin(CMC_TNMC_ADAPTIVE_FCM_LV1);

        parameters.addParameterMin(CMC_TNM_IN_Y_FCM_LV1);
        parameters.addParameterMin(CMC_TNM_OUT_Y_FCM_LV1);
        parameters.addParameterMin(CMC_TNM_OUT_C_FCM_LV1);
        parameters.addParameterMin(CMC_TNM_FLAT_FACTOR_FCM_LV1);
        parameters.addParameterMin(CMC_TNM_WEIGHT_LINE_FCM_LV1);
        parameters.addParameterMin(CMC_TNM_COLOUR_CONFIDENCE_FCM_LV1);
        parameters.addParameterMin(CMC_TNM_COLOUR_SATURATION_FCM_LV1);
        parameters.addParameterMin(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_FCM_LV1);
        parameters.addParameterMin(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_FCM_LV1);
        parameters.addParameterMin(CMC_TNMC_OVERSHOT_THRESHOLD_FCM_LV1);
        parameters.addParameterMin(CMC_TNMC_WDR_CEILING_FCM_LV1);
        parameters.addParameterMin(CMC_TNMC_WDR_FLOOR_FCM_LV1);
        parameters.addParameterMin(CMC_TNMC_GAMMA_CRV_MODE_FCM_LV1);
        parameters.addParameterMin(CMC_TNMC_GAMMA_FCM_LV1);
        parameters.addParameterMin(CMC_TNMC_BEZIER_CTRL_PNT_FCM_LV1);

        parameters.addParameterMin(CMC_DN_TARGET_IDX_FCM_LV2);

        parameters.addParameterMin(CMC_BLC_SENSOR_BLACK_FCM_LV2);
        parameters.addParameterMin(CMC_BLC_SYS_BLACK_FCM_LV2);

        parameters.addParameterMin(CMC_DNS_STRENGTH_FCM_LV2);
        parameters.addParameterMin(CMC_DNS_READ_NOISE_FCM_LV2);
        parameters.addParameterMin(CMC_DNS_WELLDEPTH_FCM_LV2);

        parameters.addParameterMin(CMC_SHA_RADIUS_FCM_LV2);
        parameters.addParameterMin(CMC_SHA_STRENGTH_FCM_LV2);
        parameters.addParameterMin(CMC_SHA_THRESH_FCM_LV2);
        parameters.addParameterMin(CMC_SHA_DETAIL_FCM_LV2);
        parameters.addParameterMin(CMC_SHA_EDGE_SCALE_FCM_LV2);
        parameters.addParameterMin(CMC_SHA_EDGE_OFFSET_FCM_LV2);
        parameters.addParameterMin(CMC_SHA_DENOISE_BYPASS_FCM_LV2);
        parameters.addParameterMin(CMC_SHADN_TAU_FCM_LV2);
        parameters.addParameterMin(CMC_SHADN_SIGMA_FCM_LV2);

        parameters.addParameterMin(CMC_R2Y_CONTRAST_FCM_LV2);
        parameters.addParameterMin(CMC_R2Y_SATURATION_FCM_LV2);
        parameters.addParameterMin(CMC_R2Y_BRIGHTNESS_FCM_LV2);
        parameters.addParameterMin(CMC_R2Y_RANGE_MUL_FCM_LV2);

        parameters.addParameterMin(CMC_HIS_GRID_START_COORDS_FCM_LV2);
        parameters.addParameterMin(CMC_HIS_GRID_TILE_DIMENSIONS_FCM_LV2);

        parameters.addParameterMin(CMC_DPF_WEIGHT_FCM_LV2);
        parameters.addParameterMin(CMC_DPF_THRESHOLD_FCM_LV2);

        parameters.addParameterMin(CMC_TNMC_HISTMIN_FCM_LV2);
        parameters.addParameterMin(CMC_TNMC_HISTMAX_FCM_LV2);
        parameters.addParameterMin(CMC_TNMC_ADAPTIVE_FCM_LV2);

        parameters.addParameterMin(CMC_TNM_IN_Y_FCM_LV2);
        parameters.addParameterMin(CMC_TNM_OUT_Y_FCM_LV2);
        parameters.addParameterMin(CMC_TNM_OUT_C_FCM_LV2);
        parameters.addParameterMin(CMC_TNM_FLAT_FACTOR_FCM_LV2);
        parameters.addParameterMin(CMC_TNM_WEIGHT_LINE_FCM_LV2);
        parameters.addParameterMin(CMC_TNM_COLOUR_CONFIDENCE_FCM_LV2);
        parameters.addParameterMin(CMC_TNM_COLOUR_SATURATION_FCM_LV2);
        parameters.addParameterMin(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_FCM_LV2);
        parameters.addParameterMin(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_FCM_LV2);
        parameters.addParameterMin(CMC_TNMC_OVERSHOT_THRESHOLD_FCM_LV2);
        parameters.addParameterMin(CMC_TNMC_WDR_CEILING_FCM_LV2);
        parameters.addParameterMin(CMC_TNMC_WDR_FLOOR_FCM_LV2);
        parameters.addParameterMin(CMC_TNMC_GAMMA_CRV_MODE_FCM_LV2);
        parameters.addParameterMin(CMC_TNMC_GAMMA_FCM_LV2);
        parameters.addParameterMin(CMC_TNMC_BEZIER_CTRL_PNT_FCM_LV2);

        if (bEnableInterpolationGamma) {
            parameters.addParameterMin(CMC_DN_TARGET_IDX_FCM_LV3);

            parameters.addParameterMin(CMC_BLC_SENSOR_BLACK_FCM_LV3);
            parameters.addParameterMin(CMC_BLC_SYS_BLACK_FCM_LV3);

            parameters.addParameterMin(CMC_DNS_STRENGTH_FCM_LV3);
            parameters.addParameterMin(CMC_DNS_READ_NOISE_FCM_LV3);
            parameters.addParameterMin(CMC_DNS_WELLDEPTH_FCM_LV3);

            parameters.addParameterMin(CMC_SHA_RADIUS_FCM_LV3);
            parameters.addParameterMin(CMC_SHA_STRENGTH_FCM_LV3);
            parameters.addParameterMin(CMC_SHA_THRESH_FCM_LV3);
            parameters.addParameterMin(CMC_SHA_DETAIL_FCM_LV3);
            parameters.addParameterMin(CMC_SHA_EDGE_SCALE_FCM_LV3);
            parameters.addParameterMin(CMC_SHA_EDGE_OFFSET_FCM_LV3);
            parameters.addParameterMin(CMC_SHA_DENOISE_BYPASS_FCM_LV3);
            parameters.addParameterMin(CMC_SHADN_TAU_FCM_LV3);
            parameters.addParameterMin(CMC_SHADN_SIGMA_FCM_LV3);

            parameters.addParameterMin(CMC_R2Y_CONTRAST_FCM_LV3);
            parameters.addParameterMin(CMC_R2Y_SATURATION_FCM_LV3);
            parameters.addParameterMin(CMC_R2Y_BRIGHTNESS_FCM_LV3);
            parameters.addParameterMin(CMC_R2Y_RANGE_MUL_FCM_LV3);

            parameters.addParameterMin(CMC_HIS_GRID_START_COORDS_FCM_LV3);
            parameters.addParameterMin(CMC_HIS_GRID_TILE_DIMENSIONS_FCM_LV3);

            parameters.addParameterMin(CMC_DPF_WEIGHT_FCM_LV3);
            parameters.addParameterMin(CMC_DPF_THRESHOLD_FCM_LV3);

            parameters.addParameterMin(CMC_TNMC_HISTMIN_FCM_LV3);
            parameters.addParameterMin(CMC_TNMC_HISTMAX_FCM_LV3);
            parameters.addParameterMin(CMC_TNMC_ADAPTIVE_FCM_LV3);

            parameters.addParameterMin(CMC_TNM_IN_Y_FCM_LV3);
            parameters.addParameterMin(CMC_TNM_OUT_Y_FCM_LV3);
            parameters.addParameterMin(CMC_TNM_OUT_C_FCM_LV3);
            parameters.addParameterMin(CMC_TNM_FLAT_FACTOR_FCM_LV3);
            parameters.addParameterMin(CMC_TNM_WEIGHT_LINE_FCM_LV3);
            parameters.addParameterMin(CMC_TNM_COLOUR_CONFIDENCE_FCM_LV3);
            parameters.addParameterMin(CMC_TNM_COLOUR_SATURATION_FCM_LV3);
            parameters.addParameterMin(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_FCM_LV3);
            parameters.addParameterMin(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_FCM_LV3);
            parameters.addParameterMin(CMC_TNMC_OVERSHOT_THRESHOLD_FCM_LV3);
            parameters.addParameterMin(CMC_TNMC_WDR_CEILING_FCM_LV3);
            parameters.addParameterMin(CMC_TNMC_WDR_FLOOR_FCM_LV3);
            parameters.addParameterMin(CMC_TNMC_GAMMA_CRV_MODE_FCM_LV3);
            parameters.addParameterMin(CMC_TNMC_GAMMA_FCM_LV3);
            parameters.addParameterMin(CMC_TNMC_BEZIER_CTRL_PNT_FCM_LV3);
        }
        #endif //INFOTM_ENABLE_COLOR_MODE_CHANGE, INFOTM_ENABLE_GAIN_LEVEL_IDX

        //3D-Denoise target index.
        parameters.addParameterMin(CMC_DN_TARGET_IDX_ACM);
        #if defined(INFOTM_ENABLE_COLOR_MODE_CHANGE)
        ///////////////////////////////////////Advanced Color Mode setting
        if (bCmcEnable) {
            //BLC SENSOR BLACK
            parameters.addParameterMin(CMC_BLC_SENSOR_BLACK_ACM);
            parameters.addParameterMin(CMC_BLC_SYS_BLACK_ACM);

            //DNS
            parameters.addParameterMin(CMC_DNS_STRENGTH_ACM);
            parameters.addParameterMin(CMC_DNS_READ_NOISE_ACM);
            parameters.addParameterMin(CMC_DNS_WELLDEPTH_ACM);

            //SHA
            parameters.addParameterMin(CMC_SHA_RADIUS_ACM);
            parameters.addParameterMin(CMC_SHA_STRENGTH_ACM);
            parameters.addParameterMin(CMC_SHA_THRESH_ACM);
            parameters.addParameterMin(CMC_SHA_DETAIL_ACM);
            parameters.addParameterMin(CMC_SHA_EDGE_SCALE_ACM);
            parameters.addParameterMin(CMC_SHA_EDGE_OFFSET_ACM);
            parameters.addParameterMin(CMC_SHA_DENOISE_BYPASS_ACM);
            parameters.addParameterMin(CMC_SHADN_TAU_ACM);
            parameters.addParameterMin(CMC_SHADN_SIGMA_ACM);

            //R2Y
            parameters.addParameterMin(CMC_R2Y_BRIGHTNESS_ACM);
            parameters.addParameterMin(CMC_R2Y_CONTRAST_ACM);
            parameters.addParameterMin(CMC_R2Y_SATURATION_ACM);
            parameters.addParameterMin(CMC_R2Y_RANGE_MUL_ACM);

            //HIS
            parameters.addParameterMin(CMC_HIS_GRID_START_COORDS_ACM);
            parameters.addParameterMin(CMC_HIS_GRID_TILE_DIMENSIONS_ACM);

            //DPF
            parameters.addParameterMin(CMC_DPF_WEIGHT_ACM);
            parameters.addParameterMin(CMC_DPF_THRESHOLD_ACM);

            //TNM
            parameters.addParameterMin(CMC_TNMC_HISTMIN_ACM);
            parameters.addParameterMin(CMC_TNMC_HISTMAX_ACM);
            parameters.addParameterMin(CMC_TNMC_ADAPTIVE_ACM);

            parameters.addParameterMin(CMC_TNM_IN_Y_ACM);
            parameters.addParameterMin(CMC_TNM_OUT_Y_ACM);
            parameters.addParameterMin(CMC_TNM_OUT_C_ACM);
            parameters.addParameterMin(CMC_TNM_FLAT_FACTOR_ACM);
            parameters.addParameterMin(CMC_TNM_WEIGHT_LINE_ACM);
            parameters.addParameterMin(CMC_TNM_COLOUR_CONFIDENCE_ACM);
            parameters.addParameterMin(CMC_TNM_COLOUR_SATURATION_ACM);
            parameters.addParameterMin(CMC_TNMC_ENABLE_GAMMA_ACM);
            parameters.addParameterMin(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_ACM);
            parameters.addParameterMin(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_ACM);
            parameters.addParameterMin(CMC_TNMC_OVERSHOT_THRESHOLD_ACM);
            parameters.addParameterMin(CMC_TNMC_WDR_CEILING_ACM);
            parameters.addParameterMin(CMC_TNMC_WDR_FLOOR_ACM);
            parameters.addParameterMin(CMC_TNMC_GAMMA_CRV_MODE_ACM);
            parameters.addParameterMin(CMC_TNMC_GAMMA_ACM);
            parameters.addParameterMin(CMC_TNMC_BEZIER_CTRL_PNT_ACM);

            //AE
            parameters.addParameterMin(CMC_AE_TARGET_BRIGHTNESS_ACM);
            parameters.addParameterMin(CMC_AE_TARGET_MIN_ACM);

            //Sensor Max Gain
            parameters.addParameterMin(CMC_SENSOR_MAX_GAIN_ACM);
        }
        #endif //INFOTM_ENABLE_COLOR_MODE_CHANGE

        #if defined(INFOTM_ENABLE_GAIN_LEVEL_IDX)
        parameters.addParameterMin(CMC_SENSOR_GAIN_LEVEL_CTRL);
        parameters.addParameterMin(CMC_SENSOR_GAIN_LV1);
        parameters.addParameterMin(CMC_SENSOR_GAIN_LV2);
        parameters.addParameterMin(CMC_SENSOR_GAIN_LV1_INTERPOLATION);
        parameters.addParameterMin(CMC_SENSOR_GAIN_LV2_INTERPOLATION);
        parameters.addParameterMin(CMC_ENABLE_INTERPOLATION_GAMMA);
        parameters.addParameterMin(CMC_INTERPOLATION_GAMMA_0);
        parameters.addParameterMin(CMC_INTERPOLATION_GAMMA_1);
        parameters.addParameterMin(CMC_INTERPOLATION_GAMMA_2);

        parameters.addParameterMin(CMC_CCM_ATTENUATION_LV1);
        parameters.addParameterMin(CMC_CCM_ATTENUATION_LV2);
        parameters.addParameterMin(CMC_CCM_ATTENUATION_LV3);

        parameters.addParameterMin(CMC_DN_TARGET_IDX_ACM_LV1);

        parameters.addParameterMin(CMC_BLC_SENSOR_BLACK_ACM_LV1);
        parameters.addParameterMin(CMC_BLC_SYS_BLACK_ACM_LV1);

        parameters.addParameterMin(CMC_DNS_STRENGTH_ACM_LV1);
        parameters.addParameterMin(CMC_DNS_READ_NOISE_ACM_LV1);
        parameters.addParameterMin(CMC_DNS_WELLDEPTH_ACM_LV1);

        parameters.addParameterMin(CMC_SHA_RADIUS_ACM_LV1);
        parameters.addParameterMin(CMC_SHA_STRENGTH_ACM_LV1);
        parameters.addParameterMin(CMC_SHA_THRESH_ACM_LV1);
        parameters.addParameterMin(CMC_SHA_DETAIL_ACM_LV1);
        parameters.addParameterMin(CMC_SHA_EDGE_SCALE_ACM_LV1);
        parameters.addParameterMin(CMC_SHA_EDGE_OFFSET_ACM_LV1);
        parameters.addParameterMin(CMC_SHA_DENOISE_BYPASS_ACM_LV1);
        parameters.addParameterMin(CMC_SHADN_TAU_ACM_LV1);
        parameters.addParameterMin(CMC_SHADN_SIGMA_ACM_LV1);

        parameters.addParameterMin(CMC_R2Y_CONTRAST_ACM_LV1);
        parameters.addParameterMin(CMC_R2Y_SATURATION_ACM_LV1);
        parameters.addParameterMin(CMC_R2Y_BRIGHTNESS_ACM_LV1);
        parameters.addParameterMin(CMC_R2Y_RANGE_MUL_ACM_LV1);

        parameters.addParameterMin(CMC_HIS_GRID_START_COORDS_ACM_LV1);
        parameters.addParameterMin(CMC_HIS_GRID_TILE_DIMENSIONS_ACM_LV1);

        parameters.addParameterMin(CMC_DPF_WEIGHT_ACM_LV1);
        parameters.addParameterMin(CMC_DPF_THRESHOLD_ACM_LV1);

        parameters.addParameterMin(CMC_TNMC_HISTMIN_ACM_LV1);
        parameters.addParameterMin(CMC_TNMC_HISTMAX_ACM_LV1);
        parameters.addParameterMin(CMC_TNMC_ADAPTIVE_ACM_LV1);

        parameters.addParameterMin(CMC_TNM_IN_Y_ACM_LV1);
        parameters.addParameterMin(CMC_TNM_OUT_Y_ACM_LV1);
        parameters.addParameterMin(CMC_TNM_OUT_C_ACM_LV1);
        parameters.addParameterMin(CMC_TNM_FLAT_FACTOR_ACM_LV1);
        parameters.addParameterMin(CMC_TNM_WEIGHT_LINE_ACM_LV1);
        parameters.addParameterMin(CMC_TNM_COLOUR_CONFIDENCE_ACM_LV1);
        parameters.addParameterMin(CMC_TNM_COLOUR_SATURATION_ACM_LV1);
        parameters.addParameterMin(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_ACM_LV1);
        parameters.addParameterMin(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_ACM_LV1);
        parameters.addParameterMin(CMC_TNMC_OVERSHOT_THRESHOLD_ACM_LV1);
        parameters.addParameterMin(CMC_TNMC_WDR_CEILING_ACM_LV1);
        parameters.addParameterMin(CMC_TNMC_WDR_FLOOR_ACM_LV1);
        parameters.addParameterMin(CMC_TNMC_GAMMA_CRV_MODE_ACM_LV1);
        parameters.addParameterMin(CMC_TNMC_GAMMA_ACM_LV1);
        parameters.addParameterMin(CMC_TNMC_BEZIER_CTRL_PNT_ACM_LV1);

        parameters.addParameterMin(CMC_DN_TARGET_IDX_ACM_LV2);

        parameters.addParameterMin(CMC_BLC_SENSOR_BLACK_ACM_LV2);
        parameters.addParameterMin(CMC_BLC_SYS_BLACK_ACM_LV2);

        parameters.addParameterMin(CMC_DNS_STRENGTH_ACM_LV2);
        parameters.addParameterMin(CMC_DNS_READ_NOISE_ACM_LV2);
        parameters.addParameterMin(CMC_DNS_WELLDEPTH_ACM_LV2);

        parameters.addParameterMin(CMC_SHA_RADIUS_ACM_LV2);
        parameters.addParameterMin(CMC_SHA_STRENGTH_ACM_LV2);
        parameters.addParameterMin(CMC_SHA_THRESH_ACM_LV2);
        parameters.addParameterMin(CMC_SHA_DETAIL_ACM_LV2);
        parameters.addParameterMin(CMC_SHA_EDGE_SCALE_ACM_LV2);
        parameters.addParameterMin(CMC_SHA_EDGE_OFFSET_ACM_LV2);
        parameters.addParameterMin(CMC_SHA_DENOISE_BYPASS_ACM_LV2);
        parameters.addParameterMin(CMC_SHADN_TAU_ACM_LV2);
        parameters.addParameterMin(CMC_SHADN_SIGMA_ACM_LV2);

        parameters.addParameterMin(CMC_R2Y_CONTRAST_ACM_LV2);
        parameters.addParameterMin(CMC_R2Y_SATURATION_ACM_LV2);
        parameters.addParameterMin(CMC_R2Y_BRIGHTNESS_ACM_LV2);
        parameters.addParameterMin(CMC_R2Y_RANGE_MUL_ACM_LV2);

        parameters.addParameterMin(CMC_HIS_GRID_START_COORDS_ACM_LV2);
        parameters.addParameterMin(CMC_HIS_GRID_TILE_DIMENSIONS_ACM_LV2);

        parameters.addParameterMin(CMC_DPF_WEIGHT_ACM_LV2);
        parameters.addParameterMin(CMC_DPF_THRESHOLD_ACM_LV2);

        parameters.addParameterMin(CMC_TNMC_HISTMIN_ACM_LV2);
        parameters.addParameterMin(CMC_TNMC_HISTMAX_ACM_LV2);
        parameters.addParameterMin(CMC_TNMC_ADAPTIVE_ACM_LV2);

        parameters.addParameterMin(CMC_TNM_IN_Y_ACM_LV2);
        parameters.addParameterMin(CMC_TNM_OUT_Y_ACM_LV2);
        parameters.addParameterMin(CMC_TNM_OUT_C_ACM_LV2);
        parameters.addParameterMin(CMC_TNM_FLAT_FACTOR_ACM_LV2);
        parameters.addParameterMin(CMC_TNM_WEIGHT_LINE_ACM_LV2);
        parameters.addParameterMin(CMC_TNM_COLOUR_CONFIDENCE_ACM_LV2);
        parameters.addParameterMin(CMC_TNM_COLOUR_SATURATION_ACM_LV2);
        parameters.addParameterMin(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_ACM_LV2);
        parameters.addParameterMin(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_ACM_LV2);
        parameters.addParameterMin(CMC_TNMC_OVERSHOT_THRESHOLD_ACM_LV2);
        parameters.addParameterMin(CMC_TNMC_WDR_CEILING_ACM_LV2);
        parameters.addParameterMin(CMC_TNMC_WDR_FLOOR_ACM_LV2);
        parameters.addParameterMin(CMC_TNMC_GAMMA_CRV_MODE_ACM_LV2);
        parameters.addParameterMin(CMC_TNMC_GAMMA_ACM_LV2);
        parameters.addParameterMin(CMC_TNMC_BEZIER_CTRL_PNT_ACM_LV2);

        if (bEnableInterpolationGamma) {
            parameters.addParameterMin(CMC_DN_TARGET_IDX_ACM_LV3);

            parameters.addParameterMin(CMC_BLC_SENSOR_BLACK_ACM_LV3);
            parameters.addParameterMin(CMC_BLC_SYS_BLACK_ACM_LV3);

            parameters.addParameterMin(CMC_DNS_STRENGTH_ACM_LV3);
            parameters.addParameterMin(CMC_DNS_READ_NOISE_ACM_LV3);
            parameters.addParameterMin(CMC_DNS_WELLDEPTH_ACM_LV3);

            parameters.addParameterMin(CMC_SHA_RADIUS_ACM_LV3);
            parameters.addParameterMin(CMC_SHA_STRENGTH_ACM_LV3);
            parameters.addParameterMin(CMC_SHA_THRESH_ACM_LV3);
            parameters.addParameterMin(CMC_SHA_DETAIL_ACM_LV3);
            parameters.addParameterMin(CMC_SHA_EDGE_SCALE_ACM_LV3);
            parameters.addParameterMin(CMC_SHA_EDGE_OFFSET_ACM_LV3);
            parameters.addParameterMin(CMC_SHA_DENOISE_BYPASS_ACM_LV3);
            parameters.addParameterMin(CMC_SHADN_TAU_ACM_LV3);
            parameters.addParameterMin(CMC_SHADN_SIGMA_ACM_LV3);

            parameters.addParameterMin(CMC_R2Y_CONTRAST_ACM_LV3);
            parameters.addParameterMin(CMC_R2Y_SATURATION_ACM_LV3);
            parameters.addParameterMin(CMC_R2Y_BRIGHTNESS_ACM_LV3);
            parameters.addParameterMin(CMC_R2Y_RANGE_MUL_ACM_LV3);

            parameters.addParameterMin(CMC_HIS_GRID_START_COORDS_ACM_LV3);
            parameters.addParameterMin(CMC_HIS_GRID_TILE_DIMENSIONS_ACM_LV3);

            parameters.addParameterMin(CMC_DPF_WEIGHT_ACM_LV3);
            parameters.addParameterMin(CMC_DPF_THRESHOLD_ACM_LV3);

            parameters.addParameterMin(CMC_TNMC_HISTMIN_ACM_LV3);
            parameters.addParameterMin(CMC_TNMC_HISTMAX_ACM_LV3);
            parameters.addParameterMin(CMC_TNMC_ADAPTIVE_ACM_LV3);

            parameters.addParameterMin(CMC_TNM_IN_Y_ACM_LV3);
            parameters.addParameterMin(CMC_TNM_OUT_Y_ACM_LV3);
            parameters.addParameterMin(CMC_TNM_OUT_C_ACM_LV3);
            parameters.addParameterMin(CMC_TNM_FLAT_FACTOR_ACM_LV3);
            parameters.addParameterMin(CMC_TNM_WEIGHT_LINE_ACM_LV3);
            parameters.addParameterMin(CMC_TNM_COLOUR_CONFIDENCE_ACM_LV3);
            parameters.addParameterMin(CMC_TNM_COLOUR_SATURATION_ACM_LV3);
            parameters.addParameterMin(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_ACM_LV3);
            parameters.addParameterMin(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_ACM_LV3);
            parameters.addParameterMin(CMC_TNMC_OVERSHOT_THRESHOLD_ACM_LV3);
            parameters.addParameterMin(CMC_TNMC_WDR_CEILING_ACM_LV3);
            parameters.addParameterMin(CMC_TNMC_WDR_FLOOR_ACM_LV3);
            parameters.addParameterMin(CMC_TNMC_GAMMA_CRV_MODE_ACM_LV3);
            parameters.addParameterMin(CMC_TNMC_GAMMA_ACM_LV3);
            parameters.addParameterMin(CMC_TNMC_BEZIER_CTRL_PNT_ACM_LV3);
        }
        #endif //INFOTM_ENABLE_GAIN_LEVEL_IDX
        break;

    case SAVE_MAX:
        parameters.addParameterMax(CMC_ENABLE);
        parameters.addParameterMax(CMC_DN_TARGET_IDX_CHANGE_ENABLE);
        parameters.addParameterMax(CMC_ENABLE_AVG_GAIN);

        parameters.addParameterMax(CMC_SHA_STRENGTH_EXPECT);

        parameters.addParameterMax(CMC_NIGHT_MODE_DETECT_ENABLE);
        parameters.addParameterMax(CMC_NIGHT_MODE_DETECT_BRIGHTNESS_ENTER);
        parameters.addParameterMax(CMC_NIGHT_MODE_DETECT_BRIGHTNESS_EXIT);
        parameters.addParameterMax(CMC_NIGHT_MODE_DETECT_GAIN_ENTER);
        parameters.addParameterMax(CMC_NIGHT_MODE_DETECT_GAIN_EXIT);
        parameters.addParameterMax(CMC_NIGHT_MODE_DETECT_WEIGHTING);

        parameters.addParameterMax(CMC_ENABLE_CAPTURE_IQ);
        parameters.addParameterMax(CMC_CAPTURE_CCM_RATIO);
        parameters.addParameterMax(CMC_R2Y_CAPTURE_RANGE_MUL);
        parameters.addParameterMax(CMC_TNM_CAPTURE_IN_Y);
        parameters.addParameterMax(CMC_TNM_CAPTURE_OUT_Y);
        parameters.addParameterMax(CMC_TNM_CAPTURE_OUT_C);

        //3D-Denoise target index.
        parameters.addParameterMax(CMC_DN_TARGET_IDX_FCM);

        #if defined(INFOTM_ENABLE_COLOR_MODE_CHANGE)
        ///////////////////////////////////////Flat Color Mode setting
        //BLC SENSOR BLACK
        parameters.addParameterMax(CMC_BLC_SENSOR_BLACK_FCM);
        parameters.addParameterMax(CMC_BLC_SYS_BLACK_FCM);

        //DNS
        parameters.addParameterMax(CMC_DNS_STRENGTH_FCM);
        parameters.addParameterMax(CMC_DNS_READ_NOISE_FCM);
        parameters.addParameterMax(CMC_DNS_WELLDEPTH_FCM);

        //SHA
        parameters.addParameterMax(CMC_SHA_RADIUS_FCM);
        parameters.addParameterMax(CMC_SHA_STRENGTH_FCM);
        parameters.addParameterMax(CMC_SHA_THRESH_FCM);
        parameters.addParameterMax(CMC_SHA_DETAIL_FCM);
        parameters.addParameterMax(CMC_SHA_EDGE_SCALE_FCM);
        parameters.addParameterMax(CMC_SHA_EDGE_OFFSET_FCM);
        parameters.addParameterMax(CMC_SHA_DENOISE_BYPASS_FCM);
        parameters.addParameterMax(CMC_SHADN_TAU_FCM);
        parameters.addParameterMax(CMC_SHADN_SIGMA_FCM);

        //R2Y
        parameters.addParameterMax(CMC_R2Y_BRIGHTNESS_FCM);
        parameters.addParameterMax(CMC_R2Y_CONTRAST_FCM);
        parameters.addParameterMax(CMC_R2Y_SATURATION_FCM);
        parameters.addParameterMax(CMC_R2Y_RANGE_MUL_FCM);

        //HIS
        parameters.addParameterMax(CMC_HIS_GRID_START_COORDS_FCM);
        parameters.addParameterMax(CMC_HIS_GRID_TILE_DIMENSIONS_FCM);

        //DPF
        parameters.addParameterMax(CMC_DPF_WEIGHT_FCM);
        parameters.addParameterMax(CMC_DPF_THRESHOLD_FCM);

        //TNM
        parameters.addParameterMax(CMC_TNMC_HISTMIN_FCM);
        parameters.addParameterMax(CMC_TNMC_HISTMAX_FCM);
        parameters.addParameterMax(CMC_TNMC_ADAPTIVE_FCM);

        parameters.addParameterMax(CMC_TNM_IN_Y_FCM);
        parameters.addParameterMax(CMC_TNM_OUT_Y_FCM);
        parameters.addParameterMax(CMC_TNM_OUT_C_FCM);
        parameters.addParameterMax(CMC_TNM_FLAT_FACTOR_FCM);
        parameters.addParameterMax(CMC_TNM_WEIGHT_LINE_FCM);
        parameters.addParameterMax(CMC_TNM_COLOUR_CONFIDENCE_FCM);
        parameters.addParameterMax(CMC_TNM_COLOUR_SATURATION_FCM);
        parameters.addParameterMax(CMC_TNMC_ENABLE_GAMMA_FCM);
        parameters.addParameterMax(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_FCM);
        parameters.addParameterMax(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_FCM);
        parameters.addParameterMax(CMC_TNMC_OVERSHOT_THRESHOLD_FCM);
        parameters.addParameterMax(CMC_TNMC_WDR_CEILING_FCM);
        parameters.addParameterMax(CMC_TNMC_WDR_FLOOR_FCM);
        parameters.addParameterMax(CMC_TNMC_GAMMA_CRV_MODE_FCM);
        parameters.addParameterMax(CMC_TNMC_GAMMA_FCM);
        parameters.addParameterMax(CMC_TNMC_BEZIER_CTRL_PNT_FCM);

        //AE
        parameters.addParameterMax(CMC_AE_TARGET_BRIGHTNESS_FCM);
        parameters.addParameterMax(CMC_AE_TARGET_MIN_FCM);

        //Sensor Max Gain
        parameters.addParameterMax(CMC_SENSOR_MAX_GAIN_FCM);
        #endif //INFOTM_ENABLE_COLOR_MODE_CHANGE

        #if defined(INFOTM_ENABLE_COLOR_MODE_CHANGE) && defined(INFOTM_ENABLE_GAIN_LEVEL_IDX)
        parameters.addParameterMax(CMC_DN_TARGET_IDX_FCM_LV1);

        parameters.addParameterMax(CMC_BLC_SENSOR_BLACK_FCM_LV1);
        parameters.addParameterMax(CMC_BLC_SYS_BLACK_FCM_LV1);

        parameters.addParameterMax(CMC_DNS_STRENGTH_FCM_LV1);
        parameters.addParameterMax(CMC_DNS_READ_NOISE_FCM_LV1);
        parameters.addParameterMax(CMC_DNS_WELLDEPTH_FCM_LV1);

        parameters.addParameterMax(CMC_SHA_RADIUS_FCM_LV1);
        parameters.addParameterMax(CMC_SHA_STRENGTH_FCM_LV1);
        parameters.addParameterMax(CMC_SHA_THRESH_FCM_LV1);
        parameters.addParameterMax(CMC_SHA_DETAIL_FCM_LV1);
        parameters.addParameterMax(CMC_SHA_EDGE_SCALE_FCM_LV1);
        parameters.addParameterMax(CMC_SHA_EDGE_OFFSET_FCM_LV1);
        parameters.addParameterMax(CMC_SHA_DENOISE_BYPASS_FCM_LV1);
        parameters.addParameterMax(CMC_SHADN_TAU_FCM_LV1);
        parameters.addParameterMax(CMC_SHADN_SIGMA_FCM_LV1);

        parameters.addParameterMax(CMC_R2Y_CONTRAST_FCM_LV1);
        parameters.addParameterMax(CMC_R2Y_SATURATION_FCM_LV1);
        parameters.addParameterMax(CMC_R2Y_BRIGHTNESS_FCM_LV1);
        parameters.addParameterMax(CMC_R2Y_RANGE_MUL_FCM_LV1);

        parameters.addParameterMax(CMC_HIS_GRID_START_COORDS_FCM_LV1);
        parameters.addParameterMax(CMC_HIS_GRID_TILE_DIMENSIONS_FCM_LV1);

        parameters.addParameterMax(CMC_DPF_WEIGHT_FCM_LV1);
        parameters.addParameterMax(CMC_DPF_THRESHOLD_FCM_LV1);

        parameters.addParameterMax(CMC_TNMC_HISTMIN_FCM_LV1);
        parameters.addParameterMax(CMC_TNMC_HISTMAX_FCM_LV1);
        parameters.addParameterMax(CMC_TNMC_ADAPTIVE_FCM_LV1);

        parameters.addParameterMax(CMC_TNM_IN_Y_FCM_LV1);
        parameters.addParameterMax(CMC_TNM_OUT_Y_FCM_LV1);
        parameters.addParameterMax(CMC_TNM_OUT_C_FCM_LV1);
        parameters.addParameterMax(CMC_TNM_FLAT_FACTOR_FCM_LV1);
        parameters.addParameterMax(CMC_TNM_WEIGHT_LINE_FCM_LV1);
        parameters.addParameterMax(CMC_TNM_COLOUR_CONFIDENCE_FCM_LV1);
        parameters.addParameterMax(CMC_TNM_COLOUR_SATURATION_FCM_LV1);
        parameters.addParameterMax(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_FCM_LV1);
        parameters.addParameterMax(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_FCM_LV1);
        parameters.addParameterMax(CMC_TNMC_OVERSHOT_THRESHOLD_FCM_LV1);
        parameters.addParameterMax(CMC_TNMC_WDR_CEILING_FCM_LV1);
        parameters.addParameterMax(CMC_TNMC_WDR_FLOOR_FCM_LV1);
        parameters.addParameterMax(CMC_TNMC_GAMMA_CRV_MODE_FCM_LV1);
        parameters.addParameterMax(CMC_TNMC_GAMMA_FCM_LV1);
        parameters.addParameterMax(CMC_TNMC_BEZIER_CTRL_PNT_FCM_LV1);

        parameters.addParameterMax(CMC_DN_TARGET_IDX_FCM_LV2);

        parameters.addParameterMax(CMC_BLC_SENSOR_BLACK_FCM_LV2);
        parameters.addParameterMax(CMC_BLC_SYS_BLACK_FCM_LV2);

        parameters.addParameterMax(CMC_DNS_STRENGTH_FCM_LV2);
        parameters.addParameterMax(CMC_DNS_READ_NOISE_FCM_LV2);
        parameters.addParameterMax(CMC_DNS_WELLDEPTH_FCM_LV2);

        parameters.addParameterMax(CMC_SHA_RADIUS_FCM_LV2);
        parameters.addParameterMax(CMC_SHA_STRENGTH_FCM_LV2);
        parameters.addParameterMax(CMC_SHA_THRESH_FCM_LV2);
        parameters.addParameterMax(CMC_SHA_DETAIL_FCM_LV2);
        parameters.addParameterMax(CMC_SHA_EDGE_SCALE_FCM_LV2);
        parameters.addParameterMax(CMC_SHA_EDGE_OFFSET_FCM_LV2);
        parameters.addParameterMax(CMC_SHA_DENOISE_BYPASS_FCM_LV2);
        parameters.addParameterMax(CMC_SHADN_TAU_FCM_LV2);
        parameters.addParameterMax(CMC_SHADN_SIGMA_FCM_LV2);

        parameters.addParameterMax(CMC_R2Y_CONTRAST_FCM_LV2);
        parameters.addParameterMax(CMC_R2Y_SATURATION_FCM_LV2);
        parameters.addParameterMax(CMC_R2Y_BRIGHTNESS_FCM_LV2);
        parameters.addParameterMax(CMC_R2Y_RANGE_MUL_FCM_LV2);

        parameters.addParameterMax(CMC_HIS_GRID_START_COORDS_FCM_LV2);
        parameters.addParameterMax(CMC_HIS_GRID_TILE_DIMENSIONS_FCM_LV2);

        parameters.addParameterMax(CMC_DPF_WEIGHT_FCM_LV2);
        parameters.addParameterMax(CMC_DPF_THRESHOLD_FCM_LV2);

        parameters.addParameterMax(CMC_TNMC_HISTMIN_FCM_LV2);
        parameters.addParameterMax(CMC_TNMC_HISTMAX_FCM_LV2);
        parameters.addParameterMax(CMC_TNMC_ADAPTIVE_FCM_LV2);

        parameters.addParameterMax(CMC_TNM_IN_Y_FCM_LV2);
        parameters.addParameterMax(CMC_TNM_OUT_Y_FCM_LV2);
        parameters.addParameterMax(CMC_TNM_OUT_C_FCM_LV2);
        parameters.addParameterMax(CMC_TNM_FLAT_FACTOR_FCM_LV2);
        parameters.addParameterMax(CMC_TNM_WEIGHT_LINE_FCM_LV2);
        parameters.addParameterMax(CMC_TNM_COLOUR_CONFIDENCE_FCM_LV2);
        parameters.addParameterMax(CMC_TNM_COLOUR_SATURATION_FCM_LV2);
        parameters.addParameterMax(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_FCM_LV2);
        parameters.addParameterMax(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_FCM_LV2);
        parameters.addParameterMax(CMC_TNMC_OVERSHOT_THRESHOLD_FCM_LV2);
        parameters.addParameterMax(CMC_TNMC_WDR_CEILING_FCM_LV2);
        parameters.addParameterMax(CMC_TNMC_WDR_FLOOR_FCM_LV2);
        parameters.addParameterMax(CMC_TNMC_GAMMA_CRV_MODE_FCM_LV2);
        parameters.addParameterMax(CMC_TNMC_GAMMA_FCM_LV2);
        parameters.addParameterMax(CMC_TNMC_BEZIER_CTRL_PNT_FCM_LV2);

        if (bEnableInterpolationGamma) {
            parameters.addParameterMax(CMC_DN_TARGET_IDX_FCM_LV3);

            parameters.addParameterMax(CMC_BLC_SENSOR_BLACK_FCM_LV3);
            parameters.addParameterMax(CMC_BLC_SYS_BLACK_FCM_LV3);

            parameters.addParameterMax(CMC_DNS_STRENGTH_FCM_LV3);
            parameters.addParameterMax(CMC_DNS_READ_NOISE_FCM_LV3);
            parameters.addParameterMax(CMC_DNS_WELLDEPTH_FCM_LV3);

            parameters.addParameterMax(CMC_SHA_RADIUS_FCM_LV3);
            parameters.addParameterMax(CMC_SHA_STRENGTH_FCM_LV3);
            parameters.addParameterMax(CMC_SHA_THRESH_FCM_LV3);
            parameters.addParameterMax(CMC_SHA_DETAIL_FCM_LV3);
            parameters.addParameterMax(CMC_SHA_EDGE_SCALE_FCM_LV3);
            parameters.addParameterMax(CMC_SHA_EDGE_OFFSET_FCM_LV3);
            parameters.addParameterMax(CMC_SHA_DENOISE_BYPASS_FCM_LV3);
            parameters.addParameterMax(CMC_SHADN_TAU_FCM_LV3);
            parameters.addParameterMax(CMC_SHADN_SIGMA_FCM_LV3);

            parameters.addParameterMax(CMC_R2Y_CONTRAST_FCM_LV3);
            parameters.addParameterMax(CMC_R2Y_SATURATION_FCM_LV3);
            parameters.addParameterMax(CMC_R2Y_BRIGHTNESS_FCM_LV3);
            parameters.addParameterMax(CMC_R2Y_RANGE_MUL_FCM_LV3);

            parameters.addParameterMax(CMC_HIS_GRID_START_COORDS_FCM_LV3);
            parameters.addParameterMax(CMC_HIS_GRID_TILE_DIMENSIONS_FCM_LV3);

            parameters.addParameterMax(CMC_DPF_WEIGHT_FCM_LV3);
            parameters.addParameterMax(CMC_DPF_THRESHOLD_FCM_LV3);

            parameters.addParameterMax(CMC_TNMC_HISTMIN_FCM_LV3);
            parameters.addParameterMax(CMC_TNMC_HISTMAX_FCM_LV3);
            parameters.addParameterMax(CMC_TNMC_ADAPTIVE_FCM_LV3);

            parameters.addParameterMax(CMC_TNM_IN_Y_FCM_LV3);
            parameters.addParameterMax(CMC_TNM_OUT_Y_FCM_LV3);
            parameters.addParameterMax(CMC_TNM_OUT_C_FCM_LV3);
            parameters.addParameterMax(CMC_TNM_FLAT_FACTOR_FCM_LV3);
            parameters.addParameterMax(CMC_TNM_WEIGHT_LINE_FCM_LV3);
            parameters.addParameterMax(CMC_TNM_COLOUR_CONFIDENCE_FCM_LV3);
            parameters.addParameterMax(CMC_TNM_COLOUR_SATURATION_FCM_LV3);
            parameters.addParameterMax(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_FCM_LV3);
            parameters.addParameterMax(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_FCM_LV3);
            parameters.addParameterMax(CMC_TNMC_OVERSHOT_THRESHOLD_FCM_LV3);
            parameters.addParameterMax(CMC_TNMC_WDR_CEILING_FCM_LV3);
            parameters.addParameterMax(CMC_TNMC_WDR_FLOOR_FCM_LV3);
            parameters.addParameterMax(CMC_TNMC_GAMMA_CRV_MODE_FCM_LV3);
            parameters.addParameterMax(CMC_TNMC_GAMMA_FCM_LV3);
            parameters.addParameterMax(CMC_TNMC_BEZIER_CTRL_PNT_FCM_LV3);
        }
        #endif //INFOTM_ENABLE_COLOR_MODE_CHANGE, INFOTM_ENABLE_GAIN_LEVEL_IDX

        //3D-Denoise target index.
        parameters.addParameterMax(CMC_DN_TARGET_IDX_ACM);

        #if defined(INFOTM_ENABLE_COLOR_MODE_CHANGE)
        ///////////////////////////////////////Advanced Color Mode setting
        if (bCmcEnable) {
            //BLC SENSOR BLACK
            parameters.addParameterMax(CMC_BLC_SENSOR_BLACK_ACM);
            parameters.addParameterMax(CMC_BLC_SYS_BLACK_ACM);

            //DNS
            parameters.addParameterMax(CMC_DNS_STRENGTH_ACM);
            parameters.addParameterMax(CMC_DNS_READ_NOISE_ACM);
            parameters.addParameterMax(CMC_DNS_WELLDEPTH_ACM);

            //SHA
            parameters.addParameterMax(CMC_SHA_RADIUS_ACM);
            parameters.addParameterMax(CMC_SHA_STRENGTH_ACM);
            parameters.addParameterMax(CMC_SHA_THRESH_ACM);
            parameters.addParameterMax(CMC_SHA_DETAIL_ACM);
            parameters.addParameterMax(CMC_SHA_EDGE_SCALE_ACM);
            parameters.addParameterMax(CMC_SHA_EDGE_OFFSET_ACM);
            parameters.addParameterMax(CMC_SHA_DENOISE_BYPASS_ACM);
            parameters.addParameterMax(CMC_SHADN_TAU_ACM);
            parameters.addParameterMax(CMC_SHADN_SIGMA_ACM);

            //R2Y
            parameters.addParameterMax(CMC_R2Y_BRIGHTNESS_ACM);
            parameters.addParameterMax(CMC_R2Y_CONTRAST_ACM);
            parameters.addParameterMax(CMC_R2Y_SATURATION_ACM);
            parameters.addParameterMax(CMC_R2Y_RANGE_MUL_ACM);

            //HIS
            parameters.addParameterMax(CMC_HIS_GRID_START_COORDS_ACM);
            parameters.addParameterMax(CMC_HIS_GRID_TILE_DIMENSIONS_ACM);

            //DPF
            parameters.addParameterMax(CMC_DPF_WEIGHT_ACM);
            parameters.addParameterMax(CMC_DPF_THRESHOLD_ACM);

            //TNM
            parameters.addParameterMax(CMC_TNMC_HISTMIN_ACM);
            parameters.addParameterMax(CMC_TNMC_HISTMAX_ACM);
            parameters.addParameterMax(CMC_TNMC_ADAPTIVE_ACM);

            parameters.addParameterMax(CMC_TNM_IN_Y_ACM);
            parameters.addParameterMax(CMC_TNM_OUT_Y_ACM);
            parameters.addParameterMax(CMC_TNM_OUT_C_ACM);
            parameters.addParameterMax(CMC_TNM_FLAT_FACTOR_ACM);
            parameters.addParameterMax(CMC_TNM_WEIGHT_LINE_ACM);
            parameters.addParameterMax(CMC_TNM_COLOUR_CONFIDENCE_ACM);
            parameters.addParameterMax(CMC_TNM_COLOUR_SATURATION_ACM);
            parameters.addParameterMax(CMC_TNMC_ENABLE_GAMMA_ACM);
            parameters.addParameterMax(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_ACM);
            parameters.addParameterMax(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_ACM);
            parameters.addParameterMax(CMC_TNMC_OVERSHOT_THRESHOLD_ACM);
            parameters.addParameterMax(CMC_TNMC_WDR_CEILING_ACM);
            parameters.addParameterMax(CMC_TNMC_WDR_FLOOR_ACM);
            parameters.addParameterMax(CMC_TNMC_GAMMA_CRV_MODE_ACM);
            parameters.addParameterMax(CMC_TNMC_GAMMA_ACM);
            parameters.addParameterMax(CMC_TNMC_BEZIER_CTRL_PNT_ACM);

            //AE
            parameters.addParameterMax(CMC_AE_TARGET_BRIGHTNESS_ACM);
            parameters.addParameterMax(CMC_AE_TARGET_MIN_ACM);

            //Sensor Max Gain
            parameters.addParameterMax(CMC_SENSOR_MAX_GAIN_ACM);
        }
        #endif //INFOTM_ENABLE_COLOR_MODE_CHANGE

        #if defined(INFOTM_ENABLE_GAIN_LEVEL_IDX)
        parameters.addParameterMax(CMC_SENSOR_GAIN_LEVEL_CTRL);
        parameters.addParameterMax(CMC_SENSOR_GAIN_LV1);
        parameters.addParameterMax(CMC_SENSOR_GAIN_LV2);
        parameters.addParameterMax(CMC_SENSOR_GAIN_LV1_INTERPOLATION);
        parameters.addParameterMax(CMC_SENSOR_GAIN_LV2_INTERPOLATION);
        parameters.addParameterMax(CMC_ENABLE_INTERPOLATION_GAMMA);
        parameters.addParameterMax(CMC_INTERPOLATION_GAMMA_0);
        parameters.addParameterMax(CMC_INTERPOLATION_GAMMA_1);
        parameters.addParameterMax(CMC_INTERPOLATION_GAMMA_2);

        parameters.addParameterMax(CMC_CCM_ATTENUATION_LV1);
        parameters.addParameterMax(CMC_CCM_ATTENUATION_LV2);
        parameters.addParameterMax(CMC_CCM_ATTENUATION_LV3);

        parameters.addParameterMax(CMC_DN_TARGET_IDX_ACM_LV1);

        parameters.addParameterMax(CMC_BLC_SENSOR_BLACK_ACM_LV1);
        parameters.addParameterMax(CMC_BLC_SYS_BLACK_ACM_LV1);

        parameters.addParameterMax(CMC_DNS_STRENGTH_ACM_LV1);
        parameters.addParameterMax(CMC_DNS_READ_NOISE_ACM_LV1);
        parameters.addParameterMax(CMC_DNS_WELLDEPTH_ACM_LV1);

        parameters.addParameterMax(CMC_SHA_RADIUS_ACM_LV1);
        parameters.addParameterMax(CMC_SHA_STRENGTH_ACM_LV1);
        parameters.addParameterMax(CMC_SHA_THRESH_ACM_LV1);
        parameters.addParameterMax(CMC_SHA_DETAIL_ACM_LV1);
        parameters.addParameterMax(CMC_SHA_EDGE_SCALE_ACM_LV1);
        parameters.addParameterMax(CMC_SHA_EDGE_OFFSET_ACM_LV1);
        parameters.addParameterMax(CMC_SHA_DENOISE_BYPASS_ACM_LV1);
        parameters.addParameterMax(CMC_SHADN_TAU_ACM_LV1);
        parameters.addParameterMax(CMC_SHADN_SIGMA_ACM_LV1);

        parameters.addParameterMax(CMC_R2Y_CONTRAST_ACM_LV1);
        parameters.addParameterMax(CMC_R2Y_SATURATION_ACM_LV1);
        parameters.addParameterMax(CMC_R2Y_BRIGHTNESS_ACM_LV1);
        parameters.addParameterMax(CMC_R2Y_RANGE_MUL_ACM_LV1);

        parameters.addParameterMax(CMC_HIS_GRID_START_COORDS_ACM_LV1);
        parameters.addParameterMax(CMC_HIS_GRID_TILE_DIMENSIONS_ACM_LV1);

        parameters.addParameterMax(CMC_DPF_WEIGHT_ACM_LV1);
        parameters.addParameterMax(CMC_DPF_THRESHOLD_ACM_LV1);

        parameters.addParameterMax(CMC_TNMC_HISTMIN_ACM_LV1);
        parameters.addParameterMax(CMC_TNMC_HISTMAX_ACM_LV1);
        parameters.addParameterMax(CMC_TNMC_ADAPTIVE_ACM_LV1);

        parameters.addParameterMax(CMC_TNM_IN_Y_ACM_LV1);
        parameters.addParameterMax(CMC_TNM_OUT_Y_ACM_LV1);
        parameters.addParameterMax(CMC_TNM_OUT_C_ACM_LV1);
        parameters.addParameterMax(CMC_TNM_FLAT_FACTOR_ACM_LV1);
        parameters.addParameterMax(CMC_TNM_WEIGHT_LINE_ACM_LV1);
        parameters.addParameterMax(CMC_TNM_COLOUR_CONFIDENCE_ACM_LV1);
        parameters.addParameterMax(CMC_TNM_COLOUR_SATURATION_ACM_LV1);
        parameters.addParameterMax(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_ACM_LV1);
        parameters.addParameterMax(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_ACM_LV1);
        parameters.addParameterMax(CMC_TNMC_OVERSHOT_THRESHOLD_ACM_LV1);
        parameters.addParameterMax(CMC_TNMC_WDR_CEILING_ACM_LV1);
        parameters.addParameterMax(CMC_TNMC_WDR_FLOOR_ACM_LV1);
        parameters.addParameterMax(CMC_TNMC_GAMMA_CRV_MODE_ACM_LV1);
        parameters.addParameterMax(CMC_TNMC_GAMMA_ACM_LV1);
        parameters.addParameterMax(CMC_TNMC_BEZIER_CTRL_PNT_ACM_LV1);

        parameters.addParameterMax(CMC_DN_TARGET_IDX_ACM_LV2);

        parameters.addParameterMax(CMC_BLC_SENSOR_BLACK_ACM_LV2);
        parameters.addParameterMax(CMC_BLC_SYS_BLACK_ACM_LV2);

        parameters.addParameterMax(CMC_DNS_STRENGTH_ACM_LV2);
        parameters.addParameterMax(CMC_DNS_READ_NOISE_ACM_LV2);
        parameters.addParameterMax(CMC_DNS_WELLDEPTH_ACM_LV2);

        parameters.addParameterMax(CMC_SHA_RADIUS_ACM_LV2);
        parameters.addParameterMax(CMC_SHA_STRENGTH_ACM_LV2);
        parameters.addParameterMax(CMC_SHA_THRESH_ACM_LV2);
        parameters.addParameterMax(CMC_SHA_DETAIL_ACM_LV2);
        parameters.addParameterMax(CMC_SHA_EDGE_SCALE_ACM_LV2);
        parameters.addParameterMax(CMC_SHA_EDGE_OFFSET_ACM_LV2);
        parameters.addParameterMax(CMC_SHA_DENOISE_BYPASS_ACM_LV2);
        parameters.addParameterMax(CMC_SHADN_TAU_ACM_LV2);
        parameters.addParameterMax(CMC_SHADN_SIGMA_ACM_LV2);

        parameters.addParameterMax(CMC_R2Y_CONTRAST_ACM_LV2);
        parameters.addParameterMax(CMC_R2Y_SATURATION_ACM_LV2);
        parameters.addParameterMax(CMC_R2Y_BRIGHTNESS_ACM_LV2);
        parameters.addParameterMax(CMC_R2Y_RANGE_MUL_ACM_LV2);

        parameters.addParameterMax(CMC_HIS_GRID_START_COORDS_ACM_LV2);
        parameters.addParameterMax(CMC_HIS_GRID_TILE_DIMENSIONS_ACM_LV2);

        parameters.addParameterMax(CMC_DPF_WEIGHT_ACM_LV2);
        parameters.addParameterMax(CMC_DPF_THRESHOLD_ACM_LV2);

        parameters.addParameterMax(CMC_TNMC_HISTMIN_ACM_LV2);
        parameters.addParameterMax(CMC_TNMC_HISTMAX_ACM_LV2);
        parameters.addParameterMax(CMC_TNMC_ADAPTIVE_ACM_LV2);

        parameters.addParameterMax(CMC_TNM_IN_Y_ACM_LV2);
        parameters.addParameterMax(CMC_TNM_OUT_Y_ACM_LV2);
        parameters.addParameterMax(CMC_TNM_OUT_C_ACM_LV2);
        parameters.addParameterMax(CMC_TNM_FLAT_FACTOR_ACM_LV2);
        parameters.addParameterMax(CMC_TNM_WEIGHT_LINE_ACM_LV2);
        parameters.addParameterMax(CMC_TNM_COLOUR_CONFIDENCE_ACM_LV2);
        parameters.addParameterMax(CMC_TNM_COLOUR_SATURATION_ACM_LV2);
        parameters.addParameterMax(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_ACM_LV2);
        parameters.addParameterMax(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_ACM_LV2);
        parameters.addParameterMax(CMC_TNMC_OVERSHOT_THRESHOLD_ACM_LV2);
        parameters.addParameterMax(CMC_TNMC_WDR_CEILING_ACM_LV2);
        parameters.addParameterMax(CMC_TNMC_WDR_FLOOR_ACM_LV2);
        parameters.addParameterMax(CMC_TNMC_GAMMA_CRV_MODE_ACM_LV2);
        parameters.addParameterMax(CMC_TNMC_GAMMA_ACM_LV2);
        parameters.addParameterMax(CMC_TNMC_BEZIER_CTRL_PNT_ACM_LV2);

        if (bEnableInterpolationGamma) {
            parameters.addParameterMax(CMC_DN_TARGET_IDX_ACM_LV3);

            parameters.addParameterMax(CMC_BLC_SENSOR_BLACK_ACM_LV3);
            parameters.addParameterMax(CMC_BLC_SYS_BLACK_ACM_LV3);

            parameters.addParameterMax(CMC_DNS_STRENGTH_ACM_LV3);
            parameters.addParameterMax(CMC_DNS_READ_NOISE_ACM_LV3);
            parameters.addParameterMax(CMC_DNS_WELLDEPTH_ACM_LV3);

            parameters.addParameterMax(CMC_SHA_RADIUS_ACM_LV3);
            parameters.addParameterMax(CMC_SHA_STRENGTH_ACM_LV3);
            parameters.addParameterMax(CMC_SHA_THRESH_ACM_LV3);
            parameters.addParameterMax(CMC_SHA_DETAIL_ACM_LV3);
            parameters.addParameterMax(CMC_SHA_EDGE_SCALE_ACM_LV3);
            parameters.addParameterMax(CMC_SHA_EDGE_OFFSET_ACM_LV3);
            parameters.addParameterMax(CMC_SHA_DENOISE_BYPASS_ACM_LV3);
            parameters.addParameterMax(CMC_SHADN_TAU_ACM_LV3);
            parameters.addParameterMax(CMC_SHADN_SIGMA_ACM_LV3);

            parameters.addParameterMax(CMC_R2Y_CONTRAST_ACM_LV3);
            parameters.addParameterMax(CMC_R2Y_SATURATION_ACM_LV3);
            parameters.addParameterMax(CMC_R2Y_BRIGHTNESS_ACM_LV3);
            parameters.addParameterMax(CMC_R2Y_RANGE_MUL_ACM_LV3);

            parameters.addParameterMax(CMC_HIS_GRID_START_COORDS_ACM_LV3);
            parameters.addParameterMax(CMC_HIS_GRID_TILE_DIMENSIONS_ACM_LV3);

            parameters.addParameterMax(CMC_DPF_WEIGHT_ACM_LV3);
            parameters.addParameterMax(CMC_DPF_THRESHOLD_ACM_LV3);

            parameters.addParameterMax(CMC_TNMC_HISTMIN_ACM_LV3);
            parameters.addParameterMax(CMC_TNMC_HISTMAX_ACM_LV3);
            parameters.addParameterMax(CMC_TNMC_ADAPTIVE_ACM_LV3);

            parameters.addParameterMax(CMC_TNM_IN_Y_ACM_LV3);
            parameters.addParameterMax(CMC_TNM_OUT_Y_ACM_LV3);
            parameters.addParameterMax(CMC_TNM_OUT_C_ACM_LV3);
            parameters.addParameterMax(CMC_TNM_FLAT_FACTOR_ACM_LV3);
            parameters.addParameterMax(CMC_TNM_WEIGHT_LINE_ACM_LV3);
            parameters.addParameterMax(CMC_TNM_COLOUR_CONFIDENCE_ACM_LV3);
            parameters.addParameterMax(CMC_TNM_COLOUR_SATURATION_ACM_LV3);
            parameters.addParameterMax(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_ACM_LV3);
            parameters.addParameterMax(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_ACM_LV3);
            parameters.addParameterMax(CMC_TNMC_OVERSHOT_THRESHOLD_ACM_LV3);
            parameters.addParameterMax(CMC_TNMC_WDR_CEILING_ACM_LV3);
            parameters.addParameterMax(CMC_TNMC_WDR_FLOOR_ACM_LV3);
            parameters.addParameterMax(CMC_TNMC_GAMMA_CRV_MODE_ACM_LV3);
            parameters.addParameterMax(CMC_TNMC_GAMMA_ACM_LV3);
            parameters.addParameterMax(CMC_TNMC_BEZIER_CTRL_PNT_ACM_LV3);
        }
        #endif //INFOTM_ENABLE_GAIN_LEVEL_IDX
        break;

    case SAVE_DEF:
        parameters.addParameterDef(CMC_ENABLE);
        parameters.addParameterDef(CMC_DN_TARGET_IDX_CHANGE_ENABLE);
        parameters.addParameterDef(CMC_ENABLE_AVG_GAIN);

        parameters.addParameterDef(CMC_SHA_STRENGTH_EXPECT);

        parameters.addParameterDef(CMC_NIGHT_MODE_DETECT_ENABLE);
        parameters.addParameterDef(CMC_NIGHT_MODE_DETECT_BRIGHTNESS_ENTER);
        parameters.addParameterDef(CMC_NIGHT_MODE_DETECT_BRIGHTNESS_EXIT);
        parameters.addParameterDef(CMC_NIGHT_MODE_DETECT_GAIN_ENTER);
        parameters.addParameterDef(CMC_NIGHT_MODE_DETECT_GAIN_EXIT);
        parameters.addParameterDef(CMC_NIGHT_MODE_DETECT_WEIGHTING);

        parameters.addParameterDef(CMC_ENABLE_CAPTURE_IQ);
        parameters.addParameterDef(CMC_CAPTURE_CCM_RATIO);
        parameters.addParameterDef(CMC_R2Y_CAPTURE_RANGE_MUL);
        parameters.addParameterDef(CMC_TNM_CAPTURE_IN_Y);
        parameters.addParameterDef(CMC_TNM_CAPTURE_OUT_Y);
        parameters.addParameterDef(CMC_TNM_CAPTURE_OUT_C);

        //3D-Denoise target index.
        parameters.addParameterDef(CMC_DN_TARGET_IDX_FCM);

        #if defined(INFOTM_ENABLE_COLOR_MODE_CHANGE)
        ///////////////////////////////////////Flat Color Mode setting
        //BLC SENSOR BLACK
        parameters.addParameterDef(CMC_BLC_SENSOR_BLACK_FCM);
        parameters.addParameterDef(CMC_BLC_SYS_BLACK_FCM);

        //DNS
        parameters.addParameterDef(CMC_DNS_STRENGTH_FCM);
        parameters.addParameterDef(CMC_DNS_READ_NOISE_FCM);
        parameters.addParameterDef(CMC_DNS_WELLDEPTH_FCM);

        //SHA
        parameters.addParameterDef(CMC_SHA_RADIUS_FCM);
        parameters.addParameterDef(CMC_SHA_STRENGTH_FCM);
        parameters.addParameterDef(CMC_SHA_THRESH_FCM);
        parameters.addParameterDef(CMC_SHA_DETAIL_FCM);
        parameters.addParameterDef(CMC_SHA_EDGE_SCALE_FCM);
        parameters.addParameterDef(CMC_SHA_EDGE_OFFSET_FCM);
        parameters.addParameterDef(CMC_SHA_DENOISE_BYPASS_FCM);
        parameters.addParameterDef(CMC_SHADN_TAU_FCM);
        parameters.addParameterDef(CMC_SHADN_SIGMA_FCM);

        //R2Y
        parameters.addParameterDef(CMC_R2Y_BRIGHTNESS_FCM);
        parameters.addParameterDef(CMC_R2Y_CONTRAST_FCM);
        parameters.addParameterDef(CMC_R2Y_SATURATION_FCM);
        parameters.addParameterDef(CMC_R2Y_RANGE_MUL_FCM);

        //HIS
        parameters.addParameterDef(CMC_HIS_GRID_START_COORDS_FCM);
        parameters.addParameterDef(CMC_HIS_GRID_TILE_DIMENSIONS_FCM);

        //DPF
        parameters.addParameterDef(CMC_DPF_WEIGHT_FCM);
        parameters.addParameterDef(CMC_DPF_THRESHOLD_FCM);

        //TNM
        parameters.addParameterDef(CMC_TNMC_HISTMIN_FCM);
        parameters.addParameterDef(CMC_TNMC_HISTMAX_FCM);
        parameters.addParameterDef(CMC_TNMC_ADAPTIVE_FCM);

        parameters.addParameterDef(CMC_TNM_IN_Y_FCM);
        parameters.addParameterDef(CMC_TNM_OUT_Y_FCM);
        parameters.addParameterDef(CMC_TNM_OUT_C_FCM);
        parameters.addParameterDef(CMC_TNM_FLAT_FACTOR_FCM);
        parameters.addParameterDef(CMC_TNM_WEIGHT_LINE_FCM);
        parameters.addParameterDef(CMC_TNM_COLOUR_CONFIDENCE_FCM);
        parameters.addParameterDef(CMC_TNM_COLOUR_SATURATION_FCM);
        parameters.addParameterDef(CMC_TNMC_ENABLE_GAMMA_FCM);
        parameters.addParameterDef(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_FCM);
        parameters.addParameterDef(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_FCM);
        parameters.addParameterDef(CMC_TNMC_OVERSHOT_THRESHOLD_FCM);
        parameters.addParameterDef(CMC_TNMC_WDR_CEILING_FCM);
        parameters.addParameterDef(CMC_TNMC_WDR_FLOOR_FCM);
        parameters.addParameterDef(CMC_TNMC_GAMMA_CRV_MODE_FCM);
        parameters.addParameterDef(CMC_TNMC_GAMMA_FCM);
        parameters.addParameterDef(CMC_TNMC_BEZIER_CTRL_PNT_FCM);

        //AE
        parameters.addParameterDef(CMC_AE_TARGET_BRIGHTNESS_FCM);
        parameters.addParameterDef(CMC_AE_TARGET_MIN_FCM);

        //Sensor Max Gain
        parameters.addParameterDef(CMC_SENSOR_MAX_GAIN_FCM);
        #endif //INFOTM_ENABLE_COLOR_MODE_CHANGE

        #if defined(INFOTM_ENABLE_COLOR_MODE_CHANGE) && defined(INFOTM_ENABLE_GAIN_LEVEL_IDX)
        parameters.addParameterDef(CMC_DN_TARGET_IDX_FCM_LV1);

        parameters.addParameterDef(CMC_BLC_SENSOR_BLACK_FCM_LV1);
        parameters.addParameterDef(CMC_BLC_SYS_BLACK_FCM_LV1);

        parameters.addParameterDef(CMC_DNS_STRENGTH_FCM_LV1);
        parameters.addParameterDef(CMC_DNS_READ_NOISE_FCM_LV1);
        parameters.addParameterDef(CMC_DNS_WELLDEPTH_FCM_LV1);

        parameters.addParameterDef(CMC_SHA_RADIUS_FCM_LV1);
        parameters.addParameterDef(CMC_SHA_STRENGTH_FCM_LV1);
        parameters.addParameterDef(CMC_SHA_THRESH_FCM_LV1);
        parameters.addParameterDef(CMC_SHA_DETAIL_FCM_LV1);
        parameters.addParameterDef(CMC_SHA_EDGE_SCALE_FCM_LV1);
        parameters.addParameterDef(CMC_SHA_EDGE_OFFSET_FCM_LV1);
        parameters.addParameterDef(CMC_SHA_DENOISE_BYPASS_FCM_LV1);
        parameters.addParameterDef(CMC_SHADN_TAU_FCM_LV1);
        parameters.addParameterDef(CMC_SHADN_SIGMA_FCM_LV1);

        parameters.addParameterDef(CMC_R2Y_CONTRAST_FCM_LV1);
        parameters.addParameterDef(CMC_R2Y_SATURATION_FCM_LV1);
        parameters.addParameterDef(CMC_R2Y_BRIGHTNESS_FCM_LV1);
        parameters.addParameterDef(CMC_R2Y_RANGE_MUL_FCM_LV1);

        parameters.addParameterDef(CMC_HIS_GRID_START_COORDS_FCM_LV1);
        parameters.addParameterDef(CMC_HIS_GRID_TILE_DIMENSIONS_FCM_LV1);

        parameters.addParameterDef(CMC_DPF_WEIGHT_FCM_LV1);
        parameters.addParameterDef(CMC_DPF_THRESHOLD_FCM_LV1);

        parameters.addParameterDef(CMC_TNMC_HISTMIN_FCM_LV1);
        parameters.addParameterDef(CMC_TNMC_HISTMAX_FCM_LV1);
        parameters.addParameterDef(CMC_TNMC_ADAPTIVE_FCM_LV1);

        parameters.addParameterDef(CMC_TNM_IN_Y_FCM_LV1);
        parameters.addParameterDef(CMC_TNM_OUT_Y_FCM_LV1);
        parameters.addParameterDef(CMC_TNM_OUT_C_FCM_LV1);
        parameters.addParameterDef(CMC_TNM_FLAT_FACTOR_FCM_LV1);
        parameters.addParameterDef(CMC_TNM_WEIGHT_LINE_FCM_LV1);
        parameters.addParameterDef(CMC_TNM_COLOUR_CONFIDENCE_FCM_LV1);
        parameters.addParameterDef(CMC_TNM_COLOUR_SATURATION_FCM_LV1);
        parameters.addParameterDef(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_FCM_LV1);
        parameters.addParameterDef(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_FCM_LV1);
        parameters.addParameterDef(CMC_TNMC_OVERSHOT_THRESHOLD_FCM_LV1);
        parameters.addParameterDef(CMC_TNMC_WDR_CEILING_FCM_LV1);
        parameters.addParameterDef(CMC_TNMC_WDR_FLOOR_FCM_LV1);
        parameters.addParameterDef(CMC_TNMC_GAMMA_CRV_MODE_FCM_LV1);
        parameters.addParameterDef(CMC_TNMC_GAMMA_FCM_LV1);
        parameters.addParameterDef(CMC_TNMC_BEZIER_CTRL_PNT_FCM_LV1);

        parameters.addParameterDef(CMC_DN_TARGET_IDX_FCM_LV2);

        parameters.addParameterDef(CMC_BLC_SENSOR_BLACK_FCM_LV2);
        parameters.addParameterDef(CMC_BLC_SYS_BLACK_FCM_LV2);

        parameters.addParameterDef(CMC_DNS_STRENGTH_FCM_LV2);
        parameters.addParameterDef(CMC_DNS_READ_NOISE_FCM_LV2);
        parameters.addParameterDef(CMC_DNS_WELLDEPTH_FCM_LV2);

        parameters.addParameterDef(CMC_SHA_RADIUS_FCM_LV2);
        parameters.addParameterDef(CMC_SHA_STRENGTH_FCM_LV2);
        parameters.addParameterDef(CMC_SHA_THRESH_FCM_LV2);
        parameters.addParameterDef(CMC_SHA_DETAIL_FCM_LV2);
        parameters.addParameterDef(CMC_SHA_EDGE_SCALE_FCM_LV2);
        parameters.addParameterDef(CMC_SHA_EDGE_OFFSET_FCM_LV2);
        parameters.addParameterDef(CMC_SHA_DENOISE_BYPASS_FCM_LV2);
        parameters.addParameterDef(CMC_SHADN_TAU_FCM_LV2);
        parameters.addParameterDef(CMC_SHADN_SIGMA_FCM_LV2);

        parameters.addParameterDef(CMC_R2Y_CONTRAST_FCM_LV2);
        parameters.addParameterDef(CMC_R2Y_SATURATION_FCM_LV2);
        parameters.addParameterDef(CMC_R2Y_BRIGHTNESS_FCM_LV2);
        parameters.addParameterDef(CMC_R2Y_RANGE_MUL_FCM_LV2);

        parameters.addParameterDef(CMC_HIS_GRID_START_COORDS_FCM_LV2);
        parameters.addParameterDef(CMC_HIS_GRID_TILE_DIMENSIONS_FCM_LV2);

        parameters.addParameterDef(CMC_DPF_WEIGHT_FCM_LV2);
        parameters.addParameterDef(CMC_DPF_THRESHOLD_FCM_LV2);

        parameters.addParameterDef(CMC_TNMC_HISTMIN_FCM_LV2);
        parameters.addParameterDef(CMC_TNMC_HISTMAX_FCM_LV2);
        parameters.addParameterDef(CMC_TNMC_ADAPTIVE_FCM_LV2);

        parameters.addParameterDef(CMC_TNM_IN_Y_FCM_LV2);
        parameters.addParameterDef(CMC_TNM_OUT_Y_FCM_LV2);
        parameters.addParameterDef(CMC_TNM_OUT_C_FCM_LV2);
        parameters.addParameterDef(CMC_TNM_FLAT_FACTOR_FCM_LV2);
        parameters.addParameterDef(CMC_TNM_WEIGHT_LINE_FCM_LV2);
        parameters.addParameterDef(CMC_TNM_COLOUR_CONFIDENCE_FCM_LV2);
        parameters.addParameterDef(CMC_TNM_COLOUR_SATURATION_FCM_LV2);
        parameters.addParameterDef(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_FCM_LV2);
        parameters.addParameterDef(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_FCM_LV2);
        parameters.addParameterDef(CMC_TNMC_OVERSHOT_THRESHOLD_FCM_LV2);
        parameters.addParameterDef(CMC_TNMC_WDR_CEILING_FCM_LV2);
        parameters.addParameterDef(CMC_TNMC_WDR_FLOOR_FCM_LV2);
        parameters.addParameterDef(CMC_TNMC_GAMMA_CRV_MODE_FCM_LV2);
        parameters.addParameterDef(CMC_TNMC_GAMMA_FCM_LV2);
        parameters.addParameterDef(CMC_TNMC_BEZIER_CTRL_PNT_FCM_LV2);

        if (bEnableInterpolationGamma) {
            parameters.addParameterDef(CMC_DN_TARGET_IDX_FCM_LV3);

            parameters.addParameterDef(CMC_BLC_SENSOR_BLACK_FCM_LV3);
            parameters.addParameterDef(CMC_BLC_SYS_BLACK_FCM_LV3);

            parameters.addParameterDef(CMC_DNS_STRENGTH_FCM_LV3);
            parameters.addParameterDef(CMC_DNS_READ_NOISE_FCM_LV3);
            parameters.addParameterDef(CMC_DNS_WELLDEPTH_FCM_LV3);

            parameters.addParameterDef(CMC_SHA_RADIUS_FCM_LV3);
            parameters.addParameterDef(CMC_SHA_STRENGTH_FCM_LV3);
            parameters.addParameterDef(CMC_SHA_THRESH_FCM_LV3);
            parameters.addParameterDef(CMC_SHA_DETAIL_FCM_LV3);
            parameters.addParameterDef(CMC_SHA_EDGE_SCALE_FCM_LV3);
            parameters.addParameterDef(CMC_SHA_EDGE_OFFSET_FCM_LV3);
            parameters.addParameterDef(CMC_SHA_DENOISE_BYPASS_FCM_LV3);
            parameters.addParameterDef(CMC_SHADN_TAU_FCM_LV3);
            parameters.addParameterDef(CMC_SHADN_SIGMA_FCM_LV3);

            parameters.addParameterDef(CMC_R2Y_CONTRAST_FCM_LV3);
            parameters.addParameterDef(CMC_R2Y_SATURATION_FCM_LV3);
            parameters.addParameterDef(CMC_R2Y_BRIGHTNESS_FCM_LV3);
            parameters.addParameterDef(CMC_R2Y_RANGE_MUL_FCM_LV3);

            parameters.addParameterDef(CMC_HIS_GRID_START_COORDS_FCM_LV3);
            parameters.addParameterDef(CMC_HIS_GRID_TILE_DIMENSIONS_FCM_LV3);

            parameters.addParameterDef(CMC_DPF_WEIGHT_FCM_LV3);
            parameters.addParameterDef(CMC_DPF_THRESHOLD_FCM_LV3);

            parameters.addParameterDef(CMC_TNMC_HISTMIN_FCM_LV3);
            parameters.addParameterDef(CMC_TNMC_HISTMAX_FCM_LV3);
            parameters.addParameterDef(CMC_TNMC_ADAPTIVE_FCM_LV3);

            parameters.addParameterDef(CMC_TNM_IN_Y_FCM_LV3);
            parameters.addParameterDef(CMC_TNM_OUT_Y_FCM_LV3);
            parameters.addParameterDef(CMC_TNM_OUT_C_FCM_LV3);
            parameters.addParameterDef(CMC_TNM_FLAT_FACTOR_FCM_LV3);
            parameters.addParameterDef(CMC_TNM_WEIGHT_LINE_FCM_LV3);
            parameters.addParameterDef(CMC_TNM_COLOUR_CONFIDENCE_FCM_LV3);
            parameters.addParameterDef(CMC_TNM_COLOUR_SATURATION_FCM_LV3);
            parameters.addParameterDef(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_FCM_LV3);
            parameters.addParameterDef(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_FCM_LV3);
            parameters.addParameterDef(CMC_TNMC_OVERSHOT_THRESHOLD_FCM_LV3);
            parameters.addParameterDef(CMC_TNMC_WDR_CEILING_FCM_LV3);
            parameters.addParameterDef(CMC_TNMC_WDR_FLOOR_FCM_LV3);
            parameters.addParameterDef(CMC_TNMC_GAMMA_CRV_MODE_FCM_LV3);
            parameters.addParameterDef(CMC_TNMC_GAMMA_FCM_LV3);
            parameters.addParameterDef(CMC_TNMC_BEZIER_CTRL_PNT_FCM_LV3);
        }
        #endif //INFOTM_ENABLE_COLOR_MODE_CHANGE, INFOTM_ENABLE_GAIN_LEVEL_IDX

        //3D-Denoise target index.
        parameters.addParameterDef(CMC_DN_TARGET_IDX_ACM);

        #if defined(INFOTM_ENABLE_COLOR_MODE_CHANGE)
        ///////////////////////////////////////Advanced Color Mode setting
        if (bCmcEnable) {
            //BLC SENSOR BLACK
            parameters.addParameterDef(CMC_BLC_SENSOR_BLACK_ACM);
            parameters.addParameterDef(CMC_BLC_SYS_BLACK_ACM);

            //DNS
            parameters.addParameterDef(CMC_DNS_STRENGTH_ACM);
            parameters.addParameterDef(CMC_DNS_READ_NOISE_ACM);
            parameters.addParameterDef(CMC_DNS_WELLDEPTH_ACM);

            //SHA
            parameters.addParameterDef(CMC_SHA_RADIUS_ACM);
            parameters.addParameterDef(CMC_SHA_STRENGTH_ACM);
            parameters.addParameterDef(CMC_SHA_THRESH_ACM);
            parameters.addParameterDef(CMC_SHA_DETAIL_ACM);
            parameters.addParameterDef(CMC_SHA_EDGE_SCALE_ACM);
            parameters.addParameterDef(CMC_SHA_EDGE_OFFSET_ACM);
            parameters.addParameterDef(CMC_SHA_DENOISE_BYPASS_ACM);
            parameters.addParameterDef(CMC_SHADN_TAU_ACM);
            parameters.addParameterDef(CMC_SHADN_SIGMA_ACM);

            //R2Y
            parameters.addParameterDef(CMC_R2Y_BRIGHTNESS_ACM);
            parameters.addParameterDef(CMC_R2Y_CONTRAST_ACM);
            parameters.addParameterDef(CMC_R2Y_SATURATION_ACM);
            parameters.addParameterDef(CMC_R2Y_RANGE_MUL_ACM);

            //HIS
            parameters.addParameterDef(CMC_HIS_GRID_START_COORDS_ACM);
            parameters.addParameterDef(CMC_HIS_GRID_TILE_DIMENSIONS_ACM);

            //DPF
            parameters.addParameterDef(CMC_DPF_WEIGHT_ACM);
            parameters.addParameterDef(CMC_DPF_THRESHOLD_ACM);

            //TNM
            parameters.addParameterDef(CMC_TNMC_HISTMIN_ACM);
            parameters.addParameterDef(CMC_TNMC_HISTMAX_ACM);
            parameters.addParameterDef(CMC_TNMC_ADAPTIVE_ACM);

            parameters.addParameterDef(CMC_TNM_IN_Y_ACM);
            parameters.addParameterDef(CMC_TNM_OUT_Y_ACM);
            parameters.addParameterDef(CMC_TNM_OUT_C_ACM);
            parameters.addParameterDef(CMC_TNM_FLAT_FACTOR_ACM);
            parameters.addParameterDef(CMC_TNM_WEIGHT_LINE_ACM);
            parameters.addParameterDef(CMC_TNM_COLOUR_CONFIDENCE_ACM);
            parameters.addParameterDef(CMC_TNM_COLOUR_SATURATION_ACM);
            parameters.addParameterDef(CMC_TNMC_ENABLE_GAMMA_ACM);
            parameters.addParameterDef(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_ACM);
            parameters.addParameterDef(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_ACM);
            parameters.addParameterDef(CMC_TNMC_OVERSHOT_THRESHOLD_ACM);
            parameters.addParameterDef(CMC_TNMC_WDR_CEILING_ACM);
            parameters.addParameterDef(CMC_TNMC_WDR_FLOOR_ACM);
            parameters.addParameterDef(CMC_TNMC_GAMMA_CRV_MODE_ACM);
            parameters.addParameterDef(CMC_TNMC_GAMMA_ACM);
            parameters.addParameterDef(CMC_TNMC_BEZIER_CTRL_PNT_ACM);

            //AE
            parameters.addParameterDef(CMC_AE_TARGET_BRIGHTNESS_ACM);
            parameters.addParameterDef(CMC_AE_TARGET_MIN_ACM);

            //Sensor Max Gain
            parameters.addParameterDef(CMC_SENSOR_MAX_GAIN_ACM);
        }
        #endif //INFOTM_ENABLE_COLOR_MODE_CHANGE

        #if defined(INFOTM_ENABLE_GAIN_LEVEL_IDX)
        parameters.addParameterDef(CMC_SENSOR_GAIN_LEVEL_CTRL);
        parameters.addParameterDef(CMC_SENSOR_GAIN_LV1);
        parameters.addParameterDef(CMC_SENSOR_GAIN_LV2);
        parameters.addParameterDef(CMC_SENSOR_GAIN_LV1_INTERPOLATION);
        parameters.addParameterDef(CMC_SENSOR_GAIN_LV2_INTERPOLATION);
        parameters.addParameterDef(CMC_ENABLE_INTERPOLATION_GAMMA);
        parameters.addParameterDef(CMC_INTERPOLATION_GAMMA_0);
        parameters.addParameterDef(CMC_INTERPOLATION_GAMMA_1);
        parameters.addParameterDef(CMC_INTERPOLATION_GAMMA_2);

        parameters.addParameterDef(CMC_CCM_ATTENUATION_LV1);
        parameters.addParameterDef(CMC_CCM_ATTENUATION_LV2);
        parameters.addParameterDef(CMC_CCM_ATTENUATION_LV3);

        parameters.addParameterDef(CMC_DN_TARGET_IDX_ACM_LV1);

        parameters.addParameterDef(CMC_BLC_SENSOR_BLACK_ACM_LV1);
        parameters.addParameterDef(CMC_BLC_SYS_BLACK_ACM_LV1);

        parameters.addParameterDef(CMC_DNS_STRENGTH_ACM_LV1);
        parameters.addParameterDef(CMC_DNS_READ_NOISE_ACM_LV1);
        parameters.addParameterDef(CMC_DNS_WELLDEPTH_ACM_LV1);

        parameters.addParameterDef(CMC_SHA_RADIUS_ACM_LV1);
        parameters.addParameterDef(CMC_SHA_STRENGTH_ACM_LV1);
        parameters.addParameterDef(CMC_SHA_THRESH_ACM_LV1);
        parameters.addParameterDef(CMC_SHA_DETAIL_ACM_LV1);
        parameters.addParameterDef(CMC_SHA_EDGE_SCALE_ACM_LV1);
        parameters.addParameterDef(CMC_SHA_EDGE_OFFSET_ACM_LV1);
        parameters.addParameterDef(CMC_SHA_DENOISE_BYPASS_ACM_LV1);
        parameters.addParameterDef(CMC_SHADN_TAU_ACM_LV1);
        parameters.addParameterDef(CMC_SHADN_SIGMA_ACM_LV1);

        parameters.addParameterDef(CMC_R2Y_CONTRAST_ACM_LV1);
        parameters.addParameterDef(CMC_R2Y_SATURATION_ACM_LV1);
        parameters.addParameterDef(CMC_R2Y_BRIGHTNESS_ACM_LV1);
        parameters.addParameterDef(CMC_R2Y_RANGE_MUL_ACM_LV1);

        parameters.addParameterDef(CMC_HIS_GRID_START_COORDS_ACM_LV1);
        parameters.addParameterDef(CMC_HIS_GRID_TILE_DIMENSIONS_ACM_LV1);

        parameters.addParameterDef(CMC_DPF_WEIGHT_ACM_LV1);
        parameters.addParameterDef(CMC_DPF_THRESHOLD_ACM_LV1);

        parameters.addParameterDef(CMC_TNMC_HISTMIN_ACM_LV1);
        parameters.addParameterDef(CMC_TNMC_HISTMAX_ACM_LV1);
        parameters.addParameterDef(CMC_TNMC_ADAPTIVE_ACM_LV1);

        parameters.addParameterDef(CMC_TNM_IN_Y_ACM_LV1);
        parameters.addParameterDef(CMC_TNM_OUT_Y_ACM_LV1);
        parameters.addParameterDef(CMC_TNM_OUT_C_ACM_LV1);
        parameters.addParameterDef(CMC_TNM_FLAT_FACTOR_ACM_LV1);
        parameters.addParameterDef(CMC_TNM_WEIGHT_LINE_ACM_LV1);
        parameters.addParameterDef(CMC_TNM_COLOUR_CONFIDENCE_ACM_LV1);
        parameters.addParameterDef(CMC_TNM_COLOUR_SATURATION_ACM_LV1);
        parameters.addParameterDef(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_ACM_LV1);
        parameters.addParameterDef(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_ACM_LV1);
        parameters.addParameterDef(CMC_TNMC_OVERSHOT_THRESHOLD_ACM_LV1);
        parameters.addParameterDef(CMC_TNMC_WDR_CEILING_ACM_LV1);
        parameters.addParameterDef(CMC_TNMC_WDR_FLOOR_ACM_LV1);
        parameters.addParameterDef(CMC_TNMC_GAMMA_CRV_MODE_ACM_LV1);
        parameters.addParameterDef(CMC_TNMC_GAMMA_ACM_LV1);
        parameters.addParameterDef(CMC_TNMC_BEZIER_CTRL_PNT_ACM_LV1);

        parameters.addParameterDef(CMC_DN_TARGET_IDX_ACM_LV2);

        parameters.addParameterDef(CMC_BLC_SENSOR_BLACK_ACM_LV2);
        parameters.addParameterDef(CMC_BLC_SYS_BLACK_ACM_LV2);

        parameters.addParameterDef(CMC_DNS_STRENGTH_ACM_LV2);
        parameters.addParameterDef(CMC_DNS_READ_NOISE_ACM_LV2);
        parameters.addParameterDef(CMC_DNS_WELLDEPTH_ACM_LV2);

        parameters.addParameterDef(CMC_SHA_RADIUS_ACM_LV2);
        parameters.addParameterDef(CMC_SHA_STRENGTH_ACM_LV2);
        parameters.addParameterDef(CMC_SHA_THRESH_ACM_LV2);
        parameters.addParameterDef(CMC_SHA_DETAIL_ACM_LV2);
        parameters.addParameterDef(CMC_SHA_EDGE_SCALE_ACM_LV2);
        parameters.addParameterDef(CMC_SHA_EDGE_OFFSET_ACM_LV2);
        parameters.addParameterDef(CMC_SHA_DENOISE_BYPASS_ACM_LV2);
        parameters.addParameterDef(CMC_SHADN_TAU_ACM_LV2);
        parameters.addParameterDef(CMC_SHADN_SIGMA_ACM_LV2);

        parameters.addParameterDef(CMC_R2Y_CONTRAST_ACM_LV2);
        parameters.addParameterDef(CMC_R2Y_SATURATION_ACM_LV2);
        parameters.addParameterDef(CMC_R2Y_BRIGHTNESS_ACM_LV2);
        parameters.addParameterDef(CMC_R2Y_RANGE_MUL_ACM_LV2);

        parameters.addParameterDef(CMC_HIS_GRID_START_COORDS_ACM_LV2);
        parameters.addParameterDef(CMC_HIS_GRID_TILE_DIMENSIONS_ACM_LV2);

        parameters.addParameterDef(CMC_DPF_WEIGHT_ACM_LV2);
        parameters.addParameterDef(CMC_DPF_THRESHOLD_ACM_LV2);

        parameters.addParameterDef(CMC_TNMC_HISTMIN_ACM_LV2);
        parameters.addParameterDef(CMC_TNMC_HISTMAX_ACM_LV2);
        parameters.addParameterDef(CMC_TNMC_ADAPTIVE_ACM_LV2);

        parameters.addParameterDef(CMC_TNM_IN_Y_ACM_LV2);
        parameters.addParameterDef(CMC_TNM_OUT_Y_ACM_LV2);
        parameters.addParameterDef(CMC_TNM_OUT_C_ACM_LV2);
        parameters.addParameterDef(CMC_TNM_FLAT_FACTOR_ACM_LV2);
        parameters.addParameterDef(CMC_TNM_WEIGHT_LINE_ACM_LV2);
        parameters.addParameterDef(CMC_TNM_COLOUR_CONFIDENCE_ACM_LV2);
        parameters.addParameterDef(CMC_TNM_COLOUR_SATURATION_ACM_LV2);
        parameters.addParameterDef(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_ACM_LV2);
        parameters.addParameterDef(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_ACM_LV2);
        parameters.addParameterDef(CMC_TNMC_OVERSHOT_THRESHOLD_ACM_LV2);
        parameters.addParameterDef(CMC_TNMC_WDR_CEILING_ACM_LV2);
        parameters.addParameterDef(CMC_TNMC_WDR_FLOOR_ACM_LV2);
        parameters.addParameterDef(CMC_TNMC_GAMMA_CRV_MODE_ACM_LV2);
        parameters.addParameterDef(CMC_TNMC_GAMMA_ACM_LV2);
        parameters.addParameterDef(CMC_TNMC_BEZIER_CTRL_PNT_ACM_LV2);

        if (bEnableInterpolationGamma) {
            parameters.addParameterDef(CMC_DN_TARGET_IDX_ACM_LV3);

            parameters.addParameterDef(CMC_BLC_SENSOR_BLACK_ACM_LV3);
            parameters.addParameterDef(CMC_BLC_SYS_BLACK_ACM_LV3);

            parameters.addParameterDef(CMC_DNS_STRENGTH_ACM_LV3);
            parameters.addParameterDef(CMC_DNS_READ_NOISE_ACM_LV3);
            parameters.addParameterDef(CMC_DNS_WELLDEPTH_ACM_LV3);

            parameters.addParameterDef(CMC_SHA_RADIUS_ACM_LV3);
            parameters.addParameterDef(CMC_SHA_STRENGTH_ACM_LV3);
            parameters.addParameterDef(CMC_SHA_THRESH_ACM_LV3);
            parameters.addParameterDef(CMC_SHA_DETAIL_ACM_LV3);
            parameters.addParameterDef(CMC_SHA_EDGE_SCALE_ACM_LV3);
            parameters.addParameterDef(CMC_SHA_EDGE_OFFSET_ACM_LV3);
            parameters.addParameterDef(CMC_SHA_DENOISE_BYPASS_ACM_LV3);
            parameters.addParameterDef(CMC_SHADN_TAU_ACM_LV3);
            parameters.addParameterDef(CMC_SHADN_SIGMA_ACM_LV3);

            parameters.addParameterDef(CMC_R2Y_CONTRAST_ACM_LV3);
            parameters.addParameterDef(CMC_R2Y_SATURATION_ACM_LV3);
            parameters.addParameterDef(CMC_R2Y_BRIGHTNESS_ACM_LV3);
            parameters.addParameterDef(CMC_R2Y_RANGE_MUL_ACM_LV3);

            parameters.addParameterDef(CMC_HIS_GRID_START_COORDS_ACM_LV3);
            parameters.addParameterDef(CMC_HIS_GRID_TILE_DIMENSIONS_ACM_LV3);

            parameters.addParameterDef(CMC_DPF_WEIGHT_ACM_LV3);
            parameters.addParameterDef(CMC_DPF_THRESHOLD_ACM_LV3);

            parameters.addParameterDef(CMC_TNMC_HISTMIN_ACM_LV3);
            parameters.addParameterDef(CMC_TNMC_HISTMAX_ACM_LV3);
            parameters.addParameterDef(CMC_TNMC_ADAPTIVE_ACM_LV3);

            parameters.addParameterDef(CMC_TNM_IN_Y_ACM_LV3);
            parameters.addParameterDef(CMC_TNM_OUT_Y_ACM_LV3);
            parameters.addParameterDef(CMC_TNM_OUT_C_ACM_LV3);
            parameters.addParameterDef(CMC_TNM_FLAT_FACTOR_ACM_LV3);
            parameters.addParameterDef(CMC_TNM_WEIGHT_LINE_ACM_LV3);
            parameters.addParameterDef(CMC_TNM_COLOUR_CONFIDENCE_ACM_LV3);
            parameters.addParameterDef(CMC_TNM_COLOUR_SATURATION_ACM_LV3);
            parameters.addParameterDef(CMC_TNMC_EQUAL_BRIGHTSUPRESSRATIO_ACM_LV3);
            parameters.addParameterDef(CMC_TNMC_EQUAL_DARKSUPRESSRATIO_ACM_LV3);
            parameters.addParameterDef(CMC_TNMC_OVERSHOT_THRESHOLD_ACM_LV3);
            parameters.addParameterDef(CMC_TNMC_WDR_CEILING_ACM_LV3);
            parameters.addParameterDef(CMC_TNMC_WDR_FLOOR_ACM_LV3);
            parameters.addParameterDef(CMC_TNMC_GAMMA_CRV_MODE_ACM_LV3);
            parameters.addParameterDef(CMC_TNMC_GAMMA_ACM_LV3);
            parameters.addParameterDef(CMC_TNMC_BEZIER_CTRL_PNT_ACM_LV3);
        }
        #endif //INFOTM_ENABLE_GAIN_LEVEL_IDX
        break;
    }
...
bool ISPC::ControlCMC::hasConverged() const
{
    return false;
}
