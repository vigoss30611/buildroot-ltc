...
const ISPC::ParamDef<double> ISPC::ControlAWB::AWB_ESTIMATION_SCALE(
    "WB_ESTIMATION_SCALE", 0.0f, 20.0f, 1.0f);
const ISPC::ParamDef<double> ISPC::ControlAWB::AWB_ESTIMATION_OFFSET(
    "WB_ESTIMATION_OFFSET", -6500.0f, 6500.0f, 0.0f);
const ISPC::ParamDef<double> ISPC::ControlAWB::AWB_TARGET_TEMPERATURE(
    "WB_TARGET_TEMPERATURE", 0.0f, AWB_MAX_TEMP, 6500.0f);
const ISPC::ParamDef<double> ISPC::ControlAWB::AWB_TARGET_PIXELRATIO(
...
IMG_RESULT ISPC::ControlAWB::setPipelineOwner(Pipeline *pipeline)
{
    IMG_RESULT ret;
    ret = ControlModule::setPipelineOwner(pipeline);
    if (ret)
    {
        MOD_LOG_ERROR("parent's setPipelineOwner failed");
        return ret;
    }
    ...
    ModuleBase::SaveType t) const
{
    ...
    {
    case SAVE_VAL:
        parameters.addParameter(AWB_ESTIMATION_SCALE, this->estimationScale);
        parameters.addParameter(AWB_ESTIMATION_OFFSET, this->estimationOffset);
        parameters.addParameter(AWB_TARGET_TEMPERATURE, this->targetTemperature);
        parameters.addParameter(AWB_TARGET_PIXELRATIO, this->targetPixelRatio);
        break;

    case SAVE_MIN:
        parameters.addParameterMin(AWB_ESTIMATION_SCALE);
        parameters.addParameterMin(AWB_ESTIMATION_OFFSET);
        parameters.addParameterMin(AWB_TARGET_TEMPERATURE);
        parameters.addParameterMin(AWB_TARGET_PIXELRATIO);
        break;

    case SAVE_MAX:
        parameters.addParameterMax(AWB_ESTIMATION_SCALE);
        parameters.addParameterMax(AWB_ESTIMATION_OFFSET);
        parameters.addParameterMax(AWB_TARGET_TEMPERATURE);
        parameters.addParameterMax(AWB_TARGET_PIXELRATIO);
        break;

    case SAVE_DEF:
        parameters.addParameterDef(AWB_ESTIMATION_SCALE);
        parameters.addParameterDef(AWB_ESTIMATION_OFFSET);
        parameters.addParameterDef(AWB_TARGET_TEMPERATURE);
        parameters.addParameterDef(AWB_TARGET_PIXELRATIO);
        break;
    }
...
ISPC::ControlAWB::getTemperatureCorrections()
{
 ...
 ISPC::TemperatureCorrection&
ISPC::ControlAWB::getTemperatureCorrectionsMutable()
{
    return this->colorTempCorrection;
}
...
bool ISPC::ControlAWB::hasConverged() const
{
