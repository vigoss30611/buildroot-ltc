...
IMG_RESULT ISPC::Sensor::waitProcessed()
{
    return Sensor_WaitProcessed(hSensorHandle);
}
    ...
    ModuleBase::SaveType t) const
{
    ...
    case ModuleBase::SAVE_VAL:
        parameters.addParameter(SENSOR_EXPOSURE,
                this->programmedExposure/1000.0);
        parameters.addParameter(SENSOR_GAIN, this->programmedGain);
        parameters.addParameter(SENSOR_BITDEPTH, this->uiBitDepth);
        parameters.addParameter(SENSOR_WELLDEPTH, this->uiWellDepth);
        parameters.addParameter(SENSOR_READNOISE, this->flReadNoise);
        parameters.addParameter(SENSOR_FRAMERATE, this->flFrameRate);

        values.clear();
        values.push_back(toString(uiWidth));
        values.push_back(toString(uiHeight));
        parameters.addParameter(SENSOR_SIZE, values);
        parameters.addParameter(SENSOR_VTOT, this->nVTot);
        break;

    case ModuleBase::SAVE_MIN:
        parameters.addParameterMin(SENSOR_EXPOSURE);
        parameters.addParameterMin(SENSOR_GAIN);
        parameters.addParameterMin(SENSOR_BITDEPTH);
        parameters.addParameterMin(SENSOR_WELLDEPTH);
        parameters.addParameterMin(SENSOR_READNOISE);
        parameters.addParameterMin(SENSOR_FRAMERATE);
        parameters.addParameterMin(SENSOR_SIZE);
        parameters.addParameterMin(SENSOR_VTOT);
        break;

    case ModuleBase::SAVE_MAX:
        parameters.addParameterMax(SENSOR_EXPOSURE);
        parameters.addParameterMax(SENSOR_GAIN);
        parameters.addParameterMax(SENSOR_BITDEPTH);
        parameters.addParameterMax(SENSOR_WELLDEPTH);
        parameters.addParameterMax(SENSOR_READNOISE);
        parameters.addParameterMax(SENSOR_FRAMERATE);
        parameters.addParameterMax(SENSOR_SIZE);
        parameters.addParameterMax(SENSOR_VTOT);
        break;

    case ModuleBase::SAVE_DEF:
        parameters.addParameterDef(SENSOR_EXPOSURE);
        parameters.addParameterDef(SENSOR_GAIN);
        parameters.addParameterDef(SENSOR_BITDEPTH);
        parameters.addParameterDef(SENSOR_WELLDEPTH);
        parameters.addParameterDef(SENSOR_READNOISE);
        parameters.addParameterDef(SENSOR_FRAMERATE);
        values.clear();
        for (i = 0; i < 2; i++)
        {
        ...
        parameters.addParameter(SENSOR_SIZE, values);
        parameters.getParameter(SENSOR_SIZE.name)->setInfo(
                getParameterInfo(SENSOR_SIZE));
        parameters.addParameterDef(SENSOR_VTOT);
        break;
    }
    ...
    : Sensor(), isInternal(_isInternal)
{
    ...
    {
#ifdef INFOTM_ISP
        IMG_BOOL8 bIsVideo = IMG_FALSE;  // always off
#else
    ...
    else
    {
        ...
        if (ret)
        {
            LOG_ERROR("Failed to set connection\n");
            state = SENSOR_ERROR;
            return;
        }
        ...
        if (ret)
        {
            LOG_ERROR("Failed to set gasket\n");
            state = SENSOR_ERROR;
            return;
        }
...
IMG_UINT8* ISPC::Sensor::ReadSensorCalibrationData(int infotm_method, int sensor_index/*0 or 1*/, IMG_FLOAT awb_convert_gain, IMG_UINT16* otp_calibration_version)
{
/*
	if(state != SENSOR_ENABLED)
	{
...
*/
	return Sensor_ReadSensorCalibrationData(hSensorHandle, infotm_method, sensor_index, awb_convert_gain, otp_calibration_version);
}

IMG_UINT8* ISPC::Sensor::ReadSensorCalibrationVersion(int infotm_method, int sensor_index/*0 or 1*/, IMG_UINT16* otp_calibration_version)
{
/*
	if(state != SENSOR_ENABLED)
	{
		LOG_ERROR("Sensor is in state %s, expecting %s\n",
			Sensor::StateName(state),
			Sensor::StateName(SENSOR_ENABLED));
		return NULL;//IMG_ERROR_NOT_SUPPORTED;
	}
*/
	return Sensor_ReadSensorCalibrationVersion(hSensorHandle, infotm_method, sensor_index, otp_calibration_version);
}

IMG_RESULT ISPC::Sensor::UpdateSensorWBGain(int infotm_method, IMG_FLOAT awb_convet_gain)
{
/*
	if(state != SENSOR_ENABLED)
	{
...
*/
	return Sensor_UpdateSensorWBGain(hSensorHandle, infotm_method, awb_convet_gain);
}

IMG_RESULT ISPC::Sensor::ExchangeCalibDirection(int flag)
{
    return Sensor_ExchangeCalibDirection(hSensorHandle, flag);
}
