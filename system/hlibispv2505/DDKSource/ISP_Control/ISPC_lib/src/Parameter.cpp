...
template <>
std::string ISPC::toString<char>(const char &value)
{
    return ISPC::toString(int(value));
}
...
template <>
char ISPC::parse<char>(const std::string &str, bool &isOk)
{
    return parse<int>(str, isOk);
}
...
ISPC::Parameter& ISPC::Parameter::setInfo(const std::string& newInfo)\
{
    info = newInfo;
    return *this;
}

const std::string& ISPC::Parameter::getInfo() const
{
    return info;
}
...
const std::vector<std::string>& ISPC::Parameter::getValues(void) const
{
    return values;
}

bool ISPC::operator==(const Parameter &lhs, const Parameter &rhs)
{
    if (lhs.getTag() != rhs.getTag() ||
        lhs.size() != rhs.size())
        return false;

    std::vector<std::string>::const_iterator thisIt = lhs.begin();
    std::vector<std::string>::const_iterator rhsIt = rhs.begin();
    while (thisIt != lhs.end() && rhsIt != rhs.end())
...
std::string ISPC::getParameterInfo(const ParamDef<short int> &t)
{
    std::ostringstream os;
    os << "int range=[" << t.min << "," << t.max << "]";
    return os.str();
...
std::string ISPC::getParameterInfo(const ParamDefArray<short int> &t)
{
    std::ostringstream os;
...
std::ostream& operator<< (std::ostream& out, const ISPC::Parameter& param)
{
    out << param.getTag() << " ";
