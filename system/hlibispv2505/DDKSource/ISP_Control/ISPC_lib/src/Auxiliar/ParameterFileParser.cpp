...
IMG_RESULT ISPC::ParameterFileParser::parse(
    ...
    const std::string &_line)
{
    ...
    // first find starting position of a comment
    for (int c = 0; c < N_COMMENTS; c++)
    {
        size_t pos = line.find(commentStr[c]);
    ...
    std::ostream &file)
{
    ...
    while (it != parameters.end())
    {
        if (!it->first.empty() && it->second.size() > 0)
        {
            ...
            if(!it->second.getInfo().empty())
            {
                file << " // " << it->second.getInfo();
            }
            file << std::endl;
        }
        else
    ...
    if (bSaveUngroupped)
    {
        ...
        while (paramIt != parameters.end() && !file.fail())
        {
                ...
                && !parameters.isInGroup(paramIt->first))
            {
                ...
                // save description string
                if(!paramIt->second.getInfo().empty())
                {
                    file << " // " << paramIt->second.getInfo();
                }
                file << std::endl;
            }
