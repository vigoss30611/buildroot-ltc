...
TEST(CI_Driver, init_finalise)
{
    ...
    EXPECT_EQ (IMG_ERROR_INVALID_PARAMETERS, KRN_CI_DriverCreate(&sCIDriver, 0, 256, 0, CI_DEF_GMACURVE, NULL));
...
TEST(CI_Driver, linestore_verif)
{
    ...
    // 8k
    EXPECT_EQ (g_context_max_width, pConnection->sHWInfo.ui32MaxLineStore) << "line store value";
...
TEST(CI_Driver, linestore_greedy0)
{
    ...
    if (krnPipe){
        krnPipe->pipelineConfig.ui8Context = 1;
    }
    else
    {
        ADD_FAILURE();
    }
    driver.pPipeline->config.ui8Context = 1;
    // end of 
    ...
    pSecondCtx->config.ui8Context = 0;
    pSecondCtx->config.ui16MaxDispOutWidth = pConnection->sHWInfo.ui32MaxLineStore;
...
TEST(CI_Driver, linestore_greedy1)
{
    ...
    IMG_MEMCPY(pSecondCtx, driver.pPipeline, sizeof(CI_PIPELINE));
    pSecondCtx->config.ui8Context = 1;
    pSecondCtx->config.ui16MaxDispOutWidth = pConnection->sHWInfo.context_aMaxWidthMult[1]; // is not necessarly the maximum linestore for ctx1
...
TEST(CI_Driver, specify_buffer)
{
    ...
    for ( int b = 0 ; b < 4 ; b++ )
    {
        ...
        if ( b == 1 ) // we triggered 2 shots, we will acquire some
        {
            ...
            EXPECT_EQ(IMG_SUCCESS, driver.fakeInterrupt(pSpecifyPipe->pipelineConfig.ui8Context)) << "fake interrupt failed";
