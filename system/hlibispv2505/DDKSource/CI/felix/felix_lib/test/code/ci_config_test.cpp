...
TEST(CI_Config, create_destroy)
{
    ...
    pConfig->config.ui16MaxEncOutWidth = 32;
    pConfig->config.ui16MaxEncOutHeight = 32;
    pConfig->sDisplayScaler.bBypassScaler = IMG_TRUE;
    PixelTransformYUV(&pConfig->config.eEncType, YVU_422_PL12_8);
    PixelTransformRGB(&pConfig->config.eDispType, PXL_NONE);
...
TEST(CI_Config, create_destroy_atFinalise_registered)
{
    ...
    pConfig->config.ui16MaxEncOutWidth = 32;
    pConfig->config.ui16MaxEncOutHeight = 32;
    PixelTransformYUV(&pConfig->config.eEncType, YVU_420_PL12_8);
    pConfig->sEncoderScaler.bOutput422 = IMG_FALSE;
    //pConfig->ui32Stride = 128;
    pConfig->sToneMapping.localColWidth[0] = TNM_MIN_COL_WIDTH;
...
// don't need it any more (since most of ci_modules are tested during
// LoadStructure test)
...
static void testUpdate(CI_PIPELINE *pUserPipeline, KRN_CI_PIPELINE *pKrnPipeline, enum CI_MODULE_UPDATE eUpd)
{
    int res;

    if (pUserPipeline->config.ui8PrivateValue != pKrnPipeline->pipelineConfig.ui8PrivateValue)
    {
    ...
    if ( eUpd & CI_UPD_LSH )
    {
        ...
        res = IMG_MEMCMP(&(pUserPipeline->sDeshading),\
            &(pKrnPipeline->lshConfig), sizeof(CI_MODULE_LSH));
        if (res != 0)
        {
            FAIL() << "wrong update for block LSH";
        }
    }
    else
    {
        // inside LoadStructure
        //--------
#if 0
        ...
        // statistics
        VERIF_MODULE_UPD(stats.sExposureStats, EXS);
        VERIF_MODULE_UPD(stats.sFocusStats, FOS);
        VERIF_MODULE_UPD(stats.sWhiteBalanceStats, WBS);
        VERIF_MODULE_UPD(stats.sAutoWhiteBalanceStats, AWS);
        VERIF_MODULE_UPD(stats.sHistStats, HIS);
        VERIF_MODULE_UPD(stats.sFlickerStats, FLD);
#endif
        //--------

        INT_PIPELINE *pIntPipe = container_of(pUserPipeline, INT_PIPELINE,\
            publicPipeline);
        res = IMG_MEMCMP(pIntPipe->pLoadStructure,\
            pKrnPipeline->pLoadStructStamp, INT_CI_SizeLS_HW2());
        if (res != 0)
        {
            FAIL() << "wrong update for block, " << " upd=" << eUpd;
        }
    }
...
static void testUpdateRegister(IMG_UINT32 regOffset, IMG_UINT32 updRegVal, int updMask, FullFelix &driver)
{
...
//  [GM] disabled until testupdate has been changed to cope with load structure from user-space
TEST(CI_Config, update)
{
    //  [GM] krnPipe has no access to userPipe
    FullFelix driver; // init the drivers
    ...
    // statistics
    driver.pPipeline->stats.sExposureStats.ui16PixelMax += 2;
    driver.pPipeline->stats.sFocusStats.ui16Left += 3;
    driver.pPipeline->stats.sWhiteBalanceStats.aYMax[0] += 5;
    driver.pPipeline->stats.sAutoWhiteBalanceStats.bDebugBitmap =
            !driver.pPipeline->stats.sAutoWhiteBalanceStats.bDebugBitmap;
    driver.pPipeline->stats.sHistStats.ui16Height += 4;
    driver.pPipeline->stats.sFlickerStats.ui32SceneChange += 3;
    driver.pPipeline->stats.sEncStats.ui8Log2NCouples += 1;

    driver.pPipeline->config.ui8PrivateValue++;
    eUpd = CI_UPD_BLC;
    updated = CI_UPD_NONE;
    ASSERT_EQ(IMG_SUCCESS, CI_PipelineUpdate(driver.pPipeline, eUpd, &updated));
    testUpdate(driver.pPipeline, pKrnConfig, eUpd);
    ...
    testUpdate(driver.pPipeline, pKrnConfig, eUpd);
    EXPECT_EQ(CI_UPD_NONE, updated);
    driver.pPipeline->stats.sExposureStats.ui16PixelMax += 2; // so it won't be equal anymore
    if(HasFailure()) return;
    ...
    testUpdate(driver.pPipeline, pKrnConfig, eUpd);
    EXPECT_EQ(CI_UPD_NONE, updated);
    driver.pPipeline->stats.sFocusStats.ui16Left += 2; // so it won't be equal anymore
    if(HasFailure()) return;
    ...
    testUpdate(driver.pPipeline, pKrnConfig, eUpd);
    EXPECT_EQ(CI_UPD_NONE, updated);
    driver.pPipeline->stats.sWhiteBalanceStats.aYMax[0] += 6; // so it won't be equal anymore
    if(HasFailure()) return;
    ...
    testUpdate(driver.pPipeline, pKrnConfig, eUpd);
    EXPECT_EQ(CI_UPD_NONE, updated);
    driver.pPipeline->stats.sAutoWhiteBalanceStats.ui16BlueClipThresh += 6; // so it won't be equal anymore
    if(HasFailure()) return;
    ...
    testUpdate(driver.pPipeline, pKrnConfig, eUpd);
    EXPECT_EQ(CI_UPD_NONE, updated);
    driver.pPipeline->stats.sHistStats.ui16Height += 2; // so it won't be equal anymore
    if(HasFailure()) return;
    ...
    testUpdate(driver.pPipeline, pKrnConfig, eUpd);
    EXPECT_EQ(CI_UPD_NONE, updated);
    driver.pPipeline->stats.sFlickerStats.ui32SceneChange += 5; // so it won't be equal anymore
    if(HasFailure()) return;
...
TEST(CI_Config, update_notstarted)
{
    ...
    // make sure the registers do not contain the current value
    for (i = 0; i < LSH_GRADS_NO; i++)
    {
        tmp = 0;
        REGIO_WRITE_FIELD(tmp, FELIX_CONTEXT0,
            LSH_GRADIENTS_Y, SHADING_GRADIENT_Y,
            driver.pPipeline->sDeshading.aGradientsY[i] + 2);
        TALREG_WriteWord32(g_psCIDriver->sTalHandles.hRegFelixContext[0],
            REGIO_TABLE_OFF(FELIX_CONTEXT0, LSH_GRADIENTS_Y, i), tmp);
    }
    ...
    for (i = 0; i < LSH_GRADS_NO; i++)
    {
        TALREG_ReadWord32(g_psCIDriver->sTalHandles.hRegFelixContext[0],
            REGIO_TABLE_OFF(FELIX_CONTEXT0, LSH_GRADIENTS_Y, i), &tmp);
        field = REGIO_READ_FIELD(tmp, FELIX_CONTEXT0,
            LSH_GRADIENTS_Y, SHADING_GRADIENT_Y);
        EXPECT_NE(driver.pPipeline->sDeshading.aGradientsY[i], field);
    }

    // starting writes to registers and device memory
    ASSERT_EQ(IMG_SUCCESS, CI_PipelineStartCapture(driver.pPipeline));

    // registers should have been updated
    for (i = 0; i < LSH_GRADS_NO; i++)
    {
        TALREG_ReadWord32(g_psCIDriver->sTalHandles.hRegFelixContext[0],
            REGIO_TABLE_OFF(FELIX_CONTEXT0, LSH_GRADIENTS_Y, i), &tmp);
        field = REGIO_READ_FIELD(tmp, FELIX_CONTEXT0,
            LSH_GRADIENTS_Y, SHADING_GRADIENT_Y);
        EXPECT_EQ(driver.pPipeline->sDeshading.aGradientsY[i], field);
    }
}
...
TEST(CI_Config, update_asap)
{
    ...
    // now verify it was not applied
    {
        ...
        while ( (pCurrentCell = List_getNext(pCurrentCell)) != NULL )
        {
            ...
            SYS_MemReadWord(&(pCurrent->hwLoadStructure.sMemory), FELIX_LOAD_STRUCTURE_BLACK_ANALYSIS_OFFSET, &reg);
...
TEST(CI_Config, dpf_write)
{
    ...
    EXPECT_EQ(0, driver.pPipeline->config.ui32DPFWriteMapSize) << "default dpf output size should be 0";

    memcpy(pConfig, driver.pPipeline, sizeof(CI_PIPELINE)); // get correct configuration

    pConfig->config.ui32DPFWriteMapSize = DPF_MAP_MAX_PER_LINE*pConfig->sImagerInterface.ui16ImagerSize[1]*DPF_MAP_OUTPUT_SIZE;
    ...
    { // run the pipeline that does not have DPF output
        ...
        KRN_CI_PIPELINE *krnPipe = driver.getKrnPipeline();
        if (krnPipe){
            EXPECT_EQ(IMG_SUCCESS, Felix::fakeInterrupt(krnPipe));
        }
        else
        {
            ADD_FAILURE();
    ...
    { // run the pipeline that has DPF output
        ...
        KRN_CI_PIPELINE *krnPipe = driver.getKrnPipeline(pConfig);
        if (krnPipe){
            EXPECT_EQ(IMG_SUCCESS, Felix::fakeInterrupt(krnPipe));
        }
        else
...
TEST(CI_Config, computeLinestore)
{
...

    {
        ...
        // try 720p
        imageWidht = 1280;
        pPipeline->config.ui8Context = 0; // the linestore needs the IIF to be configured
...

    {
        ...
        // try 720p
        imageWidht = 1280;
        pPipeline->config.ui8Context = 1; // the linestore needs the IIF to be configured
        pPipeline->sImagerInterface.ui16ImagerSize[0] = (imageWidht)/2 -1; // half the linestore converted in CFA (and -1 for the register)
        pPipeline->sImagerInterface.ui16ImagerSize[1] = 720;
...
/**
 * ensure that BRN58117 is coped with in KRN_CI_PipelineVerify
 */
TEST(CI_Config, BRN58117)
{
    Felix driver;
    CI_PIPELINE sPipeline;
    CI_HWINFO *pHWInfo = NULL;
    IMG_RESULT ret;

    driver.configure();

    pHWInfo = &(driver.getConnection()->sHWInfo);
    IMG_MEMSET(&sPipeline, 0, sizeof(CI_PIPELINE));

    /* just configure the output of the pipeline
     * - other modules are not needed */

    // DE(1)
    ret = PixelTransformBayer(&sPipeline.config.eDispType, BAYER_RGGB_10,
        MOSAIC_BGGR);
    ASSERT_EQ(IMG_SUCCESS, ret) << "convert bayer";
    sPipeline.config.eDataExtraction = CI_INOUT_BLACK_LEVEL;

    ret = KRN_CI_PipelineVerify(&sPipeline.config, pHWInfo);
    EXPECT_EQ(IMG_TRUE, ret) << "inout point 1 should work";

    // DE(2)
    sPipeline.config.eDataExtraction = CI_INOUT_FILTER_LINESTORE;
    ret = KRN_CI_PipelineVerify(&sPipeline.config, pHWInfo);
    EXPECT_EQ(IMG_FALSE, ret) << "inout point 2 should not work without "\
        "other outputs";

    // RGB
    ret = PixelTransformRGB(&sPipeline.config.eDispType, BGR_101010_32);
    ASSERT_EQ(IMG_SUCCESS, ret) << "convert rgb";

    ret = KRN_CI_PipelineVerify(&sPipeline.config, pHWInfo);
    EXPECT_EQ(IMG_TRUE, ret) << "rgb only should work";

    // DE(2) + YUV
    ret = PixelTransformBayer(&sPipeline.config.eDispType, BAYER_RGGB_10,
        MOSAIC_BGGR);
    ASSERT_EQ(IMG_SUCCESS, ret) << "convert bayer";
    ret = PixelTransformYUV(&sPipeline.config.eEncType, YUV_420_PL12_8);
    ASSERT_EQ(IMG_SUCCESS, ret) << "convert yuv";

    ret = KRN_CI_PipelineVerify(&(sPipeline.config), pHWInfo);
    EXPECT_EQ(IMG_TRUE, ret) << "DE + YUV should work";

    IMG_MEMSET(&sPipeline.config.eEncType, 0, sizeof(PIXELTYPE));
    sPipeline.config.eEncType.eFmt = PXL_NONE;

    // DE(2) + HDF
    ret = PixelTransformRGB(&sPipeline.config.eHDRExtType, BGR_101010_32);
    ASSERT_EQ(IMG_SUCCESS, ret) << "convert rgb";

    ret = KRN_CI_PipelineVerify(&sPipeline.config, pHWInfo);
    EXPECT_EQ(IMG_TRUE, ret) << "DE + HDF should work";

    IMG_MEMSET(&sPipeline.config.eHDRExtType, 0, sizeof(PIXELTYPE));
    sPipeline.config.eHDRExtType.eFmt = PXL_NONE;

    // DE(2) + RAW2D
    ret = PixelTransformBayer(&sPipeline.config.eRaw2DExtraction, BAYER_TIFF_10,
        MOSAIC_BGGR);
    ASSERT_EQ(IMG_SUCCESS, ret) << "convert tiff";

    ret = KRN_CI_PipelineVerify(&sPipeline.config, pHWInfo);
    EXPECT_EQ(IMG_TRUE, ret) << "DE + Raw2D should work";

    IMG_MEMSET(&sPipeline.config.eRaw2DExtraction, 0, sizeof(PIXELTYPE));
    sPipeline.config.eRaw2DExtraction.eFmt = PXL_NONE;

    // DE(2) + DPF
    sPipeline.sDefectivePixels.eDPFEnable = CI_DPF_WRITE_MAP_ENABLED;

    ret = KRN_CI_PipelineVerify(&sPipeline.config, pHWInfo);
    EXPECT_EQ(IMG_TRUE, ret) << "DE + DPF should work";

    sPipeline.sDefectivePixels.eDPFEnable = 0;

    // DE(2) + EXS should not work (EXS before FLS)
    sPipeline.config.eOutputConfig = CI_SAVE_EXS_GLOBAL;

    ret = KRN_CI_PipelineVerify(&sPipeline.config, pHWInfo);
    EXPECT_EQ(IMG_FALSE, ret) << "DE + EXS should not work";

    sPipeline.config.eOutputConfig = CI_SAVE_EXS_REGION;

    ret = KRN_CI_PipelineVerify(&sPipeline.config, pHWInfo);
    EXPECT_EQ(IMG_FALSE, ret) << "DE + EXS should not work";

    // DE(2) + CRC + timestamps should not work
    sPipeline.config.eOutputConfig = CI_SAVE_CRC | CI_SAVE_TIMESTAMP;

    ret = KRN_CI_PipelineVerify(&sPipeline.config, pHWInfo);
    EXPECT_EQ(IMG_FALSE, ret) << "DE + CRC + timestamps should not work";

    // DE(2) + WBS
    sPipeline.stats.sWhiteBalanceStats.ui8ActiveROI = 1;

    ret = KRN_CI_PipelineVerify(&sPipeline.config, pHWInfo);
    EXPECT_EQ(IMG_TRUE, ret) << "DE + WBS should work";

    sPipeline.stats.sWhiteBalanceStats.ui8ActiveROI = 0;

    // DE(2) + other stats should work
    int stats[] = {
        CI_SAVE_FOS_GRID,
        CI_SAVE_FOS_ROI,
        CI_SAVE_HIST_GLOBAL,
    ...
    {
        sPipeline.config.eOutputConfig = stats[i];

        ret = KRN_CI_PipelineVerify(&sPipeline.config, pHWInfo);
        EXPECT_EQ(IMG_TRUE, ret) << "DE + stat[" << i << "]=" << stats[i]
            << " should work";
    }
}
