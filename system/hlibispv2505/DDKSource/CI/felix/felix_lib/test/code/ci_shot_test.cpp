...
TEST(KRN_CI_SHOT, ShotFromConfig)
{
    ...
    driver.getKrnPipeline()->pipelineConfig.eEncType.eBuffer = TYPE_NONE;
    driver.getKrnPipeline()->pipelineConfig.eDispType.eBuffer = TYPE_NONE;
    ...
    driver.getKrnPipeline()->pipelineConfig.eEncType.eBuffer = TYPE_YUV;
    ...
    IMG_MEMSET(&(pShot->userShot), 0, sizeof(CI_SHOT));
    driver.getKrnPipeline()->pipelineConfig.eEncType.eBuffer = TYPE_NONE;
    driver.getKrnPipeline()->pipelineConfig.eDispType.eBuffer = TYPE_RGB;
...
TEST(KRN_CI_SHOT, MallocBuffers)
{
    ...
    for ( int i = 1 ; i <= 5 ; i++ ) 
    {
        ...
        EXPECT_TRUE (pShot->hwLoadStructure.sMemory.pAlloc == NULL) << "buffer " << i;
    ...
    EXPECT_TRUE (pShot->hwLoadStructure.sMemory.pAlloc != NULL);
...
TEST(CI_SHOT, missedFrames)
{
    ...
    //  eagle use virtual channel
    gasketCnt = HW_CI_GasketFrameCount(
        driver.pPipeline->sImagerInterface.ui8Imager, 0);
    
    // fake interrupt 
    EXPECT_EQ(IMG_SUCCESS, driver.fakeInterrupt(driver.pPipeline->config.ui8Context)) << "fake interrupt failed";

    //  eagle use virtual channel
    EXPECT_EQ(++gasketCnt,
        HW_CI_GasketFrameCount(driver.pPipeline->sImagerInterface.ui8Imager, 0))
        << "fake interrupt did not increment the interrutp!";
...
TEST(CI_SHOT, Gasketrollover)
{
    ...
    EXPECT_EQ(0,
        HW_CI_GasketFrameCount(driver.pPipeline->sImagerInterface.ui8Imager, 0))
        << "fake interrupt did not increment the interrutp! or it did not roll over";
