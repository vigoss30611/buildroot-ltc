...
/**
 * @brief Write a register or memory at a given offset
 *
 * @note Available only if kernel-side compiled with debug functions
 *
 * @param[in] pConnection
 ...
 * configuration
 * @param[in] eUpdateMask from enum CI_MODULE_UPDATE combination (ignored if
 ...
 * failure
 * @li CI_UPD_NONE if some error occurred that is not related to a particular
 * module (e.g. the private number)
 * @li CI_UPD_ALL if the error occurred when updating the HW configuration
 *
 * @return IMG_SUCCESS
...
/**
 * @brief Allocate a buffer to be used as a LSH matrix
 *
 * @param pPipeline
 * @param ui32Size in bytes of the matrix supporting system memory alignment
 * @param[out] pMatrixId optional pointer to store the matrix identifier
 * returned by the kernel side (that id should be used for other matrix
 * functions). 0 is an invalid number of the matrixId.
 *
 * @return IMG_SUCCESS
 * @return IMG_ERROR_INVALID_PARAMETERS if pPipeline is NULL or ui32Size is 0
 * @return IMG_ERROR_NOT_INITIALISED if the pipeline is not registered
 * @return IMG_ERROR_MALLOC_FAILED if the internal buffer allocation failed
 * @return IMG_ERROR_FATAL if the mapping to user-space failed
 */
IMG_RESULT CI_PipelineAllocateLSHMatrix(CI_PIPELINE *pPipeline,
    IMG_UINT32 ui32Size, IMG_UINT32 *pMatrixId);

/**
 * @brief Deregister a LSH matrix buffer that is not in use
 *
 * @note This function lets the kernel-side decides if the matrix is in use
 * or not to allow the destruction. If in use the matrix is not destroyed.
 *
 * @param pPipeline
 * @param matrixId
 *
 * @return IMG_SUCCESS
 * @return IMG_ERROR_INVALID_PARAMETERS if pPipeline is NULL or matrixId is 0
 * @return IMG_ERROR_COULD_NOT_OBTAIN_RESOURCE if the matrixId is not found
 ...
 * Shots can be destroyed manually with @ref CI_PipelineDeleteShots()
 *
 * @param[in,out] pPipeline
 ...
 *
 * @ could return the number of buffers that were successfully added on
 * error
 */
 ...
 * affected.
 *
 * @warning The capture has to be stopped for shots to be destroyed and the
 * user should have released all acquired shots as well.
 *
 * @param[in, out] pPipeline
 *
 * @return IMG_SUCCESS
 ...
 * point without using @ref CI_PipelineUpdateLSHMatrix().
 *
 * @param[in] pPipeline
 *
 * @return IMG_SUCCESS
...
/**
 * @brief Release the Hardware acquired with CI_PipelineStartCapture()
 *
 ...
 * be released using @ref CI_PipelineReleaseShot()
 *
 ...
 * lock-up)
 *
 * @see Non-blocking equivalent: @ref CI_PipelineAcquireShotNB()
 */
 ...
 *
 * @return IMG_SUCCESS
 * @return IMG_ERROR_NOT_INITIALISED if pShot was not acquired beforehand
 */
 ...
 * released using @ref CI_PipelineReleaseBuffer()
 *
 * @warning Acquired buffer MUST NOT be destroyed!
 *
 * @param[in] pPipeline
 ...
 * @return IMG_ERROR_COULD_NOT_OBTAIN_RESOURCE if no buffers are available
 *
 * @see Blocking equivalent: @ref KRN_CI_PipelineAcquireShot()
 */
IMG_RESULT CI_PipelineAcquireShotNB(CI_PIPELINE *pPipeline,
    CI_SHOT **ppBuffer);

/**
 * @brief Release a previously acquired with @ref CI_PipelineAcquireShot()
 * or @ref CI_PipelineAcquireShotNB()
 *
 * @param[in] pPipeline
 * @param[in] pBuffer
 *
...
/**
 * @brief Acquire a LSH matrix buffer to update its content. Should be
 * released once content has been update using
 * @ref CI_PipelineReleaseLSHMatrix()
 *
 * Once acquired cannot be used as matrix until released.
 *
 * Cannot be acquired if it is the matrix currently used by HW.
 *
 * @param pPipeline
 * @param[in] id matrix to acquire
 * @param[out] pMatrix structure to fill with information about the matrix if
 * it is available
 *
 * @return IMG_SUCCESS
 * @return IMG_ERROR_INVALID_PARAMETERS if pPipeline or pMatrix is NULL or
 * given id is 0
 * @return IMG_ERROR_COULD_NOT_OBTAIN_RESOURCE if the given matrix is not
 * available (either not found in user-space list or used by kernel)
 */
IMG_RESULT CI_PipelineAcquireLSHMatrix(CI_PIPELINE *pPipeline,
    IMG_UINT32 id, CI_LSHMAT *pMatrix);

/**
 * @brief Release an acquired LSH matrix buffer
 *
 * @return IMG_SUCCESS
 * @return IMG_ERROR_INVALID_PARAMETERS if pPipeline or pMatrix
 * @return IMG_ERROR_COULD_NOT_OBTAIN_RESOURCE if the matrix was not acquired
 * beforehand or not found in user-space list
 * @return IMG_ERROR_FATAL if releasing in kernel-side failed
 */
IMG_RESULT CI_PipelineReleaseLSHMatrix(CI_PIPELINE *pPipeline,
    CI_LSHMAT *pMatrix);

/**
 * @brief Change the LSH matrix the pipeline should use
 *
 * For HW 2 the matrix can be change if the capture is started if and only if
 * the configuration does not change compared with the previous matrix (or
 * the HW has no pending buffers).
 *
 * @param pPipeline
 * @param[in] id of the matrix to use - should not be acquired for changes
 * - if 0 is given is used to disabled the use of the LSH matrix
 *
 * @return IMG_SUCCESS
 * @return IMG_ERROR_INVALID_PARAMETERS if pPipeline is NULL
 * @return IMG_ERROR_COULD_NOT_OBTAIN_RESOURCE if the given matrix is not
 * available (either already in use as a LSH grid or acquired or not found
 * in user-space list).
 * @return IMG_ERROR_FATAL if setting up in the kernel-side failed
 */
...
/**
 * @brief The Pipeline has some Shots that can be used to trigger a new
 * capture
 */
 ...
 * CI_PipelineHasAvailableBuffers() but return TRUE only if both have
 * available elements
 */
 ...
 * This count excludes the LSH buffers.
 *
 * @note does not call an ioctl(), verifies the user-mode driver list only
 *
 * @return positive value if no error occurred (number of buffers)
 * @return negative value if an error occurred
 *
 * See @ref CI_PipelineHasLSHBuffers() for counting LSH buffers
 */
IMG_INT32 CI_PipelineHasBuffers(const CI_PIPELINE *pPipeline);

/**
 * @brief The Pipeline has some allocated or imported LSH buffers.
 *
 * @return positive value if no error occurred (number of buffers)
 * @return negative value if an error occurred
 *
 * See @ref CI_PipelineHasBuffers() for counting the other buffers
 */
 ...
 *
 * @see @ref CI_PipelineImportBuffer()
 */
...
/**
 * @brief Get the 1st available frame that can then be used to write image
 * data.
 *
 * The frame is reserved until triggered or released.
 *
 * Similar to @ref CI_DatagenGetFrame() without frame ID
 *
...
/**
 * @brief Get an available frame that can be used to write image data
 *
 * The frame is reserved until triggered or released.
 *
 * Similar to @ref CI_DatagenGetAvailableFrame() but searches for a specific
 * frame
 *
 * @return pointer to the frame to use
 * @return NULL if the given frame is not available
 */
CI_DG_FRAME* CI_DatagenGetFrame(CI_DATAGEN *pDatagen, IMG_UINT32 ui32FrameID);

/**
 * @brief Insert a frame for capture
 *
 * To cancel a frame reservation without triggering use
 * @ref CI_DatagenReleaseFrame()
 *
 * If inserted correctly the completion should be waited on using
 * @ref CI_DatagenWaitProcessedFrame() at a later point.
 *
 * @return IMG_SUCCESS
 * @return IMG_ERROR_NOT_SUPPORTED if blanking is too small or the stride is
 * not aligned to the system alignment
 * @return IMG_ERROR_UNEXPECTED_STATE if the given frame format is not
 * supported by the data-generator
 * @return IMG_ERROR_INVALID_PARAMETERS if the frame is not found or the given
 * stride*height is bigger than the allocated size
 */
IMG_RESULT CI_DatagenInsertFrame(CI_DG_FRAME *pFrame);

/**
 * @brief Release a frame without triggering a capture.
 *
 * Similar to @ref CI_DatagenInsertFrame() without triggering a capture.
 *
 * @return IMG_SUCCESS
 * @return IMG_ERROR_INVALID_PARAMETERS if the frame is not found
 */
...
/**
 * @brief Liberate the memory associated to a frame
 *
 * The frame need to be acquired with @ref CI_DatagenGetFrame() or
 * CI_DatagenGetAvailableFrame() beforehand!
 *
 * @return IMG_SUCCESS
 * @return IMG_ERROR_INVALID_PARAMETERS if the frame is not found
 */
...
IMG_RESULT CI_DriverGetHwAwbBoundary(CI_CONNECTION *pConnection,
    HW_AWB_BOUNDARY *pHwAwbBoundary);

IMG_RESULT CI_DriverSetHwAwbBoundary(CI_CONNECTION *pConnection,
    HW_AWB_BOUNDARY *pHwAwbBoundary);
