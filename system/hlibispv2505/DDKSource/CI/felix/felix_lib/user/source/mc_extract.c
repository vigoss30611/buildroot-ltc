...
#ifdef AWS_AVAILABLE
    int row, col;
    IMG_UINT32 collected;
    // throw assertion if HW struct layout is different than R,B,G,CFA
    // NOTE actually the order in VHC is different than in TRM
    // doc: R,G,B,CFA
    // VHC : R,B,G,CFA
    IMG_STATIC_ASSERT(
        FELIX_SAVE_AWS_COLLECTED_R_TILE_0_OFFSET <
        FELIX_SAVE_AWS_COLLECTED_B_TILE_0_OFFSET, checkStatsLayoutRB);
    IMG_STATIC_ASSERT(
        FELIX_SAVE_AWS_COLLECTED_B_TILE_0_OFFSET <
        FELIX_SAVE_AWS_COLLECTED_G_TILE_0_OFFSET, checkStatsLayoutBG);
    IMG_STATIC_ASSERT(
        FELIX_SAVE_AWS_COLLECTED_G_TILE_0_OFFSET <
        FELIX_SAVE_AWS_NUM_CFAS_TILE_0_OFFSET, checkStatsLayoutGCFA);

    IMG_UINT8* awsStats = (IMG_UINT8*)blob;
    // the same spacing between each row, hopefully
    const IMG_UINT32 rowDelta =
        FELIX_SAVE_AWS_COLLECTED_R_TILE_1_OFFSET -
        FELIX_SAVE_AWS_COLLECTED_R_TILE_0_OFFSET;

    IMG_ASSERT(pAWSStats);
    IMG_ASSERT(blob);

    for(row=0;row<AWS_NUM_GRID_TILES_VERT;++row)
    {
#if(1)
        // generic code, not depending on field layout in memory
        for(col=0;col<AWS_NUM_GRID_TILES_HORIZ;++col)
        {
            collected = *(IMG_UINT32 *)(awsStats +
                FELIX_SAVE_AWS_COLLECTED_R_TILE_0_OFFSET +
                col*FELIX_SAVE_AWS_COLLECTED_R_TILE_0_STRIDE);
            pAWSStats->tileStats[row][col].fCollectedRed =
                IMG_Fix_Revert(collected, PREC(AWS_COLLECTED_R));

            collected = *(IMG_UINT32 *)(awsStats +
                FELIX_SAVE_AWS_COLLECTED_B_TILE_0_OFFSET +
                col*FELIX_SAVE_AWS_COLLECTED_B_TILE_0_STRIDE);
            pAWSStats->tileStats[row][col].fCollectedBlue =
                IMG_Fix_Revert(collected, PREC(AWS_COLLECTED_B));

            collected = *(IMG_UINT32 *)(awsStats +
                FELIX_SAVE_AWS_COLLECTED_G_TILE_0_OFFSET +
                col*FELIX_SAVE_AWS_COLLECTED_G_TILE_0_STRIDE);
            pAWSStats->tileStats[row][col].fCollectedGreen =
                IMG_Fix_Revert(collected, PREC(AWS_COLLECTED_G));

            IMG_STATIC_ASSERT(AWS_NUM_CFAS_FRAC == 0, checkNUM_CFA_FRAC_isZero);
            pAWSStats->tileStats[row][col].numCFAs =
                *(IMG_UINT32 *)(awsStats +
                    FELIX_SAVE_AWS_NUM_CFAS_TILE_0_OFFSET +
                    col*FELIX_SAVE_AWS_NUM_CFAS_TILE_0_STRIDE) &
                    FELIX_SAVE_AWS_NUM_CFAS_TILE_0_AWS_NUM_CFAS_TILE_0_LSBMASK;
        }
#else
        // fast but depends of field layout in mem
        IMG_STATIC_ASSERT(0 == offsetof(MC_TILE_STATS_AWS, collectedRed),
            check_RedField);
        IMG_STATIC_ASSERT(rowDelta >= sizeof(pAWSStats->tileStats[row]),
            check_rowSize1);
        // if fails, use #pragma pack(4) before  typedef struct MC_TILE_STATS_AWS
        IMG_STATIC_ASSERT(
            sizeof(pAWSStats->tileStats[row]) ==
                FELIX_SAVE_AWS_NUM_CFAS_TILE_0_NO_ENTRIES*4*sizeof(IMG_INT32),
            check_rowSize2);

        IMG_MEMCPY(&pAWSStats->tileStats[row],
                awsStats+FELIX_SAVE_AWS_COLLECTED_R_TILE_0_OFFSET,
                sizeof(pAWSStats->tileStats[row]));
#endif
        awsStats += rowDelta;
    }
