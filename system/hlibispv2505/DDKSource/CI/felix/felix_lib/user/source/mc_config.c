...
#if WBC_AVAILABLE
    pWBC->eRGBThresholdMode = WBC_THRESHOLD;
    for (i = 0; i < WBC_CHANNEL_NO; i++)
    {
        pWBC->afRGBThreshold[i] = 1.0;
        pWBC->afRGBGain[i] = 1.0;
    }
#endif
...
IMG_RESULT MC_AWSInit(MC_AWS *pAWS, const MC_IIF *pIIF)
{
    ...
    int i;
    const MC_FLOAT coeffFrac = (MC_FLOAT)(1 << AWS_CURVE_X_COEFF_FRAC);
    const MC_FLOAT offFrac = (MC_FLOAT)(1 << AWS_CURVE_OFFSET_FRAC);
    const MC_FLOAT boundFrac = (MC_FLOAT)(1 << AWS_CURVE_BOUNDARY_FRAC);
    /* for HW 2.7 these defaults are to be "compatible" with HW 2.6 that
     * does not let the value change - but the values are expected to change
     * to allow sensor calibration */
    const MC_FLOAT coeffDefX[AWS_LINE_SEG_N] = {
        0x38C / coeffFrac,
        0x3DE / coeffFrac,
        0x3FE / coeffFrac,
        0x0,
    ...
    const MC_FLOAT coeffDefY[AWS_LINE_SEG_N] = {
        0x1D8 / coeffFrac,
        0x105 / coeffFrac,
        0x042 / coeffFrac,
        0x0,
    ...
    const MC_FLOAT offsetDef[AWS_LINE_SEG_N] = {
        0x028 / offFrac,
        0x106 / offFrac,
        0x28C / offFrac,
        0x0,
    ...
    const MC_FLOAT boundaryDef[AWS_LINE_SEG_N] = {
        0x0100 / boundFrac,
        0x7CA0 / boundFrac,
        0x7860 / boundFrac,
        0x4000 / boundFrac,
    ...
    IMG_MEMSET(pAWS, 0, sizeof(MC_AWS));

    pAWS->bEnable = IMG_FALSE;
    pAWS->bDebugBitmap = IMG_FALSE;

    pAWS->ui16GridStartColumn = 0;
    pAWS->ui16GridStartRow = 0;

    // pixels
    pAWS->ui16GridTileWidth = IMG_MAX_INT(
        pIIF->ui16ImagerSize[0]*CI_CFA_WIDTH/AWS_NUM_GRID_TILES_HORIZ,
        AWS_MIN_GRID_TILE_WIDTH);
    pAWS->ui16GridTileHeight = IMG_MAX_INT(
        pIIF->ui16ImagerSize[1]*CI_CFA_HEIGHT/AWS_NUM_GRID_TILES_VERT,
        AWS_MIN_GRID_TILE_HEIGHT);

    // values except Dark thresholds are set close to max
    pAWS->fBbDist = (1 << AWS_BB_DIST_INT) - 1;

    pAWS->fBlueClipThresh = (1 << AWS_B_CLIP_THRESH_INT) - 1;
    pAWS->fBlueDarkThresh = 0.0;

    pAWS->fRedClipThresh = (1 << AWS_R_CLIP_THRESH_INT) - 1;
    pAWS->fRedDarkThresh = 0.0;

    pAWS->fGreenClipThresh = (1 << AWS_G_CLIP_THRESH_INT) - 1;
    pAWS->fGreenDarkThresh = 0.0;

    pAWS->fLog2_B_Qeff = (1 << AWS_LOG2_B_QEFF_INT) - 1;
    pAWS->fLog2_R_Qeff = (1 << AWS_LOG2_R_QEFF_INT) - 1;

#ifdef AWS_LINE_SEG_AVAILABLE
    for (i = 0; i < AWS_LINE_SEG_N; i++)
    {
        pAWS->aCurveCoeffX[i] = coeffDefX[i];
        pAWS->aCurveCoeffY[i] = coeffDefY[i];
        pAWS->aCurveOffset[i] = offsetDef[i];
        pAWS->aCurveBoundary[i] = boundaryDef[i];
    }
#endif /* AWS_LINE_SEG_AVAILABLE */
