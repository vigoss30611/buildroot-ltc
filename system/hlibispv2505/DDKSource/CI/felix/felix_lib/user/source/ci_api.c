 ...
 * @ingroup INT_FCT
 */
static IMG_BOOL8 List_ClearIntdg(void *listConfig, void *param)
{
    INT_INTDATAGEN *pIntDG = (INT_INTDATAGEN*)listConfig;
    IMG_ASSERT(pIntDG);
    CI_DatagenDestroy(&(pIntDG->publicDatagen));
    return IMG_TRUE;
}
 ...
 *
 * @param[in] pHWInfo to get the maximum size of the linestore
 * @param[in,out] pLinestore structure to read and update
 *
 * @return IMG_SUCCESS
 * @return IMG_ERROR_VALUE_OUT_OF_RANGE if a starting point is after the last
 * possible element (max-1)
 * @return IMG_ERROR_FATAL if the sum of the element's size is > max
    ...
    CI_LINESTORE *pLinestore)
{
    ...
    for (i = 0 ; i < ui8NContext ; i++)
    {
        ...
        else if ((IMG_UINT32)pLinestore->aStart[i]
...
IMG_RESULT CI_DriverInit(CI_CONNECTION **ppConnection)
{
    ...
    if (pIntConn->fileDesc)
    {
        ret = SYS_IO_Control(pIntConn->fileDesc, CI_IOCTL_INFO,
            (long int)&(pIntConn->publicConnection));
        if (0 != ret)
        {
...
IMG_RESULT CI_DriverFinalise(CI_CONNECTION *pConnection)
{
    ...
    List_visitor(&(pIntCon->sList_datagen), NULL, &List_ClearIntdg);
...
IMG_RESULT CI_DriverGetLinestore(CI_CONNECTION *pConnection)
{
    ...
    pIntCon = container_of(pConnection, INT_CONNECTION, publicConnection);
    IMG_ASSERT(pIntCon);
    /* memset otherwise valgrind complains we use uninitialised memory
     * which is not a problem as kernel write the whole structure */
    IMG_MEMSET(&tmp, 0, sizeof(CI_LINESTORE));

    ret = SYS_IO_Control(pIntCon->fileDesc, CI_IOCTL_LINE_GET, (long)&(tmp));
    if (0 != ret)
    {
...
IMG_RESULT CI_DriverGetGammaLUT(CI_CONNECTION *pConnection)
{
    ...
    ret = SYS_IO_Control(pIntCon->fileDesc, CI_IOCTL_GMAL_GET, (long)&(tmp));
    if (0 != ret)
    {
    ...
    HW_AWB_BOUNDARY *pHwAwbBoundary)
{
    ...
    if (!pConnection || !pHwAwbBoundary) {
        LOG_ERROR("pConnection or pHwAwbBoundary is NULL\n");
        return IMG_ERROR_INVALID_PARAMETERS;
    }
    ...
    ret = SYS_IO_Control(pIntCon->fileDesc, CI_IOCTL_HW_AWB_BOUNDARY_GET,
        (long)pHwAwbBoundary);
    if (0 != ret) {
        LOG_ERROR("Failed to update the HW AWB boundary information\n");
        return toImgResult(ret);
    }
    ...
    HW_AWB_BOUNDARY *pHwAwbBoundary)
{
    ...
    ret = SYS_IO_Control(pIntCon->fileDesc, CI_IOCTL_HW_AWB_BOUNDARY_SET,
        (long)pHwAwbBoundary);
    if (0 != ret)
    {
        LOG_ERROR("Failed to set a new HW AWB boundary\n");
        return toImgResult(ret);
    }
