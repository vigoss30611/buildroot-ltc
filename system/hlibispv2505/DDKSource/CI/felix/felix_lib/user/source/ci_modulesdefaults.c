    ...
    const CI_MODULE_IIF *pImagerInterface, const CI_HWINFO *pHWInfo)
{
    ...
    //if (pMatrix->bUseDeshadingGrid == IMG_TRUE)
    {
        IMG_UINT16 uiTileSize;

        if (pMatrix->ui8BitsPerDiff < LSH_DELTA_BITS_MIN
            || pMatrix->ui8BitsPerDiff > LSH_DELTA_BITS_MAX)
        {
            LOG_ERROR("LSH %u bits per difference is not supported "\
                "(supports [%u ; %u] bits)\n",
                pMatrix->ui8BitsPerDiff,
                LSH_DELTA_BITS_MIN, LSH_DELTA_BITS_MAX);
            return IMG_ERROR_NOT_SUPPORTED;
        }

        uiTileSize = 1 << pMatrix->ui8TileSizeLog2;
        if (pMatrix->ui16SkipX + 1 > uiTileSize
            || pMatrix->ui16SkipY + 1 > uiTileSize)
        {
            LOG_ERROR("LSH: skip values %u,%u is too big for the tile size "\
                "%u (must at least hit 1 element per tile)\n",
                pMatrix->ui16SkipX, pMatrix->ui16SkipY, uiTileSize);
            return IMG_ERROR_NOT_SUPPORTED;
        }

        if ((pMatrix->ui16Width*uiTileSize)
            < (pImagerInterface->ui16ImagerSize[0]+1)
            ||
            (pMatrix->ui16Height*uiTileSize)
            < (pImagerInterface->ui16ImagerSize[1]+1) )
        {
            LOG_ERROR("LSH: grid size %dx%d with tile %d CFA does not cover "\
                ...
                pMatrix->ui16Width, pMatrix->ui16Height, uiTileSize,
                (pImagerInterface->ui16ImagerSize[0]+1),
                (pImagerInterface->ui16ImagerSize[1]+1)
            );
            return IMG_ERROR_NOT_SUPPORTED;
...
IMG_RESULT CI_ModuleWBC_verif(const struct CI_MODULE_WBC *pWBC)
{
    ...
    if (ret)
    {
        LOG_ERROR("unsupported mode %d for WBC\n", (int)pWBC->eRGBThresholdMode);
    }
#endif /* WBC_AVAILABLE */
    return ret;
}
...
{
    if (pHIS->ui16Width < HIS_MIN_TILE_WIDTH
        || pHIS->ui16Height < HIS_MIN_TILE_HEIGHT)
    {
        LOG_ERROR("invalid tile configuration in HIS\n");
        return IMG_ERROR_FATAL;
    }
    return IMG_SUCCESS;
}
...
{
    const int width = (pIIF->ui16ImagerSize[0] + 1)*CI_CFA_WIDTH;
    const int height = (pIIF->ui16ImagerSize[1] + 1)*CI_CFA_HEIGHT;
    const int tileWidth = (pAWS->ui16GridTileWidth + 1)*CI_CFA_WIDTH;
    const int tileHeight = (pAWS->ui16GridTileHeight + 1)*CI_CFA_HEIGHT;
    if(!pAWS->bEnable) {
        return IMG_SUCCESS;
    }
    // check tiles
    if(tileWidth < AWS_MIN_GRID_TILE_WIDTH ||
       tileHeight < AWS_MIN_GRID_TILE_HEIGHT ||
       pAWS->ui16GridStartColumn > width ||
       pAWS->ui16GridStartRow > height) {
        LOG_ERROR("Invalid tile configuration in AWS from %d,%d; "\
            "tile size %dx%d px\n",
            pAWS->ui16GridStartColumn, pAWS->ui16GridStartRow,
