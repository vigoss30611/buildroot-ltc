/**
*******************************************************************************
@file ci_ioctl_printer.c

@brief Utility application that prints some of the IOCTL values as HEX

This application may be missing some IOCTL, refere to ci_ioctrl.h for the
complete list!
...
int main(int argc, char **argv)
{
    int v = 0;
    int c = 1;
    int p = 0;  // number of printed ioctl

    while (c < argc)
    {
        if (0 == strncmp(argv[c], "-h", 2))
        {
            printf("Tool to print CI ioctls (may not be complete)\n");
#ifdef FELIX_HAS_DG
    ...
    PRINT_IO(CI_IOCTL_GET_LSH);
    PRINT_IO(CI_IOCTL_SET_LSH);
    PRINT_IO(CI_IOCTL_CHA_LSH);
    ...
    PRINT_IO(DG_IOCTL_CAM_WAI);
    PRINT_IO(DG_IOCTL_CAM_BDE);
...

    {
        ...
        if (total != p)
        {
            printf("WARNING: printed %d ioctl but sum reports %d"\
