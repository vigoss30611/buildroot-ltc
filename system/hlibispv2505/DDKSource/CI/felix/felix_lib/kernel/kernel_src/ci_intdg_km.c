    ...
    KRN_CI_CONNECTION *pConnection)
{
    ...
    ret = SYS_SemInit(&(pDG->sSemProcessed), 0);
    if (IMG_SUCCESS != ret)  // unlikely
    {
        CI_FATAL("Failed to initialise processed list semaphore\n");
        return ret;
    }
...
IMG_RESULT KRN_CI_DatagenClear(KRN_CI_DATAGEN *pDG)
{
    ...
    SYS_SemDestroy(&(pDG->sSemProcessed));
    ...
    IMG_BOOL8 bBlocking)
{
    ...
    if (!bBlocking)
    {
        ret = SYS_SemTryDecrement(&(pDG->sSemProcessed));
        if (IMG_SUCCESS != ret)
        {
    ...
    else
    {
        ...
        ret = SYS_SemTimedDecrement(
            &(pDG->sSemProcessed), g_psCIDriver->uiSemWait);
        if (IMG_SUCCESS != ret)
        {
...
IMG_RESULT KRN_CI_DatagenStop(KRN_CI_DATAGEN *pDG)
{
    ...
    SYS_LockAcquire(&(pDG->slistLock));
    {
        ...
        while (pHead != NULL)
        {
            ret = SYS_SemDecrement(&(pDG->sSemProcessed));
            // unlikely - could be assert but we're in a lock
            if (IMG_SUCCESS != ret)
            {
                CI_WARNING("semaphore and processed list out of sync!\n");
            }
            List_pushBack(&(pDG->sList_available), pHead);
