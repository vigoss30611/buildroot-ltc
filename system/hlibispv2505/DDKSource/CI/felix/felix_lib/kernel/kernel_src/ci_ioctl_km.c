...
#ifdef FELIX_HAS_DG
#include <dg_kernel/dg_camera.h>  // to access to suspend/resume
#endif
    ...
    unsigned long ioctl_param)
{
    ...
    case CI_IOCTL_GET_LSH:
        CI_DEBUG("ioctl 0x%X=GET_LSH\n", ioctl_cmd);
        imgRet = INT_CI_PipelineGetLSHMatrix(pKrnConnection,
            (struct CI_LSH_GET_PARAM __user *)ioctl_param);
        if (IMG_SUCCESS != imgRet)
        {
            CI_FATAL("Failed to get LSH Matrix (returned %d)\n",
                imgRet);
            ret = toErrno(imgRet);
        }
        break;

    case CI_IOCTL_SET_LSH:
        CI_DEBUG("ioctl 0x%X=SET_LSH\n", ioctl_cmd);
        imgRet = INT_CI_PipelineSetLSHMatrix(pKrnConnection,
            (struct CI_LSH_SET_PARAM __user *)ioctl_param);
        if (IMG_SUCCESS != imgRet)
        {
            CI_FATAL("Failed to set LSH Matrix (returned %d)\n", imgRet);
            ret = toErrno(imgRet);
        }
        break;

    case CI_IOCTL_CHA_LSH:
        CI_DEBUG("ioctl 0x%X=CHA_LSH\n", ioctl_cmd);
        imgRet = INT_CI_PipelineChangeLSHMatrix(pKrnConnection,
            (struct CI_LSH_CHANGE_PARAM __user *)ioctl_param);
        if (IMG_SUCCESS != imgRet)
        {
            CI_FATAL("Failed to change LSH Matrix (returned %d)\n", imgRet);
            ret = toErrno(imgRet);
        }
        break;
    ...
    */
    ...
    case CI_IOCTL_INDG_WAI:
        CI_DEBUG("ioctl 0x%X=INDG_WAI\n", ioctl_cmd);
        imgRet = INT_CI_DatagenWaitProcessed(pKrnConnection,
            (const struct CI_DG_FRAMEWAIT __user *)ioctl_param);
        if (IMG_SUCCESS != imgRet)
        {
            CI_FATAL("Failed to wait pending frame for IIF DG HW\n");
            ret = toErrno(imgRet);
        }
        break;

    case CI_IOCTL_INDG_FRE:
    ...
    case CI_IOCTL_HW_AWB_BOUNDARY_GET:
        CI_DEBUG("ioctl 0x%X=HW_AWB_BOUNDARY_GET\n", ioctl_cmd);
        imgRet = INT_CI_DriverGetHwAwbBoundary(
            pKrnConnection,
            (HW_AWB_BOUNDARY __user *)ioctl_param);
        if (IMG_SUCCESS != imgRet)
        {
            CI_FATAL("Failed to get the HW AWB boundary information "\
                "(returned %d)\n", imgRet);
            ret = toErrno(imgRet);
        }
        break;

    case CI_IOCTL_HW_AWB_BOUNDARY_SET:
        CI_DEBUG("ioctl 0x%X=HW_AWB_BOUNDARY_SET\n", ioctl_cmd);
        imgRet = INT_CI_DriverSetHwAwbBoundary(
            pKrnConnection,
            (HW_AWB_BOUNDARY __user *)ioctl_param);
        if (IMG_SUCCESS != imgRet)
        {
            CI_FATAL("Failed to set a new HW AWB boundary (returned %d)\n", imgRet);
            ret = toErrno(imgRet);
        }
        break;
...
static IMG_BOOL8 List_SearchShot(void* listElem, void* lookingFor)
{
    ...
    if (pListShot->hwLoadStructure.sMemory.uiAllocated > 0
        && pListShot->hwLoadStructure.ID == searching->mmapId)
    {
        searching->buffer = CI_BUFF_LOAD;
...
int DEV_CI_Mmap(struct file *flip, struct vm_area_struct *vma)
{
    ...
    if (pShot)
    {
        ...
        if (pShot->hwLoadStructure.sMemory.uiAllocated == 0
            || pShot->hwSave.type != 0)
        {
            i++;  // this is not likely as load struct is always needed
        }
        ...
        for (i = 0; i < CI_N_CONTEXT; i++)
        {
            ...
            if (pPipe)
            {
                CI_INFO("Stopping a running capture on context %d\n",
                    pPipe->pipelineConfig.ui8Context);
...
IMG_RESULT DEV_CI_Resume(SYS_DEVICE *pDev)
{
    ...
    if (IMG_SUCCESS == ret)
    {
        ret = DEV_DG_Resume(pDev);
        if (ret)
        {
            CI_FATAL("Failed to resume DG\n");
        }
    }
#endif
