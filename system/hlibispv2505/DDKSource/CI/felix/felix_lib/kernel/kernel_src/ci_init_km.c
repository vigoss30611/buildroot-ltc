...
/// @brief If non-0 used for tiling allocations (needs to be a power of 2)
static int tilingStride = 0;
...
#if defined(FELIX_HAS_DG)
/// @brief external data generator PLL. 0, 0 for PLL is invalid but the driver will compute default
static uint extDGPLL[2] = { 0, 0 };
#endif

#ifdef INFOTM_ISP
uint clkRate = 192000000;

module_param(clkRate, uint, S_IRUGO);
MODULE_PARM_DESC(clkRate, "isp working clock");
#endif //INFOTM_ISP
...
module_param(tilingStride, int, S_IRUGO);
MODULE_PARM_DESC(tilingStride, "MMU tiling: choice of tiling stride if not 0 - has to be a power of 2");
...
#if defined(FELIX_HAS_DG)
module_param_array(extDGPLL, uint, NULL, S_IRUGO);
MODULE_PARM_DESC(extDGPLL, "External Data-generator PLL values (multiplier and divider). HAS TO RESPECT HW LIMITATIONS or FPGA may need to be reprogrammed.");
#endif
...
static int registerDriver(struct DEV_KERNEL *pKrnInfo, struct file_operations *pDevFileOps, const char *baseDevName)
{
    ...
    ret = cdev_add(&(pKrnInfo->sCharDevice), pKrnInfo->deviceNumber, 1);
    if (ret)
    {
    ...
    ret = misc_register(&(pKrnInfo->miscdev));
    if (ret)
    {
...
static int probeSuccess(SYS_DEVICE *pDevice)
{
    ...
    ret = allocDevice(&g_CIKernelInfo, DEV_FIRST_MINOR, DEV_NUMBER, DEV_NAME);
    if (ret)
    {
        return ret;
    }

    ret = KRN_CI_DriverCreate(&sFelixDriver, mmucontrol, tilingScheme,
        tilingStride, gammaCurve, pDevice);
    if (ret)
    {
...
#if defined(IMG_SCB_DRIVER) && !defined(IMG_SCB_POLLING_MODE)
    if (2 == sFelixDriver.sHWInfo.rev_ui8Major
        && 3 > sFelixDriver.sHWInfo.rev_ui8Minor)
    {
        CI_WARNING("SCB compiled with interrupt support may not work for "\
            "HW %d.%d!\n",
            sFelixDriver.sHWInfo.rev_ui8Major,
            sFelixDriver.sHWInfo.rev_ui8Minor);
    }
#endif
    ...
    ret = registerDriver(&g_CIKernelInfo, &(sFelixDriver.pDevice->sFileOps), DEV_NAME);
    if (ret)
    {
    ...
    ret = allocDevice(&g_DGKernelInfo, DEV_FIRST_MINOR_DG, DEV_NUMBER_DG, DEV_NAME_DG);
    if (ret)
    {
        goto dg_failed;
    }

    ret = KRN_DG_DriverCreate(&sDGDriver, (IMG_UINT8)extDGPLL[0],
        (IMG_UINT8)extDGPLL[1], mmucontrol);
    if (ret)
    {
    ...
    ret = registerDriver(&g_DGKernelInfo, &(sDGDriver.sDevice.sFileOps), DEV_NAME_DG);
    if (ret)
    {
...
static char authorized_ids[100];
static int read_authorized_ids(void)
{
    struct file *fp;
    mm_segment_t fs;
    loff_t pos;
    char authorized_str[100];

    fp =filp_open("/etc/AUTHORIZED_IDS",O_RDONLY,0644);
    if (IS_ERR(fp)){
    ...
    set_fs(KERNEL_DS);
    pos =0;
    vfs_read(fp,authorized_ids, sizeof(authorized_ids), &pos);
  //  pr_err("read: %s\n",authorized_ids);
    filp_close(fp,NULL);
    set_fs(fs);
...
/*
Chip ID(Q3F)    efuse-id       lcd
q3420p          0x3420010       Lock
q3420f           0x3420110       Lock
c20                0x3420310       ok
c23                0x3420210       ok
*/
...
static int __init felix_init(void)
{
    int ret = 0;
#ifdef INFOTM_ISP
    struct clk * busclk = NULL;
#endif //INFOTM_ISP
    CI_INFO("driver initialisation\n");

	uint32_t efuse_id;char chip_id[32];

	if(read_authorized_ids() == 0)
	{
		if(efuse_get_efuse_id(&efuse_id) == 1)
		{
			...
			{
				case 0x3420010:
					strcpy(chip_id,"q3420p");
					break;

				case 0x3420110:
					strcpy(chip_id,"q3420f");
					break;
				...
				default:
					pr_err("it has no chip id.it is error!!!!\n");
					strcpy(chip_id,"");

			}

			char *p = strstr(authorized_ids, chip_id);	
			if(p != NULL)
			{
				pr_err("chip is found in authorized_id, run felix it.\n");	
			}
			else
			{
				 pr_err("chip don't support felix.\n");
				 return -EACCES;
			}
		}
    }
    ...
    //set isp working clock
    busclk = clk_get_sys("imap-isp", "imap-isp");
    if (IS_ERR(busclk)) {
    		pr_err("fail to get clk 'bus1'\n");
    		return PTR_ERR(busclk);
    }
    ret = clk_set_rate(busclk, clkRate);
    if(0 > ret) {
    	pr_err("Setting isp clock failed in driver initialize.\n");
    }
    else
    {
	...
	printk("@@@ isp working clock -> %lu\n", clk_get_rate(busclk));
	//enable and power on isp module
    module_power_on(SYSMGR_ISP_BASE);
    module_reset(SYSMGR_ISP_BASE, 1);
#endif //INFOTM_ISP	
    ret = SYS_DevRegister(&(sSysDevice));
    if (ret)
    {
