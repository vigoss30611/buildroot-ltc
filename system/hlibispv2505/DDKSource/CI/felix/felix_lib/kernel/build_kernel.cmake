...
if (${BUILD_SCB_DRIVER})
    ...
    add_definitions(
        -DIMG_SCB_DRIVER
        -DSCB_REF_CLK=${CI_HW_REF_CLK}
        )
    message(STATUS "    support SCB device")
    if (${IMG_SCB_POLLING_MODE})
        add_definitions(-DIMG_SCB_POLLING_MODE)
        message(STATUS "        -> polling mode instead of interrupts")
    else()
        message(STATUS "        -> interrupt mode")
    endif()
...
elseif (CI_ALLOC STREQUAL "IMGVIDEO")
  ...
  if(${SYSMEM_DMABUF_IMPORT})
      add_definitions(-DSYSMEM_DMABUF_IMPORT)
  endif()
  
  message(STATUS "    IMGVideo support DMABUF import: ${SYSMEM_DMABUF_IMPORT}")
...
if(CMAKE_VERSION VERSION_GREATER "3.3")
    # to handle CMP0059
    # ; separated no -D
    get_directory_property(KO_CFLAGS COMPILE_DEFINITIONS)
    # add the -D
    GenSpaceList("${KO_CFLAGS}" "-D" KO_CFLAGS)
else()
    # spaces and -D
    get_directory_property(KO_CFLAGS DEFINITIONS)
endif()
#message("Found '${KO_CFLAGS}'")
...
if (DEFINED REGDEF_BINARY_INCLUDE) # if generating VHC files we need to copy the cmake output
  set(KO_INCLUDES
    ...
    ${LINUX_KERNEL_BUILD_DIR}/arch/arm/mach-q3f/include    
    )
