...
static ion_phys_addr_t getDevMemPhysAddress(ion_phys_addr_t cpuDevMemPhysAddr)
{
     ...
     */
    ion_phys_addr_t offset = cpuDevMemPhysAddr
        - g_psCIDriver->pDevice->uiMemoryPhysical;
...
static IMG_UINT64* memGetPhysPages(const SYS_MEM *mem,
    enum physicalAddrType addrType)
{
    IMG_UINT64 *physList;
    ...
    struct vm_area_struct *vma)
{
    ...
    {
        /* convert to a page offset */
        pgoff = physPages[pageIdx] >> PAGE_SHIFT;
