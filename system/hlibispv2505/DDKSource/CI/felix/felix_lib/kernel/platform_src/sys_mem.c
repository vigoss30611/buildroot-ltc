...
#ifdef INFOTM_ISP
        IMG_STRNCPY(psMem->szMemName, pszName, sizeof(psMem->szMemName) - 1);
        psMem->pPhysAddress = (void *)Platform_MemGetDevMem(psMem, 0);
        psMem->pVirtualAddress = (void*)Platform_MemGetMemory(psMem);
    ...
    unsigned int flag)
{
    ...
    if ( psMem->pDevVirtAddr != NULL )
    {
        ...
        printk("%s, pMapping=%p, VirtualAddress=0x%p, uiAllocSize=0x%zx\n",
                    psMem->szMemName,
                    psMem->pMapping,
                    (void*)devVirtAddr.uiVirtualAddress,
                    devVirtAddr.uiAllocSize);
        printk("----------------------------------------------------------------------------------------------------\n");
#endif //INFOTM_ENABLE_ISP_DEBUG
...
#ifdef INFOTM_ENABLE_ISP_DEBUG
//	printk("@@@ SYS_MemWriteAddress: MemName=%s, pCurrMem=0x%p, uiOff2=%X\n", psMem->szMemName, pCurrMem, uiOff2);
//	printk("@@@ SYS_MemWriteAddress: MemName=%s, pToWrite=0x%p, uiFirstPage=%X, uiToWriteOffset=%X\n",
                //pToWrite->szMemName, pToWrite->pDevVirtAddr, uiFirstPage, uiToWriteOffset);
#endif //INFOTM_ENABLE_ISP_DEBUG
...
#ifdef INFOTM_ENABLE_ISP_DEBUG
//	printk("@@@ SYS_MemWriteAddress: MemName=%s, pCurrMem=0x%p, uiOff2=%X\n", psMem->szMemName, pCurrMem, uiOff2);
//	printk("@@@ SYS_MemWriteAddress: MemName=%s, pToWrite=0x%p, toWriteMem=%X, uiToWriteOffset=%X\n",
                    //pToWrite->szMemName, pToWrite->pDevVirtAddr, toWriteMem, uiToWriteOffset);
#endif //INFOTM_ENABLE_ISP_DEBUG
#endif
    }
