...
#ifdef debug_verbose
    CI_DEBUG("transform CPU virtual=0x%x (base 0x%x) to device "\
        "physical 0x%x+0x%x=0x%x\n",
        (IMG_UINT32)virtAddr, (IMG_UINT32)g_carveoutMemHeap->uiVirtAddrStart,
        (IMG_UINT32)physBaseAddr, (IMG_UINT32)offset,
        (IMG_UINT32)(physBaseAddr + offset));
#endif
    ...
    IMG_RESULT *ret)
{
    ...
    CI_DEBUG("using carveout memory cpu: 0x%" IMG_PTRDPR "x phys: 0x%" \
     ...
     * list is NULL).
     * Allocation may be bigger than a few pages, we need to check if we
     * should use vmalloc instead.
     */
    ...
    {
        /* the memory is carved out: it is linear - no need for complex
         * computation */
        memAlloc->pPhysList[i] = memAlloc->uiPhysAddress + i * PAGE_SIZE;
    }

    /* this would call TALMEM_Malloc but TAL light does not do anything for
     * allocations
...
struct MMUPage* Platform_MMU_MemPageAlloc(void)
{
    ...
    CI_DEBUG("using carveout PCI memory (MMU) cpu: 0x%" IMG_PTRDPR \
        "x phys: 0x%" IMG_I64PR "x %"IMG_SIZEPR"u bytes\n",
