...
static int fill_buffer(aac_buffer *b)
{
    ...
    if (b->bytes_consumed > 0)
    {
        ...
        if (!b->at_eof)
        {
            ...
            printf("bread=%d,b->bytes_into_buffer=%d,  b->bytes_consumed=%d\n",bread,b->bytes_into_buffer,b->bytes_consumed);
                  ...
                  float *song_length)
{
	...
	printf("def_srate=%d,object_type=%d,outputFormat=%d,downMatrix=%d,old_format=%d\n",def_srate,object_type,outputFormat,downMatrix,old_format);
    ...
    *song_length = length;
        ...
        b.bytes_into_buffer, &samplerate, &channels)) < 0)
    {
		...
		printf("4samplerate=%d,channels=%d\n",samplerate,channels);
    ...
    do
    {
       ...
       printf("frameInfo.bytesconsumed=%d,b.bytes_into_buffer=%d,frameInfo.samples=%d\n",frameInfo.bytesconsumed,b.bytes_into_buffer,frameInfo.samples);
