...
int IPU_ISPOSTV1::UnitControl(std::string func, void *arg) {
    ...
    UCFunction(videobox_control) {
        ...
        switch (c->code) {
            case VBCTRL_INDEX:
                if(c->para < 0) return IspostGetLcTargetIndex();
                else return IspostSetLcTargetIndex(c->para);
            case VBCTRL_GET:
                return IspostGetLcTargetCount();
        }
        return 0;
    }
...
toFail:
    ...
    UCFunction(camera_set_ispost) {
        //LOGE("UnitCtrl:camera set ispostv1 : %s\n", func.c_str());
  #if 0
    ...
    UCFunction(camera_get_ispost) {
        //LOGE("UnitCtrl:camera get ispostv1 : %s\n", func.c_str());
  #if 0
