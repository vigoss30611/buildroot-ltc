...
int IPU_H1264::UnitControl(std::string func, void *arg) {
    ...
    UCFunction(video_get_resolution) {
        ...
        res->w = pH264LocPpCfg.offsetW != 0 ? pH264LocPpCfg.offsetW : p->GetWidth();
        res->h = pH264LocPpCfg.offsetH != 0 ? pH264LocPpCfg.offsetH : p->GetHeight();
        return 0;
    }
    ...
    UCFunction(video_get_header) {
        ...
        if(pH264HeaderLen <= VIDEO_HEADER_MAXLEN) {
            memcpy(arg, pH264Header, pH264HeaderLen);
            LOGD("copy %d bytes header to RPC call\n", pH264HeaderLen);
            return pH264HeaderLen;
        }
        return VBENOFUNC;
    }
