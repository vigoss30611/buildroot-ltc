...
const char *GopCfgDefault_GOPSize_1[] = {
    "Frame1:  P    1   0        0.8       0     1       -1      1",
    NULL,
};

const char *GopCfgDefault_GOPSize_2[] = {
    "Frame1:  P    1   0        0.4624    1     1       -1      1",
    "Frame2:  P    2   0        0.4624    0     1       -2      1",
    NULL,
};

const char *GopCfgDefault_GOPSize_4[] = {
...
IPU_H2::IPU_H2(std::string name, Json *js) {
    ...
    if(NULL != js)
    {
        ...
        this->InfpsRatio = -1;
        this->smartrc = 0;
        this->s32RefMode = GOPCFG1;
    }

    this->rate_ctrl_info.mbrc = 1;
    ...
    memset(&this->RoiMapDeltaQpFrBuf, 0, sizeof(struct fr_buf_info));
...
void IPU_H2::Prepare() {
    ...
    this->enCtbRcChange = E_STATUS_INIT;
...
//        this->EncConfig.height = this->VencPpCfg.offsetH & (~7);
    ...
    this->EncIn.gopConfig.size = 0;
    this->EncIn.gopConfig.id = 0;

    if(SetGopConfigMode(this->s32RefMode)) {
...
void IPU_H2::Unprepare() {
    ...
    if(this->EncIn.gopConfig.pGopPicCfg != NULL) {
        free(this->EncIn.gopConfig.pGopPicCfg);
        this->EncIn.gopConfig.pGopPicCfg = NULL;
    }
    if (this->m_pstSei)
    {
        free(this->m_pstSei);
...
void sliceReadyCbFunc(HEVCEncSliceReady *Slice) {
    //uint32_t i;
    // void *SliceAppDataPtr = Slice->pAppData;
    //IPU_H2 *H2 = (IPU_H2 *)SliceAppDataPtr;
...
void IPU_H2::HevcEncodeUpdateFPS()
{
    ...
    this->EncConfig.profile = HEVCENC_MAIN_PROFILE;
    this->EncConfig.bitDepthLuma = 8;
    this->EncConfig.bitDepthChroma = 8;

    this->MutexHwLock->lock();
    //HEVCEncInitOn();
    this->EncConfig.interlacedFrame = 0;
    this->EncConfig.refFrameAmount = 1;
...
void IPU_H2::HevcBasicParaInit(void) {
    ...
    this->VencRoiAreaInfo.roi[0].enable = 0;
    this->VencRoiAreaInfo.roi[1].enable = 0;
...
int IPU_H2::HevcEncodeSetCodingCfg(void){
    HEVCEncRet ret ;
    u32 u32CtbPerRow = 0, u32CtbPerCol = 0;
...
int IPU_H2::HevcEncodeSetRateCtrl(void){
    ...
    this->EncRateCtrl.ctbRc = this->VencRateCtrl.mbrc;
    this->EncRateCtrl.blockRCSize = 2;
    ...
    this->EncRateCtrl.gopLen = this->VencRateCtrl.gop_length;
    this->EncRateCtrl.monitorFrames = 120; //this value is not care
    ...
    */
    ...
    this->EncRateCtrl.bitVarRangeI = 2000;
    this->EncRateCtrl.bitVarRangeP = 2000;
    this->EncRateCtrl.bitVarRangeB = 2000;
...
int IPU_H2::HevcInstanceInit(void) {
    ...
    uint32_t idx, maxRefPics = 0;
    int maxTemporalId = 0;;
    for (idx = 0; idx < this->EncIn.gopConfig.size; idx ++)
    {
      HEVCGopPicConfig *cfg = &(this->EncIn.gopConfig.pGopPicCfg[idx]);
    ...
    this->EncConfig.refFrameAmount = maxRefPics + this->EncConfig.interlacedFrame;
    this->EncConfig.maxTLayers = maxTemporalId + 1;

    this->EncConfig.frameRateNum = this->Infps;
    //HEVCEncInitOn();
    LOGE("##########h265 init fps ->%d\n", this->EncConfig.frameRateNum);
    HEVCEncRet encRet = HEVCEncInit(&this->EncConfig, &this->EncInst);
    if (encRet != HEVCENC_OK)
    {
        LOGE("init hevc error ->%d\n", encRet);
        ret = H2_RET_ERROR;
    }
    this->MutexHwLock->unlock();
    return ret;
...
int IPU_H2::HevcEncodeStartStream(Buffer &OutBuf) {
    ...
    printf("hevc stream header ->%d\n", OutBuf.fr_buf.size);
    char *p = (char *)OutBuf.fr_buf.virt_addr;
    for (int i = 0; i < OutBuf.fr_buf.size; i++)
    {
        if (i % 16 == 0)
        {
            printf("\n");
...
int IPU_H2::HevcEncodePictureStream(Buffer &InBuf, Buffer &OutBuf) {
    ...
    if (this->TryLockSettingResource())
    {
        ...
        if (this->VencRateCtrlUpateFlag && this->enCtbRcChange == E_STATUS_INIT) {
...
#if HEVC_PERFORMANCE_TEST == 1
    LOGE("HEVCEncStrmEncode: cnt:%4d type:%2d ->ret:%d time:%dms\n",this->TotolFrameCounter,this->EncIn.codingType, ret, tp.Peek()/10);
#endif
    ...
    switch (ret) {
        case HEVCENC_FRAME_READY:
            ...
            if(this->EncIn.codingType == HEVCENC_INTRA_FRAME) {
                if(frame_mode == VENC_HEADER_IN_FRAME_MODE)
                {
                    OutBuf.fr_buf.size += AlignSize;
                }
                if(enCtbRcChange == E_STATUS_UPDATING)
                {
                    enCtbRcChange = E_STATUS_INIT;
                }
            }
            ...
            if(!this->EncOut.streamSize)
                break;
            //deal poc
            int cur_poc, delta_poc_to_next;
            if(this->EncIn.gopSize > 1) {
                if (this->EncIn.codingType == HEVCENC_INTRA_FRAME) {
                    cur_poc = 0;
                    this->EncIn.gopPicIdx = 0;
                } else {
                    cur_poc = this->EncIn.gopConfig.pGopPicCfg[this->EncIn.gopPicIdx].poc;
                    this->EncIn.gopPicIdx = (this->EncIn.gopPicIdx + 1) % this->EncIn.gopSize;
                    if (this->EncIn.gopPicIdx == 0)
                        cur_poc -= this->EncIn.gopSize;
                }
                delta_poc_to_next = this->EncIn.gopConfig.pGopPicCfg[this->EncIn.gopPicIdx].poc - cur_poc;
                this->EncIn.poc += delta_poc_to_next;
                this->EncIn.gopConfig.id = this->EncIn.gopPicIdx;
            } else {
                this->EncIn.poc++;
            }
            break;
...
int IPU_H2::SetGopConfigMode(uint32_t mode)
{
    if(mode < 0 || mode > 2) {
    ...
    this->GopConfigMode = mode;
    switch (mode) {
        case GOPCFG1:
            config = GopCfgDefault_GOPSize_1;
            gopSize = 1;
            break;
        case GOPCFG2:
            config = GopCfgDefault_GOPSize_2;
            gopSize = 2;
            break;
        case GOPCFG4:
            config = GopCfgDefault_GOPSize_4;
            gopSize = 4;
            break;
        default:
            break;
    ...
    this->EncIn.gopSize = gopSize;
    this->EncIn.gopPicIdx = 0;
    this->EncIn.gopConfig.pGopPicCfg = (HEVCGopPicConfig*)calloc(gopSize, sizeof(HEVCGopPicConfig));

    while (config[id])
    {
      if(ParseGopConfigString (config[id], &(this->EncIn.gopConfig), id, gopSize)) {
...
const char * IPU_H2::nextToken (const char *str)
{
  char *p = strchr(str, ' ');
  if (p)
  {
...
void IPU_H2::ShowHevcEncIn(HEVCEncIn *cfg)
{
    uint32_t i, j;
  ...
  for(i = 0;i < cfg->gopConfig.size; i++, pGopPicCfg++ ){
    printf("\tpoc   -> %d\n", pGopPicCfg->poc  );
    printf("\tQpOffset   -> %d\n", pGopPicCfg->QpOffset  );
    ...
    for(j = 0;j < pGopPicCfg->numRefPics;j++){
        printf("\tref_pic   -> %d\n", pGopPicCfg->refPics[j].ref_pic);
        printf("\tused_by_cur   -> %d\n", pGopPicCfg->refPics[j].used_by_cur);
    }
  }
}

int IPU_H2::ParseGopConfigString (const char *line, HEVCGopConfig *gopCfg, int frame_idx, int gopSize)
{
  if (!line)
    return -1;
  ...
  for (i = 0; i < num_ref_pics; i ++)
  {
    ...
    {
      LOGE("GOP Config: Error, Reference picture %d for GOP size = %d is not supported\n", cfg->refPics[i].ref_pic, gopSize);
      return -1;
    }
  }
...
int IPU_H2::GetRateCtrl(struct v_rate_ctrl_info *cfg)
{
    ...
    cfg->mbrc = this->VencRateCtrl.mbrc;
    this->UnlockSettingResource();
    return 0;
}
...
int IPU_H2::SetRateCtrl(struct v_rate_ctrl_info *cfg)
{
    if(!cfg || cfg->rc_mode < VENC_CBR_MODE || cfg->rc_mode > VENC_FIXQP_MODE || cfg->idr_interval <= 0 ||
        ...
        cfg->hrd < 0 || cfg->hrd > 1 || cfg->hrd_cpbsize < 0 || cfg->mbrc < 0 || cfg->mbrc > 1) {
    ...
    enCtbRcChange = this->VencRateCtrl.mbrc == cfg->mbrc ? E_STATUS_INIT: E_STATUS_SETTING;
    this->VencRateCtrl.mbrc = cfg->mbrc;
    this->VencRateCtrlUpateFlag =  true ;
    this->UnlockSettingResource();
    return 0;
}
...
void IPU_H2::CheckCtbRcUpdate(void)
{
    Buffer bfin, bfout;
    Port *pin, *pout;
    bool bReset = false;

    bReset = this->VencRateCtrlUpateFlag && (this->enCtbRcChange == E_STATUS_SETTING);
    if (bReset)
    {
        pin = GetPort("frame");
        pout = GetPort("stream");
        bfin = pin->GetBuffer(&bfin);
        bfout = pout->GetBuffer(&bfout);
        if(HevcEncodeEndStream(&bfin, &bfout) != H2_RET_OK)
        {
            LOGE("HevcEncodeEndStream failed.\n");
            throw VBERR_BADPARAM;
        }
        ...
        pout->PutBuffer(&bfout);
        this->VencRateCtrlUpateFlag = false;
        this->enCtbRcChange = E_STATUS_UPDATING;
        this->TriggerKeyFrameFlag =  true ;
    }
}
