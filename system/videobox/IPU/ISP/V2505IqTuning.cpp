...
int IPU_ISP::GetBLCParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_MDLE_BLC* pModBLC = (ISP_MDLE_BLC*)((unsigned char*)pData + sizeof(TCPCommand));
    ISPC::ModuleBLC *pIspBLC = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleBLC>();
    int iRet = -1;

    if (!g_ISP_Camera[Idx].pCamera || !g_ISP_Camera[Idx].pCamera->sensor || !pTcp || !pModBLC || !pIspBLC) {
        LOGE("V2505 get BLC failed - Camera=%p, Sensor=%p, Tcp=%p, pModBLC=%p, pIspBLC=%p\n",
             g_ISP_Camera[Idx].pCamera, g_ISP_Camera[Idx].pCamera->sensor, pTcp, pModBLC, pIspBLC);
        return iRet;
    }
...
int IPU_ISP::SetBLCParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_MDLE_BLC* pModBLC = (ISP_MDLE_BLC*)((unsigned char*)pData + sizeof(TCPCommand));
    ISPC::ModuleBLC *pIspBLC = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleBLC>();
...
int IPU_ISP::GetDNSParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_MDLE_DNS* pModDNS = (ISP_MDLE_DNS *)((unsigned char*)pData + sizeof(TCPCommand));
    ISPC::ModuleDNS *pIspDNS = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleDNS>();
    int iRet = -1;

    if (!g_ISP_Camera[Idx].pCamera || !g_ISP_Camera[Idx].pCamera->sensor || !pTcp || !pModDNS || !pIspDNS) {
        LOGE("V2505 get DNS failed - Camera=%p, Sensor=%p, Tcp=%p, pModDNS=%p, pIspDNS=%p\n",
             g_ISP_Camera[Idx].pCamera, g_ISP_Camera[Idx].pCamera->sensor, pTcp, pModDNS, pIspDNS);
        return iRet;
    }
    switch (pTcp->cmd.ui3Type) {
        case CMD_TYPE_SINGLE:
...
int IPU_ISP::SetDNSParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_MDLE_DNS* pModDNS = (ISP_MDLE_DNS *)((unsigned char*)pData + sizeof(TCPCommand));
    ISPC::ModuleDNS *pIspDNS = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleDNS>();
...
int IPU_ISP::GetDPFParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_MDLE_DPF* pModDPF = (ISP_MDLE_DPF *)((unsigned char*)pData + sizeof(TCPCommand));
    ISPC::ModuleDPF *pIspDPF = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleDPF>();
    int iRet = -1;

    if (!g_ISP_Camera[Idx].pCamera || !g_ISP_Camera[Idx].pCamera->sensor || !pTcp || !pModDPF || !pIspDPF) {
        LOGE("V2505 get DPF failed - Camera=%p, Sensor=%p, Tcp=%p, pModDPF=%p, pIspDPF=%p\n",
             g_ISP_Camera[Idx].pCamera, g_ISP_Camera[Idx].pCamera->sensor, pTcp, pModDPF, pIspDPF);
        return iRet;
    }
    switch (pTcp->cmd.ui3Type) {
        case CMD_TYPE_SINGLE:
...
int IPU_ISP::SetDPFParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_MDLE_DPF* pModDPF = (ISP_MDLE_DPF *)((unsigned char*)pData + sizeof(TCPCommand));
    ISPC::ModuleDPF *pIspDPF = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleDPF>();
...
int IPU_ISP::GetGMAParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_MDLE_GMA* pModGMA = (ISP_MDLE_GMA *)((unsigned char*)pData + sizeof(TCPCommand));
    ISPC::ModuleGMA *pIspGMA = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleGMA>();
    int iRet = -1;

    if (!g_ISP_Camera[Idx].pCamera || !g_ISP_Camera[Idx].pCamera->sensor || !pTcp || !pModGMA || !pIspGMA) {
        LOGE("V2505 get GMA failed - Camera=%p, Sensor=%p, Tcp=%p, pModGMA=%p, pIspGMA=%p\n",
             g_ISP_Camera[Idx].pCamera, g_ISP_Camera[Idx].pCamera->sensor, pTcp, pModGMA, pIspGMA);
        return iRet;
    }
    switch (pTcp->cmd.ui3Type) {
        case CMD_TYPE_SINGLE:
...
int IPU_ISP::SetGMAParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_MDLE_GMA* pModGMA = (ISP_MDLE_GMA *)((unsigned char*)pData + sizeof(TCPCommand));
    ISPC::ModuleGMA *pIspGMA = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleGMA>();
...
int IPU_ISP::GetR2YParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_MDLE_R2Y* pModR2Y = (ISP_MDLE_R2Y*)((unsigned char*)pData + sizeof(TCPCommand));
    ISPC::ModuleR2Y *pIspR2Y = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleR2Y>();
    int iRet = -1;

    if (!g_ISP_Camera[Idx].pCamera || !g_ISP_Camera[Idx].pCamera->sensor || !pTcp || !pModR2Y || !pIspR2Y) {
        LOGE("V2505 get R2Y failed - Camera=%p, Sensor=%p, Tcp=%p, pModR2Y=%p, pIspR2Y=%p\n",
             g_ISP_Camera[Idx].pCamera, g_ISP_Camera[Idx].pCamera->sensor, pTcp, pModR2Y, pIspR2Y);
        return iRet;
    }
    switch (pTcp->cmd.ui3Type) {
        case CMD_TYPE_SINGLE:
...
int IPU_ISP::SetR2YParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_MDLE_R2Y* pModR2Y = (ISP_MDLE_R2Y*)((unsigned char*)pData + sizeof(TCPCommand));
    ISPC::ModuleR2Y *pIspR2Y = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleR2Y>();
...
int IPU_ISP::GetTNMParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_MDLE_TNM* pModTNM = (ISP_MDLE_TNM*)((unsigned char*)pData + sizeof(TCPCommand));
    ISPC::ModuleTNM *pIspTNM = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleTNM>();
    int iRet = -1;

    if (!g_ISP_Camera[Idx].pCamera || !g_ISP_Camera[Idx].pCamera->sensor || !pTcp || !pModTNM || !pIspTNM) {
        LOGE("V2505 get TNM failed - Camera=%p, Sensor=%p, Tcp=%p, pModTNM=%p, pIspTNM=%p\n",
             g_ISP_Camera[Idx].pCamera, g_ISP_Camera[Idx].pCamera->sensor, pTcp, pModTNM, pIspTNM);
        return iRet;
    }
    switch (pTcp->cmd.ui3Type) {
        case CMD_TYPE_SINGLE:
...
int IPU_ISP::SetTNMParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_MDLE_TNM* pModTNM = (ISP_MDLE_TNM*)((unsigned char*)pData + sizeof(TCPCommand));
    ISPC::ModuleTNM *pIspTNM = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleTNM>();
...
int IPU_ISP::GetSHAParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_MDLE_SHA* pModSHA = (ISP_MDLE_SHA*)((unsigned char*)pData + sizeof(TCPCommand));
    ISPC::ModuleSHA *pIspSHA = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleSHA>();
    int iRet = -1;

    if (!g_ISP_Camera[Idx].pCamera || !g_ISP_Camera[Idx].pCamera->sensor || !pTcp || !pModSHA || !pIspSHA) {
        LOGE("V2505 get SHA failed - Camera=%p, Sensor=%p, Tcp=%p, pModSHA=%p, pIspSHA=%p\n",
             g_ISP_Camera[Idx].pCamera, g_ISP_Camera[Idx].pCamera->sensor, pTcp, pModSHA, pIspSHA);
        return iRet;
    }
    switch (pTcp->cmd.ui3Type) {
        case CMD_TYPE_SINGLE:
...
int IPU_ISP::SetSHAParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_MDLE_SHA* pModSHA = (ISP_MDLE_SHA*)((unsigned char*)pData + sizeof(TCPCommand));
    ISPC::ModuleSHA *pIspSHA = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleSHA>();
...
int IPU_ISP::GetWBCAndCCMParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_MDLE_WBC_CCM* pModWbcCcm = (ISP_MDLE_WBC_CCM*)((unsigned char*)pData + sizeof(TCPCommand));
    ISPC::ModuleCCM *pIspCCM = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleCCM>();
    ISPC::ModuleWBC *pIspWBC = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleWBC>();
    ISPC::ControlAWB_PID *pIspAWB = g_ISP_Camera[Idx].pCamera->getControlModule<ISPC::ControlAWB_PID>();
    ...
    if (!g_ISP_Camera[Idx].pCamera || !g_ISP_Camera[Idx].pCamera->sensor || !pTcp || !pModWbcCcm || !pIspCCM || !pIspWBC || !pIspAWB) {
        LOGE("V2505 get WBC_CCM failed - Camera=%p, Sensor=%p, Tcp=%p, pModWbcCcm=%p, pIspCCM=%p, pIspWBC=%p, pIspAWB=%p\n",
             g_ISP_Camera[Idx].pCamera, g_ISP_Camera[Idx].pCamera->sensor, pTcp, pModWbcCcm, pIspCCM, pIspWBC, pIspAWB);
        return iRet;
    }
    switch (pTcp->cmd.ui3Type) {
        case CMD_TYPE_SINGLE:
...
int IPU_ISP::SetWBCAndCCMParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_MDLE_WBC_CCM* pModWbcCcm = (ISP_MDLE_WBC_CCM*)((unsigned char*)pData + sizeof(TCPCommand));
    ISPC::ModuleCCM *pIspCCM = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleCCM>();
...
int IPU_ISP::GetAECParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_CTRL_AE* pCtrlAE = (ISP_CTRL_AE*)((unsigned char*)pData + sizeof(TCPCommand));
    ISPC::ControlAE *pIspAE = g_ISP_Camera[Idx].pCamera->getControlModule<ISPC::ControlAE>();
    int iRet = -1;

    if (!g_ISP_Camera[Idx].pCamera || !g_ISP_Camera[Idx].pCamera->sensor || !pTcp || !pCtrlAE || !pIspAE) {
        LOGE("V2505 get AEC failed - Camera=%p, Sensor=%p, Tcp=%p, pCtrlAE=%p, pIspAE=%p\n",
             g_ISP_Camera[Idx].pCamera, g_ISP_Camera[Idx].pCamera->sensor, pTcp, pCtrlAE, pIspAE);
        return iRet;
    }
    switch (pTcp->cmd.ui3Type) {
        case CMD_TYPE_SINGLE:
        ...
        case CMD_TYPE_ALL:
            ...
            switch (pTcp->ver) {
                ...
                default:
                    ...
                    pCtrlAE->dMaxSensorGain = g_ISP_Camera[Idx].pCamera->sensor->getMaxGain();
                    ...
                    pCtrlAE->uiMaxSensorExposure = g_ISP_Camera[Idx].pCamera->sensor->getMaxExposure();
                    if (!pIspAE->isEnabled()) {
                        pCtrlAE->dGain = g_ISP_Camera[Idx].pCamera->sensor->getGain();
                    } else {
                        pCtrlAE->dGain = pIspAE->getNewGain();
                    }
                    if (!pIspAE->isEnabled()) {
                        pCtrlAE->uiExposure = g_ISP_Camera[Idx].pCamera->sensor->getExposure();
                    } else {
...
int IPU_ISP::SetAECParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_CTRL_AE* pCtrlAE = (ISP_CTRL_AE*)((unsigned char*)pData + sizeof(TCPCommand));
    ISPC::ControlAE *pIspAE = g_ISP_Camera[Idx].pCamera->getControlModule<ISPC::ControlAE>();
        ...
        case CMD_TYPE_ALL:
            ...
            switch (pTcp->ver) {
                ...
                default:
                    ...
                    g_ISP_Camera[Idx].pCamera->sensor->setMaxGain(pCtrlAE->dMaxSensorGain);
                    ...
                    //g_ISP_Camera[Idx].pCamera->sensor->setMaxExposure(pCtrlAE->uiMaxSensorExposure);
                    if (!pIspAE->isEnabled()) {
                        g_ISP_Camera[Idx].pCamera->sensor->setGain(pCtrlAE->dGain);
                    } else {
                        //pIspAE->setNewGain(pCtrlAE->dGain);                         // It's read only.
                    }
                    if (!pIspAE->isEnabled()) {
                        g_ISP_Camera[Idx].pCamera->sensor->setExposure(pCtrlAE->uiExposure);
                    } else {
...
int IPU_ISP::GetAWBParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ...
    if (!g_ISP_Camera[Idx].pCamera || !g_ISP_Camera[Idx].pCamera->sensor || !pTcp || !pCtrlAWB || !pIspAWB) {
        LOGE("V2505 get AWB failed - Camera=%p, Sensor=%p, Tcp=%p, pCtrlAWB=%p, pIspAWB=%p\n",
             g_ISP_Camera[Idx].pCamera, g_ISP_Camera[Idx].pCamera->sensor, pTcp, pCtrlAWB, pIspAWB);
        return iRet;
    }
    switch (pTcp->cmd.ui3Type) {
        case CMD_TYPE_SINGLE:
...
int IPU_ISP::SetAWBParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_CTRL_AWB_PID* pCtrlAWB = (ISP_CTRL_AWB_PID*)((unsigned char*)pData + sizeof(TCPCommand));
    ISPC::ControlAWB_PID *pIspAWB = g_ISP_Camera[Idx].pCamera->getControlModule<ISPC::ControlAWB_PID>();
...
int IPU_ISP::GetCMCParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_CTRL_CMC* pCtrlCMC = (ISP_CTRL_CMC*)((unsigned char*)pData + sizeof(TCPCommand));
    ISPC::ControlCMC *pIspCMC = g_ISP_Camera[Idx].pCamera->getControlModule<ISPC::ControlCMC>();
    unsigned int uiNoOfGainLevelUsed[ISP_CMC_COLOR_MODE_MAX];
    int iIdx;
    int iRet = -1;

    if (!g_ISP_Camera[Idx].pCamera || !g_ISP_Camera[Idx].pCamera->sensor || !pTcp || !pCtrlCMC || !pIspCMC) {
        LOGE("V2505 get CMC failed - Camera=%p, Sensor=%p, Tcp=%p, pCtrlCMC=%p, pIspCMC=%p\n",
             g_ISP_Camera[Idx].pCamera, g_ISP_Camera[Idx].pCamera->sensor, pTcp, pCtrlCMC, pIspCMC);
        return iRet;
    }
    switch (pTcp->cmd.ui3Type) {
        case CMD_TYPE_SINGLE:
...
int IPU_ISP::SetCMCParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_CTRL_CMC* pCtrlCMC = (ISP_CTRL_CMC*)((unsigned char*)pData + sizeof(TCPCommand));
    ISPC::ControlCMC *pIspCMC = g_ISP_Camera[Idx].pCamera->getControlModule<ISPC::ControlCMC>();
...
int IPU_ISP::GetTNMCParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_CTRL_TNM* pCtrlTNM = (ISP_CTRL_TNM*)((unsigned char*)pData + sizeof(TCPCommand));
    ISPC::ControlTNM *pIspTNMC = g_ISP_Camera[Idx].pCamera->getControlModule<ISPC::ControlTNM>();
    int iRet = -1;

    if (!g_ISP_Camera[Idx].pCamera || !g_ISP_Camera[Idx].pCamera->sensor || !pTcp || !pCtrlTNM || !pIspTNMC) {
        LOGE("V2505 get TNMC failed - Camera=%p, Sensor=%p, Tcp=%p, pCtrlTNM=%p, pIspTNMC=%p\n",
             g_ISP_Camera[Idx].pCamera, g_ISP_Camera[Idx].pCamera->sensor, pTcp, pCtrlTNM, pIspTNMC);
        return iRet;
    }
    switch (pTcp->cmd.ui3Type) {
        case CMD_TYPE_SINGLE:
...
int IPU_ISP::SetTNMCParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_CTRL_TNM* pCtrlTNM = (ISP_CTRL_TNM*)((unsigned char*)pData + sizeof(TCPCommand));
    ISPC::ControlTNM *pIspTNMC = g_ISP_Camera[Idx].pCamera->getControlModule<ISPC::ControlTNM>();
...
int IPU_ISP::GetIICParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
...
int IPU_ISP::SetIICParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    IIC_MDLE_IIC* pModIIC = (IIC_MDLE_IIC*)((unsigned char*)pData + sizeof(TCPCommand));
    int iRet = -1;
...
int IPU_ISP::GetOutParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
...
int IPU_ISP::SetOutParam(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_MDLE_OUT* pModOUT = (ISP_MDLE_OUT*)((unsigned char*)pData + sizeof(TCPCommand));
    int iRet = -1;
...
int IPU_ISP::GetIspVersion(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
...
int IPU_ISP::GetIqTuningDebugInfo(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
...
int IPU_ISP::SetIqTuningDebugInfo(IMG_UINT32 Idx, void* pData)
{
    TCPCommand* pTcp = (TCPCommand*)pData;
    ISP_MDLE_DBG* pModDbg = (ISP_MDLE_DBG*)((unsigned char*)pData + sizeof(TCPCommand));
    int iRet = -1;
...
int IPU_ISP::GetIQ(IMG_UINT32 Idx, void* pdata)
{
    TCPCommand* pTcp = (TCPCommand*)pdata;
    int iRet = -1;
...
int IPU_ISP::SetIQ(IMG_UINT32 Idx, void* pdata)
{
    TCPCommand* pTcp = (TCPCommand*)pdata;
    int iRet = -1;
...
int IPU_ISP::BackupIspParameters(IMG_UINT32 Idx, ISPC::ParameterList & Parameters)
{

    if (IMG_SUCCESS == g_ISP_Camera[Idx].pCamera->saveParameters(Parameters)) {
...
int IPU_ISP::SaveIspParameters(IMG_UINT32 Idx, cam_save_data_t *saveData)
{
    ...
    //ISPC::ModuleBLC *pIspMdleBLC = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleBLC>();
    //ISPC::ModuleCCM *pIspMdleCCM = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleCCM>();
    //ISPC::ModuleDNS *pIspMdleDNS = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleDNS>();
    //ISPC::ModuleDPF *pIspMdleDPF = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleDPF>();
    //ISPC::ModuleGMA *pIspMdleGMA = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleGMA>();
    //ISPC::ModuleHIS *pIspMdleHIS = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleHIS>();
    //ISPC::ModuleIIF *pIspMdleIIF = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleIIF>();
    //ISPC::ModuleLCA *pIspMdleLCA = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleLCA>();
    //ISPC::ModuleLSH *pIspMdleLSH = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleLSH>();
    //ISPC::ModuleOUT *pIspMdleOUT = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleOUT>();
    //ISPC::ModuleR2Y *pIspMdleR2Y = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleR2Y>();
    //ISPC::ModuleRLT *pIspMdleRLT = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleRLT>();
    //ISPC::ModuleSHA *pIspMdleSHA = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleSHA>();
    //ISPC::ModuleTNM *pIspMdleTNM = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleTNM>();
    //ISPC::ModuleWBC *pIspMdleWBC = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleWBC>();
    //ISPC::ModuleWBS *pIspMdleWBS = g_ISP_Camera[Idx].pCamera->pipeline->getModule<ISPC::ModuleWBS>();
    //ISPC::ControlAE *pIspCtrlAE = g_ISP_Camera[Idx].pCamera->getControlModule<ISPC::ControlAE>();
    //ISPC::ControlAWB_PID *pIspCtrlAWB = g_ISP_Camera[Idx].pCamera->getControlModule<ISPC::ControlAWB_PID>();
    ISPC::ControlCMC *pIspCtrlCMC = g_ISP_Camera[Idx].pCamera->getControlModule<ISPC::ControlCMC>();
    //ISPC::ControlLSH *pIspCtrlLSH = g_ISP_Camera[Idx].pCamera->getControlModule<ISPC::ControlLSH>();
    //ISPC::ControlTNM *pIspCtrlTNMC = g_ISP_Camera[Idx].pCamera->getControlModule<ISPC::ControlTNM>();
    int iRet = -1;
