...
#include <assert.h>
#include <stdio.h>
#define EXIF_DUMMY_HANDLE(x)  if(1){ printf("dummy funcion ->%s -->%s", __func__, x);assert(1);}
#pragma message "-------->dummy exif api"
/*
    func:jpeg_set_exif
    args:
...
void inline jpeg_set_exif(char *jpeg_file, char *buf, int size){
    EXIF_DUMMY_HANDLE("message");
}
int jpeg_get_exif(char *jpeg_file,char *buf, int size){
    EXIF_DUMMY_HANDLE("message");
    return 0;
