...
static int org_gen_griddata(fe_gd_param_t *gd_param_ptr)
{
	int ret = -1;
	int i = 0;
	...
	grid_param.fisheyeMode = FISHEYE_MODE_CENTERSCALE;
...
static int org_proc(fe_view_param_t *view_param_ptr)
{
	int ret = 0;
	...
	if (!view_param_ptr->common_param.is_global_combine) {
		...
		gd_param.common_param.out_width = gd_param.common_param.in_width;
		gd_param.common_param.out_height = gd_param.common_param.in_height;

		print_split_line();
	}
	gd_param.angle_unit = LOCAL_ANGLE_UNIT;

	ret = org_gen_griddata(&gd_param);
