...
/*local parameters*/
#define VIEW_ID "up_2p360"
#define MODE_NAME "panorama"
#define MAX_IDX 2
#define LOCAL_OUT_WIDTH   1920
#define LOCAL_OUT_HEIGHT  544
#define LOCAL_GRID_SIZE   32
#define LOCAL_ANGLE_UNIT (360 / MAX_IDX)

static int up2p360_gen_griddata(fe_gd_param_t *gd_param_ptr)
{
	int ret = -1;
	int i = 0;
	...
	int grid_size = 0;
	int org_start_theta = 0.0;
	int org_end_theta = 0.0;
	...
	grid_param.fisheyeMode = FISHEYE_MODE_UPVIEW;
	...
	grid_param.fisheyeRadius = -10;
	grid_param.fisheyeStartPhi = 80;
	grid_param.fisheyeEndPhi = 10;

	grid_param.fisheyeStartTheta = 180;
	grid_param.fisheyeEndTheta = 0;
	...
	org_start_theta = grid_param.fisheyeStartTheta;
	org_end_theta = grid_param.fisheyeEndTheta;
	for (i = 0; i < MAX_IDX; ++i) {
		grid_param.fisheyeStartTheta = org_start_theta + i * gd_param_ptr->angle_unit;
		grid_param.fisheyeEndTheta = org_end_theta + i * gd_param_ptr->angle_unit;
...
static int up2p360_proc(fe_view_param_t *view_param_ptr)
{
	int ret = 0;
	...
	ret = up2p360_gen_griddata(&gd_param);
