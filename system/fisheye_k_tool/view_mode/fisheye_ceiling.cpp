...
/*local parameters*/
#define VIEW_ID "ceiling"
#define MODE_NAME "ceiling"
#define MAX_IDX          900
#define ROTATE_IDX       90
#define PITCH_IDX        10
#define PITCH_UNIT        3
#define LOCAL_OUT_WIDTH   1280
#define LOCAL_OUT_HEIGHT  720
#define LOCAL_GRID_SIZE   32
#define LOCAL_ANGLE_UNIT (360 / ROTATE_IDX)


static int cl_gen_griddata(fe_gd_param_t *gd_param_ptr)
{
	int ret = -1;
	int i = 0;
	int j = 0;
	...
	grid_param.fisheyePitchAngle = 65;

	grid_param.fisheyeFovAngle = 65;
	...
	org_pitch_angle = grid_param.fisheyePitchAngle;
	org_rotate_angle = grid_param.fisheyeRotateAngle;
	cnt = 0;
	for (i = 0; i < ROTATE_IDX; ++i) {
		grid_param.fisheyeRotateAngle = org_rotate_angle + i * gd_param_ptr->angle_unit;

		for (j = 0; j < PITCH_IDX; ++j, ++cnt) {
			grid_param.fisheyePitchAngle = org_pitch_angle - j * PITCH_UNIT;

			printf("grid_param.fisheyePitchAngle=%f,rotate:%f \n",\
				grid_param.fisheyePitchAngle, grid_param.fisheyeRotateAngle);
			...
			if (grid_combine_fd) {
				...
				fflush(grid_combine_fd);
			} else {
				sprintf(func_name, "fisheyes");
				ret = get_view_file_name(&gd_param_ptr->common_param, 0, 0, func_name,
								(char*)MODE_NAME, cnt, grid_full_name);
				if (ret)
					goto to_out;
			...
			if (is_save_geo_file) {
				if (geo_combine_fd) {
					...
					fflush(geo_combine_fd);
				} else {
					sprintf(func_name, "geo");
					ret = get_view_file_name(&gd_param_ptr->common_param, 0, 0, func_name,
											(char*)MODE_NAME, cnt, geo_full_name);
					if (ret)
						goto to_out;
...
static int cl_proc(fe_view_param_t *view_param_ptr)
{
	int ret = 0;
	...
	ret = cl_gen_griddata(&gd_param);

	return ret;
}

fe_view_t fv_ceiling = {
