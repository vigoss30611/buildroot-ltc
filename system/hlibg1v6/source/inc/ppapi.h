...
#define PP_PIX_FMT_YCRCB_4_2_0_SEMIPLANAR               0x020003U
...
#define EXPORT  __declspec(dllexport)
    ...
    {
        u32 rgbTransform; //系数转换
        i32 contrast;  //对比
        i32 brightness; //亮度
        i32 saturation; //饱和
        u32 alpha;   // 透明度 0x0-透明, 0xFF-不透明
        u32 transparency;//透明度
        PPRgbTransform rgbTransformCoeffs;
        PPRgbBitmask rgbBitmask;
        u32 ditheringEnable; //抖动
    } PPOutRgb;
...
------------------------------------------------------------------------------*/
   EXPORT PPResult PPInit(PPInst * pPostPInst, u32 mmuEnable);

   EXPORT PPResult PPDecCombinedModeEnable(PPInst postPInst, const void *pDecInst,
                                     u32 decType);

   EXPORT PPResult PPDecCombinedModeDisable(PPInst postPInst, const void *pDecInst);

   EXPORT PPResult PPGetConfig(PPInst postPInst, PPConfig * pPpConf);

   EXPORT PPResult PPSetConfig(PPInst postPInst, PPConfig * pPpConf);

    PPResult PPDecSetMultipleOutput(PPInst postPInst,
                                    const PPOutputBuffers * pBuffers);

   EXPORT PPResult PPGetNextOutput(PPInst postPInst, PPOutput * pOut);

   EXPORT void PPRelease(PPInst pPpInst);

   EXPORT PPResult PPGetResult(PPInst postPInst);
