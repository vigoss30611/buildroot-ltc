...
#define PTR_JPGC   pJpegDecCont
    ...
    if(PTR_JPGC->info.sliceStartCount == 0)
    {
        ...
        if(G1DWLReserveHw(pJpegDecCont->dwl) == DWL_ERROR)
        {
...
#define PTR_JPGC   pJpegDecCont
    ...
    /* Flush regs to hw register */
    G1DWLWriteReg(pJpegDecCont->dwl, 0x34, PTR_JPGC->jpegRegs[13]);
    G1DWLWriteReg(pJpegDecCont->dwl, 0x38, PTR_JPGC->jpegRegs[14]);
    G1DWLWriteReg(pJpegDecCont->dwl, 0x3C, PTR_JPGC->jpegRegs[15]);
    G1DWLWriteReg(pJpegDecCont->dwl, 0x14, PTR_JPGC->jpegRegs[5]);
    G1DWLWriteReg(pJpegDecCont->dwl, 0x4, PTR_JPGC->jpegRegs[1]);
...
#define PTR_JPGC   pJpegDecCont
    ...
    /* Flush regs to hw register */
    G1DWLWriteReg(pJpegDecCont->dwl, 0x30, PTR_JPGC->jpegRegs[12]);
    G1DWLWriteReg(pJpegDecCont->dwl, 0x10, PTR_JPGC->jpegRegs[4]);
    G1DWLWriteReg(pJpegDecCont->dwl, 0x14, PTR_JPGC->jpegRegs[5]);
    G1DWLWriteReg(pJpegDecCont->dwl, 0x18, PTR_JPGC->jpegRegs[6]);
    G1DWLWriteReg(pJpegDecCont->dwl, 0x04, PTR_JPGC->jpegRegs[1]);
...
#define PTR_JPGC   pJpegDecCont
    ...
    if(PTR_JPGC->info.operationType == JPEGDEC_PROGRESSIVE)
    {
        ...
        tmp = G1DWLMallocLinear(PTR_JPGC->dwl, (sizeof(u8) * (JPEGDEC_COEFF_SIZE *
                                                            numBlocks)),
        ...
        {
            tmp = G1DWLMallocLinear(PTR_JPGC->dwl, sizeof(u8) * 100,
    ...
    if(!PTR_JPGC->tableBaseExternal) {
        ...
        tmp =
            G1DWLMallocLinear(PTR_JPGC->dwl, (sizeof(u8) * tableSize),
       ...
       (PTR_JPGC->ppInstance != NULL && !PTR_JPGC->ppControl.usePipeline))
    {
        if(PTR_JPGC->info.givenOutLuma.virtualAddress == NULL)
        {
            ...
            tmp =
                G1DWLMallocRefFrm(PTR_JPGC->dwl, (PTR_JPGC->image.sizeLuma),
        ...
        if(PTR_JPGC->image.sizeChroma)
        {
            if(PTR_JPGC->info.givenOutChroma.virtualAddress == NULL)
            {
                ...
                {
                    tmp =
                        G1DWLMallocRefFrm(PTR_JPGC->dwl,
    ...
    {
        (void) G1DWLmemset(PTR_JPGC->asicBuff.outLumaBuffer.virtualAddress,
                         128, PTR_JPGC->image.sizeLuma);
        if(PTR_JPGC->image.sizeChroma)
        {
            if(PTR_JPGC->info.operationType != JPEGDEC_BASELINE)
            {
                (void) G1DWLmemset(PTR_JPGC->asicBuff.outChromaBuffer.
                                 virtualAddress, 128,
                                 PTR_JPGC->image.sizeChroma / 2);
                (void) G1DWLmemset(PTR_JPGC->asicBuff.outChromaBuffer2.
                                 virtualAddress, 128,
                                 PTR_JPGC->image.sizeChroma / 2);
            }
            else
        ...
        (void) G1DWLmemset(PTR_JPGC->frame.pTableBase.virtualAddress, 0,
                         (sizeof(u8) * tableSize));
        if(PTR_JPGC->info.operationType == JPEGDEC_PROGRESSIVE)
        {
            (void) G1DWLmemset(PTR_JPGC->info.pCoeffBase.virtualAddress, 0,
...
void JpegRefreshRegs(JpegDecContainer * pJpegDecCont)
{
    ...
    {
        *ppRegs++ = G1DWLReadReg(pJpegDecCont->dwl, offset);
        offset += 4;
    }
}
...
void JpegFlushRegs(JpegDecContainer * pJpegDecCont)
{
    ...
    G1DWLWriteReg(pJpegDecCont->dwl, 0, 0x00000000);
    ...
    {
        G1DWLWriteReg(pJpegDecCont->dwl, offset, *ppRegs);
        *ppRegs = 0;
        ppRegs++;
        offset += 4;
    }
