...
------------------------------------------------------------------------------*/
JpegDecRet JpegDecInit(JpegDecInst * pDecInst, DWLLinearMem_t *pTableBase, u32 mmuEnable)
{
    ...
    *pDecInst = NULL;   /* return NULL instance for any error */
    ...
    pJpegCont = (JpegDecContainer *) G1DWLmalloc(sizeof(JpegDecContainer));
    if(pJpegCont == NULL)
    {
        (void) G1DWLRelease(dwl);
        return (JPEGDEC_MEMFAIL);
    }
	G1DWLmemset(pJpegCont,0,sizeof(JpegDecContainer));
    pJpegCont->dwl = dwl;
    ...
    if(pTableBase != NULL && pTableBase->busAddress != 0) {
        pJpegCont->tableBaseExternal = 1;
        memcpy(&pJpegCont->frame.pTableBase, pTableBase, sizeof(*pTableBase));
    }
...
#define PTR_JPGC ((JpegDecContainer *) decInst)
    ...
    {
        G1DWLfree(PTR_JPGC->vlc.acTable0.vals);
    }
    if(PTR_JPGC->vlc.acTable1.vals)
    {
        G1DWLfree(PTR_JPGC->vlc.acTable1.vals);
    }
    if(PTR_JPGC->vlc.acTable2.vals)
    {
        G1DWLfree(PTR_JPGC->vlc.acTable2.vals);
    }
    if(PTR_JPGC->vlc.acTable3.vals)
    {
        G1DWLfree(PTR_JPGC->vlc.acTable3.vals);
    }
    if(PTR_JPGC->vlc.dcTable0.vals)
    {
        G1DWLfree(PTR_JPGC->vlc.dcTable0.vals);
    }
    if(PTR_JPGC->vlc.dcTable1.vals)
    {
        G1DWLfree(PTR_JPGC->vlc.dcTable1.vals);
    }
    if(PTR_JPGC->vlc.dcTable2.vals)
    {
        G1DWLfree(PTR_JPGC->vlc.dcTable2.vals);
    }
    if(PTR_JPGC->vlc.dcTable3.vals)
    {
        G1DWLfree(PTR_JPGC->vlc.dcTable3.vals);
    }
    if(PTR_JPGC->frame.pBuffer)
    {
        G1DWLfree(PTR_JPGC->frame.pBuffer);
    }
    /* progressive */
    ...
    if( !PTR_JPGC->tableBaseExternal && PTR_JPGC->frame.pTableBase.virtualAddress)
    {
...
#define JPG_INFO  PTR_JPGC->info
    ...
    if(PTR_JPGC->info.progressiveFinish)
    {
        ...
        dwlret = G1DWLWaitHwReady(PTR_JPGC->dwl, PTR_JPGC->info.timeout);
        ...
        do
        {
            ...
            if(dwlret == DWL_HW_WAIT_OK)
            {
                ...
                if(asic_status & JPEGDEC_X170_IRQ_BUS_ERROR)
                {
                    ...
                    /* Release HW */
                    (void) G1G1DWLReleaseHw(PTR_JPGC->dwl);
        ...
        else
        {
            ...
            else
            {
                if(PTR_JPGC->frame.Nf != 1)
                {
                    ...
                    dwlret = G1DWLWaitHwReady(PTR_JPGC->dwl,
