...
#define NV21_ENABLE 0x0
#define NV21_FIFO_RESET 0x4
#define NV21_ADDRESS_START 0xc
#define NV21_ADDRESS_END 0x10
#define NV21_FIFO_CLEAR 0x14
...
void PPRefreshRegs(PPContainer * ppC)
{
    ...
    {
        *ppRegs++ = G1DWLReadReg(ppC->dwl, offset);
        offset += 4;
    }
}
...
void PPFlushRegs(PPContainer * ppC)
{
    ...
    for(i = PP_X170_REGISTERS; i > 0; i--)
    {
        G1DWLWriteReg(ppC->dwl, offset, *ppRegs);
        ppRegs++;
        offset += 4;
    }

    /* config nv21 register */
    if( ppOutImg->pixFormat == PP_PIX_FMT_YCBCR_4_2_0_SEMIPLANAR ) 
    {
        /*  nv21 disable */
        G1DWLWriteNV21Reg(ppC->dwl, NV21_ENABLE, 0);
        /* clear nv21 fifo */
        G1DWLWriteNV21Reg(ppC->dwl, NV21_FIFO_CLEAR, 0);
    } 


    if( ppOutImg->pixFormat == PP_PIX_FMT_YCRCB_4_2_0_SEMIPLANAR ) 
    {
        /*  nv21 enable */
        G1DWLWriteNV21Reg(ppC->dwl, NV21_ENABLE, 1);
        G1DWLWriteNV21Reg(ppC->dwl, NV21_FIFO_CLEAR, 0);
        G1DWLWriteNV21Reg(ppC->dwl, NV21_ADDRESS_START, ppOutImg->bufferBusAddr);
        G1DWLWriteNV21Reg(ppC->dwl, NV21_ADDRESS_END, ppOutImg->bufferChromaBusAddr + ppC->ppCfg.ppOutFrmBuffer.frameBufferWidth * ppC->ppCfg.ppOutFrmBuffer.frameBufferHeight/2);
    }
...
void PPInitHW(PPContainer * ppC)
{
    ...
    (void) G1DWLmemset(ppRegs, 0, PP_X170_REGISTERS * sizeof(*ppRegs));
...
void PPInitDataStructures(PPContainer * ppC)
{
    ...
    (void) G1DWLmemset(&ppC->ppCfg, 0, sizeof(PPConfig));
...
void PPSetupHW(PPContainer * ppC)
{
    ...
    if(ppOutImg->pixFormat == PP_PIX_FMT_YCBCR_4_2_0_SEMIPLANAR || 
            ppOutImg->pixFormat == PP_PIX_FMT_YCRCB_4_2_0_SEMIPLANAR )
    {
...
PPResult PPRun(PPContainer * ppC)
{
    ...
    if(G1DWLReserveHw(ppC->dwl) != DWL_OK)
    {
        return PP_BUSY;
    }
...
u32 PPIsInPixFmtOk(u32 pix_fmt, const PPContainer * ppC)
{
    ...
    case PP_PIX_FMT_YCRCB_4_2_0_SEMIPLANAR:
        break;
...
PPResult WaitForPp(PPContainer * ppC)
{
    ...
    dwlret = G1DWLWaitHwReady(dwl, (u32) (-1));
