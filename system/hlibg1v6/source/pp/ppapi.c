...
//#define PP_TRACE
#ifdef PP_TRACE
#define PP_API_TRC(str)    printf((str))
#else
...
------------------------------------------------------------------------------*/
PPResult PPInit(PPInst * pPostPInst, u32 mmuEnable)
{
    const void *dwl;
    PPContainer *pPpCont;
    ...
    *pPostPInst = NULL; /* return NULL instance for any error */
       ...
       product != 0x6731 )
    {
		...
		printf("PPInit# ERROR: Unknown hardware\n");
        return PP_DWL_ERROR;
    }
    ...
    pPpCont = (PPContainer *) G1DWLmalloc(sizeof(PPContainer));
    ...
    (void) G1DWLmemset(pPpCont, 0, sizeof(PPContainer));
...
PPResult PPGetConfig(PPInst postPInst, PPConfig * pPpConf)
{
    ...
    (void) G1DWLmemcpy(pPpConf, &ppC->ppCfg, sizeof(PPConfig));
...
PPResult PPSetConfig(PPInst postPInst, PPConfig * pPpConf)
{
    ...
    if(postPInst == NULL || pPpConf == NULL)
    {
    	printf("postPInst is Null, or pPpConf is Null\n");
        return (PP_PARAM_ERROR);
    }
    ...
    if(tmp != (i32) PP_OK)
    {	
    	printf("PPCheckConfig return error!\n");
        return (PPResult) tmp;
    }

    /* store previous cfg in order to notice any changes that require 
     * rerun of buffered pictures */
    (void) G1DWLmemcpy(&ppC->prevCfg, &ppC->ppCfg, sizeof(PPConfig));

    (void) G1DWLmemcpy(&ppC->ppCfg, pPpConf, sizeof(PPConfig));
                                ...
                                const PPOutputBuffers * pBuffers)
{
    ...
    (void) G1DWLmemcpy(&ppC->combinedModeBuffers, pBuffers,
