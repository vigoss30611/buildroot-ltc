...
int trimeSpace(const char *src,char *dest);      // 去空格
...
int PropertiesInit(const char *filepath,void **handle)
{
    ...
    while (!feof(fp)) {
		...
		//INFO("show the key:%s.\tthevalue:%s.\n",keybuff,valuebuff);
...
{
#if SYSTEM_IS_READ_ONLY
    if(!strcasecmp(filepath, RESOURCE_PATH"./config/spv_gui.config")) {
...
#if SYSTEM_IS_READ_ONLY
    fp = fopen(EXTERNAL_MEDIA_DIR"./.spv_gui.config","w");
#else
    fp = fopen(CONFIG_PATH"./spv_gui.config", "w");
#endif
    if (fp == NULL) {
        ret = -200;
