...
#include <control/ctrlmisc.h>
#include "control/scrolled.h"
...
int ScrollViewCtrlProc (HWND hWnd, int message, WPARAM wParam, LPARAM lParam)
{
    ...
    case MSG_CREATE:
    {
        ...
        if (!psvdata) {
            ERROR("malloc for psvdata failed\n");
            return -1;
        }
    ...
    case MSG_KEYUP:
    {
        ...
        while ( hsvi && ((PSVITEMDATA)hsvi)->nItemHeight <= 0 ) {
            if(wParam == SCANCODE_CURSORBLOCKDOWN || wParam == SCANCODE_HOME || wParam == SCANCODE_SPV_DOWN) {
                hsvi = scrollview_get_next_item(psvdata, hsvi);
                if (GetWindowStyle(hWnd) & SVS_LOOP && !hsvi) {
                    hsvi = scrollview_get_next_item(psvdata, 0);
                }
            } else {
                hsvi = scrollview_get_prev_item(psvdata, hsvi);
                if (GetWindowStyle(hWnd) & SVS_LOOP && !hsvi) {
                    hsvi = scrollview_get_prev_item(psvdata, 0);
                }
            }
            //hsvi = (wParam == SCANCODE_CURSORBLOCKDOWN || wParam == SCANCODE_HOME || wParam == SCANCODE_SPV_DOWN) ? 
        ...
        if (hsvi) {
            if (hsvi != curHilighted) {
                ...
                UpdateWindow(hWnd, FALSE);//force repaint to defence the residual frames
...
BOOL RegisterScrollViewControl (void)
{
    ...
    WndClass.spClassName = SPV_CTRL_SCROLLVIEW;
