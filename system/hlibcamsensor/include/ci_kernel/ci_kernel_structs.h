...
typedef struct KRN_CI_DRIVER
{
    ...
    /**
     * @brief If non 0 will be used to enforce that all tiled
     * buffers have this stride.
     *
     * If computations for tiled buffers needs bigger tiled buffers
     * then configuration will fail.
     *
     * @warning tiling stride has to be a power of 2
     */
    IMG_UINT32 uiTilingStride;
...
typedef struct KRN_CI_PIPELINE
{
     ...
     *
     * @note not updated after registration unless specified
     */
    CI_PIPELINE_CONFIG pipelineConfig;
    /**
     * @brief copy of the user-side pipeline configuration
     *
     * @note not updated after registration
     */
    ...
    /**
     * @brief list of LSH matrix buffers
     * @warning stored using KRN_CI_LSH_MATRIX::KRN_CI_BUFFER::sCell
     */
    sLinkedList_T sList_matrixBuffers;

    /**
     * @brief Matrix currently in use (can be NULL) from sList_matrixBuffers
     *
     * KRN_CI_LSH_MATRIX::sBuffer.used should be increased/decreased only if
     * the capture is started
     */
    struct KRN_CI_LSH_MATRIX *pMatrixUsed;
...
typedef struct KRN_CI_BUFFER
{
    ...
    /**
     * @brief number of shots using this
     *- as output memory should never be >1
     */
    int used;
} KRN_CI_BUFFER;
...
enum eKRN_CI_BUFFER_TYPE {
     ...
     */
    CI_BUFF_ENS_OUT,
    /** @brief LSH matrix input */
    CI_BUFF_LSH_IN,
    /**
     * @brief Load structure - used only when mapping to user-space.
     *
     * Internal type - not importable. Used to contain register values.
     */
    CI_BUFF_LOAD,
};

/**
 * @brief Attach together LSH configuration and buffer
 *
 * uses KRN_CI_BUFFER::sCell to attach to lists
 */
typedef struct KRN_CI_LSH_MATRIX
{
    CI_MODULE_LSH_MAT sConfig;
    /**
     * @brief The size the matrix needs for 1 line in bits
     *
     * Compared with the maximum size of the LSH RAM
     */
    IMG_UINT32 ui32LSHBitSize;
    KRN_CI_BUFFER sBuffer;
} KRN_CI_LSH_MATRIX;
