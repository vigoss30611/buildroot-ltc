...
{
    /**
     * @brief Gains for the white balance (Bayer domain)
     *
    ...
    /**
     * @brief Clipping for the white balance (Bayer domain)
     *
    ...
    /**
     * @brief Mode of operation - HW 2.6 see @ref WBC_MODES
     *
     * @note Load Structure: WBC_THRESHOLD:WBC_MODE
     */
    enum WBC_MODES eRGBThresholdMode;
    /**
     * @brief Threshold clipping value (RGB domain) - HW 2.6
     *
     * @note Load Structure: WBC_THRESHOLD:WBC_THRES_[0..2]
     */
    IMG_UINT16 aRGBThreshold[WBC_CHANNEL_NO];
    /**
     * @brief Gain applied before threshold is used (RGB domain) - HW 2.6
     *
     * @note Load Structure: WBC_GAIN:WBC_GAIN_[0..2]
     */
    IMG_UINT8 aRGBGain[WBC_CHANNEL_NO];
}CI_MODULE_WBC;
 ...
 *
 * See the @ref CI_MODULE_LSH_MAT structure for matrix configuration
 */
...
/*
 * If modify update CI_LSH_SET_PARAM used to send this through IOCTL
 */
...
/**
 * @brief Colour modification matrix configuration (RGB to YCC, YCC to RGB
 * and Colour Correction)
 */
struct CI_MODULE_PLANECONV
{
    /**
     * @brief To Know which direction is the conversion - set up at creation
     */
...
struct CI_MODULE_SCALER
{
    ...
    /**
     * @brief Scaler outputs 422 - if off outputs 420
     *
     * @note Load structure (enc): ENC_SCAL_V_SETUP:ENC_SCAL_422_NOT_420
     */
...
typedef struct CI_MODULE_FLD
{
    ...
    /**
     * @brief Total number of vertical line (including blanking)
     *
...
typedef struct CI_AWS_CURVE_COEFFS {
    /**
     * @brief Horizontal coefficients per line segment
     *
     * @note Registers: AWS_CURVE_COEFF_{0..4}:AWS_CURVE_{0..4}_X_COEFF
     */
    IMG_INT16 aCurveCoeffX[AWS_LINE_SEG_N];
    /**
     * @brief Vertical coefficients per line segment
     *
     * @note Registers: AWS_CURVE_COEFF_{0..4}:AWS_CURVE_{0..4}_Y_COEFF
     */
    IMG_INT16 aCurveCoeffY[AWS_LINE_SEG_N];
    /**
     * @note Registers: AWS_CURVE_OFFSET_0/2
     */
    IMG_INT16 aCurveOffset[AWS_LINE_SEG_N];
    /**
     * @note Registers: AWS_CURVE_BOUNDARY_0/2
     */
    IMG_INT16 aCurveBoundary[AWS_LINE_SEG_N];

} CI_AWS_CURVE_COEFFS;

typedef struct CI_MODULE_AWS
{
    /**
     * @brief Enable usage of AWS statistics if HW supports it
     *
     * bit 29 of SAVE_CONFIG_FLAGS
     *
     * @note Linked list: SAVE_CONFIG_FLAGS:AWS_ENABLE
     */
    IMG_BOOL8 bEnable;
    /**
     * @brief Enable debug bitmap (replace a channel by AWS information)
     *
     * @note Load Structure: AWS_DEBUG_BITMAP:AWS_DEBUG_BITMAP_ENABLE
     */
    IMG_BOOL8 bDebugBitmap;

    /**
     * @note Load Structure: AWS_LOG2_QEFF:AWS_LOG2_R_QEFF
     */
    IMG_UINT16 ui16Log2_R_Qeff;
    /**
     * @note Load Structure: AWS_LOG2_QEFF:AWS_LOG2_B_QEFF
     */
    IMG_UINT16 ui16Log2_B_Qeff;

    /**
     * @note Load Structure: AWS_THRESHOLDS_0:AWS_R_DARK_THRESH
     */
    IMG_UINT16 ui16RedDarkThresh;
    /**
     * @note Load Structure: AWS_THRESHOLDS_1:AWS_B_DARK_THRESH
     */
    IMG_UINT16 ui16BlueDarkThresh;
    /**
     * @note Load Structure: AWS_THRESHOLDS_0:AWS_G_DARK_THRESH
     */
    IMG_UINT16 ui16GreenDarkThresh;

    /**
     * @note Load Structure: AWS_THRESHOLDS_1:AWS_R_CLIP_THRESH
     */
    IMG_UINT16 ui16RedClipThresh;
    /**
     * @note Load Structure: AWS_THRESHOLDS_2:AWS_B_CLIP_THRESH
     */
    IMG_UINT16 ui16BlueClipThresh;
    /**
     * @note Load Structure: AWS_THRESHOLDS_2:AWS_G_CLIP_THRESH
     */
    IMG_UINT16 ui16GreenClipThresh;

    /**
     * @note Load Structure: AWS_BB_DIST
     */
    IMG_UINT16 ui16BbDist;

    /**
     * @brief Start row in CFA
     *
     * Should use start row -1 to ensure the start row is included
     *
     * @note Load Structure: AWS_GRID_START_COORDS:AWS_GRID_START_ROW
     */
    IMG_UINT16 ui16GridStartRow;
    /**
    * @brief Start column in CFA
    *
    * @note Load Structure: AWS_GRID_START_COORDS:AWS_GRID_START_COL
    */
    IMG_UINT16 ui16GridStartColumn;

    /**
     * @brief Width-1 in CFA
     *
     * @note Load Structure: AWS_GRID_TILE:AWS_GRID_TILE_CFA_WIDTH_MIN1
     */
    IMG_UINT16 ui16GridTileWidth;
    /**
     * @brief Height-1 in CFA
     *
     * @note Load Structure: AWS_GRID_TILE:AWS_GRID_TILE_CFA_HEIGHT_MIN1
     */
    IMG_UINT16 ui16GridTileHeight;

    CI_AWS_CURVE_COEFFS sCurveCoeffs;
} CI_MODULE_AWS;
