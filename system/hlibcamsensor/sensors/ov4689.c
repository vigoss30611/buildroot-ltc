/**
 ******************************************************************************
@file ov4689.c

@brief OV4689 camera implementation

To add a new mode declare a new array containing 8b values:
- 2 first values are addresses
- last 8b is the value

The array MUST finish with STOP_REG, STOP_REG, STOP_REG so that the size can
be computed by the OV4689_GetRegisters()

The array can have delay of X ms using DELAY_REG, X, DELAY_REG

Add the array to ov4689_modes[] with the relevant flipping and a size of 0

@copyright Imagination Technologies Ltd. All Rights Reserved.

@license Strictly Confidential.
No part of this software, either material or conceptual may be copied or
distributed, transmitted, transcribed, stored in a retrieval system or
translated into any human or computer language in any form by any means,
electronic, mechanical, manual or other-wise, or disclosed to third
parties without the express written permission of
Imagination Technologies Limited,
Unit 8, HomePark Industrial Estate,
King's Langley, Hertfordshire,
WD4 8LZ, U.K.

 *****************************************************************************/

#include "sensors/ov4689.h"
#include "sensors/ddk_sensor_driver.h"
#include <img_types.h>
#include <img_errors.h>

#include <ci/ci_api_structs.h> // access to CI_GASKET
#include "sensorapi/sensorapi.h"
#include "sensorapi/sensor_phy.h"

#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <sys/ioctl.h>

#include <pthread.h>
#include <unistd.h>
#if !file_i2c
#include <fcntl.h>
#include <linux/i2c-dev.h>
#include <sys/param.h>
#endif

#define SUPPORT_4_LANE

#define OV4689_FLIP_MIRROR_FRIST

#ifdef INFOTM_ISP
#define DEV_PATH    ("/dev/ddk_sensor")
#define EXTRA_CFG_PATH  ("/root/.ispddk/")
static IMG_UINT32 imgr = 0;
static char extra_cfg[64];
static IMG_RESULT OV4689_SetFlipMirror(SENSOR_HANDLE hHandle, IMG_UINT8 flag);
static IMG_RESULT OV4689_SetGainAndExposure(SENSOR_HANDLE hHandle, double flGain, IMG_UINT32 ui32Exposure, IMG_UINT8 ui8Context);
static IMG_RESULT OV4689_GetFixedFPS(SENSOR_HANDLE hHandle, int *pfixed);
static IMG_RESULT OV4689_SetFPS(SENSOR_HANDLE hHandle, double fps);

#endif //INFOTM_ISP

/* disabled as i2c drivers locks up if device is not present */
#define NO_DEV_CHECK

#define LOG_TAG "OV4689_SENSOR"
#include <felixcommon/userlog.h>

/** @ the setup for mode does not work if enabled is not call just after
 * - temporarily do the whole of the setup in in enable function
 * - can remove that define and its checking once fixed
 */
// if defined writes all registers at enable time instead of configure
#define DO_SETUP_IN_ENABLE

#define OV4689_WRITE_ADDR (0x6C >> 1)
#define OV4689_READ_ADDR (0x6D >> 1)
#define AUTOFOCUS_WRITE_ADDR (0x18 >> 1)
#define AUTOFOCUS_READ_ADDR (0x19 >> 1)

#define OV4689_SENSOR_VERSION "not-verified"

#define OV4689_CHIP_VERSION 0x4688

// fake register value to interpret next value as delay in ms
#define DELAY_REG 0xff
// not a real register - marks the end of register array
#define STOP_REG 0xfe

/** @brief sensor number of IMG PHY */
#define OV4689_PHY_NUM 1

/*
 * Choice:
 * copy of the registers to apply because we want to apply them all at
 * once because some registers need 2 writes while others don't (may have
 * an effect on stability but was not showing in testing)
 * 
 * If not defined will apply values register by register and not need a copy
 * of the registers for the active mode.
 */
//#define OV4689_COPY_REGS

// uncomment to write i2c commands to file
//#define ov4689_debug "ov4689_write.txt"

#ifdef WIN32 // on windows we do not need to sleep to wait for the bus
static void usleep(int t)
{
    (void)t;
}
#endif

typedef struct _OV4689cam_struct
{
    SENSOR_FUNCS sFuncs;

    // in MHz
    double refClock;

    // local stuff goes here.
    IMG_UINT16 ui16CurrentMode;
    IMG_UINT8 ui8CurrentFlipping;
    IMG_BOOL8 bEnabled;

    SENSOR_MODE currMode;
    /*
     * if OV4689_COPY_REGS is defined:
     * copy of the registers to apply because we want to apply them all at
     * once because some registers need 2 writes while others don't
     * 
     * otherwise just a pointer to the registers
     */
    IMG_UINT8 *currModeReg;
    /* number of registers in the mode
     * (each registers uses 3 bytes: first 2 are address, last one is value)
     */
    IMG_UINT32 nRegisters;
    IMG_UINT32 ui32ExposureMax;
    IMG_UINT32 ui32Exposure;
    IMG_UINT32 ui32ExposureMin;
    IMG_DOUBLE dbExposureMin;
    double flGain;
    IMG_UINT16 ui16CurrentFocus;
    IMG_UINT8 imager;
    IMG_UINT16 ui16VerticalTotal;
#ifdef INFOTM_ISP
    double fCurrentFps;
    IMG_UINT32 fixedFps;
    IMG_UINT32 initClk;
#endif
    int i2c;
    SENSOR_PHY *psSensorPhy;
}OV4689CAM_STRUCT;

static void OV4689_ApplyMode(OV4689CAM_STRUCT *psCam);
static void OV4689_ComputeGains(double flGain, IMG_UINT8 *gainSequence);
static void OV4689_ComputeExposure(IMG_UINT32 ui32Exposure,
    IMG_UINT32 ui32ExposureMin, IMG_UINT8 *exposureSequence);
static IMG_RESULT OV4689_GetModeInfo(OV4689CAM_STRUCT *psCam,
    IMG_UINT16 ui16Mode, SENSOR_MODE *info);
static IMG_UINT16 OV4689_CalculateFocus(const IMG_UINT16 *pui16DACDist,
        const IMG_UINT16 *pui16DACValues, IMG_UINT8 ui8Entries,
        IMG_UINT16 ui16Requested);
// used before implementation therefore declared here
static IMG_RESULT OV4689_SetFocus(SENSOR_HANDLE hHandle, IMG_UINT16 ui16Focus);

// from omnivision data sheet
#define SPLIT0(R) ((R>>8)&0xFF)
#define SPLIT1(R) (R&0xFF)
#define IS_REG(arr, off, name) \
    ((SPLIT0(name) == arr[off+0]) && (SPLIT1(name) == arr[off+1]))

#define REG_FORMAT1 0x3820 // used for flipping
#define REG_FORMAT2 0x3821 // used for flipping

#define REG_H_CROP_START_0 0x3800 // bits [4;0] address [12;8]
#define REG_H_CROP_START_1 0x3801 // bits [4;0] address [7;0]
#define REG_V_CROP_START_0 0x3802 // bits [3;0] address [11;8]
#define REG_V_CROP_START_1 0x3803 // bits [7;0] address [7;0]
#define REG_H_CROP_END_0 0x3804 // bits [4;0] address [12;8]
#define REG_H_CROP_END_1 0x3805 // bits [7;0] address [7;0]
#define REG_V_CROP_END_0 0x3806 // bits [3;0] address [11;8]
#define REG_V_CROP_END_1 0x3807 // bits [7;0] address [7;0]

#define REG_H_OUTPUT_SIZE_0 0x3808 // bits [4;0] size [12;8]
#define REG_H_OUTPUT_SIZE_1 0x3809 // bits [7;0] size [7;0]
#define REG_V_OUTPUT_SIZE_0 0x380A // bits [4;0] size [12;8]
#define REG_V_OUTPUT_SIZE_1 0x380B // bits [7;0] size [7;0]

#define REG_TIMING_HTS_0 0x380C // bits [6;0] size [14;8]
#define REG_TIMING_HTS_1 0x380D // bits [7;0] size [7;0]
#define REG_TIMING_VTS_0 0x380E // bits [6;0] size [14;8]
#define REG_TIMING_VTS_1 0x380F // bits [7;0] size [7;0]

#define REG_SC_CMMN_BIT_SEL 0x3031 // bits [4;0] for bitdepth
#define REG_SC_CMMN_MIPI_SC_CTRL 0x3018 // bits [7;5] for mipi lane mode

#define REG_PLL_CTRL_B 0x030B // PLL2_prediv (values see pll_ctrl_b_val)
#define REG_PLL_CTRL_C 0x030C // PLL2_mult [1;0] multiplier [9;8]
#define REG_PLL_CTRL_D 0x030D // PLL2_mult [7;0] multiplier [7;0]
#define REG_PLL_CTRL_E 0x030E // PLL2_divs (values see pll_ctrl_c_val)
#define REG_PLL_CTRL_F 0x030F // PLL2_divsp (values is 1/(r+1))
#define REG_PLL_CTRL_11 0x0311 // PLL2_predivp (values is 1/(r+1)

#define REG_SC_CMMN_CHIP_ID_0 0x300A // high bits for chip version
#define REG_SC_CMMN_CHIP_ID_1 0x300B // low bits for chip version

#define REG_EXPOSURE 0x3500
#define REG_GAIN 0x3507

// values for PLL2_prediv 1/x, see sensor data-sheet
static const double pll_ctrl_b_val[] = {
    1.0, 1.5, 2.0, 2.5, 3.0, 4.0, 6.0, 8.0
};

// values for PLL2_divs 1/x, see sensor data-sheet
static const double pll_ctrl_c_val[] = {
    1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0, 5.0
};

static const double aec_min_gain = 1.0;
static const double aec_max_gain = 15; //15.9375;
static const double wb_gain_aec_max_gain = 32.0;
static const int aec_max_exporsure = 1000000;
static const int aec_min_exporsure = 10000;
/*
 * from spreadsheet from omnivision (applied to registers 0x3507 0x3508)
 * formula is:
 * if gain < 2.0
 *    reg = gain*128
 * elif gain < 4.0
 *    reg = 256 + (gain*64 -8)
 * elif gain < 8.0
 *    reg = 256*3 + (gain*32 -12)
 * else
 *    reg = 256*7 + (gain*16 -8)
 */
static const IMG_UINT16 aec_long_gain_val[] = {
    // 1.0
    0x0080, 0x0088, 0x0090, 0x0098, 0x00A0, 0x00A8, 0x00B0, 0x00B8,
    // 1.5
    0x00C0, 0x00C8, 0x00D0, 0x00D8, 0x00E0, 0x00E8, 0x00F0, 0x00F8,
    // 2.0
    0x0178, 0x017C, 0x0180, 0x0184, 0x0188, 0x018C, 0x0190, 0x0194,
    // 2.5
    0x0198, 0x019C, 0x01A0, 0x01A4, 0x01A8, 0x01AC, 0x01B0, 0x01B4,
    // 3.0
    0x01B8, 0x01BC, 0x01C0, 0x01C4, 0x01C8, 0x01CC, 0x01D0, 0x01D4,
    // 3.5
    0x01D8, 0x01DC, 0x01E0, 0x01E4, 0x01E8, 0x01EC, 0x01F0, 0x01F4,
    // 4.0
    0x0374, 0x0376, 0x0378, 0x037A, 0x037C, 0x037E, 0x0380, 0x0382,
    // 4.5
    0x0384, 0x0386, 0x0388, 0x038A, 0x038C, 0x038E, 0x0390, 0x0392,
    // 5.0
    0x0394, 0x0396, 0x0398, 0x039A, 0x039C, 0x039E, 0x03A0, 0x03A2,
    // 5.5
    0x03A4, 0x03A6, 0x03A8, 0x03AA, 0x03AC, 0x03AE, 0x03B0, 0x03B2,
    // 6.0
    0x03B4, 0x03B6, 0x03B8, 0x03BA, 0x03BC, 0x03BE, 0x03C0, 0x03C2,
    // 6.5
    0x03C4, 0x03C6, 0x03C8, 0x03CA, 0x03CC, 0x03CE, 0x03D0, 0x03D2,
    // 7.0
    0x03D4, 0x03D6, 0x03D8, 0x03DA, 0x03DC, 0x03DE, 0x03E0, 0x03E2,
    // 7.5
    0x03E4, 0x03E6, 0x03E8, 0x03EA, 0x03EC, 0x03EE, 0x03F0, 0x03F2,
    // 8.0
    0x0778, 0x0779, 0x077A, 0x077B, 0x077C, 0x077D, 0x077E, 0x077F,
    // 8.5
    0x0780, 0x0781, 0x0782, 0x0783, 0x0784, 0x0785, 0x0786, 0x0787,
    // 9.0
    0x0788, 0x0789, 0x078A, 0x078B, 0x078C, 0x078D, 0x078E, 0x078F,
    // 9.5
    0x0790, 0x0791, 0x0792, 0x0793, 0x0794, 0x0795, 0x0796, 0x0797,
    // 10.0
    0x0798, 0x0799, 0x079A, 0x079B, 0x079C, 0x079D, 0x079E, 0x079F,
    // 10.5
    0x07A0, 0x07A1, 0x07A2, 0x07A3, 0x07A4, 0x07A5, 0x07A6, 0x07A7,
    // 11.0
    0x07A8, 0x07A9, 0x07AA, 0x07AB, 0x07AC, 0x07AD, 0x07AE, 0x07AF,
    // 11.5
    0x07B0, 0x07B1, 0x07B2, 0x07B3, 0x07B4, 0x07B5, 0x07B6, 0x07B7,
    // 12.0
    0x07B8, 0x07B9, 0x07BA, 0x07BB, 0x07BC, 0x07BD, 0x07BE, 0x07BF,
    // 12.5
    0x07C0, 0x07C1, 0x07C2, 0x07C3, 0x07C4, 0x07C5, 0x07C6, 0x07C7,
    // 13.0
    0x07C8, 0x07C9, 0x07CA, 0x07CB, 0x07CC, 0x07CD, 0x07CE, 0x07CF,
    // 13.5
    0x07D0, 0x07D1, 0x07D2, 0x07D3, 0x07D4, 0x07D5, 0x07D6, 0x07D7,
    // 14.0
    0x07D8, 0x07D9, 0x07DA, 0x07DB, 0x07DC, 0x07DD, 0x07DE, 0x07DF,
    // 14.5
    0x07E0, 0x07E1, 0x07E2, 0x07E3, 0x07E4, 0x07E5, 0x07E6, 0x07E7,
    // 15.0
    0x07E8, 0x07E9, 0x07EA, 0x07EB, 0x07EC, 0x07ED, 0x07EE, 0x07EF,
    // 15.5
    0x07F0, 0x07F1, 0x07F2, 0x07F3, 0x07F4, 0x07F5, 0x07F6, 0x07F7,
};

// minimum focus in millimetres
static const unsigned int ov4689_focus_dac_min = 50;

// maximum focus in millimetres, if >= then focus is infinity
static const unsigned int ov4689_focus_dac_max = 600;

// focus values for the ov4689_focus_dac_dist
static const IMG_UINT16 ov4689_focus_dac_val[] = {
    0x3ff, 0x180, 0x120, 0x100, 0x000
};

// distances in millimetres for the ov4689_focus_dac_val
static const IMG_UINT16 ov4689_focus_dac_dist[] = {
    50, 150, 300, 500, 600
};

//
#ifdef INFOTM_ISP
static IMG_UINT32 i2c_addr = 0;
#endif

#define ISP_VISION_LIVE
#ifdef ISP_VISION_LIVE
static double s_flGain = 1.0;
static IMG_UINT32 s_ui32Exposure = 3000;
static IMG_UINT8 s_ui8CurrentFlipping = 0;
#endif

static IMG_UINT32 s_ui16VerticalTotal = 2000;
// modes declaration
//

// 4lane 60fps
static IMG_UINT8 ui8ModeRegs_1920_1088_60_4lane[]=
{
#if 0
	0x01, 0x03, 0x01,
	0x36, 0x38, 0x00,
	0x03, 0x00, 0x04,//0x02,
	0x03, 0x02, 0x40,//0x32,
	0x03, 0x03, 0x00,
	0x03, 0x04, 0x03,

	0x03, 0x05, 0x02,
	0x03, 0x06, 0x02,

	0x03, 0x0a, 0x00,  // pll1 prediv
	0x03, 0x0b, 0x00,
	0x03, 0x0c, 0x00,
	0x03, 0x0d, 0x1e,
	0x03, 0x0e, 0x04,
	0x03, 0x0f, 0x01,
	0x03, 0x11, 0x00,
	0x03, 0x12, 0x01,
	0x03, 0x1e, 0x00,
	0x30, 0x00, 0x20,
	0x30, 0x02, 0x00,
	0x30, 0x18, 0x72, //0x72,
	,//e0
	0x38, 0x04, 0x09,
	0x38, 0x05, 0x17,
	0x38, 0x06, 0x05,
	0x38, 0x07, 0x23,//1f
	0x38, 0x08, 0x07,
	0x38, 0x09, 0x80,
	0x38, 0x0a, 0x04,
	0x38, 0x0b, 0x40,//38
	0x38, 0x0c, 0x06,//03
	0x38, 0x0d, 0xf3,//5c
	,
	//0x01, 0x00, 0x01,
	STOP_REG, STOP_REG, STOP_REG,
#else //for mode5 use mode0
	0x01, 0x03, 0x01,
	    0x36, 0x38, 0x00,
	    0x03, 0x00, 0x00,
	    0x03, 0x02, 0x2a,  // 1_LANE
	    0x03, 0x03, 0x00,  // ??? use 0x01 for test pll1_divdm 1/2
	    0x03, 0x04, 0x03,
	    0x03, 0x0b, 0x00,
	    0x03, 0x0d, 0x1e,
	    0x03, 0x0e, 0x04,
	    0x03, 0x0f, 0x01,
	    0x03, 0x11, 0x00,  // pll2 prediv
	    0x03, 0x12, 0x01,
	    0x03, 0x1e, 0x00,
	    0x30, 0x00, 0x20,
	    0x30, 0x02, 0x00,
	    0x30, 0x18, 0x12,  // 1_LANE
	    , //0xe0,
	    0x38, 0x04, 0x09,
	    0x38, 0x05, 0x17,
	    0x38, 0x06, 0x05,
	    0x38, 0x07, 0x23, //0x1f,
	    0x38, 0x08, 0x07,
	    0x38, 0x09, 0x80,
	    0x38, 0x0a, 0x04,
	    0x38, 0x0b, 0x40, //0x38,
	    ///////////////
	    0x38, 0x0c, 0x0c,  // 30fps
	    0x38, 0x0d, 0xb4,
	    ///////////////
	    0x38, 0x0e, 0x04,
	    0x38, 0x0f, 0x64, //0x8A,
	    0x38, 0x10, 0x00,
	    0x38, 0x11, 0x08,
	    0x38, 0x12, 0x00,
	    0x38, 0x13, 0x04,
	    0x38, 0x14, 0x01,
	    0x38, 0x15, 0x01,
	    0x38, 0x19, 0x01,
	#ifndef OV4689_FLIP_MIRROR_FRIST
	    0x38,0x20,0x00,
	    0x38,0x21,0x06,
	#else
	    0x38,0x20,0x06,
	    0x38,0x21,0x00,
	#endif
	    ,  // timing
	    0x48, 0x13, 0x08,
	    0x48, 0x1f, 0x40,
	    // HS-prepare//
	    //0x48, 0x26, 0x28, // - 0x10,  //HS_PREPARE_MIN
	    //0x48, 0x27, 0x55, // + 0x10,      //HS_PREPARE_MAX
	    //0x48, 0x31, 0x6C - 0x35,  //UI_HS_PREPARE_MIN
	    ///////////////
	    0x48, 0x29, 0x78,
	    0x48, 0x37, 0x10,
	    0x4b, 0x00, 0x2a,
	    0x4b, 0x0d, 0x00,
	    0x4d, 0x00, 0x04,
	    0x4d, 0x01, 0x42,
	    0x4d, 0x02, 0xd1,
	    0x4d, 0x03, 0x93,
	    0x4d, 0x04, 0xf5,
	    0x4d, 0x05, 0xc1,
	    0x50, 0x00, 0xf3,
	    0x50, 0x01, 0x11,
	    0x50, 0x04, 0x00,
	    0x50, 0x0a, 0x00,
	    0x50, 0x0b, 0x00,
	    0x50, 0x32, 0x00,
	    0x50, 0x40, 0x00,
	    0x50, 0x50, 0x0c,
	    0x55, 0x00, 0x00,
	    0x55, 0x01, 0x10,
	    0x55, 0x02, 0x01,
	    0x55, 0x03, 0x0f,
	    0x80, 0x00, 0x00,
	    0x80, 0x01, 0x00,
	    0x80, 0x02, 0x00,
	    0x80, 0x03, 0x00,
	    0x80, 0x04, 0x00,
	    0x80, 0x05, 0x00,
	    0x80, 0x06, 0x00,
	    0x80, 0x07, 0x00,
	    0x80, 0x08, 0x00,
	    0x36, 0x38, 0x00,
	    //0x01, 0x00, 0x01,
	    //0x03, 0x00, 0x60,
	    STOP_REG, STOP_REG, STOP_REG,
#endif

};

// 4lanes 30fps
static IMG_UINT8 ui8ModeRegs_1920_1088_30_4lane[]=
{
	0x01,0x03,0x01,
	0x36,0x38,0x00,
	0x03,0x00,0x00,
	0x03,0x02,0x2a,
	0x03,0x03,/*0x01*/0x01, // ??? use 0x01 for test pll1_divdm 1/2 
	0x03,0x04,0x03,
	0x03,0x0a,0x00, // pll1 prediv
	0x03,0x0b,0x00,
	0x03,0x0d,0x1e,
	0x03,0x0e,0x04,
	0x03,0x0f,0x01,
	0x03,0x11,0x00, // pll2 prediv
	0x03,0x12,0x01,
	0x03,0x1e,0x00,
	0x30,0x00,0x20,
	0x30,0x02,0x00,
	0x30,0x18,0x72,
	0x30,0x20,0x93,
	0x30,0x21,0x03,
	0x30,0x22,0x01,
	0x30,0x31,0x0a,
	0x30,0x3f,0x0c,
	0x33,0x05,0xf1,
	0x33,0x07,0x04,
	0x33,0x09,0x29,
	0x35,0x00,0x00,
	0x35,0x01,0x4c,
	0x35,0x02,0x00,
	0x35,0x03,0x04,
	0x35,0x04,0x00,
	0x35,0x05,0x00,
	0x35,0x06,0x00,
	0x35,0x07,0x00,
	0x35,0x08,0x00/*0x01*/,
	0x35,0x09,0x80/*0xb8*/,
	,//0xe0,
	0x38,0x04,0x09,
	0x38,0x05,0x17,
	0x38,0x06,0x05,
	0x38,0x07,0x23,//0x1f,
	0x38,0x08,0x07,
	0x38,0x09,0x80,
	0x38,0x0a,0x04,
	0x38,0x0b,0x40,//0x38,

	0x38,0x0c,0x0d, // 30fps
	0x38,0x0d,0xf0,

	0x38,0x0e,0x04,
	0x38,0x0f,0x64,//0x8A,
	0x38,0x10,0x00,
	0x38,0x11,0x08,
	0x38,0x12,0x00,
	0x38,0x13,0x04,
	0x38,0x14,0x01,
	0x38,0x15,0x01,
	0x38,0x19,0x01,
#ifndef OV4689_FLIP_MIRROR_FRIST
	0x38,0x20,0x00,
	0x38,0x21,0x06,
#else
	0x38,0x20,0x06,
	0x38,0x21,0x00,
#endif
	, // timing
	0x48,0x13,0x08,
	0x48,0x1f,0x40,
	// HS-prepare//
	//0x48,0x26,0x28,// - 0x10, //HS_PREPARE_MIN
	//0x48,0x27,0x55,// + 0x10,     //HS_PREPARE_MAX
	//0x48,0x31,0x6C - 0x35, //UI_HS_PREPARE_MIN
	///////////////
	0x48,0x29,0x78,
	0x48,0x37,0x10,
	0x4b,0x00,0x2a,
	0x4b,0x0d,0x00,
	0x4d,0x00,0x04,
	0x4d,0x01,0x42,
	0x4d,0x02,0xd1,
	0x4d,0x03,0x93,
	0x4d,0x04,0xf5,
	0x4d,0x05,0xc1,
	0x50,0x00,0xf3,
	0x50,0x01,0x11,
	0x50,0x04,0x00,
	0x50,0x0a,0x00,
	0x50,0x0b,0x00,
	0x50,0x32,0x00,
	0x50,0x40,0x00,
	0x50,0x50,0x0c,
	0x55,0x00,0x00,
	0x55,0x01,0x10,
	0x55,0x02,0x01,
	0x55,0x03,0x0f,
	0x80,0x00,0x00,
	0x80,0x01,0x00,
	0x80,0x02,0x00,
	0x80,0x03,0x00,
	0x80,0x04,0x00,
	0x80,0x05,0x00,
	0x80,0x06,0x00,
	0x80,0x07,0x00,
	0x80,0x08,0x00,
	0x36,0x38,0x00,
//	0x01,0x00,0x01,
//	0x03,0x00,0x60,
	STOP_REG, STOP_REG, STOP_REG,
};

// 2lane 60fps
static IMG_UINT8 ui8ModeRegs_1920_1088_60_2lane[]=
{
	0x01, 0x03, 0x01,
	0x36, 0x38, 0x00,
	0x03, 0x00, 0x04,//0x02,
	0x03, 0x02, 0x40,//0x32,
	0x03, 0x03, 0x00,
	0x03, 0x04, 0x03,

	0x03, 0x05, 0x02,
	0x03, 0x06, 0x02,

	0x03, 0x0a, 0x00,  // pll1 prediv
	0x03, 0x0b, 0x00,
	0x03, 0x0c, 0x00,
	0x03, 0x0d, 0x1e,
	0x03, 0x0e, 0x04,
	0x03, 0x0f, 0x01,
	0x03, 0x11, 0x00,
	0x03, 0x12, 0x01,
	0x03, 0x1e, 0x00,
	0x30, 0x00, 0x20,
	0x30, 0x02, 0x00,
	0x30, 0x18, 0x32, //0x72,
	,//e0
	0x38, 0x04, 0x09,
	0x38, 0x05, 0x17,
	0x38, 0x06, 0x05,
	0x38, 0x07, 0x23,//1f
	0x38, 0x08, 0x07,
	0x38, 0x09, 0x80,
	0x38, 0x0a, 0x04,
	0x38, 0x0b, 0x40,//38
	0x38, 0x0c, 0x06,//03
	0x38, 0x0d, 0xf3,//5c
	,
	//0x01, 0x00, 0x01,
	STOP_REG, STOP_REG, STOP_REG,
};

// 2lane 30fps
static IMG_UINT8 ui8ModeRegs_1920_1088_30_2lane[]=
{
	0x01,0x03,0x01,
	0x36,0x38,0x00,
	0x03,0x00,0x00,
	0x03,0x02,0x2a, // 2_LANE
	0x03,0x03,0x00, // ??? use 0x01 for test pll1_divdm 1/2 
	0x03,0x04,0x03,
	0x03,0x0b,0x00,
	0x03,0x0d,0x1e,
	0x03,0x0e,0x04,
	0x03,0x0f,0x01,
	0x03,0x12,0x01,
	0x03,0x1e,0x00,
	0x30,0x00,0x20,
	0x30,0x02,0x00,
	0x30,0x18,0x32, // 2_LANE
	,//0xe0,
	0x38,0x04,0x09,
	0x38,0x05,0x17,
	0x38,0x06,0x05,
	0x38,0x07,0x23,//0x1f,
	0x38,0x08,0x07,
	0x38,0x09,0x80,
	0x38,0x0a,0x04,
	0x38,0x0b,0x40,//0x38,
	///////////////
	0x38,0x0c,0x0c, // 30fps
	0x38,0x0d,0xb4,
	///////////////
	0x38,0x0e,0x04,
	0x38,0x0f,0x64,//0x8A,
	0x38,0x10,0x00,
	0x38,0x11,0x08,
	0x38,0x12,0x00,
	0x38,0x13,0x04,
	0x38,0x14,0x01,
	0x38,0x15,0x01,
	0x38,0x19,0x01,
#ifndef OV4689_FLIP_MIRROR_FRIST
	0x38,0x20,0x00,
	0x38,0x21,0x06,
#else
	0x38,0x20,0x06,
	0x38,0x21,0x00,
#endif

	, // timing
	0x48,0x13,0x08,
	0x48,0x1f,0x40,
	// HS-prepare//
	//0x48,0x26,0x28,// - 0x10, //HS_PREPARE_MIN
	//0x48,0x27,0x55,// + 0x10,     //HS_PREPARE_MAX
	//0x48,0x31,0x6C - 0x35, //UI_HS_PREPARE_MIN
	///////////////
	0x48,0x29,0x78,
	0x48,0x37,0x10,
	0x4b,0x00,0x2a,
	0x4b,0x0d,0x00,
	0x4d,0x00,0x04,
	0x4d,0x01,0x42,
	0x4d,0x02,0xd1,
	0x4d,0x03,0x93,
	0x4d,0x04,0xf5,
	0x4d,0x05,0xc1,
	0x50,0x00,0xf3,
	0x50,0x01,0x11,
	0x50,0x04,0x00,
	0x50,0x0a,0x00,
	0x50,0x0b,0x00,
	0x50,0x32,0x00,
	0x50,0x40,0x00,
	0x50,0x50,0x0c,
	0x55,0x00,0x00,
	0x55,0x01,0x10,
	0x55,0x02,0x01,
	0x55,0x03,0x0f,
	0x80,0x00,0x00,
	0x80,0x01,0x00,
	0x80,0x02,0x00,
	0x80,0x03,0x00,
	0x80,0x04,0x00,
	0x80,0x05,0x00,
	0x80,0x06,0x00,
	0x80,0x07,0x00,
	0x80,0x08,0x00,
	0x36,0x38,0x00,
	//0x01, 0x00, 0x01,
	//0x03, 0x00, 0x60,
	STOP_REG, STOP_REG, STOP_REG,
};

// 2lane 20fps
static IMG_UINT8 ui8ModeRegs_1920_1088_20_2lane[]=
{
	0x01,0x03,0x01,
	0x36,0x38,0x00,
	0x03,0x00,0x00,
	0x03,0x02,0x2a, // 2_LANE
	0x03,0x03,0x00, // ??? use 0x01 for test pll1_divdm 1/2 
	0x03,0x04,0x03,
	0x03,0x0b,0x00,
	0x03,0x0d,0x1e,
	0x03,0x0e,0x04,
	0x03,0x0f,0x01,
	0x03,0x12,0x01,
	0x03,0x1e,0x00,
	0x30,0x00,0x20,
	0x30,0x02,0x00,
	0x30,0x18,0x32, // 2_LANE
	,//0xe0,
	0x38,0x04,0x09,
	0x38,0x05,0x17,
	0x38,0x06,0x05,
	0x38,0x07,0x23,//0x1f,
	0x38,0x08,0x07,
	0x38,0x09,0x80,
	0x38,0x0a,0x04,
	0x38,0x0b,0x40,//0x38,
	///////////////
	0x38,0x0c,0x11, // 20fps
	0x38,0x0d,0x00,
	///////////////
	0x38,0x0e,0x04,
	0x38,0x0f,0x64,//0x8A,
	0x38,0x10,0x00,
	0x38,0x11,0x08,
	0x38,0x12,0x00,
	0x38,0x13,0x04,
	0x38,0x14,0x01,
	0x38,0x15,0x01,
	0x38,0x19,0x01,
#ifndef OV4689_FLIP_MIRROR_FRIST
	0x38,0x20,0x00,
	0x38,0x21,0x06,
#else
	0x38,0x20,0x06,
	0x38,0x21,0x00,
#endif
	, // timing
	0x48,0x13,0x08,
	0x48,0x1f,0x40,
	// HS-prepare//
	//0x48,0x26,0x28,// - 0x10, //HS_PREPARE_MIN
	//0x48,0x27,0x55,// + 0x10,     //HS_PREPARE_MAX
	//0x48,0x31,0x6C - 0x35, //UI_HS_PREPARE_MIN
	///////////////
	0x48,0x29,0x78,
	0x48,0x37,0x10,
	0x4b,0x00,0x2a,
	0x4b,0x0d,0x00,
	0x4d,0x00,0x04,
	0x4d,0x01,0x42,
	0x4d,0x02,0xd1,
	0x4d,0x03,0x93,
	0x4d,0x04,0xf5,
	0x4d,0x05,0xc1,
	0x50,0x00,0xf3,
	0x50,0x01,0x11,
	0x50,0x04,0x00,
	0x50,0x0a,0x00,
	0x50,0x0b,0x00,
	0x50,0x32,0x00,
	0x50,0x40,0x00,
	0x50,0x50,0x0c,
	0x55,0x00,0x00,
	0x55,0x01,0x10,
	0x55,0x02,0x01,
	0x55,0x03,0x0f,
	0x80,0x00,0x00,
	0x80,0x01,0x00,
	0x80,0x02,0x00,
	0x80,0x03,0x00,
	0x80,0x04,0x00,
	0x80,0x05,0x00,
	0x80,0x06,0x00,
	0x80,0x07,0x00,
	0x80,0x08,0x00,
	0x36,0x38,0x00,
	//0x01, 0x00, 0x01,
	//0x03, 0x00, 0x60,
	STOP_REG, STOP_REG, STOP_REG,
};

// 1lane 30fps
static IMG_UINT8 ui8ModeRegs_1920_1088_30_1lane[]=
{
	0x01, 0x03, 0x01,
	0x36, 0x38, 0x00,
	0x03, 0x00, 0x00,
	0x03, 0x02, 0x2a,  // 1_LANE
	0x03, 0x03, 0x00,  // ??? use 0x01 for test pll1_divdm 1/2
	0x03, 0x04, 0x03,
	0x03, 0x0b, 0x00,
	0x03, 0x0d, 0x1e,
	0x03, 0x0e, 0x04,
	0x03, 0x0f, 0x01,
	0x03, 0x11, 0x00,  // pll2 prediv
	0x03, 0x12, 0x01,
	0x03, 0x1e, 0x00,
	0x30, 0x00, 0x20,
	0x30, 0x02, 0x00,
	0x30, 0x18, 0x12,  // 1_LANE
	, //0xe0,
	0x38, 0x04, 0x09,
	0x38, 0x05, 0x17,
	0x38, 0x06, 0x05,
	0x38, 0x07, 0x23, //0x1f,
	0x38, 0x08, 0x07,
	0x38, 0x09, 0x80,
	0x38, 0x0a, 0x04,
	0x38, 0x0b, 0x40, //0x38,
	///////////////
	0x38, 0x0c, 0x0c,  // 30fps
	0x38, 0x0d, 0xb4,
	///////////////
	0x38, 0x0e, 0x04,
	0x38, 0x0f, 0x64, //0x8A,
	0x38, 0x10, 0x00,
	0x38, 0x11, 0x08,
	0x38, 0x12, 0x00,
	0x38, 0x13, 0x04,
	0x38, 0x14, 0x01,
	0x38, 0x15, 0x01,
	0x38, 0x19, 0x01,
#ifndef OV4689_FLIP_MIRROR_FRIST
	0x38,0x20,0x00,
	0x38,0x21,0x06,
#else
	0x38,0x20,0x06,
	0x38,0x21,0x00,
#endif
	,  // timing
	0x48, 0x13, 0x08,
	0x48, 0x1f, 0x40,
	// HS-prepare//
	//0x48, 0x26, 0x28, // - 0x10,  //HS_PREPARE_MIN
	//0x48, 0x27, 0x55, // + 0x10,      //HS_PREPARE_MAX
	//0x48, 0x31, 0x6C - 0x35,  //UI_HS_PREPARE_MIN
	///////////////
	0x48, 0x29, 0x78,
	0x48, 0x37, 0x10,
	0x4b, 0x00, 0x2a,
	0x4b, 0x0d, 0x00,
	0x4d, 0x00, 0x04,
	0x4d, 0x01, 0x42,
	0x4d, 0x02, 0xd1,
	0x4d, 0x03, 0x93,
	0x4d, 0x04, 0xf5,
	0x4d, 0x05, 0xc1,
	0x50, 0x00, 0xf3,
	0x50, 0x01, 0x11,
	0x50, 0x04, 0x00,
	0x50, 0x0a, 0x00,
	0x50, 0x0b, 0x00,
	0x50, 0x32, 0x00,
	0x50, 0x40, 0x00,
	0x50, 0x50, 0x0c,
	0x55, 0x00, 0x00,
	0x55, 0x01, 0x10,
	0x55, 0x02, 0x01,
	0x55, 0x03, 0x0f,
	0x80, 0x00, 0x00,
	0x80, 0x01, 0x00,
	0x80, 0x02, 0x00,
	0x80, 0x03, 0x00,
	0x80, 0x04, 0x00,
	0x80, 0x05, 0x00,
	0x80, 0x06, 0x00,
	0x80, 0x07, 0x00,
	0x80, 0x08, 0x00,
	0x36, 0x38, 0x00,
	//0x01, 0x00, 0x01,
	//0x03, 0x00, 0x60,
	STOP_REG, STOP_REG, STOP_REG,
};

static IMG_UINT8 ui8ModeRegs_2624_1520_30_2lane[]=
{
    0x01, 0x03, 0x01,
    0x36, 0x38, 0x00,
    0x03, 0x00, 0x00,   //0x01 eric
    0x03, 0x01, 0x00,
    0x03, 0x02, 0x23,  //0x28 eric
    ,  //0x40  eric
    0x38, 0x0A, 0x05,
    0x38, 0x0B, 0xF0,
    0x38, 0x0C, 0x07,  //0x0b eric
    0x38, 0x0D, 0xd0,  //0xb8 eric
    0x38, 0x0E, 0x07,  //0x07 eric
    0x38, 0x0F, 0xd0,  //0xd0  eric
    , //0x1A eric
    0x4b, 0x00, 0x2a,
    0x4b, 0x0d, 0x00,
    0x4d, 0x00, 0x04,
    0x4d, 0x01, 0x42,
    0x4d, 0x02, 0xd1,
    0x4d, 0x03, 0x93,
    0x4d, 0x04, 0xf5,
    0x4d, 0x05, 0xc1,
    0x50, 0x00, 0xf3,
    0x50, 0x01, 0x11,
    0x50, 0x04, 0x00,
    0x50, 0x0a, 0x00,
    0x50, 0x0b, 0x00,
    0x50, 0x32, 0x00,
    0x50, 0x40, 0x00,
    0x50, 0x50, 0x0c,
    0x55, 0x00, 0x00,
    0x55, 0x01, 0x10,
    0x55, 0x02, 0x01,
    0x55, 0x03, 0x0f,
    0x80, 0x00, 0x00,
    0x80, 0x01, 0x00,
    0x80, 0x02, 0x00,
    0x80, 0x03, 0x00,
    0x80, 0x04, 0x00,
    0x80, 0x05, 0x00,
    0x80, 0x06, 0x00,
    0x80, 0x07, 0x00,
    0x80, 0x08, 0x00,
    0x36, 0x38, 0x00,
	STOP_REG, STOP_REG, STOP_REG,
};

struct ov4689_mode {
    IMG_UINT8 ui8Flipping;
    const IMG_UINT8 *modeRegisters;
    IMG_UINT32 nRegisters; // initially 0 then computed the 1st time
};

struct ov4689_mode ov4689_modes[] = 
{
    { SENSOR_FLIP_BOTH, ui8ModeRegs_1920_1088_60_4lane, 0 }, //0
    { SENSOR_FLIP_BOTH, ui8ModeRegs_2624_1520_30_2lane, 0 },
    { SENSOR_FLIP_BOTH, ui8ModeRegs_1920_1088_30_4lane, 0 }, //1
    { SENSOR_FLIP_BOTH, ui8ModeRegs_1920_1088_60_2lane, 0 }, //2
    { SENSOR_FLIP_BOTH, ui8ModeRegs_1920_1088_30_2lane, 0 }, //3
	{ SENSOR_FLIP_BOTH, ui8ModeRegs_1920_1088_20_2lane, 0 }, //4
    { SENSOR_FLIP_BOTH, ui8ModeRegs_1920_1088_30_1lane, 0 }, //5
};
static IMG_UINT8 g_OV4689_ui8LinesValHiBackup = 0x00;
static IMG_UINT8 g_OV4689_ui8LinesValMidBackup = 0x2e;
static IMG_UINT8 g_OV4689_ui8LinesValLoBackup = 0x80;

static IMG_UINT8 g_OV4689_ui8GainValHiBackup = 0x00;
static IMG_UINT8 g_OV4689_ui8GainValMidBackup = 0x00;
static IMG_UINT8 g_OV4689_ui8GainValLoBackup = 0x00;
static const IMG_UINT8* OV4689_GetRegisters(OV4689CAM_STRUCT *psCam,
    unsigned int uiMode, unsigned int *nRegisters)
{
    if (uiMode < sizeof(ov4689_modes) / sizeof(struct ov4689_mode))
    {
        const IMG_UINT8 *registerArray = ov4689_modes[uiMode].modeRegisters;
        if (0 == ov4689_modes[uiMode].nRegisters)
        {
            // should be done only once per mode
            int i = 0;
            while (STOP_REG != registerArray[3 * i])
            {
                i++;
            }
            ov4689_modes[uiMode].nRegisters = i;
        }
        *nRegisters = ov4689_modes[uiMode].nRegisters;
        
        return registerArray;
    }
    // if it is an invalid mode returns NULL

    return NULL;
}

static IMG_RESULT ov4689_i2c_write8(int i2c, const IMG_UINT8 *data,
    unsigned int len)
{
#if !file_i2c
    unsigned i;
#ifdef ov4689_debug
    FILE *f=NULL;
    static unsigned nwrite=0;
#endif //ov4689_debug

    /* Every write sequence needs to have 3 elements:
     * 1) slave address high bits [15:8]
     * 2) slave address low bits [7:0]
     * 3) data
     */
    if (len % 3)
    {
        LOG_ERROR("Wrong len of data array, len = %d", len);
        return IMG_ERROR_INVALID_PARAMETERS;
    }

    /* Set I2C slave address */
	if (ioctl(i2c, /*I2C_SLAVE*/I2C_SLAVE_FORCE, i2c_addr))
    {
        LOG_ERROR("Failed to write I2C slave address!\n");
        return IMG_ERROR_BUSY;
    }
#ifdef ov4689_debug
    f = fopen("/tmp/ov4689_write.txt", "a");
    fprintf(f, "write %d\n", nwrite++);
#endif //ov4689_debug

    for (i = 0; i < len; data += 3, i += 3)
    {
        int write_len = 0;

        if(DELAY_REG == data[0])
        {
            usleep((int)data[1]*1000);
#ifdef ov4689_debug
        fprintf(f, "delay %dms\n", data[1]);
#endif //ov4689_debug
            continue;
        }

        write_len = write(i2c, data, 3);
#ifdef ov4689_debug
        fprintf(f, "0x%02X 0x%02X 0x%02X\n", data[0], data[1], data[2]);
#endif //ov4689_debug

        if (write_len != 3)
        {
            LOG_ERROR("Failed to write I2C data! write_len = %d, index = %d\n",
                    write_len, i);
#ifdef ov4689_debug
            fclose(f);
#endif //ov4689_debug
            return IMG_ERROR_BUSY;
        }
    }
#ifdef ov4689_debug
    fclose(f);
#endif //ov4689_debug
#endif //file_i2c
    return IMG_SUCCESS;
}

static IMG_RESULT ov4689_i2c_read8(int i2c, IMG_UINT16 offset,
    IMG_UINT8 *data)
{
#if !file_i2c
    int ret;
    IMG_UINT8 buff[2];
    
    IMG_ASSERT(data);  // null pointer forbidden
    
    /* Set I2C slave address */
	if (ioctl(i2c, /*I2C_SLAVE*/I2C_SLAVE_FORCE, i2c_addr))
    {
        LOG_ERROR("Failed to write I2C read address!\n");
        return IMG_ERROR_BUSY;
    }
    
    buff[0] = (offset >> 8) & 0xFF;
    buff[1] = offset & 0xFF;
    
    ret = write(i2c, buff, sizeof(buff));
    if (ret != sizeof(buff))
    {
        LOG_WARNING("Wrote %dB instead of %luB before reading\n",
            ret, sizeof(buff));
    }
    
    ret = read(i2c, data, 1);
    if (1 != ret)
    {
        LOG_ERROR("Failed to read I2C at 0x%x\n", offset);
        return IMG_ERROR_FATAL;
    }

    return IMG_SUCCESS;
#else
    return IMG_ERROR_NOT_SUPPORTED;
#endif //file_i2c
}


static IMG_RESULT ov4689_i2c_read16(int i2c, IMG_UINT16 offset,
    IMG_UINT16 *data)
{
#if !file_i2c
    int ret;
    IMG_UINT8 buff[2];
    
    IMG_ASSERT(data);  // null pointer forbidden
    
    /* Set I2C slave address */
	if (ioctl(i2c, /*I2C_SLAVE*/I2C_SLAVE_FORCE, i2c_addr))
    {
        LOG_ERROR("Failed to write I2C read address!\n");
        return IMG_ERROR_BUSY;
    }
    
    buff[0] = (offset >> 8) & 0xFF;
    buff[1] = offset & 0xFF;
    
    ret = write(i2c, buff, sizeof(buff));
    if (ret != sizeof(buff))
    {
        LOG_WARNING("Wrote %dB instead of %luB before reading\n",
            ret, sizeof(buff));
    }
    
    ret = read(i2c, buff, sizeof(buff));
    if (sizeof(buff) != ret)
    {
        LOG_ERROR("Failed to read I2C at 0x%x\n", offset);
        return IMG_ERROR_FATAL;
    }

    *data = (buff[0] << 8) | buff[1];

    return IMG_SUCCESS;
#else
    return IMG_ERROR_NOT_SUPPORTED;
#endif //file_i2c
}

static IMG_RESULT OV4689_GetInfo(SENSOR_HANDLE hHandle, SENSOR_INFO *psInfo)
{
    IMG_RESULT ret;
    IMG_UINT16 v = 0;
    OV4689CAM_STRUCT *psCam = container_of(hHandle, OV4689CAM_STRUCT, sFuncs);

    if (!psCam->psSensorPhy)
    {
        LOG_ERROR("sensor not initialised\n");
        return IMG_ERROR_NOT_INITIALISED;
    }

    IMG_ASSERT(strlen(OV4689_SENSOR_INFO_NAME) < SENSOR_INFO_NAME_MAX);
    IMG_ASSERT(strlen(OV4689_SENSOR_VERSION) < SENSOR_INFO_VERSION_MAX);

    psInfo->eBayerOriginal = MOSAIC_BGGR;
    psInfo->eBayerEnabled = psInfo->eBayerOriginal;
    // assumes that when flipping changes the bayer pattern
    /*if (SENSOR_FLIP_NONE != psInfo->sStatus.ui8Flipping)
    {
        psInfo->eBayerEnabled = MosaicFlip(psInfo->eBayerOriginal,
            0, //psInfo->sStatus.ui8Flipping&SENSOR_FLIP_HORIZONTAL?1:0,
            psInfo->sStatus.ui8Flipping&SENSOR_FLIP_VERTICAL?1:0);
        // bayer format is not affected by flipping since we change
        // registers to 6 instead of 2
    }*/
    sprintf(psInfo->pszSensorName, OV4689_SENSOR_INFO_NAME);
    
#ifndef NO_DEV_CHECK
    ret = ov4689_i2c_read16(psCam->i2c, REG_SC_CMMN_CHIP_ID_0, &v);
#else
    ret = 1;
#endif //NO_DEV_CHECK
    if (!ret)
    {
        sprintf(psInfo->pszSensorVersion, "0x%x", v);
    }
    else
    {
        sprintf(psInfo->pszSensorVersion, OV4689_SENSOR_VERSION);
    }

    psInfo->fNumber = 1.2;
    psInfo->ui16FocalLength = 30;
    psInfo->ui32WellDepth = 6040;
    // bitdepth is a mode information
    psInfo->flReadNoise = 5.0;
    psInfo->ui8Imager = imgr;
    psInfo->bBackFacing = IMG_TRUE;
    // other information should be filled by Sensor_GetInfo()
#ifdef INFOTM_ISP
    psInfo->ui32ModeCount = sizeof(ov4689_modes) / sizeof(struct ov4689_mode);
#endif //INFOTM_ISP

    LOG_DEBUG("provided BayerOriginal=%d\n", psInfo->eBayerOriginal);
    return IMG_SUCCESS;
}

static IMG_RESULT OV4689_GetMode(SENSOR_HANDLE hHandle, IMG_UINT16 nIndex,
        SENSOR_MODE *psModes)
{
    OV4689CAM_STRUCT *psCam = container_of(hHandle, OV4689CAM_STRUCT, sFuncs);
    IMG_RESULT ret;

    /*if (!psCam->psSensorPhy)
    {
        LOG_ERROR("sensor not initialised\n");
        return IMG_ERROR_NOT_INITIALISED;
    }*/

    ret = OV4689_GetModeInfo(psCam, nIndex, psModes);
    return ret;
}

static IMG_RESULT OV4689_GetState(SENSOR_HANDLE hHandle,
        SENSOR_STATUS *psStatus)
{
    OV4689CAM_STRUCT *psCam = container_of(hHandle, OV4689CAM_STRUCT, sFuncs);

    if (!psCam->psSensorPhy)
    {
        LOG_WARNING("sensor not initialised\n");
        psStatus->eState = SENSOR_STATE_UNINITIALISED;
        psStatus->ui16CurrentMode = 0;
    }
    else
    {
        psStatus->eState= (psCam->bEnabled?
            SENSOR_STATE_RUNNING:SENSOR_STATE_IDLE);
        psStatus->ui16CurrentMode=psCam->ui16CurrentMode;
        psStatus->ui8Flipping=psCam->ui8CurrentFlipping;
#ifdef INFOTM_ISP
        psStatus->fCurrentFps = psCam->fCurrentFps;
#endif

    }
    return IMG_SUCCESS;
}

static IMG_RESULT OV4689_SetMode(SENSOR_HANDLE hHandle, IMG_UINT16 nMode,
          IMG_UINT8 ui8Flipping)
{
    OV4689CAM_STRUCT *psCam = container_of(hHandle, OV4689CAM_STRUCT, sFuncs);
    unsigned int nRegisters = 0;
    const IMG_UINT8 *modeReg = NULL;
    IMG_RESULT ret;

    if (!psCam->psSensorPhy)
    {
        LOG_ERROR("sensor not initialised\n");
        return IMG_ERROR_NOT_INITIALISED;
    }

    modeReg = OV4689_GetRegisters(psCam, nMode, &nRegisters);

    if (modeReg)
    {
        if((ui8Flipping&(ov4689_modes[nMode].ui8Flipping))!=ui8Flipping)
        {
            LOG_ERROR("sensor mode does not support selected flipping "\
                "0x%x (supports 0x%x)\n",
                ui8Flipping, ov4689_modes[nMode].ui8Flipping);
            return IMG_ERROR_NOT_SUPPORTED;
        }

#if defined(OV4689_COPY_REGS)
        if (psCam->currModeReg)
        {
            IMG_FREE(psCam->currModeReg);
        }
#endif //OV4689_COPY_REGS
        psCam->nRegisters = nRegisters;
        
#if defined(OV4689_COPY_REGS)
        /* we want to apply all the registers at once - need to copy them */
        psCam->currModeReg = (IMG_UINT8*)IMG_MALLOC(psCam->nRegisters * 3);
        IMG_MEMCPY(psCam->currModeReg, modeReg, psCam->nRegisters * 3);
#else
        /* no need to copy the register as we will apply them one by one */
        psCam->currModeReg = (IMG_UINT8*)modeReg;
#endif //OV4689_COPY_REGS

        ret = OV4689_GetModeInfo(psCam, nMode, &(psCam->currMode));
        if (ret)
        {
            LOG_ERROR("failed to get mode %d initial information!\n", nMode);
            return IMG_ERROR_FATAL;
        }

#ifdef DO_SETUP_IN_ENABLE
        /* because we don't do the actual setup we do not have real
         * information about the sensor - fake it */
        psCam->ui32Exposure = psCam->currMode.ui32ExposureMin * 302;

        psCam->ui32Exposure = IMG_MIN_INT(psCam->currMode.ui32ExposureMax, 
            IMG_MAX_INT(psCam->ui32Exposure, psCam->currMode.ui32ExposureMin));
#else
        OV4689_ApplyMode(psCam);
#endif //DO_SETUP_IN_ENABLE

        psCam->ui16CurrentFocus = ov4689_focus_dac_min; // minimum focus
        psCam->ui16CurrentMode = nMode;
        psCam->ui8CurrentFlipping = ui8Flipping;
        return IMG_SUCCESS;
    }


    return IMG_ERROR_INVALID_PARAMETERS;
}

static IMG_RESULT OV4689_Enable(SENSOR_HANDLE hHandle)
{
    IMG_UINT8 aui18EnableRegs[]=
    {
#ifndef DO_SETUP_IN_ENABLE
        //set sensor to streaming mode
        0x01, 0x00, 0x01, // SC_CTRL0100 streaming
        0x30, 0x1A, 0xF9, // SC_CMMN_CLKRST0 as recommended by omnivision
        DELAY_REG, 15, DELAY_REG,// 15 ms
        0x30, 0x1A, 0xF1, // SC_CMMN_CLKRST0
        0x48, 0x05, 0x00, // MIPI_CTRL_05 
        0x30, 0x1A, 0xF0, // SC_CMMN_CLKRST0
#endif //DO_SETUP_IN_ENABLE
        //0x32, 0x0b, 0x0, // GRP_SWCTRL select group 0
        //0x32, 0x08, 0xe0 // GROUP_ACCESS quick launch group 0

		0x01, 0x00, 0x01 // streaming enable
    };
    OV4689CAM_STRUCT *psCam = container_of(hHandle, OV4689CAM_STRUCT, sFuncs);

    if (!psCam->psSensorPhy)
    {
        LOG_ERROR("sensor not initialised\n");
        return IMG_ERROR_NOT_INITIALISED;
    }

    LOG_INFO("Enabling OV4689 Camera!\n");
    psCam->bEnabled=IMG_TRUE;

    psCam->psSensorPhy->psGasket->bParallel = IMG_FALSE;
    psCam->psSensorPhy->psGasket->uiGasket = imgr;

	//printf("~~~psCam->currMode.ui8MipiLanes=%d\n", psCam->currMode.ui8MipiLanes);
	
    SensorPhyCtrl(psCam->psSensorPhy, IMG_TRUE, psCam->currMode.ui8MipiLanes,
        OV4689_PHY_NUM);

#ifdef DO_SETUP_IN_ENABLE
    // because we didn't do the actual setup in setMode we have to do it now
    // it may start the sensor so we have to do it after the gasket enable
    /// @ fix conversion from float to uint
    OV4689_ApplyMode(psCam);
#endif //DO_SETUP_IN_ENABLE

    ov4689_i2c_write8(psCam->i2c, aui18EnableRegs, sizeof(aui18EnableRegs));
#if !defined(INFOTM_ISP)
    OV4689_SetFocus(hHandle, psCam->ui16CurrentFocus);
#endif
    LOG_DEBUG("camera enabled\n");
#ifdef ISP_VISION_LIVE
    OV4689_SetFlipMirror(hHandle, s_ui8CurrentFlipping);
#endif
    return IMG_SUCCESS;
}

static IMG_RESULT OV4689_Disable(SENSOR_HANDLE hHandle)
{
    static const IMG_UINT8 disableRegs[]=
	{
	#if 0
        0x01, 0x00, 0x00, // SC_CTRL0100 set to sw standby
        DELAY_REG, 200, DELAY_REG, // 200ms delay
        0x30, 0x1a, 0xf9, // SC_CMMN_CLKRST0 as recommended by omnivision
        0x48, 0x05, 0x03, // MIPI_CTRL_05 retime manu/sel to manual
//        DELAY_REG, 10, DELAY_REG // 10ms delay
	#else
		0x01, 0x00, 0x00
	#endif
    };

    OV4689CAM_STRUCT *psCam = container_of(hHandle, OV4689CAM_STRUCT, sFuncs);

    if (!psCam->psSensorPhy)
    {
        LOG_ERROR("sensor not initialised\n");
        return IMG_ERROR_NOT_INITIALISED;
    }

    if(psCam->bEnabled)
    {
        int delay = 0;
        LOG_INFO("Disabling OV4689 camera\n");
        psCam->bEnabled = IMG_FALSE;

        ov4689_i2c_write8(psCam->i2c, disableRegs, sizeof(disableRegs));

        // delay of a frame period to ensure sensor has stopped
        // flFrameRate in Hz, change to MHz to have micro seconds
        delay = (int)floor((1.0 / psCam->currMode.flFrameRate) * 1000 * 1000);
        LOG_DEBUG("delay of %uus between disabling sensor/phy\n", delay);
        
        usleep(delay);

        SensorPhyCtrl(psCam->psSensorPhy, IMG_FALSE, 0, OV4689_PHY_NUM);
    }

    return IMG_SUCCESS;
}

static IMG_RESULT OV4689_Destroy(SENSOR_HANDLE hHandle)
{
    OV4689CAM_STRUCT *psCam = container_of(hHandle, OV4689CAM_STRUCT, sFuncs);

    if (!psCam->psSensorPhy)
    {
        LOG_ERROR("sensor not initialised\n");
        return IMG_ERROR_NOT_INITIALISED;
    }

    LOG_DEBUG("Destroying OV4689 camera\n");
    /* remember to free anything that might have been allocated dynamically
     * (like extended params...)*/
    if(psCam->bEnabled)
    {
        OV4689_Disable(hHandle);
    }
    SensorPhyDeinit(psCam->psSensorPhy);
    if (psCam->currModeReg)
    {
#if defined(OV4689_COPY_REGS)
        IMG_FREE(psCam->currModeReg);
#endif //OV4689_COPY_REGS
        psCam->currModeReg = NULL;
    }
#if !file_i2c
    close(psCam->i2c);
#endif //file_i2c
    IMG_FREE(psCam);
    return IMG_SUCCESS;
}

static IMG_RESULT OV4689_GetFocusRange(SENSOR_HANDLE hHandle,
        IMG_UINT16 *pui16Min, IMG_UINT16 *pui16Max)
{
    /*OV4689CAM_STRUCT *psCam = container_of(hHandle, OV4689CAM_STRUCT, sFuncs);

    if (!psCam->psSensorPhy)
    {
        LOG_ERROR("sensor not initialised\n");
        return IMG_ERROR_NOT_INITIALISED;
    }*/

    *pui16Min = ov4689_focus_dac_min;
    *pui16Max = ov4689_focus_dac_max;
    return IMG_SUCCESS;
}

static IMG_RESULT OV4689_GetCurrentFocus(SENSOR_HANDLE hHandle,
        IMG_UINT16 *pui16Current)
{
    OV4689CAM_STRUCT *psCam = container_of(hHandle, OV4689CAM_STRUCT, sFuncs);

    if (!psCam->psSensorPhy)
    {
        LOG_ERROR("sensor not initialised\n");
        return IMG_ERROR_NOT_INITIALISED;
    }

    *pui16Current=psCam->ui16CurrentFocus;
    return IMG_SUCCESS;
}

/*
 * calculate focus position from points and return DAC value
 */
IMG_UINT16 OV4689_CalculateFocus(const IMG_UINT16 *pui16DACDist,
    const IMG_UINT16 *pui16DACValues, IMG_UINT8 ui8Entries,
    IMG_UINT16 ui16Requested)
{
    double flMinDistRcp,flMaxDistRcp,flRequestedRcp;
    double M,C;
    int i;
    /* find the entries between which the requested distance fits
     * calculate the equation of the line between those point for the
     * reciprocal of the distance
     * special casing the case where the requested point lies on one of the
     * points of the curve*/
    if (ui16Requested == pui16DACDist[0])
    {
        return pui16DACValues[0];
    }
    for(i=0;i<ui8Entries-1;i++)
    {
        if (ui16Requested == pui16DACDist[i + 1])
        {
            return pui16DACValues[i + 1];
        }
        if (ui16Requested >= pui16DACDist[i]
            && ui16Requested <= pui16DACDist[i + 1])
        {
            break;
        }
    }
    flMinDistRcp = 1.0 / pui16DACDist[i];
    flMaxDistRcp = 1.0 / pui16DACDist[i + 1];
    flRequestedRcp = 1.0 / ui16Requested;

    M = (pui16DACValues[i] - pui16DACValues[i + 1]) /
        (flMinDistRcp - flMaxDistRcp);
    C = -(M * flMinDistRcp - pui16DACValues[i]);
    return (IMG_UINT16)(M * flRequestedRcp + C);
}

// declared beforehand
IMG_RESULT OV4689_SetFocus(SENSOR_HANDLE hHandle, IMG_UINT16 ui16Focus)
{
#if 0
#if !file_i2c
    unsigned i;
#endif //file_i2c
    IMG_UINT8 ui8Regs[4];
    IMG_UINT16 ui16DACVal;

    OV4689CAM_STRUCT *psCam = container_of(hHandle, OV4689CAM_STRUCT, sFuncs);

    if (!psCam->psSensorPhy)
    {
        LOG_ERROR("sensor not initialised\n");
        return IMG_ERROR_NOT_INITIALISED;
    }

    psCam->ui16CurrentFocus = ui16Focus;
    if (psCam->ui16CurrentFocus >= ov4689_focus_dac_max)
    {
        // special case the infinity as it doesn't fit in with the rest
        ui16DACVal = 0;
    }
    else
    {
        ui16DACVal = OV4689_CalculateFocus(ov4689_focus_dac_dist,
            ov4689_focus_dac_val,
            sizeof(ov4689_focus_dac_dist) / sizeof(IMG_UINT16),
            ui16Focus);
    }

    ui8Regs[0] = 4;
    ui8Regs[1] = ui16DACVal >> 8;
    ui8Regs[2] = 5;
    ui8Regs[3] = ui16DACVal & 0xff;

#if !file_i2c
    if (ioctl(psCam->i2c, I2C_SLAVE_FORCE, i2c_addr))
    {
        LOG_ERROR("Failed to write I2C slave address!\n");
        return IMG_ERROR_BUSY;
    }

    for (i = 0; i < sizeof(ui8Regs); i += 2)
    {
        IMG_UINT8 *data = ui8Regs + i;
        if (write(psCam->i2c, data, 2) != 2)
        {
            LOG_ERROR("Failed to write I2C data!\n");
            return IMG_ERROR_BUSY;
        }
    }
#endif //file_i2c
#endif
    return IMG_SUCCESS;
}

static IMG_RESULT OV4689_GetExposureRange(SENSOR_HANDLE hHandle,
        IMG_UINT32 *pui32Min, IMG_UINT32 *pui32Max, IMG_UINT8 *pui8Contexts)
{
    OV4689CAM_STRUCT *psCam = container_of(hHandle, OV4689CAM_STRUCT, sFuncs);

    if (!psCam->psSensorPhy)
    {
        LOG_ERROR("sensor not initialised\n");
        return IMG_ERROR_NOT_INITIALISED;
    }

    *pui32Min=psCam->currMode.ui32ExposureMin;
    *pui32Max=psCam->currMode.ui32ExposureMax;
    *pui8Contexts=1;
    return IMG_SUCCESS;
}

static IMG_RESULT OV4689_GetExposure(SENSOR_HANDLE hHandle,
        IMG_UINT32 *pi32Current, IMG_UINT8 ui8Context)
{
    OV4689CAM_STRUCT *psCam = container_of(hHandle, OV4689CAM_STRUCT, sFuncs);

    if (!psCam->psSensorPhy)
    {
        LOG_ERROR("sensor not initialised\n");
        return IMG_ERROR_NOT_INITIALISED;
    }

    *pi32Current=psCam->ui32Exposure;
    return IMG_SUCCESS;
}

void OV4689_ComputeExposure(IMG_UINT32 ui32Exposure,
    IMG_UINT32 ui32ExposureMin, IMG_UINT8 *exposureRegs)
{
    IMG_UINT32 exposureRatio = (ui32Exposure)/ui32ExposureMin;

    LOG_DEBUG("Exposure Val 0x%x\n", exposureRatio);

    exposureRegs[2]=((exposureRatio)>>12) & 0xf;
    exposureRegs[5]=((exposureRatio)>>4) & 0xff;
    exposureRegs[8]=((exposureRatio)<<4) & 0xff;
}

static IMG_RESULT OV4689_SetExposure(SENSOR_HANDLE hHandle,
        IMG_UINT32 ui32Current, IMG_UINT8 ui8Context)
{
    IMG_UINT16 ui16VerticalTotal;
    IMG_UINT8 ui8fpsRegs[] =
	{
		0x32, 0x08, 0x00, // GROUP_ACCESS start hold of group 0

		0x38, 0x0E, 0x00, // Exposure register
		0x38, 0x0F, 0x00, //

		0x32, 0x08, 0x10, // end hold of group 0 register writes
		0x32, 0x0b, 0x00, // set quick manual mode
		0x32, 0x08, 0xe0, // quick launch group 0
	};
    IMG_UINT8 exposureRegs[] =
    {
        0x32, 0x08, 0x00, // GROUP_ACCESS start hold of group 0
        
        0x35, 0x00, 0x00, // exposure register
        0x35, 0x01, 0x2E, //
        0x35, 0x02, 0x80, //
        
        0x32, 0x08, 0x10, // end hold of group 0 register writes
        0x32, 0x0b, 0x00, // set quick manual mode
        0x32, 0x08, 0xe0, // quick launch group 0
    };

    OV4689CAM_STRUCT *psCam = container_of(hHandle, OV4689CAM_STRUCT, sFuncs);

    if (!psCam->psSensorPhy)
    {
        LOG_ERROR("sensor not initialised\n");
        return IMG_ERROR_NOT_INITIALISED;
    }

    if(ui32Current > aec_max_exporsure)
	{
        ui32Current = aec_max_exporsure;
	}
    psCam->ui32Exposure = ui32Current;

#if 1
	IMG_UINT32 ui32Lines;

	ui32Lines = psCam->ui32Exposure / psCam->currMode.dbExposureMin;

	if (ui32Lines > s_ui16VerticalTotal - 4)
    {
        ui16VerticalTotal = ui32Lines + 4;
    //    ui16VerticalTotal = psCam->currMode.ui16VerticalTotal * psCam->fixedFps / fps;
        ui8fpsRegs[5] = (ui16VerticalTotal >> 8);
        ui8fpsRegs[8] = (ui16VerticalTotal & 0xff);
        ov4689_i2c_write8(psCam->i2c, ui8fpsRegs, sizeof(ui8fpsRegs));
        psCam->fCurrentFps = psCam->currMode.ui16VerticalTotal * psCam->fixedFps / ui16VerticalTotal;
	}
	exposureRegs[5] = (ui32Lines >> 12) & 0xf;
	exposureRegs[8] = (ui32Lines >> 4) & 0xff;
	exposureRegs[11] = (ui32Lines << 4) & 0xff;
#else
	
	exposureRegs[5] = (((psCam->ui32Exposure)/psCam->currMode.ui32ExposureMin)>>12) & 0xf;
	exposureRegs[8] = (((psCam->ui32Exposure)/psCam->currMode.ui32ExposureMin)>>4) & 0xff;
	exposureRegs[11] = (((psCam->ui32Exposure)/psCam->currMode.ui32ExposureMin)<<4) & 0xff;
#endif
	LOG_DEBUG("Exposure Val %x\n",(psCam->ui32Exposure/psCam->currMode.ui32ExposureMin));

//    if (psCam->bEnabled || 1)
//    {
//        OV4689_ComputeExposure(psCam->ui32Exposure, psCam->currMode.ui32ExposureMin,
//            &(exposureRegs[3]));

//        ov4689_i2c_write8(psCam->i2c, exposureRegs, sizeof(exposureRegs));
//    }

	ov4689_i2c_write8(psCam->i2c, exposureRegs, sizeof(exposureRegs));
#ifdef ISP_VISION_LIVE
    s_ui32Exposure = psCam->ui32Exposure;
    g_OV4689_ui8LinesValHiBackup = exposureRegs[5];
	g_OV4689_ui8LinesValMidBackup = exposureRegs[8];
	g_OV4689_ui8LinesValLoBackup = exposureRegs[11];
#endif
    return IMG_SUCCESS;
}

/*
static IMG_UINT16 asGainSettings[] =
{
	0x0080,
	0x0088,
	0x0090,
	0x0098,
	0x00A0,
	0x00A8,
	0x00B0,
	0x00B8,
	0x00C0,
	0x00C8,
	0x00D0,
	0x00D8,
	0x00E0,
	0x00E8,
	0x00F0,
	0x00F8,
	0x0178,
	0x017C,
	0x0180,
	0x0184,
	0x0188,
	0x018C,
	0x0190,
	0x0194,
	0x0198,
	0x019C,
	0x01A0,
	0x01A4,
	0x01A8,
	0x01AC,
	0x01B0,
	0x01B4,
	0x01B8,
	0x01BC,
	0x01C0,
	0x01C4,
	0x01C8,
	0x01CC,
	0x01D0,
	0x01D4,
	0x01D8,
	0x01DC,
	0x01E0,
	0x01E4,
	0x01E8,
	0x01EC,
	0x01F0,
	0x01F4,
	0x0374,
	0x0376,
	0x0378,
	0x037A,
	0x037C,
	0x037E,
	0x0380,
	0x0382,
	0x0384,
	0x0386,
	0x0388,
	0x038A,
	0x038C,
	0x038E,
	0x0390,
	0x0392,
	0x0394,
	0x0396,
	0x0398,
	0x039A,
	0x039C,
	0x039E,
	0x03A0,
	0x03A2,
	0x03A4,
	0x03A6,
	0x03A8,
	0x03AA,
	0x03AC,
	0x03AE,
	0x03B0,
	0x03B2,
	0x03B4,
	0x03B6,
	0x03B8,
	0x03BA,
	0x03BC,
	0x03BE,
	0x03C0,
	0x03C2,
	0x03C4,
	0x03C6,
	0x03C8,
	0x03CA,
	0x03CC,
	0x03CE,
	0x03D0,
	0x03D2,
	0x03D4,
	0x03D6,
	0x03D8,
	0x03DA,
	0x03DC,
	0x03DE,
	0x03E0,
	0x03E2,
	0x03E4,
	0x03E6,
	0x03E8,
	0x03EA,
	0x03EC,
	0x03EE,
	0x03F0,
	0x03F2,
	0x0778,
	0x0779,
	0x077A,
	0x077B,
	0x077C,
	0x077D,
	0x077E,
	0x077F,
	0x0780,
	0x0781,
	0x0782,
	0x0783,
	0x0784,
	0x0785,
	0x0786,
	0x0787,
	0x0788,
	0x0789,
	0x078A,
	0x078B,
	0x078C,
	0x078D,
	0x078E,
	0x078F,
	0x0790,
	0x0791,
	0x0792,
	0x0793,
	0x0794,
	0x0795,
	0x0796,
	0x0797,
	0x0798,
	0x0799,
	0x079A,
	0x079B,
	0x079C,
	0x079D,
	0x079E,
	0x079F,
	0x07A0,
	0x07A1,
	0x07A2,
	0x07A3,
	0x07A4,
	0x07A5,
	0x07A6,
	0x07A7,
	0x07A8,
	0x07A9,
	0x07AA,
	0x07AB,
	0x07AC,
	0x07AD,
	0x07AE,
	0x07AF,
	0x07B0,
	0x07B1,
	0x07B2,
	0x07B3,
	0x07B4,
	0x07B5,
	0x07B6,
	0x07B7,
	0x07B8,
	0x07B9,
	0x07BA,
	0x07BB,
	0x07BC,
	0x07BD,
	0x07BE,
	0x07BF,
	0x07C0,
	0x07C1,
	0x07C2,
	0x07C3,
	0x07C4,
	0x07C5,
	0x07C6,
	0x07C7,
	0x07C8,
	0x07C9,
	0x07CA,
	0x07CB,
	0x07CC,
	0x07CD,
	0x07CE,
	0x07CF,
	0x07D0,
	0x07D1,
	0x07D2,
	0x07D3,
	0x07D4,
	0x07D5,
	0x07D6,
	0x07D7,
	0x07D8,
	0x07D9,
	0x07DA,
	0x07DB,
	0x07DC,
	0x07DD,
	0x07DE,
	0x07DF,
	0x07E0,
	0x07E1,
	0x07E2,
	0x07E3,
	0x07E4,
	0x07E5,
	0x07E6,
	0x07E7,
	0x07E8,
	0x07E9,
	0x07EA,
	0x07EB,
	0x07EC,
	0x07ED,
	0x07EE,
	0x07EF,
	0x07F0,
	0x07F1,
	0x07F2,
	0x07F3,
	0x07F4,
	0x07F5,
	0x07F6,
	0x07F7,
};
*/
void OV4689_ComputeGains(double flGain, IMG_UINT8 *gainSequence)
{
#if 0
    unsigned int n = 0; // if flGain <= 1.0
    if (flGain > aec_min_gain)
    {
        n = (unsigned int)floor((flGain - aec_min_gain) * aec_max_gain);
    }
    n = IMG_MIN_INT(n, sizeof(aec_long_gain_val) / sizeof(IMG_UINT16) -1);

    LOG_DEBUG("gain n=%u from %lf\n", n, (flGain-aec_min_gain)*aec_max_gain);
    gainSequence[2] = 0;
    gainSequence[5] = (aec_long_gain_val[n] >> 8) & 0xff;
    gainSequence[8] = aec_long_gain_val[n] & 0xff;
#else
    IMG_UINT32 reg = 0;
    if (flGain < 2.0)
    {
       reg = flGain*128;
    }
    else if (flGain < 4.0)
    {
       reg = 256 + (flGain*64 - 8);
    }
    else if (flGain < 8.0)
    {
       reg = 256*3 + (flGain*32 - 12);
    }
    else
    {
       reg = 256*7 + (flGain*16 - 8);
    }
    
    gainSequence[2] = (reg >> 16)&0xff;
    gainSequence[5] = (reg >> 8) & 0xff;
    gainSequence[8] = reg & 0xff;

#endif
}

static IMG_RESULT OV4689_SetGain(SENSOR_HANDLE hHandle, double flGain,
    IMG_UINT8 ui8Context)
{
#ifdef INFOTM_ISP
	IMG_UINT32 n = 0;
#endif //INFOTM_ISP
	static int runing_flag = 0;
    IMG_UINT8 gainRegs[] =
    {
        0x32, 0x08, 0x00,  // start hold of group 0

        0x35, 0x07, 0x00,  // Gain register
        0x35, 0x08, 0x00,  //
        0x35, 0x09, 0x00,  //

        0x32, 0x08, 0x10,  // end hold of group 1 register writes
        0x32, 0x0b, 0x00,  // set quick manual mode
        0x32, 0x08, 0xe0,  // quick launch group 1
    };

	if (runing_flag == 1)
	{
		return IMG_SUCCESS;
	}

	runing_flag = 1;
    OV4689CAM_STRUCT *psCam = container_of(hHandle, OV4689CAM_STRUCT, sFuncs);

    if (!psCam->psSensorPhy)
    {
        LOG_ERROR("sensor not initialised\n");
        return IMG_ERROR_NOT_INITIALISED;
    }

    psCam->flGain = flGain;

#if 1//ndef INFOTM_ISP
    if (flGain < aec_min_gain)
    {
        flGain = aec_min_gain;
    }
    
    if (flGain > aec_max_gain)
    {
        flGain = aec_max_gain;
    }

    //if (psCam->bEnabled || 1)
    {
        OV4689_ComputeGains(flGain, &(gainRegs[3]));

        ov4689_i2c_write8(psCam->i2c, gainRegs, sizeof(gainRegs));
    }
#else
	if (flGain > aec_min_gain)
	{
		n = (IMG_UINT32)floor((flGain - 1) * 16);
	}

	n = IMG_MIN_INT(n, sizeof(aec_long_gain_val) / sizeof(IMG_UINT16) -1);

	LOG_DEBUG("gain n=%u from %lf\n", n, (flGain-1)*16);
	gainRegs[5] = 0;//asGainSettings[n] >> 16;
	gainRegs[8] = (aec_long_gain_val[n] >> 8) & 0xff;
	gainRegs[11] = aec_long_gain_val[n] & 0xff;

	ov4689_i2c_write8(psCam->i2c, gainRegs, sizeof(gainRegs));
#endif //INFOTM_ISP
#ifdef ISP_VISION_LIVE
    s_flGain = psCam->flGain;

	g_OV4689_ui8GainValHiBackup = gainRegs[5];
	g_OV4689_ui8GainValMidBackup = gainRegs[8];
	g_OV4689_ui8GainValLoBackup = gainRegs[11];
#endif
    return IMG_SUCCESS;
}

static IMG_RESULT OV4689_GetGainRange(SENSOR_HANDLE hHandle, double *pflMin,
    double *pflMax, IMG_UINT8 *pui8Contexts)
{
    /*OV4689CAM_STRUCT *psCam = container_of(hHandle, OV4689CAM_STRUCT, sFuncs);

    if (!psCam->psSensorPhy)
    {
        LOG_ERROR("sensor not initialised\n");
        return IMG_ERROR_NOT_INITIALISED;
    }*/

    *pflMin = aec_min_gain;
    *pflMax = wb_gain_aec_max_gain;//aec_max_gain;
    *pui8Contexts = 1;

    return IMG_SUCCESS;
}

static IMG_RESULT OV4689_GetGain(SENSOR_HANDLE hHandle, double *pflGain,
    IMG_UINT8 ui8Context)
{
    OV4689CAM_STRUCT *psCam = container_of(hHandle, OV4689CAM_STRUCT, sFuncs);

    if (!psCam->psSensorPhy)
    {
        LOG_ERROR("sensor not initialised\n");
        return IMG_ERROR_NOT_INITIALISED;
    }

    *pflGain=psCam->flGain;
    return IMG_SUCCESS;
}
void * test(void * arg)
{
    OV4689CAM_STRUCT *psCam = (OV4689CAM_STRUCT *) arg;
    int fixedfps = 0;
    int fps = 20;
	int cnt, i;
	IMG_UINT16 ui16VerticalTotal, tmpVal;
	static IMG_UINT16 BK_ui16VerticalTotal = 0xFFFF;

    IMG_UINT8 ui8Regs[] =
	{
		0x32, 0x08, 0x00, // GROUP_ACCESS start hold of group 0

		0x38, 0x0E, 0x00, // Exposure register
		0x38, 0x0F, 0x00, //

		0x32, 0x08, 0x10, // end hold of group 0 register writes
		0x32, 0x0b, 0x00, // set quick manual mode
		0x32, 0x08, 0xe0, // quick launch group 0
	};

    sleep(10);
    while(1)
    {
        //LOG_DEBUG("#################fps=%d#####################\n", fps);
        fixedfps = psCam->fixedFps;
        if(fps > fixedfps)
        {
            fps = fixedfps;
        }
        else if (fps < 5)
        {
            fps = 5;
        }

        if(psCam->fCurrentFps == fps)
        {
        //    return IMG_SUCCESS;
        }

        ui16VerticalTotal = psCam->currMode.ui16VerticalTotal * psCam->fixedFps / fps;
        ui8Regs[5] = (ui16VerticalTotal >> 8);
        ui8Regs[8] = (ui16VerticalTotal & 0xff);
        ov4689_i2c_write8(psCam->i2c, ui8Regs, sizeof(ui8Regs));


    //LOG_DEBUG("^^^^^^^^^^^^^^^^^0x380e=%x^0x380f=%x^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n", ui8Regs[5], ui8Regs[8]);
    //LOG_DEBUG("^^^^^^^^^^^^^^^^^psCam->fixedFps=%d^fps=%d^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n", psCam->fixedFps, fps);
    //LOG_DEBUG("^^^^^^^^^^^^^^^^^ui16VerticalTotal=%d^psCam->currMode->ui16VerticalTotal=%d^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n", ui16VerticalTotal, psCam->currMode.ui16VerticalTotal);


        psCam->fCurrentFps = fps;

        sleep(10);
        fps--;
        if(fps<8)
            fps = 20;
    }

    return IMG_SUCCESS;
}

IMG_RESULT OV4689_Create(struct _Sensor_Functions_  **phHandle,int index)
{

#if !file_i2c
    char i2c_dev_path[NAME_MAX];
    IMG_UINT16 chipV;
    char dev_nm[64];
    char adaptor[64];
    int chn=0;
    IMG_RESULT ret;

    pthread_t tid;

#endif //file_i2c
    OV4689CAM_STRUCT *psCam;
	LOG_DEBUG("-->OV4689CamCreate\n");

#if defined(INFOTM_ISP)
    int fd = 0;
    memset((void *)dev_nm,0,sizeof(dev_nm));
    memset((void *)adaptor,0,sizeof(adaptor));
    sprintf(dev_nm,"%s%d",DEV_PATH,index);

    fd = open(dev_nm, O_RDWR);
	if(fd <0)
	{
		LOG_ERROR("open %s error\n",dev_nm);
		return IMG_ERROR_FATAL;
	}

	ioctl(fd, GETI2CADDR, &i2c_addr);
        ioctl(fd, GETI2CCHN, &chn);
	ioctl(fd, GETIMAGER, &imgr);
	close(fd);
	//printf("%s opened OK,i2c-addr=0x%x,chn=%d\n",dev_nm,i2c_addr,chn);
	sprintf(adaptor,"%s-%d","i2c",chn);
	memset((void *)extra_cfg,0,sizeof(dev_nm));
	sprintf(extra_cfg,"%s%s%d-config.txt",EXTRA_CFG_PATH,"sensor",index);
#endif
    psCam=(OV4689CAM_STRUCT *)IMG_CALLOC(1, sizeof(OV4689CAM_STRUCT));
    if(!psCam)
        return IMG_ERROR_MALLOC_FAILED;

    *phHandle=&(psCam->sFuncs);
    psCam->sFuncs.GetMode = OV4689_GetMode;
    psCam->sFuncs.GetState = OV4689_GetState;
    psCam->sFuncs.SetMode = OV4689_SetMode;
    psCam->sFuncs.Enable = OV4689_Enable;
    psCam->sFuncs.Disable = OV4689_Disable;
    psCam->sFuncs.Destroy = OV4689_Destroy;
    psCam->sFuncs.GetInfo = OV4689_GetInfo;

    psCam->sFuncs.GetCurrentGain = OV4689_GetGain;
    psCam->sFuncs.GetGainRange = OV4689_GetGainRange;
    psCam->sFuncs.SetGain = OV4689_SetGain;
    psCam->sFuncs.SetExposure = OV4689_SetExposure;
    psCam->sFuncs.GetExposureRange = OV4689_GetExposureRange;
    psCam->sFuncs.GetExposure = OV4689_GetExposure;

    psCam->sFuncs.GetCurrentFocus = OV4689_GetCurrentFocus;
    psCam->sFuncs.GetFocusRange = OV4689_GetFocusRange;
    psCam->sFuncs.SetFocus = OV4689_SetFocus;
#ifdef INFOTM_ISP
    psCam->sFuncs.SetFlipMirror = OV4689_SetFlipMirror;
	psCam->sFuncs.SetGainAndExposure = OV4689_SetGainAndExposure;
	psCam->sFuncs.SetFPS = OV4689_SetFPS;
	psCam->sFuncs.GetFixedFPS =  OV4689_GetFixedFPS;
#endif //INFOTM_ISP

    // customers should change the clock!
    psCam->refClock = 24 * 1000 * 1000;

    psCam->bEnabled = IMG_FALSE;
    psCam->ui16CurrentMode = 0;
    psCam->currModeReg = NULL;
    psCam->nRegisters = 0;

    psCam->ui8CurrentFlipping = SENSOR_FLIP_HORIZONTAL;
    psCam->flGain = aec_min_gain;
    psCam->ui32Exposure = 89*302;
    psCam->imager = 0; // gasket to use - customer should change it if needed
#ifdef ISP_VISION_LIVE
    s_ui32Exposure = psCam->ui32Exposure;
    s_flGain = psCam->flGain;
    s_ui8CurrentFlipping = psCam->ui8CurrentFlipping;
#endif
#if !file_i2c
    if (find_i2c_dev(i2c_dev_path, sizeof(i2c_dev_path), i2c_addr, adaptor))
    {
        LOG_ERROR("Failed to find I2C device!\n");
        IMG_FREE(psCam);
        *phHandle=NULL;
        return IMG_ERROR_DEVICE_NOT_FOUND;
    }

    psCam->i2c = open(i2c_dev_path, O_RDWR);
    if (psCam->i2c < 0)
    {
        LOG_ERROR("Failed to open I2C device: \"%s\", err = %d\n",
            i2c_dev_path, psCam->i2c);
        IMG_FREE(psCam);
        *phHandle=NULL;
        return IMG_ERROR_DEVICE_NOT_FOUND;
    }

	#ifndef NO_DEV_CHECK
		ret = ov4689_i2c_read16(psCam->i2c, REG_SC_CMMN_CHIP_ID_0, &chipV);
		if(ret || OV4689_CHIP_VERSION != chipV)
		{
			LOG_ERROR("Failed to ensure that the i2c device has a compatible "\
				"OV4689 sensor! ret=%d chip_version=0x%x (expect chip 0x%x)\n",
				ret, chipV, OV4689_CHIP_VERSION);
			close(psCam->i2c);
			IMG_FREE(psCam);
			*phHandle = NULL;
			return IMG_ERROR_DEVICE_NOT_FOUND;
		}
	#endif //NO_DEV_CHECK

	#ifdef ov4689_debug
		// clean file
		{
			FILE *f = fopen(ov4689_debug, "w");
			fclose(f);
		}
	#endif //ov4689_debug
#endif //file_i2c
	ret = ov4689_i2c_read16(psCam->i2c, REG_SC_CMMN_CHIP_ID_0, &chipV);
	printf("check ov4689 chip version: 0x%x\n", chipV);
    // need i2c to have been found to read mode
    OV4689_GetModeInfo(psCam, psCam->ui16CurrentMode, &(psCam->currMode));
    psCam->psSensorPhy = SensorPhyInit();
    if (!psCam->psSensorPhy)
    {
        LOG_ERROR("Failed to create sensor phy!\n");
#if !file_i2c
        close(psCam->i2c);
#endif //file_i2c
        IMG_FREE(psCam);
        *phHandle = NULL;
        return IMG_ERROR_DEVICE_NOT_FOUND;
    }

 //   pthread_create(&tid, NULL, test, psCam);

    LOG_DEBUG("Sensor initialised\n");
    return IMG_SUCCESS;
}

void OV4689_ApplyMode(OV4689CAM_STRUCT *psCam)
{
    unsigned i, ret = 0;
    // index for flipping register
    unsigned flipIndex0 = psCam->nRegisters * 3;
    // index for flipping register
    unsigned flipIndex1 = psCam->nRegisters * 3;
    // index for exposure register
    unsigned exposureIndex = psCam->nRegisters * 3;
    // index for gain register
    unsigned gainIndex = psCam->nRegisters * 3;
#if !defined(OV4689_COPY_REGS)
    IMG_UINT8 values[3];
#endif //OV4689_COPY_REGS
    IMG_UINT8 ui8Regs[] =
	{
		0x32, 0x08, 0x00, // GROUP_ACCESS start hold of group 0

		0x35, 0x00, 0x00, // Exposure register
		0x35, 0x01, 0x2E, //
		0x35, 0x02, 0x80, //

		0x35, 0x07, 0x00, // Gain register
		0x35, 0x08, 0x00, //
		0x35, 0x09, 0x00, //

		0x32, 0x08, 0x10, // end hold of group 0 register writes
		0x32, 0x0b, 0x00, // set quick manual mode
		0x32, 0x08, 0xe0, // quick launch group 0
	};

    if (!psCam->currModeReg)
    {
        LOG_ERROR("current register modes not available!\n");
        return;
    }


    LOG_DEBUG("Writing I2C\n");

    printf("OV4689_ApplyMode()\n");

    for(i = 0 ; i < psCam->nRegisters * 3 ; i += 3)
    {
#if !defined(OV4689_COPY_REGS)
        values[0] = psCam->currModeReg[i+0];
        values[1] = psCam->currModeReg[i+1];
        values[2] = psCam->currModeReg[i+2];
#endif //OV4689_COPY_REGS

        if (IS_REG(psCam->currModeReg, i, REG_FORMAT1))
        {
            flipIndex0 = i;
            if (psCam->ui8CurrentFlipping&SENSOR_FLIP_VERTICAL)
            {
#if defined(OV4689_COPY_REGS)
                psCam->currModeReg[i+2] = 6;
#else
                values[2] = 6;
#endif //OV4689_COPY_REGS
            }
            else
            {
#if defined(OV4689_COPY_REGS)
                psCam->currModeReg[i+2] = 0;
#else
                values[2] = 0;
#endif //OV4689_COPY_REGS
            }
        }
        if (IS_REG(psCam->currModeReg, i, REG_FORMAT2))
        {
            flipIndex1 = i;
            if (psCam->ui8CurrentFlipping&SENSOR_FLIP_HORIZONTAL)
            {
#if defined(OV4689_COPY_REGS)
                psCam->currModeReg[i+2] = 6;
#else
                values[2] = 6;
#endif //OV4689_COPY_REGS

            }
            else
            {
#if defined(OV4689_COPY_REGS)
                psCam->currModeReg[i+2] = 0;
#else
                values[2] = 0;
#endif //OV4689_COPY_REGS
            }
        }

        if (IS_REG(psCam->currModeReg, i, REG_EXPOSURE))
        {
            exposureIndex = i;
        }

        if (IS_REG(psCam->currModeReg, i, REG_GAIN))
        {
            gainIndex = i;
        }

#if !defined(OV4689_COPY_REGS)
        ret = ov4689_i2c_write8(psCam->i2c, values, 3);
        if(ret != IMG_SUCCESS)
        {
        	break;
        }
#endif //OV4689_COPY_REGS
    }

    // if it was parallel
    //psCam->psSensorPhy->psGasket->uiWidth = psCam->currMode.ui16Width;
    //psCam->psSensorPhy->psGasket->uiHeight = psCam->currMode.ui16Height;
    //psCam->psSensorPhy->psGasket->bVSync = IMG_TRUE;
    //psCam->psSensorPhy->psGasket->bHSync = IMG_TRUE;
    //psCam->psSensorPhy->psGasket->ui8ParallelBitdepth = 10;

    psCam->ui32Exposure = psCam->currMode.ui32ExposureMin * 302;
    psCam->ui32Exposure = IMG_MIN_INT(psCam->currMode.ui32ExposureMax,
        psCam->ui32Exposure);

    //
    // before writing the registers, patch the flip horizontal/flipvertical
    // fields to match the required values
    //
    // should be found and fit in array
    if (flipIndex0 >= psCam->nRegisters * 3)
    {
        LOG_WARNING("Did not find flipIndex0 in registers for mode %d\n",
            psCam->ui16CurrentMode);
    }
    if (flipIndex1 >= psCam->nRegisters * 3)
    {
        LOG_WARNING("Did not find flipIndex1 in registers for mode %d\n",
            psCam->ui16CurrentMode);
    }

    if (gainIndex >= psCam->nRegisters * 3)
    {
        LOG_WARNING("Did not find gainIndex in registers for mode %d\n",
            psCam->ui16CurrentMode);
    }
    if (exposureIndex >= psCam->nRegisters * 3)
    {
        LOG_WARNING("Did not find exposureIndex in registers for mode %d\n",
            psCam->ui16CurrentMode);
    }
#if 0
    // should be found and fit in array
    IMG_ASSERT(gainIndex+8 < psCam->nRegisters * 3);
    {
        OV4689_ComputeExposure(psCam->ui32Exposure, psCam->currMode.ui32ExposureMin,
            &(mode1280x720[exposureIndex]));
    }

    // should be found and fit in array
    IMG_ASSERT(exposureIndex+8 < psCam->nRegisters * 3);
    {
        OV4689_ComputeGains(psCam->flGain, &(mode1280x720[gainIndex]));
    }
#endif //0

#if defined(OV4689_COPY_REGS)
    ov4689_i2c_write8(psCam->i2c, psCam->currModeReg, psCam->nRegisters * 3);
#endif //OV4689_COPY_REGS

    ui8Regs[5] = g_OV4689_ui8LinesValHiBackup ;
	ui8Regs[8] = g_OV4689_ui8LinesValMidBackup;
	ui8Regs[11] = g_OV4689_ui8LinesValLoBackup ;
	ui8Regs[14] = g_OV4689_ui8GainValHiBackup;
	ui8Regs[17] = g_OV4689_ui8GainValMidBackup ;
	ui8Regs[20] = g_OV4689_ui8GainValLoBackup;

    ov4689_i2c_write8(psCam->i2c, ui8Regs, sizeof(ui8Regs));

    LOG_DEBUG("Sensor ready to go\n");
    // setup the exposure setting information
}

IMG_RESULT OV4689_GetModeInfo(OV4689CAM_STRUCT *psCam, IMG_UINT16 ui16Mode,
    SENSOR_MODE *info)
{
    unsigned int n;
    IMG_RESULT ret = IMG_SUCCESS;

    unsigned int nRegisters = 0;
    const IMG_UINT8 *modeReg = OV4689_GetRegisters(psCam, ui16Mode,
        &nRegisters);

    double sclk = 0;

    unsigned int sc_cmmm_bit_sel = 0;
    unsigned int sc_cmmn_mipi_sc_ctrl = 0;
    unsigned int hts[2] = { 0, 0 };
    unsigned int hts_v = 0;
    unsigned int vts[2] = { 0, 0 };
    unsigned int vts_v = 0;
    unsigned int frame_t = 0;
    unsigned int h_output[2] = { 0, 0 };
    unsigned int v_output[2] = { 0, 0 };
    unsigned int pll_ctrl_d[2] = { 0, 0 };

    double pll2_predivp = 0;
    double pll2_mult = 0;
    double pll2_prediv = 0;
    double pll2_divsp = 0;
    double pll2_divs = 0;

    double trow = 0; // in micro seconds
    if (!modeReg)
    {
        LOG_ERROR("invalid mode %d\n", modeReg);
        return IMG_ERROR_NOT_SUPPORTED;
    }
    nRegisters = ov4689_modes[ui16Mode].nRegisters;

    for (n = 0 ; n < nRegisters * 3 ; n += 3)
    {
        if (IS_REG(modeReg, n, REG_SC_CMMN_BIT_SEL))
        {
            sc_cmmm_bit_sel = modeReg[n + 2];
            LOG_DEBUG("sc_cmmm_bit_sel = 0x%x\n", sc_cmmm_bit_sel);
        }

        if (IS_REG(modeReg, n, REG_SC_CMMN_MIPI_SC_CTRL))
        {
            sc_cmmn_mipi_sc_ctrl = modeReg[n + 2];
            LOG_DEBUG("sc_cmmn_mipi_sc_ctrl = 0x%x\n", sc_cmmn_mipi_sc_ctrl);
        }

        if (IS_REG(modeReg, n, REG_TIMING_VTS_0))
        {
            vts[0] = modeReg[n + 2];
            LOG_DEBUG("vts[0] = 0x%x\n", vts[0]);
        }
        if (IS_REG(modeReg, n, REG_TIMING_VTS_1))
        {
            vts[1] = modeReg[n + 2];
            LOG_DEBUG("vts[1] = 0x%x\n", vts[1]);
        }

        if (IS_REG(modeReg, n, REG_TIMING_HTS_0))
        {
            hts[0] = modeReg[n + 2];
            LOG_DEBUG("hts[0]=0x%x\n", hts[0]);
        }
        if (IS_REG(modeReg, n, REG_TIMING_HTS_1))
        {
            hts[1] = modeReg[n + 2];
            LOG_DEBUG("hts[1]=0x%x\n", hts[1]);
        }

        if (IS_REG(modeReg, n, REG_H_OUTPUT_SIZE_0))
        {
            h_output[0] = modeReg[n + 2];
            LOG_DEBUG("h_output[0] = 0x%x, n=%d\n", h_output[0], n);
        }
        if (IS_REG(modeReg, n, REG_H_OUTPUT_SIZE_1))
        {
            h_output[1] = modeReg[n + 2];
            LOG_DEBUG("h_output[1] = 0x%x\n", h_output[1]);
        }

        if (IS_REG(modeReg, n, REG_V_OUTPUT_SIZE_0))
        {
            v_output[0] = modeReg[n + 2];
            LOG_DEBUG("v_output[0] = 0x%x\n", v_output[0]);
        }
        if (IS_REG(modeReg, n, REG_V_OUTPUT_SIZE_1))
        {
            v_output[1] = modeReg[n + 2];
            LOG_DEBUG("v_output[1] = 0x%x\n", v_output[1]);
        }

        if (IS_REG(modeReg, n, REG_PLL_CTRL_11))
        {
            pll2_predivp = modeReg[n + 2] + 1.0;
            LOG_DEBUG("pll2_predivp = 0x%x -> %lf\n",
                modeReg[n + 2], pll2_predivp);
        }

        if (IS_REG(modeReg, n, REG_PLL_CTRL_B))
        {
            pll2_prediv = pll_ctrl_b_val[modeReg[n + 2]];
            LOG_DEBUG("pll2_prediv = 0x%x -> %lf\n",
                modeReg[n + 2], pll2_prediv);
        }

        if (IS_REG(modeReg, n, REG_PLL_CTRL_C))
        {
            pll_ctrl_d[0] = modeReg[n + 2];
            LOG_DEBUG("pll_ctrl_d[0] = 0x%x\n", pll_ctrl_d[0]);
        }
        if (IS_REG(modeReg, n, REG_PLL_CTRL_D))
        {
            pll_ctrl_d[1] = modeReg[n + 2];
            LOG_DEBUG("pll_ctrl_d[1] = 0x%x\n", pll_ctrl_d[1]);
        }

        if (IS_REG(modeReg, n, REG_PLL_CTRL_F))
        {
            pll2_divsp = modeReg[n + 2] + 1.0;
            LOG_DEBUG("pll2_divsp = 0x%x -> %lf\n",
                modeReg[n + 2], pll2_divsp);
        }

        if (IS_REG(modeReg, n, REG_PLL_CTRL_E))
        {
            pll2_divs = pll_ctrl_c_val[modeReg[n + 2]];
            LOG_DEBUG("pll2_divs = 0x%x -> %lf\n", modeReg[n + 2], pll2_divs);
        }
    }

    pll2_mult = (double)((pll_ctrl_d[0] << 8) | pll_ctrl_d[1]);

    if (0.0 == pll2_predivp || 0.0 == pll2_mult || 0.0 == pll2_prediv
        || 0.0 == pll2_divsp || 0.0 == pll2_divs)
    {
        sclk = 120.0 * 1000 * 1000;
        LOG_WARNING("Did not find all PLL registers - assumes "\
            "sclk of %ld MHz\n", sclk);
#ifdef INFOTM_ISP
        LOG_WARNING("pll2_predivp=%f, pll2_mult=%f, pll2_prediv=%f, pll2_divsp=%f, pll2_divs=%f\n",
        				pll2_predivp, pll2_mult, pll2_prediv, pll2_divsp, pll2_divs);
#endif //INFOTM_ISP
    }
    else
    {
        // xclk is the refClock
        // vco = xclk/PLL2_predivp * PLL2_mult/PLL2_prediv
        double vco = 0;
        // sclk = vco/PLL2_divsp/PLL2_divs

        vco = ((psCam->refClock / pll2_predivp) / pll2_prediv)*pll2_mult;
        sclk = vco / pll2_divsp / pll2_divs;

        LOG_DEBUG("xclk=%.2lfMHz / pll2_predivp=%.2lf / pll2_prediv=%.2lf "\
            "* pll2_mult=%.2lf\n",
            psCam->refClock/1000000, pll2_predivp, pll2_prediv, pll2_mult);
        LOG_DEBUG("vco=%.2lfMHz / pll2_divsp=%.2lf / pll2_divs=%.2lf "\
            "= sclk=%.2lfMHz\n",
            vco/1000000, pll2_divsp, pll2_divs, sclk/1000000);
    }

    hts_v = (hts[1] | (hts[0] << 8));
    vts_v = (vts[1] | (vts[0] << 8));
    trow = (hts_v / sclk) * 1000 * 1000; // sclk in Hz, trow in micro seconds
    frame_t = vts_v * hts_v;
    
    LOG_DEBUG("hts=%u vts=%u\n", hts_v, vts_v);
    LOG_DEBUG("trow=%lf frame_t=%u\n", trow, frame_t);

    info->ui16Width = h_output[1] | (h_output[0] << 8);
    info->ui16Height = v_output[1] | (v_output[0] << 8);
    info->ui16VerticalTotal = vts_v;
    s_ui16VerticalTotal = vts_v;

    //LOG_DEBUG("**************************hts=%u vts=%u**************************\n", hts_v, vts_v);

    info->ui8SupportFlipping = SENSOR_FLIP_BOTH;
    info->ui32ExposureMax = (IMG_UINT32)floor(trow * vts_v);
    info->ui32ExposureMin = (IMG_UINT32)floor(trow);
	
#ifdef INFOTM_ISP
	info->dbExposureMin = trow;
	psCam->ui32Exposure = trow * info->ui16VerticalTotal; //set initial exposure.
#endif //INFOTM_ISP
		
    info->flFrameRate = sclk / frame_t;
    
    info->ui8MipiLanes = ((sc_cmmn_mipi_sc_ctrl >> 5) & 0x3) + 1;
	//printf("info->ui8MipiLanes = %d\n", info->ui8MipiLanes);

    info->ui8BitDepth = (sc_cmmm_bit_sel & 0x1F);
#if defined(INFOTM_ISP)
    psCam->fCurrentFps = info->flFrameRate;
    //printf("the flFrameRate = %f\n", info->flFrameRate);
    psCam->fixedFps = psCam->fCurrentFps;
    psCam->initClk = pll2_divsp - 1;
#endif
    return IMG_SUCCESS;
}

#ifdef INFOTM_ISP
static IMG_RESULT OV4689_SetFlipMirror(SENSOR_HANDLE hHandle, IMG_UINT8 flag)
{
    OV4689CAM_STRUCT *psCam = container_of(hHandle, OV4689CAM_STRUCT, sFuncs);
    IMG_UINT16 ui16Regs;
    IMG_UINT8 aui18FlipMirrorRegs[]=
    {
        0x38, 0x21, 0x00,
        0x38, 0x20, 0x00
    };
    IMG_RESULT ret;

    switch (flag)
    {
    	default:
    	case SENSOR_FLIP_NONE:
    		ui16Regs = 0x3821;
    		ret = ov4689_i2c_read8(psCam->i2c, ui16Regs, &aui18FlipMirrorRegs[2]);
    		aui18FlipMirrorRegs[2] &= ~0x06; //clear mirror bit.

    		ui16Regs = 0x3820;
    		ret = ov4689_i2c_read8(psCam->i2c, ui16Regs, &aui18FlipMirrorRegs[5]);
    		aui18FlipMirrorRegs[5] &= ~0x06; //clear flip bit.

    		ov4689_i2c_write8(psCam->i2c, aui18FlipMirrorRegs, sizeof(aui18FlipMirrorRegs));
    		psCam->ui8CurrentFlipping = SENSOR_FLIP_NONE;
    		break;

    	case SENSOR_FLIP_HORIZONTAL:
       		ui16Regs = 0x3821;
        	ret = ov4689_i2c_read8(psCam->i2c, ui16Regs, &aui18FlipMirrorRegs[2]);
        	aui18FlipMirrorRegs[2] |= 0x06; //set mirror bit.

    		ui16Regs = 0x3820;
    		ret = ov4689_i2c_read8(psCam->i2c, ui16Regs, &aui18FlipMirrorRegs[5]);
    		aui18FlipMirrorRegs[5] &= ~0x06; //clear flip bit.

    		ov4689_i2c_write8(psCam->i2c, aui18FlipMirrorRegs, sizeof(aui18FlipMirrorRegs));
    		psCam->ui8CurrentFlipping = SENSOR_FLIP_HORIZONTAL;
    		break;

    	case SENSOR_FLIP_VERTICAL:
    		ui16Regs = 0x3821;
    		ret = ov4689_i2c_read8(psCam->i2c, ui16Regs, &aui18FlipMirrorRegs[2]);
    		aui18FlipMirrorRegs[2] &= ~0x06; //clear mirror bit.

    		ui16Regs = 0x3820;
    		ret = ov4689_i2c_read8(psCam->i2c, ui16Regs, &aui18FlipMirrorRegs[5]);
    		aui18FlipMirrorRegs[5] |= 0x06; //set flip bit.

    		ov4689_i2c_write8(psCam->i2c, aui18FlipMirrorRegs, sizeof(aui18FlipMirrorRegs));
    		psCam->ui8CurrentFlipping = SENSOR_FLIP_VERTICAL;
    		break;

    	case SENSOR_FLIP_BOTH:
    		ui16Regs = 0x3821;
    		ret = ov4689_i2c_read8(psCam->i2c, ui16Regs, &aui18FlipMirrorRegs[2]);
    		aui18FlipMirrorRegs[2] |= 0x06; //set mirror bit.

    		ui16Regs = 0x3820;
    		ret = ov4689_i2c_read8(psCam->i2c, ui16Regs, &aui18FlipMirrorRegs[5]);
    		aui18FlipMirrorRegs[5] |= 0x06; //set flip bit.

    		ov4689_i2c_write8(psCam->i2c, aui18FlipMirrorRegs, sizeof(aui18FlipMirrorRegs));
    		psCam->ui8CurrentFlipping = SENSOR_FLIP_BOTH;
    		break;

    }
#ifdef ISP_VISION_LIVE
    s_ui8CurrentFlipping = psCam->ui8CurrentFlipping;
#endif
    	return IMG_SUCCESS;
}

static IMG_RESULT OV4689_SetGainAndExposure(SENSOR_HANDLE hHandle, double flGain, IMG_UINT32 ui32Exposure, IMG_UINT8 ui8Context)
{
#if 0
    OV4689_SetExposure(hHandle, ui32Exposure, ui8Context);
    OV4689_SetGain(hHandle, flGain, ui8Context);

#else
	static int runing_flag = 0;
	IMG_UINT32 ui32Lines;
	IMG_UINT32 n = 0;
	IMG_UINT16 ui16VerticalTotal;
	IMG_UINT8 write_type = 0;

	IMG_UINT16 RGain = 0x400;
	IMG_UINT16 GGain = 0x400;
	IMG_UINT16 BGain = 0x400;

	static IMG_UINT16 bkRGain = 0x400;
	static IMG_UINT16 bkGGain = 0x400;
	static IMG_UINT16 bkBGain = 0x400;

	IMG_UINT8 wb_ui8Regs[18] = 
	{
		0x50, 0x0c, 0x04,
		0x50, 0x0d, 0x00, // long r gain

		0x50, 0x0e, 0x04,
		0x50, 0x0f, 0x00, // long g gain

		0x50, 0x10, 0x04,
		0x50, 0x11, 0x00, // long b gain
	};

//LOG_DEBUG("#########################flGain=%f##ui32Exposure=%d###########################\n", flGain, ui32Exposure);
	IMG_UINT8 ui8Regs[] =
	{
		0x32, 0x08, 0x00, // GROUP_ACCESS start hold of group 0
		
		0x35, 0x00, 0x00, // Exposure register
		0x35, 0x01, 0x2E, //
		0x35, 0x02, 0x80, //

		0x35, 0x07, 0x00, // Gain register
		0x35, 0x08, 0x00, //
		0x35, 0x09, 0x00, //

		0x32, 0x08, 0x10, // end hold of group 0 register writes
		0x32, 0x0b, 0x00, // set quick manual mode
		0x32, 0x08, 0xe0, // quick launch group 0
	};

	IMG_UINT8 ui8fpsRegs[] =
	{
		0x32, 0x08, 0x00, // GROUP_ACCESS start hold of group 0

		0x38, 0x0E, 0x00, // Exposure register
		0x38, 0x0F, 0x00, //

		0x32, 0x08, 0x10, // end hold of group 0 register writes
		0x32, 0x0b, 0x00, // set quick manual mode
		0x32, 0x08, 0xe0, // quick launch group 0
	};

	OV4689CAM_STRUCT *psCam = container_of(hHandle, OV4689CAM_STRUCT, sFuncs);

	if (!psCam->psSensorPhy)
	{
		LOG_ERROR("sensor not initialised\n");
		runing_flag = 0;
		return IMG_ERROR_NOT_INITIALISED;
	}
	if (runing_flag == 1)
	{
		return IMG_SUCCESS;
	}

	//01.Exposure part
	//
	if(ui32Exposure > aec_max_exporsure)
	{
        ui32Exposure = aec_max_exporsure;
	}
	psCam->ui32Exposure = ui32Exposure;

	ui32Lines = psCam->ui32Exposure / psCam->currMode.dbExposureMin;

	if (ui32Lines > s_ui16VerticalTotal - 4)
	{
        LOG_DEBUG("ui32Lines %d, s_ui16VerticalTotal %d\n", ui32Lines, s_ui16VerticalTotal);
        ui16VerticalTotal = ui32Lines + 4;
    //    ui16VerticalTotal = psCam->currMode.ui16VerticalTotal * psCam->fixedFps / fps;
        ui8fpsRegs[5] = (ui16VerticalTotal >> 8);
        ui8fpsRegs[8] = (ui16VerticalTotal & 0xff);
        ov4689_i2c_write8(psCam->i2c, ui8fpsRegs, sizeof(ui8fpsRegs));
        psCam->fCurrentFps = psCam->currMode.ui16VerticalTotal * psCam->fixedFps / ui16VerticalTotal;
	}

	ui8Regs[5] = (ui32Lines >> 12) & 0xf;
	ui8Regs[8] = (ui32Lines >> 4) & 0xff;
	ui8Regs[11] = (ui32Lines << 4) & 0xff;
	LOG_DEBUG("Exposure Val %f\n",(psCam->ui32Exposure/psCam->currMode.dbExposureMin));

	//02.Gain part
	//
 //flGain >= 11.0)
{
    if (1) // fabs(flGain - psCam->flGain) >= 0.0625)
    {
    	psCam->flGain = flGain;
    }
    else
    {
        flGain = psCam->flGain;
    }
}
#if 1
	if (flGain > aec_min_gain)
	{
		n = (IMG_UINT32)floor((flGain - aec_min_gain) * aec_max_gain);
	}
	n = IMG_MIN_INT(n, sizeof(aec_long_gain_val) / sizeof(IMG_UINT16) -1);

	ui8Regs[14] = 0;
	ui8Regs[17] = (aec_long_gain_val[n] >> 8) & 0xff;
	ui8Regs[20] = aec_long_gain_val[n] & 0xff;
	//LOG_DEBUG("gainReg %x, %x\n", ui8Regs[17], ui8Regs[20]);
	//printf("===>ui32Exposure %d gain %f n=%u from %lf\n", ui32Exposure, flGain, n, (flGain - aec_min_gain) * aec_max_gain);
#else
	if (flGain <= 1.0) 
	{
		// take care of possible underflow
		// unlikely to happen because minimum gain is 1.0
		n = 0;
	} 
	else 
	{
		n = (unsigned int)floor((flGain - 1.0) * 16); // always >=0
	}
	
	if (n >= sizeof(asGainSettings) / sizeof(IMG_UINT16))
	{
		n = sizeof(asGainSettings) / sizeof(IMG_UINT16) - 1;
	}	
	ui8Regs[14] = 0;//asGainSettings[n] >> 16; 
	ui8Regs[17] = (asGainSettings[n] >> 8) & 0xff;
	ui8Regs[20] = asGainSettings[n] & 0xff;
	LOG_DEBUG("gain n=%u from %lf\n", n, (flGain-1)*16);
#endif

    if (flGain > aec_max_gain)
	{
		RGain = 0x400 + 0x400*0.025*(flGain - aec_max_gain)/(wb_gain_aec_max_gain - aec_max_gain);
		GGain = RGain;
		BGain = RGain;
	}
	else
	{
		RGain = 0x400;
		GGain = RGain;
		BGain = RGain;
	}

	//
	if ((g_OV4689_ui8LinesValHiBackup == ui8Regs[5]) &&
		(g_OV4689_ui8LinesValMidBackup == ui8Regs[8]) &&
		(g_OV4689_ui8LinesValLoBackup == ui8Regs[11]) &&
		(g_OV4689_ui8GainValHiBackup == ui8Regs[14]) &&	
		(g_OV4689_ui8GainValMidBackup == ui8Regs[17]) &&
		(g_OV4689_ui8GainValLoBackup == ui8Regs[20]) ) 
	{
		//printf("The same gain and exposure as last.\n");
		//printf("ui32Lines=%d, flGain=%f\n", ui32Lines, flGain);
#ifndef ISP_VISION_LIVE
		runing_flag = 0;
		return IMG_SUCCESS;
#endif
	}
	
#if 1
if (psCam->flGain >= 11.0)
{
    if ((g_OV4689_ui8GainValMidBackup != ui8Regs[14]) || (g_OV4689_ui8GainValLoBackup != ui8Regs[17]) )
    {
        write_type |= 0x01;
        g_OV4689_ui8GainValMidBackup = ui8Regs[14];
        g_OV4689_ui8GainValLoBackup = ui8Regs[17];

    }

    if (g_OV4689_ui8LinesValHiBackup != ui8Regs[5] || 
        g_OV4689_ui8LinesValMidBackup != ui8Regs[8] || 
        g_OV4689_ui8LinesValLoBackup != ui8Regs[11])
    {
        write_type |= 0x02;        
        g_OV4689_ui8LinesValHiBackup = ui8Regs[5];
        g_OV4689_ui8LinesValMidBackup = ui8Regs[8];
        g_OV4689_ui8LinesValLoBackup = ui8Regs[11];
    }

	//04.set to sensor
	//
	
    switch (write_type)
    {
    case 1:
        ov4689_i2c_write8(psCam->i2c, ui8Regs + 12, 9);
        break;
    case 2:
        ov4689_i2c_write8(psCam->i2c, ui8Regs + 3, 9);
        break;
    case 3:
        ov4689_i2c_write8(psCam->i2c, ui8Regs, sizeof(ui8Regs));
        break;
    }
}
else
{
    ov4689_i2c_write8(psCam->i2c, ui8Regs, sizeof(ui8Regs));

    g_OV4689_ui8GainValMidBackup = ui8Regs[14];
    g_OV4689_ui8GainValLoBackup = ui8Regs[17];
    
    g_OV4689_ui8LinesValHiBackup = ui8Regs[5];
    g_OV4689_ui8LinesValMidBackup = ui8Regs[8];
    g_OV4689_ui8LinesValLoBackup = ui8Regs[11];

}
#else
    
    ov4689_i2c_write8(psCam->i2c, ui8Regs, sizeof(ui8Regs));

    g_OV4689_ui8GainValMidBackup = ui8Regs[14];
    g_OV4689_ui8GainValLoBackup = ui8Regs[17];
    
    g_OV4689_ui8LinesValHiBackup = ui8Regs[5];
    g_OV4689_ui8LinesValMidBackup = ui8Regs[8];
    g_OV4689_ui8LinesValLoBackup = ui8Regs[11];

#endif

#endif

	if (bkRGain != RGain || bkGGain != GGain || bkBGain != BGain)
	{
		wb_ui8Regs[2] = (RGain>>8); wb_ui8Regs[5] = (RGain&0xFF);
		wb_ui8Regs[8] = (GGain>>8); wb_ui8Regs[11] = (GGain&0xFF);
		wb_ui8Regs[14] = (BGain>>8); wb_ui8Regs[17] = (BGain&0xFF);

		bkRGain = RGain;
		bkGGain = GGain;
		bkBGain = BGain;
		printf("===>RGain %x, GGain %x, BGain %x\n", RGain, GGain, BGain);
		ov4689_i2c_write8(psCam->i2c, wb_ui8Regs, sizeof(wb_ui8Regs));
		//return IMG_SUCCESS;
	}

#ifdef ISP_VISION_LIVE
    s_flGain = psCam->flGain;
    s_ui32Exposure = psCam->ui32Exposure;
#endif
    runing_flag = 0;
    return IMG_SUCCESS;
}

static IMG_RESULT OV4689_GetFixedFPS(SENSOR_HANDLE hHandle, int *pfixed)
{
	if (pfixed != NULL)
	{
		OV4689CAM_STRUCT *psCam = container_of(hHandle, OV4689CAM_STRUCT, sFuncs);

		*pfixed = (int)psCam->fixedFps;
	}

	return IMG_SUCCESS;
}

static IMG_RESULT OV4689_SetFPS(SENSOR_HANDLE hHandle, double fps)
{
    OV4689CAM_STRUCT *psCam = container_of(hHandle, OV4689CAM_STRUCT, sFuncs);
    IMG_UINT8 aui8Regs[3];
    double  realFps = 0.0;
    unsigned int framlines = 0;

    if(fps > psCam->fixedFps)
    {
        realFps =  psCam->fixedFps;
    }
    else
    {
        realFps = fps;
    }

    if(psCam->fCurrentFps == fps)
    {
        return IMG_SUCCESS;
    }
    framlines = (1 * 1000 * 1000) / (realFps * psCam->currMode.ui32ExposureMin);

    aui8Regs[0] = 0x38;
    aui8Regs[1] = 0x0e;
    aui8Regs[2] = ((framlines >> 8) & 0x7f);

    ov4689_i2c_write8(psCam->i2c, aui8Regs, 3);
   // ov4689_i2c_read8(psCam->i2c, 0x380e,  &aui8Regs[2]);

    aui8Regs[0] = 0x38;
    aui8Regs[1] = 0x0f;
    aui8Regs[2] = framlines & 0xff;

    ov4689_i2c_write8(psCam->i2c, aui8Regs, 3);
    //ov4689_i2c_read8(psCam->i2c, 0x380f,  &aui8Regs[2]);

    psCam->fCurrentFps = realFps;
    s_ui16VerticalTotal = framlines;

	return IMG_SUCCESS;
}

#endif //INFOTM_ISP

