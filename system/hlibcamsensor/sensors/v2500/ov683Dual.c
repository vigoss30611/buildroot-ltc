...
static IMG_UINT8* OV683Dual_read_sensor_calibration_version(SENSOR_HANDLE hHandle, int sensor_index/*0 or 1*/, IMG_UINT16* otp_calibration_version);
static IMG_UINT8* OV683Dual_read_sensor_calibration_data(SENSOR_HANDLE hHandle, int sensor_index/*0 or 1*/, IMG_FLOAT awb_convert_gain, IMG_UINT16* otp_calibration_version);
static IMG_RESULT OV683Dual_update_sensor_wb_gain(SENSOR_HANDLE hHandle, IMG_FLOAT awb_convert_gain);
    ...
    unsigned int len, char slave_addr)
{
	...
	// switch back  to ov683 i2c channel
...
static IMG_UINT8 sensor_i2c_addr[2] = {(0x20 >> 1), (0x6c >> 1)}; // left, right
...
static IMG_UINT8* OV683Dual_read_sensor_calibration_data(SENSOR_HANDLE hHandle, int sensor_index/*0 or 1*/, IMG_FLOAT awb_convert_gain, IMG_UINT16* otp_calibration_version)
{
    ...
    if ((sensor_index == 0 && sensor_inited_flag_L) || (sensor_index == 1 && sensor_inited_flag_R))
	{
	...
	{
		OV683Dual_update_sensor_wb_gain(hHandle, awb_convert_gain);
	}
...
{
	OV683DualCAM_STRUCT *psCam = container_of(hHandle, OV683DualCAM_STRUCT, sFuncs);
	IMG_UINT16 RGain_L = 0x400, GGain_L = 0x400, BGain_L = 0x400, RGain_R = 0x400, GGain_R = 0x400, BGain_R = 0x400, BaseGain;
	IMG_UINT8 ui8Regs_L[18/**3*/] = 
	{
		0x50, 0x0c, 0x04,
...
*/
	...
	{
		GGain_R = (0x400*sensor_calibration_data[0][1]*awb_convert_gain)/(sensor_calibration_data[1][1]);
	}
	else
	{
		GGain_L = (0x400*sensor_calibration_data[1][1]*awb_convert_gain)/(sensor_calibration_data[0][1]);
	}

	// r gain
	if (sensor_calibration_data[0][0] > sensor_calibration_data[1][0])
	{
		RGain_R = (0x400*sensor_calibration_data[0][0]*awb_convert_gain)/(sensor_calibration_data[1][0]);
	}
	else
	{
		RGain_L = (0x400*sensor_calibration_data[1][0]*awb_convert_gain)/(sensor_calibration_data[0][0]);
	}		

	// b gain
	if (sensor_calibration_data[0][2] > sensor_calibration_data[1][2]) 
	{
		BGain_R = (0x400*sensor_calibration_data[0][2]*awb_convert_gain)/(sensor_calibration_data[1][2]);
	}
	else
	{
		BGain_L = (0x400*sensor_calibration_data[1][2]*awb_convert_gain)/(sensor_calibration_data[0][2]);
	}

#ifdef OTP_DBG_LOG
	printf("===>RGain_L = %x, GGain_L = %x, BGain_L = %x, RGain_R = %x, GGain_R = %x, BGain_R = %x\n", RGain_L, GGain_L, BGain_L, RGain_R, GGain_R, BGain_R);
...
///////////////////////
...
#else
    ui8Regs_L[2] = RGain_L>>8; ui8Regs_L[5] = RGain_L&0xFF;
    ui8Regs_L[8] = GGain_L>>8; ui8Regs_L[11] = GGain_L&0xFF;
    ui8Regs_L[14] = BGain_L>>8; ui8Regs_L[17] = BGain_L&0xFF;

    ui8Regs_R[2] = RGain_R>>8; ui8Regs_R[5] = RGain_R&0xFF;
    ui8Regs_R[8] = GGain_R>>8; ui8Regs_R[11] = GGain_R&0xFF;
    ui8Regs_R[14] = BGain_R>>8; ui8Regs_R[17] = BGain_R&0xFF;

#endif

	ov683Dual_sensor_i2c_write8(psCam->i2c, sensor_i2c_addr[0], ui8Regs_L, sizeof(ui8Regs_L));
	ov683Dual_sensor_i2c_write8(psCam->i2c, sensor_i2c_addr[1], ui8Regs_R, sizeof(ui8Regs_R));
    ...
    if (1)
    {
        ...
        {
            sensor_calibration_data[0][x] = (RGain_L*sensor_calibration_data[0][x])/0x400;
            sensor_calibration_data[1][x] = (RGain_R*sensor_calibration_data[1][x])/0x400;
        }

        for (x = 1; x < CALIBR_REG_CNT - 2; x += 3)
        {
            sensor_calibration_data[0][x] = (GGain_L*sensor_calibration_data[0][x])/0x400;
            sensor_calibration_data[1][x] = (GGain_R*sensor_calibration_data[1][x])/0x400;
        }
        
        for (x = 2; x < CALIBR_REG_CNT - 2; x += 3)
        {
            sensor_calibration_data[0][x] = (BGain_L*sensor_calibration_data[0][x])/0x400;
            sensor_calibration_data[1][x] = (BGain_R*sensor_calibration_data[1][x])/0x400;
        }
    }
	//ov683Dual_i2c_write8(psCam->i2c, ui8Regs, sizeof(ui8Regs));
#endif
