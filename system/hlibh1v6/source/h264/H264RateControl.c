...
i32 CalculateInitialQp(h264RateControl_s * rc, i32 bitRate)
{
    i32 qpHdr = H264Calculate(bitRate, rc->outRateDenom, rc->outRateNum);
    qpHdr = InitialQp(qpHdr, rc->mbPerPic*16*16);
    return qpHdr;
}
...
static i32 InitialQp(i32 bits, i32 pels)
{
    const i32 qp_tbl[2][9] = {
                    ...
                    {46, 43, 40, 37, 34, 31, 29, 26, 23}}; /* patch by infotm narrow init qp original is bellow*/
...
void H264BeforePicRc(h264RateControl_s * rc, u32 timeInc, u32 sliceType)
{
    ...
    if (rc->sliceTypeCur != ISLICE && rc->sliceTypeCur != ISLICES && rc->picSkip)
        PicSkip(rc);
