...
/* modify by panhn */
  //return soap->error = soap->fsend(soap, s, n);
    if(soap->fxmloutput)
    {
        if(soap->fxmloutput(soap, (char *)s, n,soap->cgihc)<0)
        {   
            soap->errnum = 32; //EPIPE
...
{ soap_int32 iadd = -1;
  ...
  if (!(host = sys_fun_gethostbyname((void*)addr)))
    soap->errnum = h_errno;
#else
  /* RM#2291: used sys_fun_gethostbyname, henry.li    2017/03/01*/
  //if (!(host = gethostbyname(addr)))
  if (!(host = sys_fun_gethostbyname(addr)))
    soap->errnum = h_errno;
#endif
#endif
  if (!host)
...
{ int n = (int)sizeof(soap->peer);
  ...
  { if (soap->accept_timeout || soap->send_timeout || soap->recv_timeout)
    { for (;;)
      { register int r;
        ...
        { soap_set_receiver_error(soap, "Timeout", "accept failed in soap_accept()", SOAP_TCP_ERROR);
		  ...
		  fprintf(stderr, "accept failed in soap_accept() 111111111111111111 \n");
        ...
        { r = soap->errnum;
          ...
          { soap_closesock(soap);
			...
			fprintf(stderr, "accept failed in soap_accept() 22222222222222222222222222222 \n");
    ...
    { DBGLOG(TEST,SOAP_MESSAGE(fdebug, "Accept failed from %s\n", soap->host));
	  ...
	  fprintf(stderr, "accept failed form %s \n", soap->host);
      soap->errnum = err;
...
soap_done(struct soap *soap)
{
  ...
  { struct soap_clist *p = soap->clist->next;
    if(soap->clist->ptr != NULL)
        {
            SOAP_FREE(soap, soap->clist->ptr);
        }
...
{ register int err;
 ...
 //   if (soap_valid_socket(soap->master) || soap_valid_socket(soap->socket)) /* standalone application */
  ...
  if ((err = soap->fposthdr(soap, "Server", "gSOAP/2.8"))  // yq 
  ...
  { if (q->expire && now > q->expire)
    ...
    { int flag;
      ...
      if (!flag)
      {
        ...
        struct hostent *hostent = sys_fun_gethostbyname((char*)domain);
        if (hostent)
...
void* soap_trace_malloc(struct soap* soap, size_t n, const char* file, int line) 
{
  ...
  if (!soap)
	...
	return soap_track_malloc(soap, file, line, n );
  ...
  { n += sizeof(short);
	...
	if (!(p = (char*)soap_track_malloc(soap, file, line, n + sizeof(void*) + sizeof(size_t) )))
...
void*
SOAP_FMAC2
soap_malloc_v2(struct soap *soap, size_t n)
{ register char *p;
  if (!n)
    return (void*)SOAP_NON_NULL;
  if (!soap)
...
{ register int i;
  ...
  fprintf(stderr, "========== soap_free_mht %s\n", __FILE__);
...
{ if (soap_check_state(soap))
  ...
  { register char **q;
    for (q = (char**)&soap->alist; *q; q = *(char***)q)
    {
      ...
      {
#ifdef SOAP_MEM_DEBUG
        fprintf(stderr, "soap_dealloc Data corruption in dynamic allocation (see logs)  print line %d \n", __LINE__);
#endif
...
{ register struct soap_clist *cp;
  ...
  for (q = (char**)&soap->alist; *q; q = *(char***)q)
  {
    ...
    {
#ifdef SOAP_MEM_DEBUG
      fprintf(stderr, "soap_delegate_deletion Data corruption in dynamic allocation (see logs)\n");
#endif
      DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Data corruption:\n"));
      DBGHEX(TEST, *q - 200, 200);
      DBGLOG(TEST, SOAP_MESSAGE(fdebug, "\n"));
...
if (soap_check_state(soap))
  ...
  { register struct soap_clist *cp = soap->clist->next;
	...
	/// yq change	
	if (soap->clist->ptr != NULL) // Modif YS 14/04/2003
			SOAP_FREE(soap, soap->clist->ptr);
  ...
  else
  {
    ...
    { if (!(soap->mode & SOAP_XML_CANONICAL)
       && soap_send(soap, soap->prolog ? soap->prolog : "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n")) {
	   	...
	   	fprintf(stderr, "============soap_element <?xml version=\"1.0\" encoding=\"UTF-8\"?> \n");
        return soap->error;
      }	
    }
...
soap_element_end_out(struct soap *soap, const char *tag)
{
  ...
  if (soap_send_raw(soap, "</", 2)
   || soap_send(soap, tag))
    return soap->error;
  soap->level--;	/* decrement level just before > */
  return soap_send_raw(soap, ">", 1);  //  return soap_send_raw(soap, ">", 1);  //
}
#endif
