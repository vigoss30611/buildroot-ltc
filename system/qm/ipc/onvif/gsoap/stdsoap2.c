...
{ if ((soap->mode & SOAP_IO) == SOAP_IO_STORE)
  ...
  if(soap->fxmloutput)
  {
	  soap->fxmloutput(soap, s, n,soap->cgihc);
  }
...
register int r;
  ...
  { if (soap->is->good())
  	{
		DBGLOG(TEST, SOAP_MESSAGE(fdebug, "soap->is->good\n"));
  ...
  { for (;;)
    { 
      ...
      if (soap->ssl)
      { 
		DBGLOG(TEST, SOAP_MESSAGE(fdebug, "soap->ssl\n"));
      ...
      { 
		DBGLOG(TEST, SOAP_MESSAGE(fdebug, "soap->bio\n"));
      ...
      { 
        ...
        { SOAP_SOCKLEN_T k = (SOAP_SOCKLEN_T)sizeof(soap->peer);
		  ...
		  DBGLOG(TEST, SOAP_MESSAGE(fdebug, "soap->omode & SOAP_IO_UDP\n"));
...
#endif
{
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "else soap->omode & SOAP_IO_UDP\n"));
...
#ifdef WITH_FASTCGI
  DBGLOG(TEST, SOAP_MESSAGE(fdebug, "WITH_FASTCGI\n"));

  return fread(s, 1, n, stdin);
#else
#ifdef UNDER_CE
  DBGLOG(TEST, SOAP_MESSAGE(fdebug, "UNDER_CE\n"));
...
#endif
#ifdef WIN32
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "WIN32\n"));

    r = _read(soap->recvfd, s, (unsigned int)n);
#else
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "else WIN32\n"));
...
zlib_again:
  ...
  { soap->bufidx = 0;
    ...
    ret = soap->buflen;
    DBGLOG(TEST, SOAP_MESSAGE(fdebug, "recv raw Read %u bytes from socket %d\n", (unsigned int)ret, soap->socket));
    DBGMSG(RECV, soap->buf, ret);
  }
#ifndef WITH_LEANER
...
#endif
  DBGLOG(TEST, SOAP_MESSAGE(fdebug, "$$$$$$$$$$ soap_recv_raw\n"));
...
#ifdef SOAP_DEBUG
  soap_set_test_logfile(soap, "/nfsroot/TEST.log");
  soap_set_sent_logfile(soap, "/nfsroot/SENT.log");
  soap_set_recv_logfile(soap, "/nfsroot/RECV.log");
...
{
    strncpy(soap->msgbuf, "Argument Value Invalid",64);
    /*    
    if (*soap->tag)
...
{ 
  ...
  if (!*c)
  { if (soap->version == 2)
      *c = "SOAP-ENV:Sender";//*c = "env:Sender";
    else
      *c = "SOAP-ENV:Client";
  }
...
{ 
    ...
    { 
        ...
        if (r > 0)
        {
            if (!(r & SOAP_TCP_SELECT_SND)|| ((r & SOAP_TCP_SELECT_RCV)
            && recv(soap->socket, soap->tmpbuf, 1, MSG_PEEK) < 0))
                r = 0;
        }
    }
