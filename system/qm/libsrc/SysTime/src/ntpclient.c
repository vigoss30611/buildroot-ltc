...
int rfc1305print(uint32_t *data, struct ntptime *arrival, unsigned long *pNtpTime);
...
int check_source(int data_len, struct sockaddr *sa_source, int sa_len, struct in_addr pHost)
{
	...
	if (1) {
		...
		if (sa_source->sa_family==AF_INET) {
            ...
            if(0 == strcmp(inet_ntoa(pHost), inet_ntoa(sa_in->sin_addr)))
                {
                //printf("%s:%d strcmp hostname:%s   success\n", __FILE__, __LINE__, inet_ntoa(pHost));
                }
            else
                {
                printf("%s:%d  strcmp failed, inet_ntoa:%s   hostname:%s\n", __FILE__, __LINE__, inet_ntoa(sa_in->sin_addr), inet_ntoa(pHost));
                return -1;
                }
		} else {
...
#undef Data
	...
	*pNtpTime = xmttime.coarse - JAN_1970;
...
/*	struct sockaddr_in *paddr;
	...
	nRet = getaddrinfo(hostname, "http", &hints, &res);
	if(nRet < 0) 
	{
	...
	else
	{
		...
		{
			paddr = (struct sockaddr_in *)cur->ai_addr;
			//printf("--------------------	%s\n", inet_ntop(AF_INET, &paddr->sin_addr, ipbuf, 16));
			p->s_addr = paddr->sin_addr.s_addr;
		}
	}*/

	if (0 == (p->s_addr = sys_fun_gethostbyname(hostname)))
	{
		herror(hostname);
        return -1;
	}
...
int setup_transmit(int usd, const char *host, short port, struct in_addr *pHost)
{
    int ret=0;
    ...
    *pHost = sa_dest.sin_addr;
    if(0 != ret)
    {
        //printf("%s:%d stuff_net_addr error, ret=%d\n", __FILE__, __LINE__, ret);
        return ret;
    }
...
void primary_loop(int usd, int num_probes, int interval, int goodness, unsigned long *pNtpTime, struct in_addr pHost)
{
	fd_set fds;
	...
	for (;;) {
		...
		if (pack_len<0) {
		    ...
		    //printf("%s:%d  recvfrom:%s", __FILE__, __LINE__, incoming);
			get_packet_timestamp(usd, &udp_arrival_ntp);
			int ret = check_source(pack_len, &sa_xmit, sa_xmit_len, pHost);
            if(0 != ret)
                {
                return ;
                }
			error = rfc1305print(incoming_word, &udp_arrival_ntp, pNtpTime);
...
//NTP对外接口传出相对秒数
int GetNtpTime(unsigned long *pNtpTime, const char * pHostname, unsigned short server_port)
{
	assert(NULL != pNtpTime && NULL != pHostname);
	*pNtpTime = 0;
	int usd;  /* socket */
	/* These parameters are settable from the command line
	   the initializations here provide default behavior */
	short int udp_local_port=0;   /* default of 0 means kernel chooses */
	int cycle_time=NTP_TIMEOUT_CNT;           /* seconds */
	...
	if (pHostname == NULL) 
	{
		printf("%s:%d hostname is not set\n", __FILE__, __LINE__);
	}
	...
	if (debug) {
		...
		"  -p local_port  %d\n"
		"  -s set_clock   %d\n",
		probe_count, debug, goodness, pHostname, cycle_time,
	...
	int ret = setup_receive(usd, INADDR_ANY, udp_local_port);
	if(0 != ret)
	{
		close(usd);
		return 0;
	...
	if(!server_port)
		ret = setup_transmit(usd, pHostname, NTP_PORT, &hostAddr);
	else
		ret = setup_transmit(usd, pHostname, server_port, &hostAddr);
	...
	primary_loop(usd, probe_count, cycle_time, goodness, pNtpTime, hostAddr);
