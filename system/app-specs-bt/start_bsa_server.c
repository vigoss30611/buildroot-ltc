...
{
	get_bt_chip_id();
	if (!strcmp(local_name, "BCM43430A1"))//bcm6212a
	{
		printf("--->start  bcm6212A \n");
		system("bsa_server -d /dev/ttyAMA0 -p /bt/bcm43438a1.hcd -u /tmp/ -k /config/bt/ble_local_keys -all=0 -r 10 &");
	} else if (!strcmp(local_name, "BCM43430B0")) {
		printf("--->start bcm6236  \n");
		system("bsa_server -d /dev/ttyAMA0 -p /bt/bcm43430b0.hcd -u /tmp/ -k /config/bt/ble_local_keys -all=0 -r 10 &");
	} else {
