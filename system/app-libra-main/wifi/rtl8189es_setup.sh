...
EOF
        ...
        if [ `wpa_cli -iwlan0 status | grep COMPLETED | wc -l` -eq 1 ]; then
...
EOF
	...
	hostapd_cli wps_config $ssid $key_mgmt CCMP $psk
	...
	udhcpd  $AP_PATH/udhcpd.conf wlan0 &
	return 0
fi
