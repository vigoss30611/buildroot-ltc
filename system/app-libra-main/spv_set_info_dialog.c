    ...
    switch(message) {
        case MSG_INITDIALOG:
        {
            ...
            switch(type){
		        ...
		        {
                    PInfotmTime pTime= (PInfotmTime)(((PDialogInfo)lParam)->infoData);
			        pTime->sel = 1;
                ...
                { 
		  	        PInfotmDate pDate= (PInfotmDate)(((PDialogInfo)lParam)->infoData);
			        pDate->sel = 1;
                ...
                {
		  	        PInfotmPlate pplate= (PInfotmPlate)(((PDialogInfo)lParam)->infoData);
			        pplate->sel = 1;
                    break;
                }
            ...
            g_content_font = CreateLogFont("ttf", "fb", "UTF-8",
                ...
                LARGE_SCREEN ? 88 : (46 * SCALE_FACTOR), 0);
        ...
        case MSG_KEYDOWN:
        {
            ...
            switch(wParam) {
                case SCANCODE_SPV_OK:
	            {
	    	        ...
	    	        int sel = -1;
	                if(INFO_DIALOG_SET_DATE == type){
		                PInfotmDate pdate = (PInfotmDate)(((PDialogInfo)GetWindowAdditionalData(hDlg))->infoData);
		                sel = pdate->sel;
                    }
	                else if(INFO_DIALOG_SET_TIME == type){
			            PInfotmTime ptime = (PInfotmTime)(((PDialogInfo)GetWindowAdditionalData(hDlg))->infoData);
			            sel = ptime->sel;
	    	        }else if(INFO_DIALOG_SET_PLATE == type){
			            PInfotmPlate pplate = (PInfotmPlate)(((PDialogInfo)GetWindowAdditionalData(hDlg))->infoData);
			            sel = pplate->sel;
                    }
