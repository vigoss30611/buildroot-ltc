...
static const u32 inputFormatMapping[19] =
{
...
static const u32 rgbMaskBits[19][3] =
{
  {  0,  0,  0 },
  {  0,  0,  0 },
  {  0,  0,  0 },
...
void EncPreProcess(asicData_s *asic, preProcess_s *preProcess)
{
  ...
  else if(preProcess->inputFormat == 15)
  {
    /* Input image position after crop and stabilization */
    tmp = preProcess->verOffsetSrc;
    tmp *= stride*2;
  ...
  else if(preProcess->inputFormat == 16)
  {
    /* Input image position after crop and stabilization */
    tmp = preProcess->verOffsetSrc;
    tmp *= stride*2;
  ...
  {
    i32 width8 = (width + 7) / 8 * 8;
    i32 height8 = (height + 7) / 8 * 8;
    ...
    regs->scaledWidthRatio =        (u32)(preProcess->scaledWidth << 16) / width + 1;      
    regs->scaledHeightRatio =        (u32)(preProcess->scaledHeight << 16) / height + 1;
