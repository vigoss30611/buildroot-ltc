...
static int bcm6212_load_driver(struct wifi_hal_t *wifi_hal, int mode)
{
	...
	if (mode == AP) {
		if (item_exist("wifi.model") && item_equal("wifi.model", "bcmdhd6210", 1))
			sprintf(bcm6212_module_arg, "firmware_path=/wifi/firmware/fw_bcm40181a2_apsta.bin nvram_path=/wifi/firmware/nvram_ap6476.txt iface_name=wlan0");
		else
		...
		if (item_exist("wifi.model") && item_equal("wifi.model", "bcmdhd6210", 1))
			sprintf(bcm6212_module_arg, "firmware_path=/wifi/firmware/fw_bcm40181a2.bin nvram_path=/wifi/firmware/nvram_ap6476.txt iface_name=wlan0");
		else
...
void bcm6212_init(struct wifi_hal_t *wifi_hal)
{
