...
static int rtl8189_driver_loaded(struct wifi_hal_t *wifi_hal, int mode) 
{
    FILE *proc;
...
static int rtl8189_load_driver(struct wifi_hal_t *wifi_hal, int mode)
{
	char cmd[256];
...
static int rtl8189_unload_driver(struct wifi_hal_t *wifi_hal, int mode)
{
	int count = 20;
...
{
	snprintf(wifi_hal->driver_module_name, sizeof(wifi_hal->driver_module_name), "8189es");
	snprintf(wifi_hal->driver_module_path, sizeof(wifi_hal->driver_module_path), "/wifi/8189es.ko");
	...
	wifi_hal->load_driver = rtl8189_load_driver;
	wifi_hal->unload_driver = rtl8189_unload_driver;
	wifi_hal->is_driver_loaded = rtl8189_driver_loaded;
