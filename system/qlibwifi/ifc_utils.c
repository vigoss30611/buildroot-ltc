...
int ifc_down(const char *name)
{
    ...
    if (DBG) printf("ifc_down(%s) = %d", name, ret);
    return ret;
}
...
int ifc_set_addr(const char *name, in_addr_t addr)
{
    ...
    if (DBG) printf("ifc_set_addr(%s, xx) = %d", name, ret);
    return ret;
}
...
int ifc_set_mask(const char *name, in_addr_t mask)
{
    ...
    if (DBG) printf("ifc_set_mask(%s, xx) = %d", name, ret);
    return ret;
}
...
int ifc_create_default_route(const char *name, in_addr_t gw)
{
    ...
    if (DBG) printf("ifc_create_default_route(%s, %d) = %d", name, gw, ret);
    return ret;
}
        ...
        const char *gw)
{
    ...
    if (ret != 0) {
        printf("getaddrinfo failed: invalid address %s\n", dst);
        return -EINVAL;
    }
    ...
    if (((addr_ai->ai_family == AF_INET6) && (prefix_length < 0 || prefix_length > 128)) ||
        ...
        printf("ifc_add_route: invalid prefix length \n");
    ...
    if (ret != 0) {
        printf("getaddrinfo failed: invalid gateway %s\n", gw);
        freeaddrinfo(addr_ai);
        return -EINVAL;
    }

    if (addr_ai->ai_family != gw_ai->ai_family) {
        printf("ifc_add_route: different address families: %s and %s\n", dst, gw);
        freeaddrinfo(addr_ai);
        freeaddrinfo(gw_ai);
        return -EINVAL;
    }
    ...
    if (addr_ai->ai_family == AF_INET6) {
        ...
        printf("ifc_add_route: getaddrinfo returned un supported address family %d\n",
                  addr_ai->ai_family);
        ret = -EAFNOSUPPORT;
    }
      ...
      struct in_addr gw)
{
    ...
    if (DBG) printf("ifc_add_ipv4_route(%s, xx, %d, xx) = %d\n", ifname, prefix_length, i);
    return i;
}
...
int ifc_add_route(const char *ifname, const char *dst, int prefix_length, const char *gw)
{
    ...
    if (DBG) printf("ifc_add_route(%s, %s, %d, %s) = %d \n", ifname, dst, prefix_length, gw, i);
    return i;
}
