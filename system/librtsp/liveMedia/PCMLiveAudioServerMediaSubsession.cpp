...
#include "PCMLiveAudioServerMediaSubsession.hh"
#include "PCMLiveAudioSource.hh"
#include "uLawAudioFilter.hh"
#include "SimpleRTPSink.hh"

PCMLiveAudioServerMediaSubsession* PCMLiveAudioServerMediaSubsession
::createNew(UsageEnvironment& env, char const* chanID, Boolean reuseFirstSource,
	    Boolean convertToULaw) {
  return new PCMLiveAudioServerMediaSubsession(env, chanID,
					       reuseFirstSource, convertToULaw);
}

PCMLiveAudioServerMediaSubsession
::PCMLiveAudioServerMediaSubsession(UsageEnvironment& env, char const* chanID,
				    Boolean reuseFirstSource, Boolean convertToULaw)
  : OnDemandServerMediaSubsession(env,reuseFirstSource),
    fConvertToULaw(convertToULaw) {
    if(chanID == NULL)
		fchanID = strDup("default_mic");
	else
		fchanID = strDup(chanID);
	
	env << "PCMLiveAudioMediaSubsession::fChanID:" << fchanID <<"\n";
}
...
char const* PCMLiveAudioServerMediaSubsession::getAuxSDPLine(RTPSink* rtpSink, FramedSource* inputSource) {
  static char auxSdp[32]={};
  sprintf(auxSdp,"a=ptime:%d\r\n",fFrameDuration / 2);
  return auxSdp;
}


FramedSource* PCMLiveAudioServerMediaSubsession
::createNewStreamSource(unsigned /*clientSessionId*/, unsigned& estBitrate) {
  FramedSource* resultSource = NULL;
  do {
    PCMLiveAudioSource* pcmSource = PCMLiveAudioSource::createNew(envir(), fchanID);
	...
	fFrameDuration = pcmSource->getGranularity();
...
RTPSink* PCMLiveAudioServerMediaSubsession
