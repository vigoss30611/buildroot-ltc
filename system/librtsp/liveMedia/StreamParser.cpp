...
void StreamParser::afterGettingBytes(void* clientData,
  ...
  if (parser != NULL) parser->afterGettingBytes1(numBytesRead, presentationTime,durationInMicroseconds);
}

void StreamParser::afterGettingBytes1(unsigned numBytesRead, struct timeval presentationTime,unsigned durationInMicroseconds) {
  ...
  fClientContinueFunc(fClientContinueClientData, ptr, numBytesRead, presentationTime,durationInMicroseconds);
}
...
void StreamParser::onInputClosure1() {
  if (!fHaveSeenEOF) {
    ...
    fHaveSeenEOF = True;
    afterGettingBytes1(0, fLastSeenPresentationTime,0);
  } else {
