...
int dramc_get_items(void)
{
	...
	dramc_debug("dram.rank_sel %d, dram.count %d, dram.reduce_flag %d\n", dram.rank_sel, dram.count, dram.reduce_flag);
        return 0;
}
...
int dramc_set_addr(void)
{
        ...
        if(dram.type == DDR3 || dram.type == DDR2) {
                if(((dram.width != IO_WIDTH8) && (dram.width != IO_WIDTH16)) || (dram.capacity < DRAM_64MB))
                        return 1;
                if(dram.capacity == DRAM_1GB) {		//if has col 11 ?, need redefined, need verify
                        dram.bank = 3;
                        dram.col = 10;
...
static const struct infotm_freq_2_param freq_2_param[] = {
        ...
        { 312, 0x2033 }, { 288, 0x102f }, { 264, 0x102b },
};
...
#define APB_CLK_BASE_NEW 0x2d00a360
        ...
        for(i = 0; i < FR_NUM; i++) {
                if(dram.freq >= freq_2_param[i].freq) {
						if (freq_2_param[i].freq == 312) {
							...
							spl_printf("WARNING: VPLL is not 312M, you may have risk in using DMA!\n");
							irf->module_set_clock(APB_CLK_BASE_NEW, PLL_E, 5); 
...
void DWC_umctl2_reg(void)
{
...
#ifdef	CONFIG_IMAPX_FPGATEST
	writel(0x00080030, ADDR_UMCTL2_RFSHTMG);
#else
	writel(tmp, ADDR_UMCTL2_RFSHTMG);
#endif

	writel((dram.type == DDR3) ? 0x00010150 : 0x000100a0, ADDR_UMCTL2_INIT0); /* time period(us) during memory power up, DDR3 at least 500us*/
	...
	tmp = (dram.type == DDR3 ? 0:5) << 16 | 0x4 << 12 | (dram.type == DDR3 ? 0xd : 0);	//set tmdr tmod 0x400d alin 0x04010
	...
	writel((dram.type == DDR3) ? 0:202, 	ADDR_UMCTL2_DRAMTMG7);
	...
	if(!dram.reduce_flag)
		...
		writel((0x1f00 | (dram.row + 0x5)), ADDR_UMCTL2_ADDRMAP0);
...
void ddr_phy_set(void)
{
	...
	tmp = 0x3 | (4 << 2) | (4 << 5) | (cl << 8) | (cl << 12) | (timing->tras << 16) | (timing->trrd << 21) | (timing->trc << 25); //0x2a8f6690 alin 0x2c906693-->tras=15
...
int ddr_training(void)
{
	...
	dqs_addr = 0x40000000 + (dram.size<<20) - 0x2000;
	if(!dramc_wake_up_flag && dram.rank_sel == 3){
		for(i=0; i<4; i++)
...
int dramc_init_one_type(void)
{
		...
		/*for apollo 1G init from 0x40000000, kernel user this*/
#if 0
	if((dram.size == 1024 || dram.size == 2048) && dram.rank_sel == 1){
		dramc_debug("capacity %dMB need cs0 remap cs1\n", dram.size);
 ...
 * V6.0  2G DDR ce0 OK								2014.07.08
 * V6.1  write latency bug fixed					2014.07.31
 * V6.2  half mode col addr bug fixed				2014.10.17
 * V7.0  DDR for q3, fpga is new					2015.01.04
 * V8.0  DDR init from 0x40000000	  				2015.01.15
*******************************************************/
int dramc_q3f_fpga_init(struct irom_export *irf, int wake_up)
{
        int i;

        spl_printf("DDR V8.0: running time: %s, for q3f fpga 0x40000000\n", __TIME__);
