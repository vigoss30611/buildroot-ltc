...
#include <asm-arm/arch-q3f/imapx_uart.h>
...
struct irom_export *irf = &irf_q3f;
...
extern int dramc_q3f_fpga_init(struct irom_export *irf, int);
...
#ifdef CONFIG_IIC
	board_init_i2c();
	uint8_t addr, buf[2]; //data width of ip620x reg is 16-bits: buf[0] refers to high 8-bits and buf[1] refers to low .
	addr = 0xf;
	iicreg_write(0, &addr, 0x01, buf, 0x00, 0x01);
	iicreg_read(0, buf, 0x02);
	irf->printf("reg 0x%x buf[0] is 0x%x, buf[1] is 0x%x \n", addr, buf[0], buf[1]);
