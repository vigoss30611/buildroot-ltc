...
#include <asm-arm/arch-imapx800/imapx_dramc.h>
...
static const struct infotm_freq_2_param freq_2_param[] = {
        ...
        { 444, 0x1049 }, { 420, 0x2045 }, { 396, 0x2041 }, { 372, 0x203d }, { 348, 0x2039 }, { 330, 0x2036 },
        { 312, 0x2033 }, { 288, 0x202f }, { 264, 0x202b },
};
...
void dramc_set_frequency(void)
{
		...
		for(i = 0; i < FR_NUM; i++) {
			if(dram.freq >= freq_2_param[i].freq) {
				if (freq_2_param[i].freq <= 420) {
					/* change bus6&apb to vpll */
					dramc_set_freqency_phy(freq_2_param[i].param, 0);
					irf->module_set_clock ( BUS6_CLK_BASE, PLL_V, 1);
					writel(0, BUS6_CLK_BASE + 0x4);
					irf->module_set_clock ( APB_CLK_BASE_NEW, PLL_V, 3 ); // set spi_clk from 27m to 43m, set pbus_clk to 118m, special --->99m
					writel(1, BUS6_CLK_BASE + 0x4);
				} else {
					/* normal mode */
...
void DWC_umctl2_reg(void)
{
	...
	}else if(dram.col == 10 && dram.type == LPDDR2S4 && dram.bank == 2){
		/*LPDDR2S4 4 bank
		  lpddr2s4 map(row13, bank4, col10)	ADDR_UMCTL2_ADDRMAP0 ~ ADDR_UMCTL2_ADDRMAP6
		  COL[9:1]->HIF[8:0],BANK[1:0]->HIF[10:9], ROW[12:0]->HIF[23:11], RANK[0]->HIF[24]*/
		writel((0x1f00 | (dram.row - 1 + (!dram.reduce_flag ? 0x6 : 0x5))), ADDR_UMCTL2_ADDRMAP0);	//rank defined
		writel(0x0f0606 + (!dram.reduce_flag ? 0x0101:0), 	ADDR_UMCTL2_ADDRMAP1);		//bank 0~1 always in HIF bit[9~11]
		if(!dram.reduce_flag){
			writel(0, 	ADDR_UMCTL2_ADDRMAP2);
			writel(0x0f000000, 	ADDR_UMCTL2_ADDRMAP3);
		}
	...
	writel(/*0x0f00c083*/0x0f004083, ADDR_UMCTL2_PCFGR_0);
	writel(0x1f004083, ADDR_UMCTL2_PCFGW_0);
	writel(/*0xff00c082*/0xff004082, ADDR_UMCTL2_PCFGR_1);
	writel(0xff004082, ADDR_UMCTL2_PCFGW_1);
	writel(/*0xff00c081*/0xff00c081, ADDR_UMCTL2_PCFGR_2);
...
void ddr_phy_set(void)
{
	...
	writel(0x0000000d,      ADDR_PHY_ZQ0CR1);       /*qaf impendance control and calibration */
	if(dramc_wake_up_flag) {
 ...
 * V7.1  LPDDR2S4 4 bank add						2015.11.04
 * V7.2  LPDDR2S4 4 bank bug fixed					2016.06.06
 * V8.0  Q3 ddr freq can not get 400M bug fixed				2016.06.06
*******************************************************/
int dramc_init(struct irom_export *irf, int wake_up)
{
        int i;

        dramc_debug("DDR V8.0: running time: %s, LPDDR2S4 func add!\n", __TIME__);
