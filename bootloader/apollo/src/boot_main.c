...
#include <asm-arm/arch-imapx800/imapx_uart.h>
...
struct irom_export *irf = &irf_4e5a75;
...
static int  mmc_read(char *buf, int start,int length, int extra)
{
	#ifdef CONFIG_BOARD_ENABLE_MMC_DRIVER
...
int  gen_flash_read(uint8_t *buf, long start, int length, int extra)
{
#ifdef CONFIG_BOARD_BOOT_DEV_FLASH
	return flash_read(buf, start, length, extra);
#else
	return irf->vs_read(buf, start, length, extra);
#endif
...
void dev_read_image(int dev_id, image_type_t image_num, int offset, int size, uint8_t *data)
{
	if (dev_id == DEV_IUS) {
		...
		gen_flash_read(data, offs_image[image_num] + offset, size, 0);
		//flash_read(data, offs_image[image_num] + offset, size, 0);
	} else {
...
void boot_main ( void )
{
	...
	writel ( 0x3f, 0x21E09C18 );
	writel ( 0x00, 0x21E093C0 );
	writel ( 0x00, 0x21E093C4 );
	writel ( 0x00, 0x21E093C8 );
