...
#include <asm-arm/arch-coronampw/ssp.h>
...
static void gdma_chan_sample_complete(uint32_t chan)
{
    unsigned int val, event = (1 << chan);

    while (1) {
		val = readl(__regs + GDMA_EIRS);
        if(val & event) {
			dma_debug("GDMA_EIRS = %x, chan event occur!\n", val);
            writel(event, __regs + GDMA_IC);
            break;
        }
		...
		const struct _xfer_spec *pxs, int cyc)
{
	...
	while (cyc--) {
		if (BRST_LEN(pxs->ccr) == 1) { 
			off += _emit_WFP(dry_run, &buf[off], SINGLE, pxs->peri_num);
			off += _emit_LDP(dry_run, &buf[off], SINGLE, pxs->peri_num);
			off += _emit_STP(dry_run, &buf[off], SINGLE, pxs->peri_num);//ALWAYS
			off += _emit_FLUSHP(dry_run, &buf[off], pxs->peri_num);
		} else {
			off += _emit_WFP(dry_run, &buf[off], BURST, pxs->peri_num);
			off += _emit_LDP(dry_run, &buf[off], BURST, pxs->peri_num);
			off += _emit_STP(dry_run, &buf[off], BURST, pxs->peri_num);
			off += _emit_FLUSHP(dry_run, &buf[off], pxs->peri_num);    
		}
	}
		...
		const struct _xfer_spec *pxs, int cyc)
{
	...
	while (cyc--) {
		if (BRST_LEN(pxs->ccr) == 1) {
			off += _emit_WFP(dry_run, &buf[off], SINGLE, pxs->peri_num);
			off += _emit_LDP(dry_run, &buf[off], SINGLE, pxs->peri_num);
			off += _emit_STP(dry_run, &buf[off], SINGLE, pxs->peri_num);
			off += _emit_FLUSHP(dry_run, &buf[off], pxs->peri_num);
		} else {
			off += _emit_WFP(dry_run, &buf[off], BURST, pxs->peri_num);
			off += _emit_LDP(dry_run, &buf[off], BURST, pxs->peri_num);
...
{
	unsigned int cyc, cycmax, szlp, szlpend, szbrst, off;
	unsigned long lcnt0, lcnt1, ljmp0, ljmp1;
...
{                                                                                                                                                                 
	...
	off+=_emit_FLUSHP(0,&buf[off],ev_num);
...
static void dma_sync_transfer_otf(struct gdma_addr *src, struct gdma_addr *dst, uint32_t len, struct gdma_reqcfg* p_req_cfg_rx, uint32_t rx_ev_num, uint8_t *rx_desc_addr, int rx_peri, uint32_t dctrl0, uint32_t dctrl1, uint32_t dctrl2, uint32_t fdctrl)                                                                  
{
	int dry_run, rx_off, rx_ccr;
	...
	dma_debug("%s:%d, len = %d, ev_num = %d, peri = %d, dctrl0 = 0x%x, dctrl1 = 0x%x,dctrl2 = 0x%x,fdctrl = 0x%x\n", __func__, __LINE__, len, rx_ev_num, rx_peri, dctrl0, dctrl1, dctrl2, fdctrl);

	writel(1 << rx_ev_num, __regs + GDMA_IE);  //enable event interrupt
	writel(0xffff, __regs + GDMA_IC);
	...
	/* init rx channel */
	rx_off+=_emit_FLUSHP(0,&rx_buf[rx_off],rx_ev_num);
	...
	gdma_chan_sample_complete(p_req_cfg_rx->port_num); //wait tx and rx channel transfer complete
...
static void dma_sync_transfer_with_cmd(struct gdma_addr *src, struct gdma_addr *dst, uint32_t len, 
        struct gdma_reqcfg* p_req_cfg_tx, uint32_t tx_ev_num, uint8_t *tx_desc_addr, int tx_peri, 
        struct gdma_reqcfg* p_req_cfg_rx, uint32_t rx_ev_num, uint8_t *rx_desc_addr, int rx_peri, uint8_t *ex, uint8_t ex_len)                                                                  
{
	...
	temp = dst->byte + 0x200000;
	memset(temp, 0, len+ex_len);
	memcpy(temp, ex, ex_len);

	dst->byte -= INVALID_DATA;
	...
	/* init tx channel */
	tx_off+=_emit_FLUSHP(0,&tx_buf[tx_off],tx_ev_num);
	tx_off+=_emit_MOV(0,&tx_buf[tx_off],CCR,tx_ccr);	
	tx_off+=_emit_MOV(0,&tx_buf[tx_off],SAR, temp);
	tx_off+=_emit_MOV(0,&tx_buf[tx_off],DAR,src->byte);        

	tx_xs.size = len + ex_len;
	tx_xs.peri_num = tx_ev_num;
	...
	//enable SSP
    writel(readl(SSP_CR1_0) | (1 << 1), SSP_CR1_0);

	/*
	for(i = 0; i < INVALID_DATA; i++) {
		while(!(readl(SSP_RIS_0) & (1 << 2)));
		readl(SSP_DR_0);
	}
	*/

	gdma_chan_sample_complete(p_req_cfg_tx->port_num); //wait tx and rx channel transfer complete
	gdma_chan_sample_complete(p_req_cfg_rx->port_num); //wait tx and rx channel transfer complete
...
void dma_peri_flush_fifo(struct gdma_addr *src_addr, struct gdma_addr *dst_addr)
{
    ...
    while(readl(SSP_SR_0) & SPI_RFIFO_NEMPTY) {
        gdma_transfer(src_addr, &temp, ONCE_READ_NUM, &req_cfg_rx, req_cfg_rx.port_num, desc_addr_rx, DEVTOMEM);
    } 
}
			...
			uint8_t *ex, uint8_t ex_len, uint32_t *data_len)
{
	...
	req_cfg_rx.brst_len = 0;
	for(i=0; i<ex_len; i++){
		gdma_transfer(&temp, src_addr, 1, &req_cfg_tx, req_cfg_tx.port_num, desc_addr_tx, MEMTODEV);	
		temp.byte += 1;
	}
	...
	gdma_transfer(src_addr, &temp, INVALID_DATA, &req_cfg_rx, req_cfg_rx.port_num, desc_addr_rx, MEMTODEV);

	req_cfg_rx.brst_len = 4;
	gdma_transfer(src_addr, dst_addr, (ex_len - INVALID_DATA), &req_cfg_rx, req_cfg_rx.port_num, desc_addr_rx, MEMTODEV);
	dst_addr->byte += (ex_len - INVALID_DATA);
	*data_len -= (ex_len - INVALID_DATA);
}

int dma_peri_transfer_otf(struct gdma_addr *src_addr, struct gdma_addr *dst_addr, uint32_t data_len, uint32_t dctrl0, uint32_t dctrl1, uint32_t dctrl2, uint32_t fdctrl, uint8_t direction)
{
	unsigned long  len = 0;

	if(direction) {			//receive
		dma_debug("dma receive\n");
		req_cfg_rx.brst_len = 16;
		req_cfg_rx.brst_size = 0; //1 bytes
		req_cfg_rx.dst_inc = 1;
		req_cfg_rx.src_inc = 0;
		req_cfg_rx.swap = SWAP_NO;

		dma_sync_transfer_otf(src_addr, dst_addr, data_len, &req_cfg_rx, req_cfg_rx.port_num, desc_addr_rx, DEVTOMEM, dctrl0, dctrl1, dctrl2, fdctrl);
	}
	else {						   //send
		dma_debug("dma send\n");
		req_cfg_tx.brst_len = 1;
		req_cfg_tx.brst_size = 0; //1 bytes
		req_cfg_tx.dst_inc = 0;
		req_cfg_tx.src_inc = 1;
		req_cfg_tx.swap = SWAP_NO;

		dma_sync_transfer_otf(src_addr, dst_addr, data_len, &req_cfg_tx, req_cfg_tx.port_num, desc_addr_tx, MEMTODEV, dctrl0, dctrl1, dctrl2, fdctrl);
	}
...
int dma_peri_transfer_with_cmd(struct gdma_addr *src_addr, struct gdma_addr *dst_addr, uint32_t data_len, uint8_t *ex, uint8_t exlen)
{
	unsigned long  len = 0;
	...
	dma_sync_transfer_with_cmd(src_addr, dst_addr, data_len, &req_cfg_tx, req_cfg_tx.port_num, desc_addr_tx, MEMTODEV, &req_cfg_rx, req_cfg_rx.port_num, desc_addr_rx, DEVTOMEM, ex, exlen);
...
int dma_peri_transfer(struct gdma_addr *src_addr, struct gdma_addr *dst_addr, uint32_t data_len)
{
	...
	if(data_len > ONCE_READ_NUM)	{ 		
		...
		while(i--){
			dma_sync_transfer(src_addr, dst_addr, ONCE_READ_NUM, &req_cfg_tx, req_cfg_tx.port_num, desc_addr_tx, MEMTODEV, &req_cfg_rx, req_cfg_rx.port_num, desc_addr_rx, DEVTOMEM);
			dst_addr->byte += ONCE_READ_NUM;
			len += ONCE_READ_NUM;
		}
		
		dma_sync_transfer(src_addr, dst_addr, j, &req_cfg_tx, req_cfg_tx.port_num, desc_addr_tx, MEMTODEV, &req_cfg_rx, req_cfg_rx.port_num, desc_addr_rx, DEVTOMEM);
		dst_addr->byte += j;
		len += j;
	}else{
		req_cfg_tx.brst_len = 4;
		...
		dma_sync_transfer(src_addr, dst_addr, data_len, &req_cfg_tx, req_cfg_tx.port_num, desc_addr_tx, DEVTOMEM, &req_cfg_rx, req_cfg_rx.port_num, desc_addr_rx, MEMTODEV);
		dst_addr->byte += data_len;
		len += data_len;
	}
