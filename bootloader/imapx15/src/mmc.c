...
static ulong mmc_bread(int dev_num, ulong start, lbaint_t blkcnt, void *dst)
{
	...
	if ((start + blkcnt) > mmc->block_dev.lba) {
		printf("MMC0: block number 0x%lx exceeds max(0x%lx)\n",
			start + blkcnt, mmc->block_dev.lba);
		return 0;
	}
...
int vs_read_mmc0(uint8_t *buf, int start,int length, int extra)
{
  ...
  align  = mmc0_vs_align();
	...
	if(prelen) {
		...
		ret = mmc0_vs_read(tmp, start & ~(align - 1), align, extra);
		if(ret < 0) {
			read = ret;
			goto __read_end__;
		}
	...
	length -= postlen;
	if(length) {
		ret = mmc0_vs_read(buf, start, length, extra);
		if(ret < 0) {
			read = ret;
			goto __read_end__;
		}
	...
	if(postlen) {
		ret = mmc0_vs_read(tmp, start, align, extra);
		if(ret < 0) {
			read = ret;
			goto __read_end__;
		}
