...
{
    unsigned int tmp;
    tmp = readl(ADDR_PHY_DLLGCR);
    writel(tmp | (1<<23), ADDR_PHY_DLLGCR);// set bps200 for 200MHZ
    writel(0x20001,         ADDR_PHY_PIR);        /*Set Dll Bypass */
    while((readl(ADDR_PHY_PGSR) & 0x1) != 0x1);     /* Wait cmd complete */
}

void dramc_set_dlloff(void)
{
    unsigned int tmp;
    do {
    tmp = readl(ADDR_UMCTL2_MRSTAT);}
    while(tmp&1);
    
    writel((dram.rtt | dram.driver|1), ADDR_UMCTL2_MRCTRL1);          /* RTT, DIC */
    writel((1<<12 | 0xf<<4), ADDR_UMCTL2_MRCTRL0);          /* RTT, DIC */
...
int dramc_get_items(void)
{
        ...
        uint32_t rank[][3] = {
            ...
            { 3, 3, 3 },        /* total width 32bit */	//1G 32bit width need verify?
        ...
        item_integer_parser("dram.bypass", &dram.dll_bypass);
	...
	dramc_debug("dram.reduce_flag %d, :dram.rank_sel %d, dram.count %d, dram.reduce_flag %d\n",dram.reduce_flag, dram.rank_sel, dram.count, dram.reduce_flag);
        return 0;
}
...
static const struct infotm_freq_2_param freq_2_param[] = {
        { 600, 0x0031, 1}, { 576, 0x002f, 1}, { 552, 0x002d, 1}, { 528, 0x002b, 1}, { 504, 0x0029, 1}, { 468, 0x104d, 1},
        { 444, 0x1049, 1}, { 420, 0x1045, 1}, { 400, 0x0031, 2}, { 396, 0x1041, 1}, {378, 0x103e, 1}, { 372, 0x103d, 1}, { 348, 0x1039, 1},
	{ 330, 0x1036, 1}, { 312, 0x1033, 1}, { 288, 0x102f, 1}, { 264, 0x102b, 1},
};

void dramc_set_freqency_phy(uint32_t pll_p, int phy_div)
{
...
void dramc_set_frequency(void)
{
        ...
        for(i = 0; i < FR_NUM; i++) {
                if(dram.freq >= freq_2_param[i].freq) {
                        dramc_set_freqency_phy(freq_2_param[i].param, freq_2_param[i].div);
                        break;
                }
        }
...
static void dramc_set_timing(void)
{
    ...
    if (dram.type == DDR2)
    {
        ...
        timing->trfc = division(trfc_param_ddr2[dram.capacity] * dram.freq, 1000000);
    }
    else
    {
...
void module_reset(void)
{
	...
	if(!dramc_wake_up_flag) {
		//writel(readl(RTC_GPDIR) & ~0x1,	0x2d009c4c);
...
#if 1
    	/* add ddr map bus<--->port
	bus3 :  fodet/ispost                                -> port 0
	cpu                                                 -> port 1
	bus1 :  isp/camif/csi/ids                           -> port 2
	bus4 :  venc                                        -> port 3
	bus6 :  usbh/usbo/sd1/sd2/sd0/gdma/eth/crypto       -> port 4
	*/
    ...
    writel(0x00005070, ADDR_UMCTL2_PCFGR_1); //bit[]--,bit[]--,bit[]--
    writel(0x00005070, ADDR_UMCTL2_PCFGW_1); //bit[]--,bit[]--,bit[]--d
    ...
    writel(0x4, EMIF_SYSM_ADDR_AWURGENT_REG);
    writel(0x00004080, ADDR_UMCTL2_PCFGR_2); //bit[]--,bit[]--,bit[]--
    writel(0x00006080, ADDR_UMCTL2_PCFGW_2); //bit[]--,bit[]--,bit[]--
...
void DWC_umctl2_reg(void)
{
	...
	tmp = (timing->tcwl + MEMC_FREQ_RATIO - 1)<< 24 | cl << 16 | (cl + 6 -timing->tcwl + MEMC_FREQ_RATIO -1 + ((dram.type == DDR3 || dram.type == DDR2)? 0 : 3)) << 8 | (timing->tcwl + 4 + 4);	//set twrt cl?=rl wl 0x506070d alin 0x0506070d
	writel(tmp/MEMC_FREQ_RATIO, ADDR_UMCTL2_DRAMTMG2);

	tmp = (dram.type == DDR3 ? 0:((dram.type == LPDDR3) ? 10:5)) << 20 | 0x4/MEMC_FREQ_RATIO << 12 | (dram.type == DDR3 ? (0xd+MEMC_FREQ_RATIO-1)/MEMC_FREQ_RATIO : 0);	//set tmdr tmod 0x400d alin 0x04010
...
void ddr_phy_set(void)
{
    ...
    else if (dram.type == DDR2 && dram.freq < 410)
    {
	...
	//	writel(0x00000009,      ADDR_PHY_PIR);          /* impendance calibrate */
		writel(0x00000001,      ADDR_PHY_PIR);  
...
int ddr_training(void)
{
	...
	writel(0x181 | (dram.dll_bypass?0x20000:0), 		ADDR_PHY_PIR);		/*PHY Initialization: Read DQS Training & Read Valid Training*/
...
int dramc_init_memory(void)
{
    ...
    {
        dramc_set_bypass();
        spl_printf("dram set dll bypass mode \n");
...
int dramc_init_one_type(void)
{
	 ...
	 //do_mem_mtest();
	/*ddr test*/
#define DDR_CMP_SIZE (63*0x100000/4)
...
static const uint32_t dram_param[2][9][2] = {
    ...
    {   //q3f
        { 1, 64 },             // count = 1, capacity = 64MB, size = 64MB
        { 2, 64 },             // count = 2, capacity = 64MB, size = 128MB 		
        { 1, 128 },             // count = 1, capacity = 128MB, size = 128MB		
        { 4, 64 },             // count = 4, capacity = 64MB, size = 256MB,  			
        { 2, 128 },             // count = 2, capacity = 128MB, size = 256MB		
        { 1, 256 },             // count = 1, capacity = 256MB, size = 256MB 
        { 2, 256 },             // count = 2, capacity = 256MB, size = 512MB			
        { 4, 128 },             // count = 4, capacity = 128MB, size = 512MB
        { 4, 256 },             // count = 4, capacity = 256MB, size = 1GB
    },
...
*  V10.0 q3f ddr2 support 						2016.6.21
*******************************************************/
int dramc_init(struct irom_export *irf, int wake_up)
{
        int i;

        dramc_debug("DDR V10.0: running time: %s, for q3f func add!\n", __TIME__);
