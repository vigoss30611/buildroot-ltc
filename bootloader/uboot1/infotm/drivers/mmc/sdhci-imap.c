...
static int imap_write_block(struct imap_host *host, void *_buf, int  bytes)
{
	...
	while(1) {
		intstatus = imap_mmc_readl(host,IMAP_NORINTSTS);
		imap_mmc_writel(host, IMAP_NORINTSTS,intstatus);
		if(imap_mmc_readl(host, IMAP_PRNSTS) & (1 << 10))
...
static int imap_mmc_initialize(void)
{
	imap_mmc_initialize_host(0);
	imap_mmc_initialize_host(1);
	imap_mmc_initialize_host(2);
	return 0;
}

int board_mmc_init(void)
{
	return imap_mmc_initialize();
}
