...
int32_t  flash_qe_enable_w25q128(void);
int32_t  flash_qe_disable_w25q128(void);
int		 flash_wait_device_status_w25q128(int cmd,int status);
int32_t  flash_qe_enable_mx25l256(void);
int32_t  flash_qe_disable_mx25l256(void);
int		 flash_wait_device_status_mx25l256(int cmd,int status);
...
const flash_dev_info_t flash_dev_list[] = {
	{
		.jedec = MX25L25635E,
		.flash_qe_enable_func = flash_qe_enable_mx25l256,
		.flash_qe_disable_func = flash_qe_disable_mx25l256,
		.flash_wait_device_status_func = flash_wait_device_status_mx25l256,
	},

	{
		.jedec = W25Q128,
		.flash_qe_enable_func = flash_qe_enable_w25q128,
		.flash_qe_disable_func = flash_qe_disable_w25q128,
		.flash_wait_device_status_func = flash_wait_device_status_w25q128,
	},
...
int32_t ssp_manager_set_flash_type(uint32_t flash_type)
{
 ...
 void spi_set_bytes(int bytes) {
    spi_manager_g.flash_sector_size = bytes;
    spi_debug("spi: bytes set to: %d\n", bytes);
}
	...
	if(spi_manager_g.flash_type == SPI_NAND_FLASH) {
		...
		for(i = 0; i < ARRAY_SIZE(flash_dev_list); i++) {
			if(flash_dev_list[i].jedec == jedec) {
				spi_manager_g.flash_device = flash_dev_list[i].dev_id;
				flash_qe_enable = flash_dev_list[i].flash_qe_enable_func;
				flash_qe_disable = flash_dev_list[i].flash_qe_disable_func;
				flash_wait_device_status = flash_dev_list[i].flash_wait_device_status_func;
				spi_debug("flash device found! dev_id = 0x%x\n", spi_manager_g.flash_device);
				break;
			}
		}
		if(i == ARRAY_SIZE(flash_dev_list))
		{ 
			printf("flash device not found! use default here!\n");
			spi_manager_g.flash_device = flash_dev_list[0].dev_id;
			flash_qe_enable = flash_dev_list[0].flash_qe_enable_func;
			flash_qe_disable = flash_dev_list[0].flash_qe_disable_func;
			flash_wait_device_status = flash_dev_list[0].flash_wait_device_status_func;
			//while(1);
		}
	}
...
int nand_read_page_to_cache(int page_offs,int line_mode)
{
	...
	//cmd+addr: 0x13+24bit
	//blk_addr	= chufa(page_offs,spi_manager_g.nd_fls_array_org.pages_per_blk);
	//page_addr	= quyu(page_offs,spi_manager_g.nd_fls_array_org.pages_per_blk);
...
int flash_vs_read(uint8_t *buf, loff_t start, int length, int extra)
{
	...
	sectors = ((uint32_t)length) / FLASH_SECTOR_SIZE;
	rest_in_sector = ((uint32_t)length) - (sectors * FLASH_SECTOR_SIZE);
...
int flash_vs_write(uint8_t *buf, loff_t offs, int len, int extra)
{
	...
	switch(line_mode) {
		...
		case 4:
			...
			else if(JEDEC_MFR(jedec) == MFR_WINBOND) {
...
int flash_vs_erase(loff_t offs, uint64_t len)
{
	...
	//jedec = flash_read_id();
	if((jedec == MX25L25635E && offs == 0x0 && len >= MX25L256_SIZE) || (jedec == W25Q128 && offs == 0x0 && len >= W25Q128_SIZE) || (jedec == MX25L12835F && offs == 0x0 && len >= MX25L128_SIZE) ) {
	...
	memset( &spi_trans, 0x0 , sizeof(spi_protl_set) );
	spi_trans.commands[0] = FLASH_CMD_ERAS_BLK;
	spi_trans.cmd_count = 1;
	spi_trans.cmd_type = CMD_ONLY;
	spi_trans.flash_addr = offs;
	spi_trans.addr_count = 3;
