...
#if 0
static void nand_uboot0_readid(void)
{
	int tmp;
	char buf[8];
	...
	writel(0x1, NF2FFCE);
	trx_afifo_ncmd(0x0, 0x0, 0x90);
	trx_afifo_ncmd(0x0, 0x1, 0x00);
	trx_afifo_nop (0, 8);
	trx_afifo_ncmd(0xf, 0x3, 0x00);
	writel(0x0, NF2FFCE);
	...
	*(unsigned int *)buf = readl(NF2STSR0);
	*(unsigned int *)(buf + 4) = readl(NF2STSR1);
	spl_printf("ID: %02x %02x %02x %02x %02x %02x %02x %02x\n", buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], buf[6], buf[7]);
}
#endif
...
static int imap_read_page(int dev_id, int dma_unit_offset, int page, unsigned char *buf, int len)
{
	...
	if (dev_id == DEV_UBOOT0) {
		if (IROM_IDENTITY == IX_CPUID_X15_NEW || IROM_IDENTITY == IX_CPUID_Q3 || IROM_IDENTITY == IX_CPUID_X9){
...
int nand_uboot0_read(unsigned char *buf, int dev_id, int offset, int len)
{
	...
	if (dev_id == DEV_UBOOT0)
		page_shift = ((IROM_IDENTITY == IX_CPUID_X15 || IROM_IDENTITY == IX_CPUID_X15_NEW || IROM_IDENTITY == IX_CPUID_Q3 || IROM_IDENTITY == IX_CPUID_X9) ? 10 : 11);
	else
...
int imap_get_para_from_page(unsigned char *buf, int len)
{
	...
	if (sizeof(struct infotm_nand_para_save) > IMAPX800_PARA_SAVE_SIZE)
		return -1;
	read_page_once = (IROM_IDENTITY == IX_CPUID_X15 || IROM_IDENTITY == IX_CPUID_X15_NEW || IROM_IDENTITY == IX_CPUID_Q3 || IROM_IDENTITY == IX_CPUID_X9) ? (IMAPX800_PARA_SAVE_SIZE >> 10) : (IMAPX800_PARA_SAVE_SIZE >> 11) ;
...
void param_transmit_to_uboot1(void)
{
	struct infotm_nand_para_save *infotm_nand_save = (struct infotm_nand_para_save *)nand_param;
	if (infotm_nand_save->head_magic == IMAPX800_UBOOT_PARA_MAGIC) {
		if((bdevice_id == DEV_MMC(1)) && item_exist("nand.erase") && (item_integer("nand.erase", 0) == 1)) {
...
int nand_uboot0_init(void)
{
	...
	uboot0_unit = (IROM_IDENTITY == IX_CPUID_X15 || IROM_IDENTITY == IX_CPUID_X15_NEW || IROM_IDENTITY == IX_CPUID_Q3 || IROM_IDENTITY == IX_CPUID_X9) ? 1024 : 2048;
