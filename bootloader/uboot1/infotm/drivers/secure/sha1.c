 ...
 */
int sha1_init(uint32_t len)
{	
	msg_len = (uint64_t)len;
        msg_len <<= 3;

	if(hash_module_init(SHA1, (uint32_t)(msg_len >> 32), (uint32_t)msg_len))
	    return -1;
        msg_len >>= 3;
 ...
 *         -2  other error
 */
int sha1_data(uint8_t *buf, uint32_t len)
{
	/* exceed the len */
