...
{
	return do_fat_read(dir, NULL, 0, LS_YES, DETECT_NO);
}

int 
file_fat_detect(const char *filename)
{
	return (do_fat_read(filename, NULL, 0, LS_NO,DETECT_YES)-FILE_EXIST);
}
long
file_fat_read(const char *filename, void *buffer, unsigned long maxsize)
{
	printf("reading %s\n",filename);
	return do_fat_read(filename, buffer, maxsize, LS_NO,DETECT_NO);
}
